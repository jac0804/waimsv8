...
class imageuploader
{
  ...
  private function imageuploadmodule(Request $request)
  {
    ...
    //upload_max_filesize
    //post_max_size
    $required = ['ext', 'filename', 'file', 'field', 'action', 'lookupclass', 'tableid', 'doc', 'center', 'user', 'companyid', 'folder', 'table', 'fieldid', 'title', 'tmline'];
    $creds = $request->only($required);
  ...
  private function imageuploadtblentry(Request $request)
  {
    ...
    if ($hasfile) {
      ...
      switch ($file_ext) {
        ...
        case 'png':
          //$fileupload = Storage::disk('sbcpath')->put('/modules/' . $filename, $img);
          $fileupload = Storage::disk('public')->put($mainfolder . $filename, $img);
          break;
        default:
          //$img->move($path,$hashfilename.'_'.$creds['tableid'].'_'.$line.'.'.$file_ext );  

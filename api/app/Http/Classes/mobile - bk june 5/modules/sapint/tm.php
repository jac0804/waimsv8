  ...
  public function getFunc() {
    return '({
      ...
      loadTableData: function () {
        console.log("tm loadTableData called");
        cfunc.showLoading();
        ...
        sbc.db.transaction(function (tx) {
          tx.executeSql("select ? as wht union all select distinct wh from head where wh is not null and wh<>? and doc=?", ["", "", sbc.doc], function (tx, res2) {
            ...
            console.log("tm err3: ", err.message);
          });
          let whfilter = "";
          let data = ["-", sbc.doc];
            ...
            if (res.rows.length > 0) {
              for (var x = 0; x < res.rows.length; x++) {
                ...
                if (parseInt(x) + 1 === res.rows.length) {
                  console.log("tmss: ", sbc.modulefunc.tableData);
                }
              }
            }
            $q.loading.hide();
          }, function (tx, err) {
            console.log("tm err: ", err.message);
          });
        }, function (err) {
          console.log("tm err2: ", err.message);
        });
      },

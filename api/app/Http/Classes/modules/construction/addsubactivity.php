...
class addsubactivity
{
  ...
  private $table = 'subactivity';
  private $othersClass;
  public $style = 'width:100%;';
  private $fields = ['subactivity', 'substage', 'stage', 'description'];
  ...
  {
    $tab = [$this->gridname => ['gridcolumns' => ['action', 'subactivity', 'description']]];

    $stockbuttons = ['save', 'delete', 'addsubitems'];

    $obj = $this->tabClass->createtab($tab, $stockbuttons);
    $obj[0][$this->gridname]['columns'][2]['label'] = 'Task';
    $obj[0][$this->gridname]['columns'][0]['btns']['addsubitems']['checkfield'] = 'newtrans';

    $obj[0][$this->gridname]['columns'][0]['style'] = 'width: 70px;whiteSpace: normal;min-width:10px;max-width:70px;';
    $obj[0][$this->gridname]['columns'][1]['style'] = 'width: 200px;whiteSpace: normal;min-width:10px;max-width:200px;';
    $obj[0][$this->gridname]['columns'][2]['style'] = 'width: 350px;whiteSpace: normal;min-width:10px;max-width:350px;';
    return $obj;
  }
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['line'] == 0) {
          ...
          if ($line != 0) {
            $this->logger->sbcmasterlog($data[$key]['substage'], $config, ' CREATE - ' . $data[$key]['subactivity'], 0, 1);
  ...
  public function save($config)
  {
    ...
    if ($row['line'] == 0) {
      ...
      if ($line != 0) {
        ...
        $this->logger->sbcmasterlog($row['substage'], $config, ' CREATE - ' . $row['subactivity'], 0, 1);
  ...
  {
    $row = $config['params']['row'];
    $check = $this->coreFunctions->getfieldvalue("subitems", "subactivity", "subactivity=?", [$row['line']]);
    if (strlen($check) > 0) {
      return ['status' => false, 'msg' => 'DELETE failed,already have subitems...'];
    } else {
      $qry = "delete from " . $this->table . " where line=?";
      $this->coreFunctions->execqry($qry, 'delete', [$row['line']]);
      $this->logger->sbcdelmaster_log($row['substage'], $config, 'REMOVE - ' . $row['subactivity'], 1);
  ...
  public function loaddata($config)
  {
    ...
    $select = $this->selectqry();
    $select = $select . ",'' as bgcolor ";
    $qry = "select " . $select . " from " . $this->table . " where substage =? order by line";

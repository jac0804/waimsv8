...
class mi
{
  ...
  public function createdoclisting($config)
  {
    switch ($config['params']['companyid']) {
      case 3: //conti
      case 15: //nathina
        $this->modulename = 'MATERIAL ISSUANCE';
        break;
    }
  ...
  public function createTab($access, $config)
  {
    ...
    if ($companyid == 43) { //mighty
      $tab['customform'] = ['event' => ['action' => 'customform', 'lookupclass' => 'tripdetails', 'access' => 'release'], 'label' => 'TRIP'];
    }
  ...
  public function createHeadField($config)
  {
    ...
    data_set($col1, 'client.label', 'Customer/Supplier');
    ...
    if ($isproject) {
      ...
      if ($companyid == 43) { // mighty
        array_push($fields, 'empname', 'odometer');
      }
      $col2 = $this->fieldClass->create($fields);
    ...
    switch ($companyid) {
      case 3: //conti
      case 15: //nathina
        data_set($col2, 'dacnoname.label', 'Expense Account');
        data_set($col2, 'dacnoname.lookupclass', 'EX');
    ...
    $fields = [];
    if ($companyid == 43) { // mighty ISTRIP-CHECKBOX
      $fields = ['istrip'];
    }
  ...
  public function updatehead($config, $isupdate)
  {
    ...
    $dataother = []; //MIGHTY-ODOMETER and itemid
    ...
    if ($config['params']['companyid'] == 43) { //mighty -odometer and itemid
    ...
    if ($this->companysetup->isinvonly($config['params'])) {
      ...
      if ($companyid == 43) { // mighty
        ...
        if ($istrip == 1) {
          ...
          if (empty($tripdata)) {
            return ['status' => false, 'msg' => 'Posting failed. Please setup trip details.'];
          }
        }
      }
      ...
      if ($companyid == 8) { //maxipro
        $checkitem = $this->coreFunctions->datareader("select line as value from lastock where amt=0 and isamt <> 0 and trno =?", [$trno]);
        if (!empty($checkitem)) {
          return ['trno' => $trno, 'status' => false, 'msg' => 'Unable to post. Please, reencode the item.'];
        }
      }
      ...
      if ($this->coreFunctions->sbcinsert($this->stock, $data) == 1) {
        ...
        if ($isnoninv == 0) {
          if ($config['params']['companyid'] == 8) { //maxipro
            $cost = $this->othersClass->computecostingmi($data['itemid'], $data['whid'], $loc, $data['expiry'], $trno, $line, $refx, $linex, $data['iss'], $config['params']['doc']);
          } else {
  ...
  public function createdistribution($config)
  {
    ...
    if ($companyid == 8) { //maxipro
      ...
      $var = $amt - $cost;
      if (abs($variance) != 0 && $var != 0) {
        $acnoid = $this->coreFunctions->getfieldvalue('coa', 'acnoid', 'alias=?', ['SA5']);
        $entry = ['acnoid' => $acnoid, 'client' => $params['client'], 'db' => 0, 'cr' => ($variance), 'postdate' => $stock[0]->dateid, 'cur' => $stock[0]->cur, 'forex' => $stock[0]->forex, 'fcr' => floatval($stock[0]->forex) == 1 ? 0 : $variance, 'fdb' => 0, 'projectid' =>  $stock[0]->projectid, 'subproject' => $stock[0]->subproject, 'stageid' =>  $stock[0]->stageid];
        ...
        $entry['line'] = $line + 1;
        $entry['trno'] = $trno;
        if ($this->coreFunctions->sbcinsert($this->detail, $entry)) {
          $status = true;
        } else {
          $this->logger->sbcwritelog($trno, $config, 'DETAILS', 'AUTOMATIC ACCOUNTING DISTRIBUTION FAILED');
          $status = false;
    ...
    switch ($companyid) {
      case 8: //maxipro
        ...
        if ($ar != 0 && $ar != $params['ext']) {
          ...
          $entry1 = ['acnoid' => $acnoid, 'client' => $params['client'], 'db' => ($ar * $forex), 'cr' => 0, 'postdate' => $params['date'], 'cur' => $cur, 'forex' => $forex, 'fdb' => floatval($forex) == 1 ? 0 : $ar, 'fcr' => 0, 'projectid' => $params['projectid'], 'subproject' => $params['subproject'], 'stageid' => $params['stage']];
          $this->acctg = $this->othersClass->upsertdetail($this->acctg, $entry1, $config);

          $acnoid = $this->coreFunctions->getfieldvalue('coa', 'acnoid', 'acno=?', [$params['inventory']]);
          $entry2 = ['acnoid' => $acnoid, 'client' => $params['wh'], 'db' => 0, 'cr' => ($params['ext'] * $forex), 'postdate' => $params['date'], 'cur' => $cur, 'forex' => $forex, 'fcr' => floatval($forex) == 1 ? 0 : $params['fcost'], 'fdb' => 0, 'projectid' => $params['projectid'], 'subproject' => $params['subproject'], 'stageid' => $params['stage']];
          $this->acctg = $this->othersClass->upsertdetail($this->acctg, $entry2, $config);
          ...
          //   $acnoid = $this->coreFunctions->getfieldvalue('coa', 'acnoid', 'alias=?', ['SA5']);
          //   $entry = ['acnoid' => $acnoid, 'client' => $params['client'], 'db' => 0, 'cr' => ($sales), 'postdate' => $params['date'], 'cur' => $cur, 'forex' => $forex, 'fcr' => floatval($forex) == 1 ? 0 : $ar, 'fdb' => 0, 'projectid' => $params['projectid'], 'subproject' => $params['subproject'], 'stageid' => $params['stage']];
          //   $this->acctg = $this->othersClass->upsertdetail($this->acctg, $entry, $config);
          // }
        } else {
      ...
      case 43: // mighty
        if (floatval($params['ext']) != 0) {
          $acnoid = $this->coreFunctions->getfieldvalue('coa', 'acnoid', 'acno=?', [$params['acno']]);
          $entry = ['acnoid' => $acnoid, 'client' => $params['client'], 'db' => (floatval($params['ext']) * $forex), 'cr' => 0, 'postdate' => $params['date'], 'cur' => $cur, 'forex' => $forex, 'fdb' => floatval($forex) == 1 ? 0 : $params['ext'], 'fcr' => 0, 'projectid' => $params['projectid'], 'subproject' => $params['subproject'], 'stageid' => $params['stage']];
          $this->acctg = $this->othersClass->upsertdetail($this->acctg, $entry, $config);
          ...
          if (floatval($params['ext']) > floatval($params['cost'])) {
            $otherincome = floatval($params['ext']) - floatval($params['cost']);
            $acnoid = $this->coreFunctions->getfieldvalue('coa', 'acnoid', 'alias=?', ['SAOI']);
            $entry = ['acnoid' => $acnoid, 'client' => $params['wh'], 'db' => 0, 'cr' => $otherincome, 'postdate' => $params['date'], 'cur' => $cur, 'forex' => $forex, 'fcr' => floatval($forex) == 1 ? 0 : $params['fcost'], 'fdb' => 0, 'projectid' => $params['projectid'], 'subproject' => $params['subproject'], 'stageid' => $params['stage']];
            $this->acctg = $this->othersClass->upsertdetail($this->acctg, $entry, $config);
          }
          $this->coreFunctions->logConsole(json_encode(floatval($params['ext']) . ' - ' . floatval($params['cost'])));
        }
        break;
      default:
        if (floatval($params['ext']) != 0) {
  ...
  public function getboqdetails($config)
  {
    ...
    if ($config['params']['companyid'] == 8) { //maxipro
      $doc = "head.doc in ('RR','MT')";
      $amt = ", (case when head.doc ='RR' then (stock.cost/uom.factor) else (stock.amt/uom.factor) end ) as rrcost";
    } else {
      $doc = "head.doc = 'RR'";

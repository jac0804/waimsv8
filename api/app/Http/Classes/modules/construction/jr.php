...
class jr
{
  ...
  public function createdoclisting($config)
  {
    ...
    $getcols = ['action', 'liststatus', 'listdocument', 'listdate', 'listprojectname', 'rem', 'listpostedby', 'listcreateby', 'listeditby', 'listviewby'];
    $stockbuttons = ['view'];
  ...
  public function loaddoclisting($config)
  {
    ...
    $viewall = $this->othersClass->checkAccess($config['params']['user'], 2445);
    ...
    if (isset($config['params']['search'])) {
      ...
      if ($config['params']['companyid'] == 8) array_push($searchfield, 'head.rem');
    ...
    $qry = "select head.trno,head.docno,p.name as projectname,head.clientname,left(head.dateid,10) as dateid, 'DRAFT' as status,head.rem,head.createby,head.editby,head.viewby,num.postedby  
     ...
     union all
     select head.trno,head.docno,p.name as projectname,head.clientname,left(head.dateid,10) as dateid,'POSTED' as status,head.rem,head.createby,head.editby,head.viewby, num.postedby  
  ...
  public function createHeadbutton($config)
  {
    ...
    if ($this->companysetup->getisshowmanual($config['params'])) {
      $buttons['others']['items']['manual'] = ['label' => 'View Manual', 'todo' => ['lookupclass' => 'jr', 'title' => 'Job Request Manual', 'action' => 'viewpdf',  'access' => 'view', 'type' => 'viewmanual']];
    }
    return $buttons;
  } // createHeadbutton
  ...
  public function createTab($access, $config)
  {
    ...
    $obj[0]['inventory']['columns'][2]['label'] = 'Approve Qty';
    $obj[0]['inventory']['columns'][2]['readonly'] = true;
    if ($config['params']['companyid'] != 8) { //not maxipro
      $obj[0]['inventory']['columns'][7]['readonly'] = true;
    }
  ...
  {
    $tbuttons = ['pendingboq', 'saveitem', 'deleteallitem'];
  ...
  public function createHeadField($config)
  {
    ...
    if ($companyid == 8) { //maxipro
      $fields = ['docno', 'subcontractor', 'dprojectname', 'subprojectname'];
    } else {
    ...
    if ($companyid == 8) { //maxipro
      data_set($col1, 'subcontractor.required', true);
    }
    ...
    if ($viewall == '0') {
      ...
      data_set($col1, 'dprojectname.type', 'lookup');
      data_set($col1, 'dprojectname.lookupclass', 'projectcode');
      data_set($col1, 'subprojectname.type', 'input');
      data_set($col1, 'subprojectname.readonly', true);
    }
  ...
  public function createnewtransaction($docno, $params)
  {
    ...
    $data[0]['projectcode'] = '';
    $data[0]['projectname'] = '';
    $data[0]['subcontractorid'] = '0';
    $data[0]['subcontractor'] = '';
    $data[0]['revision'] = '';

    $viewall = $this->othersClass->checkAccess($params['user'], 2445);
    if ($viewall == '0') {
  ...
  public function loadheaddata($config)
  {
    ...
    $qryselect = "select 
         ...
         subcont.clientname as subcontractor, head.subcontractorid, head.revision ";
    ...
    $qry = $qryselect . " from $table as head
        ...
        left join client as subcont on head.subcontractorid = subcont.clientid
  ...
  public function posttrans($config)
  {
    ...
    $qry = "insert into " . $this->hhead . "(trno,doc,docno,client,clientname,address,shipto,dateid,
      terms,rem,forex,yourref,ourref,createdate,createby,editby,editdate,lockdate,lockuser,agent,wh,due,cur,projectid,subproject,
      subcontractorid, revision)
      ...
      head.due,head.cur,head.projectid,head.subproject,
      head.subcontractorid, head.revision
    ...
    if ($posthead) {
      ...
      $qry = "insert into " . $this->hstock . "(trno,line,itemid,uom,
        ...
        encodeddate,qa,encodedby,editdate,editby,refx,linex,cdqa,rem,rqty,stageid,status)
        SELECT trno, line, itemid, uom,whid,loc,ref,disc,cost, qty,void,rrcost, rrqty, ext,
        encodeddate,qa, encodedby,editdate,editby,refx,linex,cdqa,rem,rqty,stageid,status FROM " . $this->stock . " where trno =?";
  ...
  public function unposttrans($config)
  {
    ...
    yourref,ourref,createdate,createby,editby,editdate,lockdate,lockuser,wh,due,cur,projectid,subproject,
    subcontractorid, revision)
    ...
    head.createby, head.editby, head.editdate, head.lockdate, head.lockuser,head.wh,head.due,head.cur,head.projectid,head.subproject,
    head.subcontractorid, head.revision
    ...
    if ($this->coreFunctions->execqry($qry, 'insert', [$trno])) {
      ...
      cost,qty,void,rrcost,rrqty,ext,rem,encodeddate,qa,encodedby,editdate,editby,refx,linex,cdqa,rqty,stageid,status)
      select trno, line, itemid, uom,whid,loc,ref,disc,cost, qty,void, rrcost, rrqty,
      ext,rem, encodeddate, qa, encodedby, editdate, editby,refx,linex,cdqa,rqty,stageid,status
  ...
  stock.line,
    ...
    item.brand,
    stock.rem, 
    ifnull(uom.factor,1) as uomfactor,stock.rqty,
    '' as bgcolor,stock.stageid,st.stage,
  ...
  public function openstock($trno, $config)
  {
    ...
    left join client as warehouse on warehouse.clientid=stock.whid left join stagesmasterfile as st on st.line = stock.stageid where stock.trno =? ";
  ...
  public function deleteitem($config)
  {
    ...
    $this->updateprojmngmt($config, $config['params']['row']['stageid']);
    if ($data[0]->refx !== 0) {
  ...
  public function getboqsummary($config)
  {
    ...
    foreach ($config['params']['rows'] as $key => $value) {
      $qry = "select head.docno, item.itemid,stock.trno,
        ...
        stock.disc,head.stageid
        FROM hsohead as head left join hsostock as stock on stock.trno=head.trno left join transnum on transnum.trno=head.trno left join item on item.itemid=
        stock.itemid left join uom on uom.itemid=item.itemid and 
        uom.uom=stock.uom where item.islabor= 1 and stock.trno = ? and transnum.center=? and stock.iss>stock.qa and stock.void=0";
  ...
  public function getboqdetails($config)
  {
    ...
    foreach ($config['params']['rows'] as $key => $value) {
      $qry = "
        ...
        stock.itemid left join uom on uom.itemid=item.itemid and 
        uom.uom=stock.uom where item.islabor= 1 and stock.trno = ? and stock.line=? and transnum.center=? and stock.iss>stock.qa and stock.void=0
    ";
        ...
        foreach ($data as $key2 => $value) {
          ...
          if ($return['status']) {
            if ($this->setserveditems($data[$key2]->trno, $data[$key2]->line) == 0) {
              ...
              $config['params']['line'] = $line;
              $this->coreFunctions->sbcupdate($this->stock, $data2, ['trno' => $trno, 'line' => $line]);
              $this->setserveditems($data[$key2]->trno, $data[$key2]->line, $trno, $line);
              $row = $this->openstockline($config);
              $return = ['row' => $row, 'status' => true, 'msg' => 'Item was successfully added.'];
            }
            array_push($rows, $return['row'][0]);
  ...
  {
    $qry1 = "select stock.rqty from prhead as head left join prstock as 
    stock on stock.trno=head.trno where head.doc='JR' and stock.refx=" . $refx . " and stock.linex=" . $linex;

    $qry1 = $qry1 . " union all select hprstock.rqty from hprhead left join hprstock on hprstock.trno=
    hprhead.trno where hprhead.doc='JR' and hprstock.refx=" . $refx . " and hprstock.linex=" . $linex;

    $qry2 = "select ifnull(sum(rqty),0) as value from (" . $qry1 . ") as t";
  ...
  public function updateprojmngmt($config, $stage)
  {
    $trno = $config['params']['trno'];
    ...
    stock on stock.trno=head.trno where head.doc='JR' and head.projectid = " . $proj . " and head.subproject = " . $sub . " and stock.stageid=" . $stage;

    $qry1 = $qry1 . " union all select stock." . $this->dqty . " from " . $this->hhead . " as head left join " . $this->hstock . " as stock on stock.trno=
      head.trno where head.doc='JR' and head.projectid = " . $proj . " and head.subproject = " . $sub . " and stock.stageid=" . $stage;
    ...
    $updatestage = $this->coreFunctions->execqry("update stages set jr=" . $qty . ", editdate = '" . $editdate . "', editby = '" . $editby . "'  where projectid = " . $proj . " and subproject=" . $sub . " and stage=" . $stage, 'update');

...
class wc
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'WORK ACCOMPLISMENT';
  ...
  public $head = 'wchead';
  public $hhead = 'hwchead';
  public $stock = 'wcstock';
  public $hstock = 'hwcstock';
  ...
  public function createTab($access, $config)
  {
    ...
    if ($this->companysetup->getisproject($config['params'])) {
      $obj[0]['inventory']['columns'][10]['readonly'] = true;
    }
  ...
  {
    $tbuttons = ['pendingjc', 'saveitem', 'deleteallitem'];
  ...
  public function createHeadField($config)
  {
    ...
    $fields = ['dateid', ['yourref', 'ourref'], 'dprojectname', 'subprojectname'];
  ...
  public function loadheaddata($config)
  {
    ...
    $qryselect = "select 
         ...
         head.rem, head.stageid,head.projectid,p.code as projectcode,p.name as projectname,s.line as subproject,s.subproject as subprojectname,stage.stage ";
  ...
  public function posttrans($config)
  {
    ...
    $qry = "insert into " . $this->hhead . "(trno,doc,docno,client,clientname,address,dateid,
      rem,yourref,ourref,createdate,createby,editby,editdate,lockdate,lockuser,projectid,subproject,stageid)
      ...
      head.projectid,head.subproject,head.stageid FROM " . $this->head . " as head left join " . $this->tablenum . " as cntnum on cntnum.trno=head.trno
      where head.trno=? limit 1";
    $posthead = $this->coreFunctions->execqry($qry, 'insert', [$trno]);
    if ($posthead) {
      // for glstock
      $qry = "insert into " . $this->hstock . "(trno,line,itemid,uom,
        ref,disc,cost,qty,void,rrcost,rrqty,ext,
        encodeddate,qa,encodedby,editdate,editby,refx,linex,rem,stageid)
        SELECT trno, line, itemid, uom,ref,disc,cost, qty,void,rrcost, rrqty, ext,
        encodeddate,qa, encodedby,editdate,editby,refx,linex,rem,stageid FROM " . $this->stock . " where trno =?";
  ...
  yourref,ourref,createdate,createby,editby,editdate,lockdate,lockuser,projectid,subproject,stageid)
  ...
  head.createby, head.editby, head.editdate, head.lockdate, head.lockuser,head.projectid,head.subproject,head.stageid
  ...
  where head.trno=? limit 1";
    ...
    if ($this->coreFunctions->execqry($qry, 'insert', [$trno])) {
      ...
      trno,line,itemid,uom,ref,disc,
      cost,qty,void,rrcost,rrqty,ext,rem,encodeddate,qa,encodedby,editdate,editby,refx,linex,stageid)
      select trno, line, itemid, uom,ref,disc,cost, qty,void, rrcost, rrqty,
      ext,rem, encodeddate, qa, encodedby, editdate, editby,refx,linex,stageid
  ...
  public function openstockline($config)
  {
   ...
   FROM $this->stock as stock
  left join item on item.itemid=stock.itemid 
  left join uom on uom.itemid=item.itemid and uom.uom=stock.uom  left join stagesmasterfile as st on st.line = stock.stageid  where stock.trno = ? and stock.line = ? ";
  ...
  public function stockstatus($config)
  {
    switch ($config['params']['action']) {
      ...
      case 'deleteallitem':
        ...
        break;
      case 'getjcsummary':
        return $this->getjcsummary($config);
        break;
      case 'getjcdetails':
        return $this->getjcdetails($config);
        break;
      default:
  ...
  public function deleteitem($config)
  {
    ...
    $this->logger->sbcwritelog($trno, $config, 'STOCK', 'REMOVED - Line:' . $line . ' barcode:' . $data[0]['barcode'] . ' Qty:' . $data[0]['rrqty'] . ' Amt:' . $data[0]['rrcost'] . ' Disc:' . $data[0]['disc'] . ' ext:' . $data[0]['ext']);
    return ['status' => true, 'msg' => 'Item was successfully deleted.'];
  } // end function

  public function setserveditems($refx, $linex)
  {
    $qry1 = "select stock." . $this->hqty . " from wchead as head left join wcstock as 
    stock on stock.trno=head.trno where head.doc='WC' and stock.refx=" . $refx . " and stock.linex=" . $linex;

    $qry1 = $qry1 . " union all select hwcstock." . $this->hqty . " from hwchead left join hwcstock on hwcstock.trno=
    hwchead.trno where hwchead.doc='WC' and hwcstock.refx=" . $refx . " and hwcstock.linex=" . $linex;
    ...
    return $this->coreFunctions->execqry("update hjcstock set qa=" . $qty . " where trno=" . $refx . " and line=" . $linex, 'update');
  }
  ...
  private function updateprojmngmt($config, $stage)
  {
    ...
    stock on stock.trno=head.trno where head.doc='WC' and head.projectid = " . $proj . " and head.subproject = " . $sub . " and stock.stageid=" . $stage;

    $qry1 = $qry1 . " union all select stock.ext from " . $this->hhead . " as head left join " . $this->hstock . " as stock on stock.trno=
      head.trno where head.doc='WC' and head.projectid = " . $proj . " and head.subproject = " . $sub . " and stock.stageid=" . $stage;
    ...
    $this->coreFunctions->execqry("update stages set wac=" . $qty . " where projectid = " . $proj . " and subproject=" . $sub . " and stage=" . $stage, 'update');
  ...
  public function getjcsummary($config)
  {
    $trno = $config['params']['trno'];
    $rows = [];
    foreach ($config['params']['rows'] as $key => $value) {
      $qry = "
        ...
        FROM hjchead as head left join hjcstock as stock on stock.trno=head.trno left join item on item.itemid=
  ...
  public function getjcdetails($config)
  {
    $trno = $config['params']['trno'];
    $rows = [];
    foreach ($config['params']['rows'] as $key => $value) {

...
class rq
{
  ...
  public $damt = 'rrcost';
  public $hamt = 'cost';
  private $fields = ['trno', 'docno', 'dateid', 'due',  'yourref', 'ourref', 'rem', 'projectid', 'subproject', 'revision'];
  ...
  public function loaddoclisting($config)
  {
    ...
    $qry = "select head.trno,head.docno,p.name as projectname,head.clientname,left(head.dateid,10) as dateid, 'DRAFT' as status,head.createby,head.editby,head.viewby,num.postedby  
     ...
     union all
     select head.trno,head.docno,p.name as projectname,head.clientname,left(head.dateid,10) as dateid,'POSTED' as status,head.createby,head.editby,head.viewby, num.postedby  
  ...
  public function createHeadbutton($config)
  {
    ...
    if ($this->companysetup->getisshowmanual($config['params'])) {
      $buttons['others']['items']['manual'] = ['label' => 'View Manual', 'todo' => ['lookupclass' => 'rq', 'title' => 'Purchase Requisition Manual', 'action' => 'viewpdf',  'access' => 'view', 'type' => 'viewmanual']];
    }
    return $buttons;
  } // createHeadbutton
  ...
  {
    $rq_btnvoid_access = $this->othersClass->checkAccess($config['params']['user'], 3602);
    $action = 0;
    $rqty = 1;
    $rrqty = 2;
    $uom = 3;
    ...
    $obj[0]['inventory']['columns'][$rrqty]['label'] = 'Approve Qty';
  ...
  public function createHeadField($config)
  {
    ...
    data_set($col1, 'dprojectname.required', false);
  ...
  public function createnewtransaction($docno, $params)
  {
    ...
    $viewall = $this->othersClass->checkAccess($params['user'], 2272);
    if ($viewall == '0') {
  ...
  public function updateperitem($config)
  {
    ...
    foreach ($data2 as $key => $value) {
      if ($data2[$key][$this->rqty] == 0) {
        $data[$key]->errcolor = 'bg-red-2';
        $isupdate = false;
        $msg = 'Qty greater than BOQ';
      } else {
        if ($isupdate == 0) {
          $isupdate = false;
          $msg = 'Qty already approve!';
        }
      }
    }
  ...
  public function updateitem($config)
  {
    ...
    foreach ($data2 as $key => $value) {
      if ($data2[$key][$this->rqty] == 0) {
        $data[$key]->errcolor = 'bg-red-2';
        $isupdate = false;
        $msg1 = 'Qty greater than BOQ/Qty Already Approve';
      }
    }
    ...
    if ($action == 'insert') {
      ...
      if ($refx != 0) {
        if ($this->setserveditems($refx, $linex) === 0) {
          $data2 = [$this->dqty => 0, $this->hqty => 0, $this->rqty => 0, 'ext' => 0];
  ...
  public function getboqdetails($config)
  {
    ...
    foreach ($config['params']['rows'] as $key => $value) {
      $qry = "
        ...
        left join transnum on transnum.trno=head.trno 
        left join item on item.itemid=stock.itemid 
        left join uom on uom.itemid=item.itemid and 
        uom.uom=stock.uom where item.islabor<> 1 and stock.trno = ? and stock.line=? 
        and transnum.center=? and stock.iss>stock.qa and stock.void=0
    ";
  ...
  {
    $qry1 = "select stock.rqty*uom.factor from prhead as head left join prstock as 
    stock on stock.trno=head.trno left join uom on uom.itemid = stock.itemid and uom.uom = stock.uom where head.doc='RQ' and stock.refx=" . $refx . " and stock.linex=" . $linex;

    $qry1 = $qry1 . " union all select hprstock.rqty*uom.facotr from hprhead left join hprstock on hprstock.trno=
    hprhead.trno left join uom on uom.itemid = hprstock.itemid and uom.uom = hprstock.uom where hprhead.doc='RQ' and hprstock.refx=" . $refx . " and hprstock.linex=" . $linex;
  ...
  public function updateprojmngmt($config, $stage)
  {
    ...
    stock on stock.trno=head.trno where head.doc='RQ' and head.projectid = " . $proj . " and head.subproject = " . $sub . " and stock.stageid=" . $stage;

    $qry1 = $qry1 . " union all select stock." . $this->dqty . " from " . $this->hhead . " as head left join " . $this->hstock . " as stock on stock.trno=
      head.trno where head.doc='RQ' and head.projectid = " . $proj . " and head.subproject = " . $sub . " and stock.stageid=" . $stage;
    ...
    $updatestage = $this->coreFunctions->execqry("update stages set pr=" . $qty . ", editdate = '" . $editdate . "', editby = '" . $editby . "' where projectid = " . $proj . " and subproject=" . $sub . " and stage=" . $stage, 'update');

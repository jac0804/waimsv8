...
class mj
{
  ...
  public function createdoclisting($config)
  {
    ...
    $getcols = ['action', 'liststatus', 'listdocument', 'listdate', 'listclientname', 'modeofsales', 'total', 'rem', 'ar', 'listpostedby', 'listcreateby', 'listeditby', 'listviewby'];
  ...
  public function loaddoclisting($config)
  {
    ...
    if (isset($config['params']['search'])) {
      $searchfield = ['head.docno', 'head.clientname', 'mode.name', 'head.yourref', 'head.ourref', 'num.postedby', 'head.createby', 'head.editby', 'head.viewby', 'head.rem'];
    ...
    head.createby,head.editby,head.viewby,num.postedby,mode.name as modeofsales,
  ...
  {
    $financing_access = $this->othersClass->checkAccess($config['params']['user'], 4606);
    ...
    $financing = ['customform' => ['action' => 'customform', 'lookupclass' => 'viewsjfinancing']];
    $recons = ['customform' => ['action' => 'customform', 'lookupclass' => 'reconstruct']];
    // $viewcost = $this->othersClass->checkAccess($config['params']['user'], 368);
    $tab = [
      'tableentry' =>
      ['action' => 'documententry', 'lookupclass' => 'entrycntnumpicture', 'label' => 'Attachment', 'access' => 'view']
    ];
  ...
  public function createHeadField($config)
  {
    ...
    $fields = ['docno', 'client', 'clientname', 'address', 'modeofsales', 'dfpname'];
    // 
    $col1 = $this->fieldClass->create($fields);
    ...
    if ($inv) {
      $fields = [['cur', 'forex'], 'dagentname']; //yourref,ourref
    } else {
      $fields = ['downpayment', ['cur', 'forex'], 'dvattype', 'dagentname']; // yourref,ourref,crref,rfno
    } //chsino,swsno
    ...
    data_set($col3, 'dagentname.label', 'SR NAME');
  ...
  public function loadheaddata($config)
  {
    ...
    $qryselect = "
        ...
        head.ewt,head.ewtrate,'' as dewt,mode.line,mode.name as modeofsales,head.fpid,ifnull(f.client,'') as fp,ifnull(f.clientname,'') as fpname,'' as dfpname,
    ...
    $qry = $qryselect . " from $table as head
        ...
        left join client as f on f.clientid = head.fpid
        left join mode_masterfile as mode on mode.line = head.modeofsales and mode.ismc =1
  ...
  public function posttrans($config)
  {
    ...
    $qry = "select trno from " . $this->stock . " where trno=? and isamt=0";
    $isitemzeroamt = $this->coreFunctions->opentable($qry, [$trno]);
    if (!empty($isitemzeroamt)) {
    ...
    if ($modename == "INHOUSE INSTALLMENT") {
      $isdetailinfo = $this->coreFunctions->getfieldvalue('detailinfo', 'trno', 'trno = ?', [$trno]);
      if (empty($isdetailinfo)) {
        return ['status' => false, 'msg' => 'Posting failed. In-house installment transactions require a financing schedule; please verify.'];
      }
    }
  ...
  public function stockstatus($config)
  {
    switch ($config['params']['action']) {
      ...
      case 'getserialout':
        ...
        break;
      case 'getserialoutsingle':
        return $this->getserialoutsingle($config);
        break;
      default:
  ...
  public function createdistribution($config)
  {
    ...
    if ($modename == "INHOUSE INSTALLMENT") {
      return $this->financingdistribution($config);
    }
    ...
    $qry = 'select head.dateid,head.client,head.tax,head.contra,head.cur,head.forex,stock.ext,wh.client as wh,ifnull(item.asset,"") as asset,ifnull(item.revenue,"") as revenue,
      item.expense,stock.isamt,stock.disc,stock.isqty,stock.cost,stock.iss,stock.fcost,head.projectid,client.rev,stock.rebate,head.taxdef,head.deldate,head.ewt,head.ewtrate,head.fpid,i.downpayment
          ...
          left join cntnuminfo as i on i.trno = head.trno where head.trno=?';
    ...
    if (!empty($stock)) {
      ...
      if ($modename == "CASH") {
        $revacct = $this->coreFunctions->getfieldvalue('coa', 'acno', 'alias=?', ['SA7']);
      } else {
      ...
      foreach ($stock as $key => $value) {
        ...
        // if ($modename == "CASH") {
        //   $contra = $this->coreFunctions->getfieldvalue('coa', 'acno', 'alias=?', ['CA1']);
        // }
  ...
  public function financingdistribution($config)
  {
    $trno = $config['params']['trno'];
    $d = [];
    $det = [];
    ...
    $intacnoid = $this->coreFunctions->getfieldvalue("coa", "acnoid", "alias = 'SA3'"); //unearnedinterest
    $arintacnoid = $this->coreFunctions->getfieldvalue("coa", "acnoid", "alias = 'AR2'"); //ar interest
    $maacnoid = $this->coreFunctions->getfieldvalue("coa", "acnoid", "alias = 'AR1'"); //artrade
    $gpacnoid = $this->coreFunctions->getfieldvalue("coa", "acnoid", "alias = 'SA1'"); //sales
    $miscacnoid = $this->coreFunctions->getfieldvalue("coa", "acnoid", "alias = 'SA2'"); //misc
    $caacnoid = $this->coreFunctions->getfieldvalue("coa", "acnoid", "alias = 'ARDP'"); //dp
    $txacnoid = $this->coreFunctions->getfieldvalue("coa", "acnoid", "alias = 'TX2'"); //output
    $inacnoid = $this->coreFunctions->getfieldvalue("coa", "acnoid", "alias = 'IN1'"); //inventory
    $cgacnoid = $this->coreFunctions->getfieldvalue("coa", "acnoid", "alias = 'CG1'"); //cos
    $arrbacnoid = $this->coreFunctions->getfieldvalue("coa", "acnoid", "alias = 'AR5'"); //arrebate
    $aprbacnoid = $this->coreFunctions->getfieldvalue("coa", "acnoid", "alias = 'AP3'"); //aprebate
    ...
    ifnull(hinfo.termsmonth,0) as termsmonth,ifnull(hinfo.fma1,0) as ma,head.tax,
    (select sum(stock.ext) as ext from lastock as stock where stock.trno = head.trno) as amt,
    (select sum(stock.iss*stock.cost) as ext from lastock as stock where stock.trno = head.trno) as cost from lahead as head 
    ...
    if (!empty($data)) {
      ...
      $ma =  $data[0]->ma; //round($data[0]->factor*$financeamt,0,PHP_ROUND_HALF_UP);
      $prevbal = $financeamt;
      //monthly ma
      $balmons = $data[0]->days;
      ...
      $d['db'] = $data[0]->downpayment;
      $d['cr'] = 0;
      $d['rem'] = 'Advance Down Payment';
      ...
      for ($y = 1; $y <= $balmons; $y++) {
        ...
        // extension = php_intl . dll pag error (NumberFormatter) uncomment php.ini
        $locale = 'en_US';
        $nf = new \NumberFormatter($locale, \NumberFormatter::ORDINAL);
        $d['rem'] = $nf->format($y) . ' Rebate';
        $this->acctg = $this->othersClass->upsertdetail($this->acctg, $d, $config);
        $i += 1;
      ...
      //output entry
      $gtotal = ($total + $data[0]->downpayment) - $data[0]->fmiscfee; //($total + $data[0]->downpayment + $totalint);
      $gptotal = ($total + $data[0]->downpayment) - $data[0]->fmiscfee;
      $this->coreFunctions->LogConsole('sales:' . $gtotal);
      ...
      if ($data[0]->tax != 0) {
        ...
        $gptotal = $gtotal;
        // $tax = ($gtotal / 1.12) * .12;
        // $gptotal = $gptotal / 1.12;
        // $totalint = $totalint / 1.12;
        // $misc = $data[0]->fmiscfee / 1.12;
      }
      ...
      if ($data[0]->cost != 0) {
        ...
        $d['line'] = $i;
        $d['acnoid'] = $cgacnoid;
        $d['client'] = $this->coreFunctions->getfieldvalue($this->head, "wh", "trno=?", [$trno]);
        $d['postdate'] = $data[0]->dateid;
        $d['db'] = $data[0]->cost;
        $d['cr'] = 0;
        $d['rem'] = '';
    ...
    $diff = $tdb - $tcr;
    $this->coreFunctions->LogConsole('difference:' . $diff);

    if ($diff != 0) {
      $this->coreFunctions->LogConsole('pumasok:' . $diff . 'tax' . $tax);
      $acnoid = $this->coreFunctions->getfieldvalue('coa', 'acnoid', 'alias=?', ['TX2']);
      $d['trno'] = $trno;
      $d['acnoid'] = $acnoid;
      $d['client'] = $this->coreFunctions->getfieldvalue($this->head, "client", "trno=?", [$trno]);
    ...
    if ($this->coreFunctions->sbcinsert($this->detail, $this->acctg)) {
      $this->logger->sbcwritelog($trno, $config, 'DETAILS', 'AUTOMATIC ACCOUNTING DISTRIBUTION SUCCESS');
      $status = true;
    } else {
      $status = false;
  ...
  public function distribution($params, $config)
  {
    ...
    if ($params['fpid'] != 0) {
      $client = $this->coreFunctions->getfieldvalue('client', 'client', 'clientid=?', [$params['fpid']]);
    }

    if (floatval($params['dp']) != 0) {
      $acnoid = $this->coreFunctions->getfieldvalue('coa', 'acnoid', 'alias=?', ['ARDP']);
      $entry = ['acnoid' => $acnoid, 'client' => $client, 'db' => ($dp * $forex), 'cr' => 0, 'postdate' => $params['date'], 'cur' => $cur, 'forex' => $forex, 'fdb' => floatval($forex) == 1 ? 0 : $dp, 'fcr' => 0, 'projectid' => $params['projectid']];
    ...
    if (floatval($params['ar']) != 0) {
      $acnoid = $this->coreFunctions->getfieldvalue('coa', 'acnoid', 'acno=?', [$params['acno']]);
      $entry = ['acnoid' => $acnoid, 'client' => $client, 'db' => (($params['ar'] - $ewtamt - $dp) * $forex), 'cr' => 0, 'postdate' => $params['date'], 'cur' => $cur, 'forex' => $forex, 'fdb' => floatval($forex) == 1 ? 0 : ($params['ar'] - $ewtamt - $dp), 'fcr' => 0, 'projectid' => $params['projectid']];
    ...
    if (floatval($params['discamt']) != 0) {
      ...
      $entry = ['acnoid' => $input, 'client' => $client, 'db' => ($params['discamt'] * $forex), 'cr' => 0, 'postdate' => $params['date'], 'cur' => $cur, 'forex' => $forex, 'fcr' => 0, 'fdb' => floatval($forex) == 1 ? 0 : ($params['discamt']), 'projectid' => $params['projectid']];
    ...
    if (floatval($params['rebate']) != 0) {
      ...
      $entry = ['acnoid' => $acnoid, 'client' => $client, 'db' => 0, 'cr' => $params['rebate'] * $forex, 'postdate' => $params['date'], 'cur' => $cur, 'forex' => $forex, 'fcr' => floatval($forex) == 1 ? 0 : $params['rebate'], 'fdb' => 0, 'projectid' => $params['projectid']];
      $this->acctg = $this->othersClass->upsertdetail($this->acctg, $entry, $config);
    }
    ...
    if (floatval($params['tax']) != 0) {
      ...
      if (floatval($sales) != 0) {
        ...
        $entry = ['acnoid' => $acnoid, 'client' => $client, 'cr' => ($sales * $forex), 'db' => 0, 'postdate' => $params['date'], 'cur' => $cur, 'forex' => $forex, 'fcr' => floatval($forex) == 1 ? 0 : $sales, 'fdb' => 0, 'projectid' => $params['projectid']];
      ...
      $entry = ['acnoid' => $input, 'client' => $client, 'cr' => ($params['tax'] * $forex), 'db' => 0, 'postdate' => $params['date'], 'cur' => $cur, 'forex' => $forex, 'fcr' => floatval($forex) == 1 ? 0 : ($params['tax']), 'fdb' => 0, 'projectid' => $params['projectid']];
  ...
  public function getserialoutsingle($config)
  {
    $dinsert = [];
    $return = true;
    $msg = 'Engine/Chassis was successfully added.';
    $trno = $config['params']['row']['stocktrno'];
    $itemid = $config['params']['row']['itemid'];
    $doc = $config['params']['doc'];
    $whid = $config['params']['row']['whid'];
    $loc = $config['params']['row']['loc'];
    $line = $config['params']['row']['stockline'];

    $dinsert['trno'] = $trno;
    $dinsert['line'] = $config['params']['row']['stockline'];
    $dinsert['serial'] = $config['params']['row']['serial'];
    $sline = $config['params']['row']['sline'];

    $dinsert['chassis'] = $config['params']['row']['chassis'];
    $dinsert['color'] = $config['params']['row']['color'];
    $dinsert['pnp'] = $config['params']['row']['pnp'];
    $dinsert['csr'] = $config['params']['row']['csr'];

    $prevoutline = $this->coreFunctions->getfieldvalue("serialout", "sline", "trno=? and line=?", [$trno, $line], '', true);

    $tstock = $this->coreFunctions->opentable("select s.isamt,s.isqty,s.disc,s.uom,u.factor as uomfactor from " . $this->stock . " as s 
    left join uom as u on u.itemid = s.itemid and u.uom= s.uom where s.trno = ? and s.line =? ", [$trno, $line]);

    try {
      if ($prevoutline != 0) {
        $qry = "delete from serialout where trno=? and line =? ";
        $this->coreFunctions->execqry($qry, 'update', [$trno, $line]);

        $qry = "update serialin set outline=0 where outline=? ";
        $this->coreFunctions->execqry($qry, 'update', [$prevoutline]);
      }
      ...
      if ($outline != 0) {
        ...
        $qry = "update " . $this->stock . " set color=? where trno=? and line=?";
        $this->coreFunctions->execqry($qry, 'update', [$config['params']['row']['color'], $trno,  $line]);
        ...
        if ($cost != -1) {
          ...
          $ext = number_format($computedata['ext'], $this->companysetup->getdecimal('currency', $config['params']), '.', '');
          $this->coreFunctions->execqry("update " . $this->stock . " set ext = $ext where trno =? and line =?", "update", [$trno, $line]);

          //CHECK BELOW COST
          if ($this->companysetup->checkbelowcost($config['params'])) {
            $belowcost = $this->othersClass->checkbelowcost($trno, $line, $config);
            if ($belowcost == 1) {
              $msg = '(' . $config['params']['row']['barcode'] . ') Is this free of charge? Please check.';
            } elseif ($belowcost == 2) {
              $this->coreFunctions->sbcupdate($this->stock, [$this->dqty => 0, $this->hqty => 0, 'ext' => 0, 'editby' => 'BELOW COST', 'editdate' => $this->othersClass->getCurrentTimeStamp()], ['trno' => $trno, 'line' => $line]);
              $this->coreFunctions->execqry('delete from costing where trno=? and line=?', 'delete', [$trno, $line]);
              $this->logger->sbcwritelog($trno, $config, 'STOCK', 'BELOW COST - Line:' . $line . ' barcode:' . $config['params']['row']['barcode'], $this->tablelogs);
              $msg = "(" . $config['params']['row']['barcode']  . ") You can't issue this item/s because it's BELOW COST!!!";
            }
          }
        } else {
          $this->coreFunctions->sbcupdate($this->stock, [$this->dqty => 0, $this->hqty => 0, 'ext' => 0, 'editby' => 'OUT_STOCK', 'editdate' => $this->othersClass->getCurrentTimeStamp()], ['trno' => $trno, 'line' => $line]);
          ...
          $this->logger->sbcwritelog($trno, $config, 'STOCK', 'OUT OF STOCK - Line:' . $line . ' barcode:' . $config['params']['row']['barcode'], $this->tablelogs);
          $return = false;
          $msg = 'Out of Stock';
        }
      }
      ...
      $data = $this->openstock($trno, $config);
      //return ['status' => true, 'action' => 'loaddocument', 'trno' => $trno, 'access' => 'view','lookupclass'=>''];
      return ['reloadgriddata' => ['inventory' => $data], 'status' => $return, 'msg' => $msg];
    } catch (Exception $e) {
      $data = $this->openstock($trno, $config);

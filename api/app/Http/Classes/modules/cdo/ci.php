...
class ci
{
    ...
    public function createTab($access, $config)
    {
        ...
        $column = ['action', 'isqty', 'uom',  'isamt', 'disc', 'ext',  'cost', 'markup', 'rebate',  'wh', 'ref', 'loc', 'expiry', 'rem', 'itemname',  'noprint', 'barcode'];
        $sortcolumn = ['action',   'isqty', 'uom',  'isamt', 'disc', 'ext',  'cost', 'markup', 'rebate',  'wh',  'ref', 'loc', 'expiry', 'rem', 'itemname',  'noprint', 'barcode'];
    ...
    {
        $tbuttons = ['quickadd', 'saveitem', 'deleteallitem'];
    ...
    public function createHeadField($config)
    {
        ...
        $fields = ['docno', 'dateid', 'modeofpayment', 'client', 'clientname',  'address', 'trnxtype', ['crno', 'rfno'], ['chsino', 'yourref'], ['ourref', 'swsno'], 'rem2', 'dvattype', 'rem'];
        $col1 = $this->fieldClass->create($fields);
        data_set($col1, 'client.lookupclass', 'customer');
        data_set($col1, 'trnxtype.label', 'Mode of Transaction');
        data_set($col1, 'modeofpayment.label', 'Mode of Payment');
        data_set($col1, 'modeofpayment.lookupclass', 'modeofpayment');
        data_set($col1, 'modeofpayment.action', 'lookuprandom');
        data_set($col1, 'client.required', false);
        data_set($col1, 'docno.label', 'Transaction#');
        data_set($col1, 'yourref.label', 'DR#');
        data_set($col1, 'ourref.label', 'SI#');

        data_set($col1, 'rem2.label', 'Mechanic:');
        data_set($col1, 'rem2.type', 'input');

        if ($noeditdate) {
            data_set($col1, 'dateid.class', 'sbccsreadonly');
        }

        $fields = ['terms', 'due', 'dacnoname', 'dwhname'];
    ...
    public function createnewtransaction($docno, $params)
    {
        ...
        $data[0]['clientname'] = $this->coreFunctions->getfieldvalue("client", "clientname", "client='WALK-IN'");
        $data[0]['yourref'] = '';
        $data[0]['address'] = '';
        $data[0]['ourref'] = '';
        $data[0]['rem'] = '';
        ...
        $data[0]['trnxtype'] = $this->coreFunctions->getfieldvalue("mode_masterfile", "name", "name='WALK-IN'");
        $data[0]['pline'] = $this->coreFunctions->getfieldvalue("mode_masterfile", "line", "name='WALK-IN'");
        $data[0]['modeofpayment'] = 'CASH';
    ...
    public function loadheaddata($config)
    {
        ...
        $qryselect = "select
         ...
         head.pono,ifnull(po.sano,'') as podesc,mode.line as pline,mode.name as trnxtype,head.modeofpayment,head.swsno,head.crno,head.rfno, head.chsino
         ";
    ...
    public function openstock($trno, $config)
    {
        ...
        warehouse.clientname,stock.loc,stock.expiry,stock.rem,stock.palletid,stock.locid,
        pallet.name,location.loc,uom.factor,head.forex,stock.rebate,
        prj.name,stock.projectid,stock.sgdrate,stock.noprint,brand.brand_desc,i.itemdescription,stock.itemstatus, stock.isqty order by sortline, line";
    ...
    public function quickadd($config)
    {
        ...
        $partno = $config['params']['barcode'];

        $wh = $config['params']['wh'];
        $item = $this->coreFunctions->opentable("select item.itemid,item.barcode,item.amt,item.disc,'' as loc,'" . $wh . "' as wh, 1 as qty, uom, '' as expiry from item where replace(partno,'-','')=?", [$partno]);


        if (!empty($item)) {
            $config['params']['barcode'] = $item[0]->barcode; //$barcode;
            ...
            return ['status' => false, 'msg' => 'Part No. not FOUND...' . $partno, ''];
        }
    }
    ...
    public function createdistribution($config)
    {
        ...
        $qry = 'select head.dateid,head.client,head.tax,head.contra,head.cur,head.forex,stock.ext,wh.client as wh,ifnull(item.asset,"") as asset,ifnull(item.revenue,"") as revenue,
          item.expense,stock.isamt,stock.disc,stock.isqty,stock.cost,stock.iss,stock.fcost,head.projectid,client.rev,stock.rebate,head.taxdef,m.name as trnxtype,head.modeofpayment
          ...
          left join client as wh on wh.clientid = stock.whid
          left join mode_masterfile as m on m.line = head.trnxtype where head.trno=?';
        ...
        if (!empty($stock)) {
            ...
            $revacct = $this->coreFunctions->getfieldvalue('coa', 'acno', 'alias=?', ['SA5']);
            ...
            switch (strtoupper($stock[0]->trnxtype)) {
                case 'WALK-IN':
                case 'SERVICE':
                case 'WHOLESALE':
                    if (strtoupper($stock[0]->modeofpayment) == 'CREDIT') {
                        $contra = $this->coreFunctions->getfieldvalue('coa', 'acno', 'alias=?', ['AR1']);
                        $revacct = $this->coreFunctions->getfieldvalue('coa', 'acno', 'alias=?', ['SA4']);
                    }
                    break;
                case 'CHARGE TO INSALES':
                case 'SERVICE UNIT':
                case 'MRC CLAIM':
                case 'PROMOTIONAL':
                    $contra = $this->coreFunctions->getfieldvalue('coa', 'acno', 'alias=?', ['AR4']);
                    $revacct = $this->coreFunctions->getfieldvalue('coa', 'acno', 'alias=?', ['SA4']);
                    break;
                case 'CHARGE TO MOTORMATE':
                    $contra = $this->coreFunctions->getfieldvalue('coa', 'acno', 'alias=?', ['AR1']);
                    $revacct = $this->coreFunctions->getfieldvalue('coa', 'acno', 'alias=?', ['SA4']);
                    break;
                case 'YAMAHA BUYBACK':
                    $contra = $this->coreFunctions->getfieldvalue('coa', 'acno', 'alias=?', ['AR1']);
                    $revacct = $this->coreFunctions->getfieldvalue('coa', 'acno', 'alias=?', ['IN2']);
                    break;
            }
            ...
            foreach ($stock as $key => $value) {
                ...
                if ($stock[$key]->trnxtype == 'YAMAHA BUYBACK') {
                    $cost = 0;
                }
    ...
    public function distribution($params, $config)
    {
        ...
        if (floatval($params['tax']) != 0) {
            ...
            $this->coreFunctions->LogConsole('sales' . $sales);
            if (floatval($sales) != 0) {

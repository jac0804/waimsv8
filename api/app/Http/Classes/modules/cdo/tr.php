...
class tr
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'REQUEST TO TRANSFER';
  ...
  public function createTab($access, $config)
  {
    ...
    $headgridbtns = ['viewref', 'disapprove'];

    if ($tr_btndisapprove_access == 0) {
      unset($headgridbtns[1]);
    }
    $action = 0;
    $reqqty = 1;
    $rrqty = 2;
  ...
  public function createHeadField($config)
  {
    ...
    if ($viewall == '0') {
      ...
      data_set($col1, 'wh2.readonly', true);
    } else {
      data_set($col1, 'client.action', 'lookupcenter');
      data_set($col1, 'client.lookupclass', 'trbranch');
      data_set($col1, 'docno.label', 'Transaction#');
    }

    $fields = ['dateid', 'wh', 'wh2'];
    $col2 = $this->fieldClass->create($fields);
    data_set($col2, 'wh.label', 'Request from');
    if ($viewall == '0') {
    ...
    data_set($col2, 'wh2.labeldata', 'whs2~wh2name');
    data_set($col2, 'wh2.required', true);
    data_set($col2, 'wh2.label', 'Requestor');
  ...
  {
    $viewall = $this->othersClass->checkAccess($params['user'], 4454);
    $data = [];
    $data[0]['trno'] = 0;
    $data[0]['docno'] = $docno;
    $data[0]['dateid'] = $this->othersClass->getCurrentDate();
    if ($viewall == '0') {
      $data[0]['client'] = $params['center'];
      $data[0]['clientname'] = $this->coreFunctions->getfieldvalue("center", "name", "code='" . $params['center'] . "'");
    } else {
      $data[0]['client'] = '';
      $data[0]['clientname'] = '';
    }
    ...
    $wh2id = $this->coreFunctions->getfieldvalue('client', 'clientid', 'client=?', [$data[0]['wh2']]);
  ...
  public function loadheaddata($config)
  {
    ...
    $qryselect = "select 
         ...
         head.wh,
         info.trnxtype,
         info.wh2 ,wh2.clientid as whid2,wh2.clientname as wh2name,
    ...
    $qry = $qryselect . ",null as approvedate,'' as approved from $table as head
        ...
        left join client as wh2 on wh2.client = info.wh2
        left join center as client on head.client = client.code
  ...
  public function unposttrans($config)
  {
    ...
    $qry = "insert into " . $this->head . "(trno,doc,docno,client,clientname,address,shipto,dateid,terms,rem,forex,
        ...
        select head.trno, head.doc, head.docno, center.code, head.clientname, head.address, head.shipto,
        ...
        from (" . $this->hhead . " as head left join cntnum on cntnum.trno=head.trno) left join center on center.code=head.client
        where head.trno=? limit 1";

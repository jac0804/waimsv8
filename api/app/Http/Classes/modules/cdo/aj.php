...
class aj
{
  ...
  public function createTab($access, $config)
  {
    ...
    $column =  ['action', 'itemdescription', 'serialno', 'pnp',  'rrqty', 'uom', 'kgs', 'rrcost', 'cost', 'ext', 'wh', 'whname', 'ref', 'loc', 'expiry', 'rem', 'pallet', 'location', 'itemname', 'stock_projectname', 'subcode', 'partno', 'boxcount', 'barcode'];
    $sortcolumn =  ['action', 'itemdescription', 'serialno', 'pnp',  'rrqty', 'uom', 'kgs', 'rrcost', 'cost', 'ext', 'wh', 'whname', 'ref', 'loc', 'expiry', 'rem', 'pallet', 'location', 'itemname', 'stock_projectname', 'subcode', 'partno', 'boxcount', 'barcode'];
    ...
    $obj[0]['inventory']['columns'][$serial]['type'] = 'textarea2';
  ...
  public function createHeadField($config)
  {
    ...
    data_set($col1, 'dwhname.readonly', true);
  ...
  public function posttrans($config)
  {
    ...
    if ($serial) {
      if (!$this->othersClass->checkserialout($config)) {
        return ['trno' => $trno, 'status' => false, 'msg' => 'Posting failed. Engine numbers do not match the quantity of the item.'];
      }
    }
  ...
  private function getserialout($config)
  {
    ...
    if (!empty($config['params']['rows'])) {
      ...
      $eline = $this->coreFunctions->datareader("select group_concat(sline separator ',') as value from serialin as sj  where sj.outline in (" . $soutline . ")");
      if ($eline <> '') {
        ...
        if ($cost != -1) {
          ...
          $cost2 = $cost / $item[0]->uomfactor;
          $this->coreFunctions->sbcupdate($this->stock, ['cost' => $cost, 'ext' => $computedata['ext']], ['trno' => $trno, 'line' => $line]);
      ...
      $this->coreFunctions->sbcupdate($this->stock, ["iss" => $iss], ['trno' => $trno, 'line' => $line]);
  ...
  public function additem($action, $config)
  {
    ...
    if ($rrqty < 0) {
      if ($isserial == 1) {
        if ($action == 'update' && $config['params']['data']['iss'] != 0) {
        ...
        if ($ispallet) {
          ...
          if ($isserial == 0) {
            ...
            if ($rrref <> '') {
              ...
              $this->coreFunctions->execqry('delete from costing where trno=? and line=?', 'delete', [$trno, $line]);
              $this->logger->sbcwritelog($trno, $config, 'STOCK', 'NO SERIAL - Line:' . $line . ' barcode:' . $item[0]->barcode .  ' Amt:' . $amt . ' Disc:' . $disc . ' wh:' . $wh . ' ext:0.0');
  ...
  public function createdistribution($config)
  {
    ...
    $qry = 'select head.dateid,head.client,head.tax,head.contra,head.cur,head.forex,((stock.qty-stock.iss) * stock.cost) as ext,wh.client as wh,ifnull(item.asset,"") as asset,ifnull(item.revenue,"") as revenue,stock.rrcost,stock.disc,stock.rrqty,head.projectid,cat.stockgrp_name as category

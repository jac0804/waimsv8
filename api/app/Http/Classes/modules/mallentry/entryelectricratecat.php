...
class entryelectricratecat
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'ELECTRICITY RATE CATEGORY';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'ratecategory';
  private $othersClass;
  public $style = 'width:100%;';
  public $tablelogs = 'masterfile_log';
  public $tablelogs_del = 'del_masterfile_log';
  private $fields = ['category', 'iselec'];
  ...
  public function createTab($config)
  {
    ...
    $obj[0][$this->gridname]['columns'][$category]['style'] = "width:90%;whiteSpace: normal;min-width:90%;";
  ...
  {
    $qry = "
    head.line, head.category, head.iselec";
    return $qry;
  }
  ...
  public function delete($config)
  {
    ...
    $qry = "
      select categoryid as value from electricrate where categoryid=?
      union all
      select categoryid as value from selectricrate where categoryid=?
      LIMIT 1
    ";
    $count = $this->coreFunctions->datareader($qry, [$row['line'], $row['line']]);

    if ($count != '') {
      return ['clientid' => $row['category'], 'status' => false, 'msg' => 'Already Transaction...'];
    }
  ...
  private function loaddataperrecord($line)
  {
    ...
    where head.iselec = 1 and head.line=?";
  ...
  public function loaddata($config)
  {
    ...
    $qry = "select " . $select . " 
    from " . $this->table . " as head
    where head.iselec=1 " . $filtersearch . "
  ...
  private function rpt_default_header($data, $filters)
  {
    ...
    $str .= $this->reporter->begintable('800');
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('Category', '800', null, false, '1px solid ', 'B', 'L', 'Century Gothic', '12', 'B', '', '2px');
    $str .= $this->reporter->endrow();
    return $str;
  }
  ...
  private function rpt_project_masterfile_layout($data, $filters)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $str .= $this->reporter->col($data[$i]['category'], '800', null, false, '1px solid ', '', 'L', 'Century Gothic', '11', '', '', '3px');
      $str .= $this->reporter->endrow();
  ...
  private function rpt_part_PDF_header_PDF($data, $filters)
  {
    ...
    PDF::SetFont($fontbold, '', 11);
    PDF::MultiCell(700, 20, "Category", '', 'L', false, 0);
  ...
  private function rpt_project_PDF($data, $filters)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      PDF::MultiCell(700, 10, $data[$i]['category'], '', 'L', 0, 0, '', '', true, 0, true, false);

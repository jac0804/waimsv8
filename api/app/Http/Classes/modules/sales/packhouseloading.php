...
class packhouseloading
{
    private $btnClass;
    private $fieldClass;
    private $tabClass;
    public $modulename = 'Pack House Loading';
    ...
    private $stockselect;
    private $fields = ['trno', 'docno', 'dateid', 'due', 'client', 'clientname', 'yourref', 'ourref', 'rem', 'terms', 'forex', 'cur', 'wh', 'address', 'contra', 'tax', 'vattype', 'agent', 'projectid', 'creditinfo', 'billid', 'shipid', 'branch', 'deptid', 'taxdef', 'billcontactid', 'shipcontactid', 'ms_freight', 'mlcp_freight', 'shipto', 'salestype', 'sotrno', 'statid'];
    ...
    public function createdoclisting($config)
    {
        ...
        $getcols = ['action', 'liststatus', 'listdocument', 'listdate', 'listclientname', 'yourref', 'ourref', 'total', 'ar', 'listpostedby', 'listcreateby', 'listeditby', 'listviewby', 'receiveby', 'receivedate'];
        ...
        if ($companyid == 10 || $companyid == 12) { //afti, afti usd
            ...
            $cols[$editby]['type'] = 'coldel';
            $cols[$viewby]['type'] = 'coldel';
    ...
    public function loaddoclisting($config)
    {
        ...
        $qry = "select head.trno,head.docno,head.clientname,$dateid, $lstat as status, $lstatcolor as statuscolor,
        head.createby,head.editby,head.viewby,num.postedby,
        head.yourref, head.ourref $lfield
        from " . $this->head . " as head left join " . $this->tablenum . " as num
        on num.trno=head.trno left join cntnuminfo as info on info.trno=head.trno
        $ljoin
        " . $join . "
        where num.statid=19 and head.doc=? and num.center = ? and CONVERT(head.dateid,DATE)>=? and CONVERT(head.dateid,DATE)<=? " . $condition . $addparams . " $filtersearch
        and num.bref <> 'SJS' and info.packdate is null $group
        $orderby $limit";
    ...
    public function createTab($access, $config)
    {
        ...
        $column = ['action', 'itemdescription', 'serialno', 'isqty', 'uom', 'kgs', 'isamt', 'disc', 'ext', 'markup', 'rebate', 'wh', 'whname', 'ref', 'loc', 'expiry', 'rem', 'itemname', 'stock_projectname', 'noprint', 'barcode'];
        $sortcolumn = ['action', 'itemdescription', 'serialno', 'isqty', 'uom', 'kgs', 'isamt', 'disc', 'ext', 'markup', 'rebate', 'wh', 'whname', 'ref', 'loc', 'expiry', 'rem', 'itemname', 'stock_projectname', 'noprint', 'barcode'];
        ...
        if ($companyid == 15 || $companyid == 17 || $companyid == 20) {
            if ($release) {
    ...
    {
        $companyid = $config['params']['companyid'];
        $fields = ['docno', 'client', 'clientname', 'address', ['hauler', 'driver'], ['weightin', 'weightintime']];
        $col1 = $this->fieldClass->create($fields);
        data_set($col1, 'client.lookupclass', 'customer');
        data_set($col1, 'client.required', false);
        data_set($col1, 'docno.label', 'Transaction#');

        $fields = [['dateid', 'terms'], 'due', 'dacnoname', 'dwhname', 'statname', ['plateno', 'licenseno'], ['weightout', 'weightouttime']];
        $col2 = $this->fieldClass->create($fields);
        ...
        $fields = ['rem', 'creditinfo', ['updatepostedinfo', 'tagpacked']];
        if ($this->companysetup->getistodo($config['params'])) {
            array_push($fields, 'donetodo');
        }
        $col4 = $this->fieldClass->create($fields);
        data_set($col4, 'updatepostedinfo.label', 'UPDATE INFO');
        data_set($col4, 'updatepostedinfo.style', "font-size:84%");
        data_set($col4, 'tagpacked.style', "font-size:84%;");
    ...
    public function loadheaddata($config)
    {
        ...
        $qryselect = "select
         ...
         head.driver, ifnull(hinfo.hauler,'') as hauler, ifnull(hinfo.plateno,'') as plateno, ifnull(hinfo.licenseno,'') as licenseno
         , ifnull(hinfo.batchno,'') as batchno
        ...
        and head.lockdate is not null and num.postdate is null and num.statid=19 ";
    ...
    prj.name,stock.projectid,stock.sgdrate,stock.noprint,brand.brand_desc,i.itemdescription order by sortline, line";
    ...
    warehouse.clientname,stock.loc,stock.expiry,stock.rem,stock.palletid,stock.locid,
    pallet.name,location.loc,uom.factor,head.forex,stock.rebate,
    prj.name,stock.projectid,stock.sgdrate,stock.noprint,brand.brand_desc,i.itemdescription";
    ...
    private function tagpacked($config)
    {
        $trno = $config['params']['trno'];
        $islocked = $this->othersClass->islocked($config);

        $batchno = $this->coreFunctions->getfieldvalue('cntnuminfo', 'batchno', 'trno=?', [$trno]);
        $assignedlane = $this->coreFunctions->getfieldvalue('cntnuminfo', 'assignedlane', 'trno=?', [$trno]);

        if (empty($batchno) || empty($assignedlane)) {
            return ['status' => false, 'msg' => 'Batch No. And Assigned Lane Must Not Be Empty'];
        } else {
            if ($islocked) {
                $result = $this->generateAJComponent($config, $batchno);
                if (!$result['status']) {
                    return ['status' => false, 'msg' => $result['msg']];
                }
                $this->coreFunctions->sbcupdate($this->tablenum, ['statid' => 41], ['trno' => $trno]);
                $this->coreFunctions->sbcupdate('cntnuminfo', [
                    'packdate' => $this->othersClass->getCurrentTimeStamp(),
                    'editdate' => $this->othersClass->getCurrentTimeStamp(),
                    'editby' =>  $config['params']['user']
                ], ['trno' => $trno]);
                $this->logger->sbcwritelog($trno, $config, 'HEAD', 'Pack House Loaded');

                return ['status' => true, 'msg' => 'Successfully tagged', 'backlisting' => true];
            } else {
                return ['status' => false, 'msg' => 'Please lock the transaction first'];
                // return ['status' => false, 'msg' => 'Check fields'];
            }
        }
    }

    public function generateAJComponent($config, $batchno)
    {
        $companyid = $config['params']['companyid'];
        $status = true;
        $msg = '';
        ...
        $qry = "SELECT ic.itemid, c.barcode, sum(c.isqty) AS cqty, c.uom AS cuom, s.isqty, s.uom, s.whid, c.isloc
                FROM lastock AS s LEFT JOIN component AS c ON c.itemid=s.itemid LEFT JOIN item AS ic ON ic.barcode=c.barcode
                WHERE s.trno=? GROUP BY ic.itemid, c.barcode, c.uom, s.isqty, s.uom, s.whid, c.isloc";
        $comp = $this->coreFunctions->opentable($qry, [$trno]);

        $havestock = true;

        try {

            $tmpcomp = $this->coreFunctions->opentable("select line from lastock where iscomponent=1 and trno=?", [$trno]);
            if (!empty($tmpcomp)) {
                foreach ($tmpcomp as $key => $v) {
                    $this->coreFunctions->execqry('delete from lastock where trno=? and line=?', 'delete', [$trno, $v->line]);
                    $this->coreFunctions->execqry('delete from costing where trno=? and line=?', 'delete', [$trno, $v->line]);
                }
            }

            if (!empty($comp)) {
                // $this->coreFunctions->LogConsole(json_encode($comp));
                foreach ($comp as $key => $val) {
                    if ($val->isloc == 1) {
                        if ($batchno == '') {
                            return ['status' => false, 'msg' => 'Some item components required batch no, please input valid batch no for this transaction'];
                        }
                    }
                }
                ...
                foreach ($comp as $key => $val) {
                    ...
                    $item = $this->coreFunctions->opentable($qry, [$val->cuom, $val->itemid]);
                    $factor = 1;
                    if (!empty($item)) {
                    ...
                    $rrqty = ($val->cqty *  $val->isqty);
                    $computedata = $this->othersClass->computestock(0, '', $rrqty, $factor);
                    ...
                    $stock = [
                        ...
                        'uom' => $val->cuom,
                        'isqty' =>  $rrqty,
                        'iss' => abs($computedata['qty']),
                        'ext' => 0,
                        'loc' => $val->isloc == 1 ? $batchno : '',
                    ...
                    foreach ($stock as $skey => $sval) {
                         $stock[$skey] = $this->othersClass->sanitizekeyfieldFast($skey, $stock[$skey], $lookups);
                    }

                    // $this->coreFunctions->LogConsole(json_encode($stock));

                    if ($stock['itemid'] == "") {
                        return ['status' => false, 'msg' => 'Invalid component'];
                    }

                    if ($this->coreFunctions->sbcinsert('lastock', $stock)) {

                        $cost = $this->othersClass->computecosting($stock['itemid'], $stock['whid'], $stock['loc'], '', $trno, $line, $stock['iss'], 'SJ', $config['params']['companyid']);
                        // $this->coreFunctions->LogConsole('line: ' . $line . ' - cost:' . $cost);
                        if ($cost != -1) {
                            ...
                            $havestock = false;
                            $this->coreFunctions->sbcupdate($this->stock, ['isqty' => 0, 'iss' => 0, 'ext' => 0, 'editby' => 'OUT_STOCK', 'editdate' => $this->othersClass->getCurrentTimeStamp()], ['trno' => $trno, 'line' => $line]);
                            $this->coreFunctions->execqry('delete from costing where trno=? and line=?', 'delete', [$trno, $line]);

                            $out_of_stock =  'GENERATE AJ FAILED: OUT OF STOCK - Line:' . $line . ' Barcode:' . $item[0]->barcode . ' Item Name:' . $item[0]->itemname . ' wh:' . $wh . ' batch no: ' . $stock['loc'];
                            $this->logger->sbcwritelog($trno, $config, 'STOCK', $out_of_stock, "table_log");
                            $msg .= $out_of_stock;
                            $status = false;
                        }
                    } else {
                        $msg .= "Failed to insert " . $item[0]->barcode;
                    }
                }
            }
        } catch (Exception $e) {
        ...
        // Generates another AJ transaction
        // try {
        //     if (!empty($comp)) {
        //         $sjdocno = $this->coreFunctions->getfieldvalue("lahead", "docno", "trno=?", [$trno]);
        //         $wh = $this->coreFunctions->getfieldvalue("lahead", "wh", "trno=?", [$trno]);
        ...
        //                 'createby' => $user,
        //                 'wh' => $wh,
        //                 'rem' => 'Auto-generated adjustment for components used in ' . $sjdocno
        //             ];
        ...
        //                 foreach ($comp as $key => $val) {
        //                     $qry = "select line as value from lastock where trno=? order by line desc limit 1";
        //                     $line = $this->coreFunctions->datareader($qry, [$ajtrno], '', true);
        //                     $line = $line + 1;
        ...
        //                     if ($this->coreFunctions->sbcinsert('lastock', $stock)) {
        //                         $cost = $this->othersClass->computecosting($stock['itemid'], $stock['whid'], '', '', $ajtrno, $line, $stock['iss'], 'AJ');

        //                         if ($cost != -1) {
        //                             $this->coreFunctions->sbcupdate($this->stock, ['cost' => $cost, 'rrcost' => round($cost * $factor, $this->companysetup->getdecimal('price', $config['params'])), 'ext' => round($cost * $factor, $this->companysetup->getdecimal('price', $config['params'])) * $rrqty], ['trno' => $ajtrno, 'line' => $line]);
        //                         } else {
        //                             $havestock = false;
        //                             $this->coreFunctions->sbcupdate($this->stock, ['rrqty' => 0, 'iss' => 0, 'ext' => 0, 'editby' => 'OUT_STOCK', 'editdate' => $this->othersClass->getCurrentTimeStamp()], ['trno' => $ajtrno, 'line' => $line]);
        //                             $this->coreFunctions->execqry('delete from costing where trno=? and line=?', 'delete', [$ajtrno, $line]);

        //                             $out_of_stock =  'OUT OF STOCK - Line:' . $line . ' barcode:' . $item[0]->barcode . ' wh:' . $wh;
        //                             $this->logger->sbcwritelog($trno, $config, 'STOCK', $out_of_stock);
        //                             $msg .= $out_of_stock;
        //                         }
        //                     }
        //                 }
        ...
        //                     if ($return['status']) {
        //                         $this->coreFunctions->sbcupdate("lahead", ['ajtrno' => $ajtrno], ['trno' => $trno]);
        //                         $status = true;
        //                     } else {
        //                         $msg = 'Failed to post AJ ' . $return['msg'];
        //                         $status = false;
        //                         goto deletehere;
        //                     }
        //                 }
    ...
    private function downloadexcel($config)
    {
        ...
        if ($cntnum[0]->postdate == "") {
            return ['status' => false, 'msg' => 'Failed to download, please post the transaction first', 'name' => 'dr', 'data' => []];
        }
        $data = $this->coreFunctions->opentable("select item.barcode as `itemcode`, s.uom, s.isqty as `qty`, s.disc, s.isamt as `cost`, s.kgs from glhead as h left join glstock as s on s.trno=h.trno left join item on item.itemid=s.itemid where h.trno=? order by s.sortline, s.line", [$trno]);
    ...
    public function additem($action, $config, $setlog = false)
    {
        ...
        if ($companyid == 10) {
            if ($disc != "") {
                $computedata = $this->othersClass->computestock($amt, $disc, $qty, $factor, 0, $cur, $kgs, 1);
            } else {
        ...
        $hamt = number_format($computedata['amt'] * $curtopeso, $this->companysetup->getdecimal('price', $config['params']), '.', '');
        if ($companyid == 10) {
            if ($disc != "") {
        ...
        if ($action == 'insert') {
            ...
            if ($this->coreFunctions->sbcinsert($this->stock, $data) == 1) {
                ...
                $this->logger->sbcwritelog($trno, $config, 'STOCK', 'ADD - Line:' . $line . ' barcode:' . $item[0]->barcode . ' Qty' . $qty . ' Amt:' . $amt . ' Disc:' . $disc . ' wh:' . $wh . ' ext:' . $computedata['ext'], $setlog ? $this->tablelogs : '');
                if ($isnoninv == 0) {
                    if ($ispallet) {
    ...
    {
        return "
        select head.docno,head.client, head.clientname, head.address, ifnull(head.rem,'') as rem, head.cur, head.forex, head.shipto, head.ourref, head.yourref, head.terms, ifnull(head.branch,0) as branch,
        ...
        stock.disc,stock.loc,stock.expiry,stock.projectid,head.shipto,head.mlcp_freight,head.ms_freight,head.agent,head.projectid as hprojectid,wh.client as swh
        FROM hsohead as head left join hsostock as stock on stock.trno=head.trno left join item on item.itemid=stock.itemid left join uom on uom.itemid=item.itemid and uom.uom=stock.uom
    ...
    public function getsodetails($config)
    {
        ...
        round((stock.iss-stock.qa)/ case when ifnull(uom.factor,0)=0 then 1 else uom.factor end," . $this->companysetup->getdecimal('qty', $config['params']) . ") as isqty,
        stock.disc,stock.loc,stock.expiry,stock.projectid,wh.client as swh
        FROM hsohead as head left join hsostock as stock on stock.trno=head.trno left join item on item.itemid=
    ...
    public function createdistribution($config)
    {
        ...
        stock.rebate,head.deptid,head.branch,head.taxdef,head.yourref,head.sotrno
        ...
        left join coa as e on e.acnoid = p.expenseid where head.trno=?';
            ...
            $qry = 'select head.dateid,head.client,head.tax,head.contra,head.cur,head.forex,stock.ext,wh.client as wh,ifnull(item.asset,"") as asset,ifnull(item.revenue,"") as revenue,item.expense,stock.isamt,stock.disc,stock.isqty,stock.cost,stock.iss,stock.fcost,head.projectid,client.rev,stock.rebate,head.taxdef

...
class vt
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'VOID SALES ORDER';
  ...
  public $head = 'vthead';
  public $hhead = 'hvthead';
  public $stock = 'vtstock';
  public $hstock = 'hvtstock';
    ...
    if (isset($config['params']['doclistingparam'])) {
      ...
      if (isset($test['selectprefix'])) {
        if ($test['selectprefix'] != "") {
          ...
          if (isset($test)) {
            $join = " left join vtstock on vtstock.trno = head.trno
            left join item on item.itemid = vtstock.itemid left join item as item2 on item2.itemid = vtstock.itemid
            ...
            $hjoin = " left join hvtstock on hvtstock.trno = head.trno
            left join item on item.itemid = hvtstock.itemid left join item as item2 on item2.itemid = hvtstock.itemid
            left join model_masterfile as model on model.model_id = item.model 
            left join model_masterfile as model2 on model2.model_id = item2.model
            left join frontend_ebrands as brand on brand.brandid = item.brand 
            left join frontend_ebrands as brand2 on brand2.brandid = item2.brand
  ...
  public function createtabbutton($config)
  {
    ...
    $tbuttons = ['saveitem', 'deleteallitem', 'pendingqs'];
    $obj = $this->tabClass->createtabbutton($tbuttons);
    $obj[2]['action'] = 'pendingqsdetail';
    return $obj;
  }
  ...
  public function createHeadField($config)
  {
    ...
    data_set($col2, 'yourref.lookupclass', 'lookupsoref_void');
    data_set($col2, 'yourref.action', 'lookupsoref_void');
  ...
  public function loadheaddata($config)
  {
    ...
    $qry = $qryselect . " from $table as head
        ...
        left join hsqhead as sqhead on sqhead.trno = head.sotrno
        left join hqshead as qt on qt.sotrno=sqhead.trno
  ...
  public function posttrans($config)
  {
    ...
    $qry = "insert into " . $this->hhead . "(trno, doc, docno, client, clientname, address, shipto, tel, dateid, yourref, ourref, lockuser, 
      lockdate, openby, users, createdate, createby, editby, editdate, viewby, viewdate, projectid, 
      sotrno,rem,newpo)
      SELECT 
      ...
      head.lockdate, head.openby, head.users, head.createdate, head.createby, head.editby, head.editdate, head.viewby, head.viewdate, head.projectid, 
      head.sotrno,head.rem,head.newpo
  ...
  public function stockstatus($config)
  {
    switch ($config['params']['action']) {
      ...
      case 'getqsdetails':
        ...
        break;
      case 'getqssummary':
        return $this->getqssummary($config);
        break;
      default:
  ...
  public function diagram($config)
  {
    ...
    $sotrno = $this->coreFunctions->datareader("select sotrno as value from hvthead where trno = ? LIMIT 1", [$config['params']['trno']]);

    $qstrno = $this->coreFunctions->datareader("
      select qthead.trno as value
      from hqshead as qthead
      left join hsqhead as sqhead on sqhead.trno = qthead.sotrno
      where sqhead.trno = ? LIMIT 1", [$sotrno]);
    ...
    if (!empty($t)) {
      ...
      foreach ($t as $key => $value) {
        ...
        array_push($links, ['from' => $t[$key]->docno, 'to' => 'SQ']);
        $a = $a + 100;

        $qry = "select head.trno,head.docno,left(head.dateid,10) as dateid,
        CAST(concat('Total SQ Amt: ',round(sum(s.ext),2)) as CHAR) as rem
        from hsqhead as head
        left join hqshead as qthead on qthead.sotrno=head.trno
        left join hqsstock as s on s.trno = qthead.trno
        where s.trno = ?
        group by head.trno,head.docno,head.dateid";
        $sqdata = $this->coreFunctions->opentable($qry, [$qstrno]);

        if (!empty($sqdata)) {
          foreach ($sqdata as $k => $sq) {
            data_set(
              $nodes,
              'SQ',
              [
            ...
            array_push($links, ['from' => 'SQ', 'to' => 'VT']);
            $a = $a + 100;
            ...
            left join hvtstock as s on head.trno=s.trno
            where head.trno = ?
            group by head.trno,head.docno,head.dateid";
            $vtdata = $this->coreFunctions->opentable($qry, [$config['params']['trno']]);

            if (!empty($vtdata)) {
              foreach ($vtdata as $kk => $vt) {
                data_set(
                  $nodes,
                  'VT',
                  [
    ...
    $t = $this->coreFunctions->opentable($qry, [$sotrno, $sotrno]);
    if (!empty($t)) {
      data_set(
        $nodes,
        'sj',
  ...
  public function updatevoidQty($refx, $line, $iss)
  {
    $qttrno = $this->coreFunctions->getfieldvalue('hqshead', 'trno', 'sotrno=?', [$refx]);
    $voiddata = ['voidqty' => $iss];
    $this->coreFunctions->sbcupdate('hqsstock', $voiddata, ['trno' => $qttrno, 'line' => $line]);
  }
  ...
  public function getqssummary($config)
  {
    $trno = $config['params']['trno'];
    $rows = [];
    $msg = '';
    foreach ($config['params']['rows'] as $key => $value) {
      $qry = "
      select qtstock.trno, qtstock.line, head.docno, head.dateid, item.barcode, item.itemname, 
      qtstock.uom, (qtstock.isqty-qtstock.sjqa-qtstock.voidqty)/uom.factor as isqty, qtstock.iss,
      ...
      left join uom on uom.itemid = item.itemid and uom.uom = qtstock.uom
      where head.trno = ? and qtstock.iss != (qtstock.sjqa + qtstock.voidqty) and qtstock.void != 1";
  ...
  public function getqsdetails($config)
  {
    ...
    foreach ($config['params']['rows'] as $key => $value) {
      $qry = "
      select qtstock.trno, qtstock.line, head.docno, head.dateid, item.barcode, item.itemname, 
      qtstock.uom, (qtstock.iss-qtstock.sjqa-qtstock.voidqty)/uom.factor as isqty, qtstock.iss,
      qtstock.isamt, qtstock.amt, qtstock.ext,
      qtstock.itemid
      from hsqhead as head
      left join hqshead as qthead on qthead.sotrno = head.trno
      ...
      left join item as item on item.itemid = qtstock.itemid
      left join uom on uom.itemid = item.itemid and uom.uom = qtstock.uom
      where head.trno = ? and qtstock.line = ?
  ...
  public function setserveditems($refx, $linex)
  {
    ...
    stock on stock.trno=head.trno where head.doc='VT' and stock.refx=" . $refx . " and stock.linex=" . $linex;

    $qry1 = $qry1 . " union all select stock." . $this->hqty . " from " . $this->hhead . " as head left join " . $this->hstock . " as stock on stock.trno=
    head.trno where head.doc='VT' and stock.refx=" . $refx . " and stock.linex=" . $linex;
    ...
    //update void=1
    $qtqty = $this->coreFunctions->getfieldvalue("hqsstock", "isqty", "trno=? and line =?", [$refx, $linex]);
    if ($qtqty == $qty) {
      $this->coreFunctions->execqry("update hqsstock set void=1  where  trno=" . $refx . " and line=" . $linex, 'update');
    } else {
      $this->coreFunctions->execqry("update hqsstock set void=0  where  trno=" . $refx . " and line=" . $linex, 'update');
    }
    return $this->coreFunctions->execqry("update hqsstock set voidqty=" . $qty . " where trno=" . $refx . " and line=" . $linex, 'update');
  }

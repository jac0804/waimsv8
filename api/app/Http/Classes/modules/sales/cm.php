...
class cm
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'SALES RETURN';
  ...
  public function createdoclisting($config)
  {
    ...
    $getcols = ['action', 'liststatus', 'listdocument', 'whname', 'listdate', 'listclientname', 'shipto', 'yourref', 'ourref', 'rem', 'postdate', 'listpostedby', 'listcreateby', 'listeditby', 'listviewby'];
    ...
    if ($companyid != 22) $cols[$shipto]['type'] = 'coldel'; //not eipi
  ...
  public function paramsdatalisting($config)
  {
    ...
    switch ($companyid) {
      ...
      case 39: //cbbsi
        $fields = [];
        $allownew = $this->othersClass->checkAccess($config['params']['user'], 192);
        if ($allownew == '1') $fields = ['pickpo'];
        $col1 = $this->fieldClass->create($fields);
        data_set($col1, 'pickpo.label', 'pick request for return');
        data_set($col1, 'pickpo.lookupclass', 'pendingrfshortcut');
        data_set($col1, 'pickpo.action', 'pendingrfshortcut');
        data_set($col1, 'pickpo.confirmlabel', 'Proceed to pick Return Request?');
        data_set($col1, 'pickpo.addedparams', ['docno', 'selectprefix']);

        $fields = ['selectprefix', 'docno'];
        $col2 = $this->fieldClass->create($fields);

        $prefix = $this->coreFunctions->getfieldvalue('profile', 'pvalue', 'doc=? and psection=?', ['SED', 'DR']);
        if ($prefix != '') {
  ...
  public function loaddoclisting($config)
  {
    ...
    $status = " case when num.statid=36 then 'Approved' when num.statid=10 then 'For approval' else 'DRAFT' end ";
    switch ($itemfilter) {
      case 'draft':
    ...
    if ($companyid != 60) { //transpower
      $filterbref = " and left(num.bref,3) <> 'SRS'";
    }
    ...
    date(num.postdate) as postdate,  head.yourref, head.ourref, head.rem,head.shipto $field
  ...
  public function createHeadbutton($config)
  {
    ...
    if ($config['params']['companyid'] == 56) { // homeworks
      ...
      $buttons['others']['items']['downloadexcel'] = ['label' => 'Download CM Template', 'todo' => ['type' => 'downloadexcel', 'action' => 'downloadexcel', 'lookupclass' => 'downloadexcel', 'access' => 'view']];
    }
    return $buttons;
  } // createHeadbutton
  ...
  public function createtab2($access, $config)
  {
    ...
    if ($config['params']['companyid'] == 60) { //transpower      
      $changecode = $this->othersClass->checkAccess($config['params']['user'], 5490);
      if ($changecode) {
  ...
  public function createtabbutton($config)
  {
    switch ($config['params']['companyid']) {
      case 39: //cbbsi
        $tbuttons = ['pendingdr', 'additem', 'quickadd', 'saveitem', 'deleteallitem'];
        break;
      case 63: //ericco
        $tbuttons = ['pendingsj', 'multiitem', 'quickadd', 'saveitem', 'deleteallitem'];
        break;
      default:
        $tbuttons = ['pendingsj', 'additem', 'quickadd', 'saveitem', 'deleteallitem'];
        break;
    }
  ...
  public function createHeadField($config)
  {
    ...
    if ($companyid == 22) { //eipi
      data_set($col1, 'shipto.label', 'Delivered To');
    }
    ...
    switch ($companyid) {
      case 39: //cbbsi
        data_set($col2, 'dacnoname.cleartxt', true);
    ...
    if ($config['params']['companyid'] == 10) { //afti
      ...
      data_set($col4, 'ddeptname.required', true);
    }
    if ($systemtype == 'REALESTATE') {
  ...
  public function loadheaddata($config)
  {
    ...
    if ($trno == 0) {
      ...
      if ($trno == '') {
        if ($companyid != 60) {
          ...
          where doc=? and center=? and left(cntnum.bref,3) <> 'SRS'
          order by trno desc 
          limit 1", [$doc, $center]);
        }
      }
    ...
    $qryselect = "select
         ...
         head.branch,'' as dbranchname, head.billid, head.shipid, head.billcontactid, head.shipcontactid,
          head.phaseid, ps.code as phase,  head.modelid, hm.model as housemodel, head.blklotid, 
            bl.blk as blklot,  bl.lot, amen.line as amenityid, amen.description as amenityname, 
            subamen.line as subamenityid, subamen.description as subamenityname,num.statid ";
    ...
    $qry = $qryselect . " from $table as head
        ...
        where head.trno = ? and num.doc=? and num.center = ? $filterbref
  ...
  public function posttrans($config)
  {
    ...
    if ($config['params']['companyid'] == 19) { //housegem
      $zerocost = $this->coreFunctions->opentable('select trno from lastock where cost=0 and trno=?', [$trno]);
      if ($zerocost) {
        return ['trno' => $trno, 'status' => false, 'msg' => 'Posting failed. Check carefully, some items have no cost.'];
      }
    }
    if ($config['params']['companyid'] == 60) { //transpower
      $si_cr = $this->coreFunctions->opentable('select yourref,ourref from lahead where trno=?', [$trno]);
      if (empty($si_cr)) {
        return ['trno' => $trno, 'status' => false, 'msg' => 'Both Your Ref and Our Ref are empty.'];
      } else {
        if ($si_cr[0]->yourref == '') {
          return ['trno' => $trno, 'status' => false, 'msg' => 'Posting failed. Check the SI# / CR# field is empty.'];
        }
        if ($si_cr[0]->ourref == '') {
          return ['trno' => $trno, 'status' => false, 'msg' => 'Posting failed. Check the Ourref field is empty.'];
        }
      }
    }
  ...
  {
    return "
      select head.docno, head.clientid, client.client, client.clientname, head.address, ifnull(head.rem,'') as rem, head.cur, head.forex, head.shipto, head.ourref, head.yourref, head.projectid, head.terms,
      item.itemid,stock.trno, stock.line, item.barcode,stock.uom, stock.cost, (stock.qty-stock.qa) as qty,stock.rrcost,stock.ext, head.whid, wh.client as wh,
  ...
  public function getrfsummaryqry($config)
  {
    return "
    select head.docno, client.clientid, client.client, client.clientname, head.address, ifnull(head.rem,'') as rem, head.cur, head.forex, head.shipto, head.ourref, head.yourref, head.projectid, head.terms,
    item.itemid,stock.trno, stock.line, item.barcode,stock.uom,  (stock.iss-stock.qa) as qty,stock.isamt,stock.ext, stock.whid, wh.client as wh,
    round((stock.iss-stock.qa)/ case when ifnull(uom.factor,0)=0 then 1 else uom.factor end," . $this->companysetup->getdecimal('qty', $config['params']) . ") as isqty,stock.rem as srem,
    stock.disc,stock.stageid,head.branch,head.tax,head.vattype,head.yourref,wh.client as swh,stock.loc,stock.refx,stock.linex,head.agent,head.due,stock.ref
    FROM hckhead as head
    left join hckstock as stock on stock.trno=head.trno
    left join item on item.itemid=stock.itemid
    left join uom on uom.itemid=item.itemid and uom.uom=stock.uom
    left join client as wh on wh.clientid=stock.whid
    ...
    foreach ($config['params']['rows'] as $key => $value) {
      ...
      if (!empty($data)) {
        if ($companyid == 39) { //cbbsi
          $this->coreFunctions->execqry("update lahead set client='" . $data[0]->client . "', clientname='" . $data[0]->clientname . "', address='" . $data[0]->address . "' where trno=" . $trno, 'update');
        }
  ...
  public function additem($action, $config, $setlog = false)
  {
    ...
    if (isset($config['params']['data']['ckrefx'])) {
      $ckrefx = $config['params']['data']['ckrefx'];
    }

    if (isset($config['params']['data']['cklinex'])) {
      $cklinex = $config['params']['data']['cklinex'];
    }
    ...
    if ($action == 'insert') {
      ...
      if ($ckrefx != 0) {
        if ($this->setservedrqitems($refx, $linex, $ckrefx, $cklinex) === 0) {
          $data2 = [$this->dqty => 0, $this->hqty => 0, 'ext' => 0];
          $this->coreFunctions->sbcupdate($this->stock, $data2, ['trno' => $trno, 'line' => $line]);
          $this->setservedrqitems($refx, $linex, $ckrefx, $cklinex);
          $return = false;
        }
      } else {
  ...
  {
    $trno = $config['params']['trno'];
    $data = $this->coreFunctions->opentable('select refx,linex,ckrefx,cklinex from ' . $this->stock . ' where trno=? and refx<>0', [$trno]);
    ...
    foreach ($data as $key => $value) {
      if ($data[$key]->ckrefx <> 0) {
        $this->setservedrqitems($data[$key]->refx, $data[$key]->linex, $data[$key]->ckrefx, $data[$key]->cklinex);
      } else {
  ...
  public function setservedrqitems($refx, $linex, $ckrefx, $cklinex)
  {
    if ($refx == 0) {
      return 1;
    }
    ...
    return $this->coreFunctions->execqry("update hckstock set qa=" . $qty . " where trno=" . $ckrefx . " and line=" . $cklinex, 'update');
  }
  ...
  public function deleteitem($config)
  {
    ...
    if ($data[0]->ckrefx !== 0) {
      $this->setservedrqitems($data[0]->refx, $data[0]->linex, $data[0]->ckrefx, $data[0]->cklinex);
    } else {
      if ($data[0]->refx !== 0) {
  ...
  public function createdistribution($config)
  {
    ...
    if ($companyid == 10) { //afti
      $qry = 'select head.dateid,head.client,head.tax,head.contra,head.cur,head.forex,stock.ext,wh.client as wh,ifnull(a.acno,"") as asset,ifnull(r.salesreturn,"") as revenue,item.expense,stock.isamt,stock.cost,
    ...
    stock.isamt,stock.cost,stock.amt,stock.disc,stock.rrqty,stock.qty,stock.fcost,head.projectid,item.amt9
          from ' . $this->head . ' as head left join ' . $this->stock . ' as stock on stock.trno=head.trno
          left join client as wh on wh.clientid=stock.whid

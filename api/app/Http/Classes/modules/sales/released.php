...
class released
{
    ...
    public function createdoclisting($config)
    {
        ...
        $getcols = ['action', 'statname', 'listdocument', 'listdate', 'listclientname', 'yourref', 'ourref', 'total', 'ar', 'listpostedby', 'listcreateby', 'listeditby', 'listviewby', 'receiveby', 'receivedate'];
    ...
    public function loaddoclisting($config)
    {
        ...
        head.createby,head.editby,head.viewby,num.postedby,ifnull(stat.status,'DRAFT') as statname,
        head.yourref, head.ourref $lfield
        from " . $this->head . " as head left join " . $this->tablenum . " as num
        on num.trno=head.trno left join trxstatus as stat on stat.line=head.statid
        $ljoin
        " . $join . "
        where num.statid=41 and head.doc=? and num.center = ? and CONVERT(head.dateid,DATE)>=? and CONVERT(head.dateid,DATE)<=? " . $condition . $addparams . " $filtersearch
    ...
    public function createTab($access, $config)
    {
        ...
        if ($companyid == 15 || $companyid == 17 || $companyid == 20) { //nathina, unihome, proline
            if ($release) {
    ...
    public function createHeadField($config)
    {
        ...
        $fields = ['rem', 'creditinfo', ['updatepostedinfo', 'tagreleased']];
        if ($this->companysetup->getistodo($config['params'])) {
    ...
    public function loadheaddata($config)
    {
        ...
        and head.lockdate is not null and num.postdate is null and num.statid=41 ";
    ...
    public function stockstatusposted($config)
    {
        ...
        switch ($action) {
            ...
            case 'downloadexcel':
                ...
                break;
            case 'tagreleased':
                return $this->tagreleased($config);
                break;
            default:
    ...
    private function tagreleased($config)
    {
        $trno = $config['params']['trno'];
        $islocked = $this->othersClass->islocked($config);
        $weightout = $this->coreFunctions->getfieldvalue('cntnuminfo', 'weightout', 'trno=?', [$trno]);
        $weightouttime = $this->coreFunctions->getfieldvalue('cntnuminfo', 'weightouttime', 'trno=?', [$trno]);
        $weightin = $this->coreFunctions->getfieldvalue('cntnuminfo', 'weightin', 'trno=?', [$trno]);
        $weightintime = $this->coreFunctions->getfieldvalue('cntnuminfo', 'weightintime', 'trno=?', [$trno]);

        if ($weightout == 0 || empty($weightouttime) || $weightin == 0 || empty($weightintime)) {
            return ['status' => false, 'msg' => 'Weight In/Out And Weight In/Out Time Must Not Be Empty'];
        } else {
            if ($islocked) {

                // 2023.09.20 - instruct to remove

                $post = $this->posttrans($config);
                if ($post['status']) {
                    TagReleaseHere:
                    $this->coreFunctions->sbcupdate($this->tablenum, ['statid' => 42], ['trno' => $trno]);
                    $stat = $this->coreFunctions->getfieldvalue('cntnum', 'statid', 'trno=?', [$trno]);
                    $this->coreFunctions->sbcupdate('hcntnuminfo', [
                        'releasedate' => $this->othersClass->getCurrentTimeStamp(),
                        'editdate' => $this->othersClass->getCurrentTimeStamp(),
                        'editby' =>  $config['params']['user']
                    ], ['trno' => $trno]);
                    $this->logger->sbcwritelog($trno, $config, 'HEAD', 'Released');
                } else {
                    return ['status' => false, 'msg' => $post['msg']];
                }


                return ['status' => true, 'msg' => 'Successfully released', 'backlisting' => true];
            } else {

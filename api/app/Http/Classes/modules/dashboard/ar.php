    ...
    switch ($classid) {
      ...
      case 'unposted':
        $qry = "select trno,  doc, docno, date_format(dateid,'%m/%d/%y') as dateid, 
            ...
            0 as `db`,ifnull(detail.ext,0) as `cr`,ifnull(detail.ext,0) as `bal`,
            `client`.`clientid` as `clientid`,'' as `ref`,'' as `agent`,`detail`.`rem` as `rem`,
            detail.ext as `balance`,0 as `fbal`,'' as `reference`,'UNPOSTED' as `status`
            from `lahead` as head
            ...
            ) as t group by trno,line,doc,docno,dateid,ref,rem,status order by dateid desc,docno";
        break;
    }

...
class aj
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'AJ TRANSACTION';
  ...
  public function createTab($config)
  {
    $tab = [
      $this->gridname => [
        'gridcolumns' => ['action', 'status', 'dateid', 'docno', 'clientname', 'bal', 'rem']
      ]
    ];
    ...
    $obj[0][$this->gridname]['columns'][4]['label'] = 'Warehouse';
  ...
  public function paramsdata($config)
  {
    ...
    switch ($classid) {
      case 'posted':
        $this->modulename = 'POSTED - AJ TRANSACTION';
        break;
      case 'unposted':
        $this->modulename = 'UNPOSTED - AJ TRANSACTION';
        break;
    }
    return $this->coreFunctions->opentable("select left(now(),10) as dateid,'' as clientname, 0.0 as ext,? as classid, '" . $doc . "' as doc ", [$classid]);
  }
    ...
    switch ($classid) {
      case 'posted':
        $qry = "select trno,  doc, docno, date_format(dateid,'%m/%d/%y') as dateid, 'DBTODO' as tabtype, '$url' as url,
              ...
              format(sum(ext),2) as bal,clientname,
              rem, status from (select `cntnum`.`doc` as `doc`,glhead.`docno`,`glhead`.`trno` as `trno`,
              `glhead`.`dateid` as `dateid`,stock.ext,
              (`glhead`.`rem`) as `rem`,client.clientname,
              `glhead`.`ourref` as `reference`,'POSTED' as `status` from `glhead`
              left join `cntnum` on `cntnum`.`trno` = `glhead`.`trno` 
              left join glstock as stock on stock.trno = glhead.trno
              left join client on client.clientid = glhead.whid where cntnum.doc='AJ' and  glhead.dateid='$date'
              and cntnum.center = '$center'
              ) as t  group by trno,  doc, docno,dateid,rem,status,clientname order by dateid desc,docno";
        break;
      case 'unposted':
        $qry = "select trno,  doc, docno, date_format(dateid,'%m/%d/%y') as dateid, 'DBTODO' as tabtype, '$url' as url,
          'module' as moduletype,clientname,
          format(sum(ext),2) as bal,
           rem, status from (select `cntnum`.`doc` as `doc`,lahead.`docno`,`lahead`.`trno` as `trno`,
          `lahead`.`dateid` as `dateid`,stock.ext,
          (`lahead`.`rem`) as `rem`,client.clientname,
          ...
          left join client on client.client = lahead.wh where cntnum.doc='AJ' and  lahead.dateid ='$date'
          and cntnum.center = '$center'
          ) as t  group by trno,  doc, docno,dateid,rem,status,clientname order by dateid desc,docno";
        break;
    }

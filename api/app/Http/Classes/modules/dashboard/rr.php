...
class rr
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'RR TRANSACTION';
  ...
  public function createTab($config)
  {
    $tab = [
      $this->gridname => [
        'gridcolumns' => ['action', 'status', 'dateid', 'docno', 'db', 'cr', 'bal', 'ref', 'rem']
      ]
    ];
    ...
    switch ($classid) {
      case 'posted':
        $qry = "select trno, line, doc, docno, date_format(dateid,'%m/%d/%y') as dateid, 'DBTODO' as tabtype, '$url' as url,
            ...
            left join `glhead` as head on((`head`.`trno` = `cntnum`.`trno`)))) where cntnum.doc='RR' and apledger.bal>0 and $filter

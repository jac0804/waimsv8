            ...
            if (!$status) {
                    ...
                    if (count($approversetup) > 1) {
                        ...
                        if (count($approversetup) == 1) {
                            ...
                            if ($bothapprover) {
                                ...
                                $data['approvedby_disapprovedby2'] = $config['params']['user'];
                            }
                            break;
                        }
                    }
                ...
                if ($update) {
                    ...
                    if ($companyid == 53 || $companyid == 51) { // camera|ulitc
                        ...
                        $filter = "emp.issupervisor = 1 and emp.empid = " . $leave[0]->supervisorid . "";
                        if ($companyid == 51) { //ulitc
                            $filter = "emp.isapprover = 1"; //to hr-manager
                        }

                        $qry = "select emp.isapprover,emp.email,cl.email as username,cl.clientname as appname from employee as emp
                                          left join client as cl on cl.clientid = emp.empid where $filter";
                    ...
                    return ['status' => true, 'msg' => 'Successfully ' . $label . ' ', 'data' => [], 'reloadsbclist' => true, 'action' => 'leaveapplication'];
                }
            } else {

...
class co
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'CONSIGNMENT DR';
  ...
  private $fields = ['trno', 'docno', 'dateid', 'due', 'client', 'clientname', 'yourref', 'ourref', 'rem', 'wh', 'projectid'];
  ...
  public function createTab($access, $config)
  {
    ...
    $column =  ['action', 'isqty', 'uom', 'isamt', 'disc', 'ext', 'wh', 'ref', 'loc', 'expiry', 'loc2', 'rem', 'location', 'pallet', 'location2', 'pallet2', 'itemname'];
    $sortcolumn =  ['action', 'isqty', 'uom', 'isamt', 'disc', 'ext', 'wh', 'ref', 'loc', 'expiry', 'loc2', 'rem', 'location', 'pallet', 'location2', 'pallet2', 'itemname'];
  ...
  {
    $tbuttons = ['pendingcn', 'additem', 'quickadd', 'saveitem', 'deleteallitem'];
  ...
  public function stockstatus($config)
  {
    switch ($config['params']['action']) {
      ...
      case 'getsplitqtydetails':
        ...
        break;
      case 'getcnsummary':
        return $this->getcnsummary($config);
        break;
      case 'getcndetails':
        return $this->getcndetails($config);
        break;
      default:
  ...
  public function setserveditems($refx, $linex)
  {
    ...
    stock on stock.trno=head.trno where head.doc='CO' and stock.refx=" . $refx . " and stock.linex=" . $linex;

    $qry1 = $qry1 . " union all select glstock." . $this->hqty . " from glhead left join glstock on glstock.trno=
    glhead.trno where glhead.doc='CO' and glstock.refx=" . $refx . " and glstock.linex=" . $linex;
    ...
    return $this->coreFunctions->execqry("update hcnstock set qa=" . $qty . " where trno=" . $refx . " and line=" . $linex, 'update');
  }
  ...
  public function getlatestprice($config)
  {
    ...
    $qry = "select docno,left(dateid,10) as dateid,round(amt,2) as amt,'' as disc,uom from(
          ...
          where head.doc in ('RR','IS','CM','AJ','CO') and cntnum.center = ?
          and item.barcode = ? 
  ...
  public function getcnsummary($config)
  {
    $trno = $config['params']['trno'];
    $wh = $config['params']['wh'];
    $rows = [];
    ...
    foreach ($config['params']['rows'] as $key => $value) {
      $qry = "
        ...
        FROM hcnhead as head left join hcnstock as stock on stock.trno=head.trno left join item on item.itemid=
  ...
  public function getcndetails($config)
  {
    $trno = $config['params']['trno'];
    $wh = $config['params']['wh'];
    $rows = [];
  ...
  private function report_default_query($trno)
  {
    ...
    $query = "select head.vattype, head.tax, stock.rem as remarks, client.tel, wh.tel as wtel, date(head.dateid) as dateid, head.docno, client.client, client.clientname, head.address, head.terms,
      ...
      left join item on item.itemid=stock.itemid
      where head.doc='CO' and stock.tstrno=0 and md5(head.trno)='" . md5($trno) . "'
      union all

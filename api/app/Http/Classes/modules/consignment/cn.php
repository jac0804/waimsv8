...
class cn
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'CONSIGNMENT REQUEST';
  ...
  public $head = 'cnhead';
  public $hhead = 'hcnhead';
  public $stock = 'cnstock';
  public $hstock = 'hcnstock';
  ...
  public $damt = 'isamt';
  public $hamt = 'amt';
  public $fields = ['trno', 'docno', 'dateid', 'due', 'client', 'clientname', 'yourref', 'ourref', 'rem', 'terms', 'forex', 'cur', 'wh', 'address', 'agent', 'creditinfo', 'projectid'];
  ...
  public function createTab($access, $config)
  {
    ...
    $column = ['action', 'isqty', 'uom', 'isamt', 'disc', 'ext', 'wh', 'rem', 'loc', 'qa', 'void', 'ref', 'itemname'];
    $sortcolumn = ['action', 'isqty', 'uom', 'isamt', 'disc', 'ext', 'wh', 'rem', 'loc', 'qa', 'void', 'ref', 'itemname'];
  ...
  public function createHeadField($config)
  {
    ...
    $col1 = $this->fieldClass->create($fields);
    data_set($col1, 'client.lookupclass', 'lookupconsign');
  ...
  public function posttrans($config)
  {
      ...
      if ($override == '0') {
        if (floatval($overdue) <> 0) {
          if (floatval($crline) < floatval($totalso) || $cstatus <> 'ACTIVE') {
            ...
            return ['status' => false, 'msg' => 'Posting failed. SO was disapproved and cannot be posted.'];
          }
        }
      }
    }
    ...
    $qry = "insert into " . $this->hhead . "(trno,doc,docno,client,clientname,address,shipto,dateid,
      terms,rem,forex,yourref,ourref,createdate,createby,editby,editdate,lockdate,lockuser,agent,wh,due,cur,creditinfo,crline,overdue, projectid)
    ...
    if ($posthead) {
      ...
      $qry = "insert into " . $this->hstock . "(trno,line,itemid,uom,
        ...
        encodeddate,encodedby,editdate,editby,refx,linex,ref)
        SELECT trno, line, itemid, uom,whid,loc,expiry,disc, iss,void,isamt,amt, isqty, ext,
        encodeddate, encodedby,editdate,editby,refx,linex,ref FROM " . $this->stock . " where trno =?";
  ...
  yourref,ourref,createdate,createby,editby,editdate,lockdate,lockuser,wh,due,cur,creditinfo,crline,overdue,agent, projectid)
  ...
  where head.trno=? limit 1";
    ...
    if ($this->coreFunctions->execqry($qry, 'insert', [$trno])) {
      ...
      amt,iss,void,isamt,isqty,ext,rem,encodeddate,encodedby,editdate,editby,refx,linex,ref)
      select trno, line, itemid, uom,whid,loc,expiry,disc,amt, iss,void, isamt, isqty,
      ext,ifnull(rem,''), encodeddate,encodedby, editdate, editby,refx,linex,ref
  ...
  public function diagram($config)
  {
    ...
    $qry = "select so.trno,so.docno,left(so.dateid,10) as dateid,
     ...
     from hcnhead as so 
     left join hcnstock as s on s.trno = so.trno
     where so.trno = ? 
  ...
  public function setserveditems($refx, $linex)
  {
    ...
    stock on stock.trno=head.trno where head.doc='CN' and stock.refx=" . $refx . " and stock.linex=" . $linex;

    $qry1 = $qry1 . " union all select stock." . $this->hqty . " from " . $this->hhead . " as head left join " . $this->hstock . " as stock on stock.trno=
    head.trno where head.doc='cn' and stock.refx=" . $refx . " and stock.linex=" . $linex;
  ...
  private function report_default_query($trno)
  {
    ...
    $query = "select head.rtype,head.rdate,cust.tel2,cust.email,head.docno,head.trno, head.clientname, head.address, 
      ...
      from cnhead as head left join cnstock as stock on stock.trno=head.trno 
      left join item on item.itemid=stock.itemid
      ...
      left join hcnstock as stock on stock.trno=head.trno
      left join item on item.itemid=stock.itemid 
      left join model_masterfile as m on m.model_id = item.model
      left join client on client.client=head.wh
      left join client as cust on cust.client = head.client
      where head.doc='cn' and head.trno='$trno' order by line";
  ...
  public function default_header($params, $data)
  {
    ...
    $str .= $this->reporter->col('CONSIGNMENT REQUEST', '600', null, false, $border, '', 'L', $font, '18', 'B', '', '');

...
class ratecodesetup
{
  ...
  private $table = 'hmsratesetup';
  private $othersClass;
  public $style = 'width:100%;';
  public $tablelogs = 'masterfile_log';
  public $tablelogs_del = 'del_masterfile_log';
  private $fields = ['code', 'description', 'isinactive'];
  ...
  public function createTab($config)
  {
    $tab = [
      $this->gridname => [
        'gridcolumns' => ['action', 'code', 'description', 'isinactive']
      ]
    ];
  ...
  {
    $qry = "line, code, description, (case when isinactive=0 then 'false' else 'true' end) as isinactive";
    return $qry;
  }
  ...
  private function report_default_query($config)
  {
    ...
    $query = "
      select code, description, (case when isinactive=0 then 'false' else 'true' end) as isinactive from hmsratesetup
    ";
  ...
  private function default_ratecode_PDF($params, $data)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      PDF::MultiCell(360, $max_height, $data[$i]['description'], '', 'L', 0, 1, '', '');
      if (intVal($i) + 1 == $page) {
        $this->default_ratecode_header_PDF($params, $data);
        $page += $count;
      }

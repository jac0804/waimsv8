...
class othercharges
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'OTHER CHARGES SETUP';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'hmscharges';
  private $othersClass;
  public $style = 'width:100%;';
  public $tablelogs = 'masterfile_log';
  public $tablelogs_del = 'del_masterfile_log';
  private $fields = ['description', 'amt', 'isinactive'];
  ...
  public function createTab($config)
  {
    $tab = [
      $this->gridname => [
        'gridcolumns' => ['action', 'description', 'amt', 'isinactive']
      ]
    ];
  ...
  {
    $qry = "line, description, (case when isinactive=0 then 'false' else 'true' end) as isinactive, amt";
    return $qry;
  }
  ...
  private function report_default_query($config)
  {
    ...
    $query = "
      select line, description, (case when isinactive=0 then 'false' else 'true' end) as isinactive, amt from hmscharges
    ";
  ...
  private function rpt_ratecode_LAYOUT($filters, $data)
  {
    ...
    foreach ($data as $key => $value) {
      $str .= $this->reporter->startrow();
      $str .= $this->reporter->col($value['description'], '50px', null, false, '1px solid ', '', 'C', 'Century Gothic', '11', '', '', '2px');
      $str .= $this->reporter->col($value['amt'], '50px', null, false, '1px solid ', '', 'C', 'Century Gothic', '11', '', '', '2px');
  ...
  private function default_ratecode_header_PDF($params, $data)
  {
    ...
    PDF::MultiCell(360, 0, "AMOUT", 'B', 'L', false, 1);
    PDF::MultiCell(0, 0, "\n");
  }
  ...
  private function default_ratecode_PDF($params, $data)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      $section_height = PDF::GetStringHeight(360, $data[$i]['description']);
      $section_desc_height = PDF::GetStringHeight(360, $data[$i]['amt']);
      ...
      PDF::MultiCell(360, $max_height, $data[$i]['amt'], '', 'L', 0, 1, '', '');
      if (intVal($i) + 1 == $page) {
        $this->default_ratecode_header_PDF($params, $data);
        $page += $count;
      }

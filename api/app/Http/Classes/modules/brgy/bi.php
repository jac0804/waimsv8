...
class bi
{
    ...
    private $fieldClass;
    private $tabClass;
    public $modulename = 'INFRASTRUCTURE CLEARANCE';
    ...
    public function createdoclisting($config)
    {
        ...
        $cols[$listdocument]['label'] = 'Doc No.';
    ...
    public function createHeadField($config)
    {
        ...
        data_set($col1, 'client.label', 'Infrastructure');
        ...
        $fields = ['dateid', 'amount']; // last move
        ...
        data_set($col3, 'picture.fieldid', 'trno');
        data_set($col3, 'picture.folder', 'bi');
        data_set($col3, 'picture.table', 'la_picture');
        ...
        if (!$this->createdistribution($config)) {
            ...
            if ($msg == '') {
                $qry = "insert into " . $this->hhead . "(
                    trno,docno,doc,dateid,clientid,clientname,address,amount)
                    select 
                    head.trno,head.docno,head.doc,head.dateid,client.clientid,head.clientname,head.address,head.amount
    ...
    public function unposttrans($config)
    {
        ...
        if ($msg == '') {
            ...
            $qry = "insert into " . $this->head . "(
                trno,docno,doc,dateid,client,clientname,address,amount)
                select 
                head.trno,head.docno,head.doc,head.dateid,client.client,head.clientname,head.address,head.amount

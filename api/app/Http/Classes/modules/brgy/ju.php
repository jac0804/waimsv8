...
class ju
{
    ...
    private $fieldClass;
    private $tabClass;
    public $modulename = 'COMPLAINTS FILE';
    ...
    public function createHeadField($config)
    {
        ...
        $fields = [['dateid', 'bstype'], 'ownername', 'owneraddr', 'orderno'];
        $col2 = $this->fieldClass->create($fields);

        data_set($col2, 'bstype.label', 'Time');
    ...
    public function createTab($config)
    {
        ...
        $tab['tableentry2'] = ['action' => 'tableentry', 'lookupclass' => 'casehistory', 'label' => 'CASE HISTORY'];
    ...
    public function createdoclisting($config)
    {
        ...
        $cols[$listdate]['label'] = 'Date Complained';
    ...
    public function reportsetup($config)
    {
        ...
        $printcount = isset($data2) ? $data2 : 0;

        $date = $this->othersClass->getCurrentDate();
        $printcount++;

        $ctnsno = 'SUMMONS ' . $printcount;

        if ($printcount == 1) {
            $bpo = 'SUMMONS / OFFICER\'S RETURN';
        } elseif ($printcount == 2) {
            $bpo = 'NOTICE OF HEARING';
        } elseif ($printcount == 3) {
            $bpo = 'SOMMUNS PRINTOUT';
        }

        $updatehead = $this->coreFunctions->sbcupdate($head, ['petrno' => $printcount], ['trno' => $trno]);
        if ($printcount <= 3) {
            if ($updatehead) {
                $datahere = [
                    'category' =>  $ctnsno,
                ...
                $req = $this->coreFunctions->insertGetId('reqcategory', $datahere);
            }
        }

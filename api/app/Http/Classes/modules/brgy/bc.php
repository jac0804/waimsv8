...
class bc
{
    ...
    public function createHeadField($config)
    {
        ...
        $fields = ['contact',  'ownertype', 'ownername', 'owneraddr', 'plateno', 'dateid'];
        ...
        data_set($col2, 'plateno.class', 'sbccsreadonly');
    ...
    public function createdoclisting($config)
    {
        ...
        $cols[$listdocument]['style'] = 'width:125px;whiteSpace: normal;min-width:125px;';
    ...
    public function loadheaddata($config)
    {
        ...
        ifnull(head.owneraddr,'') as owneraddr,ifnull(head.crno,'') as crno,ifnull(head.checker,'') as checker,cp.picture,ifnull(head.trnxtype,'') as trnxtype,cl.plateno
        from lahead as head
        left join cntnum as num on num.trno = head.trno
        left join client as cl on cl.client=head.client
        ...
        if (!$this->createdistribution($config)) {
            ...
            if ($msg == '') {
                $qry = "insert into " . $this->hhead . "(
                    trno,doc,docno,dateid,clientid,clientname,address,yourref,ourref,due,amount,purposeid,rem,isfee,bstype,contact,ownertype,ownername,owneraddr,trnxtype)
                    select 
                    head.trno,head.doc,head.docno,head.dateid,client.clientid,head.clientname,
                    head.address,head.yourref,head.ourref,head.due,head.amount,head.purposeid,head.rem,head.isfee,head.bstype,head.contact,
                    head.ownertype,head.ownername,head.owneraddr,head.trnxtype
    ...
    public function unposttrans($config)
    {
        ...
        if ($msg == '') {
            ...
            $qry = "insert into " . $this->head . "(
                trno,doc,docno,dateid,client,clientname,address,yourref,ourref,due,amount,purposeid,rem,isfee,bstype,contact,ownertype,ownername,owneraddr,trnxtype)
                select 

...
class bt
{
    ...
    public function createHeadField($config)
    {
        ...
        $fields = ['dateid', 'tldocno', 'bonafide', 'trutype', 'amount'];
        ...
        data_set($col3, 'plateno.class', 'csplateno sbccsreadonly');
        data_set($col3, 'plateno.readonly', true);

        data_set($col3, 'sidecarno.class', 'cssidecarno sbccsreadonly');
        data_set($col3, 'sidecarno.readonly', true);
    ...
    public function loadheaddata($config)
    {
        ...
        tru.description as trutype,bona.description as bonafide,
        cl.plateno,cl.make,cl.motorno,cl.color,info.chassisno,info.sidecarno,
        format(head.amount,2) as amount,cl.picture
        ...
        if (!$this->createdistribution($config)) {
            ...
            if ($msg == '') {
                $qry = "insert into " . $this->hhead . "(
                    trno,doc,docno,dateid,clientid,clientname,address,amount,truid,bonafideid,createdate,createby,editby,editdate,lockdate,lockuser,viewby,viewdate)
                    select 
                    head.trno,head.doc,head.docno,head.dateid,client.clientid,head.clientname,head.address,head.amount,head.truid,head.bonafideid,head.createdate,head.createby,head.editby,head.editdate,head.lockdate,head.lockuser,head.viewby,head.viewdate
    ...
    public function unposttrans($config)
    {
        ...
        if ($msg == '') {
            ...
            $qry = "insert into " . $this->head . "(
                trno,doc,docno,dateid,client,clientname,address,amount,truid,bonafideid,createdate,createby,editby,editdate,lockdate,lockuser,viewby,viewdate)
                select 
                head.trno,head.doc,head.docno,head.dateid,client.client,head.clientname,
                head.address,head.amount,head.truid,head.bonafideid,head.createdate,head.createby,head.editby,head.editdate,head.lockdate,head.lockuser,head.viewby,head.viewdate

...
class wr
{
    ...
    private $fieldClass;
    private $tabClass;
    public $modulename = 'WORKING CLEARANCE';
    ...
    public function createHeadField($config)
    {
        ...
        data_set($col1, 'client.label', 'Working ID');
        data_set($col1, 'client.action', 'lookupworkingledger');
        ...
        $fields = ['dateid', 'bday', 'sex', 'civilstatus'];
        ...
        $fields = ['employer', 'companyaddress', 'position', 'amount'];
        $col3 = $this->fieldClass->create($fields);
        data_set($col3, 'amount.label', 'Amount');
        data_set($col3, 'employer.class', 'csemployer sbccsreadonly');
        data_set($col3, 'companyaddress.class', 'cscompanyaddress sbccsreadonly');
    ...
    public function loadheaddata($config)
    {
        ...
        $query = "
        select head.trno,head.docno,head.client,head.clientname, head.address, head.dateid,format(head.amount,2) as amount,
        date_format(cl.bday,'%Y-%m-%d') as bday,   ifnull(info.civilstatus,'') as civilstatus,cl.sex,
        ifnull(info.employer,'') as employer,ifnull(info.companyaddress,'') as companyaddress,ifnull(cl.position,'') as position,cl.picture
        from lahead as head
        left join cntnum as num on num.trno = head.trno
        left join client as cl on cl.client=head.client
        ...
        if (!$this->createdistribution($config)) {
            ...
            if ($msg == '') {
                $qry = "insert into " . $this->hhead . "(
                    trno,doc,docno,dateid,clientid,clientname,address,amount,createdate,createby,editby,editdate,lockdate,lockuser,viewby,viewdate)
                    select 
                    head.trno,head.doc,head.docno,head.dateid,client.clientid,head.clientname,head.address,head.amount,head.createdate,head.createby,head.editby,head.editdate,head.lockdate,head.lockuser,head.viewby,head.viewdate
    ...
    public function unposttrans($config)
    {
        ...
        if ($msg == '') {
            ...
            $qry = "insert into " . $this->head . "(
                trno,doc,docno,dateid,client,clientname,address,amount,createdate,createby,editby,editdate,lockdate,lockuser,viewby,viewdate)
                select 
                head.trno,head.doc,head.docno,head.dateid,client.client,head.clientname,
                head.address,head.amount,head.createdate,head.createby,head.editby,head.editdate,head.lockdate,head.lockuser,head.viewby,head.viewdate

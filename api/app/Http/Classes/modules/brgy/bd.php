...
class bd
{
    ...
    public function loaddoclisting($config)
    {
        ...
        head.clientname,
        head.address,
        head.rem as detailof,head.createby, 'DRAFT' as status,format(head.amount,2) as amount,
        '' as ref                                                                                
        from lahead as head
        ...
        head.clientname,
        head.address,
        head.rem as detailof ,head.createby,'POSTED' as status,format(head.amount,2) as amount,
    ...
    {
        $getcols = ['action', 'liststatus', 'listdocument', 'listclientname', 'listdate', 'ref', 'amount'];
        ...
        if (!$this->createdistribution($config)) {
            ...
            if ($msg == '') {
                $qry = "insert into " . $this->hhead . "(
                    trno,doc,docno,dateid,clientid,clientname,address,yourref,ourref,due,amount,purposeid,rem,isfee)
                    select 
    ...
    public function unposttrans($config)
    {
        ...
        if ($msg == '') {
            ...
            $qry = "insert into " . $this->head . "(
                trno,doc,docno,dateid,client,clientname,address,yourref,ourref,due,amount,purposeid,rem,isfee)
                select 

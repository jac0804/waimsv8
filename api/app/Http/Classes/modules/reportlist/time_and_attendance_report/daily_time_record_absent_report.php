...
class daily_time_record_absent_report
{
    public $modulename = 'Daily Time Record Absent Report';
    ...
    public function defualt_query($config)
    {
        ...
        dateid,daytype,empid,detpname,jobtitle,reason";
    ...
    public function getcountdata($config, $empid)
    {
        ...
        left join employee as emp on emp.empid=timecard.empid where dateid between '" . $start . "' and '" . $end . "' and emp.empid =? and timecard.absdays <> 0 and emp.level in $emplvl  ";
    ...
    public function displayHeader($config, $layoutsize)
    {
        ...
        $str .= $this->reporter->col('DAILY TIME RECORD - ABSENT REPORTS', null, null, false, $border, '', 'C', $font, '14', 'B', '', '') . '</br>';
    ...
    public function report_default_detailed($config, $data)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if ($client == "" || $client != $data[$key]->empname) {
                ...
                $str .= $this->reporter->col('Abs', '100', null, false, $border, 'B', 'C', $font, $font_size, 'B', '', '');
                $str .= $this->reporter->col('Day Type', '100', null, false, $border, 'B', 'C', $font, $font_size, 'B', '', '');
                $str .= $this->reporter->col('Reason', '200', null, false, $border, 'B', 'C', $font, $font_size, 'B', '', '');
                $str .= $this->reporter->endrow();
                $totalabs = 0;
                $nodays = 0;
                $clno++;
            ...
            $str .= $this->reporter->col($data[$key]->absdays, '100', null, false, $border, '', 'C', $font, $font_size, '', '', '');
            $str .= $this->reporter->col($data[$key]->daytype, '100', null, false, $border, '', 'C', $font, $font_size, '', '', '');
            $str .= $this->reporter->col($data[$key]->reason, '200', null, false, $border, '', 'C', $font, $font_size, '', '', '');
            $str .= $this->reporter->endrow();
            $totalabs += $data[$key]->absdays;
            $nodays++;
            ...
            if ($nodays == $count) {
                ...
                $str .= $this->reporter->endrow();
                $str .= $this->reporter->startrow();
                $str .= $this->reporter->col('No. of days: ' . $nodays, '100', null, false, $border, 'T', 'L', $font, $font_size, 'B', '', '');
                $str .= $this->reporter->col('', '100', null, false, $border, 'T', 'C', $font, $font_size, 'B', '', '');
                $str .= $this->reporter->col('', '100', null, false, $border, 'T', 'C', $font, $font_size, 'B', '', '');
                $str .= $this->reporter->col('', '100', null, false, $border, 'T', 'C', $font, $font_size, 'B', '', '');
                $str .= $this->reporter->col('', '100', null, false, $border, 'T', 'C', $font, $font_size, 'B', '', '');
                $str .= $this->reporter->col('Total: ', '100', null, false, $border, 'T', 'C', $font, $font_size, 'B', '', '');
                $str .= $this->reporter->col(number_format($totalabs, 2), '100', null, false, $border, 'T', 'C', $font, $font_size, 'B', '', '');

                $totaldays = $totalabs / 8;

                $str .= $this->reporter->col('(' . $totaldays . ' days)', '100', null, false, $border, 'T', 'C', $font, $font_size, 'B', '', '');
                $str .= $this->reporter->col('', '200', null, false, $border, 'T', 'C', $font, $font_size, 'B', '', '');
                $str .= $this->reporter->endrow();
            }
        }
    ...
    public function report_default_summary($config, $data)
    {
        ...
        $str .= $this->reporter->col('Total Absent (hrs)', '100', null, false, $border, 'B', 'C', $font, $font_size, 'B', '', '');
        $str .= $this->reporter->col('Total Absent (days)', '100', null, false, $border, 'B', 'C', $font, $font_size, 'B', '', '');
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();
        foreach ($data as $key => $value) {
            $str .= $this->reporter->begintable(1000);
            ...
            $str .= $this->reporter->col($data[$key]->absdays, '100', null, false, '1px dotted', 'B', 'C', $font, $font_size, '', '', '');

            $totaldays = $data[$key]->absdays / 8;

            $str .= $this->reporter->col($totaldays, '100', null, false, '1px dotted', 'B', 'C', $font, $font_size, '', '', '');
            $str .= $this->reporter->endrow();
            $str .= $this->reporter->endtable();
            $str .= $this->reporter->endtable();
        }

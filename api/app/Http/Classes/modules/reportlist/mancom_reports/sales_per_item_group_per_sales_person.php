...
class sales_per_item_group_per_sales_person
{
  ...
  public function default_QUERY($config)
  {
    ...
    $query = "
        ...
        from (
          select sum(hqsstock.ext*(case $forex when 0 then hqsstock.sgdrate else $forex end)) as ext, ag.clientname as sperson,prj.code as igroup, 
          ...
          group by ag.clientname,prj.code,sgq.amt,hqsstock.sgdrate
          union all
          select sum(hqtstock.ext*(case $forex when 0 then hqtstock.sgdrate else $forex end)) as ext, ag.clientname as sperson,prj.code as igroup, 
                  sgq.amt as target, timestampdiff(month, '$start', '$end' )+1 as m
          ...
          left join salesgroupqouta as sgq on sgq.projectid = prj.line and sgq.agentid = ag.clientid and year(sohead.pdate) = sgq.yr
           where  sohead.pdate between '$start' and '$end' 
          group by ag.clientname,prj.code,sgq.amt,hqtstock.sgdrate
        ) as a
  ...
  public function reportDefaultLayout_SUMMARIZED($config)
  {
    ...
    foreach ($cols as $key => $colx) {
      $str .= $this->reporter->col('', '85', null, false, $border, 'TL', 'C', $font, '14', 'B', '', '4px');
      $str .= $this->reporter->col($colx->sperson, '130', null, false, $border, 'T', 'C', $font, '12', 'B', '', '4px');
      $str .= $this->reporter->col('', '85', null, false, $border, 'TR', 'C', $font, '14', 'B', '', '4px');
      $agents[$colx->sperson] = 0;
      $agents2[$colx->sperson] = 0;
    }
    $str .= $this->reporter->endrow();
    // next col
    ...
    foreach ($cols as $key => $colx) {
      if ($key == 0) {
        $str .= $this->reporter->col('', '85', null, false, $border, 'TL', 'C', $font, '14', 'B', '', '4px');
        $str .= $this->reporter->col('Item Group', '130', null, false, $border, 'T', 'C', $font, '12', 'B', '', '4px');
        $str .= $this->reporter->col('', '85', null, false, $border, 'TR', 'C', $font, '14', 'B', '', '4px');
      } elseif (count($cols) - 1 == $key) {
        $str .= $this->reporter->col('', '85', null, false, $border, 'TL', 'C', $font, '14', 'B', '', '4px');
        $str .= $this->reporter->col('Total Target', '130', null, false, $border, 'T', 'C', $font, '12', 'B', '', '4px');
        $str .= $this->reporter->col('', '85', null, false, $border, 'TR', 'C', $font, '14', 'B', '', '4px');
      } else {
        $str .= $this->reporter->col('Target', '85', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '4px');
        $str .= $this->reporter->col('Actual Sales', '130', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '4px');
        $str .= $this->reporter->col('%', '85', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '4px');
      }
    }
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();
    ...
    foreach ($result as $keyx => $value) {
      ...
      if ($igroup != $value->igroup) {
        $str .= $this->reporter->begintable($layoutsize);
        $str .= $this->reporter->startrow($layoutsize);
        $str .= $this->reporter->col($value->igroup, '300', null, false, $border, 'TLB', 'C', $font, '12', 'B', '', '4px');
        ...
        foreach ($cols as $key => $colx) {
          if ($sperson != $value->sperson || $igroup != $value->igroup) {
            ...
            if (!empty($colx)) {
              $str .= $this->reporter->col(number_format($colx[0]->target, 2), '85', null, false, $border, 'TLRB', 'C', $font, '12', '', '', '4px');
              $str .= $this->reporter->col(($colx[0]->totaldollar != 0 ? number_format($colx[0]->totaldollar, 2) : '-'), '130', null, false, $border, 'TLRB', 'C', $font, '12', '', '', '4px');
              if ($colx[0]->totaldollar == 0 || $colx[0]->target == 0) {
                $str .= $this->reporter->col('-', '85', null, false, $border, 'TLRB', 'C', $font, '12', '', '', '');
              } else {
                $str .= $this->reporter->col(number_format(($colx[0]->totaldollar / $colx[0]->target) * 100, 2), '85', null, false, $border, 'TLRB', 'C', $font, '12', '', '', '4px');
              }
              $totaltarget += $colx[0]->target;
            } else {
              if (count($cols) == $key) {
                $str .= $this->reporter->col('', '85', null, false, $border, 'TLB', 'C', $font, '12', 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totaltarget, 2), '130', null, false, $border, 'TB', 'C', $font, '12', 'B', '', '4px');
                $str .= $this->reporter->col('', '85', null, false, $border, 'TRB', 'C', $font, '12', 'B', '', '4px');
                $totaltarget = 0;
              } else {
                $str .= $this->reporter->col('-', '85', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '4px');
                $str .= $this->reporter->col('-', '130', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '4px');
                $str .= $this->reporter->col('-', '85', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '4px');
              }
            }
          }
      ...
      $agents2[$value->sperson] += $value->totaldollar;
    }
    $str .= $this->reporter->begintable($layoutsize);
    $str .= $this->reporter->startrow($layoutsize);

    $finaltotal = $finaltotalx = 0;
    foreach ($agents as $keyx => $totalcol) {
      if ($keyx == 'YEAR TO DATE SALES PERFORMANCE') {
        ...
        $str .= $this->reporter->col('', '85', null, false, $border, 'TLB', 'C', $font, '12', 'B', '', '4px');
        $str .= $this->reporter->col(number_format($finaltotal, 2), '130', null, false, $border, 'TB', 'C', $font, '12', 'B', '', '4px','', 0, '', 1);
        $str .= $this->reporter->col('', '85', null, false, $border, 'TRB', 'C', $font, '12', 'B', '', '4px');
      } else {
        $str .= $this->reporter->col(number_format($totalcol, 2), '85', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '4px','', 0, '', 1);
        $str .= $this->reporter->col(number_format($agents2[$keyx], 2), '130', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '4px','', 0, '', 1);
        $str .= $this->reporter->col('', '85', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '4px');
        $finaltotal += $totalcol;
        $finaltotalx += $agents2[$keyx];
      }
    }
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endreport($layoutsize);

...
class sales_per_sales_group_per_person
{
  ...
  public function paramsdata($config)
  {
    ...
    select 
      ...
      0 as salesgroupid,'' as salesgroup
    ");
  }
  ...
  private function sales_group_query($config)
  {
    ...
    $query1 = "
        select ifnull(a.sgroup,'no group') as sgroup,
        sum(a.ext) as ext, sum(a.ext) as totaldollar,timestampdiff(month, '$start', '$end' )+1 as m
        from (
          select sum(hqsstock.ext*(case $forex when 0 then hqsstock.sgdrate else $forex end)) as ext,sg.groupname as sgroup
          from hsqhead as sohead
          left join hqshead as hqshead on hqshead.sotrno = sohead.trno
          left join hqsstock as hqsstock on hqsstock.trno = hqshead.trno
          left join item as item on item.itemid = hqsstock.itemid
          ...
          union all
          select sum(hqtstock.ext*(case $forex when 0 then hqtstock.sgdrate else $forex end)) as ext,sg.groupname as sgroup
          from hqshead as sohead
          left join hqtstock as hqtstock on sohead.trno = hqtstock.trno
          left join item as item on item.itemid = hqtstock.itemid
          left join client as ag on ag.client = sohead.agent
    ...
    $query2 = "
        select ifnull(a.sgroup,'NO BRANCH') as sgroup,
        sum(a.ext) as ext, sum(a.ext) as totaldollar,
        sum(ifnull(target,0)) as target,timestampdiff(month, '$start', '$end' )+1 as m
        from (
          select sum(hqsstock.ext*(case $forex when 0 then hqsstock.sgdrate else $forex end)) as ext,branch.clientname as sgroup,
                  sgq.amt as target
          from hsqhead as sohead
          left join hqshead as hqshead on hqshead.sotrno = sohead.trno
          left join hqsstock as hqsstock on hqsstock.trno = hqshead.trno
          ...
          left join client as ag on ag.client = hqshead.agent
          left join client on client.client=hqshead.client
          left join client  as branch on branch.clientid=hqshead.branch
          left join salesgroup as sg on sg.line=ag.salesgroupid
          left join projectmasterfile as prj on prj.line = hqsstock.projectid
          left join salesgroupqouta as sgq on sgq.projectid = prj.line and sgq.agentid = ag.clientid and year(sohead.pdate) = sgq.yr
          where  hqsstock.void<>1 and date(sohead.dateid) between '$start' and '$end'  $filter
          group by branch.clientname,sgq.amt
          union all
          select sum(hqtstock.ext*(case $forex when 0 then hqtstock.sgdrate else $forex end)) as ext,branch.clientname as sgroup,
                  sgq.amt as target
          from hqshead as sohead
          left join hqtstock as hqtstock on sohead.trno = hqtstock.trno
          left join item as item on item.itemid = hqtstock.itemid
          left join client as ag on ag.client = sohead.agent
          left join client on client.client=sohead.client
          left join client  as branch on branch.clientid=sohead.branch
          left join salesgroup as sg on sg.line=ag.salesgroupid
          left join projectmasterfile as prj on prj.line = hqtstock.projectid
          left join salesgroupqouta as sgq on sgq.projectid = prj.line and sgq.agentid = ag.clientid and year(sohead.pdate) = sgq.yr
           where  hqtstock.void<>1 and date(sohead.due) between '$start' and '$end'  $filter
    ...
    return ['qry1' => $query1, 'qry2' => $query2];
  }
  public function header_DEFAULT($config)
  {
    ...
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->startrow(NULL, null, false, $border, '', 'R', $font, $fontsize, '', '', '', '');
    $str .= $this->reporter->col('Industry: ' . $indus, '300', null, false, $border, '', '', $font, $fontsize, 'B', '', '');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();
    $str .= '<br/>';
    return $str;
  ...
  public function reportDefaultLayout_SUMMARIZED($config)
  {
    ...
    if (date("Y", strtotime($config['params']['dataparams']['start'])) != date("Y", strtotime($config['params']['dataparams']['end']))) {
      return "
      <div style='position:relative;'>
        <div class='text-center' style='position:absolute; top:150px; left:400px;'>
          <div><i class='fas fa-calendar-alt' style='font-size:120px; color: #1E1E1E';></i></div>
          <br>
          <div style='font-size:32px; color:#1E1E1E'>Date Range must be same year</div>
        </div>
      </div>
    ";
    }
    ...
    foreach ($result as $keyx => $value) {
      if ($sgroup != $value->sgroup) {
        ...
        $str .= $this->reporter->begintable($layoutsize);
        $str .= $this->reporter->startrow($layoutsize);
        $str .= $this->reporter->col($value->sgroup, $layoutsize, null, false, $border, 'L', 'L', $font, '12', 'B', '', '4px');
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();
      }
      $str .= $this->reporter->begintable($layoutsize);
    ...
    $datax = $this->sales_group_query($config);

    $data1 = $this->coreFunctions->opentable($datax['qry1']);
    $data2 = $this->coreFunctions->opentable($datax['qry2']);

    $str .= '<br/><br/>';
    $str .= $this->reporter->begintable('1000');
    $str .= $this->reporter->startrow('1000');
    $str .= $this->reporter->col('Summary Per Sales Group : ', '600', null, false, $border, 'TLRB', 'L', $font, '12', 'B', '', '4px');
    $str .= $this->reporter->col('', '200', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '4px');
    $str .= $this->reporter->col('', '200', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '4px');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();
    ...
    $str .= $this->reporter->begintable('1000');
    $str .= $this->reporter->col('', '200', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '4px');
    $str .= $this->reporter->col('Actual', '200', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '4px');
    $str .= $this->reporter->col('Target', '200', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '4px');
    $str .= $this->reporter->col('%', '200', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '4px');
    $str .= $this->reporter->col('Variance +/-', '200', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '4px');
    $str .= $this->reporter->endrow();
    foreach ($data1 as $key => $colx) {
      $target = floatval($this->coreFunctions->datareader("select ifnull(sum(s.amt),0) as value from salesgroupqouta as s
        left join client as a on a.clientid = s.agentid left join salesgroup as sg on sg.line = a.salesgroupid
        left join projectmasterfile as p on p.line = s.projectid where s.yr = ? and sg.groupname =?  and s.amt<>0 group by sg.groupname", [$yr, $colx->sgroup]));

      $str .= $this->reporter->startrow('1000');
      $str .= $this->reporter->col($colx->sgroup, '200', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '4px');
      $str .= $this->reporter->col(number_format($colx->totaldollar, 2), '200', null, false, $border, 'TLRB', 'C', $font, '12', '', '', '4px');
      $str .= $this->reporter->col(number_format($target * $colx->m, 2), '200', null, false, $border, 'TLRB', 'C', $font, '12', '', '', '4px');
      if ($colx->totaldollar == 0 || $target == 0) {
        $str .= $this->reporter->col('-', '200', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '');
      } else {
        $str .= $this->reporter->col(number_format(($colx->totaldollar / $target) * 100, 2), '200', null, false, $border, 'TLRB', 'C', $font, '12', '', '', '4px');
      }

      $str .= $this->reporter->col(number_format($target - $colx->totaldollar, 2), '200', null, false, $border, 'TLRB', 'C', $font, '12', '', '', '4px');
      $str .= $this->reporter->endrow();
    }
    ...
    $str .= $this->reporter->begintable('1000');
    $str .= $this->reporter->startrow('1000');
    $str .= $this->reporter->col('Summary Per Branch : ', '600', null, false, $border, 'TLRB', 'L', $font, '12', 'B', '', '4px');
    $str .= $this->reporter->col('', '200', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '4px');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();
    ...
    $str .= $this->reporter->begintable('1000');
    $str .= $this->reporter->col('', '200', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '4px');
    $str .= $this->reporter->col('Total Sales', '200', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '4px');
    $str .= $this->reporter->endrow();
    foreach ($data2 as $key => $colx) {
      $str .= $this->reporter->startrow('1000');
      $str .= $this->reporter->col($colx->sgroup, '200', null, false, $border, 'TLRB', 'C', $font, '12', 'B', '', '4px');
      $str .= $this->reporter->col(number_format($colx->totaldollar, 2), '200', null, false, $border, 'TLRB', 'C', $font, '12', '', '', '4px');
      $str .= $this->reporter->endrow();

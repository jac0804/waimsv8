...
class mtd_sales_report_db
{
    public $modulename = 'MTD Sales Report';
    ...
    public function default_QUERY($config, $igrp = '')
    {
        ...
        $query = "
      select ifnull(igroup,'no item group') as itemgroup, case igroup when 'others' then 100 else 1 end as sort,
      sum(mjan) as mjan,
      sum(mfeb) as mfeb,
      sum(mmar) as mmar,
      sum(mapr) as mapr,
      ...
      sum(mdec) as mdec
          ...
          where hqsstock.void<>1 and hqsstock.ext is not null and  date(sohead.dateid) between '" . $start . "' and '" . $end . "' $filter
          group by prj.groupid,sohead.dateid,target
          union all
          select
          (case when month(sohead.due)=1 then sum(hqtstock.ext*(case $forex when 0 then hqtstock.sgdrate else $forex end)) else 0 end) as mjan,
          (case when month(sohead.due)=2 then sum(hqtstock.ext*(case $forex when 0 then hqtstock.sgdrate else $forex end)) else 0 end) as mfeb,
          (case when month(sohead.due)=3 then sum(hqtstock.ext*(case $forex when 0 then hqtstock.sgdrate else $forex end)) else 0 end) as mmar,
          (case when month(sohead.due)=4 then sum(hqtstock.ext*(case $forex when 0 then hqtstock.sgdrate else $forex end)) else 0 end) as mapr,
          (case when month(sohead.due)=5 then sum(hqtstock.ext*(case $forex when 0 then hqtstock.sgdrate else $forex end)) else 0 end) as mmay,
          (case when month(sohead.due)=6 then sum(hqtstock.ext*(case $forex when 0 then hqtstock.sgdrate else $forex end)) else 0 end) as mjun,
          (case when month(sohead.due)=7 then sum(hqtstock.ext*(case $forex when 0 then hqtstock.sgdrate else $forex end)) else 0 end) as mjul,
          (case when month(sohead.due)=8 then sum(hqtstock.ext*(case $forex when 0 then hqtstock.sgdrate else $forex end)) else 0 end) as maug,
          (case when month(sohead.due)=9 then sum(hqtstock.ext*(case $forex when 0 then hqtstock.sgdrate else $forex end)) else 0 end) as msep,
          (case when month(sohead.due)=10 then sum(hqtstock.ext*(case $forex when 0 then hqtstock.sgdrate else $forex end)) else 0 end) as moct,
          (case when month(sohead.due)=11 then sum(hqtstock.ext*(case $forex when 0 then hqtstock.sgdrate else $forex end)) else 0 end) as mnov,
          (case when month(sohead.due)=12 then sum(hqtstock.ext*(case $forex when 0 then hqtstock.sgdrate else $forex end)) else 0 end) as mdec,
          prj.groupid as igroup,igq.amt as target
          ...
          left join itemgroupqouta igq on prj.line = igq.projectid and igq.yr = '$year'
          where hqtstock.void<>1 and hqtstock.ext is not null and  date(sohead.due) between '" . $start . "' and '" . $end . "' $filter
          group by prj.groupid,sohead.due,target
          ) as a 
          group by a.igroup order by sort,igroup";
    ...
    public function layout_DEFAULT($config, $result)
    {
        ...
        $layoutsize = 150 + (400 * count($period)) + 400; //'5350';
        ...
        foreach ($period as $dt) {
            $str .= $this->reporter->col(strtoupper($dt->format("F")), '400', null, false, $border, 'TRB', 'C', $font, $fontsize, 'B', '', '4px');
        }
        ...
        foreach ($igrp as $p => $v) {
            ...
            if (!empty($result)) {
                foreach ($result as $key => $value) {
                    ...
                    $str .= $this->reporter->col(number_format($target * count($period), $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, '', '', '4px');
                    if ($totalactual != 0 && $target != 0) {
                        $str .= $this->reporter->col(number_format(($totalactual / ($target * count($period))) * 100, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, '', '', '4px');
                    } else {
                        $str .= $this->reporter->col('-', '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, '', '', '4px');
                    }
                    if ($target == 0) {
                        ...
                        $str .= $this->reporter->col(number_format(($target * count($period)) - $totalactual, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, '', '', '4px');
                    }
                    $totaltarget = $totaltarget + ($target * count($period));
                    $str .= $this->reporter->endrow();
                }
            } else {
                for ($i = 1; $i <= count($period); $i++) {
        ...
        for ($i = 1; $i <= count($period); $i++) {
            if ($i == 1) {
                ...
                $str .= $this->reporter->col(number_format($totjan, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($tottjan, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totpjan, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totvjan, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
            }

            if ($i == 2) {
                $str .= $this->reporter->col(number_format($totfeb, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($tottfeb, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totpfeb, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totvfeb, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
            }

            if ($i == 3) {
                $str .= $this->reporter->col(number_format($totmar, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($tottmar, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totpmar, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totvmar, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
            }

            if ($i == 4) {
                $str .= $this->reporter->col(number_format($totapr, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($tottapr, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totpapr, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totvapr, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
            }

            if ($i == 5) {
                $str .= $this->reporter->col(number_format($totmay, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($tottmay, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totpmay, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totvmay, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
            }

            if ($i == 6) {
                $str .= $this->reporter->col(number_format($totjun, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($tottjun, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totpjun, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totvjun, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
            }

            if ($i == 7) {
                $str .= $this->reporter->col(number_format($totjul, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($tottjul, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totpjul, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totvjul, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
            }

            if ($i == 8) {
                $str .= $this->reporter->col(number_format($totaug, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($tottaug, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totpaug, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totvaug, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
            }

            if ($i == 9) {
                $str .= $this->reporter->col(number_format($totsep, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($tottsep, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totpsep, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totvsep, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
            }

            if ($i == 10) {
                $str .= $this->reporter->col(number_format($totoct, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($tottoct, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totpoct, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totvoct, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
            }

            if ($i == 11) {
                $str .= $this->reporter->col(number_format($totnov, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($tottnov, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totpnov, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totvnov, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
            }

            if ($i == 12) {
                $str .= $this->reporter->col(number_format($totdec, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($tottdec, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totpdec, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
                $str .= $this->reporter->col(number_format($totvdec, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
            }
        }

        
        $totalactual = $totjan + $totfeb + $totmar + $totapr + $totmay + $totjun + $totjul + $totaug + $totsep + $totoct + $totnov + $totdec;
        $str .= $this->reporter->col(number_format($totalactual, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
        $str .= $this->reporter->col(number_format($totaltarget, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
        if($totaltarget == 0 || $totalactual == 0){
            $str .= $this->reporter->col('0', '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
        }else{
            $str .= $this->reporter->col(number_format(($totalactual / $totaltarget) * 100, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');
        }
        
        $str .= $this->reporter->col(number_format($totaltarget - $totalactual, $decimalprice), '100', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '4px');

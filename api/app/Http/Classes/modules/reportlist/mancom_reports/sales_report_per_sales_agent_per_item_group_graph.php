...
class sales_report_per_sales_agent_per_item_group_graph
{
  public $modulename = 'Sales Report Per Sales Agent Per Item Group Graph';
  ...
  public function reportdata($config)
  {
    ...
    $str = $this->layout_DEFAULT($config, $result, $overall);
    $graph = $this->report_graph($config, $result, $overall);
  ...
  public function default_QUERY($config)
  {
    ...
    $query = "select ifnull(a.igroup,'NO GROUP') as itemgroup, ifnull(sum(ext),0) as ext, sum(ext * $forex) as totaldollar
      from (
      select sum(stock.ext) as ext,prj.code as igroup
  ...
  public function QUERY_for_overall($config)
  {
    ...
    $query = "select ifnull(sum(ext),0) as ext, sum(ext * $forex) as totaldollar
      from (
      select sum(stock.ext) as ext,prj.code as igroup
      from hsqhead as sohead
      left join hqshead as head on head.sotrno = sohead.trno
  ...
  public function header_DEFAULT($config)
  {
    ...
    $str .= $this->reporter->col('AMOUNT', '50', null, false, $border, 'TRB', 'C', $font, $fontsize, 'B', '', '');
  ...
  public function layout_DEFAULT($config, $result, $result2)
  {
    ...
    foreach ($result as $key => $data) {
      ...
      $str .= $this->reporter->col(number_format(($data->ext / $result2[0]['ext']) * 100, 2) . '%', '50', null, false, $border, '', 'R', $font, $fontsize, '', '', '');
      $str .= $this->reporter->endrow();
      $gtotal += $data->ext;
    }
    $str .= $this->reporter->startrow();
  ...
  private function report_graph($config, $data, $data2)
  {
    $proj = [];
    $strprj = [];

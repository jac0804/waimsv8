...
class accounts_receivable_reports
{
    ...
    private function default_displayHeader($config)
    {
        ...
        $str .=  $this->reporter->col($reporttimestamp, '600', null, false, '1px solid ', '', 'L', $font,  '12', '', '', '', 0, '', 0, 5);
        $str .=  $this->reporter->endrow();
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col(strtoupper($headerdata[0]->name), null, null, false, '1px solid ', '', 'C', $font,  $font_size, 'B', '', '');
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col(strtoupper($headerdata[0]->address), null, null, false, '1px solid ', '', 'C', $font,  $font_size, 'B', '', '');
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col(strtoupper($headerdata[0]->tel), null, null, false, '1px solid ', '', 'C', $font,  $font_size, 'B', '', '');
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();

        $str .= $this->reporter->begintable($layoutsize);
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col('ACCOUNTS RECEIVABLE REPORTS', null, null, false, $border, '', 'C', $font, $font_size, 'B', '', '') . '<br />';
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();

        $startdate = $start;
        $startt = new DateTime($startdate);
        $start = strtoupper($startt->format('F d, Y')); // September 13,2025

        $str .= $this->reporter->begintable($layoutsize);
        $str .= $this->reporter->startrow(null, null, '', $border, '', 'r', $font,  $font_size, '', '');
        $str .= $this->reporter->col('AS OF ' . $start, null, null, '', $border, '', 'C', $font,  $font_size, 'B', '', '');
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();
        ...
        $str .= $this->reporter->begintable($layoutsize);
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col('NO.', '50', '', '', $border, 'TB', 'L', $font,  $font_size, 'B', '', '');
        $str .= $this->reporter->col('CUSTOMER', '600', '', '', $border, 'TB', 'L', $font,  $font_size, 'B', '', '');
        $str .= $this->reporter->col('ACCOUNT BALANCE', '225', '', '', $border, 'TB', 'R', $font,  $font_size, 'B', '', '');
        $str .= $this->reporter->col('EFFECTIVE BALANCE', '225', '', '', $border, 'TB', 'R', $font,  $font_size, 'B', '', '');
        $str .= $this->reporter->endrow();

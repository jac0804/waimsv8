...
class sales_report_per_product
{
  ...
  {
    $fields = ['radioprint', 'start', 'end', 'cur', 'radioposttype'];
  ...
  public function default_QUERY($config)
  {
    ...
    switch ($posttype) {
      case '0'; // posted
        $query = "select prj.name as prjname, sum(stock.ext) as ext, sum(stock.ext * " . $forex . ") as totaldollar
        from glhead as head
        left join glstock as stock on stock.trno = head.trno
        left join projectmasterfile as prj on prj.line = stock.projectid
        where stock.projectid <> 0 and date(head.dateid) between '" . $start . "' and '" . $end . "'
        group by prj.name";
        break;
      case '1': // unposted
        $query = "select prj.name as prjname, sum(stock.ext) as ext, sum(stock.ext * " . $forex . ") as totaldollar
      ...
      case '2': // all 
        $query = "
          select prjname, sum(ext) as ext, sum(ext * " . $forex . ") as totaldollar 
          from (
          select prj.name as prjname, sum(stock.ext) as ext
          from glhead as head
          left join glstock as stock on stock.trno = head.trno
          left join projectmasterfile as prj on prj.line = stock.projectid
          where stock.projectid <> 0 and date(head.dateid) between '" . $start . "' and '" . $end . "'

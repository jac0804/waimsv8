...
class approved_with_vehicle_list
{
  public $modulename = 'Approved With Vehicle List';
  ...
  vehiclename, drivername, approveddate from(
  select head.docno, date(head.dateid) as dateid,
        dept.clientname as department, customer.clientname as customername, p.purpose, 
        head.schedin as timein, head.schedout as timeout, ship.addr as loc, head.approveddate,
        ...
        group by stock.trno,stock.line,head.docno, date(head.dateid),dept.clientname, customer.clientname, p.purpose, head.schedin, head.schedout, ship.addr,vehicle.clientname, vehicle.plateno, driver.clientname, head.approveddate
        union all
        select head.docno, date(head.dateid) as dateid,
        dept.clientname as department, customer.clientname as customername, p.purpose, 
        head.schedin as timein, head.schedout as timeout, ship.addr as loc, head.approveddate,
  ...
  vehiclename, drivername
        order by timein, docno, dateid, department, docno, customername, purpose, timein, timeout, loc, itemname, passengername,
  vehiclename, drivername
  ...
  public function reportDefaultLayout($config)
  {
    ...
    foreach ($result as $key => $data) {
      ...
      if ($dateid == date_format(date_create($data->timein), 'm/d/Y')) {
        $dateid = "";
      } else {
        $dateid = date_format(date_create($data->timein), 'm/d/Y');
      }
      ...
      $str .= $this->reporter->col($department, '90', null, false, $border, $bordrline, '', $font, $font_size, '', '', '');
      ...
      $str .= $this->reporter->col(nl2br(htmlspecialchars($data->itemname)), '90', null, false, $border, 'TLRB', '', $font, $font_size, '', '', '');
      $str .= $this->reporter->col(nl2br(htmlspecialchars($passengername)), '90', null, false, $border, 'TLRB', '', $font, $font_size, '', '', '');
      $str .= $this->reporter->endrow();

...
class turn_over_of_items
{
  ...
  left join turnoveritemdetail as detail on detail.trno=head.trno
  ...
  left join client as em on em.clientid=head.empid
  left join client as d on d.clientid=head.deptid
  left join hturnoveritemdetail as detail on detail.trno=head.trno
  where head.dateid between '" . $start . "' and '" . $end . "' $filter $filter1 
  order by docno ";
  ...
  private function displayHeader($config)
  {
    ...
    $str .= $this->reporter->begintable($this->reportParams['layoutSize']);
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('TURN OVER OF ITEMS REPORTS', null, null, false, $border, '', '', $font, '18', 'B', '', '');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();
    ...
    $str .= $this->reporter->col('DEPARTMENT', '140', null,  $bgcolors, $border, 'TB', 'C', $font, $font_size, 'B', $fontcolor, '8px');
    $str .= $this->reporter->col('ITEMS', '140', null,  $bgcolors, $border, 'TB', 'C', $font, $font_size, 'B', $fontcolor, '8px');
    $str .= $this->reporter->col('AMOUNT', '100', null,  $bgcolors, $border, 'TB', 'C', $font, $font_size, 'B', $fontcolor, '8px');
  ...
  public function reportDefaultLayout($config)
  {
    ...
    foreach ($result as $key => $data) {
      ...
      $str .= $this->reporter->col($data->deptname, '140', null, false, $border, 'LB', 'LT', $font, $font_size, '', '', '');
      $str .= $this->reporter->col($data->itemname, '140', null, false, $border, 'LB', 'LT', $font, $font_size, '', '', '');
      $str .= $this->reporter->col(number_format($data->amt, 2) . '&nbsp&nbsp&nbsp', '100', null, false, $border, 'LB', 'RT', $font, $font_size, '', '', '');

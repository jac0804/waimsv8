...
class tax_withheld_report
{
  public $modulename = 'Tax Withheld Report';
  ...
  public function reportDefault($config)
  {
    ...
    if ($deptname != "") {
      if ($deptid != 0) {
        $filter1 .= " and dept.deptid = $deptid";
      }
    }
    if ($divname != "") {
      if ($divid != 0) {
        $filter2 .= " and d.divid = $divid";
      }
    }
    $emplvl = $this->othersClass->checksecuritylevel($config);

    $query = "select e.clientname,e.client,emp.tin, sum(p.ee) as ee,sum(p.er) as er,sum(p.ec) as ec,sum(p.cr) as cr from
        (select empid,cr as ee,0 as er,0 as ec,0 as cr from paytrancurrent as pt left join paccount as pa on pa.line=pt.acnoid
        where pa.alias ='YWT' and month(dateid)= '" . $month . "' and year(dateid)= '" . $year . "'
        union all
        ...
        group by e.clientname,e.client,emp.tin";
  ...
  private function displayHeader($config)
  {
    ...
    $str .= $this->reporter->col('TAX WITHHELD REPORT', null, null, false, $border, '', '', $font, '18', 'B', '', '');
    ...
    $str .= $this->reporter->col('T I N #', '100', null, false, $border, 'TB', 'C', $font, $font_size, 'B', '', '');
  ...
  public function reportDefaultLayout($config)
  {
    ...
    foreach ($result as $key => $data) {
      ...
      $str .= $this->reporter->col($data->cr == 0 ? '-' : number_format($data->cr, 2), '80', null, false, $border, '', 'R', $font, $font_size, '', '', '');
      $str .= $this->reporter->col($data->ee == 0 ? '-' : number_format($data->ee, 2), '80', null, false, $border, '', 'R', $font, $font_size, '', '', '');

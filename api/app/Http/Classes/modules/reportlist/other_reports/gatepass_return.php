...
class gatepass_return
{
  public $modulename = 'GATEPASS RETURNED REPORT';
  ...
  public function defaultQuery($config)
  {
    ...
    $query = "select head.trno, head.docno, head.client, emp.clientname, left(head.dateid, 10) as dateid,
    stock.itemid, item.barcode, item.itemname, stock.serialno, left(stock.returndate, 10) as returndate, stock.returnby
  ...
  public function defaultHeader_layout($config)
  {
    ...
    $str .= $this->reporter->begintable($layoutsize);
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col("GATEPASS RETURNED ITEM", null, null, '', '', 'LRTB', 'L', $font, '18', 'B', '', '<br/>');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->startrow();
    ...
    $str .= '<br>';
    $str .= $this->reporter->begintable($layoutsize);
    $str .= $this->reporter->col('Return Date', '100', null, '', $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '');
    $str .= $this->reporter->col('Return By', '100', null, '', $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '');
  ...
  public function reportDefaultLayout($config)
  {
    ...
    foreach ($result as $key => $data) {
      ...
      if ($docno != $data->docno) {
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col($data->returndate, '100', null, '', '', 'LRTB', 'L', $font, $fontsize, 'B', '', '');
        $str .= $this->reporter->col($data->returnby, '100', null, '', '', 'LRTB', 'L', $font, $fontsize, 'B', '', '');

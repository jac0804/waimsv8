...
class lubricant_consumption_report
{
  public $modulename = 'Lubricant Consumption Report';
  ...
  private function default_displayHeader($config)
  {
    ...
    $str .= $this->reporter->col('LUBRICANT CONSUMPTION REPORT', null, null, false, $border, '', '', $font, '18', 'B', '', '') . '<br />';
    $str .= $this->reporter->startrow();
  ...
  public function reportDefaultLayout($config)
  {
    ...
    $str .= $this->reporter->col('C O S T &nbsp&nbsp C E N T E R', 500, null, false, $border, 'T', 'C', $font, $fontsize, 'B', '', '');
    ...
    //first row
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('LUBE DESC', 200, null, false, $border, 'TL', 'C', $font, $fontsize, 'B', '', '');
    //for displaying all the distinct departments
    foreach ($dept as $key => $count) {
      $str .= $this->reporter->col($count->dept, 150, null, false, $border, 'TL', 'C', $font, $fontsize, 'B', '', '');
    }
    ...
    foreach ($result as $key => $data) {
      //end of group
      if ($lubegroup != '' && $lubegroup != $data->sidecol) {
      ...
      if ($i == 0) {
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col($lubegroup, 200, null, false, $border, 'TLB', 'C', $font, $fontsize, 'B', '', '');
        foreach ($dept as $key => $count2) {
          foreach ($result as $key => $displayamt) {
            //checking if plot is equal dept and account title
            if ($count2->dept == $displayamt->dept && $lubegroup == $displayamt->sidecol) {
        ...
        $str .= $this->reporter->col(number_format($grouptotal, 2), 100, null, false, $border, 'TLBR', 'C', $font, $fontsize, 'B', '', '');
        $i++;
      }
    } //loop end
    //last row
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('Grand Total', 200, null, false, $border, 'TBL', 'C', $font, $fontsize, 'B', '', '');
    foreach ($dept as $key => $count3) {
      ...
      $str .= $this->reporter->col(number_format($subtotal, 2), 150, null, false, $border, 'BTL', 'C', $font, $fontsize, 'B', '', '');
      $grandtotal += $subtotal;
      $subtotal = 0;
    }

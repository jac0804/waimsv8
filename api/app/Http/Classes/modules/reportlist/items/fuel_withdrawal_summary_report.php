...
class fuel_withdrawal_summary_report
{
  public $modulename = 'Fuel Withdrawal Summary Report';
  ...
  public function DEFAULT_QUERY($config)
  {
    ...
    // default://ALL
    $query = "select right(a.dept,4) as dept,sum(a.iss) as qty,sum(a.ext) as amt from (
          ...
          where doc in ('RN','RM') and stock.iscomponent = 0 and date(head.dateid) between '$start' and '$end' and i.itemname like '%diesoline%'
          union all
  ...
  private function default_displayHeader($config)
  {
    ...
    $str .= $this->reporter->col('Fuel Withdrawal Summary Report', '800', null, false, $border, '', 'C', $font, $fontsize + 4, 'B', '', '');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();
    ...
    $str .= $this->reporter->col('Ltr', '150', null, false, $border, 'TLR', 'C', $font, $fontsize + 2, 'B', '', '');
    $str .= $this->reporter->col('Dissoline Php', '150', null, false, $border, 'TLR', 'C', $font, $fontsize + 2, 'B', '', '');
  ...
  public function reportDefaultLayout($config, $result)
  {
    ...
    foreach ($result as $key => $data) {
      ...
      $str .= $this->reporter->col(number_format($data->qty, 2), '150', null, false, $border, 'TLR', 'R', $font, $fontsize - 1, '', '', '');
      $str .= $this->reporter->col(number_format($data->amt, 2), '150', null, false, $border, 'TLR', 'R', $font, $fontsize - 1, '', '', '');
      $str .= $this->reporter->col('', '350', null, false, $border, '', 'C', $font, $fontsize, '', '', '');
      $count++;
    ...
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('', '150', null, false, $border, 'T', 'C', $font, $fontsize - 1, 'B', '', '');
    $str .= $this->reporter->col('', '150', null, false, $border, 'T', 'C', $font, $fontsize - 1, 'B', '', '');
    $str .= $this->reporter->col('', '150', null, false, $border, 'T', 'R', $font, $fontsize - 1, 'B', '', '');
    $str .= $this->reporter->col('', '350', null, false, $border, '', 'C', $font, $fontsize, '', '', '');
    $str .= $this->reporter->endtable();
    ...
    $str .= $this->reporter->col('', '350', null, false, $border, '', 'C', $font, $fontsize, '', '', '');
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('Total =', '150', null, false, $border, 'TBLR', 'C', $font, $fontsize - 1, 'B', '', '');
    $str .= $this->reporter->col(number_format($totalqty, 2), '150', null, false, $border, 'TBLR', 'R', $font, $fontsize - 1, 'B', '', '');
    $str .= $this->reporter->col(number_format($totalamt, 2), '150', null, false, $border, 'TBLR', 'R', $font, $fontsize - 1, 'B', '', '');
    $str .= $this->reporter->col('', '350', null, false, $border, '', 'C', $font, $fontsize, '', '', '');
    $str .= $this->reporter->endtable();

...
class item_purchase_report
{
    ...
    public function reportplotting($config)
    {
        ...
        switch ($companyid) {
            ...
            case 59: //roosevelt
                ...
                break;
            case 71: //buenatech
                $result = $this->buenatech_layout($config);
                break;
            default:
    ...
    public function reportRoosevelt($config)
    {
        ...
        if ($barcode != "")
            $filter .= " and stock.itemid=" . $config['params']['dataparams']['itemid'];
        if ($groupname != "")
            $filter .= " and item.groupid=" . $config['params']['dataparams']['groupid'];
        if ($categoryname != "")
            $filter .= " and item.category='" . $config['params']['dataparams']['category'] . "'";
        if ($subcatname != "")
            $filter .= " and item.subcat='" . $config['params']['dataparams']['subcat'] . "'";
        if ($brandname != "")
            $filter .= " and item.brand=" . $config['params']['dataparams']['brandid'];
        if ($classname != "")
            $filter .= " and item.class=" . $config['params']['dataparams']['classid'];
        if ($wh != "")
            $filter .= " and stock.whid=" . $config['params']['dataparams']['whid'];
        if ($client != '') {
    ...
    public function reportDefault($config)
    {
        ...
        if ($companyid == 71){
            $addfield .= ", stock.loc as lotnum, stock.expiry";
            $addselect .= ", cost, lotnum, expiry";
            $addgrpby .= ", lotnum, expiry";
        }

        $query = "select barcode, itemname, docno, dateid , qty, price,
        uom, brand, part,model,sizeid,body,ponum $addselect
        from (
        select item.barcode, item.itemname, head.docno, date(head.dateid) as dateid, sum(stock.qty) as qty,
        sum(stock.cost) as cost, sum(stock.ext) as price, item.uom, brand.brand_desc as brand,
        part.part_name as part,model.model_name as model,item.sizeid,item.body,stock.ref as ponum $addfield
        from lahead as head
        left join lastock as stock on stock.trno = head.trno
        left join item on item.itemid=stock.itemid
        left join frontend_ebrands as brand on brand.brandid = item.brand
        ...
        where head.doc in ('rr','rp') and item.barcode is not null and date(head.dateid) between '$start' and '$end' $filter $filter1 and item.isofficesupplies=0
        group by item.barcode, item.itemname, head.dateid,item.uom, brand.brand_desc, part.part_name,
        model.model_name,item.sizeid,item.body, head.docno,stock.ref $addgrpby
        ...
        group by item.barcode, item.itemname, docno, head.dateid,item.uom, brand.brand_desc,
        part.part_name,model.model_name,item.sizeid,item.body,stock.ref $addgrpby) as ip
        order by brand, part, itemname";

        return $this->coreFunctions->opentable($query);
    }

    private function buenatech_displayHeader($config)
    {
        $border = '1px solid';
        ...
        $str .= $this->reporter->col('ITEM PURCHASE REPORT', null, null, false, $border, '', '', $font, '14', 'B', '', '') . '<br />';
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();
        ...
        $str .= $this->reporter->begintable($layoutsize);
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col('ITEM CODE', '110', null, false, $border, 'B', 'C', 'Verdana', $font_size, 'B', '', '', '');
        $str .= $this->reporter->col('ITEM DESCRIPTION', '250', null, false, $border, 'B', 'L', 'Verdana', $font_size, 'B', '', '', '');
        $str .= $this->reporter->col('DOC #', '100', null, false, $border, 'B', 'C', 'Verdana', $font_size, 'B', '', '', '');
        $str .= $this->reporter->col('DATE PURCHASE', '90', null, false, $border, 'B', 'C', 'Verdana', $font_size, 'B', '', '', '');
        $str .= $this->reporter->col('PRICE', '80', null, false, $border, 'B', 'C', 'Verdana', $font_size, 'B', '', '', '');
        $str .= $this->reporter->col('QTY', '90', null, false, $border, 'B', 'C', 'Verdana', $font_size, 'B', '', '', '');
        $str .= $this->reporter->col('TOTAL', '90', null, false, $border, 'B', 'C', 'Verdana', $font_size, 'B', '', '', '');
        $str .= $this->reporter->col('LOT #', '90', null, false, $border, 'B', 'C', 'Verdana', $font_size, 'B', '', '', '');
        $str .= $this->reporter->col('EXPIRY DATE', '100', null, false, $border, 'B', 'C', 'Verdana', $font_size, 'B', '', '', '');
        $str .= $this->reporter->endrow();

        return $str;
    }

    public function buenatech_layout($config)
    {
        $companyid = $config['params']['companyid'];
        $border = '1px solid';
        $font = $this->companysetup->getrptfont($config['params']);
        ...
        $str .= $this->buenatech_displayHeader($config);
        $item = null;
        $subtotal = 0;
        $amt = 0;
        ...
        foreach ($result as $key => $data) {
            ...
            if ($item != $data->barcode) {
                ...
                if ($item != "") {
                    $str .= $this->reporter->begintable($layoutsize);
                    $str .= $this->reporter->startrow();
                    $str .= $this->reporter->col('', '110', null, false, $border, 'T', 'L', $font, $font_size, '', '', '8px');
                    ...
                    $str .= $this->reporter->col('', '80', null, false, $border, 'T', 'R', $font, $font_size, 'B', '', '8px');
                    $str .= $this->reporter->col('SUBTOTAL', '90', null, false, $border, 'T', 'RT', 'VERDANA', $font_size, 'B', '', '8px');
                    $str .= $this->reporter->col(number_format($subtotal, 2), '90', null, false, $border, 'T', 'RT', 'VERDANA', $font_size, '', '', '');
                    $str .= $this->reporter->col('', '90', null, false, $border, 'T', 'RT', $font, $font_size, '', '', '');
                    $str .= $this->reporter->col('', '100', null, false, $border, 'T', 'RT', $font, $font_size, '', '', '');
                    $str .= $this->reporter->endrow();
                    $str .= $this->reporter->endtable();
                    $subtotal = 0;
                }
                $str .= $this->reporter->begintable($layoutsize);
                $str .= $this->reporter->startrow();
                $str .= $this->reporter->col($data->barcode . '&nbsp', '110', null, false, $border, '', 'CT', 'Verdana', $font_size, 'B', '', '8px');
                $str .= $this->reporter->col($data->itemname . ' - ' . $data->brand . ' - ' . $data->part . ' - ' . $data->model . ' - ' . $data->sizeid . ' - ' . $data->body, '250', null, false, $border, '', 'LT', 'Verdana', $font_size, '', '', '8px');
                ...
                $str .= $this->reporter->col('', '90', null, false, $border, '', 'R', 'Verdana', $font_size, '', '', '8px');
                $str .= $this->reporter->col('', '90', null, false, $border, '', 'C', 'Verdana', $font_size, '', '', '8px');
                $str .= $this->reporter->col('', '90', null, false, $border, '', 'C', 'Verdana', $font_size, '', '', '8px');
                $str .= $this->reporter->col('', '100', null, false, $border, '', 'C', 'Verdana', $font_size, '', '', '8px');
                $str .= $this->reporter->endrow();
            ...
            $str .= $this->reporter->begintable($layoutsize);
            $str .= $this->reporter->startrow();
            $str .= $this->reporter->col('&nbsp', '110', null, false, $border, '', 'C', 'Verdana', $font_size, '', '', '');
            $str .= $this->reporter->col('&nbsp', '250', null, false, $border, '', 'L', 'Verdana', $font_size, '', '', '');
            $str .= $this->reporter->col($data->docno, '100', null, false, $border, '', 'CT', 'Verdana', $font_size, '', '', '');
            $str .= $this->reporter->col($data->dateid, '90', null, false, $border, '', 'CT', 'Verdana', $font_size, '', '', '');
            $str .= $this->reporter->col(number_format($data->cost, 2), '80', null, false, $border, '', 'RT', 'Verdana', $font_size, '', '', '');
            $str .= $this->reporter->col(number_format($data->qty, 2), '90', null, false, $border, '', 'RT', 'Verdana', $font_size, '', '', '');
            $str .= $this->reporter->col(number_format($data->price, 2), '90', null, false, $border, '', 'RT', 'Verdana', $font_size, '', '', '');
            $str .= $this->reporter->col($data->lotnum, '90', null, false, $border, '', 'CT', 'Verdana', $font_size, '', '', '');
            $str .= $this->reporter->col($data->expiry, '100', null, false, $border, '', 'CT', 'Verdana', $font_size, '', '', '');
            $str .= $this->reporter->endrow();
            $str .= $this->reporter->endtable();
        ...
        $str .= $this->reporter->col('', '100', null, false, $border, 'T', 'C', $font, $font_size, '', '', '8px');
        $str .= $this->reporter->col('', '90', null, false, $border, 'T', 'R', $font, $font_size, '', '', '8px');
        $str .= $this->reporter->col('', '80', null, false, $border, 'T', 'C', $font, $font_size, 'B', '', '8px');
        $str .= $this->reporter->col('TOTAL', '60', null, false, $border, 'T', 'RT', 'VERDANA', $font_size, 'B', '', '8px');
        $str .= $this->reporter->col(number_format($amt, 2), '120', null, false, $border, 'T', 'RT', 'VERDANA', $font_size, '', '', '');
        $str .= $this->reporter->col('', '90', null, false, $border, 'T', 'R', $font, $font_size, '', '', '');

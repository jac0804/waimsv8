...
class item_sales_report
{
    ...
    public function reportplotting($config)
    {
        ...
        if ($companyid == 71){
            return $this->buenatech_DefaultLayout($config);
        }else {
    ...
    public function default_query($config)
    {
        ...
        if ($companyid == 71){
            $addfield .= ", stock.loc, stock.expiry";
            $addfield2 .= ", ip.loc, ip.expiry";
            $grp .= ", loc, expiry";
        }
        if ($client != '')
            $filter .= " and c.clientid=" . $config['params']['dataparams']['clientid'];
        if ($area != '')
        ...
        if ($partname != '')
            $filter .= " and item.part='" . $config['params']['dataparams']['part'] . "'";
        if ($wh != '')
        ...
        switch ($posttype) {
            case '0': // posted
                ...
                part.part_name,model.model_name,item.sizeid,item.body,stock.ref $grp";
                break;
            case '1': // unposted
                $query = "select item.barcode, item.itemname, head.docno, date(head.dateid) as dateid, sum(stock.isqty) as qty,
                sum(stock.amt) as cost, sum(stock.ext) as price, item.uom, brand.brand_desc as brand,
                ...
                model.model_name,item.sizeid,item.body, head.docno,stock.ref $grp";
                break;
            default: // all
                $query = "select barcode, itemname, cost, docno, dateid , qty, price,
                uom, brand, part,model,sizeid,body,ponum $addfield2
                from (
                ...
                where head.doc in ('sj', 'cm') and item.barcode is not null and head.dateid between '$start' and '$end' $filter and item.isofficesupplies=0
                group by item.barcode, item.itemname, docno, head.dateid,item.uom, brand.brand_desc,
                part.part_name,model.model_name,item.sizeid,item.body,stock.ref $grp) as ip
    ...
    private function buenatech_displayHeader($config)
    {
        ...
        $str .= $this->reporter->begintable($layoutsize);
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col('ITEM SALES REPORT', null, null, false, $border, '', 'C', $font, '15', 'B', '', '') . '<br />';
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();
        ...
        $str .= $this->reporter->col('DOC #', '110', null, false, $border, 'B', 'C', 'Verdana', $font_size, 'B', '', '', '');
        ...
        $str .= $this->reporter->col('QTY', '80', null, false, $border, 'B', 'C', 'Verdana', $font_size, 'B', '', '', '');
    ...
    public function buenatech_DefaultLayout($config)
    {
        $border = '1px solid';
        $font = $this->companysetup->getrptfont($config['params']);
        $font_size = '8';
        ...
        foreach ($result as $key => $data) {
            ...
            if ($item != $data->barcode) {
                if ($item != "") {
                    $str .= $this->reporter->begintable($layoutsize);
                    $str .= $this->reporter->startrow();
                    $str .= $this->reporter->col('&nbsp;', '110', null, false, $border, 'LRB', 'C', 'Verdana', $font_size, '', '', '', '');
                    $str .= $this->reporter->col('', '250', null, false, $border, 'LRB', 'C', 'Verdana', $font_size, '', '', '', '');
                    $str .= $this->reporter->col('', '110', null, false, $border, 'LRB', 'C', 'Verdana', $font_size, '', '', '', '');
                    $str .= $this->reporter->col('', '90', null, false, $border, 'LRB', 'C', 'Verdana', $font_size, '', '', '', '');
                    $str .= $this->reporter->col('', '80', null, false, $border, 'LRB', 'C', 'Verdana', $font_size, '', '', '', '');
                    $str .= $this->reporter->col('', '80', null, false, $border, 'LRB', 'C', 'Verdana', $font_size, '', '', '', '');
                    $str .= $this->reporter->col('', '90', null, false, $border, 'LRB', 'C', 'Verdana', $font_size, '', '', '', '');
                    $str .= $this->reporter->col('', '90', null, false, $border, 'LRB', 'C', 'Verdana', $font_size, '', '', '', '');
                    $str .= $this->reporter->col('', '100', null, false, $border, 'LRB', 'C', 'Verdana', $font_size, '', '', '', '');
                    $str .= $this->reporter->endrow();
                    $str .= $this->reporter->endtable();
                    $subtotal = 0;
                }
                $str .= $this->reporter->begintable($layoutsize);
                $str .= $this->reporter->startrow();
                $str .= $this->reporter->col($data->barcode, '110', null, false, $border, 'LRB', 'C', 'Verdana', $font_size, '', '', '', '');
                $str .= $this->reporter->col($data->itemname, '250', null, false, $border, 'LRB', 'C', 'Verdana', $font_size, '', '', '', '');
                $str .= $this->reporter->col($data->docno, '110', null, false, $border, 'LRB', 'C', 'Verdana', $font_size, '', '', '', '');
                $str .= $this->reporter->col($data->dateid, '90', null, false, $border, 'LRB', 'C', 'Verdana', $font_size, '', '', '', '');
                $str .= $this->reporter->col(number_format($data->cost, 2), '80', null, false, $border, 'LRB', 'C', 'Verdana', $font_size, '', '', '', '');
                $str .= $this->reporter->col(number_format($data->qty, 2), '80', null, false, $border, 'LRB', 'C', 'Verdana', $font_size, '', '', '', '');
                $str .= $this->reporter->col(number_format($data->price, 2), '90', null, false, $border, 'LRB', 'C', 'Verdana', $font_size, '', '', '', '');
                $str .= $this->reporter->col($data->loc, '90', null, false, $border, 'LRB', 'C', 'Verdana', $font_size, '', '', '', '');
                $str .= $this->reporter->col($data->expiry, '100', null, false, $border, 'LRB', 'C', 'Verdana', $font_size, '', '', '', '');
                $str .= $this->reporter->endrow();
                $str .= $this->reporter->endtable();
            } else {
                $str .= $this->reporter->begintable($layoutsize);
        ...
        $str .= $this->reporter->col('', '250', null, false, $border, 'TLBR', 'C', 'Verdana', $font_size, '', '', '', '');
        $str .= $this->reporter->col('', '110', null, false, $border, 'TLBR', 'C', 'Verdana', $font_size, '', '', '', '');
        $str .= $this->reporter->col('', '90', null, false, $border, 'TLRB', 'C', 'Verdana', $font_size, '', '', '', '');
        $str .= $this->reporter->col('', '80', null, false, $border, 'TLBR', 'C', 'Verdana', $font_size, '', '', '', '');
        $str .= $this->reporter->col(number_format($totalcount, 2), '80', null, false, $border, 'TLBR', 'C', 'Verdana', $font_size, 'B', '', '', '');
        $str .= $this->reporter->col(number_format($amt, 2), '90', null, false, $border, 'TLBR', 'C', 'Verdana', $font_size, 'B', '', '', '');
        $str .= $this->reporter->col('', '90', null, false, $border, 'TLBR', 'C', 'Verdana', $font_size, 'B', '', '', '');
        $str .= $this->reporter->col('', '100', null, false, $border, 'TLBR', 'C', 'Verdana', $font_size, 'B', '', '', '');
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();

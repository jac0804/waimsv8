...
class business_clearance_list_report
{
    public $modulename = 'Business Clearance List Report';
    ...
    public function reportplotting($config)
    {
        ...
        $companyid = $config['params']['companyid'];
        return $this->business_clearance_list_layout($config);
    }
    ...
    public function default_qry($config)
    {
        ...
        $query = "select doc, head.clientname, c.addr, head.address, c.client as brgyid
                from glhead as head
                left join client as c on c.clientid = head.clientid
                where doc = 'BC' and date(head.dateid) between '$start' and '$end' $filter 

                union all

                select doc, head.clientname, c.addr, head.address, c.client as brgyid
                from lahead  as head
    ...
    public function business_clearance_header($config)
    {
        $center     = $config['params']['center'];
        $username   = $config['params']['user'];
        $companyid = $config['params']['companyid'];
        ...
        $str .= $this->reporter->col('BARANGAY DONA IMELDA', '500', '20', false, '2px solid', '', 'L', $font, '12', 'B', '', '');
        ...
        $str .= $this->reporter->begintable($layoutsize);
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col('BUSINESS CLEARANCE LIST', null, null, false, '2px solid', '', 'L', $font, '12', 'B', '', '');
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();
        ...
        if ($area != '') {
            ...
            $str .= $this->reporter->col('Section: &nbsp&nbspALL STREET', 250, null, false, '2px solid', '', 'LT', $font, $fontsize, 'B', '', '');
        }
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();
    ...
    public function business_clearance_list_layout($config)
    {
        ...
        $str .= $this->business_clearance_header($config);

...
class profit_and_loss_per_branch
{
    public $modulename = 'Profit and Loss per Branch';
    ...
    {
        $query = "select 0 as clientid,'' as client,'' as clientname union all select clientid, client, clientname from client where isbranch=1  order by clientname";
        return $query;
    }
    ...
    public function default_layout($config)
    {
        ...
        $start      = date("Y-m-d", strtotime($config['params']['dataparams']['start']));
        $end        = date("Y-m-d", strtotime($config['params']['dataparams']['end']));
        $branch = $config['params']['dataparams']['brandname'];
        ...
        foreach ($result as $key => $data) {
            ...
            $str .= $this->reporter->col(($data->clientname == '' ? 'No Branch' : $data->clientname), $wpercol, null, false, $border, $b, 'C', $font, $fontsize, 'B', '', '');
    ...
    private function getaccqry($acnoid, $start, $end, $filter, $addfilters = '')
    {
        ...
        ifnull(branchname,'') as branchname,ifnull(branch,0) as branch,ifnull(deptid,0) as deptid
        from (
        select coa.acnoid,coa.acno, coa.acnoname, coa.levelid, coa.cat, coa.parent, coa.detail,ifnull(sum(tb.db),0)as db , ifnull(sum(tb.cr),0) as cr,tb.branch,tb.branchname,tb.deptid
        from coa left join (
        select detail.acnoid,sum(detail.db) as db,sum(detail.cr) as cr,detail.branch,b.clientname as branchname,detail.branch as deptid from glhead as head
        left join gldetail as detail on detail.trno=head.trno
        left join client as b on b.clientid = detail.branch
        left join cntnum on cntnum.trno=head.trno
        where head.dateid between '" . $start . "' and '" . $end . "' " . $filter . "
        group by detail.acnoid, head.dateid,detail.branch,b.clientname,detail.projectid
        ) as tb on tb.acnoid=coa.acnoid where coa.acnoid =" . $acnoid . $addfilters . " 
        group by coa.acnoid,coa.acno, coa.acnoname, coa.levelid, coa.cat, coa.parent, coa.detail, tb.branchname, tb.branch,tb.deptid
    ...
    private function getsumaccqry($cat, $start, $end, $filter, $addfilters = '')
    {
        ...
        from (
        select coa.acnoid,coa.acno, coa.acnoname, coa.levelid, coa.cat, coa.parent, coa.detail,ifnull(sum(tb.db),0)as db , ifnull(sum(tb.cr),0) as cr,branch,branchname,deptid
        from coa left join (
        select detail.acnoid,sum(detail.db) as db,sum(detail.cr) as cr,detail.branch,ifnull(b.clientname,'') as branchname,detail.branch as deptid from glhead as head
        left join gldetail as detail on detail.trno=head.trno
        left join client as b on b.clientid = detail.branch
        left join cntnum on cntnum.trno=head.trno
        where head.dateid between '" . $start . "' and '" . $end . "' " . $filter . "
        group by detail.acnoid, head.dateid,detail.branch,b.clientname
        ) as tb on tb.acnoid=coa.acnoid where coa.cat ='" . $cat . "' " . $addfilters . " 
        group by coa.acnoid,coa.acno, coa.acnoname, coa.levelid, coa.cat, coa.parent, coa.detail, tb.branch, tb.branchname,tb.deptid
    ...
    private function plotting($cat, $xdept, $start, $end, $filter, $counter, $wpercol)
    {
        ...
        foreach ($accounts as $k => $vaccount) {
            ...
            foreach ($xdept as $value) { // dept array
                $depts = explode("~", $value);
                $addfilters = " and tb.branch = " . $depts[0];
        ...
        foreach ($xdept as $value) { // dept array total per dept
            // $trev = 0;
            // $texp = 0;
            // $tinc = 0;
            $depts = explode("~", $value);

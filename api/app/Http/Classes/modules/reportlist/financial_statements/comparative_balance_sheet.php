...
class comparative_balance_sheet
{
  public $modulename = 'Comparative Balance Sheet';
    ...
    switch ($companyid) {
      ...
      default:
        ...
        if ($companyid == 8) { //maxipro
          $addparams = " , '0' as posttype,'" . $defaultcenter[0]['center'] . "' as center,'' as code,'' as name,
        '" . $defaultcenter[0]['centername'] . "' as centername,
        '" . $defaultcenter[0]['dcentername'] . "' as dcentername ";
        } else {
          $addparams = ",'' as center,'' as code,'' as name,'' as centername,'' as dcentername";
        }
        $paramstr = "select 'default' as print, adddate(left(now(),10),-360) as dateid,left(now(),10) as due,left(now(),4) as year,
                   '' as costcenter " . $addparams;
        break;
    }
  ...
  public function default_query($filters)
  {
    ...
    $this->PLANTTREE($coa, '\\\\', 'A', $year1, $year2, $view, $month, $month2, $filter, $filter1, $filter2, $company, $params);
    $this->PLANTTREE($coa, '\\\\', 'L', $year1, $year2, $view, $monthL, $monthL2, $filter, $filter1, $filter2, $company, $params);
  ...
  public function aftech_default_query($filters, $company)
  {
    ...
    $cost_filter = '';
    if ($costcenter != "") {
      $filter1 = " and detail.project = '" . $costcenter . "' ";
    }
  ...
  public function reportplotting($config)
  {
    ...
    if ($company == 10 || $company == 12) { //afti, afti usd
      ...
      $reportdata =  $this->AFTECH_DEFAULT_COMPARATIVE_BALANCE_SHEET_LAYOUT($config, $result);
    } else {
      $result = $this->default_query($config);
      $reportdata =  $this->DEFAULT_COMPARATIVE_BALANCE_SHEET_LAYOUT($config, $result);
    }

    return $reportdata;
  }

  private function PLANTTREE(&$a, $acno, $cat, $year1, $year2, $view, &$month, &$month2, $filters, $filter1, $filter2, $company, $params)
  {
    $query2 = $this->DEFAULT_BALANCE_SHEET_QUERY($cat, $acno, $year1, $year2, $view, $filters, $filter1, $filter2, $company, $params);
    ...
    if (!empty($result2)) {
      for ($b = 0; $b < count($result2); $b++) {
        ...
        if ($result2[$b]['detail'] == 0) {
          if ($this->PLANTTREE($a, '\\' . $result2[$b]['acno'], $result2[$b]['cat'], $year1, $year2, $view, $month, $month2, $filters, $filter1, $filter2, $company, $params)) {
  ...
  private function DEFAULT_BALANCE_SHEET_QUERY($cat, $acno, $year1, $year2, $view, $filters, $filter1, $filter2, $company, $params)
  {
    $field = '';
    $center = '';
    ...
    if ($company == 8) {
      ...
      $query1 = "select acno, acnoname, levelid, cat, parent, detail,
                    ...
                    from (select coa.acno, coa.acnoname, coa.levelid, coa.cat, coa.parent, coa.detail, tb.mon, tb.yr, 
                                 round(ifnull(sum(tb.amt),0),2) as amt
                          from coa 
      ...
      switch ($view) {
        ...
        from coa left join (
        select detail.acnoid, month(head.dateid) as mon,year(head.dateid) as yr, $field as amt from glhead as head left join gldetail as detail on detail.trno=head.trno left join cntnum on cntnum.trno=head.trno where year(head.dateid) between '" . $year1 . "' and '" . $year2 . "' " . $filters . " " . $filter1 . " " . $filter2 . "
        group by detail.acnoid, month(head.dateid), year(head.dateid)
        ) as tb on tb.acnoid=coa.acnoid
        where coa.parent='$acno' and coa.cat='$cat'
        group by coa.acno, coa.acnoname, coa.levelid, coa.cat, coa.parent, coa.detail, tb.mon, tb.yr
          ...
          case 8:
            $query1 = "select yr,ifnull(sum(case when yr=$year2-2 then cr else 0 end),0) as year1,
                      ...
                      from (select $field as cr, year(head.dateid) as yr, month(head.dateid) as mon
                            ...
                            left join coa on coa.acnoid=detail.acnoid 
                            left join cntnum on cntnum.trno=head.trno
                            where year(head.dateid) between  '" . $year1 . "' and '" . $year2 . "' 
                                  and coa.cat in " . $cat . " " . $filters . "
  ...
  private function DEFAULT_HEADER($params, $data)
  {
    ...
    $str .= $this->reporter->begintable('800');
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('COMPARATIVE BALANCE SHEET', null, null, false, '1px solid ', '', '', $font, $fontsize15, 'B', '', '');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();
    ...
    if ($companyid == 8) { //maxipro
      ...
      if ($companyid == 8) { //maxipro
        if ($costcenter != '') {
          $project = $params['params']['dataparams']['costcenter'];
        } else {
          $project = 'ALL';
        }
        $str .= $this->reporter->begintable('1300');
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col('Project :  ' . $project, 100, null, false, '1px solid ', '', '', $font, $fontsize12, 'B', '', '');
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();
      }
    }
  ...
  private function DEFAULT_COMPARATIVE_BALANCE_SHEET_LAYOUT($params, $data)
  {
    $companyid = $params['params']['companyid'];
    $border = '1px solid';
  ...
  private function AFTECH_DEFAULT_COMPARATIVE_BALANCE_SHEET_LAYOUT($params, $data)
  {
    $companyid = $params['params']['companyid'];
    switch ($companyid) {
      case 10: //afti
        ...
        if ($data[$i]['acnoname'] != '') {
          ...
          if ($data[$i]['detail'] != 0) {
            ...
            if ($data[$i]['year2'] == 0) {
              ...
              $str .= $this->reporter->col(number_format($data[$i]['year2'], 2), '110', null, false, '1px solid ', '', 'R', $font, $fontsize, $bold, '', '');
            }
            if ($data[$i]['year3'] == 0) {
            } else {
            }
  ...
  private function AFTECH_DEFAULT_HEADER($params, $data)
  {
    ...
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('Comparative Balance Sheet (Yearly)', null, null, false, '1px solid ', '', 'L', $font, $fontsize18, 'B', '', '');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->startrow();

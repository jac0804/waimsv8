...
class profit_and_loss_statement_by_product
{
    public $modulename = 'Profit and Loss Statement by Product';
    ...
    public function default_layout($config)
    {
        ...
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col('', '200', null, false, $border, 'B', 'L', $font, $fontsize, 'B', '', '');
        ...
        $str .= $this->plot_gross('G', $xdept, $start, $end, $filter, $counter, $wpercol);
        $str .= $this->plot_grossrate('GR', $xdept, $start, $end, $filter, $counter, $wpercol);
    ...
    private function plotting($cat, $xdept, $start, $end, $filter, $counter, $wpercol)
    {
        ...
        switch ($cat) {
            case 'R':
                $str .= $this->reporter->col('REVENUE', '200', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '');
                break;
            case 'E':
                $str .= $this->reporter->col('COST OF SALES', '200', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '');
                break;
        }
        ...
        foreach ($xdept as $value) { // dept array
            ...
            if (count($sumqry) == 0) {
                $str .= $this->reporter->col('-', $wpercol, null, false, $border, '', 'R', $font, $fontsize, $fontstyle, '', '');
                $xx++;
            } else {
                foreach ($sumqry as $kk => $sumdata) {

                    $str .= $this->reporter->col(number_format($sumdata->amt, 2), $wpercol, null, false, $border, '', 'R', $font, $fontsize, '', '', '');
            ...
            if ($xx == $counter) {
                ...
                $str .= $this->reporter->col($print_rowtotal, $wpercol, null, false, $border, '', 'R', $font, $fontsize, '', '', '');
            }
        }
    ...
    private function plot_gross($cat, $xdept, $start, $end, $filter, $counter, $wpercol)
    {
        $accounts = $this->coreFunctions->opentable("select acno,acnoid,acnoname,parent,detail,cat,levelid from coa where cat in ('" . $cat . "') order by acno");
        $levelid = 0;
        $font =  "cambria";
        ...
        $str .= $this->reporter->col('GROSS SALES', '200', null, false, $border, 'T', 'L', $font, $fontsize, 'B', '', '');
        ...
        foreach ($xdept as $value) { // dept array
            $depts = explode("~", $value);
            $addfilters = " and tb.deptid = " . $depts[0];
            $sumr = $this->coreFunctions->opentable($this->getsumaccqry('R', $start, $end, $filter, $addfilters));
            $sume = $this->coreFunctions->opentable($this->getsumaccqry('E', $start, $end, $filter, $addfilters));

            if (count($sumr) == 0 && count($sume) == 0) {
                $str .= $this->reporter->col('-', $wpercol, null, false, $border, 'T', 'R', $font, $fontsize, $fontstyle, '', '');
                $xx++;
            } else {

                if (count($sumr) != 0) {
                    foreach ($sumr as $kk => $sumdata) {
                        $rowtotal += $sumr[$kk]->amt - $sume[$kk]->amt;
                        $tot = $sumr[$kk]->amt - $sume[$kk]->amt;
                        $str .= $this->reporter->col(number_format($tot, 2), $wpercol, null, false, $border, 'T', 'R', $font, $fontsize, '', '', '');
                        $xx++;
                    }
                } else {
                    foreach ($sume as $kk => $sumdata) {
                        $r = 0;
                        if (isset($sumr[$kk]->amt)) {
                            $r = $sumr[$kk]->amt;
                        }
                        $rowtotal +=  $r - $sume[$kk]->amt;
                        $tot = $r - $sume[$kk]->amt;
                        $str .= $this->reporter->col(number_format($tot, 2), $wpercol, null, false, $border, 'T', 'R', $font, $fontsize, '', '', '');
                        $xx++;
                    }
    ...
    private function plot_grossrate($cat, $xdept, $start, $end, $filter, $counter, $wpercol)
    {
        $accounts = $this->coreFunctions->opentable("select acno,acnoid,acnoname,parent,detail,cat,levelid from coa where cat in ('" . $cat . "') order by acno");
        $levelid = 0;
        $font =  "cambria";
        ...
        $str .= $this->reporter->col('GROSS RATE', '200', null, false, $border, 'T', 'L', $font, $fontsize, 'B', '', '');
            ...
            if (count($sumr) == 0 && count($sume) == 0) {
                ...
                if (count($sumr) != 0) {
                    foreach ($sumr as $kk => $sumdata) {
                        $rowtotal += $sumr[$kk]->amt - $sume[$kk]->amt;
                        $tot = $sumr[$kk]->amt - $sume[$kk]->amt;
                        $totgr = ($tot / $sumr[$kk]->amt) * 100;
                        $str .= $this->reporter->col(number_format($totgr, 2) . ' %', $wpercol, null, false, $border, 'T', 'R', $font, $fontsize, '', '', '');
                        $xx++;
                    }
                } else {
                    foreach ($sume as $kk => $sumdata) {

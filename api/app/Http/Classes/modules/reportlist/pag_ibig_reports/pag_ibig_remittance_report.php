...
class pag_ibig_remittance_report
{
  public $modulename = 'Pag Ibig Remittance Report';
  ...
  public function reportDefault($config)
  {
    ...
    $query = "select e.clientname,e.client,emp.hdmf, sum(p.ee) as ee,sum(p.er) as er,sum(p.ec) as ec,sum(p.cr) as cr 
              from (select empid,cr as ee,0 as er,0 as ec,0 as cr 
                    from paytrancurrent as pt 
                    left join paccount as pa on pa.line=pt.acnoid
                    where pa.alias = 'YPE' and month(dateid)= '" . $month . "' and year(dateid)= '" . $year . "'
                    union all
                    select empid,0 as ee,cr as er,0 as ec,0 as cr 
                    from paytrancurrent as pt 
                    left join paccount as pa on pa.line=pt.acnoid
                    where pa.alias = 'YPR' and month(dateid)= '" . $month . "' and year(dateid)= '" . $year . "'
                    union all
                    select empid,cr as ee,0 as er,0 as ec,0 as cr 
                    ...
                    where pa.alias = 'YPR' and month(dateid)= '" . $month . "' and year(dateid)= '" . $year . "'
                    union all
                    SELECT vtran.empid,0 as ee,0 as er,0 as ec,IFNULL(SUM(vtran.db), 0) - IFNULL(SUM(vtran.cr), 0) AS cr 
                    FROM paytrancurrent as vtran 
                    LEFT JOIN paccount ON paccount.line=vtran.acnoid 
                    WHERE month(dateid)= '" . $month . "' and year(dateid)= '" . $year . "' AND paccount.istax =1
              ...
              group by e.clientname,e.client,emp.hdmf";
  ...
  private function displayHeader($config)
  {
    ...
    $str .= $this->reporter->col('PAG-IBIG REMITTANCE REPORT', null, null, false, $border, '', '', $font, '18', 'B', '', '');
    ...
    $str .= $this->reporter->col('EMPLOYEE' . '&nbsp&nbsp', '80', null, false, $border, 'TB', 'R', $font, $font_size, 'B', '', '');
    $str .= $this->reporter->col('EMPLOYER' . '&nbsp&nbsp', '80', null, false, $border, 'TB', 'R', $font, $font_size, 'B', '', '');
    $str .= $this->reporter->col('', '80', null, false, $border, 'TB', 'C', $font, $font_size, 'B', '', '');
    ...
    $str .= $this->reporter->col('H D M F - E E', '80', null, false, $border, 'TB', 'R', $font, $font_size, 'B', '', '');
    $str .= $this->reporter->col('H D M F - E R', '80', null, false, $border, 'TB', 'R', $font, $font_size, 'B', '', '');

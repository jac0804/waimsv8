...
class pag_ibig_loan_payment_report
{
  public $modulename = 'Pag Ibig Loan Payment Report';
  ...
  public function reportDefault($config)
  {
    ...
    $query = "select e.clientname,e.client,emp.hdmf, sum(p.ee) as ee,sum(p.er) as er,sum(p.ec) as ec,sum(p.cr) as cr from
              (select empid,cr as ee,0 as er,0 as ec,0 as cr from paytrancurrent as pt left join paccount as pa on pa.line=pt.acnoid
              where pa.code ='PT11' and month(dateid)= '" . $month . "' and year(dateid)= '" . $year . "'
              union all
  ...
  private function displayHeader($config)
  {
    ...
    $str .= $this->reporter->col('PAG-IBIG LOAN PAYMENT REPORT', null, null, false, $border, '', '', $font, '18', 'B', '', '');

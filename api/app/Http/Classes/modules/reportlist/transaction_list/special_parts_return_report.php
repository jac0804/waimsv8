...
class special_parts_return_report
{
  public $modulename = 'Special Parts Return Report';
  ...
  public function default_QUERY_POSTED($config)
  {
    ...
    switch ($reporttype) {
      case '1': //detailed
        ...
        left(head.dateid,10) as dateid,stock.ref,head.rem as hrem,item.subcode,item.partno,m.model_name as model from glstock as stock
        ...
        where head.doc='si' and head.dateid between '$start' 
        and '$end' $filter
      ...
      case '0': //summary
        ...
        date(head.dateid) as dateid,stock.ref
        ...
        wh.clientname, wh.client, head.dateid,stock.ref
        order by docno $sorting";
        break;
    }
  ...
  public function default_QUERY_UNPOSTED($config)
  {
    ...
    switch ($reporttype) {
      case '1': //detailed
        $query = "select head.yourref,head.docno,head.clientname as supplier,item.barcode,item.itemname,stock.uom," . $isqty . " as iss,
        stock.isamt,stock.disc,stock.ext,wh.clientname,head.createby,stock.loc,stock.rem,
        left(head.dateid,10) as dateid,stock.ref,head.rem as hrem,item.subcode,item.partno,m.model_name as model from lastock as stock
      ...
      case '0': //summary
        $query = "select 'UNPOSTED' as status ,head.yourref,
          ...
          left(head.dateid,10) as dateid,stock.ref
          from lastock as stock
          left join lahead as head on head.trno=stock.trno
          left join cntnum on cntnum.trno=head.trno
          left join client on client.client=head.client
          left join client as wh on wh.client = head.wh
          where head.doc='si' and head.dateid between '$start' and '$end' $filter 
          group by head.docno,head.yourref,head.clientname,
          wh.clientname,head.dateid, wh.client,stock.ref
  ...
  public function header_DEFAULT($config)
  {
    ...
    $str .= $this->reporter->col('Special Parts Return Report (' . $reporttype . ')', null, null, false, $border, '', '', $font, '18', 'B', '', '') . '<br />';
    ...
    if (!empty($result)) {
      foreach ($result as $key => $data) {
        ...
        if ($docno == "" || $docno != $data->docno) {
          ...
          $str .= $this->reporter->col('Reference: ' . $data->ref, '125', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '', '2px');
          ...
          $str .= $this->reporter->begintable($layoutsize);
          $str .= $this->reporter->startrow();
          $str .= $this->reporter->col('Quantity', '49', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '', '', '');
          $str .= $this->reporter->col('UOM', '49', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '', '', '');
          ...
          $str .= $this->reporter->col('PART-NO', '100', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '', '', '');
          $str .= $this->reporter->col('Item Description', '114', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '', '', '');
          ...
          $str .= $this->reporter->col('Unit Price', '50', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '', '', '');
          $str .= $this->reporter->col('(+/-) %', '50', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '', '', '');
          $str .= $this->reporter->col('Total Price', '50', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '', '', '');
          $str .= $this->reporter->col('Location', '66', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '', '', '');
        ...
        $str .= $this->reporter->begintable($layoutsize);
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col(number_format($data->iss, 2), '49', null, false, $border, '', 'C', $font, $fontsize, '', '', '', '');
        $str .= $this->reporter->col($data->uom, '49', null, false, $border, '', 'C', $font, $fontsize, '', '', '', '');
        ...
        $str .= $this->reporter->col($data->itemname, '114', null, false, $border, '', 'L', $font, $fontsize, '', '', '', '');
        ...
        $str .= $this->reporter->col(number_format($data->ext, 2), '50', null, false, $border, '', 'C', $font, $fontsize, '', '', '', '');
        $str .= $this->reporter->col($data->loc, '66', null, false, $border, '', 'C', $font, $fontsize, '', '', '', '');

...
class packing_list_receiving_report
{
  public $modulename = 'Packing List Receiving Report';
  ...
  public function default_QUERY_DETAILED($config)
  {
    ...
    switch ($posttype) {
      case 0: // posted
        $query = "select * from ( select head.docno,supp.clientname as supplier,
      item.barcode,item.itemname,stock.uom,stock.rrqty,stock.rrcost,stock.disc,stock.ext,
      client.clientname,head.createby,stock.loc,stock.rem,left(head.dateid,10) as dateid,stock.ref,cntnum.center
      ...
      left join client as supp on supp.clientid=stock.suppid
      ...
      case 1: // unposted
        $query = "select * from (
      select head.docno,supp.clientname as supplier,
      item.barcode,item.itemname,stock.uom,stock.rrqty,stock.rrcost,stock.disc,stock.ext,
      client.clientname,head.createby,stock.loc,stock.rem,left(head.dateid,10) as dateid,stock.ref,cntnum.center
  ...
  public function default_QUERY_SUMMARIZED($config)
  {
    ...
    switch ($posttype) {
      ...
      left join client on client.clientid=stock.whid
      left join client as supp on supp.clientid=stock.suppid
      where head.doc='RP'  and date(head.dateid) between '$start' and '$end' $filter
      ) as a 
      ...
      group by docno,dateid,supplier,wh,clientname,ext,hrem ,center
      order by docno $sorting";
        break;
  ...
  public function header_DEFAULT($config)
  {
    ...
    $str .= $this->reporter->col('Packing List Receiving Report (' . $reporttype . ')', null, null, false, $border, '', '', $font, '18', 'B', '', '') . '<br />';
    ...
    if (!empty($result)) {
      foreach ($result as $key => $data) {
        ...
        if ($docno == "" || $docno != $data->docno) {
          ...
          $str .= $this->reporter->col('Barcode', '55', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '', '', '');
          $str .= $this->reporter->col('Item Description', '300', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '', '', '');

          $str .= $this->reporter->col('Price', '40', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '', '', '');
          $str .= $this->reporter->col('Discount', '40', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '', '', '');
          $str .= $this->reporter->col('Total Price', '40', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '', '', '');
        ...
        $str .= $this->reporter->begintable($layoutsize);
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col(number_format($data->rrqty, 2), '55', null, false, $border, '', 'R', $font, $fontsize, '', '', '', '');
        ...
        $str .= $this->reporter->col(number_format($data->rrcost, 2), '40', null, false, $border, '', 'R', $font, $fontsize, '', '', '', '');
        $str .= $this->reporter->col($data->disc, '40', null, false, $border, '', 'C', $font, $fontsize, '', '', '', '');
        $str .= $this->reporter->col(number_format($data->ext, 2), '40', null, false, $border, '', 'R', $font, $fontsize, '', '', '', '');
  ...
  public function reportDefaultLayout_SUMMARIZED($config)
  {
    ...
    if (!empty($result)) {
      foreach ($result as $key => $data) {
        $str .= $this->reporter->begintable($layoutsize);
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col($data->docno, '133', null, false, $border, '', 'C', $font, $fontsize, '', '', '', '');
        $str .= $this->reporter->col($data->dateid, '133', null, false, $border, '', 'C', $font, $fontsize, '', '', '', '');
        $str .= $this->reporter->col($data->supplier, '133', null, false, $border, '', 'C', $font, $fontsize, '', '', '', '');
        $str .= $this->reporter->col($data->whname, '133', null, false, $border, '', 'C', $font, $fontsize, '', '', '', '');
        $str .= $this->reporter->col(number_format($data->amount, 2), '133', null, false, $border, '', 'C', $font, $fontsize, '', '', '', '');
        $str .= $this->reporter->col($data->hrem, '133', null, false, $border, '', 'C', $font, $fontsize, '', '', '', '');
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->addline();
        ...
        if ($i == (count((array)$result) - 1)) {
          ...
          $str .= $this->reporter->col(number_format($total, 2), '150', null, false, $border, 'TB', 'C', $font, $fontsize, 'B', '', '', '');

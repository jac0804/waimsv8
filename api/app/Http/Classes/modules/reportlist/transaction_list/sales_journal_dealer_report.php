...
class sales_journal_dealer_report
{
  public $modulename = 'Sales Journal Dealer Report';
  ...
  public function default_QUERY_POSTED($config)
  {
    ...
    switch ($reporttype) {
      case '1': //detailed
        ...
        where head.doc='sd' and date(head.dateid) between '$start' 
        and '$end' $filter
    ...
    switch ($reporttype) {
      ...
      case '0': //summary
        $query = "select 'UNPOSTED' as status ,head.yourref,
          ...
          left join client as wh on wh.client = head.wh
          left join client as agent on agent.client = head.agent
          where head.doc='sd' and date(head.dateid) between '$start' and '$end' $filter 
  ...
  public function header_DEFAULT($config)
  {
    ...
    $str .= $this->reporter->col('Sales Journal Dealer Report (' . $reporttype . ')', null, null, false, $border, '', '', $font, '18', 'B', '', '') . '<br />';

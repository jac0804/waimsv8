...
class jo_list
{
    ...
    switch ($reporttype) {
      case 0: // summarized
        switch ($posttype) {
          case 0: // posted
            $query = "
          select 'POSTED' as status, head.docno, cl.clientname as supplier,
          ...
          where transnum.doc = 'JO' and date(head.dateid) between '$start' and '$end' $filter
          group by head.docno, cl.clientname,
          ...
          case 1: // unposted
            $query = "
          select 'UNPOSTED' as status, head.docno, cl.clientname as supplier,
          sum(stock.ext) as ext,
          wh.clientname, head.createby, left(head.dateid,10) as dateid
          from johead as head
      ...
      case 1: // detailed
        switch ($posttype) {
          case 0: // posted
            $query = "
          select head.docno, cl.client, cl.clientname as supplier, 
          cl.addr as address, cl.email, cl.tel, 
          cl.contact as contactperson, cl.clientname,
          ...
          head.workloc, item.barcode, item.itemname, stock.rrqty, stock.uom, 
          stock.rrcost, stock.ext, stock.disc,
          head.createby,stock.loc, stock.ref, head.rem
          from hjohead as head
          left join hjostock as stock on stock.trno = head.trno
          ...
          case 1: // unposted
            $query = "select head.docno, cl.client, cl.clientname as supplier, 
          cl.addr as address, cl.email, cl.tel, 
          cl.contact as contactperson, cl.clientname,
          prj.name as projectname, prj.code as projectcode,
          sprj.subproject as subprojectname,
  ...
  public function header_DEFAULT($config)
  {
    ...
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('JO List (' . $reporttype . ')', null, null, false, $border, '', '', $font, '18', 'B', '', '') . '<br />';
    $str .= $this->reporter->endrow();
  ...
  public function tableheader($layoutsize, $config)
  {
    ...
    $str .= $this->reporter->col('SUBCON', '300', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '', '');

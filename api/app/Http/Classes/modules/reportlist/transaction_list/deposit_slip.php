...
class deposit_slip
{
  ...
  public function default_QUERY_SUMMARIZED($config)
  {
    ...
    $query = "select docno, createby, date(dateid) as dateid, GROUP_CONCAT(distinct IF(checkno='', NULL, checkno)) as checkno, sum(db) as debit, sum(cr) as credit, rem
    from (
    ...
    if (!empty($result)) {
      foreach ($result as $key => $data) {
        if ($docno != "" && $docno != $data->docno) {
          $str .= $this->showTotal($debit, $credit, $config);
        }
  ...
  public function showTotal($debit, $credit, $config)
  {
    $str = '';
    $font = $this->companysetup->getrptfont($config['params']);
    $fontsize = "11";
  ...
  public function reportDefaultLayout_SUMMARIZED($config)
  {
    ...
    $str .= $this->summarized_headers_table($config, $layoutsize);
    $totaldb = 0;
    $totalcr = 0;
  ...
  public function summarized_unihome_header_table($config, $layoutsize)
  {
    $str = "";
    $font = $this->companysetup->getrptfont($config['params']);
    $fontsize = "10";
  ...
  public function summarized_headers_table($config, $layoutsize)
  {
    $companyid = $config['params']['companyid'];
    switch ($companyid) {
      case 17: //unihome
        return $this->summarized_unihome_header_table($config, $layoutsize);
        break;
      default:
        return $this->summarized_header_table($config, $layoutsize);
        break;
    }
  }

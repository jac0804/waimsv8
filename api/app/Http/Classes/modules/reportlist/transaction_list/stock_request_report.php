...
class stock_request_report
{
  public $modulename = 'Stock Request Report';
  ...
  {
    $fields = ['radioprint', 'start', 'end', 'ddeptname', 'wh', 'reportusers', 'approved'];
  ...
  public function paramsdata($config)
  {
    ...
    '' as username,
    '' as center,
    '' as ddeptname,'' as dept,'0' as deptid, '' as deptname,
    '' as wh,'' as whname,'' as reportusers, '' as approved
    ");
  }
    ...
    switch ($reporttype) {
      case 0: // summarized
        switch ($posttype) {
          case 0: // posted
            $query = "
          select 'POSTED' as status,head.clientname as dept,
          head.docno,sum(stock.reqqty) as reqqty,
          wh.clientname as wh,head.createby,
          date(head.dateid) as dateid
          from htrhead as head
          left join htrstock as stock on stock.trno=head.trno
          left join client as dept on dept.client=head.client
          left join client as wh on wh.client=head.wh
          where head.doc='TR'and date(head.dateid) between '$start' and '$end' $filter
          group by head.docno,head.clientname,wh.clientname,head.createby,head.dateid
          order by head.docno $sorting";
            break;

          case 1: // unposted
            $query = "select 'UNPOSTED' as status,head.clientname as dept,
          head.docno,sum(stock.reqqty) as reqqty,
          wh.clientname as wh,head.createby,
          date(head.dateid) as dateid
          from trhead as head
          left join trstock as stock on stock.trno=head.trno
          left join client as dept on dept.client=head.client
          left join client as wh on wh.client=head.wh
          where head.doc='TR'and date(head.dateid) between '$start' and '$end' $filter
          group by head.docno,head.clientname,wh.clientname,head.createby,head.dateid
      ...
      case 1: // detailed
        switch ($posttype) {
          case 0: // posted
            $query = "
          select head.clientname as dept,
          head.docno,
          item.itemname,item.barcode,stock.uom,stock.reqqty,stock.loc,stock.rem,stock.ref,
          wh.clientname as wh,head.createby,
          date(head.dateid) as dateid
          from htrhead as head
          left join htrstock as stock on stock.trno=head.trno
          ...
          case 1: // unposted
            $query = "select head.clientname as dept,
          head.docno,
          item.itemname,item.barcode,stock.uom,stock.reqqty,stock.loc,stock.rem,stock.ref,
          wh.clientname as wh,head.createby,
          date(head.dateid) as dateid
    ...
    if (!empty($result)) {
      foreach ($result as $key => $data) {
        ...
        if ($docno == "" || $docno != $data->docno) {
          ...
          $str .= $this->reporter->startrow();
          $str .= $this->reporter->col('Warehouse: ' . $data->wh, '125', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '', '8px');
          $str .= $this->reporter->col('Department: ' . $data->dept, '125', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '', '8px');
          $str .= $this->reporter->endrow();
          $str .= $this->reporter->endtable();
        ...
        $str .= $this->reporter->begintable($layoutsize);
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col(number_format($data->reqqty, 2), '100', null, false, $border, '', 'C', $font, $fontsize, '', '', '', '');
  ...
  public function reportDefaultLayout_SUMMARIZED($config)
  {
    ...
    if (!empty($result)) {
      foreach ($result as $key => $data) {
        ...
        $str .= $this->reporter->col($data->dept, '300', null, false, $border, '', 'L', $font, $fontsize, '', '', '');
        ...
        $str .= $this->reporter->col(number_format($data->reqqty, 2), '100', null, false, $border, '', 'R', $font, $fontsize, '', '', '');
        $str .= $this->reporter->col($data->status, '100', null, false, $border, '', 'C', $font, $fontsize, '', '', '');
        $totalqty = $totalqty + $data->reqqty;
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();
  ...
  public function header_DEFAULT($config)
  {
    ...
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('Stock Request Report (' . $reporttype . ')', null, null, false, $border, '', '', $font, '18', 'B', '', '') . '<br />';
    $str .= $this->reporter->endrow();
  ...
  public function tableheader($layoutsize, $config)
  {
    ...
    $str .= $this->reporter->col('DEPARTMENT', '300', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '', '');

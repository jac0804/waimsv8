...
class debit_memo_summary
{
  public $modulename = 'Debit Memo Summary';
  ...
  public function detailed_header_DEFAULT($config)
  {
    ...
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('Debit Memo Summary', null, null, false, $border, '', '', $font, '18', 'B', '', '') . '<br />';
    $str .= $this->reporter->endrow();
  ...
  public function reportDefaultLayout_SUMMARIZED($config)
  {
    ...
    if (!empty($result)) {
      foreach ($result as $key => $data) {
        ...
        $str .= $this->reporter->col($data->rem, '100', null, false, $border, '', 'R', $font, $fontsize, 'R', '', '');

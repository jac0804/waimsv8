...
class sales_order_dealer_report
{
  public $modulename = 'Sales Order Dealer Report';
    ...
    switch ($reporttype) {
      case 0: // summarized
        switch ($posttype) {
          ...
          where head.doc='SA'  and date(head.dateid) between '$start' and '$end' $filter
      ...
      case 1: // detailed
        switch ($posttype) {
          ...
          from hsastock as stock
          left join hsahead as head on head.trno=stock.trno
          left join item on item.itemid=stock.itemid
          left join transnum on transnum.trno=head.trno
          ...
          left join model_masterfile as m on m.model_id = item.model
          where head.doc='SA' and date(head.dateid) between '$start' and '$end' $filter ) as a
          ...
          client.clientname,head.createby,stock.loc,stock.rem,head.dateid,
          round((stock.iss-stock.qa)/ case when ifnull(uom.factor,0)=0 then 1 else uom.factor end,2) as qa,item.subcode,item.partno,m.model_name as model, agent.clientname as agentname
          from sastock as stock
          left join sahead as head on head.trno=stock.trno
          left join item on item.itemid=stock.itemid
          left join transnum on transnum.trno=head.trno
          left join client on client.clientid=stock.whid
          left join client as agent on agent.client = head.agent
  ...
  public function header_DEFAULT($config)
  {
    ...
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('Sales Order Dealer Report (' . $reporttype . ')', null, null, false, $border, '', '', $font, '18', 'B', '', '') . '<br />';
    $str .= $this->reporter->endrow();

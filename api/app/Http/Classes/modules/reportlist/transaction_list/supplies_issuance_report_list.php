...
class supplies_issuance_report_list
{
  public $modulename = 'Supplies Issuance Report List';
  ...
  public function default_QUERY_DETAILED($config)
  {
    ...
    switch ($posttype) {
      ...
      " . $addjoin . "
      where head.doc='RN'  and head.dateid between '$start' and '$end' $filter $filter1
  ...
  public function default_QUERY_SUMMARIZED($config)
  {
    ...
    switch ($posttype) {
      ...
      left join client as supp on supp.clientid=head.clientid
      left join client as dept on dept.clientid = head.deptid
      where head.doc='RN'  and date(head.dateid) between '$start' and '$end' $filter $filter1
      ) as a 
  ...
  public function header_DEFAULT($config)
  {
    ...
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('Supplies Issuance Report List (' . $reporttype . ')', null, null, false, $border, '', '', $font, '18', 'B', '', '') . '<br />';
    $str .= $this->reporter->endrow();

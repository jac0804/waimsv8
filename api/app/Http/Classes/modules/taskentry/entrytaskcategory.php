...
class entrytaskcategory
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'CATEGORY MASTERFILE';
  ...
  private $fields = ['category', 'istaskcat'];
  ...
  public function createTab($config)
  {
    ...
    $obj[0][$this->gridname]['columns'][1]['style'] = 'width:125px;whiteSpace: normal;min-width:150px;';

    $obj[0][$this->gridname]['columns'][1]['label'] = 'Task Category';
        ...
        if ($data[$key]['line'] == 0 && $data[$key]['category'] != '') {
          ...
          if (!empty($resultdata[0]['category'])) {
            if (trim($resultdata[0]['category']) == trim($data[$key]['category'])) {
              return ['status' => false, 'msg' => ' Task ( ' . $resultdata[0]['category'] . ' )' . '  already exist', 'data' => [$resultdata]];
            }
          }
        }
          ...
          if ($data[$key]['line'] != 0 && $data[$key]['category'] != '') {
            ...
            if (!empty($resultdata[0]['category'])) {
              if (trim($resultdata[0]['category']) == trim($data[$key]['category'])) {
                ...
                return ['status' => false, 'msg' => ' Task ( ' . $resultdata[0]['category'] . ' )' . ' is already exist', 'data' => [$resultdata], 'rowid' => [$data[$key]['line']  . ' -- ' . $resultdata[0]['line']]];
              } else {
                update:
    ...
    if ($row['line'] == 0 && $row['category'] != '') {
      ...
      if (!empty($resultdata[0]['category'])) {
        if (trim($resultdata[0]['category']) == trim($row['category'])) {
          return ['status' => false, 'msg' => ' Task Category - ( ' . $resultdata[0]['category'] . ' )' . ' is already exist', 'data' => [$resultdata]];
        }
      }
    }
      ...
      if ($row['category'] != 0 || $row['category'] != '') {
        ...
        if (!empty($resultdata[0]['category'])) {
          if (trim($resultdata[0]['category']) == trim($row['category'])) {
            ...
            return ['status' => false, 'msg' => ' Task ( ' . $resultdata[0]['category'] . ' )' . ' is already exist', 'data' => [$resultdata], 'rowid' => [$row['line']  . ' -- ' . $resultdata[0]['line']]];
          } else {
            update:
  ...
  private function loaddataperrecord($line)
  {
    ...
    $qry = "select line,category,istaskcat,'' as bgcolor from " . $this->table . " as r where r.istaskcat =1 and r.line=?";
  ...
  public function loaddata($config)
  {
    ...
    $qry = "select line,category,istaskcat,'' as bgcolor from " . $this->table . " as r  where r.istaskcat =1  " . $filtersearch . " order by line $l";
  ...
  private function report_default_query($config)
  {
    ...
    $query = "select line, category, reqtype from reqcategory where istaskcat=1
      order by line";
  ...
  public function reportdata($config)
  {
    ...
    if ($config['params']['dataparams']['print'] == "default") {
      $str = $this->rpt_taskcategory_masterfile_layout($data, $config);
    } else if ($config['params']['dataparams']['print'] == "PDFM") {
      $str = $this->rpt_taskcategory_PDF($data, $config);
    }
  ...
  private function rpt_default_header($data, $filters)
  {
    ...
    $str .= $this->reporter->begintable('800');
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('TASK CATEGORY MASTERFILE', '800', null, false, '1px solid ', '', 'L', 'Century Gothic', '18', 'B', '', '');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();
  ...
  private function rpt_taskcategory_masterfile_layout($data, $filters)
  {
    $companyid = $filters['params']['companyid'];
    $decimal = $this->companysetup->getdecimal('currency', $filters['params']);
  ...
  private function rpt_class_PDF_header_PDF($data, $filters)
  {
    ...
    PDF::MultiCell(600, 20, "Task Category", '', 'L', false, 0);
  ...
  private function rpt_taskcategory_PDF($data, $filters)
  {
    $companyid = $filters['params']['companyid'];
    $decimal = $this->companysetup->getdecimal('currency', $filters['params']);

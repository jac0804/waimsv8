...
class historicalcomments
{
  ...
  public function createTab($config)
  {
    ...
    $obj[0][$this->gridname]['columns'][$seendate]['type'] = 'label';
    return $obj;
  }
  ...
  public function loaddata($config)
  {
    ...
    if ($trno != 0 && $line != 0) {
      ...
      $qry = "select ifnull(pr.rem,'') as rem, cl.clientname as createby, pr.createdate, pr.seendate from headprrem as pr left join client as cl on cl.email = pr.createby 
      where pr.tmtrno=$trno and pr.tmline=$line
              union all
              select concat('Solution Remarks: ','\r\n',hd.rem1) as rem,  cl.clientname as createby,  hd.donedate as createdate,'' as seendate
              from hdailytask as hd
              left join client as cl on cl.clientid = hd.userid
              where hd.tasktrno = $trno and hd.taskline = $line and hd.rem1 <> ''
      ...
      switch ($otherTrnoField) {
        case 'dytrno':
          ...
          $origtrno = isset($origtrno) ? $origtrno : 0;

          if ($origTrno != 0) { //kapag may refx
            $qry = "select ifnull(pr.rem,'') as rem,cl.clientname as createby,pr.createdate,pr.seendate from headprrem as pr left join client as cl on cl.email = pr.createby
                  where pr." . $otherTrnoField . "<>0 and pr." . $otherTrnoField . " in ($otherTrnoVal,$origTrno)
                  union all
                  select concat('Solution Remarks: ','\r\n',hd.rem1) as rem,  cl.clientname as createby,  hd.donedate as createdate,'' as seendate
                  from hdailytask as hd
                  left join client as cl on cl.clientid = hd.userid
                  where (hd.trno= $origTrno or hd.refx = $origTrno) and hd.rem1 <> ''
                  order by createdate asc";
          } else { //kpag bago wala refx
            $qry = "select ifnull(pr.rem,'') as rem,cl.clientname as createby,pr.createdate,pr.seendate from headprrem as pr left join client as cl on cl.email = pr.createby
                  where pr." . $otherTrnoField . "<>0 and pr." . $otherTrnoField . " =$otherTrnoVal
                  union all
                  select concat('Solution Remarks: ','',hd.rem1) as rem,  cl.clientname as createby,  hd.donedate as createdate,'' as seendate
                  from hdailytask as hd
                  left join client as cl on cl.clientid = hd.userid
                  where hd.trno= $otherTrnoVal  and hd.rem1 <> ''
                  order by createdate asc";
          }
          $data = $this->coreFunctions->opentable($qry);
          return $data;

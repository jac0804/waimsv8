...
class allowancesetup
{
  private $btnClass;
  private $fieldClass;
  private $tabClass;
  public $modulename = 'ALLOWANCE SETUP';
  ...
  public function loaddoclisting($config)
  {
    ...
    if (isset($config['params']['search'])) {
      $searchfield = ['e.empid', 'c.client', 'e.empid', 'c.client', 'e.emplast', 'e.empfirst', 'e.empmiddle'];
  ...
  {
    $tab = ['tableentry' => ['action' => 'payrollentry', 'lookupclass' => 'viewallowancesetupdetails', 'label' => 'DETAILS']];
    $stockbuttons = [];
  ...
  public function createHeadField($config)
  {
    ...
    data_set($col2, 'tbasicrate.name', 'allowance');
    data_set($col2, 'tbasicrate.label', 'Allowance');
  ...
  public function updatehead($config, $isupdate)
  {
    ...
    if ($type == '') {
      return ['status' => false, 'msg' => 'Please setup Allowance RATE first.', 'clientid' => $clientid];
    }

    if ($allowance != 0) {
      $sql = "update allowsetup  set dateend='" . $effdate . "'

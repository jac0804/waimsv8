...
class ti
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'TRIPPING INCENTIVE';
  ...
  public function unposttrans($config)
  {
    ...
    $applied = $this->coreFunctions->opentable("select trno from htripdetail where titrno=" . $trno);
    if ($applied) {
      return ['trno' => $trno, 'status' => false, 'msg' => 'Unable to unpost. Already applied in payroll'];
    }
  ...
  private function getstockselect($config, $loadtrip = false)
  {
    $trno = $config['params']['trno'];

    //false, load specific
    $filter = 'and t.titrno=' . $trno;
    $start = $this->coreFunctions->datareader("select date_format(head.start,'%Y-%m-%d') as value from " . $this->head . " as head where trno=$trno limit 1");
    $end = $this->coreFunctions->datareader("select date_format(head.enddate,'%Y-%m-%d') as value from " . $this->head . " as head where trno=$trno limit 1");

    if ($loadtrip) { //titrno is 0 and all trip under start/end
      $filter = " and titrno=0 and date(head.dateid) between '$start' and '$end' and hinfo.receivedate is not null";
    }

    $qry = "select t.trno,t.line,t.itemid,t.clientid,t.activity,t.titrno,date(head.dateid) as dateid,
    c.client,c.clientname as empname,ifnull(j.jobtitle,'') as jobtitle,t.rate as amt,
    '' as bgcolor, t.rate as ext,hinfo.receivedate,hinfo.receiveby,b.batch
    from htripdetail as t
    left join glhead as head on head.trno=t.trno
    ...
    left join hcntnuminfo as hinfo on hinfo.trno=t.trno
    left join batch as b on b.line=t.batchid
    where 1=1 " . $filter;
  ...
  public function loadtrip($config)
  {
    $trno = $config['params']['trno'];
    ...
    if ($islocked) {
      ...
      foreach ($trip as $key => $data) {
        $this->coreFunctions->sbcupdate('htripdetail', ['titrno' => $trno], ['trno' =>  $data->trno, 'line' =>  $data->line]);
      }
    }


    return ['status' => true, 'reloaddata' => true, 'msg' => 'Load Trip Details', 'griddata' => ['inventory' => $trip]];
  }
  ...
  {
    $trno = $config['params']['trno'];
    $this->coreFunctions->execqry("update htripdetail set titrno =0  where titrno=$trno", 'update');
  ...
  public function deleteitem($config)
  {
    ...
    $this->coreFunctions->execqry("update htripdetail set titrno =0  where trno=$trno and line=$line", 'update');
    $this->othersClass->logConsole("update htripdetail set titrno =0  where trno=$trno and line=$line");



    $data = $this->opendetail($config);
    return  ['status' => true, 'griddata' => ['inventory' => $data],  'msg' => 'Trip Deleted'];
  } // end function

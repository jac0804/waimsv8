...
class st
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'Transfer Slip - Materials';
  ...
  private $fields = ['trno', 'docno', 'dateid', 'due', 'client', 'clientname', 'yourref', 'ourref', 'rem', 'wh','pdtrno'];
  ...
  public function createTab($access, $config)
  {
    ...
    $columns = ['action', 'isqty', 'uom', 'isamt',  'ext', 'wh', 'ref', 'rem', 'loc', 'expiry', 'itemname', 'barcode'];
  ...
  public function createHeadField($config)
  {
    ...
    data_set($col1, 'sodocno.label', 'Job Order #');
    data_set($col1, 'sodocno.class', 'cssodocno sbccsreadonly');
    data_set($col1, 'sodocno.action', 'pendingjodetail');
    data_set($col1, 'sodocno.lookupclass', 'pendingjoheaddetail');
    data_set($col1, 'sodocno.readonly', true);

    data_set($col1, 'itemname.class', 'sbccsreadonly');
    data_set($col1, 'itemname.label', 'Itemname');
    data_set($col1, 'barcode.class', 'sbccsreadonly');
    data_set($col1, 'barcode.type', 'input');

    $fields = [['qty', 'uom'],'dateid', 'client', 'fname'];
    $col2 = $this->fieldClass->create($fields);
    data_set($col2, 'client.label', 'Destination Code');
    data_set($col2, 'client.lookupclass', 'whonly');
    data_set($col2, 'fname.label', 'Destination Name');
    data_set($col2, 'fname.class', 'sbccsreadonly');

    data_set($col2, 'uom.class', 'sbccsreadonly');
    data_set($col2, 'qty.class', 'sbccsreadonly');

    $fields = [['yourref', 'ourref'], 'wh', 'whname'];
    $col3 = $this->fieldClass->create($fields);
    data_set($col3, 'wh.label', 'Source Warehouse Code');
    data_set($col3, 'whname.label', 'Source Warehouse Name');
    data_set($col3, 'whname.class', 'sbccsreadonly');
  ...
  public function loadheaddata($config)
  {
    ...
    $qryselect = "select 
         ...
         head.trno, 
         head.docno,
         client.client, client.clientname as fname,
         head.terms,
         head.cur,
         head.forex,
         head.yourref,
    ...
    foreach ($data2 as $key => $value) {
      if ($data2[$key][$this->dqty] == 0) {
        ...
        if ($data[$key]->refx == 0) {
          ...
          $msg2 = ' Qty transferred is Greater than JO Qty ';
        }
      }
    }
  ...
  public function setserveditems($refx, $linex)
  {
    ...
    stock on stock.trno=head.trno where head.doc='ST'  and stock.refx=" . $refx . " and stock.linex=" . $linex;

    $qry1 = $qry1 . " union all select glstock." . $this->hqty . " from glhead left join glstock on glstock.trno=
    glhead.trno where glhead.doc='ST'  and glstock.refx=" . $refx . " and glstock.linex=" . $linex;
    ...
    return $this->coreFunctions->execqry("update hpdstock set tsqa=" . $qty . " where trno=" . $refx . " and line=" . $linex, 'update');
  }

...
class oq
{
  private $modulename = "Oracle Request Code";
  ...
  public function createreportfilter($config)
  {
    ...
    data_set($col1, 'radioreporttype.options', [
      ...
      ['label' => 'OVP Format', 'value' => '2', 'color' => 'green'],
    ]);
  ...
  {
    $query = "select date(head.dateid) as dateid, head.trno,stock.line, head.docno, head.rem, stock.oraclecode, item.itemname, info.itemdesc, 
        ...
        UNION ALL
        select date(head.dateid) as dateid,head.trno,stock.line, head.docno, head.rem, stock.oraclecode, item.itemname, info.itemdesc, 
        stock.rrqty, stock.uom, stock.rrcost, stock.ext, stock.disc, hinfo.inspo,pr.clientname as customer
        from hoqhead as head left join hoqstock as stock on stock.trno=head.trno
        left join client on client.client=head.client
        left join item on item.itemid = stock.itemid
  ...
  public function reportplotting($params, $data)
  {
    if ($params['params']['dataparams']['print'] == "default") {
      ...
      switch ($format) {
        case '2':
          return $this->default_PO_PDF_OVP($params, $data);
          break;
        default:
      ...
      $format = $params['params']['dataparams']['reporttype'];
      if ($format == '2') {
        return $this->excel_po_ovp_layout($params, $data);
      }
    }
  }
  ...
  public function default_PO_header_PDF_OVP($params, $data)
  {
    $companyid = $params['params']['companyid'];
    $center = $params['params']['center'];
    $username = $params['params']['user'];
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(200, 0, (isset($data[0]['docno']) ? $data[0]['docno'] : ''), 'B', 'L', false, 0, '',  '');
    PDF::MultiCell(270, 0, "", '', 'C', false, 0);
    PDF::SetFont($fontbold, '', $fontsize);
    ...
    PDF::MultiCell(200, 0, "ORACLE CODE", '', 'C', false, 0);
    PDF::MultiCell(225, 0, "ITEM NAME", '', 'L', false, 0);
    PDF::MultiCell(275, 0, "DESCRIPTION", '', 'L', false, 1);

    PDF::SetFont($font, '', 5);
    PDF::MultiCell(700, 0, '', 'B');
  }

  public function default_PO_PDF_OVP($params, $data)
  {
    $companyid = $params['params']['companyid'];
    ...
    $this->default_PO_header_PDF_OVP($params, $data);
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $arr_barcode = $this->reporter->fixcolumn([$barcode], '40', 0);
      $arr_itemname = $this->reporter->fixcolumn([$itemname], '32', 0);
      $arr_itemdesc = $this->reporter->fixcolumn([$itemdesc], '32', 0);

      $maxrow = $this->othersClass->getmaxcolumn([$arr_barcode, $arr_itemname, $arr_itemdesc]);

      for ($r = 0; $r < $maxrow; $r++) {
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(200, 15, ' ' . (isset($arr_barcode[$r]) ? $arr_barcode[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
        PDF::MultiCell(225, 15, ' ' . (isset($arr_itemname[$r]) ? $arr_itemname[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
        PDF::MultiCell(275, 15, ' ' . (isset($arr_itemdesc[$r]) ? $arr_itemdesc[$r] : ''), '', 'L', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
      }
  ...
  public function excel_po_ovp_layout($params, $data)
  {
    $companyid = $params['params']['companyid'];
    $center = $params['params']['center'];
    $username = $params['params']['user'];
    ...
    $str .= $this->reporter->begintable('800');
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('TRNO', '50', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '', '');
    $str .= $this->reporter->col('LINE', '50', null, false, $border, 'BL', 'C', $font, $fontsize, 'B', '', '');
    $str .= $this->reporter->col('CUSTOMER', '300', null, false, $border, 'BL', 'C', $font, $fontsize, 'B', '', '');
    $str .= $this->reporter->col('ORACLE CODE', '100', null, false, $border, 'BL', 'C', $font, $fontsize, 'B', '', '');
    $str .= $this->reporter->col('DESCRIPTION', '300', null, false, $border, 'BL', 'C', $font, $fontsize, 'B', '', '');
    $str .= $this->reporter->col('REMARKS', '200', null, false, $border, 'BLR', 'C', $font, $fontsize, 'B', '', '');
    $str .= $this->reporter->endrow();

    for ($i = 0; $i < count($data); $i++) {
      $str .= $this->reporter->startrow();
      $str .= $this->reporter->addline();
      $str .= $this->reporter->col($data[$i]['trno'], '50', null, false, $border, 'B', 'C', $font, $fontsize, '', '', '');
      $str .= $this->reporter->col($data[$i]['line'], '50', null, false, $border, 'BL', 'L', $font, $fontsize, '', '', '');
      $str .= $this->reporter->col($data[$i]['customer'], '300', null, false, $border, 'BL', 'L', $font, $fontsize, '', '', '');
      $str .= $this->reporter->col($data[$i]['oraclecode'], '100', null, false, $border, 'BL', 'C', $font, $fontsize, '', '', '');
      $str .= $this->reporter->col($data[$i]['itemdesc'], '300', null, false, $border, 'BL', 'L', $font, $fontsize, '', '', '');
      $str .= $this->reporter->col('', '200', null, false, $border, 'BLR', 'C', $font, $fontsize, '', '', '');
      $str .= $this->reporter->endrow();
    }
    $str .= $this->reporter->endtable();
    $str .= $this->reporter->endreport();
  ...
  public function default_PO_PDF($params, $data)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $arr_barcode = $this->reporter->fixcolumn([$barcode], '35', 0);
      ...
      $arr_amt = $this->reporter->fixcolumn([$netamt], '10', 0);
      ...
      for ($r = 0; $r < $maxrow; $r++) {
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(170, 15, ' ' . (isset($arr_barcode[$r]) ? $arr_barcode[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
  ...
  public function default_PO_header_PDF($params, $data)
  {
    ...
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(50, 0, "Type: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(100, 0, (isset($data[0]['inspo']) ? $data[0]['inspo'] : ''), 'B', 'L', false, 0, '',  '');
    ...
    PDF::SetFont($font, 'B', 12);
    PDF::MultiCell(170, 0, "ORACLE CODE", '', 'C', false, 0);

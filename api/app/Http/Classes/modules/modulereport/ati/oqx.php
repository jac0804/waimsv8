...
class oq
{
  ...
  {
    $query = "select date(head.dateid) as dateid, head.docno, head.rem, stock.oraclecode, item.itemname, info.itemdesc, 
        stock.rrqty, stock.uom, stock.rrcost, stock.ext, stock.disc, hinfo.inspo
        ...
        UNION ALL
        select date(head.dateid) as dateid, head.docno, head.rem, stock.oraclecode, item.itemname, info.itemdesc, 
        stock.rrqty, stock.uom, stock.rrcost, stock.ext, stock.disc, hinfo.inspo
  ...
  public function default_PO_header_PDF_OVP($params, $data)
  {
    ...
    PDF::MultiCell(150, 0, "ORACLE CODE", '', 'C', false, 0);
    PDF::MultiCell(275, 0, "ITEM NAME", '', 'L', false, 0);
  ...
  public function default_PO_PDF_OVP($params, $data)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      for ($r = 0; $r < $maxrow; $r++) {
        ...
        PDF::MultiCell(275, 15, ' ' . (isset($arr_itemname[$r]) ? $arr_itemname[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
  ...
  public function default_PO_header_PDF($params, $data)
  {
    ...
    PDF::SetFont($font, 'B', 12);
    PDF::MultiCell(100, 0, "ORACLE CODE", '', 'C', false, 0);

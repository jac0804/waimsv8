...
class pr
{
  ...
  public function report_default_query($trno)
  {
    ...
    $query = "select date(head.dateid) as dateid, head.docno, client.client, client.clientname, head.address, head.terms,head.rem, item.barcode,
        item.itemname, stock.rrqty as qty, stock.rrcost as netamt, stock.disc, stock.ext,item.sizeid, dept.clientname as deptname, 
        hinfo.prepared, sano.sano, reqcat.category, sinfo.itemdesc, sinfo.unit as uom, sinfo.purpose, sinfo.specs
        ...
        left join clientsano as sano on sano.line=head.sano
        left join reqcategory as reqcat on reqcat.line=head.ourref
        left join stockinfotrans as sinfo on sinfo.trno=head.trno
        ...
        left join clientsano as sano on sano.line=head.sano
        left join reqcategory as reqcat on reqcat.line=head.ourref
        left join hstockinfotrans as sinfo on sinfo.trno=head.trno
        where head.doc='PR' and head.trno='$trno'";
  ...
  private function reportheader($params, $data)
  {
    ...
    $str .= $this->reporter->begintable('800');
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('DEPARTMENT : ', '80', null, false, $border, '', 'L', $font, '12', 'B', '30px', '4px');
    $str .= $this->reporter->col((isset($data[0]['deptname']) ? $data[0]['deptname'] : ''), '520', null, false, $border, 'B', 'L', $font, '12', '', '30px', '4px');
    $str .= $this->reporter->col('PREPARED BY : ', '60', null, false, $border, '', 'L', $font, '12', 'B', '', '');
  ...
  public function default_PR_header_PDF($params, $data)
  {
    ...
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(100, 0, "Date Requested: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(120, 0, (isset($data[0]['dateid']) ? $data[0]['dateid'] : ''), 'B', 'L', false);

    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(80, 0, "Department: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(400, 0, (isset($data[0]['deptname']) ? $data[0]['deptname'] : ''), 'B', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(100, 0, "Prepared by: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(120, 0, (isset($data[0]['prepared']) ? $data[0]['prepared'] : ''), 'B', 'L', false, 0, '',  '');

    PDF::MultiCell(0, 0, "\n");

    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(80, 0, "SA No: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(400, 0, (isset($data[0]['sano']) ? $data[0]['sano'] : ''), 'B', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(100, 0, "Category: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(120, 0, (isset($data[0]['category']) ? $data[0]['category'] : ''), 'B', 'L', false, 0, '',  '');
    ...
    PDF::MultiCell(200, 0, "SPECIFICATION", '', 'L', false, 0);
    PDF::MultiCell(100, 0, "PURPOSE", '', 'L', false);
  ...
  public function default_PR_PDF($params, $data)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $itemname = $this->reporter->fixcolumn([$data[$i]['itemdesc']], '100', 0);
      $maxrow = 1;
      ...
      if ($data[$i]['itemdesc'] == '') {
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(200, 0, $data[$i]['specs'], '', 'L', false, 0, '', '', false, 1);
          PDF::MultiCell(100, 0, $data[$i]['purpose'], '', 'L', false, 1, '', '', false, 0);
        }
      }

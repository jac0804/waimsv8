...
class po
{
  ...
  public function report_default_query($trno)
  {
    $query = "select date(head.dateid) as dateid, head.docno, client.client, client.clientname, head.address,
        ...
        if(stock.rem != '',    concat(item.itemname, ' - ', stock.rem), item.itemname) as itemname, stock.rrqty as qty, stock.uom, stock.rrcost as netamt, stock.disc, 
        stock.ext,m.model_name as model,item.sizeid,center.shortname,wh.clientname as whname,head.yourref,client.tel
  ...
  public function default_po_layout($params, $data)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $str .= $this->reporter->col($data[$i]['itemname'], '525', null, false, $border, '', 'L', $font, $fontsize, '', '', '2px');//475
  ...
  public function default_PO_header_PDF($params, $data)
  {
    ...
    PDF::SetFont($fontbold, '', 18);
    PDF::MultiCell(430, 0, "", '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
    PDF::SetFont($font, '', 12);
    PDF::MultiCell(90, 0, "Document # : ", '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
    PDF::MultiCell(5, 0, "", '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
    PDF::SetFont($fontbold, '', 15);
    PDF::MultiCell(175, 0, (isset($data[0]['docno']) ? $data[0]['docno'] : ''), 'B', 'L', false, 1, '',  '', true, 0, false, true, 0, 'B', true);
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(70, 20, "WH Pick Up", '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(10, 20, ': ', '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
    ...
    PDF::SetFont($font, 'B', $fontsize);
        ...
        PDF::MultiCell(60, 25, "QTY", 'TB', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', true);
        PDF::MultiCell(60, 25, "UNIT", 'TB', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', true);
        PDF::MultiCell(355, 25, "DESCRIPTION", 'TB', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', true);//250
  ...
  public function default_PO_PDF($params, $data)
  {
    ...
    $this->po_new_header($params, $data);
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      for ($r = 0; $r < $maxrow; $r++) {
        PDF::SetFont($font, '', $fontsize);
            ...
            PDF::MultiCell(100, 15, ' ' . (isset($arr_disc[$r]) ? $arr_disc[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
    ...
    PDF::MultiCell(700, 0, '', 'B');
        ...
        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(600, 15, 'GRAND TOTAL: ', 'B', 'R', false, 0);
        PDF::MultiCell(100, 15, number_format($totalext, $decimalcurr), 'B', 'R');
   ...
   public function po_new_header($params, $data)
  {
    $center = $params['params']['center'];
    $username = $params['params']['user'];
    ...
    PDF::MultiCell(10, 20, ":", '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(440, 20, (isset($data[0]['whname']) ? $data[0]['whname'] : ''), 'B', 'L', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(80, 20, "REFERENCE : ", '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(100, 20, (isset($data[0]['yourref']) ? $data[0]['yourref'] : ''), 'B', 'L', false, 1, '',  '', true, 0, false, true, 0, 'B', true);
    ...
    PDF::MultiCell(700, 0, '', 'T');
              ...
              PDF::MultiCell(290, 0, "DESCRIPTION", '', 'L', false, 0);

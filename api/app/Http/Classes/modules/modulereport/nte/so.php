...
class so
{
  ...
  public function report_default_query($trno)
  {
    $query = "select head.rtype,head.rdate,cust.tel2,cust.email,head.docno,head.trno, head.clientname, head.address, 
      ...
      item.sizeid,m.model_name as model,date(head.due) as due,head.yourref, stockinfo.itemdesc
  ...
  public function default_so_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $maxrow = $this->othersClass->getmaxcolumn([$arr_itemname, $arr_qty, $arr_uom, $arr_gross, $arr_disc, $arr_ext]);
        for ($r = 0; $r < $maxrow; $r++) {

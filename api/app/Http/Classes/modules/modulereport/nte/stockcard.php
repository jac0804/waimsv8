...
class stockcard
{
  ...
  public function PDF_LEDGER_HEADER($config, $data)
  {
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(390, 0, $barcode, '', 'L', false, 0);
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(620, 0, $itemname, '', 'L', false);
    ...
    PDF::MultiCell(170, 0, "CLIENTNAME", 'TLB', 'C', false, 0);
    PDF::MultiCell(70, 0, "EXPIRY", 'TLB', 'C', false, 0);
    PDF::MultiCell(100, 0, "DOCUMENT#", 'TLB', 'C', false, 0);
    PDF::MultiCell(80, 0, "QTY IN", 'TLB', 'C', false, 0);
    PDF::MultiCell(80, 0, "QTY OUT", 'TBL', 'C', false, 0);
    PDF::MultiCell(80, 0, "BALANCE", 'TLB', 'C', false, 0);
  ...
  public function report_PDF_LEDGER($config, $data)
  {
    ...
    foreach ($data as $key => $data) {
      ...
      $arr_dateid = $this->reporter->fixcolumn([$data->dateid], '10', 0);
      $arr_clientname = $this->reporter->fixcolumn([$data->clientname], '31', 0);
      $arr_expiry = $this->reporter->fixcolumn([$data->expiry], '10', 0);
      $arr_docno = $this->reporter->fixcolumn([$data->docno], '15', 0);
      $arr_qty = $this->reporter->fixcolumn([$qty == 0 ? '-' : number_format($qty, $qtydec)], '14', 2);
      $arr_iss = $this->reporter->fixcolumn([$iss == 0 ? '-' : number_format($iss, $qtydec)], '14', 2);
      $arr_tobal = $this->reporter->fixcolumn([number_format($tobal, $qtydec)], '14', 2);
      $arr_rem = $this->reporter->fixcolumn([$data->rem], '14', 2);

      $maxrow = $this->othersClass->getmaxcolumn([$arr_dateid, $arr_clientname, $arr_expiry, $arr_docno, $arr_qty, $arr_iss, $arr_tobal, $arr_rem]);
      for ($j = 0; $j < $maxrow; $j++) {
        if ($j == 0 && $data->docno == 'beginning bal.') {
          PDF::SetFont($font, '', 11);
          PDF::MultiCell(70, 0, "", '', 'C', false, 0);
          PDF::MultiCell(170, 0, $data->docno, '', 'L', false, 0);
          PDF::MultiCell(70, 0, "", '', 'L', false, 0);
          PDF::MultiCell(100, 0, "", '', 'L', false, 0);
          PDF::MultiCell(80, 0, "", '', 'R', false, 0);
          PDF::MultiCell(80, 0, "", '', 'R', false, 0);
          PDF::MultiCell(80, 0, number_format($data->bal, 2), '', 'R', false, 0);
          PDF::MultiCell(70, 0, "", '', 'R', false);
        } else {
          PDF::SetFont($font, '', 11);
          PDF::MultiCell(70, 0, isset($arr_dateid[$j]) ? $arr_dateid[$j] : '', '', 'C', false, 0);
          PDF::MultiCell(170, 0, isset($arr_clientname[$j]) ? $arr_clientname[$j] : '', '', 'L', false, 0);
          PDF::MultiCell(70, 0, isset($arr_expiry[$j]) ? $arr_expiry[$j] : '', '', 'C', false, 0);
          PDF::MultiCell(100, 0, isset($arr_docno[$j]) ? $arr_docno[$j] : '', '', 'C', false, 0);
          PDF::MultiCell(80, 0, isset($arr_qty[$j]) ? $arr_qty[$j] : '', '', 'R', false, 0);
          PDF::MultiCell(80, 0, isset($arr_iss[$j]) ? $arr_iss[$j] : '', '', 'R', false, 0);
          PDF::MultiCell(80, 0, isset($arr_tobal[$j]) ? $arr_tobal[$j] : '', '', 'R', false, 0);
          PDF::MultiCell(70, 0, isset($arr_rem[$j]) ? $arr_rem[$j] : '', '', 'L', false);
        } //end if
      }
    ...
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(70, 0, "", '', 'C', false, 0);
    PDF::MultiCell(170, 0, "", '', 'C', false, 0);
    PDF::MultiCell(70, 0, "", '', 'C', false, 0);
    PDF::MultiCell(100, 0, 'TOTAL QTY : ', '', 'R', false, 0);
    PDF::MultiCell(80, 0, number_format($totalqty, $qtydec), '', 'R', false, 0);
    PDF::MultiCell(80, 0, number_format($totaliss, $qtydec), '', 'R', false, 0);
    PDF::MultiCell(80, 0, number_format($tobal, $qtydec), '', 'R', false, 0);
    PDF::MultiCell(70, 0, "", '', 'R', false);
    PDF::MultiCell(0, 0, "\n\n\n");

...
class ar
{
  ...
  public function createreportfilter($config)
  {
    ...
    data_set($col1, 'radioreporttype.options', [
      ...
      ['label' => 'Billing', 'value' => 'billing', 'color' => 'red'],
  ...
  public function reportplotting($params, $data)
  {
    if ($params['params']['dataparams']['print'] == "default") {
      ...
      if ($params['params']['dataparams']['reporttype'] == "default") {
        return $this->default_AR_PDF($params, $data);
      } else {
        return $this->billing_AR_PDF($params, $data);
      }
    }
  }
  ...
  public function default_query($config)
  {
    ...
    $qry = "
      ...
      detail.cr, detail.client as dclient, detail.checkno,proj.name as project
      from lahead as head 
      ...
      coa.acno, coa.acnoname, detail.ref, 
      date(detail.postdate) as postdate, detail.db, 
      detail.cr, dclient.client as dclient, detail.checkno,proj.name as project
      from glhead as head 
  ...
  public function billing_query($config)
  {
    ...
    $qry = "
      ...
      left join coa on coa.acnoid=detail.acnoid
      left join projectmasterfile as proj on proj.line=head.projectid
      where head.doc='ar' and detail.db <> 0 and head.trno='$trno'
      union all
      select head.rem, detail.rem as remarks, 
      date(head.dateid) as dateid, head.docno, 
      client.client, client.clientname, head.address, 
  ...
  public function billing_AR_header_PDF($params, $data)
  {
    $center = $params['params']['center'];
    $username = $params['params']['user'];
    ...
    PDF::MultiCell(520, 0, "Billing", '', 'L', false, 0, '',  '100');
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(100, 0, (isset($data[0]['project']) ? $data[0]['project'] : ''), 'B', 'L', false, 1, '',  '');
    ...
    PDF::MultiCell(520, 0, "DESCRIPTION", '', 'L', false, 0);
    PDF::MultiCell(200, 0, "AMOUNT", '', 'C', false);

    PDF::SetFont($font, '', 5);
    PDF::MultiCell(720, 0, '', 'B');
  }

  public function billing_AR_PDF($params, $data)
  {
    ...
    $this->billing_AR_header_PDF($params, $data);
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $debit = $debit < 0 ? '-' : $debit;
        $arr_remarks = $this->reporter->fixcolumn([$remarks], '50', 0);
        $arr_debit = $this->reporter->fixcolumn([$debit], '15', 0);
        $maxrow = $this->othersClass->getmaxcolumn([$arr_remarks, $arr_debit]);

        for ($r = 0; $r < $maxrow; $r++) {
          PDF::SetFont($font, '', $fontsize);
          PDF::MultiCell(520, 15, ' ' . (isset($arr_remarks[$r]) ? $arr_remarks[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(200, 15, ' ' . (isset($arr_debit[$r]) ? $arr_debit[$r] : ''), '', 'R', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
        }
    ...
    PDF::MultiCell(520, 0, 'Amount in Words : ' . $this->reporter->ftNumberToWordsConverter($totaldb) . ' ONLY', '', 'L', false, 0);
    PDF::MultiCell(200, 0, 'TOTAL AMOUNT DUE:' . number_format($totaldb, 2), '', 'R', false);

    PDF::MultiCell(0, 0, "\n\n");

    $center = $params['params']['center'];
    $qry = "select code,upper(name) as name,address,tel,accountno from center where code = '" . $center . "'";
    $companydata = json_decode(json_encode($this->coreFunctions->opentable($qry)), true);

    PDF::SetFont($fontitalicbold, '', $fontsize);
    PDF::MultiCell(720, 0, 'For payments through bank deposits and online transfers:', '', 'L', false);
    PDF::MultiCell(720, 0, $companydata[0]['name'], '', 'L', false);
    PDF::MultiCell(720, 0, $companydata[0]['accountno'] . ' and email and your transaction slip to ' . $companydata[0]['tel'], '', 'L', false);

...
class gp
{
  ...
  private $modulename = "Gatepass OUT";
  private $fieldClass;
  ...
  public function report_default_query($trno)
  {
    $query = "select date(head.dateid) as dateid, head.docno, client.client, client.clientname, head.address,
        ...
        item.itemname, stock.isqty as qty, stock.uom, stock.isamt as netamt, stock.disc, stock.ext,m.model_name as model,item.sizeid,
        iteminfo.serialno
        from gphead as head 
        ...
        where head.doc='GP' and head.trno='$trno'
        union all
  ...
  public function default_PO_header_PDF($params, $data)
  {
    ...
    PDF::MultiCell(100, 0, "SERIAL NO.", '', 'C', false, 0);
  ...
  public function default_PO_PDF($params, $data)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $arr_qty = $this->reporter->fixcolumn([$qty], '13');
      $arr_serialno = $this->reporter->fixcolumn([$serialno], '16', 0);

      $maxrow = $this->othersClass->getmaxcolumn([$arr_barcode, $arr_itemname, $arr_qty, $arr_serialno]);
      ...
      for ($r = 0; $r < $maxrow; $r++) {
        ...
        PDF::MultiCell(100, 0, (isset($arr_serialno[$r]) ? $arr_serialno[$r] : ''), '', 'C', 0, 0, '', '', true, 0, true, false);
        PDF::MultiCell(350, 0, '', '', 'R', 0, 1, '', '', true, 0, false, false);
      }

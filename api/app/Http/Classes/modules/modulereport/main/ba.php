...
class ba
{
  ...
  public function default_BA_header_PDF($params, $data)
  {
    ...
    PDF::MultiCell(200, 0, "DESCRIPTION", '', 'C', false);
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(200, 0, (isset($arr_itemname[$r]) ? $arr_itemname[$r] : ''), '', 'R', false, 1, '', '', false, 1);
        }

        // if ($data[$i]['itemname'] == '') {
        //   PDF::SetFont($font, '', $fontsize);
        //   PDF::MultiCell(100, 0, $data[$i]['barcode'], '', 'C', false, 0, '', '', true, 1);
        //   PDF::MultiCell(100, 0, number_format($data[$i]['qty'], $decimalqty), '', 'R', false, 0, '', '', false, 1);
        //   PDF::MultiCell(100, 0, $data[$i]['uom'], '', 'C', false, 0, '', '', false, 1);
        //   PDF::MultiCell(200, 0, $data[$i]['itemname'], '', 'R', false, 1, '', '', false, 1);
        // } else {
        //   for($r = 0; $r < $maxrow; $r++) {
        //     if($r == 0) {
        //       $barcode =  $data[$i]['barcode'];
        ...
        //     PDF::MultiCell(200, 0, isset($itemcodedescs[$r]) ? $itemcodedescs[$r] : '', '', 'R', false, 1, '', '', false, 1);
        //   }
        // }
        $totalext += $data[$i]['ext'];

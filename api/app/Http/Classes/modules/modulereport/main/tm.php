...
class tm
{
    ...
    public function report_default_query($trno)
    {
        $query = "select h.trno as clientid,c.client,c.clientname,date(h.dateid) as dateid,ifnull(u.username,e.clientname) as requestby,h.rem,i.itemid as sysid,i.itemname as systype,r.line as taskid,
            ...
            d.task,d.title,d.userid as assignedid,date(d.startdate) as startdate,date(d.fcheckingdate) as enddate,d.percentage,
            tcat.category as comment1 ,assigned.clientname as assignedname
            from tmhead as h
            ...
            left join reqcategory as tcat on tcat.line = d.taskcatid
    ...
    public function default_tm_header_PDF($params, $data)
    {
        ...
        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(40, 20, "Client : ", '', 'L', false, 0, '', '', true, 0, false, true, 0, 'B', true);
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(510, 20, (isset($data[0]['clientname']) ? $data[0]['clientname'] : ''), 'B', 'L', false, 0, '', '', true, 0, false, true, 0, 'B', true);
        PDF::SetFont($fontbold, '', $fontsize);
        ...
        PDF::MultiCell(380, 25, "TASK", 'TB', 'C', false, 0, '', '', true, 0, false, true, 0, 'M', true);
        PDF::MultiCell(50, 25, "%", 'TB', 'C', false, 0, '', '', true, 0, false, true, 0, 'M', true);
        PDF::MultiCell(75, 25, "START DATE", 'TB', 'C', false, 0, '', '', true, 0, false, true, 0, 'M', true);
        PDF::MultiCell(75, 25, "END DATE", 'TB', 'C', false, 0, '', '', true, 0, false, true, 0, 'M', true);
        PDF::MultiCell(120, 25, "TASK CATEGORY", 'TB', 'C', false, 1, '', '', true, 0, false, true, 0, 'M', true);
    }
    ...
    public function default_tm_PDF($params, $data)
    {
        ...
        $pageMaxY = 900;
        $footerReservedSpace = 30;
        $pageBreakThreshold = $pageMaxY - $footerReservedSpace;

        $groupSeparatorHeight = 10;
        $groupHeaderRowHeight = 20;
        $minTaskRowHeight = 15;
        $groupHeaderReserved = $groupSeparatorHeight + $groupHeaderRowHeight + $minTaskRowHeight;
        ...
        foreach ($data as $row) {
            ...
            $key = isset($row['assignedname']) ? $row['assignedname'] : '';
            $grouped[$key][] = $row;
        }

        $isfirstgroup = true;

        foreach ($grouped as $assignedname => $rows) {

            if ((PDF::getY() + $groupHeaderReserved) > $pageBreakThreshold) {
                $this->default_tm_header_PDF($params, $data);
            }
            ...
            PDF::SetFont($fontbold, 'B', $fontsize);
            PDF::MultiCell(700, 0, ' ' . strtoupper($assignedname), 'B', 'L', false, 1, '', '', true, 0, false, true, 0, 'M', false);
            ...
            foreach ($rows as $row) {
                ...
                for ($r = 0; $r < $maxrow; $r++) {
                    if (PDF::getY() > ($pageBreakThreshold - 15)) {
                        $this->default_tm_header_PDF($params, $data);
                    }

                    $countNum = ($r === 0) ? $taskCounter : '';

                    PDF::SetFont($fontbold, 'B', $fontsize);
                    PDF::MultiCell(20, 15, ' ' . $countNum, '', 'C', false, 0, '', '', true, 0, false, true, 0, 'M', false);
                    PDF::SetFont($font, '', $fontsize);
                    PDF::MultiCell(360, 15, ' ' . (isset($arr_title[$r]) ? $arr_title[$r] : ''), '', 'L', false, 0, '', '', true, 0, false, true, 0, 'M', false);
                    PDF::MultiCell(50, 15, ' ' . (isset($arr_percentage[$r]) ? $arr_percentage[$r] : ''), '', 'C', false, 0, '', '', true, 0, false, true, 0, 'M', false);
                    PDF::MultiCell(75, 15, ' ' . (isset($arr_startdate[$r]) ? $arr_startdate[$r] : ''), '', 'C', false, 0, '', '', true, 0, false, true, 0, 'M', false);
                    PDF::MultiCell(75, 15, ' ' . (isset($arr_enddate[$r]) ? $arr_enddate[$r] : ''), '', 'C', false, 0, '', '', true, 0, false, true, 0, 'M', false);
                    PDF::MultiCell(120, 15, ' ' . (isset($arr_comment[$r]) ? $arr_comment[$r] : ''), '', 'C', false, 1, '', '', true, 0, false, true, 0, 'M', false);
                }
        ...
        PDF::MultiCell(0, 0, "\n");
        PDF::MultiCell(253, 0, isset($params['params']['dataparams']['prepared']) ? $params['params']['dataparams']['prepared'] : '', '', 'L', false, 0);
        PDF::MultiCell(253, 0, isset($params['params']['dataparams']['approved']) ? $params['params']['dataparams']['approved'] : '', '', 'L', false, 0);
        PDF::MultiCell(253, 0, isset($params['params']['dataparams']['received']) ? $params['params']['dataparams']['received'] : '', '', 'L');

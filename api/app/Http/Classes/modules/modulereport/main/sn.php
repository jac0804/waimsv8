...
class sn
{
  ...
  public function report_default_query($trno)
  {
    ...
    $query = "select snhead.docno, snhead.trno, snhead.clientname, snhead.address, date(snhead.dateid) as dateid, 
        ...
        date(stock.expiry) as expiry, stock.rem as srem, item.sizeid, m.model_name as model
        from glhead as head 
  ...
  public function default_SN_header_PDF($params, $data)
  {
    ...
    // PDF::SetFont($font, '', 11);
    // PDF::MultiCell(100,70,(isset($data[0]['docno']) ? $data[0]['docno'] . "". 
    // PDF::write2DBarcode($data[0]['docno'].'-'.$data[0]['trno'], 'QRCODE', PDF::GetX() + 25, PDF::GetY() + 20, 50, '', [], 'C'): ""), '', 'C', false);
  ...
  public function default_SN_PDF($params, $data)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $maxrow = $this->othersClass->getmaxcolumn([$arr_qty, $arr_uom, $arr_itemname, $arr_expiry, $arr_netamt, $arr_disc, $arr_ext]);

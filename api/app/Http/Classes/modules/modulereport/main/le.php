...
class le
{
    ...
    public function report_default_query($config)
    {
        ...
        $qryselect = "select num.center,  head.trno, 
         ...
         ifnull(info.entryfee,0) as entryfee,
         ifnull(info.lrf,0) as lrf, 
         ifnull(info.itfee,0) as itfee, 
         ifnull(info.regfee,0) as regfee, 
         ifnull(info.docstamp1,0) as docstamp1,
         ifnull(info.nf,0) as nf, 
         ifnull(info.annotationfee,0) as annotationfee,
         ifnull(info.articles,0) as articles,
         ifnull(info.annotationexp,0) as annotationexp,
         ifnull(info.otransfer,0) as otransfer,
         info.pf,
         ifnull(info.rpt,0) as rpt,
         ifnull(info.docstamp,0) as docstamp, 
         ifnull(info.mri,0) as fmri,
         ifnull(info.handling,0) as handling,
         ifnull(info.appraisal,0) as appraisal,
         ifnull(info.filing,0) as filing,
         ifnull(info.nf2,0) as nf2,
         ifnull(info.nf3,0) as nf3,
         ifnull(info.ofee,0) as ofee,
         ifnull(info.referral,0) as referral,
         ifnull(info.cancellation4,0) as cancellation4,
         ifnull(info.cancellation7,0) as cancellation7,
         ifnull(info.annotationoc1,0) as annotationoc1,
         ifnull(info.annotationoc2,0) as annotationoc2,
         ifnull(info.cancellationu,0) as cancellationu,
         ifnull(info.entryfee + info.lrf + info.itfee + info.regfee+info.docstamp+info.pf + info.nf + info.nf2+info.nf3+info.ofee+info.annotationfee+info.docstamp1+info.articles+info.annotationexp+info.otransfer+info.rpt+info.handling+info.appraisal+info.filing+info.referral+info.cancellation4+info.cancellation7+info.annotationoc1+info.annotationoc2+info.cancellationu+info.mri,0) as totalcharges,r.isdiminishing";

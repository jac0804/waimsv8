...
class mr
{
  ...
  private function report_default_header($params, $data)
  {
    ...
    $label = ($companyid == 43) ? 'NAME/ASSET' : 'CUSTOMER : ';
    $str .= $this->reporter->col($label, '80', null, false, $border, '', 'L', $font, '12', 'B', '30px', '4px');
    ...
    switch ($companyid) {
      case 43:
        ...
        $str .= $this->reporter->col('YOURREF : ', '60', null, false, $border, '', 'L', $font, '12', 'B', '', '');
        $str .= $this->reporter->col((isset($data[0]['yourref']) ? $data[0]['yourref'] : ''), '140', null, false, $border, 'B', 'R', $font, '12', '', '', '');
        $str .= $this->reporter->endrow();

        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col('PROJECT : ', '80', null, false, $border, '', 'L', $font, '12', 'B', '30px', '4px');
        $str .= $this->reporter->col((isset($data[0]['project']) ? $data[0]['project'] : ''), '520', null, false, $border, 'B', 'L', $font, '12', '', '30px', '4px');
        $str .= $this->reporter->col('OURREF : ', '60', null, false, $border, '', 'L', $font, '12', 'B', '', '');
        $str .= $this->reporter->col((isset($data[0]['ourref']) ? $data[0]['ourref'] : ''), '140', null, false, $border, 'B', 'R', $font, '12', '', '', '');
        break;
      default:
    ...
    switch ($companyid) {
      case 43:
        ...
        $str .= $this->reporter->col('DESCRPTION', '500px', null, false, $border, 'B', 'C', $font, '12', 'B', '30px', '8px');
        break;
      default:
        $str .= $this->reporter->col('QTY', '50px', null, false, $border, 'B', 'C', $font, '12', 'B', '30px', '8px');
        $str .= $this->reporter->col('UNIT', '50px', null, false, $border, 'B', 'C', $font, '12', 'B', '30px', '8px');

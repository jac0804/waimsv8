...
class hi
{
  ...
  { 
    ...
    if ($companyid == 58) { // cdohris
      ...
      $query = "select division from employee where empid = " . $data[0]['fempid'] . "";
      ...
      PDF::SetFont($font, '', 11);
      PDF::MultiCell(300, 18, (isset($data[0]['docno']) ? $data[0]['docno'] : ''), 'B', 'L', false,0);
      PDF::SetFont($fontbold, '', 11);
      PDF::MultiCell(150, 18, "DATE : ", '', 'R', false,0);
      PDF::SetFont($font, '', 11);
      ...
      PDF::SetFont($font, '', 11);
      PDF::MultiCell(300, 18, (isset($data[0]['fempname']) ? $data[0]['fempname'] : ''), 'B', 'L', false,0);
      PDF::SetFont($fontbold, '', 11);
      PDF::MultiCell(150, 18, "Job Title : ", '', 'R', false,0);
      PDF::SetFont($font, '', 11);
      ...
      PDF::SetFont($fontbold, '', 11);
      PDF::MultiCell(80, 0, "Description : ", '', 'L', false,0);
      PDF::SetFont($font, '', 11);
      PDF::MultiCell(180, 0, (isset($data[0]['idescription']) ? $data[0]['idescription'] : ''), 'B', 'L', false,0);
      PDF::SetFont($fontbold, '', 11);
      PDF::MultiCell(60, 0, "Place : ", '', 'R', false,0);
      PDF::SetFont($font, '', 11);
      PDF::MultiCell(120, 0, (isset($data[0]['iplace']) ? $data[0]['iplace'] : ''), 'B', 'L', false,0);
      PDF::SetFont($fontbold, '', 11);
      PDF::MultiCell(55, 0, "Incident : ", '', 'L', false,0);
      PDF::SetFont($font, '', 11);
      PDF::MultiCell(100, 0, (isset($data[0]['idate']) ? $data[0]['idate'] : ''), 'B', 'L', false,0);
      PDF::SetFont($fontbold, '', 11);
      PDF::MultiCell(55, 0, "Incident : ", '', 'L', false,0);
      PDF::SetFont($font, '', 11);
      PDF::MultiCell(100, 0, (isset($data[0]['itime']) ? $data[0]['itime'] : ''), 'B', 'L', false);
    ...
    public function rpt_HI_PDF($config, $data)
    {
      ...
      for ($i = 0; $i < count($data); $i++) {
        ...
        $arr_dempname = $this->reporter->fixcolumn([$data[$i]['dempname']],'25',0);
        $arr_djobtitle = $this->reporter->fixcolumn([$data[$i]['djobtitle']],'25',0);

        $maxrow = $this->othersClass->getmaxcolumn([$arr_dempcode, $arr_dempname, $arr_djobtitle]);

        for($r = 0; $r < $maxrow; $r++) {
          PDF::SetFont($font, '', $fontsize);
          PDF::MultiCell(253, 20, (isset($arr_dempcode[$r]) ? $arr_dempcode[$r] : ''), '', 'L', 0, 0, '', '', true, 0, true, false);
          PDF::MultiCell(253, 20, (isset($arr_dempname[$r]) ? $arr_dempname[$r] : ''), '', 'L', 0, 0, '', '', true, 0, true, false);
      ...
      PDF::SetFont($fontbold, '', 11);
      PDF::MultiCell(150, 0, "INCIDENT DETAILS : ", '', 'L', false,0);
      PDF::SetFont($font, '', 11);
      PDF::MultiCell(610, 0, $data[0]['idetails'], '', 'L', false);

      PDF::SetFont($fontbold, '', 11);
      PDF::MultiCell(150, 0, "INCIDENT COMMENTS : ", '', 'L', false,0);
      PDF::SetFont($font, '', 11);
      PDF::MultiCell(610, 0, $data[0]['icomments'], '', 'L', false);

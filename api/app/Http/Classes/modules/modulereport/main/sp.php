...
class sp
{
  ...
  private $modulename = "Stock Return";
  private $fieldClass;
...
public function reportparamsdata($config){
  ...
  public function report_default_query($trno){
    ...
    $query = "select head.vattype, head.tax, client.tel, stock.rem as remarks, m.model_name as model,item.sizeid,
      ...
      where head.doc='sp' and md5(head.trno)='".md5($trno)."'
      union all
  ...
  public function default_cm_PDF($params, $data)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      for($r = 0; $r < $maxrow; $r++) {
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(80, 0, (isset($arr_qty[$r]) ? $arr_qty[$r] : ''), '', 'R', false, 0, '', '', true, 1);
        PDF::MultiCell(100, 0, (isset($arr_uom[$r]) ? $arr_uom[$r] : ''), '', 'C', false, 0, '', '', false, 1);
        PDF::MultiCell(220, 0, (isset($arr_itemname[$r]) ? $arr_itemname[$r] : ''), '', 'L', false, 0, '', '', false, 1);
      ...
      //       }
      //       PDF::SetFont($font, '', $fontsize);
      //       PDF::MultiCell(80, 0, $qty, '', 'R', false, 0,'','',true,1);
      //       PDF::MultiCell(100, 0, $uom, '', 'C', false, 0, '', '', false, 1);
      //       PDF::MultiCell(220, 0, isset($itemname[$r]) ? $itemname[$r] : '', '', 'L', false, 0, '', '', false, 1);

...
class pp
{
    ...
    private $modulename = "PROMO PER ITEM";
    ...
    public function report_default_query($config)
    {
	 ...
	 item.barcode, item.itemname,stock.pstart,stock.pend,stock.pqty,head.isall,head.isamt,head.isqty,head.isbuy1 as isbuy
	 from pphead as head
	 left join ppstock as stock on stock.trno = head.trno
    ...
    public function default_header($params, $data)
    {
        ...
        <label for="agree1" style="font-size: 11; display: inline-block;"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;All Branch/Warehouse</label>
        ';

        $h2 = '
       
        <input type="checkbox" name="agree1" value="1" readonly="true" ' . $r4 . '/>  
        <label for="agree1" style="font-size: 11; display: inline-block;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;By 1 Take 1</label>
        ';


        $h3 = ';
        <input type="checkbox" name="agree2" value="2" readonly="true" ' . $r2 . '/> 
        <label for="agree2" style="font-size: 11; display: inline-block;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Quantity</label>
       ';

        $h4 = ';
        <input type="checkbox" name="agree3" value="3" readonly="true" ' . $r3 . '/>  
        <label for="agree3" style="font-size: 11; display: inline-block;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Amount</label>
       ';

        $str .= $this->reporter->begintable('800');
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col($h1, '800', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '');
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();
        $str .= $this->reporter->begintable('800');
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col($h2, '800', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '');
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();
        $str .= $this->reporter->begintable('800');
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col($h3, '800', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '');
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();

        $str .= $this->reporter->begintable('800');
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col($h4, '800', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '');
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();
        ...
        $str .= $this->reporter->col('DESCRIPTION', '380', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '', '8px');
        $str .= $this->reporter->col('START', '75', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '', '8px');
        $str .= $this->reporter->col('END', '75', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '', '8px');
        $str .= $this->reporter->col('PROMO PER ITEM COUNT', '100', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '', '8px');
    ...
    public function default_so_layout($params, $data)
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            $str .= $this->reporter->col($data[$i]['pstart'], '75', null, false, $border, '', 'R', $font, $fontsize, '', '', '2px');
            $str .= $this->reporter->col($data[$i]['pend'], '75', null, false, $border, '', 'C', $font, $fontsize, '', '', '');
            $str .= $this->reporter->col($data[$i]['pqty'], '120', null, false, $border, '', 'R', $font, $fontsize, '', '', '2px');
    ...
    public function default_so_header_PDF($params, $data)
    {
        ...
        PDF::MultiCell(75, 0, "START", '', 'C', false, 0);
        PDF::MultiCell(75, 0, "END", '', 'R', false, 0);
        PDF::MultiCell(120, 0, "PROMO ITEM COUNT", '', 'C', false, 1);
    ...
    public function default_so_PDF($params, $data)
    {
        ...
        if (!empty($data)) {
            ...
            for ($i = 0; $i < count($data); $i++) {
                ...
                $pstart = $data[$i]['pstart'];
                $pend = number_format($data[$i]['pend'], 2);
                $pqty = $data[$i]['pqty'];

                $arr_barcode = $this->reporter->fixcolumn([$barcode], '15', 0);
                $arr_itemname = $this->reporter->fixcolumn([$itemname], '35', 0);
                $arr_pstart = $this->reporter->fixcolumn([$pstart], '13', 0);
                $arr_pend = $this->reporter->fixcolumn([$pend], '13', 0);
                $arr_pqty = $this->reporter->fixcolumn([$pqty], '13', 0);


                $maxrow = $this->othersClass->getmaxcolumn([$arr_barcode, $arr_itemname, $arr_pstart, $arr_pend, $arr_pqty]);
                for ($r = 0; $r < $maxrow; $r++) {
                    ...
                    PDF::MultiCell(75, 15, ' ' . (isset($arr_pstart[$r]) ? $arr_pstart[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                    PDF::MultiCell(75, 15, ' ' . (isset($arr_pend[$r]) ? $arr_pend[$r] : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                    PDF::MultiCell(120, 15, ' ' . (isset($arr_pqty[$r]) ? $arr_pqty[$r] : ''), '', 'R', false, 1, '',  '', true, 0, false, true, 0, 'M', false);

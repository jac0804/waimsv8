...
class pc
{
    ...
    where head.doc='pc' and head.trno='$trno'
    union all
    select date(head.dateid) as dateid, head.docno, client.client, client.clientname, head.address, 
    head.terms,head.rem, item.barcode,
    item.itemname, item.color, item.sizeid, stock.rrqty as qty, stock.uom, stock.rrcost as cost, stock.disc, stock.ext,
    ...
    where head.doc='pc' and head.trno='$trno'";
        ...
        $result = json_decode(json_encode($this->coreFunctions->opentable($query)));
    ...
    public function header_pc_layout($config, $result)
    {
        ...
        $str .= $this->reporter->col('PHYSICAL COUNT', '800', null, false, '1px solid ', '', 'L', 'Century Gothic', '18', 'B', '', '');
        ...
        $str .= $this->reporter->col('', '50', null, false, '1px solid ', '', 'L', 'Century Gothic', '12', '', '', '');
        $str .= $this->reporter->col('', '455', null, false, '1px solid ', '', 'L', 'Century Gothic', '12', '', '', '');
        $str .= $this->reporter->col('DOCUMENT # :', '125', null, false, '1px solid ', '', 'L', 'Century Gothic', '13', 'B', '', '');
        $str .= $this->reporter->col((isset($result[0]->docno) ? $result[0]->docno : ''), '120', null, false, '1px solid ', 'B', 'L', 'Century Gothic', '13', '', '', '') . '<br />';
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();
        ...
        $str .= $this->reporter->col((isset($result[0]->clientname) ? $result[0]->clientname : ''), '50', null, false, '1px solid ', '', 'L', 'Century Gothic', '12', '', '', '');
        $str .= $this->reporter->col('', '455', null, false, '1px solid ', '', 'L', 'Century Gothic', '12', '', '', '');
        $str .= $this->reporter->col('DATE :', '125', null, false, '1px solid ', '', 'L', 'Century Gothic', '13', 'B', '', '');
        $str .= $this->reporter->col((isset($result[0]->dateid) ? $result[0]->dateid : ''), '120', null, false, '1px solid ', 'B', 'L', 'Century Gothic', '13', '', '', '') . '<br />';
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();
        ...
        $str .= $this->reporter->col('D E S C R I P T I O N', '450', null, false, '1px solid ', 'B', 'C', 'Century Gothic', '12', 'B', '30px', '8px');
        $str .= $this->reporter->col('COST', '125', null, false, '1px solid ', 'B', 'C', 'Century Gothic', '12', 'B', '30px', '8px');
        $str .= $this->reporter->col('TOTAL', '120', null, false, '1px solid ', 'B', 'C', 'Century Gothic', '12', 'B', '30px', '8px');
        $str .= $this->reporter->endrow();
        return $str;
    }
    ...
    public function default_pc_layout($config, $result)
    {
        ...
        $str .= $this->header_pc_layout($config, $result);
        ...
        foreach ($result as $key => $data) {
            ...
            $str .= $this->reporter->col($data->itemname, '455', null, false, '1px solid ', '', 'L', 'Century Gothic', '11', '', '', '2px');
            $str .= $this->reporter->col(number_format($data->cost, 2), '125', null, false, '1px solid ', '', 'R', 'Century Gothic', '11', '', '', '2px');
            $str .= $this->reporter->col(number_format($data->ext, 2), '120', null, false, '1px solid ', '', 'R', 'Century Gothic', '11', '', '', '2px');
        ...
        $str .= $this->reporter->col('', '50', null, false, '1px dotted ', 'T', 'C', 'Century Gothic', '12', 'B', '', '');
        $str .= $this->reporter->col('', '455', null, false, '1px dotted ', 'T', 'C', 'Century Gothic', '12', 'B', '', '');
        $str .= $this->reporter->col('GRAND TOTAL :', '125', null, false, '1px dotted ', 'T', 'R', 'Century Gothic', '12', 'B', '', '');
        $str .= $this->reporter->col(number_format($totalext, 2), '120', null, false, '1px dotted ', 'T', 'R', 'Century Gothic', '12', 'B', '', '');
        $str .= $this->reporter->endrow();
        ...
        $str .= $this->reporter->col((isset($result[0]->rem) ? $result[0]->rem : ''), '600', null, false, '1px solid ', '', 'L', 'Century Gothic', '12', '', '', '');

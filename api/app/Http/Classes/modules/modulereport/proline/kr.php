...
class kr
{
  ...
  public function report_default_query($filters)
  {
    ...
    coa.acno, coa.acnoname, ar.db, ar.cr, date(ar.dateid) as postdate,head.rem,head2.rem as rem2, ar.docno as ref,client.tel
    from (krhead as head 
  ...
  public function default_KR_header_PDF($params, $data)
  {
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(470, 0, (isset($data[0]['address']) ? $data[0]['address'] : ''), '', 'L');

    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(80, 0, "Tel No: ", '', 'L', false, 0, '', '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(470, 0, (isset($data[0]['tel']) ? $data[0]['tel'] : ''), '', 'L');

    PDF::MultiCell(0, 0, "\n");

    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(0, 0, "Received the following document(s) for purpose of processing, rechecking and vouchering in order to expedite complete payment:", '', 'L');

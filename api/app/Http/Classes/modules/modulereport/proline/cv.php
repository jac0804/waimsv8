...
class cv
{
  ...
  public function report_default_query($filters)
  {
    ...
    switch ($filters['params']['dataparams']['reporttype']) {
      case 2:
        ...
        (select ifnull(sum(ladetail.cr), 0) as cr from ladetail where ladetail.trno=head.trno and ladetail.acnoid=" . $acnoid . ") as ewt
        from lahead as head
        ...
        detail.ewtcode,ewtlist.description as ewtdesc,detail.ewtrate,detail.isvewt,
        client.zipcode, center.tin as payortin, center.address as payoraddress, center.zipcode as payorzipcode, center.name as payorcompname,
        (select ifnull(sum(gldetail.cr), 0) as cr from gldetail where gldetail.trno=head.trno and gldetail.acnoid=" . $acnoid . ") as ewt
        from glhead as head
      ...
      default:
        ...
        DATE_FORMAT(head.dateid,'%Y-%m-%d') as dateid, 
        DATE_FORMAT(cntnum.postdate,'%Y-%m-%d') as postdate,client.client, head.clientname, head.address,
  ...
  public function PDF_CV_WTAXREPORT($data, $params)
  {
    ...
    foreach ($data['detail'] as $key => $value) {
      ...
      $ewtamt = $this->coreFunctions->datareader("select sum(d.cr-d.db) as value from ladetail as d left join coa on coa.acnoid=d.acnoid where d.trno=? and d.ewtcode=? and coa.alias='APWT1'
      union all
      select sum(d.cr-d.db) as value from gldetail as d left join coa on coa.acnoid=d.acnoid where d.trno=? and d.ewtcode=? and coa.alias='APWT1'", [$params['params']['dataid'], $key, $params['params']['dataid'], $key]);
      if ($ewtamt == '') $ewtamt = 0;
      PDF::MultiCell(120, $max_height, number_format($ewtamt, 2), 'LRB', 'R', false);
    ...
    PDF::MultiCell(120, 10, number_format($total_ewtamt, 2), 'LR', 'R', false);
    PDF::SetFont($font, '', 9);
    $totaltax = $total_ewtamt;

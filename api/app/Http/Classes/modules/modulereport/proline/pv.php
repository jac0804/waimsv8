...
class pv
{
  ...
  public function PDF_DEFAULT_CCVOUCHER_LAYOUT1($data, $params)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $acnonamedescs = $data[$i]['acnoname'] . ' ' . $data[$i]['project'];
      ...
      for ($r = 0; $r < $maxrow; $r++) {
        PDF::SetFont($font, '', 11);
        PDF::MultiCell(500, 15, ' ' . (isset($arr_acnonamedescs[$r]) ? $arr_acnonamedescs[$r] : ''), 'TBL', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
        PDF::MultiCell(100, 15, ' ' . (isset($arr_debit[$r]) ? $arr_debit[$r] : ''), 'TBL', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
        PDF::MultiCell(100, 15, ' ' . (isset($arr_credit[$r]) ? $arr_credit[$r] : ''), 'TBLR', 'R', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
      }
    }
  ...
  public function report_default_query($filters)
  {
    ...
    switch ($filters['params']['dataparams']['reporttype']) {
      ...
      default:
        $query = "select DATE_FORMAT(head.dateid,'%Y-%m-%d') as dateid, head.docno, client.client, client.clientname, head.address, head.terms,
          ...
          union all
          select DATE_FORMAT(head.dateid,'%Y-%m-%d') as dateid, head.docno, client.client, client.clientname, head.address, head.terms,
          head.rem as hrem,head.rem, head.yourref, head.ourref,
          coa.acno, coa.acnoname, detail.ref, detail.postdate, detail.db, detail.cr,

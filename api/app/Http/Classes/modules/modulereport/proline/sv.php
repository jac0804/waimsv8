...
class sv
{
  ...
  public function default_SV_header_PDF($params, $data)
  {
    ...
    PDF::MultiCell(90, 0, "DEBIT", '', 'C', false, 0);
    PDF::MultiCell(90, 0, "CREDIT", '', 'C', false, 0);
  ...
  public function default_SV_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      ...
      for ($i = 0; $i < count($data); $i++) {
        ...
        $arr_remarks = $this->reporter->fixcolumn([$remarks], '15', 0);

        $maxrow = $this->othersClass->getmaxcolumn([$arr_acno, $arr_acnonamedescs, $arr_postdate, $arr_debit, $arr_credit, $arr_remarks]);
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(100, 15, ' ' . (isset($arr_postdate[$r]) ? $arr_postdate[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          ...
          PDF::MultiCell(100, 15, ' ' . (isset($arr_remarks[$r]) ? $arr_remarks[$r] : ''), '', 'L', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
        }
        $totaldb += $data[$i]['db'];
        $totalcr += $data[$i]['cr'];
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      if ($data[$i]['acnoname'] == '') {
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(220, 0, isset($acnoname[$r]) ? $acnoname[$r] : '', '', 'L', false, 0, '', '', false, 1);
          PDF::MultiCell(100, 0, $postdate, '', 'C', false, 0, '', '', false, 1);
          PDF::MultiCell(80, 0, $debit, '', 'R', false, 0, '', '', false, 1);
          PDF::MultiCell(80, 0, $credit, '', 'R', false, 0, '', '', false, 1);
          PDF::MultiCell(20, 0, '', '', 'R', false, 0, '', '', false, 1);
          PDF::MultiCell(100, 0, $remarks, '', 'L', false, 1, '', '', false, 1);
        }
      }
      $totaldb += $data[$i]['db'];
      $totalcr += $data[$i]['cr'];

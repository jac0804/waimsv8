...
class dm
{
    ...
    public function default_DM_header_PDF($params, $data)
    {
        ...
        // }
        if (Storage::disk('sbcpath')->exists('/fonts/bookantiqua.ttf')) {
            $font = TCPDF_FONTS::addTTFfont(database_path() . '/images/fonts/bookantiqua.ttf');
            $fontbold = TCPDF_FONTS::addTTFfont(database_path() . '/images/fonts/bookantiqua_bold.ttf');
        }
        ...
        PDF::MultiCell(130, 0, "BARCODE", '', 'C', false, 0);
    ...
    public function default_DM_PDF($params, $data)
    {
        ...
        if (!empty($data)) {
            for ($i = 0; $i < count($data); $i++) {
                ...
                $arr_itemname = $this->reporter->fixcolumn([$itemname], '27', 0);

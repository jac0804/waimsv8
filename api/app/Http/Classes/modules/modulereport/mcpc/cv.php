...
class cv
{
    ...
    public function PDF_default_header($params, $data)
    {
        ...
        PDF::MultiCell(445, 5, (isset($data[0]['clientname']) ? $data[0]['clientname'] : ''), '', 'L', false, 0, '',  '');
        PDF::SetFont($fontbold, '', $fontsize);
        ...
        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(105, 5, 'REFERENCE # :', '', 'R', false, 0);
        PDF::SetFont($font, '', $fontsize);
        ...
        PDF::MultiCell(120, 0, 'CHECK DETAILS', '', 'C', false, 0);
    ...
    public function PDF_DEFAULT_CCVOUCHER_LAYOUT1($data, $params)
    {
        ...
        if (!empty($data)) {
            ...
            for ($i = 0; $i < count($data); $i++) {
                ...
                $pdate = date('m-d-Y', strtotime($data[$i]['pdate']));
                ...
                for ($r = 0; $r < $maxrow; $r++) {
                    ...
                    PDF::MultiCell(120, 15, ' ' . (isset($arr_checkno[$r]) ? $arr_checkno[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                    PDF::MultiCell(80, 15, ' ' . (isset($arr_pdate[$r]) ? $arr_pdate[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
        ...
        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(447, 5, 'GRAND TOTAL : ', '', 'R', false, 0);
    ...
    ";
        ...
        for ($i = 0; $i < count($data2); $i++) {
            ...
            PDF::SetFont($font, '', 13);
            PDF::MultiCell(590, 0, date('m', strtotime($data2[$i]['postdate'])) . '-', '', 'R', false, 0);
            PDF::MultiCell(50, 0, date('d', strtotime($data2[$i]['postdate'])) . '-', '', 'C', false, 0);
            PDF::MultiCell(50, 0, date('Y', strtotime($data2[$i]['postdate'])), '', 'C');
            ...
            PDF::SetFont($font, '', 13);
            PDF::MultiCell(160, 0, (isset($cc) ? '' . number_format($cc, $decimal) . '' : ''), '', 'C', false);

            PDF::SetFont($font, '', 11);
            PDF::MultiCell(690, 5, "\n", '', 'L', false);
            $dd = number_format((float)$cc, 2, '.', '');
            PDF::MultiCell(30, 0, '', '', 'L', false, 0);
            PDF::MultiCell(660, 0, '' . $this->reporter->ftNumberToWordsConverter($dd) . ' ONLY', '', 'L', false);
        }

...
class pv
{
  ...
  public function createreportfilter($config)
  {
    ...
    data_set($col1, 'approved.label', 'Certified Correct by');
    data_set($col1, 'received.label', 'Approved by');
  ...
  public function PDF_default_header_ccvoucher($params, $data)
  {
    ...
    PDF::SetMargins(40, 10);

    PDF::SetFont($font, '', 2);
    PDF::MultiCell(0, 0, '', '', 'L');

    PDF::SetFont($fontbold, '', 22);
    PDF::MultiCell(760, 15, strtoupper($this->modulename), '', 'C', false);
    ...
    PDF::MultiCell(70, 15, '', '', 'L', false, 0);
    PDF::SetFont($font, '',  $fontsize);
    PDF::MultiCell(330, 15, '', '', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsizeh);
    PDF::MultiCell(165, 15, "Docno # : ", '', 'R', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsizeh1);
    PDF::MultiCell(130, 15, (isset($data[0]['docno']) ? $data[0]['docno'] : ''), '', 'R', false);
    ...
    PDF::SetFont($fontbold, '',  $fontsizeh);
    PDF::MultiCell(70, 15, 'PAID TO : ', '', 'L', false, 0);
    PDF::SetFont($font, '',  $fontsizeh1);
    PDF::MultiCell(330, 15, (isset($data[0]['clientname']) ? strtoupper($data[0]['clientname']) : ''), '', 'L', false, 0, '',  '');

    PDF::SetFont($fontbold, '',  $fontsizeh);
    PDF::MultiCell(165, 15, 'DATE : ', '', 'R', false, 0);
    PDF::SetFont($font, '',  $fontsizeh);
    PDF::MultiCell(130, 15, (isset($date) ? $date : ''), '', 'R', false);

    PDF::SetFont($fontbold, '',  $fontsizeh);
    PDF::MultiCell(70, 15, 'ADDRESS : ', '', 'L', false, 0);
    PDF::SetFont($font, '',  $fontsizeh);
    PDF::MultiCell(330, 15, (isset($data[0]['address']) ? $data[0]['address'] : ''), '', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '',  $fontsizeh);
    PDF::MultiCell(165, 15, 'YOURREF : ', '', 'R', false, 0);
    PDF::SetFont($font, '',  $fontsizeh);
    PDF::MultiCell(130, 15, (isset($data[0]['yourref']) ? $data[0]['yourref'] : ''), '', 'R', false);
  ...
  public function PDF_DEFAULT_CCVOUCHER_LAYOUT1($data, $params)
  {
    ...
    $rem = $this->reporter->fixcolumn([$data[0]['headernotes']], '80', 0);
    $remcount = count($rem);
    $maxrow1 = 1;
    $maxrow1 = $remcount;

    $qry = "select a.acnoname,a.db,a.cr from (
    select coa.acnoname,format(sum(detail.db),2) as db,format(sum(detail.cr),2) as cr from ladetail as detail
    left join coa on coa.acnoid=detail.acnoid
    where detail.trno='" . $data[0]['trno'] . "'  and coa.alias!='AP2'
    group by coa.acnoname
    union all
    select coa.acnoname,format(sum(detail.db),2) as db,format(sum(detail.cr),2) from gldetail as detail
    left join coa on coa.acnoid=detail.acnoid
    where detail.trno='" . $data[0]['trno'] . "'  and coa.alias!='AP2'
    group by coa.acnoname
    order by cr,db
    ...
    where detail.trno='" . $data[0]['trno'] . "'  and coa.alias='AP2'
    group by coa.acnoname";
    $notesdata = json_decode(json_encode($this->coreFunctions->opentable($qry)), true);

    for ($k = 0; $k < $maxrow1; $k++) {
      PDF::SetFont($font, '', 11);
      if ($remarks == $data[0]['headernotes']) {
        PDF::MultiCell(550, 15, '', 'L', 'L', false, 0, '', '', true, 1);
        PDF::MultiCell(75, 15, '', 'L', 'L', false, 0, '', '', false, 1);
        PDF::MultiCell(75, 15, '', 'R', 'L', false, 1, '', '', false, 1);
      } else {
        PDF::MultiCell(550, 15, (isset($rem[$k]) ? ' ' . $rem[$k] : ''), 'L', 'L', false, 0, '', '', true, 1);
        PDF::MultiCell(75, 15, '', 'L', 'L', false, 0, '', '', false, 1);
        PDF::MultiCell(75, 15, '', 'R', 'L', false, 1, '', '', false, 1);
      }
    }

    for ($z = 0; $z < count($notesdata); $z++) {
      $acnoname = $this->reporter->fixcolumn([$notesdata[$z]['acnoname']], '80', 0);
      $acnocount = count($acnoname);
      $maxrow = 1;
      $maxrow = $acnocount;
      ...
      for ($r = 0; $r < $maxrow; $r++) {
        ...
        if ($notesdata[$z]['cr'] == 0) {
          PDF::MultiCell(550, 15, (isset($acnoname[$r]) ? ' ' . $acnoname[$r] : ''), 'L', 'L', false, 0, '', '', true, 1);
        } else {
          PDF::MultiCell(70, 15, '', 'L', 'L', false, 0, '', '', true, 1);
          PDF::MultiCell(480, 15, (isset($acnoname[$r]) ? ' ' . $acnoname[$r] : ''), '', 'L', false, 0, '', '', false, 1);
        }

        if ($notesdata[$z]['db'] == 0) {
          PDF::MultiCell(75, 15, '', 'L', 'R', false, 0);
        } else {
          PDF::MultiCell(75, 15, $db . ' ', 'L', 'R', false, 0);
        }

        if ($notesdata[$z]['cr'] == 0) {
          PDF::MultiCell(75, 15, '', 'R', 'R', false);
        } else {
          PDF::MultiCell(75, 15, $cr . ' ', 'R', 'R', false);
        }
      }
    }

    PDF::MultiCell(700, 25, '', 'T', 'R', false);
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $noteslen = strlen($notes) / 50;
      if (strstr($notes, "\n")) {
        $noteslen = strlen($notes) / 30;
      }
      ...
      if ($noteslen > $acnolen) {
        ...
        if (strstr($notes, "\n")) {
          $substrnline = substr_count($notes, "\n");
          if ($substrnline <= 3) {
            $padding = 20 * $noteslen;
          }

          if ($substrnline >= 4) {
            $padding = 70 + (($substrnline - 4) * 15);
          }
        }
      }

      PDF::SetFont($font, '', 11);
      PDF::MultiCell(200, $padding, ' ' . $acname, 'L', 'L', false, 0);
      PDF::MultiCell(300, $padding, ' ' . $notes, 'LR', 'L', false, 0);
      PDF::MultiCell(100, $padding, $debit . ' ', 'R', 'R', false, 0);
      PDF::MultiCell(100, $padding, $credit . ' ', 'R', 'R', false);
      $subtotaldb += $data[$i]['db'];
      $subtotalcr += $data[$i]['cr'];
    }

    PDF::SetFont($font, '', 5);
    PDF::MultiCell(500, 0, '', 'T', 'L', false, 0);

    PDF::MultiCell(100, 0, '', 'T', 'R', false, 0);
    PDF::MultiCell(100, 0, '', 'T', 'R', false);

    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(200, 0, '', '', 'R', false, 0);
    PDF::MultiCell(300, 0, '', '', 'R', false, 0);
    PDF::MultiCell(100, 0, number_format($subtotaldb, $decimalprice) . ' ', '', 'R', false, 0);
    PDF::MultiCell(100, 0, number_format($subtotalcr, $decimalprice) . ' ', '', 'R', false);

    PDF::SetFont($font, '', 2);
    PDF::MultiCell(200, 0, '', '', 'R', false, 0);
    PDF::MultiCell(300, 0, '', '', 'R', false, 0);
    PDF::MultiCell(100, 0, '', 'B', 'R', false, 0);
    PDF::MultiCell(100, 0, '', 'B', 'R', false);
    PDF::SetFont($font, '', 2);
    PDF::MultiCell(200, 0, '', '', 'R', false, 0);
    PDF::MultiCell(300, 0, '', '', 'R', false, 0);
    PDF::MultiCell(100, 0, '', 'B', 'R', false, 0);
    ...
    PDF::MultiCell(253, 0, 'Certified Correct By: ', '', 'L', false, 0);
  ...
  public function report_default_query($filters)
  {
    ...
    switch ($filters['params']['dataparams']['reporttype']) {
      ...
      default:
        $query = "select detail.line,date(head.dateid) as dateid, head.docno, client.client, client.clientname, head.address, head.terms,
          head.rem as hrem,head.rem, head.yourref, head.ourref,
          coa.acno, coa.acnoname, detail.ref, detail.postdate, detail.db, detail.cr,
          detail.client as dclient, detail.checkno, head.project,detail.rem,head.trno,head.rem as headernotes,detail.sortline
          from lahead as head 
          left join ladetail as detail on detail.trno=head.trno 
          left join client on client.client=head.client
          left join coa on coa.acnoid=detail.acnoid
          ...
          union all
          select detail.line,date(head.dateid) as dateid, head.docno, client.client, client.clientname, head.address, head.terms,
          head.rem as hrem,head.rem, head.yourref, head.ourref,
          coa.acno, coa.acnoname, detail.ref, detail.postdate, detail.db, detail.cr,
          dclient.client as dclient, detail.checkno, head.project,detail.rem,head.trno,head.rem as headernotes,detail.sortline
          from glhead as head 
          left join gldetail as detail on detail.trno=head.trno 
          left join client on client.clientid=head.clientid

...
class pc
{
  ...
  public function default_query($filters)
  {
    ...
    item.itemname, stock.rrqty as qty, stock.uom, stock.rrcost as cost, stock.disc, stock.ext,stock.line,stock.sortline,
    ...
    left join client on client.client=head.wh
    left join item on item.itemid = stock.itemid
    where head.doc='pc' and md5(head.trno)='$trno' order by sortline,line";
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(125, 15, ' ' . (isset($arr_cost[$r]) ? $arr_cost[$r] : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);

...
class gj
{
  ...
  public function createreportfilter($config)
  {
    ...
    data_set($col1, 'received.label', 'Certified Correct By');
  ...
  public function report_default_query($filters)
  {
    ...
    detail.db, detail.cr, detail.line,detail.sortline,detail.rem
    from ((lahead as head 
    ...
    where head.doc='gj' and md5(head.trno)='$trno' order by sortline, line";
  ...
  public function default_GJ_header_PDF($params, $data)
  {
    ...
    PDF::SetFont($fontbold, '',  $fontsizeh);
    PDF::MultiCell(70, 18, 'SUPPLIER : ', '', 'L', false, 0);
    PDF::SetFont($font, '',  $fontsizeh1);
    PDF::MultiCell(330, 18, (isset($data[0]['clientname']) ? strtoupper($data[0]['clientname']) : ''), '', 'L', false, 0, '',  '');

    PDF::SetFont($fontbold, '',  $fontsizeh);
    PDF::MultiCell(165, 18, 'DATE : ', '', 'R', false, 0);
    PDF::SetFont($font, '',  $fontsizeh);
    PDF::MultiCell(130, 18, (isset($date) ? $date : ''), '', 'R', false);
  ...
  public function default_GJ_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      ...
      for ($i = 0; $i < count($data); $i++) {
        ...
        $maxrow = 1;
        $acnoname = $this->reporter->fixcolumn([$data[$i]['acnoname']], '25', 0);
        $ref = $this->reporter->fixcolumn([$data[$i]['ref']], '15', 0);
        $rem = $this->reporter->fixcolumn([$data[$i]['rem']], '31', 0);

        $countarr = count($acnoname);
        $countref = count($ref);
        $countrem = count($rem);

        $maxrow = max($countarr, $countref, $countrem);

        if ($data[$i]['acnoname'] == '') {
        } else {
          for ($r = 0; $r < $maxrow; $r++) {
            if ($r == 0) {
              $postdate = date("m-d-Y", strtotime($data[$i]['postdate']));
    ...
    PDF::MultiCell(253, 0, $params['params']['dataparams']['received'], '', 'L', false, 0);

...
class cv
{
  ...
  public function createreportfilter($config)
  {
    ...
    data_set(
      ...
      [
        ...
        ['label' => 'BDO CHECK', 'value' => '6', 'color' => 'blue'],
    ...
    data_set($col1, 'received.label', 'Received/Certified Correct By');
    ...
    switch ($filters['params']['dataparams']['reporttype']) {
      case 0:
        $query = "select head.trno, head.docno,date(head.dateid) as dateid, 
                        client.client, head.clientname, head.address,client.tin, head.terms, head.yourref, head.ourref, head.rem, coa.acno,
                        coa.acnoname, round(sum(detail.db),2) as db,round(sum(detail.cr),2) as cr,left(coa.alias,2) as alias
                ...
                group by coa.acno,coa.acnoname,head.dateid,head.trno, head.docno,
                          client.client, head.clientname, head.address,
                          client.tin, head.terms, head.yourref, head.ourref, head.rem, coa.alias
                union all
                ...
                group by coa.acno,coa.acnoname,head.dateid,head.trno, head.docno,
                          client.client, head.clientname, head.address,
                          client.tin, head.terms, head.yourref, head.ourref, head.rem,
                          coa.alias
                        order by cr";
      ...
      default:
        $query = "select head.trno, head.docno, 
            date(head.dateid) as dateid, detail.line,detail.sortline,
                          client.client, head.clientname, head.address,
                          client.tin, head.terms, head.yourref, head.ourref, head.rem, coa.acno,
                          coa.acnoname, round(sum(detail.db),2) as db,round(detail.cr,2) as cr, 
                          detail.checkno, left(coa.alias,2) as alias
                from ((lahead as head left join ladetail as detail on detail.trno=head.trno)
                left join client on client.client=head.client)left join coa on coa.acnoid=detail.acnoid
                left join cntnum on cntnum.trno=head.trno
                where head.doc='cv' and head.trno ='$trno'
                group by coa.acno,coa.acnoname,head.dateid,head.trno, head.docno,
                          client.client, head.clientname, head.address,
                          client.tin, head.terms, head.yourref, head.ourref, head.rem,
                          detail.cr,detail.checkno, coa.alias,detail.line,detail.sortline
                union all
  ...
  public function PDF_default_header($params, $data)
  {
    ...
    $qry = "select substring_index(checkno,',',-1) as checknum,substring_index(checkno,',',1) as bank from ladetail
    where trno='" . $data[0]['trno'] . "' and checkno<>''
    union all
    select substring_index(checkno,',',-1) as checknum,substring_index(checkno,',',1) as bank from gldetail
    where trno='" . $data[0]['trno'] . "' and checkno<>''";
    $bankdata = $this->coreFunctions->opentable($qry);

    $qry = "select sum(total) as total from (select sum(db) as total from ladetail where trno='" . $data[0]['trno'] . "'
    union all
    select sum(db) as total from gldetail where trno='" . $data[0]['trno'] . "' ) as a";

    $totaldata = $this->coreFunctions->opentable($qry);
    ...
    $d = date_create($d); // initialize for formating
    $d = date_format($d, "F d, Y"); // format for adding 

    PDF::SetFont($fontbold, '', 15);
    PDF::MultiCell(50, 0, '', '', 'L', false, 0); //-35
    PDF::MultiCell(530, 0, isset($data[0]['clientname']) ? strtoupper($data[0]['clientname']) : '', '', 'L', false, 0, '92',  '35');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(140, 0, isset($d) ? $d : '', '', 'R', false, 1, '580',  '35');


    PDF::MultiCell(620, 0, isset($data[0]['rem']) ? $data[0]['rem'] : '', '', 'L', false, 0, '40', '100');
    PDF::SetFont($fontbold, '', 15);
    PDF::MultiCell(140, 0, isset($totaldata[0]->total) ? number_format($totaldata[0]->total, 2) : '', '', 'R', false, 1, '550', '145');

    $bank = "select sum(cr) as cr from (select round(cr,2) as cr from ladetail as detail
          left join coa on coa.acnoid=detail.acnoid where trno = '" . $data[0]['trno'] . "' and (left(coa.alias,2) = 'CB' or coa.alias = 'AP99') 
          union all
          select round(cr,2) as cr from gldetail as detail
          left join coa on coa.acnoid=detail.acnoid where trno = '" . $data[0]['trno'] . "' and (left(coa.alias,2) = 'CB' or coa.alias = 'AP99')) as a";

    $bankresult = $this->coreFunctions->opentable($bank);

    $amtinwords = $this->ftNumberToWordsConverter($bankresult[0]->cr) . ' ONLY';
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(580, 0, isset($amtinwords) ? $amtinwords : '', '', 'L', false, 1, '175', '270');

    PDF::MultiCell(100, 0, isset($bankdata[0]->bank) ? $bankdata[0]->bank : '', '', 'L', false, 0, '522', '291');
    PDF::MultiCell(100, 0, isset($bankdata[0]->checknum) ? $bankdata[0]->checknum : '', '', 'L', false, 1, '695', '291');
  }
  ...
  public function PDF_DEFAULT_CCVOUCHER_LAYOUT1($data, $params)
  {
    ...
    PDF::SetFont($font, '', 3);
    PDF::MultiCell(720, 0, '', '', 'R', false, 1, '', '310');
      ...
      for ($i = 0; $i < count($data); $i++) {
        ...
        if ($data[$i]['acnoname'] == '') {
          ...
          for ($r = 0; $r < $maxrow; $r++) {
            ...
            PDF::SetFont($font, '', $fontsize);
            // PDF::MultiCell(70, 0, $acno, '', 'L', false, 0, '', '', true, 1);
            PDF::MultiCell(20, 0, '', '', 'L', false, 0);
            PDF::MultiCell(160, 0, isset($acnoname[$r]) ? $acnoname[$r] : '', '', 'L', false, 0, '40', '', true, 1);
            PDF::MultiCell(85, 0, $debit, '', 'R', false, 0, '266', '', false, 1);
            PDF::MultiCell(85, 0, $credit, '', 'R', false, 1, '386', '', false, 1);
          }
        }
        $totaldb += $data[$i]['db'];
        $totalcr += $data[$i]['cr'];
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(240, 0, $params['params']['dataparams']['prepared'], '', 'C', false, 0, '2',  '406');
    PDF::MultiCell(240, 0, $params['params']['dataparams']['received'], '', 'C', false, 0, '270', '');
    PDF::MultiCell(240, 0, $params['params']['dataparams']['approved'], '', 'C', false, 1, '', '');


    return PDF::Output($this->modulename . '.pdf', 'S');
  } //end fn

  public function PDF_BDO_CHECK_LAYOUT($data, $params)
  {
    $companyid = $params['params']['companyid'];
    ...
    $qry = "select date(detail.postdate) as postdate,detail.checkno,coa.acno,detail.cr 
      from ladetail as detail left join coa on coa.acnoid = detail.acnoid
      where trno = " . $data[0]['trno'] . " and left(coa.alias,2) = 'CB' and left(coa.alias,2) = 'CB' 
      group by detail.checkno,coa.acno,detail.cr,detail.postdate
      UNION ALL
      select date(detail.postdate) as postdate,detail.checkno,coa.acno,detail.cr 
      from gldetail as detail left join coa on coa.acnoid = detail.acnoid
      where trno = " . $data[0]['trno'] . " and left(coa.alias,2) = 'CB' and left(coa.alias,2) = 'CB' 
      group by detail.checkno,coa.acno,detail.cr, detail.postdate";
    ...
    for ($i = 0; $i < count($data2); $i++) {
      ...
      PDF::MultiCell(220, 0, isset($cdate) ? date('m', strtotime($cdate)) . '   ' . date('d', strtotime($cdate)) . '  ' . date('Y', strtotime($cdate)) : '', '', 'L', false, 1);

      PDF::MultiCell(720, 10, '', '', 'L', false, 1);

      PDF::MultiCell(80, 0, '', '', 'C', false, 0);
      PDF::MultiCell(390, 0, $data[0]['clientname'], '', 'L', false, 0);
      PDF::MultiCell(250, 0, (isset($cc) ? number_format($cc, $decimal) : ''), '', 'L', false, 1);
      ...
      PDF::MultiCell(640, 0, $this->ftNumberToWordsConverter($dd) . ' ONLY', '', 'L', false);
  ...
  public function PDF_BPI_CHECK_LAYOUT($data, $filters)
  {
    ...
    for ($i = 0; $i < count($data2); $i++) {
      ...
      PDF::SetFont($font, '', 11);
      PDF::MultiCell(500, 0, '', '', 'C', false, 0);
      PDF::MultiCell(220, 0, isset($data2[$i]['postdate']) ? date('m', strtotime($data2[$i]['postdate'])) . ' ' . date('d', strtotime($data2[$i]['postdate'])) . ' ' . date('Y', strtotime($data2[$i]['postdate'])) : '', '', 'L', false, 1);

      PDF::MultiCell(720, 5, '', '', 'L', false, 1);

      PDF::MultiCell(80, 0, '', '', 'C', false, 0);
      PDF::MultiCell(390, 0, '**' . strtoupper($data[0]['clientname']) . '**', '', 'L', false, 0);
      PDF::MultiCell(250, 0, isset($cc) ? '**' . number_format($cc, $decimal) . '**' : '', '', 'L', false, 1);
      ...
      PDF::MultiCell(660, 0, '**' . $this->ftNumberToWordsConverter($dd) . ' ONLY**', '', 'L', false);
    }
    return PDF::Output($this->modulename . '.pdf', 'S');
  } //end fn
  ...
  public function PDF_EASTWEST_CHECK_LAYOUT($data, $filters)
  {
    ...
    for ($i = 0; $i < count($data2); $i++) {
      ...
      $dd = number_format((float)$cc, 2, '.', '');
      PDF::MultiCell(30, 0, '', '', 'C', false, 0);
      PDF::MultiCell(690, 0, '**' . $this->ftNumberToWordsConverter($dd) . ' ONLY**', '', 'L', false);
    }
    return PDF::Output($this->modulename . '.pdf', 'S');
  } //end fn

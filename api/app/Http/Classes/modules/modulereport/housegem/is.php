  ...
  public function default_IS_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $maxrow = $this->othersClass->getmaxcolumn([$arr_qty, $arr_uom,$arr_itemname, $arr_gross, $arr_ext]);
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(125, 15, ' ' . (isset($arr_gross[$r]) ? $arr_gross[$r] : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        if ($data[$i]['itemname'] == '') {
          ...
          PDF::MultiCell(125, 0, number_format($data[$i]['gross'], $decimalprice), '', 'R', false, 0, '', '', false, 1);
          PDF::MultiCell(125, 0, number_format($data[$i]['ext'], $decimalcurr), '', 'R', false, 1, '', '', false, 1);
        } else {
          for ($r = 0; $r < $maxrow; $r++) {
            if ($r == 0) {
              $barcode =  $data[$i]['barcode'];

...
class ap
{
  ...
  public function report_default_query($config)
  {
    ...
    $query = "
      ...
      detail.cr, detail.client as dclient, detail.checkno,detail.line,detail.sortline
      ...
      coa.acno, coa.acnoname, detail.ref, date(detail.postdate) as postdate, detail.db, detail.cr, dclient.client as dclient, detail.checkno,detail.line,detail.sortline
      ...
      where head.doc='ap' and head.trno='$trno' order by sortline,line";
  ...
  public function default_AP_header_PDF($params, $data)
  {
    ...
    PDF::MultiCell(115, 0, "CLIENT", '', 'C', false);
    ...
    if (!empty($data)) {
      ...
      for ($i = 0; $i < count($data); $i++) {
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(115, 15, ' ' . (isset($arr_client[$r]) ? $arr_client[$r] : ''), '', 'L', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
        }
        $totaldb += $data[$i]['db'];
        $totalcr += $data[$i]['cr'];

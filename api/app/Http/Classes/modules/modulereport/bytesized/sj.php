...
namespace App\Http\Classes\modules\modulereport\bytesized;
...
class sj
{
  ...
  public function createreportfilter($config)
  {
    ...
    $fields = ['radioprint', 'prepared', 'approved', 'received', 'radiotypeofreportformat', 'print'];

    $col1 = $this->fieldClass->create($fields);
    data_set($col1, 'radioprint.options', [
      ['label' => 'PDF', 'value' => 'PDFM', 'color' => 'red'],
    ]);
    data_set($col1, 'radiotypeofreportformat.options', [
      ['label' => 'Default', 'value' => 'default', 'color' => 'red'],
      ['label' => 'RS', 'value' => 'rs', 'color' => 'red']
    ]);
  ...
  public function report_default_query($config)
  {
    ...
    $query = "select stock.line,stock.rem as srem,head.rem,date_format(head.dateid,'%m/%d') as monthid,
            ...
            if(info.itemdesc<>'',info.itemdesc,item.itemname) as itemname, stock.isqty as qty, stock.uom , stock.isamt as amt, stock.disc, stock.ext, head.agent,
            ...
            right(year(head.dateid),2) as year,left(head.dateid,10) as dateid, head.docno, client.client, client.clientname,
            head.address, head.terms, item.barcode, head.shipto, client.tin, head.yourref, head.ourref,
            if(info.itemdesc<>'',info.itemdesc,item.itemname) as itemname, stock.isqty as qty, stock.uom , stock.isamt as amt, stock.disc, stock.ext, ag.client as agent,
            item.sizeid, ag.clientname as agname, item.brand,
  ...
  public function default_sj_header_PDF($params, $data)
  {
    ...
    $logoonly = false;
    if ($params['params']['dataparams']['typeofformat'] == 'rs') {
      $params['params']['logoonly'] = true;
      $logoonly = true;
    }
    ...
    if ($logoonly) {
      PDF::Image($this->companysetup->getlogopath($params['params']) . 'rssclogo.jpg', 250, 30, 320, 50);
    }
  ...
  public function default_sj_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $uom = $data[$i]['uom'];
        $amt = number_format($data[$i]['ext'] / $data[$i]['qty'], 2);
        // $disc = $data[$i]['disc'];

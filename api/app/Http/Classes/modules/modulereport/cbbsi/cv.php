...
class cv
{
  ...
  public function report_default_query($filters)
  {
    ...
    switch ($filters['params']['dataparams']['reporttype']) {
      ...
      default:
        ...
        $query = "select ifnull(DATE_FORMAT(cb.checkdate,'%Y-%m-%d'),DATE_FORMAT(head.dateid,'%Y-%m-%d')) as kdate, ifnull(head2.yourref,'') as dyourref,
        DATE_FORMAT(left(detail.postdate,10),'%b %d %Y') as pdate,detail.ref,head.trno, head.docno, 
        date(head.dateid) as dateid,
        (case
       when head3.yourref = 'sv' then head3.yourref
       else head2.yourref  end)  as allyourref,
        ...
        detail.checkno, left(coa.alias,2) as alias, head2.yourref as invoiceno ,ifnull(head2.rem,'') as snrem,left(detail.ref,2) as reff,head3.yourref as svyourref
        ...
        left join cntnum on cntnum.trno=head.trno
        left  join hsvhead as head3 on head3.trno=detail.pcvtrno
        LEFT JOIN cbledger AS cb ON cb.trno = detail.trno AND cb.line = detail.line
        where head.doc='cv' and head.trno ='$trno' and left(coa.alias,2) not in ('CA', 'CB', 'CR')
        union all
        select ifnull(DATE_FORMAT(cb.checkdate,'%Y-%m-%d'),DATE_FORMAT(head.dateid,'%Y-%m-%d')) as kdate, ifnull(head2.yourref,'') as dyourref,
        DATE_FORMAT(left(detail.postdate,10),'%b %d %Y') as pdate,detail.ref,head.trno, head.docno, 
        date(head.dateid) as dateid,
        (case
       when head3.yourref = 'sv' then head3.yourref
        ...
        client.tin, '' as busstyle, head.terms, head.yourref, head.ourref, head.rem, coa.acno,
        coa.acnoname, detail.rem as drem,round(detail.db,2) as db,round(detail.cr,2) as cr, 
        detail.checkno, left(coa.alias,2) as alias, head2.yourref as invoiceno ,ifnull(head2.rem,'') as rem,left(detail.ref,2) as reff,head3.yourref as yourref3
  ...
  public function PDF_default_header($params, $data)
  {
    ...
    $arr_address = $this->reporter->fixcolumn([$address], '60');
    $addr_maxrow = $this->othersClass->getmaxcolumn([$arr_address]);
    PDF::MultiCell(425, 5, $address, '', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', 14);
    PDF::MultiCell(105, 5, 'REFERENCE # :', '', 'L', false, 0);
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(100, 5, (isset($data[0]['yourref']) ? $data[0]['yourref'] : ''), '', 'R', false);

    for ($am = 1; $am < $addr_maxrow; $am++) {
      PDF::MultiCell(0, 0, "\n");
    }
    ...
    PDF::MultiCell(90, 5, 'ACCT #', '', 'C', false, 0);
    PDF::MultiCell(165, 5, 'ACCOUNT NAME', '', 'C', false, 0);
    PDF::MultiCell(100, 5, 'YOURREF', '', 'C', false, 0);
    PDF::MultiCell(75, 5, 'DATE', '', 'C', false, 0);
    PDF::MultiCell(85, 5, 'DEBIT', '', 'R', false, 0);
    PDF::MultiCell(85, 5, 'CREDIT', '', 'R', false, 0);
    PDF::MultiCell(10, 5, '', '', 'C', false, 0);
    PDF::MultiCell(110, 5, 'NOTES', '', 'C', false);
  ...
  public function PDF_DEFAULT_CCVOUCHER_LAYOUT1($data, $params)
  {
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        if ($data[$i]['reff'] == 'SN') {
          $drem = $data[$i]['snrem'];
        } elseif ($data[$i]['reff'] == 'SV') {
          $drem = $data[$i]['drem'];
        } else {
          $drem = $data[$i]['rem'];
        }
        $youref = $data[$i]['allyourref'];
        $drem = $data[$i]['drem'];
        ...
        $arr_yourref = $this->reporter->fixcolumn([$youref], '28', 0);
        ...
        $arr_drem = $this->reporter->fixcolumn([$drem], '15', 0);

        $maxrow = $this->othersClass->getmaxcolumn([$arr_acno, $arr_acnonamedescs, $arr_yourref, $arr_pdate, $arr_debit, $arr_credit, $arr_drem]);

        for ($r = 0; $r < $maxrow; $r++) {
          PDF::SetFont($font, '', $fontsize);
          PDF::MultiCell(90, 15, ' ' . (isset($arr_acno[$r]) ? $arr_acno[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(165, 15, ' ' . (isset($arr_acnonamedescs[$r]) ? $arr_acnonamedescs[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(100, 15, ' ' . (isset($arr_yourref[$r]) ? $arr_yourref[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(75, 15, ' ' . (isset($arr_pdate[$r]) ? $arr_pdate[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
    ...
    $qry2 = "select ifnull(DATE_FORMAT(cb.checkdate,'%Y-%m-%d'),DATE_FORMAT(detail.postdate,'%Y-%m-%d')) as kdate,detail.checkno,
      coa.acnoname as checkname,round(detail.cr,2)  as amount
      from lahead as head
      left join ladetail as detail on detail.trno=head.trno
      LEFT JOIN cbledger AS cb ON cb.trno = detail.trno AND cb.line = detail.line
      left join coa as coa on coa.acnoid=detail.acnoid
      where detail.cr<>0 and
      head.doc='cv' and head.trno ='$trno' and left(coa.alias,2)='CB'
      union all
      select ifnull(DATE_FORMAT(cb.checkdate,'%Y-%m-%d'),DATE_FORMAT(detail.postdate,'%Y-%m-%d')) as kdate,detail.checkno,
      coa.acnoname as checkname,round(detail.cr,2) as amount
      from glhead as head
      left join gldetail as detail on detail.trno=head.trno
      LEFT JOIN cbledger AS cb ON cb.trno = detail.trno AND cb.line = detail.line
    ...
    PDF::SetFont($fontbold, '', 11);
    PDF::MultiCell(300, 0, 'CHECK NO:', '', 'L', false, 0);
    PDF::MultiCell(150, 0, 'DATE:', '', 'L', false, 0);
    PDF::MultiCell(250, 0, 'AMOUNT:', '', 'L', false);
    ...
    if (!empty($data2)) {
      foreach ($data2 as $i => $value) {
        $acnonamedescs = $value->checkname;
        $amtdate = $value->kdate;
        $amount = number_format($value->amount,  $decimalcurr);
        $amount = $amount < 0 ? '-' : $amount;
        $checkno = $value->checkno;

        $arr_amtdate = $this->reporter->fixcolumn([$amtdate], '15', 0);
        $arr_amount = $this->reporter->fixcolumn([$amount], '15', 0);
        $arr_checkno = $this->reporter->fixcolumn([$checkno], '30', 0);

        $maxrow = $this->othersClass->getmaxcolumn([$arr_amtdate, $arr_amount, $arr_checkno]);

        for ($r = 0; $r < $maxrow; $r++) {
          PDF::SetFont($font, '', $fontsize);
          PDF::MultiCell(300, 0, ' ' . (isset($arr_checkno[$r]) ? $arr_checkno[$r] : ''), '', 'L', false, 0);
          PDF::MultiCell(150, 0, ' ' . (isset($arr_amtdate[$r]) ? $arr_amtdate[$r] : ''), '', 'L', false, 0);
          PDF::MultiCell(250, 0, ' ' . (isset($arr_amount[$r]) ? $arr_amount[$r] : ''), '', 'L', false);
        }
    ...
    PDF::SetFont($font, '', 5);
    PDF::MultiCell(700, 0, '', '');
    $qry = "select name from center where code = '" . $params['params']['center'] . "'";
    $centername = $this->coreFunctions->opentable($qry);
    $fntotalbal = number_format((float) $totalamt, 2, '.', '');
    ...
    PDF::MultiCell(0, 0, "\n\n");
    PDF::SetFont($fontbold, '', 11);
    PDF::MultiCell(700, 0, 'Remarks: ', '', 'L', false);


    PDF::MultiCell(0, 0, "\n\n\n");
    PDF::SetFont($font, '', 12);

    PDF::MultiCell(222, 0, 'Prepared By: ', '', 'L', false, 0);
    PDF::MultiCell(17, 0, '', '', 'L', false, 0);
    PDF::MultiCell(222, 0, 'Checked By: ', '', 'L', false, 0);
    PDF::MultiCell(17, 0, '', '', 'L', false, 0);
    PDF::MultiCell(222, 0, 'Received By: ', '', 'L');


    PDF::MultiCell(0, 0, "\n");

    PDF::MultiCell(222, 0, $params['params']['dataparams']['prepared'], 'B', 'L', false, 0);
    PDF::MultiCell(17, 0, '', '', 'L', false, 0);
    PDF::MultiCell(222, 0, $params['params']['dataparams']['checked'], 'B', 'L', false, 0);
    PDF::MultiCell(17, 0, '', '', 'L', false, 0);
    PDF::MultiCell(222, 0, $params['params']['dataparams']['received'], 'B', 'L');

    PDF::MultiCell(0, 0, "\n");

    PDF::SetFont($font, '', 12);

    PDF::MultiCell(222, 0, 'Approved By: ', '', 'L', false, 0);
    PDF::MultiCell(256, 0, '', '', 'L', false, 0);
    PDF::MultiCell(50, 0, 'OR No: ', '', 'L', false, 0);
    PDF::MultiCell(172, 0, '', 'B', 'L', false);

    PDF::MultiCell(0, 0, "\n");
    PDF::SetFont($font, '', 12);
    PDF::MultiCell(222, 0, $params['params']['dataparams']['approved'], 'B', 'L', false, 0);
    PDF::MultiCell(256, 0, '', '', 'L', false, 0);

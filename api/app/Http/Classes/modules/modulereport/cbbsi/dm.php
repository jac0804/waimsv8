...
class dm
{
  ...
  {
    $query = "select m.model_name as model,item.sizeid,date(head.dateid) as dateid, head.docno,whname.clientname as wh, concat(client.client ,'~',client.clientname) as clname,
  head.address, head.terms,head.rem, item.barcode,stock.rrcost,head.ourref,head.yourref,stock.cost,
...
left join client as whname on whname.client = head.wh
  left join item on item.itemid = stock.itemid
  left join model_masterfile as m on m.model_id = item.model
  where head.doc='dm' and head.trno ='$trno'
  union all
  select m.model_name as model,item.sizeid,
  date(head.dateid) as dateid, head.docno, whname.clientname as wh,concat(client.client ,'~',client.clientname) as clname,
  head.address, head.terms,head.rem, item.barcode,stock.rrcost,head.ourref,head.yourref,stock.cost,
  item.itemname, stock.isqty as qty, stock.uom, stock.isamt as amt, stock.disc, stock.ext,stock.ref,date(stock.expiry) as expiry
  from glhead as head
  left join glstock as stock on stock.trno=head.trno
  left join client as whname on whname.clientid = head.whid
  ...
  public function default_DM_header_PDF($params, $data)
  {
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(470, 0, (isset($data[0]['clname']) ? $data[0]['clname'] : ''), 'B', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(120, 0, (isset($data[0]['ourref']) ? $data[0]['ourref'] : ''), 'B', 'L', false, 1, '',  '');
    ...
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(50, 0, "Yorref: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    ...
    PDF::MultiCell(100, 0, "BASE PRICE", '', 'R', false, 0);
  ...
  public function default_DM_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $uom = $data[$i]['uom'];
        $basecost = number_format($data[$i]['amt'], 2);
        $disc = $data[$i]['disc'];

        if ($disc != '') {
          $discamt = ($this->othersClass->discount($data[$i]['amt'], $disc));
          $price =  $discamt;
        ...
        $arr_basecost = $this->reporter->fixcolumn([$basecost], '13', 0);
        $arr_disc = $this->reporter->fixcolumn([$disc], '13', 0);
        $arr_price = $this->reporter->fixcolumn([$price], '13', 0);
        $arr_totalamt = $this->reporter->fixcolumn([number_format($totalamt, 2)], '13', 0);


        $maxrow = $this->othersClass->getmaxcolumn([$arr_barcode, $arr_itemname, $arr_qty, $arr_uom, $arr_basecost, $arr_disc, $arr_price, $arr_totalamt]);
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(100, 15, ' ' . (isset($arr_basecost[$r]) ? $arr_basecost[$r] : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          ...
          PDF::MultiCell(100, 15, ' ' . (isset($arr_totalamt[$r]) ? $arr_totalamt[$r] : ''), '', 'R', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
        }
    ...
    PDF::MultiCell(100, 0, number_format($grandtotal, $decimalcurr), '', 'R');

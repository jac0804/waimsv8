...
class rt
{
  ...
  private $modulename = "Temporary Receiving Report";
  private $fieldClass;
  ...
  public function report_default_query($trno)
  {
    $query = "select date(head.dateid) as dateid, head.docno, client.client, client.clientname, head.address,
        ...
        head.yourref,head.ourref,hinfo.carrier,hinfo.waybill,client.fax,stock.ref,concat(left(head.wh,2),right(head.wh,1)) as wh,
        warehouse.clientname as whname,ifnull(uom.factor,1) as factor
        ...
        where head.doc='rt' and head.trno='$trno'
        union all
  ...
  public function reportplotting($params, $data)
  {
    if ($params['params']['dataparams']['print'] == "default") {
      return $this->default_rt_layout($params, $data);
    } else if ($params['params']['dataparams']['print'] == "PDFM") {
      return $this->default_RT_PDF($params, $data);
    }
  }
  ...
  public function default_rt_layout($params, $data)
  {
    $companyid = $params['params']['companyid'];
  ...
  public function default_RT_header_PDF($params, $data)
  {
    $companyid = $params['params']['companyid'];
    $center = $params['params']['center'];
    $username = $params['params']['user'];
    ...
    PDF::MultiCell(150, 0, (isset($data[0]['docno']) ? $data[0]['docno'] : ''), '', 'L', false, 1);
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(150, 0, (isset($data[0]['dateid']) ? $data[0]['dateid'] : ''), 'B', 'L', false, 0, '',  '');

    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(0, 30, "", '', 'L');
    PDF::MultiCell(80, 0, "Supplier: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(400, 0, (isset($data[0]['clientname']) ? $data[0]['clientname'] : ''), 'B', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(70, 0, "Terms:", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(150, 0, (isset($data[0]['terms']) ? $data[0]['terms'] : ''), 'B', 'L', false, 0, '',  '');
    ...
    PDF::MultiCell(400, 0, $address, 'B', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(70, 0, "Ourref:", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(150, 0, (isset($data[0]['ourref']) ? $data[0]['ourref'] : ''), 'B', 'L', false, 0, '',  '');

    for ($am = 0; $am < $addr_maxrow; $am++) {
      PDF::MultiCell(0, 0, "\n");
    }
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(400, 0, (isset($data[0]['wh']) ? $data[0]['wh'] : ''), 'B', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(70, 0, "Yourref:", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(150, 0, (isset($data[0]['yourref']) ? $data[0]['yourref'] : ''), 'B', 'L', false, 1);


    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(80, 0, "Carrier: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(400, 0, (isset($data[0]['carrier']) ? $data[0]['carrier'] : ''), 'B', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(70, 0, "Way Bill:", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(150, 0, (isset($data[0]['waybill']) ? $data[0]['waybill'] : ''), 'B', 'L', false, 0, '',  '');
    ...
    PDF::MultiCell(50, 0, "PCS", '', 'C', false, 0);
    PDF::MultiCell(255, 0, "DESCRIPTION", '', 'L', false, 0);
    PDF::MultiCell(50, 0, "WH", '', 'C', false, 0);
    PDF::MultiCell(110, 0, "REMARKS", '', 'C', false);
    PDF::SetFont($font, '', 5);
    PDF::MultiCell(700, 0, '', 'B');
  }

  public function default_RT_PDF($params, $data)
  {
    $companyid = $params['params']['companyid'];
    ...
    $this->default_RT_header_PDF($params, $data);
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $uom = $data[$i]['uom'];
      $pcs =  number_format($data[$i]['qty'] * $data[$i]['factor'], $decimalcurr);
      ...
      $arr_pcs = $this->reporter->fixcolumn([$pcs], '15', 0);
      $arr_itemname = $this->reporter->fixcolumn([$itemname], '36', 0);
      $arr_wh = $this->reporter->fixcolumn([$wh], '15', 0);

      $maxrow = $this->othersClass->getmaxcolumn([$arr_barcode, $arr_itemname, $arr_qty, $arr_uom,  $arr_pcs, $arr_wh]);

      for ($r = 0; $r < $maxrow; $r++) {

        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(125, 0, ' ' . (isset($arr_barcode[$r]) ? $arr_barcode[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
        PDF::MultiCell(50, 0, ' ' . (isset($arr_qty[$r]) ? $arr_qty[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
        PDF::MultiCell(60, 0, ' ' . (isset($arr_uom[$r]) ? $arr_uom[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
        PDF::MultiCell(50, 0, ' ' . (isset($arr_pcs[$r]) ? $arr_pcs[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
        PDF::MultiCell(255, 0, ' ' . (isset($arr_itemname[$r]) ? $arr_itemname[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
        PDF::MultiCell(50, 0, ' ' . (isset($arr_wh[$r]) ? $arr_wh[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
        if ($r >= 1) {
          PDF::MultiCell(110, 0, '', '', 'L', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
        } else {
          PDF::MultiCell(110, 0, '', 'B', 'L', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
        }
      }
    ...
    PDF::MultiCell(650, 0, (isset($data[0]['rem']) ? $data[0]['rem'] : ''), '', 'L', false, 1);

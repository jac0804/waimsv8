...
class ar
{
  ...
  {
    $fields = ['radioprint', 'radioreporttype', 'prepared', 'approved', 'checked', 'print'];
    ...
    data_set($col1, 'radioreporttype.options', [
      ...
      ['label' => 'Charge Slip', 'value' => '1', 'color' => 'orange']
    ]);
  ...
  public function reportplotting($params, $data)
  {
    ...
    switch ($reporttype) {
      ...
      case '1':
        return $this->default_Charge_Slip_PDF($params, $data);
        break;
    }
  }
    ...
    switch ($reporttype) {
      ...
      case '1':
        $query = "select head.trno, date(head.dateid) as dateid, head.docno, head.clientname, head.address,
          ...
          where head.doc='" . $config['params']['doc'] . "' and head.trno='" . $trno . "'
          union all
          ...
          where head.doc='" . $config['params']['doc'] . "' and head.trno='" . $trno . "' order by line";
        break;
    }
  ...
  public function default_AR_header_PDF($params, $data)
  {
    ...
    PDF::MultiCell(520, 0, 'AR Setup', '', 'L', false, 0, '',  '100');
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(90, 0, (isset($data[0]['ourref']) ? $data[0]['ourref'] : ''), 'B', 'L', false, 1, '',  '');
    ...
    PDF::MultiCell(95, 0, "REFERENCE #", '', 'L', false, 0);
    PDF::MultiCell(75, 0, "DATE", '', 'C', false, 0);
    PDF::MultiCell(90, 0, "DEBIT", '', 'R', false, 0);
    PDF::MultiCell(90, 0, "CREDIT", '', 'R', false, 0);
  ...
  public function default_AR_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(160, 15, ' ' . (isset($arr_acnonamedescs[$r]) ? $arr_acnonamedescs[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(95, 15, ' ' . (isset($arr_ref[$r]) ? $arr_ref[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(75, 15, ' ' . (isset($arr_postdate[$r]) ? $arr_postdate[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(90, 15, ' ' . (isset($arr_debit[$r]) ? $arr_debit[$r] : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(90, 15, ' ' . (isset($arr_credit[$r]) ? $arr_credit[$r] : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
    ...
    PDF::MultiCell(240, 0, 'Approved By: ', '', 'L');
    ...
    PDF::MultiCell(240, 0, $params['params']['dataparams']['approved'], '', 'L');

    return PDF::Output($this->modulename . '.pdf', 'S');
  }
  public function default_charge_slip_header_PDF($params, $data)
  {
    $center = $params['params']['center'];
    $username = $params['params']['user'];
    ...
    PDF::SetFont($fontbold, '', 18);
    PDF::MultiCell(570, 0, 'CHARGE SLIP', '', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(60, 0, "Docno #: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', 10);
    PDF::MultiCell(90, 0, (isset($data[0]['docno']) ? $data[0]['docno'] : ''), 'B', 'L', false, 1, '',  '');
    PDF::MultiCell(0, 0, "\n\n");

    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(80, 15, "Customer: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(490, 15, (isset($data[0]['clientname']) ? $data[0]['clientname'] : ''), 'B', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(60, 15, "Date: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(90, 15, (isset($data[0]['dateid']) ? $data[0]['dateid'] : ''), 'B', 'L', false, 1, '',  '');

    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(80, 15, "Address: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(490, 15, (isset($data[0]['address']) ? $data[0]['address'] : ''), 'B', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(60, 15, "YOURREF: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(90, 15, (isset($data[0]['yourref']) ? $data[0]['yourref'] : ''), 'B', 'L', false, 1, '',  '');

    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(80, 15, "Notes : ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(490, 15, (isset($data[0]['rem']) ? $data[0]['rem'] : ''), 'B', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(60, 15, "OURREF: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(90, 15, (isset($data[0]['ourref']) ? $data[0]['ourref'] : ''), 'B', 'L', false, 1, '',  '');
    ...
    PDF::SetFont($font, 'B', 12);
    PDF::MultiCell(420, 0, "PARTICULARS", '', 'L', false, 0);
    PDF::MultiCell(150, 0, "", '', 'L', false, 0);
    PDF::MultiCell(150, 0, "AMOUNT", '', 'R', false);

    PDF::SetFont($font, '', 5);
    PDF::MultiCell(720, 0, '', 'B');
  }

  public function  default_Charge_Slip_PDF($params, $data)
  {
    $font = "";
    $fontbold = "";
    $border = "1px solid ";
    ...
    $this->default_charge_slip_header_PDF($params, $data);
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $arr_amount = $this->reporter->fixcolumn([number_format($data[$i]['amount'], 2)], '20', 0);
        $maxrow = $this->othersClass->getmaxcolumn([$arr_part, $arr_qty, $arr_amount]);
        for ($r = 0; $r < $maxrow; $r++) {
          PDF::SetFont($font, '', $fontsize);
          PDF::MultiCell(420, 0, ' ' . (isset($arr_part[$r]) ? $arr_part[$r] : ''), '', 'L', false, 0);
          PDF::MultiCell(150, 0, ' ' . (isset($arr_qty[$r]) ? $arr_qty[$r] : ''), '', 'L', false, 0);
          PDF::MultiCell(150, 0, ' ' . (isset($arr_amount[$r]) ? $arr_amount[$r] : ''), '', 'R', false);
        }
        $totalamt += $data[$i]['amount'];
        if (PDF::getY() > 900) $this->default_charge_slip_header_PDF($params, $data);
      }
    }
    ...
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(570, 0, 'TOTAL: ', '', 'R', false, 0);
    PDF::MultiCell(150, 0, number_format($totalamt, 2), '', 'R', false, 1);

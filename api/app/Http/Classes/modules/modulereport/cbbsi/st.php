...
class st
{
  ...
  {
    $fields = ['radioprint', 'radioreporttype', 'approved', 'issued', 'checked', 'received', 'print'];
    $col1 = $this->fieldClass->create($fields);

    data_set($col1, 'radioreporttype.options', [
      ['label' => 'Stock Transfer', 'value' => 0, 'color' => 'red'],
      ['label' => 'Stock Transfer with ID', 'value' => 1, 'color' => 'red'],
  ...
  public function report_default_query($trno)
  {
    ...
    $query = "select head.docno,head.trno, head.clientname, head.address, date(head.dateid) as dateid, head.terms, head.rem,
        item.itemid as items,item.barcode, item.itemname, stock.rrcost as gross, stock.isamt as netamt, stock.isqty as qty,
        stock.uom, stock.disc, stock.ext, stock.line,wh.client as wh,wh.clientname as whname,stock.loc,date(stock.expiry) as expiry,stock.rem as srem,item.sizeid,m.model_name as model,head.wh as source,head.client as destination,head.yourref,head.ourref,head.trnxtype
        from lahead as head 
        ...
        item.itemid as items,item.barcode, item.itemname, stock.rrcost as gross, stock.isamt as netamt, stock.isqty as qty,
        stock.uom, stock.disc, stock.ext, stock.line,wh.client as wh,wh.clientname as whname,stock.loc,date(stock.expiry) as expiry,stock.rem as srem,item.sizeid,m.model_name as model,source.client as source,dest.client as destination,head.yourref,head.ourref,head.trnxtype
        from glhead as head 
  ...
  public function reportplotting($params, $data)
  {
    ...
    switch ($print) {
      case 'PDFM':
        ...
        switch ($reporttype) {
          case 0: //default stock transfer 
            return $this->default_ST_PDF($params, $data);
            break;
          case 1: //stock transfer with id
            return $this->default_ST_withid_PDF($params, $data);
            break;
        }
        break;
      default:
  ...
  public function default_header($params, $data)
  {
    ...
    $str .= $this->reporter->col('', '20', null, false, $border, '', 'L', $font, $fontsize, '', '30px', '4px');
    $str .= $this->reporter->col('DATE : ', '80', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '');
    $str .= $this->reporter->col((isset($data[0]['dateid']) ? $data[0]['dateid'] : ''), '80', null, false, $border, 'B', 'L', $font, $fontsize, '', '', '');
    $str .= $this->reporter->endrow();
    ...
    $str .= $this->reporter->col('', '20', null, false, $border, '', 'L', $font, $fontsize, '', '30px', '4px');
    $str .= $this->reporter->col('Ourref : ', '80', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '');
    $str .= $this->reporter->col((isset($data[0]['ourref']) ? $data[0]['ourref'] : ''), '80', null, false, $border, 'B', 'L', $font, $fontsize, '', '', '');
    $str .= $this->reporter->endrow();

    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('Destination : ', '120', null, false, $border, '', 'L', $font, $fontsize, 'B', '30px', '4px');
    $str .= $this->reporter->col((isset($data[0]['destination']) ? $data[0]['destination'] : ''), '520', null, false, $border, 'B', 'L', $font, $fontsize, '', '30px', '4px');
    $str .= $this->reporter->col('', '20', null, false, $border, '', 'L', $font, $fontsize, '', '30px', '4px');
    $str .= $this->reporter->col('Yourref : ', '80', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '');
    $str .= $this->reporter->col((isset($data[0]['yourref']) ? $data[0]['yourref'] : ''), '80', null, false, $border, 'B', 'L', $font, $fontsize, '', '30px', '4px');
    $str .= $this->reporter->endrow();

    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('', '120', null, false, $border, '', 'L', $font, $fontsize, '', '30px', '4px');
    $str .= $this->reporter->col('', '520', null, false, $border, '', 'L', $font, $fontsize, '', '30px', '4px');
    $str .= $this->reporter->col('', '20', null, false, $border, '', 'L', $font, $fontsize, '', '30px', '4px');
    $str .= $this->reporter->col('Trnx Type:', '80', null, false, $border, '', 'L', $font, $fontsize, 'B', '30px', '4px');
    $str .= $this->reporter->col((isset($data[0]['trnxtype']) ? $data[0]['trnxtype'] : ''), '80', null, false, $border, 'B', 'L', $font, $fontsize, '', '30px', '4px');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();
    ...
    switch ($params['params']['dataparams']['reporttype']) {
      case 1:
        $str .= $this->reporter->col('ITEM ID', '75', null, false, $border, 'TB', 'C', $font, '12', 'B', '30px', '8px');
        $str .= $this->reporter->col('BARCODE', '75', null, false, $border, 'TB', 'C', $font, '12', 'B', '30px', '8px');
        $str .= $this->reporter->col('QTY', '75', null, false, $border, 'TB', 'C', $font, '12', 'B', '30px', '8px');
        $str .= $this->reporter->col('UNIT', '75', null, false, $border, 'TB', 'C', $font, '12', 'B', '30px', '8px', 0, '', 0, 1);
        $str .= $this->reporter->col('DESCRIPTION', '400', null, false, $border, 'TB', 'C', $font, '12', 'B', '30px', '8px');
        $str .= $this->reporter->col('PRICE', '100', null, false, $border, 'TB', 'C', $font, '12', 'B', '30px', '8px');
        break;
      default:
        $str .= $this->reporter->col('BARCODE', '50', null, false, $border, 'TB', 'C', $font, '12', 'B', '30px', '8px');
        ...
        $str .= $this->reporter->col('DESCRIPTION', '375', null, false, $border, 'TB', 'C', $font, '12', 'B', '30px', '8px');
        $str .= $this->reporter->col('UNIT PRICE', '100', null, false, $border, 'TB', 'C', $font, '12', 'B', '30px', '8px');
        $str .= $this->reporter->col('(+/-) %', '75', null, false, $border, 'TB', 'C', $font, '12', 'B', '30px', '8px');
        $str .= $this->reporter->col('TOTAL', '100', null, false, $border, 'TB', 'C', $font, '12', 'B', '30px', '8px');
        break;
    }
  ...
  public function default_ST_LAYOUT($params, $data)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      switch ($params['params']['dataparams']['reporttype']) {
        case 1:
          $str .= $this->reporter->col($data[$i]['items'], '75', null, false, $border, '', 'C', $font, $fontsize, '', '', '2px');
          $str .= $this->reporter->col($data[$i]['barcode'], '75', null, false, $border, '', 'C', $font, $fontsize, '', '', '2px');
          $str .= $this->reporter->col(number_format($data[$i]['qty'], $this->companysetup->getdecimal('qty', $params['params'])), '75', null, false, $border, '', 'C', $font, $fontsize, '', '', '2px');
          $str .= $this->reporter->col($data[$i]['uom'], '75', null, false, $border, '', 'C', $font, $fontsize, '', '', '2px', 0, '', 0, 1);
    ...
    switch ($params['params']['dataparams']['reporttype']) {
      case 1:
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col('', '75', null, false, $border, 'T', '', $font, $fontsize, '', '', '2px');
        $str .= $this->reporter->col('', '75', null, false, $border, 'T', '', $font, $fontsize, '', '', '2px');
        $str .= $this->reporter->col('', '75', null, false, $border, 'T', '', $font, $fontsize, '', '', '2px');
        $str .= $this->reporter->col('', '75', null, false, $border, 'T', '', $font, $fontsize, '', '', '2px', 0, '', 0, 1);
        $str .= $this->reporter->col('', '400', null, false, $border, 'T', '', $font, $fontsize, '', '', '2px');
        $str .= $this->reporter->col('', '100', null, false, $border, 'T', '', $font, $fontsize, '', '', '2px');
        $str .= $this->reporter->endrow();
        break;
      default:
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col('', '50', null, false, $border, 'T', 'C', $font, $fontsize, '', '', '2px');
        $str .= $this->reporter->col('', '50', null, false, $border, 'T', 'C', $font, $fontsize, '', '', '2px');
        $str .= $this->reporter->col('', '50', null, false, $border, 'T', 'C', $font, $fontsize, '', '', '2px');
        $str .= $this->reporter->col('', '375', null, false, $border, 'T', 'L', $font, $fontsize, '', '', '2px');
        $str .= $this->reporter->col('GRAND TOTAL :', '175', null, false, $border, 'T', 'R', $font, $fontsize, '', '', '2px', 0, '', 0, 2);
        $str .= $this->reporter->col(number_format($totalext, $decimal), '100', null, false, $border, 'T', 'R', $font, $fontsize, '', '', '2px');
        $str .= $this->reporter->endrow();
        break;
    }
    ...
    $str .= $this->reporter->col('Issued By : ', '200', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '');
    $str .= $this->reporter->col('Checked By : ', '200', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '');
    $str .= $this->reporter->col('Received By : ', '200', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();
    ...
    $str .= $this->reporter->begintable('800');
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col($params['params']['dataparams']['approved'], '200', null, false, $border, '', 'L', $font, $fontsize, '', '', '');
    $str .= $this->reporter->col($params['params']['dataparams']['issued'], '200', null, false, $border, '', 'L', $font, $fontsize, '', '', '');
    $str .= $this->reporter->col($params['params']['dataparams']['checked'], '200', null, false, $border, '', 'L', $font, $fontsize, '', '', '');
    $str .= $this->reporter->col($params['params']['dataparams']['received'], '200', null, false, $border, '', 'L', $font, $fontsize, '', '', '');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();
  ...
  public function ST_header_PDF($params, $data)
  {
    ...
    PDF::MultiCell(50, 0, "Trnx Type: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(100, 0, (isset($data[0]['trnxtype']) ? $data[0]['trnxtype'] : ''), 'B', 'L', false, 1, '',  '');
  ...
  public function default_ST_PDF($params, $data)
  {
    ...
    PDF::MultiCell(175, 0, 'Issued By: ', '', 'L', false, 0);
    ...
    PDF::MultiCell(175, 0, $params['params']['dataparams']['issued'], '', 'L', false, 0);
  ...
  public function ST_header_withid_PDF($params, $data)
  {
    $center = $params['params']['center'];
    $username = $params['params']['user'];
    ...
    // MultiCell($w, $h, $txt, $border=0, $align='J', $fill=0, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0)
    PDF::SetFont($fontbold, '', 18);
    PDF::MultiCell(520, 0, $this->modulename . ' with ID', '', 'L', false, 0, '',  '100');
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(80, 0, "Docno #: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', 10);
    PDF::MultiCell(100, 0, (isset($data[0]['docno']) ? $data[0]['docno'] : ''), 'B', 'L', false, 0, '',  '');
    ...
    PDF::MultiCell(140, 0, "ITEM ID", '', 'C', false, 0);
    PDF::MultiCell(110, 0, "BARCODE", '', 'C', false, 0);
    PDF::MultiCell(50, 0, "QTY", '', 'C', false, 0);
    PDF::MultiCell(60, 0, "UNIT", '', 'C', false, 0);
    PDF::MultiCell(250, 0, "DESCRIPTION", '', 'L', false, 0);
    PDF::MultiCell(90, 0, "PRICE", '', 'R', false);

    PDF::SetFont($font, '', 5);
    PDF::MultiCell(700, 0, '', 'B');
  }

  public function default_ST_withid_PDF($params, $data)
  {
    $companyid = $params['params']['companyid'];
    $decimalcurr = $this->companysetup->getdecimal('currency', $params['params']);
    $decimalqty = $this->companysetup->getdecimal('qty', $params['params']);
    ...
    $this->ST_header_withid_PDF($params, $data);
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $arr_itemid = $this->reporter->fixcolumn([$itemid], '30', 0);
        $arr_barcode = $this->reporter->fixcolumn([$barcode], '14', 0);
        $arr_qty = $this->reporter->fixcolumn([$qty], '13', 0);
        $arr_uom = $this->reporter->fixcolumn([$uom], '13', 0);
        $arr_itemname = $this->reporter->fixcolumn([$itemname], '41', 0);
        ...
        $maxrow = $this->othersClass->getmaxcolumn([$arr_itemid, $arr_barcode, $arr_itemname, $arr_qty, $arr_uom, $arr_netamt]);

        for ($r = 0; $r < $maxrow; $r++) {
          PDF::SetFont($font, '', $fontsize);
          PDF::MultiCell(140, 15, ' ' . (isset($arr_itemid[$r]) ? $arr_itemid[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(110, 15, ' ' . (isset($arr_barcode[$r]) ? $arr_barcode[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(50, 15, ' ' . (isset($arr_qty[$r]) ? $arr_qty[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(60, 15, ' ' . (isset($arr_uom[$r]) ? $arr_uom[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(250, 15, ' ' . (isset($arr_itemname[$r]) ? $arr_itemname[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(90, 15, ' ' . (isset($arr_netamt[$r]) ? $arr_netamt[$r] : ''), '', 'R', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
        }
        if (PDF::getY() > 900) {
          $this->ST_header_PDF($params, $data);
        }

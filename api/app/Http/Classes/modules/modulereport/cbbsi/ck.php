...
class ck
{
    ...
    private $modulename = "Request For Sales Return";
    ...
    {
        $query = "select x.*
        from (select  head.vattype, head.tax, client.tel, stock.rem as remarks, m.model_name as model, item.sizeid, concat(coa.acno, '~', coa.acnoname) as aracc, date(head.dateid) as dateid, head.docno, client.client, client.clientname, concat(client.client, '~', client.clientname) as customer, head.address, head.terms, head.rem, head.yourref, head.ourref, item.barcode, item.brand, item.itemname, stock.isqty as qty, stock.uom, stock.isamt as amt, stock.disc, stock.ext, stock.ref, ag.clientname as agname, ag.client as agcode, wh.client as whcode, wh.clientname as whname, stock.line
        ...
        where head.doc = 'CK' and md5(head.trno)='" . md5($trno) . "'
        union all
        select head.vattype, head.tax, client.tel, stock.rem as remarks, m.model_name as model, item.sizeid, concat(coa.acno, '~', coa.acnoname) as aracc, date(head.dateid) as dateid, head.docno, client.client, client.clientname, concat(client.client, '~', client.clientname) as customer, head.address, head.terms, head.rem, head.yourref, head.ourref, item.barcode, item.brand, item.itemname, stock.isqty as qty, stock.uom, stock.isamt as amt, stock.disc, stock.ext, stock.ref, ag.clientname as agname, ag.client as agcode, wh.client as whcode, wh.clientname as whname, stock.line
        from ckhead as head
        left join ckstock as stock on stock.trno = head.trno
        left join client on client.client = head.client
        left join item on item.itemid = stock.itemid
        left join model_masterfile as m on m.model_id = item.model
        left join client as ag on ag.client = head.agent
    ...
    public function reportplotting($params, $data)
    {
        if ($params['params']['dataparams']['print'] == "default") {
            return $this->default_CK_layout($params, $data);
        } else if ($params['params']['dataparams']['print'] == "PDFM") {
            return $this->default_CK_PDF($params, $data);
        }
    }
    ...
    private function report_default_header($params, $data)
    {
        ...
        $str .= $this->reporter->col('DESCRIPTION', '500', null, false, $border, 'B', 'L', $font, '12', 'B', '30px', '8px');
    ...
    public function default_CK_layout($params, $data)
    {
        $companyid = $params['params']['companyid'];
        $decimal = $this->companysetup->getdecimal('currency', $params['params']);
    ...
    public function default_CK_header_PDF($params, $data)
    {
        $center = $params['params']['center'];
        $username = $params['params']['user'];
        ...
        PDF::MultiCell(520, 20, $this->modulename, '', 'L', false, 0);
    ...
    public function default_CK_PDF($params, $data)
    {
        $companyid = $params['params']['companyid'];
        ...
        $this->default_CK_header_PDF($params, $data);
        PDF::SetFont($font, '', 5);
        PDF::MultiCell(700, 0, '', '');
        ...
        if (!empty($data)) {
            for ($i = 0; $i < count($data); $i++) {
                ...
                for ($r = 0; $r < $maxrow; $r++) {
                    ...
                    PDF::MultiCell(230, 15, ' ' . (isset($arr_itemname[$r]) ? $arr_itemname[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);

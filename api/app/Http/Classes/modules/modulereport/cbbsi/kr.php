...
class kr
{
  ...
  public function report_default_query($filters)
  {
    ...
    head2.yourref as krourref,detail.rem as notes,
    coa.acno, coa.acnoname, ar.db, ar.cr, date(ar.dateid) as postdate,head.rem, ar.docno as ref,client.tel,head2.due,datediff(head.dateid,head2.due) as age,right(ag.client,4) as agent,client.tel,head2.rem as rem2
    from (krhead as head 
  ...
  public function default_KR_header_PDF($params, $data)
  {
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(50, 0, (isset($data[0]['tel']) ? $data[0]['tel'] : ''), '', 'L', false, 1, '',  '');
    ...
    PDF::MultiCell(100, 0, "DOC NO", '', 'L', false, 0);
    ...
    PDF::MultiCell(99, 0, "CLIENT", '', 'C', false, 0);
    PDF::MultiCell(51, 0, "NOTES", '', 'C', false, 0);
    PDF::MultiCell(50, 0, "AGENT", '', 'R', false, 0);
    PDF::MultiCell(20, 0, "", '', 'R', false, 0);
    PDF::MultiCell(30, 0, "AGE", '', 'R', false);
  ...
  public function default_KR_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      ...
      for ($i = 0; $i < count($data); $i++) {
        ...
        $arr_ref = $this->reporter->fixcolumn([$ref], '30', 0);
        $arr_rem2 = $this->reporter->fixcolumn([$rem2], '20', 0);
        ...
        $arr_age = $this->reporter->fixcolumn([$age], '10', 0);
        $arr_notes = $this->reporter->fixcolumn([$notes], '15', 0);
        $maxrow = $this->othersClass->getmaxcolumn([$arr_ref, $arr_rem2, $arr_yourref, $arr_due, $arr_postdate, $arr_debit, $arr_credit, $arr_client, $arr_agent, $arr_age, $arr_notes]);

        for ($r = 0; $r < $maxrow; $r++) {

          PDF::SetFont($font, '', $fontsize);
          PDF::MultiCell(100, 15, ' ' . (isset($arr_ref[$r]) ? $arr_ref[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(100, 15, ' ' . (isset($arr_rem2[$r]) ? $arr_rem2[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(65, 15, ' ' . (isset($arr_due[$r]) ? $arr_due[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(65, 15, ' ' . (isset($arr_postdate[$r]) ? $arr_postdate[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          ...
          PDF::MultiCell(99, 15, ' ' . (isset($arr_client[$r]) ? $arr_client[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(51, 15, ' ' . (isset($arr_notes[$r]) ? $arr_notes[$r] : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(50, 15, ' ' . (isset($arr_agent[$r]) ? $arr_agent[$r] : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(20, 15, ' ' . '', '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(30, 15, ' ' . (isset($arr_age[$r]) ? $arr_age[$r] : ''), '', 'R', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
        }
        $totaldb += $data[$i]['db'];
        $totalcr += $data[$i]['cr'];
    ...
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(330, 0, 'GRAND TOTAL: ', '', 'R', false, 0);

...
class pv
{
  ...
  public function PDF_default_header_ccvoucher($params, $data)
  {
    ...
    PDF::MultiCell(150, 25, 'NET OF AP', 'TBLR', 'R', false);
  }
  ...
  public function PDF_DEFAULT_CCVOUCHER_LAYOUT1($data, $params)
  {
    ...
    $netofap = abs($totalcr - $totaldb);

    $netofap = $totalcr - $totaldb;
    ...
    PDF::MultiCell(150, 25, number_format($netofap, 2), 'TBLR', 'R', false);


    PDF::MultiCell(300, 25, 'ACCOUNTING ENTRY', 'TBL', 'C', false, 0);
    PDF::MultiCell(200, 25, '', 'B', 'C', false, 0);
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      PDF::MultiCell(300, 25, $acname, 'TBL', 'L', false, 0);
      PDF::MultiCell(200, 25, $data[$i]['projectname'], 'B', 'L', false, 0);
      PDF::MultiCell(100, 25, $debit, 'TBL', 'R', false, 0);
      PDF::MultiCell(100, 25, $credit, 'TBLR', 'R', false);
    }
  ...
  public function report_default_query($filters)
  {
    ...
    switch ($filters['params']['dataparams']['reporttype']) {
      ...
      default:
        $query = "select date(head.dateid) as dateid, head.docno, client.client, head.clientname, head.address, head.terms,
          head.rem as hrem,head.rem, head.yourref, head.ourref,
          coa.acno, coa.acnoname, detail.ref, detail.postdate, detail.db, detail.cr,
          detail.client as dclient, detail.checkno, head.project,ifnull(project.name,'') as projectname, left (coa.alias,2) as alias
          from lahead as head 
          left join ladetail as detail on detail.trno=head.trno 
          left join client on client.client=head.client
          left join coa on coa.acnoid=detail.acnoid
          ...
          union all
          select date(head.dateid) as dateid, head.docno, client.client, head.clientname, head.address, head.terms,
          head.rem as hrem,head.rem, head.yourref, head.ourref,
          coa.acno, coa.acnoname, detail.ref, detail.postdate, detail.db, detail.cr,
          dclient.client as dclient, detail.checkno, head.project,ifnull(project.name,'') as projectname, left (coa.alias,2) as alias
          from glhead as head 
          left join gldetail as detail on detail.trno=head.trno 
          left join client on client.clientid=head.clientid

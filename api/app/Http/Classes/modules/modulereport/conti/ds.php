...
class ds
{
  ...
  public function default_ds_layout($config, $result)
  {
    ...
    $str .= $this->reporter->col('', '75', null, false, '1px dotted ', 'T', 'C', 'Century Gothic', '12', 'B', '', '2px');
    $str .= $this->reporter->col('', '75', null, false, '1px dotted ', 'T', 'C', 'Century Gothic', '12', 'B', '', '2px');
    $str .= $this->reporter->col('GRAND TOTAL :', '280', null, false, '1px dotted ', 'T', 'R', 'Century Gothic', '12', 'B', '', '2px');
    $str .= $this->reporter->col(number_format($totaldb, 2), '100', null, false, '1px dotted ', 'T', 'R', 'Century Gothic', '12', 'B', '', '2px');
    $str .= $this->reporter->col(number_format($totalcr, 2), '100', null, false, '1px dotted ', 'T', 'R', 'Century Gothic', '12', 'B', '', '2px');
    $str .= $this->reporter->col('', '20', null, false, '1px dotted ', 'T', 'C', 'Century Gothic', '12', 'B', '', '2px');
    $str .= $this->reporter->col('', '75', null, false, '1px dotted ', 'T', 'C', 'Century Gothic', '12', 'B', '', '2px');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();
  ...
  public function default_DS_header_PDF($params, $data)
  {
    ...
    foreach ($data as $key => $datas) {
      if (substr($datas->alias, 0, -1) == 'CB') {
        ...
        if ($ii > 1) {
          ...
          PDF::MultiCell(400, 0, $datas->acnoname, '', 'L', 0, 1, '', '');
        } else {
          PDF::MultiCell(400, 0, $datas->acnoname, '', 'L', 0, 1, '', '');
        }
      }
    ...
    PDF::MultiCell(70, 0, "ACCT. NO.", '', 'L', false, 0);
    PDF::MultiCell(200, 0, "ACCOUNT NAME", '', 'C', false, 0);
    PDF::MultiCell(105, 0, "REFERENCE #", '', 'C', false, 0);
  ...
  public function default_DS_PDF($params, $data)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      for ($r = 0; $r < $maxrow; $r++) {
        ...
        PDF::MultiCell(105, 15, ' ' . (isset($arr_ref[$r]) ? $arr_ref[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);

...
class cm
{
  ...
  private function report_default_header($params, $data)
  {
    ...
    $str .= $this->reporter->col('SALES RETURN', '560', null, false, $border, '', 'L', $font, '18', 'B', '', '');
    $str .= $this->reporter->col('DOCUMENT # :', '120', null, false, $border, '', 'L', $font, '13', 'B', '', '');
    $str .= $this->reporter->col((isset($data[0]['docno']) ? $data[0]['docno'] : ''), '120', null, false, $border, 'B', 'L', $font, '13', '', '', '') . '<br />';
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();
    $str .= $this->reporter->begintable('800');
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('CUSTOMER : ', '100', null, false, $border, '', 'L', $font, '12', 'B', '30px', '4px');
    $str .= $this->reporter->col((isset($data[0]['clientname']) ? $data[0]['clientname'] : ''), '460', null, false, $border, 'B', 'L', $font, '12', '', '30px', '4px');
    $str .= $this->reporter->col('DATE : ', '80', null, false, $border, '', 'L', $font, '12', 'B', '', '');
    $str .= $this->reporter->col((isset($data[0]['dateid']) ? $data[0]['dateid'] : ''), '160', null, false, $border, 'B', 'L', $font, '12', '', '', '');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();
    $str .= $this->reporter->begintable('800');
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('ADDRESS : ', '100', null, false, $border, '', 'L', $font, '12', 'B', '30px', '4px');
    $str .= $this->reporter->col((isset($data[0]['address']) ? $data[0]['address'] : ''), '460', null, false, $border, 'B', 'L', $font, '12', '', '30px', '4px');
    $str .= $this->reporter->col('TERMS : ', '80', null, false, $border, '', 'L', $font, '12', 'B', '', '');
    $str .= $this->reporter->col((isset($data[0]['terms']) ? $data[0]['terms'] : ''), '160', null, false, $border, 'B', 'L', $font, '12', '', '', '');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();
    ...
    $str .= $this->reporter->startrow();
    //($txt='',$w=null,$h=null, $bg=false,$b=false,$b_='', $al='', $f='', $fs='',$fw='',$fc='',$pad='',$m='')
    $str .= $this->reporter->col('QTY', '75', null, false, $border, 'B', 'C', $font, '12', 'B', '', '2px');
    $str .= $this->reporter->col('UNIT', '70', null, false, $border, 'B', 'C', $font, '12', 'B', '', '2px');
    $str .= $this->reporter->col('D E S C R I P T I O N', '380', null, false, $border, 'B', 'L', $font, '12', 'B', '', '2px');
    $str .= $this->reporter->col('UNIT PRICE', '100', null, false, $border, 'B', 'R', $font, '12', 'B', '', '2px');
    $str .= $this->reporter->col('DISC', '50', null, false, $border, 'B', 'R', $font, '12', 'B', '', '2px');
    $str .= $this->reporter->col('TOTAL', '125', null, false, $border, 'B', 'R', $font, '12', 'B', '', '2px');
    return $str;
  }
  ...
  public function default_cm_layout($params, $data)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $str .= $this->reporter->col($data[$i]['uom'], '70', null, false, $border, '', 'C', $font, $fontsize, '', '', '2px');
      $str .= $this->reporter->col($data[$i]['itemname'], '380', null, false, $border, '', 'L', $font, $fontsize, '', '', '2px');
      $str .= $this->reporter->col(number_format($data[$i]['amt'], $this->companysetup->getdecimal('price', $params['params'])), '100', null, false, $border, '', 'R', $font, $fontsize, '', '', '2px');
    ...
    $str .= $this->reporter->col('', '50', null, false, $border, 'T', 'C', $font, '12', 'B', '', '2px');
    $str .= $this->reporter->col('', '50', null, false, $border, 'T', 'C', $font, '12', 'B', '', '2px');
    $str .= $this->reporter->col('', '400', null, false, $border, 'T', 'L', $font, '12', 'B', '', '2px');
    $str .= $this->reporter->col(' ', '125', null, false, $border, 'T', 'R', $font, '12', 'B', '', '2px');
    $str .= $this->reporter->col('GRAND TOTAL :', '150', null, false, $border, 'T', 'L', $font, '12', 'B', '', '2px');
    $str .= $this->reporter->col(number_format($totalext, $decimal), '125', null, false, $border, 'T', 'R', $font, '12', 'B', '', '2px');
    $str .= $this->reporter->endrow();
  ...
  public function default_cm_header_PDF($params, $data)
  {
    ...
    PDF::MultiCell(520, 0, 'SALES RETURN', '', 'L', false, 0, '',  '100');
    ...
    PDF::MultiCell(50, 0, "QTY", '', 'L', false, 0);
    PDF::MultiCell(50, 0, "UNIT", '', 'L', false, 0);
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(50, 15, ' ' . (isset($arr_qty[$r]) ? $arr_qty[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);

...
class ts
{
  ...
  public function default_TS_header_PDF($params, $data)
  {
    ...
    PDF::MultiCell(350, 0, "DESCRIPTION", '', 'L', false, 0);
    PDF::MultiCell(200, 0, "FROM WAREHOUSE", '', 'L', false);
  ...
  public function default_TS_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $arr_swh = $this->reporter->fixcolumn([$swh], '15', 0);

        $maxrow = $this->othersClass->getmaxcolumn([$arr_itemname, $arr_qty, $arr_uom, $arr_swh]);
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(350, 15, ' ' . (isset($arr_itemname[$r]) ? $arr_itemname[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(200, 15, ' ' . (isset($arr_swh[$r]) ? $arr_swh[$r] : ''), '', 'L', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
        }
    ...
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(80, 0, number_format($totalqty, $decimalcurr), '', 'C');

...
class po
{
  ...
  public function createreportfilter($config)
  {
    ...
    $fields = ['radioprint', 'prepared', 'approved', 'received', 'audited'];
  ...
  public function report_default_query($trno)
  {
    $query = "select date(head.dateid) as dateid, head.docno, client.client, client.clientname, head.address,
        ...
        stock.ext,m.model_name as model,item.sizeid,if(ifnull(sit.itemdesc,'')='',item.itemname,sit.itemdesc) as itemdesc, head.vattype, head.tax,head.trno
  ...
  public function default_PO_header_PDF($params, $data)
  {
    ...
    $draftqry = "select case when postdate is null then true else false end as isdraft from transnum where trno =  '" . $data[0]['trno'] . "'";
    $checking = $this->coreFunctions->opentable($draftqry);

    if ($checking[0]->isdraft == 1) {
      PDF::SetFont($fontbold, '', 23);
      PDF::MultiCell(0, 0, 'DRAFT', '', 'C');
    } else {
      PDF::MultiCell(0, 0, "\n\n\n" . ' ', '', 'C');
    }
    ...
    PDF::MultiCell(275, 0, "DESCRIPTION", '', 'L', false, 0);
  ...
  public function default_PO_PDF($params, $data)
  {
    ...
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(600, 0, 'Net of VAT : ', '', 'R', false, 0);
    PDF::MultiCell(100, 0, number_format($net, $decimalcurr), '', 'R');

    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(600, 0, 'TAX : ', '', 'R', false, 0);
    PDF::MultiCell(100, 0, number_format($tax, $decimalcurr), '', 'R');
    ...
    PDF::MultiCell(175, 0, 'Audited By: ', '', 'L');
    ...
    PDF::MultiCell(175, 0, $params['params']['dataparams']['audited'], '', 'L');

...
class sj
{
  ...
  public function report_default_query($config)
  {
    ...
    right(year(head.dateid),2) as year,left(head.dateid,10) as dateid, head.docno,concat(left(client.client,2),right(client.client,6)) as client, client.clientname,
    ...
    wh.client as whcode, wh.clientname as whname,client.bstyle,head.tax,head.vattype,stock.loc,stock.expiry
    from lahead as head
  ...
  public function default_SI_PDF($params, $data)
  {
    ...
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(200, 0, (isset($data[0]['docno']) ? $data[0]['docno'] : ''), '', 'L', false, 1, '620',  '85', true, 0, false, true, 0, 'B', true);

    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(400, 0, (isset($data[0]['clientname']) ? $data[0]['client'] . ' ' . $data[0]['clientname'] : ''), '', 'L', false, 1, '70',  '105', true, 0, false, true, 0, 'B', true);

    $date = date_create(isset($data[0]['dateid']) ? $data[0]['dateid'] : '');
    $date = date_format($date, "F j, Y");

    PDF::MultiCell(400, 0, $date, '', 'L', false, 1, '620',  '105', true, 0, false, true, 0, 'B', true);

    PDF::MultiCell(400, 0, (isset($data[0]['address']) ? $data[0]['address'] : ''), '', 'L', false, 1, '70',  '120', true, 0, false, true, 0, 'B', true);
    PDF::MultiCell(400, 0, (isset($data[0]['terms']) ? $data[0]['terms'] : ''), '', 'L', false, 1, '620',  '120', true, 0, false, true, 0, 'B', true);

    PDF::MultiCell(400, 0, (isset($data[0]['tin']) ? $data[0]['tin'] : ''), '', 'L', false, 1, '70',  '135', true, 0, false, true, 0, 'B', true);
    PDF::MultiCell(400, 0, (isset($data[0]['yourref']) ? $data[0]['yourref'] : ''), '', 'L', false, 1, '620',  '135', true, 0, false, true, 0, 'B', true);

    PDF::MultiCell(400, 0, (isset($data[0]['bstyle']) ? $data[0]['bstyle'] : ''), '', 'L', false, 1, '100',  '150', true, 0, false, true, 0, 'B', true);
    PDF::MultiCell(400, 0, (isset($data[0]['agent']) ? $data[0]['agent'] : ''), '', 'L', false, 1, '620',  '150', true, 0, false, true, 0, 'B', true);
    ...
    $countarr = 0;
    PDF::MultiCell(720, 0, '', '', 'L', false, 1, '600',  '180', true, 0, false, true, 0, 'B', true);

    $displayBarcode = '';
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        if ($displayBarcode == '' || $displayBarcode != $data[$i]['barcode']) {
          $maxrow = 1;
          ...
          $qtyqry = "
          select sum(a.isqty) as totalqty,sum(a.ext) as totalext from (
            select stock.trno,stock.itemid,stock.isqty,stock.iss,stock.ext,i.barcode from lastock as stock
            left join item as i on i.itemid=stock.itemid
            union all
            select stock.trno,stock.itemid,stock.isqty,stock.iss,stock.ext,i.barcode from glstock as stock
            left join item as i on i.itemid=stock.itemid
          ) as a
          where a.barcode='" . $barcode . "' and a.trno='" . $trno . "'";

          $qtyresult = json_decode(json_encode($this->coreFunctions->opentable($qtyqry)), true);
          $qty = number_format($qtyresult[0]['totalqty']);
          $totalExtDisplay = number_format($qtyresult[0]['totalext'], 2);
          ...
          $ext = number_format($data[$i]['ext'], 2);
          $arr_barcode = $this->reporter->fixcolumn([$barcode], '15', 0);
          $arr_itemname = $this->reporter->fixcolumn([$itemname], '85', 0); #35 default
          $arr_qty = $this->reporter->fixcolumn([$qty], '13', 0);
          $arr_uom = $this->reporter->fixcolumn([$uom], '13', 0);
          $arr_amt = $this->reporter->fixcolumn([$amt], '13', 0);
          $arr_disc = $this->reporter->fixcolumn([$disc], '13', 0);
          $arr_ext = $this->reporter->fixcolumn([$totalExtDisplay], '15', 0);
          ...
          for ($r = 0; $r < $maxrow; $r++) {
            ...
            PDF::MultiCell(75, 15, ' ' . (isset($arr_qty[$r]) ? $arr_qty[$r] : ''), '', 'C', false, 0, '20',  '', true, 0, false, true, 0, 'M', false);
            PDF::MultiCell(75, 15, ' ' . (isset($arr_uom[$r]) ? $arr_uom[$r] : ''), '', 'C', false, 0, '95',  '', true, 0, false, true, 0, 'M', false);
        ...
        $displayBarcode = $data[$i]['barcode'];

        $loc = $data[$i]['loc'];
        $expiry = $data[$i]['expiry'];
        $qty = number_format($data[$i]['qty']);

        $arr2_loc = $this->reporter->fixcolumn([$loc], '35', 0);
        $arr2_expiry = $this->reporter->fixcolumn([$expiry], '13', 0);
        $arr2_qty = $this->reporter->fixcolumn([$qty], '13', 0);


        $maxrow = $this->othersClass->getmaxcolumn([$arr2_loc, $arr2_expiry, $arr2_qty]);
        for ($r = 0; $r < $maxrow; $r++) {

          PDF::SetFont($font, '', $fontsize);
          $qty = isset($arr2_qty[$r]) ? $arr2_qty[$r] : 0;
          if ($qty !=  0) {

            PDF::MultiCell(75, 15, ' ', '', 'C', false, 0, '95',  '', true, 0, false, true, 0, 'M', false);
            PDF::MultiCell(100, 15, ' ' . (isset($arr2_loc[$r]) ? $arr2_loc[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
            PDF::MultiCell(85, 15, ' ' . (isset($arr2_expiry[$r]) ? $arr2_expiry[$r] : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
            PDF::MultiCell(85, 15, ' ' . $arr2_qty[$r] ==  0 ? '' : $arr2_qty[$r], '', 'R', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
          } else {
            // PDF::MultiCell(75, 15, ' ', '', 'C', false, 0, '95',  '', true, 0, false, true, 0, 'M', false);
            // PDF::MultiCell(100, 15, ' ', '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
            // PDF::MultiCell(85, 15, ' ', '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
            // PDF::MultiCell(85, 15, ' ', '', 'R', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
          }
        }
    ...
    switch ($data[0]['vattype']) {
      case 'VATABLE':
        $amountnetofvat = $totalext / $vat2;
        $lessvat = ($totalext / $vat2) * $vat;
        $amountdue = $amountnetofvat;
        $addvat = ($totalext / $vat2) * $vat;

        $totalamountdue = $totalext;

        break;
      case 'NON-VATABLE':
        $totalamountdue = $totalext;
        $vatexemptsales = $totalext;
        break;

      default: //ZERO RATED
        $zeroratedsales = $totalext;
        $totalamountdue = $totalext;

        break;
    }

    PDF::MultiCell(150, 0, ' ' . number_format($amountnetofvat, 2), '', 'R', false, 1, '380',  '830', true, 0, false, true, 0, 'M', false);
    PDF::MultiCell(150, 0, ' ' . number_format($vatexemptsales, 2), '', 'R', false, 1, '380',  '845', true, 0, false, true, 0, 'M', false);
    PDF::MultiCell(150, 0, ' ' . number_format($zeroratedsales, 2), '', 'R', false, 1, '380',  '860', true, 0, false, true, 0, 'M', false);
    PDF::MultiCell(150, 0, ' ' . number_format($lessvat, 2), '', 'R', false, 1, '380',  '875', true, 0, false, true, 0, 'M', false);
    PDF::MultiCell(150, 0, ' ' . number_format($totalamountdue, 2), '', 'R', false, 1, '380',  '890', true, 0, false, true, 0, 'M', false);

    PDF::MultiCell(150, 0, ' ' . number_format($lessvat, 2), '', 'R', false, 1, '600',  '830', true, 0, false, true, 0, 'M', false);
    PDF::MultiCell(150, 0, ' ' . number_format($amountnetofvat, 2), '', 'R', false, 1, '600',  '845', true, 0, false, true, 0, 'M', false);
    PDF::MultiCell(150, 0, ' ' . number_format($lessscpwddisc, 2), '', 'R', false, 1, '600',  '860', true, 0, false, true, 0, 'M', false);
    PDF::MultiCell(150, 0, ' ' . number_format($amountdue, 2), '', 'R', false, 1, '600',  '875', true, 0, false, true, 0, 'M', false);
    PDF::MultiCell(150, 0, ' ' . number_format($addvat, 2), '', 'R', false, 1, '600',  '890', true, 0, false, true, 0, 'M', false);
    PDF::MultiCell(150, 0, ' ' . number_format($totalamountdue, 2), '', 'R', false, 1, '600',  '905', true, 0, false, true, 0, 'M', false);

    PDF::MultiCell(150, 0, $params['params']['dataparams']['prepared'], '', 'C', false, 1, '20',  '910', true, 0, false, true, 0, 'M', false);
    PDF::MultiCell(150, 0, $params['params']['dataparams']['approved'], '', 'C', false, 1, '150',  '910', true, 0, false, true, 0, 'M', false);

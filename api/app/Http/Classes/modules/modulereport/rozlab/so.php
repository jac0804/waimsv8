...
class so
{
  ...
  public function report_default_query($trno)
  {
    $query = "select head.rtype,head.rdate,cust.tel2,cust.email,head.docno,head.trno, head.clientname, head.address, 
      ...
      item.sizeid,m.model_name as model,date(head.due) as due,head.yourref, stockinfo.itemdesc,stock.loc,stock.expiry
  ...
  public function default_so_header_PDF($params, $data)
  {
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(220, 0, (isset($data[0]['agent']) ? $data[0]['agent'] : ''), 'B', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    ...
    PDF::MultiCell(135, 0, "LOT NO", '', 'C', false, 0);
    PDF::MultiCell(75, 0, "EXPIRY", '', 'C', false, 0);
  ...
  public function default_so_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $arr_loc = $this->reporter->fixcolumn([$loc], '25', 0);
        ...
        $maxrow = $this->othersClass->getmaxcolumn([$arr_itemname, $arr_qty, $arr_uom, $arr_loc, $arr_expiry, $arr_gross, $arr_disc, $arr_ext]);
        for ($r = 0; $r < $maxrow; $r++) {

          PDF::SetFont($font, '', $fontsize);
          PDF::MultiCell(60, 15, ' ' . (isset($arr_qty[$r]) ? $arr_qty[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(55, 15, ' ' . (isset($arr_uom[$r]) ? $arr_uom[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(200, 15, ' ' . (isset($arr_itemname[$r]) ? $arr_itemname[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(135, 15, ' ' . (isset($arr_loc[$r]) ? $arr_loc[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(75, 15, ' ' . (isset($arr_expiry[$r]) ? $arr_expiry[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(75, 15, ' ' . (isset($arr_gross[$r]) ? $arr_gross[$r] : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(50, 15, ' ' . (isset($arr_disc[$r]) ? $arr_disc[$r] : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(50, 15, ' ' . (isset($arr_ext[$r]) ? $arr_ext[$r] : ''), '', 'R', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
        }

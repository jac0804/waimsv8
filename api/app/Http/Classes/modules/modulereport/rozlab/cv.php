...
class cv
{
  ...
  public function createreportfilter($config)
  {
    ...
    $fields = ['radioprint', 'radioreporttype', 'prepared', 'received', 'checked', 'payor', 'tin', 'position', 'audited', 'print'];
  ...
  public function PDF_default_header($params, $data)
  {
    ...
    $addrborder = 'T';
    if (strlen($data[0]['address']) > 80) {
      $addrborder = 'B';
    }
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(70, 5, '', '', 'L', false, 0);
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(425, 5, '', $addrborder, 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(425, 5, (isset($data[0]['rem']) ? $data[0]['rem'] : ''), 'B', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(85, 5, 'OURREF :', '', 'R', false, 0);
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(120, 5, (isset($data[0]['ourref']) ? $data[0]['ourref'] : ''), 'B', 'R', false);

    PDF::SetFont($fontbold, '', 14);
    PDF::MultiCell(70, 5, '', '', 'L', false, 0);
    PDF::MultiCell(630, 5, '', '', 'L', false);
  ...
  public function PDF_DEFAULT_CCVOUCHER_LAYOUT1($data, $params)
  {
    ...
    if (!empty($data)) {
      ...
      for ($i = 0; $i < count($data); $i++) {
        ...
        $maxrow = $this->othersClass->getmaxcolumn([$arr_acno, $arr_acnonamedescs, $arr_checkno, $arr_dyourref, $arr_pdate, $arr_debit, $arr_credit, $arr_drem]);
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(110, 15, ' ' . (isset($arr_dyourref[$r]) ? $arr_dyourref[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
    ...
    PDF::MultiCell(280, 0, 'Prepared By: ', '', 'L', false, 0);
    PDF::MultiCell(260, 0, 'Audited By: ', '', 'L', false, 0);
    PDF::MultiCell(180, 0, 'Received By: ', '', 'L');


    PDF::MultiCell(0, 0, "\n");

    PDF::MultiCell(280, 0, $params['params']['dataparams']['prepared'], '', 'L', false, 0);
    PDF::MultiCell(260, 0, $params['params']['dataparams']['audited'], '', 'L', false, 0);
  ...
  public function PDF_DEFAULT_CCVOUCHER_LAYOUT2($data, $params)
  {
    ...
    for ($i = 0; $i < count($data2); $i++) {
      ...
      PDF::MultiCell(420, 5, ('' . isset($date) ? $date : ''), '', 'C', false, 0);

      PDF::MultiCell(120, 5, '', '', 'C', false);

      PDF::MultiCell(20, 5, '', '', 'C', false, 0);
      PDF::MultiCell(250, 5, $data[0]['clientname'], '', 'L', false, 1, '160', '53');
      PDF::MultiCell(420, 5, (isset($cc) ? number_format($cc, $decimal) : ''), '', 'C', false, 1, '310', '53');
      ...
      PDF::MultiCell(320, 5, $this->reporter->ftNumberToWordsConverter($dd) . ' ONLY', '', 'L', false, 1, '160', '65');

...
class pc
{
  ...
  public function default_query($filters)
  {
    ...
    (select sum(rr.bal) as bal from rrstatus as rr where rr.itemid = stock.itemid and rr.whid = client.clientid) as bal
    from pchead as head 
  ...
  public function default_PC_header_PDF($params, $data)
  {
    ...
    PDF::MultiCell(60, 0, "BALANCE", '', 'C', false, 0);
    PDF::MultiCell(70, 0, "VARIANCE", '', 'C', false, 0);
  ...
  public function default_PC_PDF($params, $data)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $var = number_format($data[$i]['qty']-$data[$i]['bal'],2);
      ...
      $arr_bal = $this->reporter->fixcolumn([$bal], '15', 0);
      $arr_var = $this->reporter->fixcolumn([$var], '15', 0);
      ...
      for ($r = 0; $r < $maxrow; $r++) {
        ...
        PDF::MultiCell(60, 15, ' ' . (isset($arr_bal[$r]) ? $arr_bal[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
        PDF::MultiCell(70, 15, ' ' . (isset($arr_var[$r]) ? $arr_var[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
        ...
        if ($viewcost == '1') {
          ...
          PDF::MultiCell(60, 15, '', '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
  ...
  public function fams_PC_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $arr_itemname = $this->reporter->fixcolumn([$data[$i]['itemname']], '35', 0);
        $arr_consignee = $this->reporter->fixcolumn([$data[$i]['consignee']], '25', 0);
        $arr_srem = $this->reporter->fixcolumn([$data[$i]['srem']], '25', 0);

        $maxrow = $this->othersClass->getmaxcolumn([$arr_barcode, $arr_itemame, $arr_consignee, $arr_srem]);
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(100, 0, (isset($arr_barcode[$r]) ? $arr_barcode[$r] : ''), '', 'L', $setfill, 0, '', '', true, 1);
          PDF::MultiCell(200, 0, (isset($arr_itemname[$r]) ? $arr_itemname[$r] : ''), '', 'L', $setfill, 0, '', '', false, 1);
          PDF::MultiCell(200, 0, (isset($arr_consignee[$r]) ? $arr_consignee[$r] : ''), '', 'L', $setfill, 0, '', '', false, 1);
          PDF::MultiCell(200, 0, (isset($arr_srem[$r]) ? $arr_srem[$r] : ''), '', 'L', $setfill, 1, '', '', false, 1);
        }
    ...
    if (!empty($result2)) {
      for ($i = 0; $i < count($result2); $i++) {
        ...
        $maxrow = 1;
        $arr_barcode = $this->reporter->fixcolumn([$result2[$i]['barcode']], '16', 0);
        $arr_itemname = $this->reporter->fixcolumn([$result2[$i]['itemname']], '35', 0);
        $arr_consignee = $this->reporter->fixcolumn([$result2[$i]['consignee']], '25', 0);
        $arr_dateid = $this->reporter->fixcolumn([$result2[$i]['dateid']], '16', 0);

        $maxrow = $this->othersClass->getmaxcolumn([$arr_barcode, $arr_itemname, $arr_consignee, $arr_dateid]);
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(200, 0, (isset($arr_itemname[$r]) ? $arr_itemname[$r] : ''), '', 'L', $setfill, 0, '', '', false, 1);
          PDF::MultiCell(200, 0, (isset($arr_consignee[$r]) ? $arr_consignee[$r] : ''), '', 'L', $setfill, 0, '', '', false, 1);
          PDF::MultiCell(200, 0, (isset($arr_dateid[$r]) ? $arr_dateid[$r] : ''), '', 'L', $setfill, 1, '', '', false, 1);
        }

...
class cv
{
  ...
  public function report_default_query($filters)
  {
    ...
    switch ($filters['params']['dataparams']['reporttype']) {
      case 0:
        ...
        head.hacno,head.hacnoname,subproj.subproject,proj.name as projname,head.projectid,headproj.code as headprojcode,headproj.name as headprojname
  ...
  public function PDF_maxipro_voucher_header($params, $data)
  {
    ...
    PDF::SetFont($fontbold, '', 11);
    PDF::MultiCell(600, 20, ':  ' . (isset($data[0]['rem']) ? $data[0]['rem'] : ''), 'B', 'L', false, 0);
    ...
    PDF::MultiCell(60, 20, 'ACCT #', 'B', 'C', false, 0);
    PDF::MultiCell(160, 20, 'ACCOUNT NAME', 'B', 'C', false, 0);
    PDF::MultiCell(130, 20, 'PROJECT', 'B', 'C', false, 0);
    PDF::MultiCell(150, 20, 'SUBPROJECT', 'B', 'C', false, 0);
    PDF::MultiCell(100, 20, 'DEBIT', 'B', 'C', false, 0);
    PDF::MultiCell(100, 20, 'CREDIT', 'B', 'C', false);
  }
  ...
  public function PDF_maxipro_voucher_layout($data, $params)
  {
    ...
    if (!empty($data)) {
      ...
      for ($i = 0; $i < count($data); $i++) {
        $acnonamedescs = $this->reporter->fixcolumn([$data[$i]['acnoname']], 28, 0);
        $countarr = count($acnonamedescs);

        $project = $this->reporter->fixcolumn([$data[$i]['projcode']], 20, 0);
        $countarrp = count($project);

        $subproject = $this->reporter->fixcolumn([$data[$i]['subproject']], 21, 0);
        $countarrsp = count($subproject);

        $maxrow = max($countarr, $countarrp, $countarrsp);
        ...
        if ($data[$i]['acnoname'] == '') {
          ...
          for ($r = 0; $r < $maxrow; $r++) {
            ...
            PDF::MultiCell(60, 15, $acno, '', 'C', false, 0);
            PDF::MultiCell(160, 15, isset($acnonamedescs[$r]) ? $acnonamedescs[$r] : '', '', 'L', false, 0);
            PDF::MultiCell(130, 15, isset($project[$r]) ? $project[$r] : '', '', 'L', false, 0);
            PDF::MultiCell(150, 15, isset($subproject[$r]) ? $subproject[$r] : '', '', 'L', false, 0);
            PDF::MultiCell(100, 15, $debit, '', 'R', false, 0);
            PDF::MultiCell(100, 15, $credit, '', 'R', false);
          }
        }
        $totaldb += $data[$i]['db'];
        $totalcr += $data[$i]['cr'];
    ...
    PDF::SetFont($fontbold, '', 12);
    PDF::MultiCell(90, 25, '', '', 'C', false, 0);
    PDF::MultiCell(160, 25, '', '', 'L', false, 0);
    PDF::MultiCell(120, 25, '', '', 'L', false, 0);
    PDF::MultiCell(120, 25, 'GRAND TOTAL', '', 'L', false, 0);
    PDF::MultiCell(105, 25, number_format($totaldb, $decimal), '', 'R', false, 0);
    PDF::MultiCell(105, 25, number_format($totalcr, $decimal), '', 'R', false);
    ...
    PDF::MultiCell(253, 50, 'Prepared By: ', '', 'L', false);

    PDF::SetFont($font, '', 10);
    PDF::MultiCell(23, 0, '', '', 'L', false, 0);
    PDF::MultiCell(220, 0, $params['params']['dataparams']['prepared'], 'B', 'C', false);

    PDF::MultiCell(0, 0, "\n\n");

    PDF::SetFont($fontbold, '', 10);
    PDF::MultiCell(253, 50, 'Checked By: ', '', 'L', false);

    PDF::SetFont($font, '', 10);
    PDF::MultiCell(23, 0, '', '', 'L', false, 0);
    PDF::MultiCell(220, 0, $params['params']['dataparams']['checked'], 'B', 'C', false);

    PDF::MultiCell(0, 0, "\n\n");

    PDF::SetFont($fontbold, '', 10);
    PDF::MultiCell(253, 50, 'Approved By: ', '', 'L', false);

    PDF::SetFont($font, '', 10);
    PDF::MultiCell(23, 0, '', '', 'L', false, 0);
    PDF::MultiCell(220, 0, $params['params']['dataparams']['approved'], 'B', 'C', false);

    PDF::MultiCell(0, 0, "\n\n");

    PDF::SetFont($fontbold, '', 10);
    PDF::MultiCell(380, 50, '', '', 'L', false, 0);
    PDF::MultiCell(253, 50, 'Received By: ', '', 'L', false);
    ...
    PDF::MultiCell(80, 0, '', 'B', 'C', false);

    PDF::SetFont($font, '', 9);
    PDF::MultiCell(380, 0, '', '', 'L', false, 0);
    PDF::MultiCell(220, 0, 'Signature over Printed Name', '', 'C', false, 0);
    PDF::MultiCell(10, 0, '', '', 'C', false, 0);
    PDF::MultiCell(80, 0, 'Date', '', 'C', false);
  ...
  public function PDF_DEFAULT_CCVOUCHER_LAYOUT2($data, $params)
  {
    ...
    for ($i = 0; $i < count($data2); $i++) {
      ...
      PDF::SetFont($font, '', 12);
      PDF::MultiCell(570, 0, date('m', strtotime($data2[$i]['postdate'])), '', 'L', false, 0, '520', '40');
      PDF::setFontSpacing(11);
      PDF::MultiCell(50, 0, date('d', strtotime($data2[$i]['postdate'])), '', 'L', false, 0, '565', '40');
      PDF::setFontSpacing(12);
      PDF::MultiCell(100, 0, date('Y', strtotime($data2[$i]['postdate'])), '', 'L', false, 0, '605', '40');
      ...
      PDF::MultiCell(420, 5, '***' . $params['params']['dataparams']['payor'] . '***', '', 'L', false, 0, 115, 72);
      PDF::MultiCell(20, 5, '', '', 'L', false, 0);
      PDF::MultiCell(235, 5, (isset($cc) ? number_format($cc, $decimal) : ''), '', 'L', false, 0, 530, 72);
      ...
      PDF::MultiCell(730, 5, '***' . $this->ftNumberToWordsConverter($dd) . ' Only' . '***', '', 'L', false, 1, 90, 102);
  ...
  public function PDF_METROBANK_CHECK_LAYOUT($data, $filters)
  {
    ...
    for ($i = 0; $i < count($data2); $i++) {
      ...
      $dd = number_format((float)$cc, 2, '.', '');
      PDF::MultiCell(720, 0, '***' . $this->ftNumberToWordsConverter($dd) . ' ONLY***', '', 'L', false);                  
    }
    return PDF::Output($this->modulename . '.pdf', 'S');
  } //end fn

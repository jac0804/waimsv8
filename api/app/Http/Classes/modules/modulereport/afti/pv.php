...
class pv
{
  ...
  public function report_default_query($filters)
  {
    ...
    switch ($filters['params']['dataparams']['reporttype']) {
      ...
      case 3:
        ...
        client.zipcode,b.addrline1 as billaddress, b.zipcode as billzipcode
        from lahead as head
        ...
        where head.doc='PV' and head.trno =" . $trno . " and " . $filter . "
        union all
        select detail.line,month(head.dateid) as month,right(year(head.dateid),2) as yr, head.docno, client.client, client.clientname,
        head.address,detail.rem, head.yourref, head.ourref,client.tin,
        coa.acno, coa.acnoname, detail.ref, detail.postdate,
      ...
      default:
        $query = "select date(head.dateid) as dateid, head.docno, client.client, client.clientname, head.address, client.terms,
          head.rem as hrem,head.rem, head.yourref as custpo, head.ourref,
          coa.acno, coa.acnoname, detail.ref, detail.postdate, detail.db, detail.cr,
          detail.client as dclient, detail.checkno, head.project,client.tin as supptin,concat(b.addrline1,' ',b.addrline2,' ',b.city,' ',b.province,' ',b.country,' ',b.zipcode) as billaddress,head.trno,head.invoiceno,date(head.invoicedate) as invoicedate,coa.cat
          from lahead as head 
          left join ladetail as detail on detail.trno=head.trno 
          left join client on client.client=head.client
          left join billingaddr as b on b.line=client.billid
          ...
          union all
          select date(head.dateid) as dateid, head.docno, client.client, client.clientname, head.address, client.terms,
          head.rem as hrem,head.rem, head.yourref as custpo, head.ourref,
          coa.acno, coa.acnoname, detail.ref, detail.postdate, detail.db, detail.cr,
          dclient.client as dclient, detail.checkno, head.project,client.tin as supptin,concat(b.addrline1,' ',b.addrline2,' ',b.city,' ',b.province,' ',b.country,' ',b.zipcode) as billaddress,head.trno,head.invoiceno,date(head.invoicedate) as invoicedate,coa.cat
          from glhead as head 
          left join gldetail as detail on detail.trno=head.trno 
          left join client on client.clientid=head.clientid
          left join billingaddr as b on b.line=client.billid
  ...
  public function PDF_default_header_ccvoucher($params, $data)
  {
    ...
    $qry = "select name,address,tin from center where code = '" . $center . "'";
    ...
    PDF::MultiCell(55, $padsupp, "SUPPLIER: ", '', 'L', false, 0);
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(300, $padsupp, (isset($data[0]['clientname']) ? $data[0]['clientname'] : ''), 'B', 'L', false, 0);
    PDF::SetFont($font, 'B', $fontsize9);
    PDF::MultiCell(75, $padsupp, 'APV#: ', '', 'R', false, 0);
    PDF::SetFont($font, '', $fontsize9);
    ...
    PDF::SetFont($font, 'B', $fontsize9);
    PDF::MultiCell(55, 0, "TIN: ", '', 'L', false, 0);
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(105, 0, (isset($data[0]['supptin']) ? $data[0]['supptin'] : ''), 'B', 'L', false, 0);
    PDF::SetFont($font, 'B', $fontsize9);
    PDF::MultiCell(75, 0, 'CUSTOMER PO: ', '', 'L', false, 0);
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(130, 0, (isset($data[0]['custpo']) ? $data[0]['custpo'] : ''), 'B', 'L', false, 0);
    PDF::SetFont($font, 'B', $fontsize9);
    PDF::MultiCell(65, 0, 'DATE: ', '', 'R', false, 0);
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(100, 0, (isset($data[0]['dateid']) ? $data[0]['dateid'] : ''), 'B', 'L', false);

    PDF::SetFont($font, 'B', $fontsize9);
    PDF::MultiCell(55, $padbill, "ADDRESS: ", '', 'L', false, 0);
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(310, $padbill, (isset($data[0]['billaddress']) ? $data[0]['billaddress'] : ''), 'B', 'L', false, 0);
    PDF::SetFont($font, 'B', $fontsize9 - 1);
    PDF::MultiCell(65, $padbill, 'Supp Inv. #: ', '', 'R', false, 0);
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(100, 0, (isset($data[0]['invoiceno']) ? $data[0]['invoiceno'] : ''), 'B', 'L', false);
    if (isset($data[0]['billaddress'])) {
      $ncount = round(strlen($bill) / 70, 0);
      PDF::SetFont($font, '', 1);
      for ($i = 1; $i < $ncount; $i++) {
    ...
    PDF::SetFont($font, 'B', $fontsize9 - 1);
    PDF::MultiCell(55, 0, "Terms: ", '', 'L', false, 0);
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(310, 0, (isset($data[0]['terms']) ? $data[0]['terms'] : ''), 'B', 'L', false, 0);

    PDF::SetFont($font, 'B', $fontsize9 - 1);
    PDF::MultiCell(65, 0, 'Supp Inv. Date: ', '', 'R', false, 0);
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(100, 0, (isset($data[0]['invoicedate']) ? $data[0]['invoicedate'] : ''), 'B', 'L', false);
  ...
  public function RR_Details($config, $trno)
  {
    ...
    PDF::MultiCell(90, 0, "ACCOUNT NAME", '', 'C', false, 0);
    ...
    ifnull(b.clientname,'') as branch,ifnull(d.clientname,'') as dept,c.cat
    from glhead as h
    ...
    left join projectmasterfile as p on p.line=g.projectid
    left join client as b on b.clientid=g.branch
    left join client as d on d.clientid=g.deptid
    where g.trno=$trno
    union all
    select c.acno,c.acnoname,g.db as debit,g.cr as credit,ifnull(p.name,'') as itemgroup,
    ifnull(b.clientname,'') as branch,ifnull(d.clientname,'') as dept,c.cat
    from lahead as h
    left join ladetail as g on g.trno=h.trno
    left join lastock as s on s.trno=g.trno and s.line=g.line
    left join coa as c on c.acnoid=g.acnoid
    left join projectmasterfile as p on p.line=g.projectid
    left join client as b on b.clientid=g.branch
    left join client as d on d.clientid=g.deptid
    ...
    if (!empty($data2)) {
      ...
      for ($i = 0; $i < count($data2); $i++) {
        ...
        $arr_acnoname = $this->reporter->fixcolumn([$acnoname], '18', 0);
        ...
        $arr_dept = $this->reporter->fixcolumn([$dept], '10', 0);
        $arr_branch = $this->reporter->fixcolumn([$branch], '10', 0);
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(90, 0, ' ' . (isset($arr_acnoname[$r]) ? $arr_acnoname[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(75, 0, ' ' . (isset($arr_db[$r]) ? $arr_db[$r] : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(75, 0, ' ' . (isset($arr_cr[$r]) ? $arr_cr[$r] : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          ...
          switch ($data2[$i]['cat']) {
            ...
            default:
              PDF::MultiCell(75, 0, '', '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
              PDF::MultiCell(75, 0, '', '', 'L', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
              break;
          }
        }
  ...
  public function PDF_DEFAULT_CCVOUCHER_LAYOUT1($data, $params)
  {
      ...
      for ($x = 0; $x < count($particulars); $x++) {
        if ($particulars[$x]['rem'] != '') {
          ...
          for ($r = 0; $r < $maxrow; $r++) {
            ...
            if ($particulars[$x]['amount'] != 0) {
              PDF::MultiCell(180, 0, $partiamt, '', 'R', false, 350);
            } else {
              PDF::MultiCell(180, 0, '', '', 'R', false, 350);
            }
          }
        }
    ...
    PDF::MultiCell(0, 0, "\n\n\n");
    PDF::MultiCell(530, 0, '', 'T', 'C', false);
    $this->RR_Details($params, $data[0]['trno']);
  ...
  public function PDF_CV_WTAXREPORT($data, $params)
  {
    ...
    PDF::MultiCell(610, 30, (isset($data['head'][0]['payee']) ? $data['head'][0]['payee'] : ''), 'TBLR', 'C', false, 0, 165, 302);
    ...
    PDF::MultiCell(410, 30, (isset($data['head'][0]['billaddress']) ? $data['head'][0]['billaddress'] : ''), 'TBLR', 'C', false, 0, 165, 348);
    PDF::SetFont($fontbold, '', 11);
    ...
    PDF::MultiCell(410, 30, (isset($data['head'][0]['address']) ? $data['head'][0]['address'] : ''), 'TBLR', 'C', false, 0, 165, 385);
    PDF::SetFont($fontbold, '', 11);
    ...
    PDF::MultiCell(610, 30, (isset($data['head'][0]['payee']) ? $data['head'][0]['payee'] : ''), 'TBLR', 'C', false, 0, 165, 495);
    PDF::MultiCell(15, 30, '', 'R', 'C', false);
    ...
    PDF::MultiCell(410, 30, (isset($data['head'][0]['address']) ? $data['head'][0]['address'] : ''), 'TBLR', 'C', false, 0, 165, 540);
    PDF::SetFont($fontbold, '', 11);
    ...
    PDF::MultiCell(245, 20, ucwords($params['params']['dataparams']['payor']), 'B', 'R', false, 0);
    PDF::MultiCell(15, 20, '', '', 'R', false, 0);
    PDF::MultiCell(245, 20, $params['params']['dataparams']['tin'], 'B', 'R', false, 0);
    PDF::MultiCell(15, 20, '', '', 'R', false, 0);
    PDF::MultiCell(250, 20, ucwords($params['params']['dataparams']['position']), 'B', 'R', false, 0);
    ...
    PDF::MultiCell(780, 10, 'Conforme', 'LTR', 'L', false, 1, 10, 1078);

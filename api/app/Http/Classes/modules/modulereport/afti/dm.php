    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        if ($data[$i]['itemname'] == '') {
          ...
          PDF::MultiCell(100, 0, number_format($data[$i]['amt'], $decimalcurr), '', 'R', false, 0, '', '', false, 1);
          PDF::MultiCell(100, 0, $data[$i]['disc'], '', 'R', false, 0, '', '', false, 1);
          PDF::MultiCell(100, 0, number_format($data[$i]['ext'], $decimalcurr), '', 'R', false, 1, '', '', false, 1);
        } else {
          for ($r = 0; $r < $maxrow; $r++) {
            if ($r == 0) {

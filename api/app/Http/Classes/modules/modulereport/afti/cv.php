...
class cv
{
  ...
  {
    $fields = ['radioprint', 'radioreporttype', 'prepared', 'approved', 'received', 'checked', 'empcode', 'empname', 'tin', 'position', 'print'];
  ...
  public function reportplotting($config, $data)
  {
    if ($config['params']['dataparams']['print'] == "default") {
      ...
      switch ($config['params']['dataparams']['reporttype']) {
        case '0': // VOUCHER
          $str = $this->CCVOUCHER_PDF($data, $config);
          break;
        case '1':
  ...
  public function reportparamsdata($config)
  {
    ...
    $user = $this->coreFunctions->opentable("
      select cl.clientname as prepared, 'Marilou Aranas' as approved, emp.clientname as checked
      from client as cl
    ...
    $checked = !empty($user) ? $user[0]->checked : '';
  ...
  public function report_default_query($filters)
  {
    ...
    switch ($filters['params']['dataparams']['reporttype']) {
      case 0:
        $query = "select detail.rem as drem,
        group_concat(DISTINCT concat(left(detail.ref,3),right(detail.ref,5)) separator '/') as ref,head.trno, head.docno, 
        date(head.dateid) as dateid, 
        date(cntnum.postdate) as postdate,client.client, head.clientname, head.address,
        client.tin, head.terms, head.yourref, head.ourref, head.rem, coa.acno,
        coa.acnoname,round(sum(detail.db),2) as db,round(sum(detail.cr),2) as cr, 
        detail.checkno, left(coa.alias,2) as alias,p.name as costcenter,dept.clientname as department,ifnull(branch.clientname,'') as branch,gclient.client as gclient, detail.refx,coa.cat
        ...
        left join client as gclient on gclient.client=detail.client
        where head.doc='cv' and head.trno ='$trno'
        group by detail.rem, head.trno, head.docno, date(head.dateid), date(cntnum.postdate), client.client, head.clientname, head.address,
        client.tin, head.terms, head.yourref, head.ourref, head.rem, coa.acno,
        coa.acnoname, detail.checkno, left(coa.alias,2), p.name, dept.clientname, branch.clientname, gclient.client, detail.refx,coa.cat
        union all
        select detail.rem as drem,
        group_concat(DISTINCT concat(left(detail.ref,3),right(detail.ref,5)) separator '/') as ref,head.trno, head.docno, 
        date(head.dateid) as dateid, 
        date(cntnum.postdate) as postdate,client.client, head.clientname, head.address,
        client.tin, head.terms, head.yourref, head.ourref, head.rem, coa.acno,
        ...
        left join client as dept on dept.clientid=detail.deptid
        left join client as branch on branch.clientid=detail.branch
        left join client as gclient on gclient.clientid=detail.clientid
        where head.doc='cv' and head.trno ='$trno'
        group by detail.rem, head.trno, head.docno, date(head.dateid), date(cntnum.postdate), client.client, head.clientname, head.address,
        client.tin, head.terms, head.yourref, head.ourref, head.rem, coa.acno,
        coa.acnoname, detail.checkno, left(coa.alias,2), p.name, dept.clientname, branch.clientname, gclient.client, detail.refx,coa.cat
      ...
      case 2:
        ...
        $query = "select * from(
          ...
          b.addrline1 as billaddress, b.zipcode as billzipcode
          from lahead as head
          ...
          where head.doc='cv' and head.trno =" . $trno . " and " . $filter . "
          union all
          select detail.line, month(head.dateid) as month,right(year(head.dateid),2) as yr, head.docno, client.client, client.clientname,
          head.address,detail.rem, head.yourref, head.ourref,client.tin,
          coa.acno, coa.acnoname, detail.ref, detail.postdate,
  ...
  public function CCVOUCHER_Header_PDF($params, $data)
  {
    ...
    PDF::MultiCell(530, 0, 'CHECK VOUCHER', '', 'C');
    PDF::MultiCell(0, 0, "\n");

    $client = $data[0]['clientname'];
    $clientlen = strlen($client);
    if ($clientlen > 50) {
      $clientlen = $clientlen / 35;
      $padding = 10 * $clientlen;
      $n = "\n\n\n";
      $fontspace = $fontsize9;
    } else {
      $fontspace = 5;
    ...
    PDF::SetFont($font, 'B', $fontsize9 + 2);
    PDF::MultiCell(70, $padding, "Pay To: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(275, $padding, $data[0]['clientname'], 'B', 'L', false, 0, '',  '');
    PDF::SetFont($font, 'B', $fontsize9 + 2);
    PDF::MultiCell(85, 0, "CV#: ", '', 'R', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(100, 0, (isset($data[0]['docno']) ? $data[0]['docno'] : ''), 'B', 'L', false);

    PDF::SetFont($fontbold, '', $fontspace);
    PDF::MultiCell(0, 0, $n);

    PDF::SetFont($fontbold, '', $fontsize9);
    PDF::MultiCell(70, 15, "Customer PO: ", '', 'L', false, 0, '',  '');
    PDF::MultiCell(275, 15, '', 'B', 'L', false, 0, '',  '');
    PDF::MultiCell(85, 15, "Date: ", '', 'R', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(100, 15, (isset($data[0]['dateid']) ? $data[0]['dateid'] : ''), 'B', 'L', false);

    PDF::SetFont($fontbold, '', $fontsize9);
    PDF::MultiCell(70, 15, "Supp Inv. #: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(275, 15,  $this->getinvoiceno($data[0]['refx']), 'B', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize9);
    PDF::MultiCell(85, 15, "Supp Inv. Date: ", '', 'R', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(100, 15, $this->getinvoicedate($data[0]['refx']), 'B', 'L', false);

    PDF::MultiCell(0, 0, "\n");

    $trno = $data[0]['trno'];
    $checkqry = "select d.postdate as chequedate,c.acnoname,d.checkno,d.cr from ladetail as d
    left join coa as c on c.acnoid=d.acnoid
    where d.trno=$trno and left(c.alias,2) in ('CB','CA','PC')
    union all
    select d.postdate as chequedate,c.acnoname,d.checkno,d.cr from gldetail as d
    left join coa as c on c.acnoid=d.acnoid
    where d.trno=$trno and left(c.alias,2) in ('CB','CA','PC')";

    $cheque = json_decode(json_encode($this->coreFunctions->opentable($checkqry)), true);

    PDF::SetFont($fontbold, '', $fontsize9);

    PDF::MultiCell(250, 15, "Bank", 'TBL', 'C', false, 0);
    PDF::MultiCell(120, 15, "Cheque No.", 'TBL', 'C', false, 0);
    PDF::MultiCell(160, 15, "Payment Amount", 'TBLR', 'C', false);

    PDF::SetFont($font, '', $fontsize9);

    PDF::MultiCell(250, 15, isset($cheque[0]['acnoname']) ? $cheque[0]['acnoname'] : '', 'TBL', 'C', false, 0);
    PDF::MultiCell(120, 15, isset($cheque[0]['checkno']) ? $cheque[0]['checkno'] : '', 'TBL', 'C', false, 0);
    PDF::MultiCell(160, 15, number_format(isset($cheque[0]['cr']) ? $cheque[0]['cr'] : 0, 2), 'TBLR', 'R', false);
  ...
  public function CCVOUCHER_Details($config, $data)
  {
    $fontsize = "11";
    $font = "";
    $fontbold = "";
    ...
    PDF::MultiCell(70, 0, "ITEM GROUP", '', 'L', false, 0);
    PDF::MultiCell(45, 0, "DEPT.", '', 'L', false, 0);
    PDF::MultiCell(45, 0, "BRANCH", '', 'L', false, 0);
    PDF::MultiCell(45, 0, "REF", '', 'L', false, 0);
    PDF::MultiCell(45, 0, "INV#", '', 'L', false);

    PDF::MultiCell(530, 0, '', 'B');
  }

  public function CCVOUCHER_PDF($data, $params)
  {
    ...
    $this->CCVOUCHER_Header_PDF($params, $data);
    ...
    if (!empty($particulars)) {
      ...
      for ($x = 0; $x < count($particulars); $x++) {
        if ($particulars[$x]['rem'] != '') {
          ...
          $arrrem = $this->reporter->fixcolumn([$particulars[$x]['rem']], 60, 0);
          $crem = count($arrrem);
          ...
          for ($r = 0; $r < $maxrow; $r++) {
            ...
            if ($r == 0) {
              $partiamt = number_format($particulars[$x]['amount'], 2);
            } else {
              $partiamt = '';
            }
            PDF::MultiCell(180, 0, ($partiamt == '0.00' ? '' : $partiamt), '', 'R', false, 350);
          }
        }
      ...
      if ($remarks == $data[0]['rem']) {
        ...
        $remarks = $data[0]['rem'];
        $arrrem = $this->reporter->fixcolumn([$remarks], 100, 0);

        for ($r = 0; $r < count($arrrem); $r++) {
          PDF::SetFont($font, '', $fontsize9);
          PDF::MultiCell(500, 0, isset($arrrem[$r]) ? ' ' . $arrrem[$r] : '', '', 'L', false);
        }
      }
    }
    ...
    $this->CCVOUCHER_Details($params, $data);
    ...
    if (!empty($data)) {
      ...
      foreach ($data as $key => $value) {
        array_push($data2, [
          ...
          'ref' => $value['ref'],
          'alias' => $value['alias'],
          'invoiceno' => $this->getinvoiceno($value['refx']),
          'debit' => $value['db'],
          'credit' => $value['cr'],
          'drem' => $value['drem'],
          'cat' => $value['cat']
      ...
      for ($i = 0; $i < count($data2); $i++) {
        ...
        $acnonamedescs = $this->reporter->fixcolumn([$data2[$i]['acnoname']], 25, 0);
        $countarr = count($acnonamedescs);

        $arrref = $this->reporter->fixcolumn([$data2[$i]['ref']], 9, 0);
        $countref = count($arrref);

        $centerdescs = $this->reporter->fixcolumn([$data2[$i]['costcenter']], 10, 0);
        $countc = count($centerdescs);

        $deptdescs = $this->reporter->fixcolumn([$data2[$i]['department']], 8, 0);
        $countd = count($deptdescs);

        $branchdescs = $this->reporter->fixcolumn([$data2[$i]['branch']], 8, 0);
        $countb = count($branchdescs);

        $invdescs = $this->reporter->fixcolumn([$data2[$i]['invoiceno']], 8, 0);
        $countinv = count($invdescs);
        ...
        if ($data2[$i]['acnoname'] == '') {
          ...
          for ($r = 0; $r < $maxrow; $r++) {
            if ($r == 0) {
              $acno =  $data2[$i]['acno'];
              $checkno = $data2[$i]['checkno'];
              $center = $data2[$i]['costcenter'];
              $dept = $data2[$i]['department'];
              $branch = $data2[$i]['branch'];
              $ref = $data2[$i]['ref'];
              $alias = $data2[$i]['alias'];
              $invoiceno = $data2[$i]['invoiceno'];
              $debit = number_format($data2[$i]['debit'], 2);
              if ($debit < 1) {
                $debit = '-';
              }

              $credit = number_format($data2[$i]['credit'], 2);
              if ($credit < 1) {
                $credit = '-';
              }
              $drem = $data2[$i]['drem'];
            ...
            PDF::MultiCell(160, 0, isset($acnonamedescs[$r]) ? $acnonamedescs[$r] : '', '', 'L', false, 0, '', '', true, 1, false, true, 0, 'B', true);
            PDF::MultiCell(60, 0, $debit, '', 'R', false, 0, '', '', true, 1, false, true, 0, 'B', true);
            PDF::MultiCell(60, 0, $credit, '', 'R', false, 0, '', '', true, 1, false, true, 0, 'B', true);
            PDF::MultiCell(70, 0, '  ' . (isset($centerdescs[$r]) ? $centerdescs[$r] : ''), '', 'L', false, 0, '', '', true, 1, false, true, 0, 'B', true);

            switch ($cat) {
              case 'R':
              case 'C':
              case 'E':
                PDF::MultiCell(45, 0, isset($deptdescs[$r]) ? $deptdescs[$r] : '', '', 'L', false, 0, '', '', true, 1, false, true, 0, 'B', true);
                PDF::MultiCell(45, 0, isset($branchdescs[$r]) ? $branchdescs[$r] : '', '', 'L', false, 0, '', '', true, 1, false, true, 0, 'B', true);
                break;
            ...
            PDF::MultiCell(45, 0, isset($arrref[$r]) ? $arrref[$r] : '', '', 'C', false, 0, '', '', true, 1, false, true, 0, 'B', true);
            PDF::MultiCell(45, 0, isset($invdescs[$r]) ? $invdescs[$r] : '', '', 'C', false, 1, '', '', true, 1, false, true, 0, 'B', true);
          }
        }
        $totaldb += $data2[$i]['debit'];
        $totalcr += $data2[$i]['credit'];
      }
    }

    PDF::MultiCell(160, 0, '', 'T', 'L', false, 0);
    PDF::MultiCell(60, 0, number_format($totaldb, 2), 'T', 'R', false, 0);
    PDF::MultiCell(60, 0, number_format($totalcr, 2), 'T', 'R', false, 0);
    PDF::MultiCell(75, 0, '', 'T', 'R', false, 0);
    PDF::MultiCell(50, 0, '', 'T', 'R', false, 0);
    PDF::MultiCell(50, 0, '', 'T', 'R', false, 0);
    PDF::MultiCell(50, 0, '', 'T', 'R', false, 0);
    PDF::MultiCell(50, 0, '', 'T', 'R', false);
    ...
    PDF::MultiCell(235, 0, 'Prepared By: ', '', 'L', false, 0);
    PDF::MultiCell(60, 0, '', '', 'L', false, 0);
    PDF::MultiCell(235, 0, 'Approved By: ', '', 'L', false);

    PDF::MultiCell(0, 0, "\n\n\n");

    PDF::MultiCell(235, 0, $params['params']['dataparams']['prepared'], 'T', 'C', false, 0);
    PDF::MultiCell(60, 0, '', '', 'L', false, 0);
    PDF::MultiCell(235, 0, $params['params']['dataparams']['approved'], 'T', 'C', false);

    PDF::SetFont($fontbold, '', $fontsize9);
    PDF::MultiCell(0, 0, "\n\n\n");

    PDF::MultiCell(235, 0, 'Checked By: ', '', 'L', false, 0);
    PDF::MultiCell(60, 0, '', '', 'L', false, 0);
    PDF::MultiCell(235, 0, 'Received By: ', '', 'L');

    PDF::MultiCell(0, 0, "\n\n\n");

    PDF::MultiCell(235, 0, $params['params']['dataparams']['checked'], 'T', 'C', false, 0);
    PDF::MultiCell(60, 0, '', '', 'L', false, 0);
    PDF::SetFont($fontbold, '', $fontsize9 - 2);
    PDF::MultiCell(235, 0, 'Signature over Printed Name/Date', 'T', 'C');

    PDF::SetFont($fontbold, '', $fontsize9);
    PDF::MultiCell(280, 0, ' ', '', 'L', false, 0);
    PDF::MultiCell(60, 0, 'OR No.', '', 'R', false, 0);
    PDF::MultiCell(190, 0, '', 'B', 'L', false);

    PDF::MultiCell(280, 0, ' ', '', 'L', false, 0);
    PDF::MultiCell(60, 0, 'OR Date.', '', 'R', false, 0);
    PDF::MultiCell(190, 0, '', 'B', 'L', false);
  ...
  public function PDF_NEW_default_header($params, $data)
  {
    $center = $params['params']['center'];
    $username = $params['params']['user'];
    ...
    PDF::MultiCell(70, 0, "CV#: ", '', 'R', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(100, 0, (isset($data[0]['docno']) ? $data[0]['docno'] : ''), 'B', 'L', false);

    PDF::SetFont($fontbold, '', $fontsize9);
    PDF::MultiCell(40, 0, "Pay To: ", '', 'L', false, 0, '',  '');
    ...
    $checkqry = "select d.postdate as chequedate,c.acnoname,d.checkno,d.cr from ladetail as d
    left join coa as c on c.acnoid=d.acnoid
    where d.trno=$trno and left(c.alias,2)='CB'";

    $cheque = json_decode(json_encode($this->coreFunctions->opentable($checkqry)), true);

    PDF::SetFont($fontbold, '', $fontsize9);
    PDF::MultiCell(70, 15, "Cheque Date", 'TBL', 'C', false, 0);
    PDF::MultiCell(220, 15, "Bank", 'TBL', 'C', false, 0);
    PDF::MultiCell(100, 15, "Cheque No.", 'TBL', 'C', false, 0);
    PDF::MultiCell(140, 15, "Payment Amount", 'TBLR', 'C', false);

    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(70, 15, $cheque[0]['chequedate'], 'TBL', 'C', false, 0);
    PDF::MultiCell(220, 15, $cheque[0]['acnoname'], 'TBL', 'C', false, 0);
    PDF::MultiCell(100, 15, $cheque[0]['checkno'], 'TBL', 'C', false, 0);
    PDF::MultiCell(140, 15, number_format($cheque[0]['cr'], 2), 'TBLR', 'R', false);

    PDF::MultiCell(0, 0, "\n");

    PDF::SetFont($fontbold, '', $fontsize9 + 1);

    PDF::MultiCell(530, 450, "", 'TBLR', 'L', false, 1, '40',  '240');

    PDF::MultiCell(70, 450, '', 'TBLR', 'L', false, 0, '40', '240');
    ...
    PDF::MultiCell(70, 15, "COST CENTER", 'TBL', 'C', false, 0, '40', '240');
  ...
  public function PDF_DEFAULT_CCVOUCHER_NEW_LAYOUT1($data, $params)
  {
    $companyid = $params['params']['companyid'];
    $decimalcurr = $this->companysetup->getdecimal('currency', $params['params']);
    $decimalqty = $this->companysetup->getdecimal('qty', $params['params']);
    ...
    $this->PDF_NEW_default_header($params, $data);
    ...
    PDF::SetFont($fontbold, '', $fontsize9);
    PDF::MultiCell(70, 20, '', 'TLB', 'C', false, 0, '40', '690');
    ...
    PDF::SetFont($fontbold, '', $fontsize9 + 2);
    PDF::MultiCell(15, 0, ' ', 'TL', 'L', false, 0);
    PDF::MultiCell(100, 0, 'Prepared By: ', 'T', 'L', false, 0);
    PDF::MultiCell(15, 0, ' ', 'TL', 'L', false, 0);
    PDF::MultiCell(100, 0, 'Approved By: ', '', 'L', false, 0);
    PDF::MultiCell(15, 0, ' ', 'L', 'L', false, 0);
    ...
    PDF::MultiCell(180, 0, 'Signature over Printed Name/Date', '', 'C', false, 0);
    PDF::MultiCell(25, 0, '', 'R', 'L', false);
    ...
    PDF::MultiCell(15, 0, ' ', 'L', 'L', false, 0);
    PDF::MultiCell(80, 0, '', '', 'L', false, 0);
    PDF::MultiCell(50, 0, 'OR No.', '', 'R', false, 0);
    PDF::MultiCell(130, 0, '', 'B', 'C', false, 0);
    PDF::MultiCell(25, 0, '', 'R', 'L', false);

    PDF::SetFont($fontbold, '', $fontsize9);
    PDF::MultiCell(15, 0, ' ', 'L', 'L', false, 0);
    PDF::MultiCell(80, 0, $params['params']['dataparams']['prepared'], 'T', 'C', false, 0);
    PDF::MultiCell(20, 0, ' ', '', 'L', false, 0);
    PDF::MultiCell(15, 0, ' ', 'L', 'L', false, 0);
    PDF::MultiCell(80, 0, $params['params']['dataparams']['approved'], 'T', 'C', false, 0);
    PDF::MultiCell(20, 0, ' ', '', 'L', false, 0);
    PDF::MultiCell(15, 0, ' ', 'L', 'L', false, 0);
    PDF::MultiCell(80, 0, '', '', 'L', false, 0);
    PDF::MultiCell(50, 0, 'OR Date.', '', 'R', false, 0);
    PDF::MultiCell(130, 0, '', 'B', 'C', false, 0);
    PDF::MultiCell(25, 0, '', 'R', 'L', false);

    PDF::SetFont($fontbold, '', $fontsize9);
    PDF::MultiCell(15, 0, ' ', 'BL', 'L', false, 0);
    PDF::MultiCell(80, 0, '', 'B', 'L', false, 0);
    PDF::MultiCell(20, 0, ' ', 'B', 'L', false, 0);
    PDF::MultiCell(15, 0, ' ', 'LB', 'L', false, 0);
    PDF::MultiCell(80, 0, '', 'B', 'L', false, 0);
    PDF::MultiCell(20, 0, ' ', 'B', 'L', false, 0);
    PDF::MultiCell(15, 0, ' ', 'BL', 'L', false, 0);
    PDF::MultiCell(80, 0, '', 'B', 'L', false, 0);
  ...
  public function PDF_default_header($params, $data)
  {
    ...
    PDF::MultiCell(340, 25, $this->modulename, '', 'L', false, 0);
    PDF::SetFont($fontbold, '', $fontsize9);
    PDF::MultiCell(50, 25, "Docno #: ", '', 'R', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(100, 25, (isset($data[0]['docno']) ? $data[0]['docno'] : ''), '', 'R', false);

    PDF::SetFont($fontbold, '', $fontsize9);
    PDF::MultiCell(50, 5, 'PAYEE : ', '', 'L', false, 0);
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(300, 5, (isset($data[0]['clientname']) ? $data[0]['clientname'] : ''), 'B', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize9);
    PDF::MultiCell(50, 5, 'DATE:', '', 'L', false, 0);
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(120, 5, (isset($data[0]['dateid']) ? $data[0]['dateid'] : ''), 'B', 'L', false);


    PDF::SetFont($fontbold, '', $fontsize9);
    PDF::MultiCell(50, 5, 'ADDRESS : ', '', 'L', false, 0);
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(300, 5, (isset($data[0]['address']) ? $data[0]['address'] : ''), 'B', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize9);
    PDF::MultiCell(70, 5, 'REFERRENCE # :', '', 'L', false, 0);
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(100, 5, (isset($data[0]['yourref']) ? $data[0]['yourref'] : ''), 'B', 'L', false);

    PDF::MultiCell(0, 0, "\n\n\n");

    PDF::SetFont($fontbold, '', $fontsize9);
    PDF::MultiCell(50, 5, 'NOTES : ', '', 'L', false, 0);
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(470, 5, (isset($data[0]['rem']) ? $data[0]['rem'] : ''), 'B', 'L', false);

    PDF::MultiCell(520, 5, "\n");
    PDF::MultiCell(520, 5, "\n");

    PDF::SetFont($fontbold, '', $fontsize9);
    PDF::MultiCell(50, 5, 'ACCT#', 'TB', 'L', false, 0);
    PDF::MultiCell(100, 5, 'ACCOUNT NAME', 'TB', 'L', false, 0);
    PDF::MultiCell(75, 5, 'CHECK DETAILS', 'TB', 'L', false, 0);
    PDF::MultiCell(75, 5, 'DATE', 'TB', 'C', false, 0);
    PDF::MultiCell(60, 5, 'DEBIT', 'TB', 'R', false, 0);
    PDF::MultiCell(60, 5, 'CREDIT', 'TB', 'R', false, 0);
    PDF::MultiCell(100, 5, 'REMARKS', 'TB', 'L', false);
  }
  ...
  public function PDF_DEFAULT_CCVOUCHER_LAYOUT1($data, $params)
  {
      ...
      for ($i = 0; $i < count($data); $i++) {
        ...
        if ($data[$i]['acnoname'] == '') {
          ...
          for ($r = 0; $r < $maxrow; $r++) {
            ...
            PDF::SetFont($font, '', $fontsize9);
            PDF::MultiCell(50, 0, $acno, '', 'L', false, 0, '', '', true, 1);
            PDF::MultiCell(100, 0, isset($acnonamedescs[$r]) ? $acnonamedescs[$r] : '', '', 'L', false, 0, '', '', false, 1);
            PDF::MultiCell(75, 0, $checkno, '', 'L', false, 0, '', '', false, 1);
            PDF::MultiCell(75, 0, $pdate, '', 'C', false, 0, '', '', false, 1);
            PDF::MultiCell(60, 0, $debit, '', 'R', false, 0, '', '', false, 1);
            PDF::MultiCell(60, 0, $credit, '', 'R', false, 0, '', '', false, 1);
            PDF::MultiCell(100, 0, $drem, '', 'L', false, 1, '', '', false, 1);
          }
        }
        $totaldb += $data[$i]['db'];
        $totalcr += $data[$i]['cr'];
    ...
    PDF::MultiCell(0, 0, "\n");
    PDF::SetFont($fontbold, '', $fontsize9);
    PDF::MultiCell(300, 5, 'GRAND TOTAL : ', '', 'C', false, 0);
    PDF::MultiCell(60, 5, number_format($totaldb, $decimal), '', 'R', false, 0);
    PDF::MultiCell(60, 5, number_format($totalcr, $decimal), '', 'R', false);
  ...
  public function PDF_DEFAULT_CCVOUCHER_LAYOUT2($data, $filters)
  {
    ...
    for ($i = 0; $i < count($data2); $i++) {
      ...
      PDF::SetFont($font, '', 9);
      PDF::MultiCell(570, 0, date('d', strtotime($data2[$i]['postdate'])) . ' / ', '', 'R', false, 0);
      PDF::MultiCell(50, 0, date('m', strtotime($data2[$i]['postdate'])) . ' / ', '', 'C', false, 0);
      PDF::MultiCell(50, 0, date('Y', strtotime($data2[$i]['postdate'])), '', 'C', false, 0);
      PDF::MultiCell(30, 0, '', '', 'C');

      PDF::MultiCell(720, 15, "\n", '', 'L', false);

      PDF::MultiCell(30, 0, '', '', 'C', false, 0);
      PDF::MultiCell(420, 0, '**' . strtoupper($data[0]['clientname']) . '**', '', 'L', false, 0);
      PDF::MultiCell(50, 0, '', '', 'L', false, 0);
  ...
  public function PDF_METROBANK_CHECK_LAYOUT($data, $filters)
  {
    ...
    for ($i = 0; $i < count($data2); $i++) {
      ...
      PDF::MultiCell(450, 0, strtoupper($data[0]['clientname']), '', 'L', false, 0);
      PDF::MultiCell(50, 0, '', '', 'L', false, 0);
      PDF::MultiCell(220, 0, (isset($cc) ? '***' . number_format($cc, $decimal) . '***' : ''), '', 'L', false);
  ...
  private function getinvoiceno($trno)
  {
    return $this->coreFunctions->datareader("select head.invoiceno as value from glhead as head 
      where head.trno=?", [$trno]);
  }

  private function getinvoicedate($trno)
  {
    return $this->coreFunctions->datareader("select date(head.invoicedate) as value from glhead as head 
      where head.trno=?", [$trno]);
  }
}

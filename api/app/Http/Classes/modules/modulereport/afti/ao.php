...
class ao
{
  ...
  private $modulename = "Service Sales Order";
  private $fieldClass;
  ...
  {
    // PLACEHOLDER QRY
    $query = "select cat.cat_name as bstyle, srhead.trno, head.docno,left(head.dateid,10) as dateid,srhead.client,srhead.clientname,srhead.address,srhead.due,srhead.terms,
    srstock.line,srstock.isqty,srstock.iss,srstock.iss,srstock.amt,srstock.ext,srstock.uom,srstock.rem,item.itemname,
    '' as termsdetails,
    infotab.inspo,srhead.due as deldate, infotab.ispartial,infotab.instructions, infotab.period,
    infotab.isvalid,
    ...
    srstock.isamt as gross, srstock.disc,
    srstock.isqty as qty, brands.brand_desc, srhead.vattype, srhead.docno as qsdocno,
    srhead.yourref, cust.tin, cust.addr, cust.tel, '' as industry,
    ...
    conbill.email as billemail,stockinfo.leaddur as itemleadtime,left(srhead.dateid,10) as qtdate,
    bill.addrline1 as baddrline1, bill.addrline2 as baddrline2, bill.city as bcity, bill.zipcode as bzipcode, bill.province as bprovince, bill.country as bcountry,
            ship.addrline1 as saddrline1, ship.addrline2 as saddrline2, ship.city as scity, ship.zipcode as bzipcode, ship.province as sprovince, ship.country as scountry, srhead.cur, branch.clientname as branchname
    from sshead as head
    ...
    left join hheadinfotrans as infotab on infotab.trno = head.qtrno
    left join stockinfotrans as stockinfo on stockinfo.line = srstock.line and stockinfo.trno = srstock.trno
    left join iteminfo on iteminfo.itemid = item.itemid
    left join frontend_ebrands as brands on brands.brandid = item.brand
    left join client as cust on cust.client = srhead.client
    left join client as agent on agent.client= srhead.agent
    left join contactperson as conbill on conbill.line=srhead.billcontactid
    left join contactperson as conship on conship.line=srhead.shipcontactid
    left join billingaddr as bill on bill.line = srhead.billid and bill.clientid = cust.clientid
    left join billingaddr as ship on ship.line = srhead.shipid and ship.clientid = cust.clientid
    ...
    where head.trno='$trno' and head.doc='AO'
    union all
    select cat.cat_name as bstyle, srhead.trno, head.docno,left(head.dateid,10) as dateid,srhead.client,srhead.clientname,srhead.address,srhead.due,srhead.terms,
    srstock.line,srstock.isqty,srstock.iss,srstock.iss,srstock.amt,srstock.ext,srstock.uom,srstock.rem,item.itemname,
    '' as termsdetails, 
    infotab.inspo, srhead.due as deldate, infotab.ispartial,infotab.instructions, infotab.period,
    infotab.isvalid,
    ...
    left join hsrstock as srstock on srstock.trno=srhead.trno
    left join item on item.itemid=srstock.itemid
    left join hheadinfotrans as infotab on infotab.trno = srhead.qtrno
    left join hstockinfotrans as stockinfo on stockinfo.line = srstock.line and stockinfo.trno = srstock.trno
    left join iteminfo on iteminfo.itemid = item.itemid
    left join frontend_ebrands as brands on brands.brandid = item.brand    
    left join client as cust on cust.client = srhead.client
    left join client as agent on agent.client= srhead.agent
  ...
  public function reportplotting($params, $data)
  {
    if ($params['params']['dataparams']['print'] == "default") {
      return $this->default_ao_layout($params, $data);
    } else if ($params['params']['dataparams']['print'] == "PDFM") {
      return $this->AO_SQformat_PDF($params, $data);
    }
  }

  //PDF
  public function AO_SQformat_headerpdf($params, $data, $font)
  {
    ...
    PDF::Image('public/images/afti/qslogo.png', '', '', 310, 80);
    PDF::MultiCell(0, 20, "\n");
    PDF::SetFont($font, 'B', 18, $border);
    PDF::MultiCell(330, 0, '', '', 'L', 0, 0, '', '', false, 0, false, false, 0);
    PDF::MultiCell(245, 0, 'QUOTATION', '', 'C', 0, 0, '', '', false, 0, false, false, 0);
    PDF::MultiCell(0, 30, "\n");

    PDF::SetFont($font, 'B', $fontsize11);
    PDF::MultiCell(255, 15, '', '', 'R', false, 0);
    PDF::MultiCell(75, 15, '', '', 'L', false, 0);
    PDF::SetFillColor(211, 211, 211);
    PDF::MultiCell(100, 15, ' ' . 'Quotation No.',  '1', 'L', 1, 0);
    PDF::SetFont($font, '', $fontsize11);
    PDF::MultiCell(145, 15, ' ' . (isset($data[0]['docno']) ? $data[0]['docno'] : ''), $border, 'L', false);
    ...
    PDF::SetFont($font, '', $fontsize11);
    PDF::MultiCell(145, 15, ' ' . date("F d, Y", strtotime($data[0]['qtdate'])), $border, 'L', false);



    PDF::SetFont($font, '', $fontsize13);
    PDF::MultiCell(255, 15, $headerdata[0]->name, '', 'L', false, 0);
    PDF::MultiCell(75, 15, '', '', 'L', false, 0);
    PDF::SetFont($font, 'B', $fontsize11);
    PDF::SetFillColor(211, 211, 211);
    PDF::MultiCell(100, 15, ' ' . 'Page No.',  '1', 'L', 1, 0);
    PDF::SetFont($font, '', $fontsize11);
    PDF::MultiCell(145, 15, ' ' . 'Page    ' . PDF::PageNo() . '    of    ' . PDF::getAliasNbPages(), $border, 'L', false);

    PDF::SetFont($font, '', $fontsize13);
    PDF::MultiCell(300, 15, $headerdata[0]->address, '', 'L', false, 0);
    PDF::MultiCell(50, 15, '', '', 'L', false, 0);
    PDF::MultiCell(50, 15, '',  '', 'L', false, 0);
    PDF::MultiCell(145, 15, '', '', 'L', false);

    PDF::MultiCell(0, 0, "\n");

    PDF::SetFont($font, '', $fontsize13);
    PDF::MultiCell(300, 15, $headerdata[0]->tel, '', 'L', false, 0);
    PDF::MultiCell(50, 15, '', '', 'L', false, 0);
    PDF::MultiCell(50, 15, '',  '', 'L', false, 0);
    PDF::MultiCell(145, 15, '', '', 'L', false);

    PDF::SetFont($font, '', $fontsize13);
    PDF::MultiCell(300, 15, 'Email: ' . $headerdata[0]->email, '', 'L', false, 0);
    PDF::MultiCell(50, 15, '', '', 'L', false, 0);
    PDF::MultiCell(50, 15, '',  '', 'L', false, 0);
    PDF::MultiCell(145, 15, '', '', 'L', false);

    PDF::SetFont($font, '', $fontsize13);
    PDF::MultiCell(300, 15, 'VAT REG TIN: ' . $headerdata[0]->tin, '', 'L', false, 0);
    PDF::MultiCell(50, 15, '', '', 'L', false, 0);
    PDF::MultiCell(50, 15, '',  '', 'L', false, 0);
    PDF::MultiCell(145, 15, '', '', 'L', false);
    ...
    PDF::MultiCell(275, $max_heights, (isset($data[0]['clientname']) ? $data[0]['clientname'] : ''), 'R', 'L', false, 0, '', '', true, 0, false, true, 0, 'M', true);
    ...
    $arrbstyle = $this->reporter->fixcolumn([$branch, $bstyle, $tin, $data[0]['baddrline1'], $data[0]['baddrline2'], $data[0]['bcity'] . ' ' . $data[0]['bzipcode'], $data[0]['bprovince'], $data[0]['bcountry']], 55, 0);
    $cbstyle = count($arrbstyle);

    $arrbillcon = $this->reporter->fixcolumn([$billcon, $mobile, $billemail], 55, 0);
    ...
    PDF::MultiCell(70, 15, 'Order Code', '1', 'C', 1, 0);
    PDF::MultiCell(70, 15, 'Mfr', '1', 'C', 1, 0);
    PDF::MultiCell(160, 15, 'Description', '1', 'C', 1, 0);
    PDF::MultiCell(60, 15, 'Quantity', '1', 'C', 1, 0);
    PDF::MultiCell(90, 15, 'Unit Price', '1', 'C', 1, 0);
    PDF::MultiCell(100, 15, 'Line Total', '1', 'C', 1, 0);
  }

  //PDF
  public function AO_SQformat_PDF($params, $data)
  {
    ...
    $this->AO_SQformat_headerpdf($params, $data, $font);
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $arrprice = (str_split(trim('PHP ' . number_format($unitprice, $decimalqty)) . ' ', 18));
        $countarrprice = count($arrprice);

        $arrlinetotal = (str_split(trim('PHP ' . number_format($linetotal, $decimalqty)) . ' ', 18));
        ...
        $arrordercode = $this->reporter->fixcolumn([$itemcode], '11');
        $countarrcode = count($arrordercode);

        $arrmfr = $this->reporter->fixcolumn([$itembrand], '11');
        $countarrmfr = count($arrmfr);


        $itemcoldes = $this->reporter->fixcolumn([$itemdescription, $itemaccessories], '30');
        ...
        if ($data[$i]['itemname'] == '') {
          ...
          for ($r = 0; $r < $maxrow; $r++) {
            ...
            PDF::SetFont($font, '', $fontsize9);
            PDF::MultiCell(25, 15, $inum, 'LR', 'C', false, 0, '', '', true, 1, false, true, 0, 'B', true);
            PDF::MultiCell(70, 15, isset($arrordercode[$r]) ? ' ' . $arrordercode[$r] : '', 'LR', 'L', false, 0, '', '', true, 1, false, true, 0, 'B', true);
            PDF::MultiCell(70, 15, isset($arrmfr[$r]) ? ' ' . $arrmfr[$r] : '', 'LR', 'L', false, 0, '', '', true, 1, false, true, 0, 'B', true);
            PDF::MultiCell(160, 15, isset($itemcoldes[$r]) ? ' ' . $itemcoldes[$r] : '', 'LR', 'L', false, 0, '', '', true, 1, false, true, 0, 'B', true);

            PDF::MultiCell(60, 15, isset($arrqty[$r]) ? $arrqty[$r] : '', 'LR', 'C', false, 0, '', '', true, 1, false, true, 0, 'B', true);

            PDF::MultiCell(90, 15, isset($arrprice[$r]) ? $arrprice[$r] : '', 'LR', 'R', false, 0, '', '', true, 1, false, true, 0, 'B', true);
            PDF::MultiCell(100, 15, isset($arrlinetotal[$r]) ? $arrlinetotal[$r] : '', 'LR', 'R', false, 1, '', '', true, 1, false, true, 0, 'B', true);
            if (PDF::getY() >= $page) {
              $this->addrow('LRB');
          ...
          if ($iteminfo != '') {
            $iteminfo = str_replace("</div>", "", $iteminfo);
            $iteminfo = str_replace(["<div>", "<br>", "&#13;", "<br/>", "\n"], "<br />", $iteminfo);

            $height_header = PDF::GetStringHeight(150, $iteminfo, true);


            PDF::SetFont($font, '', $fontsize9);
            PDF::MultiCell(25, $height_header, '', 'LR', 'C', false, 0, '', '', true, 1, false, true, 0, 'B', true);
            PDF::MultiCell(70, $height_header, '', 'LR', 'L', false, 0, '', '', true, 1, false, true, 0, 'B', true);
            PDF::MultiCell(70, $height_header, '', 'LR', 'L', false, 0, '', '', true, 1, false, true, 0, 'B', true);
            PDF::MultiCell(160, $height_header,  isset($iteminfo) ? $iteminfo : '', 'LR', 'L', false, 0, '', '', true, 1, true, true, 0, 'T', true);

            PDF::MultiCell(60, $height_header, '', 'LR', 'C', false, 0, '', '', true, 1, false, true, 0, 'B', true);

            PDF::MultiCell(90, $height_header, '', 'LR', 'R', false, 0, '', '', true, 1, false, true, 0, 'B', true);
            PDF::MultiCell(100, $height_header, '', 'LR', 'R', false, 1, '', '', true, 1, false, true, 0, 'B', true);
          }


          if ($itemleadtime != '') {
            $itemleadtime = str_replace("</div>", "", $itemleadtime);
            $itemleadtime = str_replace(["<div>",  "&#13;", "<br/>", "\n"], "<br />", $itemleadtime);

            $height_header = PDF::GetStringHeight(150, $itemleadtime, true);
            ...
            PDF::MultiCell(70, $height_header, '', 'LR', 'L', false, 0, '', '', true, 1, false, true, 0, 'B', true);
            PDF::MultiCell(70, $height_header, '', 'LR', 'L', false, 0, '', '', true, 1, false, true, 0, 'B', true);
            PDF::MultiCell(160, $height_header,  isset($itemleadtime) ? $itemleadtime : '', 'LR', 'L', false, 0, '', '', true, 1, true, true, 0, 'T', true);
    ...
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(25, 15, 'PHP', 'TB', 'L', false, 0);
    ...
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(25, 15, 'PHP', 'B', 'L', false, 0);
    ...
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(330, 20, 'All Goods Returned by reasons of client`s fault will be charged 20% re-stocking fee of invoice value and shall bear all the costs of returning the goods.', '', 'L', false);
    PDF::MultiCell(0, 0, "");
    ...
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(330, 20, 'All Goods Returned must be reported within 7 (seven days and returned within 15 (fifteen) days from date of delivery undamaged and in its original packaging together with a written incidence report.', '', 'L', false);
    ...
    PDF::SetFont($font, '', $fontsize12);
    PDF::MultiCell(225, 20, (isset($data[0]['clientname']) ? $data[0]['clientname'] : ''), 'B', 'C', false, 0);
    ...
    $c = ceil(strlen((isset($data[0]['clientname']) ? $data[0]['clientname'] : ' ')) / 45);
    for ($i = 0; $i < $c; $i++) {
      PDF::MultiCell(0, 0, "\n");
    }
    ...
    $srtrno = $this->coreFunctions->datareader("select trno as value from hsrhead where sotrno=?", [$trno]);
    $total = $this->coreFunctions->getfieldvalue("hsrstock", "sum(ext)", "trno=?", [$srtrno]);
    $tax = $this->coreFunctions->getfieldvalue("hsrhead", "tax", "trno=?", [$srtrno]);
  ...
  public function default_header($params, $data)
  {
    ...
    $str .= $this->reporter->col('SERVICE SALES ORDER', '600', null, false, $border, '', 'L', $font, '18', 'B', '', '');
  ...
  public function default_ao_layout($params, $data)
  {
  ...
  public function default_ao_header_PDF($params, $data)
  {
    $center = $params['params']['center'];
    $username = $params['params']['user'];
    ...
    PDF::SetFont($font, '', $fontsize9);
    PDF::MultiCell(90, 0, (isset($data[0]['terms']) ? $data[0]['terms'] : ''), 'B', 'L', false, 0, '',  '');
  ...
  public function default_ao_PDF($params, $data)
  {
    $decimalcurr = $this->companysetup->getdecimal('currency', $params['params']);
    $decimalqty = $this->companysetup->getdecimal('qty', $params['params']);
    ...
    $this->default_ao_header_PDF($params, $data);
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        if ($data[$i]['itemname'] == '') {
          PDF::SetFont($font, '', $fontsize9);
          PDF::MultiCell(75, 0, number_format($data[$i]['isqty'], $decimalqty), '', 'C', false, 0, '', '', false, 1);
          PDF::MultiCell(75, 0, $data[$i]['uom'], '', 'C', false, 0, '', '', false, 1);
          PDF::MultiCell(150, 0, $data[$i]['itemname'], '', 'L', false, 0, '', '', false, 1);
          PDF::MultiCell(100, 0, number_format($data[$i]['amt'], $decimalprice), '', 'R', false, 0, '', '', false, 1);
          PDF::MultiCell(100, 0, number_format($data[$i]['ext'], 2), '', 'R', false, 1, '', '', false, 1);
        } else {
          for ($r = 0; $r < $maxrow; $r++) {
            if ($r == 0) {
            ...
            PDF::SetFont($font, '', $fontsize9);
            PDF::MultiCell(75, 0, $qty, '', 'R', false, 0, '', '', false, 1);
            PDF::MultiCell(75, 0, $uom, '', 'C', false, 0, '', '', false, 1);
            PDF::MultiCell(150, 0, isset($itemname[$r]) ? $itemname[$r] : '', '', 'L', false, 0, '', '', false, 1);

...
class customer
{
    ...
    public function reportdefaultAR($config, $data)
    {
        ...
        foreach ($data as $key => $data) {
            ...
            $str .= $this->reporter->startrow();
            $str .= $this->reporter->col($data->docno, '120', null, false, $border, '', 'LT', $font, $fontsize, '', '', '');
            $str .= $this->reporter->col($data->dateid, '50', null, false, $border, '', 'CT', $font, $fontsize, '', '', '');
            $str .= $this->reporter->col($data->agent, '100', null, false, $border, '', 'CT', $font, $fontsize, '', '', '');
            $str .= $this->reporter->col($data->rem, '100', null, false, $border, '', 'CT', $font, $fontsize, '', '', '');
            $str .= $this->reporter->col($debit, '100', null, false, $border, '', 'RT', $font, $fontsize, '', '', '');
            $str .= $this->reporter->col($credit, '100', null, false, $border, '', 'RT', $font, $fontsize, '', '', '');
            $str .= $this->reporter->col($balance, '100', null, false, $border, '', 'RT', $font, $fontsize, '', '', '');
            $str .= $this->reporter->col($data->ref, '130', null, false, $border, '', 'CT', $font, $fontsize, '', '', '');
        ...
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col('', '120', null, false, '1px dotted ', 'T', 'L', $font, $fontsize, 'B', '', '3px');
        ...
        $str .= $this->reporter->col('', '130', null, false, '1px dotted ', 'T', 'C', $font, $fontsize, 'B', '', '3px');
        $str .= $this->reporter->endrow();
    ...
    {
        // $result = $this->generateResult($config);
    ...
    public function reportdefaultAR_PDF($config, $data)
    {
        ...
        if (empty($data)) {
            ...
            PDF::MultiCell(760, 30, "Either there is no transaction or it hasn't been posted yet.", '', 'L', false);
        ...
        foreach ($data as $key => $data) {
            ...
            PDF::SetFont($font, '', $fontsize);
            // MultiCell($w, $h, $txt, $border=0, $align='J', $fill=0, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0)
            PDF::MultiCell(110, 15, $data->docno, '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'B', true);

...
class cv
{
    ...
    public function report_default_query($filters)
    {
        ...
        as tbl order by tbl.ewtdesc";
            ...
            default:
                $query = "select ifnull(DATE_FORMAT(cb.checkdate,'%Y-%m-%d'),
                                DATE_FORMAT(head.dateid,'%Y-%m-%d')) as kdate, ifnull(head2.yourref,'') as dyourref,
                                detail.rem as drem,DATE_FORMAT(left(detail.postdate,10),'%b %d %Y') as pdate,
                                detail.ref,head.trno, head.docno,date(head.dateid) as dateid, 
                        ...
                        union all
                        select ifnull(DATE_FORMAT(cb.checkdate,'%Y-%m-%d'),
                               DATE_FORMAT(head.dateid,'%Y-%m-%d')) as kdate, ifnull(head2.yourref,'') as dyourref,
                               detail.rem as drem,DATE_FORMAT(left(detail.postdate,10),'%b %d %Y') as pdate,
                               detail.ref,head.trno, head.docno,date(head.dateid) as dateid, 
                               date(cntnum.postdate) as postdate,client.client, head.clientname, head.address,
    ...
    public function PDF_default_header($params, $data)
    {
        ...
        PDF::SetFont($fontbold, '', 14);
        PDF::MultiCell(700, 5, (isset($data[0]['projname']) ? $data[0]['projname'] : ''), '', 'L', false);

        PDF::SetFont($fontbold, '', 14);
        PDF::MultiCell(300, 5, '', '', 'L', false, 0);
        PDF::MultiCell(280, 5, "", '', 'R', false, 0, '',  '');
        PDF::MultiCell(120, 5, '', '', 'R', false);
    ...
    public function PDF_DEFAULT_CCVOUCHER_LAYOUT2($data, $params)
    {
        ...
        for ($i = 0; $i < count($data2); $i++) {
            ...
            PDF::SetFont($font, '', 12);
            PDF::MultiCell(400, 0, date('m', strtotime($cdate)), '', 'C', false, 0, 373, 20);
            PDF::MultiCell(400, 0, date('d', strtotime($cdate)), '', 'C', false, 0, 416, 20);
            PDF::MultiCell(400, 0, date('Y', strtotime($cdate)), '', 'C', false, 0, 481, 20);

            PDF::setFontSpacing(1);

            PDF::MultiCell(120, 5, '', '', 'C', false, 0);
            PDF::MultiCell(450, 5, $data[0]['clientname'], '', 'L', false, 0, 105, 55);
            PDF::MultiCell(150, 5, (isset($cc) ? number_format($cc, $decimal) : ''), '', 'L', false, 0, 560, 55);
            ...
            PDF::MultiCell(700, 5, $this->reporter->ftNumberToWordsConverter($dd), '', 'L', false, 0, 83, 85);
    ...
    public function PDF_BDO_CHECK_LAYOUT($data, $params)
    {
        ...
        for ($i = 0; $i < count($data2); $i++) {
            ...
            $clientname = strlen($data[0]['clientname']);
            PDF::MultiCell(120, 5, '', '', 'C', false, 0);

            if ($clientname <= 60) {
                PDF::MultiCell(450, 5, $data[0]['clientname'], '', 'L', false, 0, 105, 55);
            } else {
                PDF::MultiCell(450, 5, $data[0]['clientname'], '', 'L', false, 0, 105, 40);
            }
    ...
    ";
        ...
        for ($i = 0; $i < count($data2); $i++) {
            ...
            $dd = number_format((float)$cc, 2, '.', '');
            PDF::MultiCell(760, 0, '***' . $this->reporter->ftNumberToWordsConverter($dd) . '***', '', 'L', false);
        }
    ...
    ";
        ...
        for ($i = 0; $i < count($data2); $i++) {
            ...
            PDF::MultiCell(470, 0, '**' . strtoupper($data[0]['clientname']) . '**', '', 'L', false, 0);
            PDF::MultiCell(320, 0, (isset($cc) ? '**' . number_format($cc, $decimal) . '**' : ''), '', 'L', false);

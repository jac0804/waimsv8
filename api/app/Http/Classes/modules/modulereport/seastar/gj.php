...
class gj
{
    ...
    public function report_default_query($filters)
    {
        ...
        $query = " select head.trno, date(head.dateid) as dateid, head.docno, 
                    ...
                    detail.db, detail.cr, detail.line,proj.name as projname,head.rem
                    from ((lahead as head 
    ...
    public function default_GJ_header_PDF($params, $data)
    {
        ...
        PDF::MultiCell(550, 5, '', '', 'L', false, 0);
        PDF::MultiCell(150, 5, (isset($data[0]['projname']) ? $data[0]['projname'] : ''), '', 'L', false);


        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(120, 5, "Customer/Supplier: ", '', 'L', false, 0, '',  '');
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(430, 5, (isset($data[0]['clientname']) ? $data[0]['clientname'] : ''), 'B', 'L', false, 0, '',  '');
        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(50, 5, "Date: ", '', 'L', false, 0, '',  '');
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(100, 5, (isset($data[0]['dateid']) ? $data[0]['dateid'] : ''), 'B', 'L', false);
        ...
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(490, 5, (isset($data[0]['address']) ? $data[0]['address'] : ''), 'B', 'L', false, 0, '',  '');
        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(50, 5, "Ref: ", '', 'L', false, 0, '',  '');
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(100, 5, (isset($data[0]['yourref']) ? $data[0]['yourref'] : ''), 'B', 'L', false);

        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(60, 5, 'NOTES : ', '', 'L', false, 0);
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(640, 5, (isset($data[0]['rem']) ? $data[0]['rem'] : ''), 'B', 'L', false);

...
class stockcard
{
    ...
    {
        $fields = ['radioprint', 'radiotypeofreport', 'start', 'end', 'wh', 'luom', 'loc', 'prepared', 'approved', 'received'];
        ...
        if ($view_history || $view_intransaction || $view_po_jo_order_history || $view_sales_order_history) {
        ...
        if ($view_history) array_push($reportoption, ['label' => 'Ledger Report', 'value' => 'ledger', 'color' => 'red']);
        if ($view_intransaction) array_push($reportoption, ['label' => 'Receiving Report', 'value' => 'receiving', 'color' => 'red']);
        if ($view_po_jo_order_history) array_push($reportoption, ['label' => 'Purchase Order Report', 'value' => 'po', 'color' => 'red']);
        if ($view_sales_order_history) array_push($reportoption, ['label' => 'Sales Order Report', 'value' => 'so', 'color' => 'red']);

        data_set($col1, 'radiotypeofreport.options', $reportoption);
    ...
    public function QUERY_LEDGER($config)
    {
        ...
        $query = "select '' as expiry, '' as posted,  '' as itemname,  '' as barcode, 0 as trno, '' as doc, 'beginning bal.' as docno,null as  dateid, 
    ...
    '' as tel,'' as  email, '' as tin, '' as mobile, '' as contact, '' as fax,'' as headrem, sum(qty-iss) as bal,'' as statux 
    ...
    client.client,client.clientname,client.addr,client.tel,client.email,client.tin,client.mobile,client.contact,client.fax,head.rem as headrem,'Posted' as statux
    from glhead as head
    ...
    where md5(item.itemid)='$itemid' and head.dateid < '$start' " . $whbyfield . " $loc
    union all
    ...
    head.isimport as isimport,stock.line as line,head.cur as cur,head.forex as forex,head.factor as factor,
    stock.rem as rem,stock.encodeddate as encoded,
    client.client,client.clientname,client.addr,client.tel,client.email,client.tin,client.mobile,client.contact,client.fax,head.rem as headrem,'Unposted' as statux
    from lahead as head
    ...
    head.isimport as isimport,stock.line as line,head.cur as cur,head.forex as forex,head.factor as factor,
    stock.rem as rem,stock.encodeddate as encoded,
    client.client,client.clientname,client.addr,client.tel,client.email,client.tin,client.mobile,client.contact,client.fax,head.rem as headrem,0 as bal,'Posted' as statux
    from glhead as head
    left join glstock as stock on stock.trno=head.trno
    left join item on item.itemid=stock.itemid
    left join uom on uom.itemid=item.itemid and uom.uom='" . $uom . "'
    ...
    left join cntnum on cntnum.trno=head.trno
    left join client on client.clientid=head.clientid
    where md5(item.itemid)='$itemid' and head.dateid between '$start' and '$end' " . $whbyfield . " $loc
    union all
    select stock.expiry as expiry, '' as posted,item.itemname,item.barcode,head.trno as trno,head.doc as doc,head.docno as docno,
    left(head.dateid,10) as dateid,
    round(case when uom.factor <= 1 then ifnull((stock.cost / (case when ifnull(uom.factor,0)=0 then 1 else uom.factor end)),0) else ifnull((stock.cost * (case when ifnull(uom.factor,0)=0 then 1 else uom.factor end)),0) end,2) as cost,
    ...
    head.isimport as isimport,stock.line as line,head.cur as cur,head.forex as forex,head.factor as factor,
    stock.rem as rem,stock.encodeddate as encoded,
    client.client,client.clientname,client.addr,client.tel,client.email,client.tin,client.mobile,client.contact,client.fax,head.rem as headrem,0 as bal,'Unposted' as statux
    from lahead as head
    left join lastock as stock on stock.trno=head.trno
    left join item on item.itemid=stock.itemid
    left join uom on uom.itemid=item.itemid and uom.uom='" . $uom . "'
    ...
    public function default_ledger_tablecols($layoutsize, $border, $font, $fontsize, $companyid, $config)
    {
        ...
        $str .= $this->reporter->col('QTY IN ', '65', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '1px');
        $str .= $this->reporter->col('QTY OUT ', '75', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '1px');
        $str .= $this->reporter->col('BALANCE', '70', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '1px');
        $str .= $this->reporter->col('PARTICULAR', '95', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '1px');
        $str .= $this->reporter->col('STATUS', '50', null, false, $border, 'LRTB', 'C', $font, $fontsize, 'B', '', '1px');
        return $str;
    }
    ...
    public function report_default_LEDGER($config, $result)
    {
        ...
        foreach ($result as $key => $data) {
            ...
            if ($data->docno == 'beginning bal.') {
                ...
                $str .= $this->reporter->col($data->docno, '100', null, false, $border, '', 'L', $font, $fontsize, '', '', '');
                $str .= $this->reporter->col($qty == 0 ? '-' : number_format($qty, $qtydec), '65', null, false, $border, '', 'R', $font, $fontsize, '', '', '');
                $str .= $this->reporter->col($iss == 0 ? '-' : number_format($iss, $qtydec), '75', null, false, $border, '', 'R', $font, $fontsize, '', '', '');
                $str .= $this->reporter->col(number_format($tobal, $qtydec), '70', null, false, $border, '', 'R', $font, $fontsize, '', '', '');
                $str .= $this->reporter->col($data->rem, '95', null, false, '1psx solid ', '', 'R', $font, $fontsize, '', '', '');
                $str .= $this->reporter->col($data->statux, '50', null, false, '1psx solid ', '', 'C', $font, $fontsize, '', '', '');
        ...
        $str .= $this->reporter->col('TOTAL QTY : ', '100', null, false, $border, 'T', 'R', $font, $fontsize, '', '', '');
        $str .= $this->reporter->col(number_format($totalqty, $qtydec), '65', null, false, $border, 'T', 'R', $font, $fontsize, '', '', '');
        $str .= $this->reporter->col(number_format($totaliss, $qtydec), '75', null, false, $border, 'T', 'R', $font, $fontsize, '', '', '');
        $str .= $this->reporter->col(number_format($tobal, $qtydec), '70', null, false, $border, 'T', 'R', $font, $fontsize, '', '', '');
    ...
    public function report_default_RECEIVING($config, $result)
    {
        ...
        $str .= $this->reporter->col('View by Unit :', '75', null, false, $border, '', 'L', $font, $fontsize, '', '', '1px');
    ...
    public function report_default_PO($config, $result)
    {
        ...
        $str .= $this->reporter->begintable('800');
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col('Item Code:', '125', null, false, $border, '', 'L', $font, $fontsize, '', '', '1px');
        $str .= $this->reporter->col((isset($result[0]->barcode) ? $result[0]->barcode : ''), '150', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '1px');
        $str .= $this->reporter->col('Price Levels', '75', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '1px');
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();

        $str .= $this->reporter->begintable('800');
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col('Item Name:', '125', null, false, $border, '', 'L', $font, $fontsize, '', '', '1px');
        $str .= $this->reporter->col((isset($result[0]->itemname) ? $result[0]->itemname : ''), '150', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '1px');
        $str .= $this->reporter->col('Retail:', '75', null, false, $border, '', 'L', $font, $fontsize, '', '', '1px');
        $str .= $this->reporter->col((isset($result[0]->priceretail) ? number_format($result[0]->priceretail, 2) : ''), '100', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '1px');
        $str .= $this->reporter->col('Disc 1:', '75', null, false, $border, '', 'L', $font, $fontsize, '', '', '1px');
        $str .= $this->reporter->col((isset($result[0]->discretail) ? $result[0]->discretail : ''), '100', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '1px');
        ...
        $str .= $this->reporter->begintable('800');
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col('Brand:', '125', null, false, $border, '', 'L', $font, $fontsize, '', '', '1px');
        $str .= $this->reporter->col((isset($result[0]->brand) ? $result[0]->brand : ''), '150', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '1px');
        $str .= $this->reporter->col('Wholesale:', '75', null, false, $border, '', 'L', $font, $fontsize, '', '', '1px');
        $str .= $this->reporter->col((isset($result[0]->pricewhole) ? number_format($result[0]->pricewhole, 2) : ''), '100', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '1px');
        $str .= $this->reporter->col('Disc 2:', '75', null, false, $border, '', 'L', $font, $fontsize, '', '', '1px');
        $str .= $this->reporter->col((isset($result[0]->discwhole) ? $result[0]->discwhole : ''), '100', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '1px');
        ...
        $str .= $this->reporter->begintable('800');
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col('Model:', '125', null, false, $border, '', 'L', $font, $fontsize, '', '', '1px');
        $str .= $this->reporter->col((isset($result[0]->model) ? $result[0]->model : ''), '150', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '1px');
        $str .= $this->reporter->col('Group 1:', '75', null, false, $border, '', 'L', $font, $fontsize, '', '', '1px');
        $str .= $this->reporter->col((isset($result[0]->pricegrp1) ? number_format($result[0]->pricegrp1, 2) : ''), '100', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '1px');
        $str .= $this->reporter->col('Disc 3:', '75', null, false, $border, '', 'L', $font, $fontsize, '', '', '1px');
        $str .= $this->reporter->col((isset($result[0]->discgrp1) ? $result[0]->discgrp1 : ''), '100', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '1px');
        ...
        $str .= $this->reporter->begintable('800');
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col('Part#:', '125', null, false, $border, '', 'L', $font, $fontsize, '', '', '1px');
        $str .= $this->reporter->col((isset($result[0]->part) ? $result[0]->part : ''), '150', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '1px');
        $str .= $this->reporter->col('Group 2:', '75', null, false, $border, '', 'L', $font, $fontsize, '', '', '1px');
        $str .= $this->reporter->col((isset($result[0]->pricegrp2) ? number_format($result[0]->pricegrp2, 2) : ''), '100', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '1px');
        $str .= $this->reporter->col('Disc 4:', '75', null, false, $border, '', 'L', $font, $fontsize, '', '', '1px');
        $str .= $this->reporter->col((isset($result[0]->discgrp2) ? $result[0]->discgrp2 : ''), '100', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '1px');
        ...
        $str .= $this->reporter->begintable('800');
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col('Size:', '125', null, false, $border, '', 'L', $font, $fontsize, '', '', '1px');
        $str .= $this->reporter->col((isset($result[0]->sizeid) ? $result[0]->sizeid : ''), '150', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '1px');
        $str .= $this->reporter->col('', '75', null, false, $border, '', 'L', $font, $fontsize, '', '', '1px');
        if ((isset($result[0]->isinactive) ? $result[0]->isinactive : '') == 1) {
            $str .= $this->reporter->col('Innactive', '100', null, false, $border, '', 'L', $font, $fontsize, 'B', '', '1px');
        } else {
        ...
        foreach ($result as $key => $data) {
            ...
            $str .= $this->reporter->col('', '70', null, false, $border, '', 'R', $font, $fontsize, '', '', '');
            $str .= $this->reporter->endrow();
        }
        $str .= $this->reporter->endtable();
    ...
    public function PDF_LEDGER_HEADER($config, $data)
    {
        ...
        PDF::SetFont($font, '', 11);
        PDF::MultiCell(70, 0, "DATE", 'TLB', 'C', false, 0);
        PDF::MultiCell(155, 0, "CLIENTNAME", 'TLB', 'C', false, 0);
        ...
        PDF::MultiCell(50, 0, "STATUS", 'TLB', 'C', false, 0);
    ...
    public function report_PDF_LEDGER($config, $data)
    {
        ...
        foreach ($data as $key => $data) {
            ...
            if ($data->docno == 'beginning bal.') {
                ...
                PDF::MultiCell(155, 0, $data->docno, '', 'L', false, 0);
                ...
                PDF::MultiCell(70, 0, "", '', 'R', false, 1);
            } else {

                PDF::SetFont($font, '', 11);
                PDF::MultiCell(70, 0, $data->dateid, '', 'C', false, 0);
                PDF::MultiCell(155, 0, $data->clientname, '', 'L', false, 0);
                ...
                PDF::MultiCell(50, 0, $data->statux, '', 'C', false, 0);
    ...
    public function report_PDF_PO($config, $data)
    {
        ...
        foreach ($data as $key => $data) {
            ...
            PDF::MultiCell(200, 0, $data->clientname, 'B', 'L', false, 0);
            PDF::MultiCell(100, 0, number_format($data->qty, 2), 'B', 'R', false, 0);
            PDF::MultiCell(100, 0, number_format($data->qa, 2), 'B', 'R', false);

            if ($companyid == 23 || $companyid == 41 || $companyid == 52) { //labsol
                PDF::MultiCell(100, 0, number_format($data->rrcost, 2), 'B', 'R', false);
            }
        }
    ...
    public function report_PDF_SO($config, $data)
    {
        ...
        foreach ($data as $key => $data) {
            ...
            PDF::MultiCell(100, 0, $data->dateid, 'B', 'C', false, 0);
            PDF::MultiCell(200, 0, $data->clientname, 'B', 'L', false, 0);
            PDF::MultiCell(150, 0, number_format($data->qty, 2), 'B', 'R', false, 0);
            PDF::MultiCell(150, 0, number_format($data->qa, 2), 'B', 'R', false);
        }

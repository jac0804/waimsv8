...
class cv
{
    ...
    public function reportparamsdata($config)
    {
        ...
        $approved = $this->coreFunctions->datareader("select fieldvalue as value from signatories where fieldname = 'approved' and doc =? limit 1 ", [$config['params']['doc']]);
        $received = $this->coreFunctions->datareader("select fieldvalue as value from signatories where fieldname = 'received' and doc =? and userid = ? ", [$config['params']['doc'],$config['params']['user']]);
        $checked = $this->coreFunctions->datareader("select fieldvalue as value from signatories where fieldname = 'checked' and doc =? and userid = ? ", [$config['params']['doc'],$config['params']['user']]);
        $payor = $this->coreFunctions->datareader("select fieldvalue as value from signatories where fieldname = 'payor' and doc =? and userid = ? ", [$config['params']['doc'],$config['params']['user']]);
        $tin = $this->coreFunctions->datareader("select fieldvalue as value from signatories where fieldname = 'tin' and doc =? and userid = ? ", [$config['params']['doc'],$config['params']['user']]);
        $position = $this->coreFunctions->datareader("select fieldvalue as value from signatories where fieldname = 'position' and doc =? and userid = ? ", [$config['params']['doc'],$config['params']['user']]);
    ...
    public function report_default_query($filters)
    {
        ...
        as tbl order by tbl.ewtdesc";
            ...
            default:
                ...
                $query = "select ifnull(DATE_FORMAT(head.dateid,'%Y-%m-%d'),'') as kdate, ifnull(head2.yourref,'') as dyourref,detail.rem as drem,
                            ...
                            date(head.dateid) as dateid, abs(detail.db - detail.cr) as amount,
    ...
    public function PDF_default_header($params, $data)
    {
        ...
        PDF::MultiCell(300, 5, "CHECK NO. : ", '', 'R', false, 0, '',  '');
        PDF::SetFont($font, '', 11);
        PDF::MultiCell(100, 5, (isset($data[0]['docno']) ? $data[0]['docno'] : ''), '', 'L', false, 1);

        PDF::MultiCell(0, 0, "\n\n");

        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(40, 5, 'Date : ', '', 'L', false, 0);
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(660, 5, (isset($data[0]['vcdateid']) ? $data[0]['vcdateid'] : ''), '', 'L', false, 1, '',  '');


        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(45, 5, 'Name : ', '', 'L', false, 0);
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(655, 5, (isset($data[0]['clientname']) ? $data[0]['clientname'] : ''), '', 'L', false, 1, '',  '');



        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(60, 5, 'ADDRESS : ', '', 'L', false, 0);
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(640, 5, (isset($data[0]['address']) ? $data[0]['address'] : ''), '', 'L', false, 1, '',  '');
        ...
        PDF::SetFont($font, '', 5);
        PDF::MultiCell(720, 0, '', 'T', '', false);

        PDF::SetFont($fontbold, '', 11);
        PDF::MultiCell(400, 25, 'PARTICULARS', '', 'C', false, 0);
        PDF::MultiCell(110, 25, 'DEBIT', '', 'R', false, 0);
        PDF::MultiCell(140, 25, 'CREDIT', '', 'R', false, 0);
    ...
    public function PDF_DEFAULT_CCVOUCHER_LAYOUT1($data, $params)
    {
        ...
        if (!empty($data)) {
            ...
            for ($i = 0; $i < count($data); $i++) {
                ...
                $arr_acnonamedescs = $this->reporter->fixcolumn([$acnonamedescs], '100', 0);
                $arr_debit = $this->reporter->fixcolumn([$debit], '15', 0);
                $arr_credit = $this->reporter->fixcolumn([$credit], '15', 0);

                $maxrow = $this->othersClass->getmaxcolumn([$arr_acno, $arr_acnonamedescs, $arr_debit, $arr_credit]);

                for ($r = 0; $r < $maxrow; $r++) {
                    PDF::SetFont($font, '', $fontsize);
                    PDF::MultiCell(400, 15, ' ' . (isset($arr_acno[$r]) ? $arr_acno[$r] . ' ' . $arr_acnonamedescs[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                    PDF::MultiCell(110, 15, ' ' . (isset($arr_debit[$r]) ? $arr_debit[$r] : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                    PDF::MultiCell(140, 15, ' ' . (isset($arr_credit[$r]) ? $arr_credit[$r] : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                    PDF::MultiCell(50, 15, ' ' . '', '', 'L', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
                }
                if (PDF::getY() > 900) {
                    $this->PDF_default_header($params, $data);
                }
        ...
        PDF::MultiCell(0, 0, "\n\n");
        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(90, 5, 'EXPLANATION : ', '', 'R', false, 0);
        PDF::SetFont($font, '', 10);
        PDF::MultiCell(610, 0, (isset($data[0]['rem']) ? $data[0]['rem'] : ''), '', 'B', false, 1);
        ...
        $qry = "
        select count(alias) as alias,acnoname,amount,checkno,pdate from ( 
        ...
        left join gldetail as detail on detail.trno=head.trno
        left join coa on coa.acnoid=detail.acnoid
        where head.doc = 'cv' and left(coa.alias,2) = 'cb' and head.trno = " . $trno . " ) as v group by checkno,amount,alias,acnoname,pdate";
        ...
        if (count($cbdata) == 0) {
            ...
            for ($j = 0; $j < 1; $j++) {
                ...
                for ($c = 0; $c < 4; $c++) {
                    ...
                    PDF::SetFont($fontbold, '', 11);
                    if ($c == 0) {
                        PDF::MultiCell(60, 15, 'CHECK :', '', 'L', false, 0);
                    }
                    PDF::SetFont($font, '', $fontsize);
                    PDF::MultiCell(10, 15, '', '', 'L', false, 0);
                    PDF::MultiCell(130, 15, '' . (isset($cbdata[$c]['checkno']) ? $cbdata[$c]['checkno'] : ""), 'B', 'L', false, 0);
                }
                PDF::MultiCell(0, 15, '', '', 'L', false, 1);
                for ($d = 0; $d < 4; $d++) {
                    ...
                    PDF::SetFont($font, '', $fontsize);
                    PDF::MultiCell(10, 15, '', '', 'L', false, 0);
                    PDF::MultiCell(130, 15, '' . (isset($cbdata[$d]['pdate']) ? $cbdata[$d]['pdate'] : ""), 'B', 'L', false, 0);
                }
                PDF::MultiCell(0, 15, '', '', 'L', false, 1);
                for ($a = 0; $a < 4; $a++) {
                    ...
                    PDF::SetFont($fontbold, '', 11);
                    if ($a == 0) {
                        PDF::MultiCell(60, 15, 'AMOUNT :', '', 'L', false, 0);
                    }
                    PDF::SetFont($font, '', $fontsize);
                    PDF::MultiCell(10, 15, '', '', 'L', false, 0);
                    PDF::MultiCell(130, 15, '' . (isset($cbdata[$a]['amount']) ? number_format($cbdata[$a]['amount'], 2) : ""), 'B', 'L', false, 0);
                }
                PDF::MultiCell(0, 15, '', '', 'L', false, 1);
            }
        }

...
class st
{
  ...
  public function report_default_query($trno)
  {
    ...
    $query = "select head.docno,head.trno, head.clientname, head.address, date(head.dateid) as dateid, head.terms, head.rem,
        item.barcode, item.itemname, stock.rrcost as gross, stock.isamt as netamt, case item.isserial when 1 then 1 else stock.isqty end as qty,
        ...
        ifnull(rr.chassis,'') as chassis, ifnull(rr.serial,'') as engine,item.isserial
        from lahead as head 
  ...
  public function default_ST_LAYOUT($params, $data)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      if($data[$i]['isserial'] == 0){
        $ext = number_format($data[$i]['ext'], $decimal);
      }else{
        $ext = $data[$i]['netamt'];
        $ext = number_format($ext, $decimal);
      }
  ...
  public function vitaline_ST_header_PDF($params, $data)
  {
    ...
    PDF::MultiCell(100, 0, "DESCRIPTION", '', 'L', false, 0);
    PDF::MultiCell(85, 0, "ENGINE", '', 'C', false, 0);
    PDF::MultiCell(85, 0, "CHASSIS", '', 'C', false, 0);
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        if($data[$i]['isserial'] == 0){
          ...
          $ext = $data[$i]['netamt'];
          $ext = number_format($ext, $decimalcurr);
          $extamt = $data[$i]['netamt'];
        }

        
        // $partno = $data[$i]['partno'];
        $engine = $data[$i]['engine'];
        $chassis = $data[$i]['chassis'];
        ...
        $arr_engine = $this->reporter->fixcolumn([$engine], '16', 0);
        $arr_chassis = $this->reporter->fixcolumn([$chassis], '16', 0);

        $maxrow = $this->othersClass->getmaxcolumn([$arr_barcode, $arr_qty, $arr_uom, $arr_itemname, $arr_netamt, $arr_disc, $arr_ext, $arr_engine, $arr_chassis]);
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(100, 0, (isset($arr_itemname[$r]) ? $arr_itemname[$r] : ''), '', 'L', 0, 0, '', '', true, 0, true, false);
          PDF::MultiCell(85, 0, (isset($arr_engine[$r]) ? $arr_engine[$r] : ''), '', 'L', 0, 0, '', '', true, 0, true, false);
          PDF::MultiCell(85, 0, (isset($arr_chassis[$r]) ? $arr_chassis[$r] : ''), '', 'L', 0, 0, '', '', true, 0, true, false);

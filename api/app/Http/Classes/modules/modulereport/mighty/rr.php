...
class rr
{
  ...
  private $modulename = "Receiving Reports";
  private $fieldClass;
  ...
  public function report_default_query($config)
  {
    ...
    $query = "select head.docno,head.trno, head.clientname, head.address, date(head.dateid) as dateid, head.terms, head.rem,head.ourref,head.yourref,
    ...
    union all
    select head.docno, head.trno, head.clientname, head.address, date(head.dateid) as dateid, head.terms, head.rem,head.ourref,head.yourref,
    item.barcode, item.itemname, stock.rrcost as gross, stock.cost as netamt, stock.rrqty as qty,
    stock.uom, stock.disc, stock.ext, stock.line,wh.client as wh,wh.clientname as whname,stock.loc,date(stock.expiry) as expiry,stock.rem as srem,item.sizeid,m.model_name as model
    from (glhead as head
  ...
  public function default_RR_header_PDF($params, $data)
  {
    ...
    PDF::MultiCell(800, 0, $this->modulename, '', 'L', false);


    PDF::MultiCell(0, 0, "\n");

    PDF::SetFont($fontbold, '', 18);
    PDF::MultiCell(600, 0, "", '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
    PDF::SetFont($fontbold, '', $fontsize);
    ...
    PDF::MultiCell(570, 20, (isset($data[0]['clientname']) ? $data[0]['clientname'] : ''), 'B', 'L', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
    PDF::SetFont($fontbold, '', $fontsize);
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(570, 20, (isset($data[0]['address']) ? $data[0]['address'] : ''), 'B', 'L', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
    PDF::SetFont($fontbold, '', $fontsize);
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(570, 20, (isset($data[0]['ourref']) ? $data[0]['ourref'] : ''), 'B', 'L', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
    PDF::SetFont($fontbold, '', $fontsize);
  ...
  public function default_RR_PDF($params, $data)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $maxrow = $this->othersClass->getmaxcolumn([$arr_barcode, $arr_itemname, $arr_wh, $arr_qty, $arr_uom, $arr_amt, $arr_disc, $arr_ext]);
      for ($r = 0; $r < $maxrow; $r++) {
        ...
        PDF::MultiCell(100, 15, ' ' . (isset($arr_wh[$r]) ? $arr_wh[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
    ...
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(700, 0, 'GRAND TOTAL: ', '', 'R', false, 0);
    ...
    PDF::MultiCell(750, 0, $data[0]['rem'], '', 'L');

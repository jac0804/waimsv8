...
class customer
{
    ...
    public function reportdefaultPDC_PDF($config, $data)
    {
        ...
        foreach ($data as $key => $data1) {
            ...
            for ($r = 0; $r < $maxrow; $r++) {
                ...
                PDF::MultiCell(100, 15, (isset($arr_rem[$r]) ? $arr_rem[$r] : ''), '', 'L', 0, 0, '', '', true, 0, true, false);
                PDF::MultiCell(90, 15, (isset($arr_debit[$r]) ? $debit[$r] : ''), '', 'R', 0, 0, '', '', true, 0, true, false);
                PDF::MultiCell(90, 15, (isset($arr_credit[$r]) ? $credit[$r] : ''), '', 'R', 0, 0, '', '', true, 0, true, false);
                PDF::MultiCell(5, 15, '', '', 'R', 0, 0, '', '', true, 0, true, false);
    ...
    public function reportdefaultRC_PDF($config, $data)
    {
        ...
        foreach ($data as $key => $data) {
            ...
            for ($r = 0; $r < $maxrow; $r++) {
                ...
                PDF::MultiCell(90, 15, (isset($arr_balance[$r]) ? $arr_balance[$r] : ''), '', 'R', 0, 0, '', '', true, 0, true, false);
                PDF::MultiCell(5, 15, '', '', 'R', 0, 0, '', '', true, 0, true, false);
                PDF::MultiCell(95, 15, (isset($arr_ref[$r]) ? $arr_ref[$r] : ''), '', 'L', 0, 1, '', '', true, 0, true, false);
            }
    ...
    public function reportdefaultSTOCK_PDF($config, $data)
    {
        ...
        foreach ($data as $key => $data1) {
            ...
            $arr_rrqty = $this->reporter->fixcolumn([$rrqty], '10', 0);
            $arr_isqty = $this->reporter->fixcolumn([$isqty], '10', 0);
            ...
            for ($r = 0; $r < $maxrow; $r++) {
                ...
                PDF::MultiCell(60, 15, (isset($arr_isqty[$r]) ? $arr_isqty[$r] : ''), '', 'R', 0, 1, '', '', true, 0, true, false);
            }
        }

...
class aj
{
  ...
  public function report_default_query($filters)
  {
    ...
    wh.clientname as whname,stock.loc,stock.expiry
    from lahead as head 
  ...
  public function default_AJ_header_PDF($params, $data)
  {
    ...
    PDF::MultiCell(125, 0, "LOCATION", '', 'C', false, 0);
    PDF::MultiCell(125, 0, "EXPIRY", '', 'C', false);
  ...
  public function default_AJ_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $arr_swh = $this->reporter->fixcolumn([$swh], '16', 0);
        $arr_loc = $this->reporter->fixcolumn([$loc], '16', 0);
        $arr_expiry = $this->reporter->fixcolumn([$expiry], '16', 0);

        $maxrow = $this->othersClass->getmaxcolumn([$arr_qty, $arr_uom, $arr_itemname, $arr_swh, $arr_loc, $arr_expiry]);
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(100, 0, (isset($arr_swh[$r]) ? $arr_swh[$r] : ''), '', 'C', false, 0, '', '', false, 1);
          PDF::MultiCell(125, 0, (isset($arr_loc[$r]) ? $arr_loc[$r] : ''), '', 'C', false, 0, '', '', false, 1);
          PDF::MultiCell(125, 0, (isset($arr_expiry[$r]) ? $arr_expiry[$r] : ''), '', 'C', false, 1, '', '', false, 1);
        }
    ...
    PDF::MultiCell(50, 0, number_format($totalqty, $decimalcurr), '', 'R');

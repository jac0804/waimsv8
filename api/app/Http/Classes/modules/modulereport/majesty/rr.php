...
class rr
{
  ...
  public function report_default_query($config)
  {
    ...
    $query = "select head.docno,head.trno, head.clientname, head.address, date(head.dateid) as dateid, head.terms, head.rem,
        ...
        stock.uom, stock.disc, stock.ext, stock.line,wh.client as wh,wh.clientname as whname,stock.loc,date(stock.expiry) as expiry,stock.rem as srem,item.sizeid,m.model_name as model,client.clientname as hwh,stock.loc,stock.expiry
        from lahead as head
  ...
  public function default_RR_header_PDF($params, $data)
  {
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(470, 0, (isset($data[0]['ourref']) ? $data[0]['ourref'] : ''), 'B', 'L', false, 0, '', '');
    PDF::SetFont($fontbold, '', $fontsize);
    ...
    PDF::MultiCell(40, 0, "UNIT", '', 'C', false, 0);
    PDF::MultiCell(225, 0, "DESCRIPTION", '', 'L', false, 0);
    PDF::MultiCell(100, 0, "WH", '', 'C', false, 0);
    PDF::MultiCell(75, 0, "LOCATION", '', 'R', false, 0);
    PDF::MultiCell(85, 0, "EXPIRY", '', 'C', false);
  ...
  public function default_RR_PDF($params, $data)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $arr_wh = $this->reporter->fixcolumn([$wh],'16',0);
      $arr_loc = $this->reporter->fixcolumn([$loc],'16',0);
      $arr_expiry = $this->reporter->fixcolumn([$expiry],'16',0);

      $maxrow = $this->othersClass->getmaxcolumn([$arr_barcode, $arr_qty, $arr_uom, $arr_itemname, $arr_wh, $arr_loc, $arr_expiry]);

      for($r = 0; $r < $maxrow; $r++) {
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(100, 0, (isset($arr_barcode[$r]) ? $arr_barcode[$r] : ''), '', 'C', 0, 0, '', '', true, 0, true, false);
        PDF::MultiCell(75, 0, (isset($arr_qty[$r]) ? $arr_qty[$r] : ''), '', 'C', 0, 0, '', '', true, 0, true, false);
        PDF::MultiCell(40, 0, (isset($arr_uom[$r]) ? $arr_uom[$r] : ''), '', 'C', 0, 0, '', '', true, 0, true, false);
        PDF::MultiCell(225, 0, (isset($arr_itemname[$r]) ? $arr_itemname[$r] : ''), '', 'L', 0, 0, '', '', true, 0, true, false);
        PDF::MultiCell(100, 0, (isset($arr_wh[$r]) ? $arr_wh[$r] : ''), '', 'C', 0, 0, '', '', true, 0, true, false);
        PDF::MultiCell(75, 0, (isset($arr_loc[$r]) ? $arr_loc[$r] : ''), '', 'R', 0, 0, '', '', true, 0, true, false);
        PDF::MultiCell(85, 0, (isset($arr_expiry[$r]) ? $arr_expiry[$r] : ''), '', 'C', 0, 1, '', '', true, 0, false, false);
      }

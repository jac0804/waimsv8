...
class stockcard
{
  ...
  public function createreportfilter($config)
  {
    ...
    data_set($col1, 'loc.lookupclass', 'replookuplocitem');
    data_set(
      $col1,
  ...
  public function QUERY_LEDGER($config)
  {
    ...
    $query = "select '' as expiry, '' as posted,  '' as itemname,  '' as barcode, 0 as trno, '' as doc, 
                    ...
                    '' as  email, '' as tin, '' as mobile, '' as contact, '' as fax, sum(qty-iss) as bal,'' as stockref 
              from (select '' as expiry, '' as posted,item.itemname,item.barcode,head.trno as trno,
                          ...
                          stock.disc as disc,item.itemid as itemid
                          from glhead as head
  ...
  public function report_PDF_LEDGER_Header($config, $data)
  {
    ...
    PDF::MultiCell(200, 0, (isset($data[0]['barcode']) ? $data[0]['barcode'] : ''), '', 'L', false);

    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(100, 0, "ITEM NAME: ", '', 'L', false, 0, '',  '');

    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(250, 0, (isset($data[0]['itemname']) ? $data[0]['itemname'] : ''), '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
  ...
  public function report_PDF_LEDGER($params, $data)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $arr_qty = $this->reporter->fixcolumn([$qty == '-' ? $qty : number_format($qty, 2)], '10', 0);
      $arr_iss = $this->reporter->fixcolumn([$iss == '-' ? $iss : number_format($iss, 2)], '10', 0);
    ...
    PDF::SetFont($font, '', 5);
    PDF::MultiCell(800, 0, "", 'B', 'C', false);

    PDF::SetFont($font, '', 5);
    PDF::MultiCell(800, 0, "", '', 'C', false);

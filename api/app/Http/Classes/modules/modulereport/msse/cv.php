...
class cv
{
  ...
  public function PDF_default_header($params, $data)
  {
    ...
    PDF::MultiCell(270, 5, "Docno #: ", '', 'R', false, 0, '',  '');
    PDF::MultiCell(130, 5, (isset($data[0]['docno']) ? $data[0]['docno'] : ''), '', 'R', false);


    PDF::SetFont($fontbold, '', 17);
    PDF::MultiCell(300, 5, '', '', 'L', false, 0);
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(270, 5, "", '', 'R', false, 0, '',  '');
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(130, 5, '', 'T', 'R', false);
    ...
    PDF::MultiCell(400, 5, '', 'T', 'L', false, 0, '',  '');
    PDF::MultiCell(20, 5, '', '', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(90, 5, '', '', 'R', false, 0);
    PDF::SetFont($font, '', $fontsize);
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(400, 5, (isset($data[0]['address']) ? $data[0]['address'] : ''), '', 'L', false, 0, '',  '');
    PDF::MultiCell(20, 5, '', '', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    ...
    PDF::MultiCell(170, 0, 'ACCT #', '', 'L', false, 0);
    PDF::MultiCell(170, 0, 'DATE', '', 'C', false, 0);
    PDF::MultiCell(185, 0, 'DEBIT' . ' ', '', 'R', false, 0);
    PDF::MultiCell(185, 0, 'CREDIT' . ' ', '', 'R', false);
  ...
  public function PDF_DEFAULT_CCVOUCHER_LAYOUT1($data, $params)
  {
    ...
    if (!empty($data)) {
      ...
      for ($i = 0; $i < count($data); $i++) {
        ...
        $arr_checkno = $this->reporter->fixcolumn([$checkno], '13', 0);
        ...
        $arr_dyourref = $this->reporter->fixcolumn([$dyourref], '13', 0);
        $arr_ref = $this->reporter->fixcolumn([$ref], '16', 0);
        $maxrow = $this->othersClass->getmaxcolumn([$arr_acno, $arr_pdate, $arr_debit, $arr_credit]);

        for ($r = 0; $r < $maxrow; $r++) {
     

          PDF::SetFont($fontbold, '', $fontsize);
          PDF::MultiCell(170, 5, (isset($arr_acno[$r]) ? $arr_acno[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(170, 5, ' ' . (isset($arr_pdate[$r]) ? $arr_pdate[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(185, 5, $arr_debit[$r] != 0 ? $arr_debit[$r] : '', '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(185, 5,  $arr_credit[$r] != 0 ? $arr_credit[$r] : '', '', 'R', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
        }
        $totaldb += $data[$i]['db'];
        $totalcr += $data[$i]['cr'];
    ...
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(330, 10, 'GRAND TOTAL : ', '', 'R', false, 0);
    PDF::MultiCell(185, 10, number_format($totaldb, $decimal), '', 'R', false, 0);
    PDF::MultiCell(185, 10, number_format($totalcr, $decimal), '', 'R', false, 0);

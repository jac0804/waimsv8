...
class ts
{
  ...
  public function report_default_query($filters)
  {
    ...
    client.addr as fromaddr, stock.loc, stock.loc2, head.ourref, stock.expiry
    from lahead as head 
  ...
  public function default_TS_header_PDF($params, $data)
  {
    ...
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(80, 0, "From Warehouse: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    ...
    PDF::MultiCell(0, 30, "", '', 'L');
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(80, 0, "To Warehouse: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
  ...
  public function default_TS_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $arr_qty = $this->reporter->fixcolumn([$qty], '14', 0);
        $arr_uom = $this->reporter->fixcolumn([$uom], '7', 0);
        $arr_cost = $this->reporter->fixcolumn([$loc], '15', 0);
        $arr_ext = $this->reporter->fixcolumn([$expiry], '15', 0);
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::SetFont($font, '', $fontsize);
          PDF::MultiCell(108, 15, ' ' . (isset($arr_barcode[$r]) ? $arr_barcode[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          ...
          PDF::MultiCell(70, 15, ' ' . (isset($arr_cost[$r]) ? $arr_cost[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(70, 15, ' ' . (isset($arr_ext[$r]) ? $arr_ext[$r] : ''), '', 'L', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
        }
    ...
    PDF::MultiCell(80, 0, number_format($totalqty, $decimalcurr), '', 'L', false, 0);
    PDF::MultiCell(440, 0, '', '', 'L', false, 0);
    PDF::MultiCell(70, 0, '', '', 'L', false);

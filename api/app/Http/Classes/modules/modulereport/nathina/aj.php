...
class aj
{
  ...
  public function report_default_query($filters)
  {
    ...
    stock.uom, stock.disc, stock.ext, stock.line,wh.client as swh, 
    wh.clientname as whname,stock.loc,stock.expiry,head.yourref,head.ourref
    from lahead as head 
  ...
  public function reportplotting($params, $data)
  {
    if ($params['params']['dataparams']['print'] == "default") {
      ...
      return $this->nathina_AJ_PDF($params, $data);
    }
  }

  public function nathina_AJ_header_PDF($params, $data)
  {
    ...
    PDF::MultiCell(120, 0, "WAREHOUSE", '', 'C', false, 0);
    PDF::MultiCell(80, 0, "LOCATION", '', 'C', false, 0);
    PDF::MultiCell(100, 0, "EXPIRY", '', 'C', false);

    PDF::SetFont($font, '', 5);
    PDF::MultiCell(700, 0, '', 'B');
  }

  public function nathina_AJ_PDF($params, $data)
  {
    ...
    $this->nathina_AJ_header_PDF($params, $data);
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $arr_barcode = $this->reporter->fixcolumn([$barcode], '17', 0);
        ...
        $arr_swh = $this->reporter->fixcolumn([$swh], '17', 0);
        $arr_loc = $this->reporter->fixcolumn([$loc], '14', 0);
        $arr_expiry = $this->reporter->fixcolumn([$expiry], '14', 0);
        

        $maxrow = $this->othersClass->getmaxcolumn([$arr_barcode, $arr_qty, $arr_uom, $arr_itemname, $arr_swh, $arr_loc, $arr_expiry]);
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(120, 15, ' ' . (isset($arr_swh[$r]) ? $arr_swh[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(80, 15, ' ' . (isset($arr_loc[$r]) ? $arr_loc[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(100, 15, ' ' . (isset($arr_expiry[$r]) ? $arr_expiry[$r] : ''), '', 'C', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
        }
    ...
    PDF::MultiCell(100, 0, number_format($totalqty, $decimalcurr), '', 'R', false, 0);
    PDF::MultiCell(405, 0, 'GRAND TOTAL : ', '', 'R', false, 0);
    PDF::MultiCell(125, 0, number_format($totalext, $decimalcurr), '', 'R', false);
  ...
  public function default_AJ_header_PDF($params, $data)
  {
    ...
    PDF::MultiCell(240, 0, "DESCRIPTION", '', 'L', false, 0);
    PDF::MultiCell(120, 0, "COST", '', 'R', false, 0);
    PDF::MultiCell(120, 0, "TOTAL", '', 'R', false);
  ...
  public function default_AJ_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $uom = $data[$i]['uom'];
        $itemname =  $data[$i]['itemname'];
        $cost = number_format($data[$i]['netamt'], $decimalcurr);
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(240, 15, ' ' . (isset($arr_itemname[$r]) ? $arr_itemname[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
    ...
    PDF::MultiCell(50, 0, number_format($totalqty, $decimalcurr), '', 'R', false, 0);

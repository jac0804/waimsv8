...
class ds
{
  ...
  public function report_default_query($filters)
  {
    ...
    detail.cr, detail.line,coa.alias,hc.acnoname as contraname, hc.acno as headacno,detail.checkno
    from ((lahead as head 
    ...
    where head.doc='ds' and md5(head.trno)='$trno' and left(coa.alias,2)='CB'
    union all
    select head.trno, date(head.dateid) as dateid, head.docno, 
    ...
    where head.doc='ds' and md5(head.trno)='$trno' and left(coa.alias,2)='CB' order by line";
  ...
  public function default_DS_header_PDF_orig($params, $data)
  {
  ...
  public function default_DS_PDF_ORIG($params, $data)
  {
    ...
    public function default_DS_header_PDF($params, $data)
  {
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(115, 0, "DOCNO#", '', 'L', false, 0, '',  '');
    PDF::MultiCell(15, 0, ":", '', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(590, 0, (isset($data[0]->docno) ? $data[0]->docno : ''), '', 'L', false, 1, '',  '');

  
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(115, 0, "BANK", '', 'L', false, 0, '',  '');
    PDF::MultiCell(15, 0, ":", '', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(590, 0, $data[0]->contraname, '', 'L', false, 1, '', '');

    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(115, 0, "BRANCH", '', 'L', false, 0, '',  '');
    PDF::MultiCell(15, 0, ":", '', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(590, 0, '', '', 'L', false, 1, '', '');

    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(115, 0, "TYPE OF CHECK", '', 'L', false, 0, '',  '');
    PDF::MultiCell(15, 0, ":", '', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(590, 0, '', '', 'L', false, 1, '', '');

    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(115, 0, "ACCT NAME", '', 'L', false, 0, '',  '');
    PDF::MultiCell(15, 0, ":", '', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(590, 0, $data[0]->contraname, '', 'L', false, 1, '', '');

    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(115, 0, "ACCT NO", '', 'L', false, 0, '',  '');
    PDF::MultiCell(15, 0, ":", '', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(590, 0,  $data[0]->headacno, '', 'L', false, 1, '', '');


    PDF::MultiCell(0, 0, "\n\n");

    PDF::SetFont($font, '', 5);
    PDF::MultiCell(720, 0, "", 'B', 'C', false, 1);
    ...
    PDF::SetFont($font, '', 5);
    PDF::MultiCell(420, 0, "", 'TR', 'C', false, 0);
    PDF::MultiCell(150, 0, "", 'TR', 'C', false, 0);
    PDF::MultiCell(150, 0, "", 'T', 'C', false, 1);

    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(420, 0, "BANK / BRANCH", 'R', 'C', false, 0);
    PDF::MultiCell(150, 0, "CHECK NUMBER", 'R', 'C', false, 0);
    PDF::MultiCell(150, 0, "CHECK AMOUNT", '', 'C', false, 1);

    PDF::SetFont($font, '', 5);
    PDF::MultiCell(420, 0, "", 'BR', 'C', false, 0);
    PDF::MultiCell(150, 0, "", 'BR', 'C', false, 0);
    PDF::MultiCell(150, 0, "", 'B', 'C', false, 1);

    PDF::SetFont($font, '', 2);
    PDF::MultiCell(720, 0, "", '', 'C', false, 1);
    PDF::SetFont($font, '', 5);
    PDF::MultiCell(720, 0, "", 'T', 'C', false, 1);
  }
   ...
   public function default_DS_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      ...
      $totalamt = $this->coreFunctions->datareader("
              select sum(db) as value from (
              select sum(d.db) as db from lahead as h 
                     left join ladetail as d on d.trno=h.trno
                     left join coa on coa.acnoid=d.acnoid
                    where h.trno='" . $trno . "' and left(coa.alias,2)='CB'
               union all
               select sum(d.db) as db from glhead as h 
                     left join gldetail as d on d.trno=h.trno
                     left join coa on coa.acnoid=d.acnoid
                     where h.trno='" . $trno . "' and left(coa.alias,2)='CB' ) as a");          
      ...
      for ($i = 0; $i < count($data); $i++) {
        ...
        $maxrow = $this->othersClass->getmaxcolumn([ $arr_checkno, $arr_acnoname, $arr_debit]);
        $no++;        // display counter

        for ($r = 0; $r < $maxrow; $r++) {
          PDF::SetFont($font, '', $fontsize);
          PDF::MultiCell(20, 0, ($r == 0 ? $no : '') .' .', '', 'L', false, 0, '', '', true, 1);
          PDF::MultiCell(400, 0, (isset($arr_acnoname[$r]) ? $arr_acnoname[$r] : ''), '', 'L', false, 0, '', '', true, 1);
          PDF::MultiCell(150, 0, (isset($arr_checkno[$r]) ? $arr_checkno[$r] : ''), '', 'L', false, 0, '', '', false, 1);
          PDF::MultiCell(150, 0, (isset($arr_debit[$r]) ? $arr_debit[$r] : ''), '', 'R', false, 1, '', '', false, 1);
        }
        ...
        if (intval($i) + 1 == $page) {
            ...
            PDF::SetFont($font, '', $fontsize);
            PDF::MultiCell(240, 0, 'DEPOSIT DATE', '', 'C', false, 0);
            PDF::MultiCell(240, 0, 'TOTAL CHECKS', '', 'C', false, 0);
            PDF::MultiCell(240, 0, 'TOTAL AMOUNT', '', 'C', false, 1);


            PDF::MultiCell(0, 0, "\n");

            PDF::SetFont($fontbold, '', $fontsize);
            PDF::MultiCell(240, 0, $depositdate, '', 'C', false, 0);
            PDF::MultiCell(240, 0, count($data), '', 'C', false, 0);
            PDF::MultiCell(240, 0, number_format($totalamt, $decimalcurr), '', 'C', false, 1);


            PDF::SetFont($font, '', 12);
            PDF::SetXY(40, 950);
            PDF::MultiCell(720, 0, date_format(date_create($current_timestamp), 'Y/m/d H:i:s') , '', 'L', false, 1);

            $this->default_DS_header_PDF($params, $data);
            $page += $count;
            $no = 0; // reset per page only
        }

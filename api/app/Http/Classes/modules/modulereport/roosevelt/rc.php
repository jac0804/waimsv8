...
class rc
{
    ...
    public function report_default_query($filters)
    {
        ...
        $query = "select detail.line, head.trno, date(head.dateid) as dateid, head.docno,ag.clientname as agent,
                        head.yourref,head.ourref,head.rem,detail.bank,detail.amount, detail.checkno as dcheckno,
                        detail.checkdate,detail.branch
                from rchead as head
    ...
    public function default_RC_PDF($params, $data)
    {
        ...
        if (!empty($data)) {
            for ($i = 0; $i < count($data); $i++) {
                ...
                $maxrow = $this->othersClass->getmaxcolumn([$arrbank, $arrbranch, $arrdcheckno, $arrdcheckdate, $arramount]);

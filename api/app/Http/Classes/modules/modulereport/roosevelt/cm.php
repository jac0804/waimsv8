...
class cm
{
  ...
  public function reportplotting($params, $data)
  {
    if ($params['params']['dataparams']['print'] == "default") {
      ...
      return $this->roosevelt_cm_PDF($params, $data);
    }
  }
  ...
  private function report_default_header($params, $data)
  {
    ...
    $str .= $this->reporter->col('NOTES', '50', null, false, $border, 'B', 'L', $font, '12', 'B', '30px', '8px');
  ...
  public function default_cm_layout($params, $data)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $str .= $this->reporter->col($data[$i]['remarks'], '50', null, false, $border, '', 'L', $font, $fontsize, '', '', '');
  ...
  public function default_cm_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $arr_remarks = $this->reporter->fixcolumn([$remarks], '30', 0);
        $arr_ext = $this->reporter->fixcolumn([$ext], '15', 0);

        $maxrow = $this->othersClass->getmaxcolumn([$arr_barcode, $arr_itemname, $arr_qty, $arr_uom, $arr_amt, $arr_remarks, $arr_ext]);
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(190, 15, ' ' . (isset($arr_itemname[$r]) ? $arr_itemname[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          ...
          PDF::MultiCell(120, 15, ' ' . (isset($arr_remarks[$r]) ? $arr_remarks[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
  ...
  public function roosevelt_cm_header_PDF($params, $data)
  {
    // var_dump($y);
    $center = $params['params']['center'];
    $username = $params['params']['user'];
    ...
    PDF::MultiCell(720, 0, 'Sales Credit Note', '', 'C', false, 1,  $x, $y + 105);
    ...
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(450, 0, 'DESCRIPTION', 'TB', 'L', false, 0);
  ...
  public function roosevelt_cm_PDF($params, $data)
  {
    $companyid = $params['params']['companyid'];
    $decimalcurr = $this->companysetup->getdecimal('currency', $params['params']);
    $decimalqty = $this->companysetup->getdecimal('qty', $params['params']);
    ...
    $this->roosevelt_cm_header_PDF($params, $data);
    PDF::SetFont($font, '', 5);
    PDF::MultiCell(720, 0, '', '');
  ...
  public function default_footer($params, $data)
  {
    ...
    PDF::MultiCell(90, 0,  number_format($totalext, 2), 'T', 'R', false, 1);

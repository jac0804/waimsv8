...
class gc
{
  ...
  public function report_default_query($filters)
  {
    ...
    $query = "
     select trno, dateid, docno, clientname, address,yourref, alias,acno,acnoname,client, sum(db) as db, sum(cr) as cr,rem,concat(acnoname,' -',ifnull(rem,'')) as particular
     from (
      ...
      where head.doc='" . $filters['params']['doc'] . "' and md5(head.trno)='$trno' and detail.cr<>0 
      ...
      where head.doc='" . $filters['params']['doc'] . "' and md5(head.trno)='$trno'  and detail.cr<>0 
      group by head.trno, head.dateid, head.docno, head.clientname, head.address, head.yourref, coa.alias, coa.acno,
      coa.acnoname, client.client, detail.ref,  detail.checkno,detail.rem  order by acnoname ) as a 
      group by  trno, dateid, docno, clientname, address,yourref, alias,acno,acnoname,client,rem 
      order by acnoname";
  ...
  public function reportplotting($params, $data)
  {
    if ($params['params']['dataparams']['print'] == "default") {
      ...
      return $this->roosevelt_gc_PDF($params, $data);
    }
  }
  ...
  public function default_GC_header_PDFc($params, $data)
  {
    ...
    if ($clnames > $maxChars) {
      PDF::SetFont($fontbold, '', $fontsize);
      PDF::MultiCell(555, 0, strtoupper($firstLine), '', 'L', false, 0,  $x + 176, $y + 85);
      $datenow = strtoupper((new DateTime($data[0]['dateid']))->format('F j, Y'));
      PDF::SetFont($font, '', $fontsize);
      PDF::MultiCell(145, 0, $datenow, '', 'L', false, 0,   $x + 578, $y + 85);
      $lineY = PDF::GetY();
      for ($i = 0; $i < $lineCount; $i++) {
        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(555, 0, strtoupper($remainingLines[$i]), '', 'L', false, 0,  $x + 176, $lineY + 13);
        PDF::MultiCell(145, 0, '', '', 'C', false, 1,  $x + 578, $lineY - 5);
        $lineY = PDF::GetY();
      }
    } else {
      PDF::SetFont($fontbold, '', $fontsize);
      PDF::MultiCell(555, 0, strtoupper($clnamesz), '', 'L', false, 0,  $x + 176, $y + 85);
      PDF::SetFont($font, '', $fontsize);
      $datenow = strtoupper((new DateTime($data[0]['dateid']))->format('F j, Y'));
      PDF::MultiCell(145, 0, $datenow, '', 'L', false, 0,   $x + 578, $y + 85);
      ...
      PDF::SetFont($font, '', $fontsize);
      PDF::MultiCell(720, 0, "", '', 'L', false, 1,  $x + 90, $y + 95);
    }
    ...
    if ($whadds > $maxCharss) {
      ...
      PDF::SetFont($font, '', $fontsize);
      PDF::MultiCell(480, 0, strtoupper($firstLines), '', 'L', false, 0,  $x + 100, $y + 110);
      PDF::SetFont($fontbold, '', $fontsize);
      PDF::MultiCell(170, 0, strtoupper($this->modulename), '', 'C', false, 0, $x + 565, $y + 130);
      $lineY = PDF::GetY();
      // var_dump($lineY);
      for ($i = 0; $i < $lineCount; $i++) {
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(480, 0,  strtoupper($remainingLiness[$i]), '', 'L', false, 0,  $x + 100, $lineY - 5);
        PDF::MultiCell(170, 0, '', '', 'C', false, 1, $x + 90, $lineY - 5);
        $lineY = PDF::GetY();
      }
    } else {
      PDF::SetFont($font, '', $fontsize);
      PDF::MultiCell(70, 0, "", '', 'L', false, 0, '',  '');
      PDF::SetFont($font, '', $fontsize);
      PDF::MultiCell(480, 0, strtoupper($whaddsz), '', 'L', false, 0,  $x + 100, $y + 110);
      PDF::SetFont($fontbold, '', $fontsize);
      PDF::MultiCell(170, 0, strtoupper($this->modulename), '', 'C', false, 1, $x + 565, $y + 130);

      PDF::SetFont($font, '', $fontsize);
      PDF::MultiCell(720, 0, "", '', 'L', false, 1,  $x + 90, $y + 135);
    }
    ...
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(720, 0, number_format($total, 2), '', 'L', false, 0,  $x + 525, $y + 148);
  ...
  public function default_GC_PDFc($params, $data)
  {
    ...
    if (!empty($data)) {
      ...
      for ($i = 0; $i < count($data); $i++) {
        ...
        $maxrow = $this->othersClass->getmaxcolumn([$arr_acnoname, $arr_debit, $arr_client]);

        for ($r = 0; $r < $maxrow; $r++) {
          PDF::SetFont($font, '', $fontsize);
          PDF::MultiCell(20, 0, '', '', 'L', false, 0, '', '', true, 1);
          PDF::MultiCell(600, 0, (isset($arr_acnoname[$r]) ? strtoupper($arr_acnoname[$r]) : ''), '', 'L', false, 0,  $x + 20, $y, true, 1);
          PDF::MultiCell(100, 0, (isset($arr_debit[$r]) ? $arr_debit[$r] : ''), '', 'R', false, 1, $x + 625, $y, true, 1);
          $y = PDF::GetY();
        }
    ...
    PDF::MultiCell(100, 0, number_format($totaldb, $decimalprice), '', 'R', false, 1,  $x + 625, $y + 146);
  ...
  public function default_GC_header_PDF($params, $data, $yy)
  {
    $center = $params['params']['center'];
    $username = $params['params']['user'];
    $companyid = $params['params']['companyid'];
    ...
    if ($whadds > $maxCharss) {
      ...
      for ($i = 0; $i < $lineCount; $i++) {
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(480, 0,  strtoupper($remainingLiness[$i]), '', 'L', false, 0,  $x + 45, $lineY - 5);
  ...
  public function default_GC_PDF($params, $data)
  {
    ...
    $this->default_GC_header_PDF($params, $data, $y = (float) 10.00125);
    PDF::SetFont($font, '', 5);
    PDF::MultiCell(720, 0, '', '');
    ...
    if (!empty($data)) {
      ...
      for ($i = 0; $i < count($data); $i++) {
        ...
        $credit = number_format($data[$i]['db'], $decimalcurr);
        $credit = $credit < 0 ? '-' : $credit;
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          if ($rowCount >= $page && $i < count($data) - 1) {
            $y = (float) 10.00125;
            $this->default_GC_header_PDF($params, $data, $y);
            // $y = PDF::GetY() + 35;
            $y = (float) 215.00125;
    ...
    PDF::MultiCell(200, 0, number_format($totaldb, $decimalprice), '', 'R', false, 1,  $x + 480, $y + 5);
  ...
  public function roosevelt_gc_header_PDF($params, $data)
  {
    // var_dump($y);
    $center = $params['params']['center'];
    $username = $params['params']['user'];
    ...
    $x = PDF::getX();
    PDF::SetFont($fontbold, '', 15);
    PDF::MultiCell(720, 0, 'Customer Credit Note', '', 'C', false, 1,  $x, $y + 105);
  ...
  public function roosevelt_gc_PDF($params, $data)
  {
    $companyid = $params['params']['companyid'];
    $decimalcurr = $this->companysetup->getdecimal('currency', $params['params']);
    ...
    $this->roosevelt_gc_header_PDF($params, $data);
    PDF::SetFont($font, '', 5);
    PDF::MultiCell(720, 0, '', '');
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $maxrow = $this->othersClass->getmaxcolumn([$arr_credit, $arr_particular]);
        for ($r = 0; $r < $maxrow; $r++) {
          PDF::SetFont($font, '', $fontsize);
          PDF::SetXY($x, $y);
          PDF::MultiCell(605, 15, ' ' . (isset($arr_particular[$r]) ? $arr_particular[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);

          PDF::MultiCell(115, 15, ' ' . (isset($arr_credit[$r]) ? $arr_credit[$r] : ''), '', 'R', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
          $y = PDF::getY();
          $rowCount++;
          if ($rowCount >= $page && $i < count($data) - 1) {
  ...
  public function default_footer($params, $data)
  {
    ...
    PDF::MultiCell(720, 0, 'Inclusion Series: CN000000001 To: CN999999999', '', 'L', false, 1, '', '');

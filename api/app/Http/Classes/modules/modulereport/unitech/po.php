...
class po
{
    ...
    {
        $query = "select date(head.dateid) as dateid, head.docno, client.client, client.clientname, head.address,head.terms,head.rem, item.partno, item.barcode,item.itemname, stock.rrqty as qty, stock.uom, stock.rrcost as netamt, stock.disc, stock.ext,m.model_name as model,item.sizeid,stock.qty as qtypcs,ifnull((select 1 from uom where itemid=stock.itemid and uom in ('PC','PCS','PIECE','PIECES') limit 1),0) as uompcs
        from pohead as head 
        ...
        head.address, head.terms,head.rem, item.partno, item.barcode,item.itemname, stock.rrqty as qty, stock.uom, stock.rrcost as netamt, stock.disc, stock.ext,m.model_name as model,item.sizeid,stock.qty as qtypcs,ifnull((select 1 from uom where itemid=stock.itemid and uom in ('PC','PCS','PIECE','PIECES') limit 1),0) as uompcs
    ...
    public function default_PO_header_PDF($params, $data)
    {
        ...
        PDF::MultiCell(90, 0, "BARCODE", 'TB', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', true);
        PDF::MultiCell(75, 0, "QTY", 'TB', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', true);
        PDF::MultiCell(50, 0, "UNIT", 'TB', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', true);
        PDF::MultiCell(90, 0, "TOTAL QTY (PCS)", 'TB', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', true);

        PDF::MultiCell(140, 0, "DESCRIPTION", 'TB', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', true);
        PDF::MultiCell(80, 0, "UNIT PRICE", 'TB', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', true);
        PDF::MultiCell(75, 0, "(+/-) %", 'TB', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', true);
        PDF::MultiCell(100, 0, "TOTAL", 'TB', 'R', false, 1, '',  '', true, 0, false, true, 0, 'M', true);
    }
    ...
    public function default_PO_PDF($params, $data)
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            if ($data[$i]['uompcs'] == 1) {
                $qtypcs = number_format($data[$i]['qtypcs'], 2);
            } else {
                $qtypcs = '-';
            }
            $uom = $data[$i]['uom'];
            ...
            $arr_qtypcs = $this->reporter->fixcolumn([$qtypcs], '10', 0);
            $arr_uom = $this->reporter->fixcolumn([$uom], '6', 0);
            $arr_amt = $this->reporter->fixcolumn([$amt], '13', 0);
            $arr_disc = $this->reporter->fixcolumn([$disc], '13', 0);
            $arr_ext = $this->reporter->fixcolumn([$ext], '15', 0);

            $maxrow = $this->othersClass->getmaxcolumn([$arr_barcode, $arr_itemname, $arr_qty, $arr_qtypcs, $arr_uom, $arr_amt, $arr_disc, $arr_ext]);

            for ($r = 0; $r < $maxrow; $r++) {
                PDF::SetFont($font, '', $fontsize);
                PDF::MultiCell(90, 0, ' ' . (isset($arr_barcode[$r]) ? $arr_barcode[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                PDF::MultiCell(75, 0, ' ' . (isset($arr_qty[$r]) ? $arr_qty[$r] : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                PDF::MultiCell(50, 0, ' ' . (isset($arr_uom[$r]) ? $arr_uom[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                PDF::MultiCell(90, 0, ' ' . (isset($arr_qtypcs[$r]) ? $arr_qtypcs[$r] : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                PDF::MultiCell(140, 0, ' ' . (isset($arr_itemname[$r]) ? $arr_itemname[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                PDF::MultiCell(80, 0, ' ' . (isset($arr_amt[$r]) ? $arr_amt[$r] : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                PDF::MultiCell(75, 0, ' ' . (isset($arr_disc[$r]) ? $arr_disc[$r] : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);

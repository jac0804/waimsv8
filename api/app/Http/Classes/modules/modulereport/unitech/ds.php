...
class ds
{
  ...
  public function report_default_query($filters)
  {
    ...
    coa.acnoname as clname, coa.acno,
    coa.acnoname, client.client, detail.ref,
    date(detail.postdate) as postdate, detail.db, 
    detail.cr, detail.line,coa.alias,hc.acnoname as contraname, client.clientname, detail.checkno $project
    from ((lahead as head 
    ...
    union all
    select head.trno, date(head.dateid) as dateid, head.docno, 
    hcoa.acnoname as clname, coa.acno,
    coa.acnoname, client.client, detail.ref,
     date(detail.postdate) as postdate, detail.db, 
     detail.cr, detail.line,coa.alias,hc.acnoname as contraname, client.clientname, detail.checkno $project
    from (((glhead as head 
  ...
  public function default_DS_header_PDF($params, $data)
  {
    ...
    PDF::MultiCell(110, 0, "CHECK #", '', 'L', false, 0);
    PDF::MultiCell(95, 0, "CLIENT NAME", '', 'L', false, 0);
    PDF::MultiCell(90, 0, "ACCOUNT NO.", '', 'L', false, 0);
    PDF::MultiCell(110, 0, "ACCOUNT NAME", '', 'C', false, 0);
    PDF::MultiCell(100, 0, "REFERENCE #", '', 'C', false, 0);
    PDF::MultiCell(75, 0, "DATE", '', 'C', false, 0);
    PDF::MultiCell(60, 0, "DEBIT", '', 'R', false, 0);
    PDF::MultiCell(60, 0, "CREDIT", '', 'R', false);
  ...
  public function default_DS_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      ...
      for ($i = 0; $i < count($data); $i++) {
        ...
        $arr_acnoname = $this->reporter->fixcolumn([$acnoname], '36', 0);
        ...
        $maxrow = $this->othersClass->getmaxcolumn([$arr_acno, $arr_acnoname, $arr_ref, $arr_postdate, $arr_debit, $arr_credit, $arr_client, $arr_checkno]);

        for ($r = 0; $r < $maxrow; $r++) {
          PDF::SetFont($font, '', $fontsize);
          PDF::MultiCell(110, 0, (isset($arr_checkno[$r]) ? $arr_checkno[$r] : ''), '', 'L', false, 0, '', '', true, 1);
          PDF::MultiCell(95, 0, (isset($arr_client[$r]) ? $arr_client[$r] : ''), '', 'L', false, 0, '', '', true, 1);
          PDF::MultiCell(90, 0, (isset($arr_acno[$r]) ? $arr_acno[$r] : ''), '', 'L', false, 0, '', '', true, 1);
          PDF::MultiCell(110, 0, (isset($arr_acnoname[$r]) ? $arr_acnoname[$r] : ''), '', 'L', false, 0, '', '', false, 1);
          ...
          PDF::MultiCell(60, 0, (isset($arr_debit[$r]) ? $arr_debit[$r] : ''), '', 'R', false, 0, '', '', false, 1);
          PDF::MultiCell(60, 0, (isset($arr_credit[$r]) ? $arr_credit[$r] : ''), '', 'R', false, 1, '', '', false, 1);
        }
    ...
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(540, 0, 'GRAND TOTAL: ', '', 'R', false, 0);
    PDF::MultiCell(80, 0, number_format($totaldb, $decimalprice), '', 'R', false, 0);

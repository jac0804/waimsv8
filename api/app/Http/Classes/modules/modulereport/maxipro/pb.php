...
class pb
{
  ...
  public function default_query($config)
  {
    ...
    $query = "select head.trno, date(head.dateid) as dateid, head.docno, head.clientname, 
        ...
        where head.doc='PB' and head.trno= " . $trno . " order by db desc";
    ...
    switch ($bill) {
      ...
      case '3':
        $query = "select trno, dateid, dco, docno, clientname, address,
          ...
          sum(ewt) as ewt,
          sum(ar) as ar,
          sum(totalamt) as totalamt,clientid
          from (
          ...
          tcp.tcp,tcp.ocp,client.clientid
          from lahead as head
          left join ladetail as detail on detail.trno=head.trno
          left join coa on coa.acnoid = detail.acnoid
          left join client on client.client=detail.client
          ...
          tcp.tcp,tcp.ocp,coa.alias,detail.db,client.clientid
          union all
          select head.trno, date(head.dateid) as dateid,left(head.docno,2) as dco, head.docno, head.clientname,
          head.address, head.yourref,client.client, head.rem,
          date(detail.postdate) as postdate,head.terms,proj.code as projcode,
          ...
          date(detail.postdate),
          head.terms,proj.code,proj.name, 
          tcp.tcp,tcp.ocp,coa.alias,detail.db,head.clientid) as x
          group by trno, dateid, dco, docno, clientname, address,
          yourref, client, rem, postdate, terms, projcode, projname, tcp, ocp,clientid
        ";
        break;
      case 2:
        $query = "select trno, dateid, dco, docno, clientname, address,
        ...
        break;
      case '4':
        $query = "select trno,docnum,docno,dateid,projectid,projcode,projname,tcp,ocp,ceffect,cdue,subproject,refx,
                         conduration,yourref,ourref,rem2,orderno,rem,dollarprice
                  from (select head.trno,right(head.docno,13) as docnum,head.docno,head.dateid,head.projectid,
                               ...
                               head.subproject,detail.refx,pm.conduration,head.yourref,
                               head.ourref,head.rem2,head.orderno,head.rem,pm.dollarprice
                        from lahead as head
                        left join ladetail as detail on detail.trno=head.trno
                        left join projectmasterfile as proj on proj.line= head.projectid
                        left join (select projectid,tcp,dateid,due,ocp,conduration,dollarprice from pmhead
                                  union all
                                  select projectid,tcp,dateid,due,ocp,conduration,dollarprice from hpmhead as pmhead) as pm on pm.projectid = head.projectid
                        ...
                        select head.trno,right(head.docno,13) as docnum,head.docno,head.dateid,head.projectid,proj.code as projcode, proj.name as projname,
                               ...
                               head.rem2,head.orderno,head.rem,pm.dollarprice
                        from glhead as head
                        left join gldetail as detail on detail.trno=head.trno
                        left join projectmasterfile as proj on proj.line= head.projectid
                        left join (select projectid,tcp,dateid,due,ocp,conduration,dollarprice from pmhead
                    ...
                    group by refx,trno,docnum,docno,dateid,projectid,projcode,projname,tcp,ceffect,cdue,subproject,ocp,conduration,yourref,ourref,rem2,orderno,rem,dollarprice";
        break;
  ...
  public function reportplotting($config, $data)
  {
    ...
    switch ($bill) {
      ...
      case '3':
        ...
        break;
      case '4':
        if ($data[0]->ocp == 0 && $data[0]->tcp == 0) {
          return $this->notallowtoprint($config, "Input Contract Price in Project Management module.");
        } else {
          return $this->billing_invoice_summary_new_PDF($config, $data);
        }
  ...
  //old header layout (Billing Summary option)
  //start
  public function billing_invoice_PDF_SUMM_HEADER($params, $data)
  {
    ...
    PDF::MultiCell(580, 0, 'MAXIPRO DEVELOPMENT CORPORATION', '', 'L', false);
  ...
  public function billing_invoice_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      ...
      PDF::MultiCell(475, 0, $data[0]->projname . "\n" . $data[0]->projcode, '', 'L', false, 0, 118, 260);
      ...
      PDF::SetFont($fontitalic, '', $fontsize);
      PDF::MultiCell(250, 0, '', '', 'L', false, 0);
      if ($data[0]->clientid == 2584) {
        PDF::MultiCell(100, 0, '10% Recoupment', '', 'L', false, 0, 290, 463);
      } else {
  ...
  //old layout [Billing Summary option]
  //start
  public function billing_invoice_summary_PDF($params, $data)
  {
    ...
    for ($k = 0; $k < count($spresult); $k++) {
      $arr_subproject = $this->reporter->fixcolumn([$spresult[$k]['subproject']], '70', 0);
      $maxrow = count($arr_subproject);

      PDF::SetFont($fontbold, '', 1);
      PDF::SetFillColor(220, 255, 255);
      PDF::MultiCell(360, 0, '', 'TLR', 'L', 1, 0); //360
      PDF::MultiCell(140, 0, '', 'TLR', 'C', 1, 0);
      PDF::MultiCell(90, 0, '', 'TLR', 'C', 1, 0);
      PDF::MultiCell(110, 0, '', 'TLR', 'C', 1, 0);
      PDF::MultiCell(60, 0, '', 'TLR', 'C', 1, 0);
      PDF::MultiCell(83, 0, '', 'TLR', 'C', 1, 0);
      PDF::MultiCell(60, 0, '', 'TLR', 'C', 1, 0);
      PDF::MultiCell(83, 0, '', 'TLR', 'C', 1, 0);
      PDF::MultiCell(60, 0, '', 'TLR', 'C', 1, 0);
      PDF::MultiCell(84, 0, '', 'TLR', 'C', 1);
      ...
      for ($r = 0; $r < $maxrow; $r++) {
        ...
        PDF::SetFont($fontbold, '', 10);
        PDF::SetFillColor(220, 255, 255);
        PDF::MultiCell(360, 0, ' ' . $a .  (isset($arr_subproject[$r]) ? $arr_subproject[$r] : ''), 'LR', 'L', 1, 0); //360
        PDF::MultiCell(140, 0, '', 'LR', 'C', 1, 0);
        PDF::MultiCell(90, 0, '', 'LR', 'C', 1, 0);
        PDF::MultiCell(110, 0, '', 'LR', 'C', 1, 0);
        PDF::MultiCell(60, 0, '', 'LR', 'C', 1, 0);
        PDF::MultiCell(83, 0, '', 'LR', 'C', 1, 0);
        PDF::MultiCell(60, 0, '', 'LR', 'C', 1, 0);
        PDF::MultiCell(83, 0, '', 'LR', 'C', 1, 0);
        PDF::MultiCell(60, 0, '', 'LR', 'C', 1, 0);
        PDF::MultiCell(84, 0, '', 'LR', 'C', 1);
      }

      PDF::SetFont($fontbold, '', 1);
      PDF::SetFillColor(220, 255, 255);
      PDF::MultiCell(360, 0, '', 'LRB', 'L', 1, 0); //360
      PDF::MultiCell(140, 0, '', 'LRB', 'C', 1, 0);
      PDF::MultiCell(90, 0, '', 'LRB', 'C', 1, 0);
      PDF::MultiCell(110, 0, '', 'LRB', 'C', 1, 0);
      PDF::MultiCell(60, 0, '', 'LRB', 'C', 1, 0);
      PDF::MultiCell(83, 0, '', 'LRB', 'C', 1, 0);
      PDF::MultiCell(60, 0, '', 'LRB', 'C', 1, 0);
      PDF::MultiCell(83, 0, '', 'LRB', 'C', 1, 0);
      PDF::MultiCell(60, 0, '', 'LRB', 'C', 1, 0);
      PDF::MultiCell(84, 0, '', 'LRB', 'C', 1);

      $subprojid = $spresult[$k]['subprojid'];

      //getting stage & activity for each subproject
      $substage = "select distinct head.projectid,a.line as activity,a.substage,s.description,s.line as stage,s.stage as stagename from hbahead as head
      ...
      for ($i = 0; $i < count($ssresult); $i++) {
        ...
        $projid =  $ssresult[$i]['projectid'];
        $act =   $ssresult[$i]['activity'];

        $works = "select subactid,headsubact,sum(prrqty) as prrqty, sum(prrcost) as prrcost, sum(pext) as pext, 
                         0 as prevrrqty, 0 as prevext, sum(nowrrqty) as nowrrqty,sum(nowext) as nowext, task,puom 
                  from (select head.projectid,psub.rrqty as prrqty, psub.rrcost as prrcost, psub.ext as pext, psub.uom as puom,
                        stock.activity,stock.subactivity as headsubact,sum(stock.rrqty) as nowrrqty,
                        sum(stock.ext) as nowext,sp.subproject,s.stage,s.description,a.substage,sa.subactivity,
                        concat(sa.description,'(',psub.subactid,')') as task,psub.subactid
                        ...
                        left join psubactivity as psub on psub.line=sa.line and psub.trno = pm.pmtrno and psub.subproject = $subprojid 
                        and psub.stage = $stageid and psub.substage = stock.activity and psub.subactid = stock.subactid                       
                        where pbtrno = $trno and head.subproject=$subprojid and stock.stage=$stageid and psub.rrqty <> 0  and stock.activity = $act
                        and psub.rrcost <> 0 and psub.ext <> 0
                        ...
                        psub.rrqty,psub.rrcost,psub.ext,stock.rrcost,stock.subactivity,psub.subactid
                        order by a.substage,sa.subactivity) as x
                         group by puom, task,headsubact,subactid order by task";

        $worksresult = json_decode(json_encode($this->coreFunctions->opentable($works)), true);

        for ($e = 0; $e < count($worksresult); $e++) {
          $subact = $worksresult[$e]['headsubact'];
          $subactid = $worksresult[$e]['subactid'];

          $prevrrqty  = $this->coreFunctions->datareader("select sum(hs.rrqty) as value from hbahead as hb left join hbastock as hs on hs.trno = hb.trno
          where hb.pbtrno <  $trno and hb.projectid = $projid and hb.subproject = $subprojid 
          and  hs.stage = $stageid and hs.activity = $act and hs.subactivity=$subact and hs.subactid = '" . $subactid . "'");


          $prevext  = $this->coreFunctions->datareader("select sum(hs.rrqty*hs.rrcost) as value from hbahead as hb left join hbastock as hs on hs.trno = hb.trno
          where hb.pbtrno <  $trno and hb.projectid = $projid and hb.subproject = $subprojid 
          and  hs.stage = $stageid and hs.activity = $act and hs.subactivity=$subact and hs.subactid = '" . $subactid . "'");

          $todateqty = $prevrrqty + $worksresult[$e]['nowrrqty'];
          $todateext = $prevext + $worksresult[$e]['nowext'];
          $totalorigprice += $worksresult[$e]['pext'];
          $totalprevamt += $prevext;
          ...
          $prevrrqty = number_format($prevrrqty, 2) == 0.00 ? '-' : number_format($prevrrqty, 2);
          $prevext = number_format($prevext, 2) == 0.00 ? '-' : number_format($prevext, 2);
          ...
          $todateqty = number_format($todateqty, 2);
          $todateext = number_format($todateext, 2);
          ...
          $maxrow = $this->othersClass->getmaxcolumn([
            $countarrtask, $countarrqty, $countarruom, $countarrcost, $countarrext,
            $countarrnowqty, $countarrnowext, $countarrtoqty, $countarrtoext, $countarrprevqty, $countarrprevext
          ]);
          ...
          for ($b = 0; $b < $maxrow; $b++) {
            ...
            PDF::SetFont($font, '', 9);
            PDF::setCellPadding($left, $top, $right, $bottom);
            PDF::MultiCell(60, 0, $i + 1 . '.' . $r . '.' . $s, 'LR', 'C', false, 0);
  ...
  //new layout Billing Summary option
  public function billing_invoice_PDF_SUMM_HEADER_new($params, $data)
  {
    $center = $params['params']['center'];
    $username = $params['params']['user'];
    ...
    // if ($data[0]->ocp == 0 && $data[0]->tcp == 0) {
    //   return $this->billing_invoice_summary_blank_PDF($params, $data);
    // }
    ...
    PDF::MultiCell(50, 15, "", '', 'C', false, 0);
    PDF::MultiCell(900, 0, "SUMMARY OF ACCOMPLISHMENT", '', 'C', false, 0);
    PDF::MultiCell(200, 15, "", '', 'C', false);

    PDF::SetFont($fontbold, '', $fontsize11);
    $dcno = $data[0]->yourref;
    PDF::MultiCell(50, 15, "", '', 'C', false, 0);
    PDF::MultiCell(900, 15, "PROGRESS BILLING NO. " . $dcno, '', 'C', false, 0);
    PDF::MultiCell(200, 15, "", '', 'C', false);
    ...
    PDF::SetFont($font, '', $fontsize11);
    PDF::MultiCell(50, 15, "", '', 'C', false, 0);
    PDF::MultiCell(900, 15, "Erection/ Installation/ Civil Works/ Testing/ Commisioning/ Trainings", '', 'C', false, 0, 90, 70);
    PDF::SetFont($fontbold, '', $fontsize11);
    PDF::MultiCell(100, 15, "Total %", '', 'C', false, 0, '900', '70');
    PDF::MultiCell(100, 15, "", '', 'C', false);

    PDF::SetFont($fontbold, '', $fontsize11);
    PDF::MultiCell(50, 15, "", '', 'C', false, 0);
    PDF::MultiCell(900, 15, "ONSHORE PORTION (PHP)", '', 'C', false, 0);
    PDF::MultiCell(100, 15, "Accomplishment", '', 'C', false, 0, '900', '90');
    PDF::MultiCell(100, 15, "", '', 'C', false);
    ...
    PDF::MultiCell(20, 0, "I.", '', 'L', false, 0);
    PDF::MultiCell(100, 0, "Project: ", '', 'L', false, 0);
    PDF::MultiCell(590, 0, $data[0]->projname . ' - ' . $data[0]->projcode, '', 'L', false, 0);
    ...
    PDF::MultiCell(20, 0, "", '', 'L', false, 0);
    PDF::MultiCell(100, 0, "", '', 'L', false, 0);
    PDF::MultiCell(590, 0, '', '', 'L', false, 0);
    PDF::MultiCell(220, 0, 'IV. Original Contract: ', '', 'L', false, 0);
    PDF::MultiCell(120, 0, 'VI. Accomplishment: ', '', 'R', false, 0);
    PDF::MultiCell(100, 0, '', '', 'R', false);


    PDF::MultiCell(20, 0, "II.", '', 'L', false, 0);
    PDF::MultiCell(100, 0, "Contract: ", '', 'L', false, 0);
    PDF::MultiCell(590, 0, 'FUJIAN ELECTRIC POWER ENGINEERING COMPANY (MAIN CONTRACTOR)', '', 'L', false, 0);
    ...
    PDF::MultiCell(20, 0, "", '', 'L', false, 0);
    PDF::MultiCell(100, 0, "", '', 'L', false, 0);
    PDF::MultiCell(590, 0, 'MAXIPRO DEVELOPMENT CORPORATION (LCOAL CONTRACTOR)', '', 'L', false, 0);
    PDF::MultiCell(40, 0, 'USD', '', 'L', false, 0);
    PDF::SetFont($font, '', $fontsize10);
    PDF::MultiCell(180, 0, number_format($data[0]->dollarprice, 2), '', 'L', false, 0);


    PDF::SetFont($fontbold, '', $fontsize10);
    PDF::MultiCell(120, 0, 'Previous', '', 'R', false, 0, 970, 170);
    PDF::MultiCell(100, 0, '', '', 'R', false);


    PDF::MultiCell(20, 0, "III.", '', 'L', false, 0);
    PDF::MultiCell(100, 0, "Contract Period: ", '', 'L', false, 0);
    PDF::SetFont($font, '', $fontsize10);
    PDF::MultiCell(590, 0, 'EFFECTIVITY: ' . date('M d, Y', strtotime($data[0]->ceffect)), '', 'L', false, 0);
    PDF::SetFont($fontbold, '', $fontsize10);
    PDF::MultiCell(40, 0, 'PHP', '', 'L', false, 0);
    PDF::SetFont($font, '', $fontsize10);
    if ($data[0]->ocp == '0.00' || $data[0]->ocp == '') {
      PDF::MultiCell(180, 0, number_format($data[0]->tcp, 2), '', 'L', false, 0);
    } else {
      PDF::MultiCell(180, 0, number_format($data[0]->ocp, 2), '', 'L', false, 0);
    }

    PDF::SetFont($fontbold, '', $fontsize10);
    PDF::MultiCell(120, 0, 'This Period', '', 'R', false, 0, 970, 185);
    PDF::MultiCell(100, 0, '', '', 'R', false);

    PDF::MultiCell(20, 0, "", '', 'L', false, 0);
    PDF::MultiCell(100, 0, "", '', 'L', false, 0);
    PDF::SetFont($font, '', $fontsize10);
    PDF::MultiCell(590, 0, 'EXPIRY: ' . date('M d, Y', strtotime($data[0]->cdue)), '', 'L', false, 0);
    PDF::MultiCell(40, 0, '', '', 'L', false, 0);
    PDF::MultiCell(180, 0, '', '', 'L', false, 0);
    PDF::SetFont($fontbold, '', $fontsize10);
    PDF::MultiCell(120, 0, 'To Date', '', 'R', false, 0, 970, 200);
    PDF::MultiCell(100, 0, '', '', 'R', false);
    ...
    PDF::MultiCell(100, 0, '', '', 'L', false, 0);
    PDF::SetFont($fontbold, '', $fontsize10);
    PDF::MultiCell(100, 0, 'V. Period Covered: ', '', 'L', false, 0);
    PDF::SetFont($font, '', $fontsize10);
    PDF::MultiCell(200, 0, $data[0]->orderno, '', 'L', false);

    PDF::MultiCell(0, 0, "\n");

    PDF::SetFont($fontbold, '', 9);
    PDF::MultiCell(60, 15, '', 'TL', 'C', false, 0);
    PDF::SetFont($fontbold, '', 10);
    PDF::MultiCell(300, 15, '', 'TL', 'C', false, 0);
    PDF::MultiCell(80, 15, '', 'TL', 'C', false, 0);
    PDF::MultiCell(50, 15, '', 'TL', 'C', false, 0);
    PDF::MultiCell(100, 15, 'UNIT PRICE', 'TLB', 'C', false, 0);
    PDF::MultiCell(110, 15, 'TOTAL PRICE', 'TLB', 'C', false, 0);
    PDF::MultiCell(430, 15, 'ACCOMPLISHMENT', 'TLRB', 'C', false);

    PDF::SetFont($fontbold, '', 9);
    PDF::MultiCell(60, 15, '', 'L', 'C', false, 0);
    PDF::SetFont($fontbold, '', 10);
    PDF::MultiCell(300, 15, '', 'L', 'C', false, 0);
    PDF::MultiCell(80, 15, '', 'L', 'C', false, 0);
    PDF::MultiCell(50, 15, '', 'L', 'C', false, 0);
    PDF::SetFont($fontbold, '', 9);
    PDF::MultiCell(100, 15, 'Erection/Installation/', 'L', 'C', false, 0);
    PDF::MultiCell(110, 15, 'Erection/Installation/', 'L', 'C', false, 0);
    PDF::SetFont($fontbold, '', 10);
    PDF::MultiCell(143, 15, '', 'L', 'C', false, 0);
    PDF::MultiCell(143, 15, '', 'L', 'C', false, 0);
    PDF::MultiCell(144, 15, '', 'LR', 'C', false);
    ...
    PDF::MultiCell(50, 15, '', 'L', 'C', false, 0);
    PDF::SetFont($fontbold, '', 9);
    PDF::MultiCell(100, 15, 'Civil & Other Works/', 'L', 'C', false, 0);
    PDF::MultiCell(110, 15, 'Civil & Other Works/', 'L', 'C', false, 0);
    PDF::SetFont($fontbold, '', 10);
    PDF::MultiCell(143, 15, 'PREVIOUS', 'L', 'C', false, 0);
    PDF::MultiCell(143, 15, 'THIS PERIOD', 'L', 'C', false, 0);
    PDF::MultiCell(144, 15, 'TO DATE', 'LR', 'C', false);

    PDF::SetFont($fontbold, '', 9);
    PDF::MultiCell(60, 15, 'ITEM NO.', 'L', 'C', false, 0);
    PDF::SetFont($fontbold, '', 10);
    PDF::MultiCell(300, 15, 'DESCRIPTION', 'L', 'C', false, 0);
    PDF::MultiCell(80, 15, 'QTY', 'L', 'C', false, 0);
    PDF::MultiCell(50, 15, 'UNIT', 'L', 'C', false, 0);
    PDF::SetFont($fontbold, '', 9);
    PDF::MultiCell(100, 15, 'Testing/', 'L', 'C', false, 0);
    PDF::MultiCell(110, 15, 'Testing/', 'L', 'C', false, 0);
    PDF::SetFont($fontbold, '', 10);
    PDF::MultiCell(143, 15, '', 'L', 'C', false, 0);
    PDF::MultiCell(143, 15, '', 'L', 'C', false, 0);
    PDF::MultiCell(144, 15, '', 'LR', 'C', false);
    ...
    PDF::MultiCell(50, 15, '', 'L', 'C', false, 0);
    PDF::SetFont($fontbold, '', 9);
    PDF::MultiCell(100, 15, 'Commissioning/', 'L', 'C', false, 0);
    PDF::MultiCell(110, 15, 'Commissioning/', 'L', 'C', false, 0);
    PDF::SetFont($fontbold, '', 10);
    PDF::MultiCell(60, 15, '', 'TL', 'C', false, 0);
    PDF::MultiCell(83, 15, '', 'TL', 'C', false, 0);
    PDF::MultiCell(60, 15, '', 'TL', 'C', false, 0);
    PDF::MultiCell(83, 15, '', 'TL', 'C', false, 0);
    PDF::MultiCell(60, 15, '', 'TL', 'C', false, 0);
    PDF::MultiCell(84, 15, '', 'TLR', 'C', false);
    ...
    PDF::MultiCell(50, 15, '', 'L', 'C', false, 0);
    PDF::SetFont($fontbold, '', 9);
    PDF::MultiCell(100, 15, 'Trainings', 'L', 'C', false, 0);
    PDF::MultiCell(110, 15, 'Trainings', 'L', 'C', false, 0);
    PDF::SetFont($fontbold, '', 10);
    PDF::MultiCell(60, 15, 'QTY', 'L', 'C', false, 0);
    PDF::MultiCell(83, 15, 'AMOUNT', 'L', 'C', false, 0);
    PDF::MultiCell(60, 15, 'QTY', 'L', 'C', false, 0);
    PDF::MultiCell(83, 15, 'AMOUNT', 'L', 'C', false, 0);
    PDF::MultiCell(60, 15, 'QTY', 'L', 'C', false, 0);
    PDF::MultiCell(84, 15, 'AMOUNT', 'LR', 'C', false);

    PDF::SetFont($fontbold, '', 9);
    PDF::MultiCell(60, 15, '', 'LB', 'C', false, 0);
    PDF::SetFont($fontbold, '', 10);
    PDF::MultiCell(300, 15, '', 'LB', 'C', false, 0);
    PDF::MultiCell(80, 15, '', 'LB', 'C', false, 0);
    PDF::MultiCell(50, 15, '', 'LB', 'C', false, 0);
    PDF::MultiCell(100, 15, 'PHP', 'TLB', 'C', false, 0);
    PDF::MultiCell(110, 15, 'PHP', 'TLB', 'C', false, 0);
    PDF::SetFont($fontbold, '', 10);
    PDF::MultiCell(60, 15, '', 'LB', 'C', false, 0);
    PDF::MultiCell(83, 15, '(PHP)', 'LB', 'C', false, 0);
    PDF::MultiCell(60, 15, '', 'LB', 'C', false, 0);
    PDF::MultiCell(83, 15, '(PHP)', 'LB', 'C', false, 0);
    PDF::MultiCell(60, 15, '', 'LB', 'C', false, 0);
    PDF::MultiCell(84, 15, '(PHP)', 'LRB', 'C', false);
  }

  //New Layout for Billing Summary option
  public function billing_invoice_summary_new_PDF($params, $data)
  {
    $decimalcurr = $this->companysetup->getdecimal('currency', $params['params']);
    $decimalqty = $this->companysetup->getdecimal('qty', $params['params']);
    $decimalprice = $this->companysetup->getdecimal('price', $params['params']);
    ...
    $this->billing_invoice_PDF_SUMM_HEADER_new($params, $data);
    ...
    for ($k = 0; $k < count($spresult); $k++) {
      ...
      PDF::SetFillColor(220, 255, 255);
      PDF::MultiCell(360, 0, '', 'TLR', 'L', 1, 0); //360
      PDF::MultiCell(130, 0, '', 'TLR', 'C', 1, 0);
      PDF::MultiCell(100, 0, '', 'TLR', 'C', 1, 0);
      PDF::MultiCell(110, 0, '', 'TLR', 'C', 1, 0);
      PDF::MultiCell(60, 0, '', 'TLR', 'C', 1, 0);
      PDF::MultiCell(83, 0, '', 'TLR', 'C', 1, 0);
      PDF::MultiCell(60, 0, '', 'TLR', 'C', 1, 0);
      ...
      for ($r = 0; $r < $maxrow; $r++) {
        ...
        PDF::SetFillColor(220, 255, 255);
        PDF::MultiCell(360, 0, ' ' . $a .  (isset($arr_subproject[$r]) ? $arr_subproject[$r] : ''), 'LR', 'L', 1, 0); //360
        PDF::MultiCell(130, 0, '', 'LR', 'C', 1, 0);
        PDF::MultiCell(100, 0, '', 'LR', 'C', 1, 0);
        PDF::MultiCell(110, 0, '', 'LR', 'C', 1, 0);
        PDF::MultiCell(60, 0, '', 'LR', 'C', 1, 0);
        PDF::MultiCell(83, 0, '', 'LR', 'C', 1, 0);
        PDF::MultiCell(60, 0, '', 'LR', 'C', 1, 0);
      ...
      PDF::SetFillColor(220, 255, 255);
      PDF::MultiCell(360, 0, '', 'LRB', 'L', 1, 0); //360
      PDF::MultiCell(130, 0, '', 'LRB', 'C', 1, 0);
      PDF::MultiCell(100, 0, '', 'LRB', 'C', 1, 0);
      PDF::MultiCell(110, 0, '', 'LRB', 'C', 1, 0);
      PDF::MultiCell(60, 0, '', 'LRB', 'C', 1, 0);
      PDF::MultiCell(83, 0, '', 'LRB', 'C', 1, 0);
      PDF::MultiCell(60, 0, '', 'LRB', 'C', 1, 0);
      ...
      for ($i = 0; $i < count($ssresult); $i++) {
        ...
        $ssubstage = $this->reporter->fixcolumn([$ssresult[$i]['substage']], '50', 0);
        $countarrs = count($ssubstage);
        $maxrow2 = $countarrs;
        $ctr = 0;
        $sssubstage = '';
        ...
        for ($a = 0; $a < $maxrow2; $a++) {
          ...
          PDF::SetFont($fontbold, '', 10);
          PDF::MultiCell(60, 15, $ctr <> 0 ? $ctr : '', 'LR', 'C', false, 0);
          PDF::MultiCell(300, 15, $sssubstage, 'LR', 'L', false, 0);
          PDF::MultiCell(130, 15, '', 'LR', 'C', false, 0);
          PDF::MultiCell(100, 15, '', 'LR', 'C', false, 0);
          PDF::MultiCell(110, 15, '', 'LR', 'C', false, 0);
          PDF::MultiCell(60, 15, '', 'LR', 'C', false, 0);
          PDF::MultiCell(83, 15, '', 'LR', 'C', false, 0);
          PDF::MultiCell(60, 15, '', 'LR', 'C', false, 0);
          PDF::MultiCell(83, 15, '', 'LR', 'C', false, 0);
          PDF::MultiCell(60, 15, '', 'LR', 'C', false, 0);
          PDF::MultiCell(84, 15, '', 'LR', 'C', false);
        }

        PDF::SetFont($fontbold, '', 1);
        PDF::MultiCell(60, 0, '', 'B', 'C', false, 0);
        PDF::MultiCell(300, 0, '', 'B', 'L', false, 0);
        PDF::MultiCell(130, 0, '', 'B', 'C', false, 0);
        PDF::MultiCell(100, 0, '', 'B', 'C', false, 0);
        PDF::MultiCell(110, 0, '', 'B', 'C', false, 0);
        PDF::MultiCell(60, 0, '', 'B', 'C', false, 0);
        PDF::MultiCell(83, 0, '', 'B', 'C', false, 0);
        PDF::MultiCell(60, 0, '', 'B', 'C', false, 0);
        PDF::MultiCell(83, 0, '', 'B', 'C', false, 0);
        PDF::MultiCell(60, 0, '', 'B', 'C', false, 0);
        PDF::MultiCell(84, 0, '', 'B', 'C', false);
        ...
        for ($m = 0; $m < $maxrow; $m++) {
          ...
          PDF::MultiCell(60, 0, '', 'LR', 'C', false, 0);
          PDF::MultiCell(300, 0, isset($wdescname[$m]) ? ' ' . $wdescname[$m] : '', 'LR', 'L', false, 0);
          PDF::MultiCell(130, 0, '', 'LR', 'C', false, 0);
          PDF::MultiCell(100, 0, '', 'LR', 'C', false, 0);
          PDF::MultiCell(110, 0, '', 'LR', 'C', false, 0);
          PDF::MultiCell(60, 0, '', 'LR', 'C', false, 0);
          PDF::MultiCell(83, 0, '', 'LR', 'C', false, 0);
          PDF::MultiCell(60, 0, '', 'LR', 'C', false, 0);
        ...
        PDF::MultiCell(60, 0, '', 'LRB', 'C', false, 0);
        PDF::MultiCell(300, 0, '', 'LRB', 'L', false, 0);
        PDF::MultiCell(130, 0, '', 'LRB', 'C', false, 0);
        ...
        for (
          ...
          for ($b = 0; $b < $maxrow; $b++) {
            ...
            PDF::MultiCell(80, 0, isset($wprrqty[$b]) ? ' ' . $wprrqty[$b] : '', 'L', 'R', false, 0);
            PDF::MultiCell(50, 0, isset($wpuom[$b]) ? ' ' . $wpuom[$b] : '', 'R', 'L', false, 0);

            PDF::MultiCell(100, 0, isset($wprrcost[$b]) ? ' ' . $wprrcost[$b] : '', 'LR', 'R', false, 0);
            PDF::MultiCell(110, 0, isset($wpext[$b]) ? ' ' . $wpext[$b] : '', 'LR', 'R', false, 0);
            PDF::MultiCell(60, 0, isset($wprevrrqty[$b]) ? ' ' . $wprevrrqty[$b] : '', 'LR', 'R', false, 0);
            PDF::MultiCell(83, 0, isset($wprevext[$b]) ? ' ' . $wprevext[$b] : '', 'LR', 'R', false, 0);
            PDF::MultiCell(60, 0, isset($wnowrrqty[$b]) ? ' ' . $wnowrrqty[$b] : '', 'LR', 'R', false, 0);
    ...
    if ($data[0]->ocp == '0.00' || $data[0]->ocp == '') {
      $percent = ($gtotaltodateamt / $data[0]->tcp) * 100;
      PDF::MultiCell(180, 0, number_format(($percent), 2) . '%', '', 'L', false, 0, 1050, 80);
    } else {
      $percent = ($gtotaltodateamt / $data[0]->ocp) * 100;
      PDF::MultiCell(180, 0, number_format($percent, 2) . '%', '', 'L', false, 0, 1050, 80);
    }

    PDF::MultiCell(180, 0, number_format($gtotalprevamt, 2), '', 'L', false, 0, 1100, 170);
    PDF::MultiCell(180, 0, number_format($gtotalnowamt, 2), '', 'L', false, 0, 1100, 185);
    PDF::MultiCell(180, 0, number_format($gtotaltodateamt, 2), '', 'L', false, 0, 1100, 200);

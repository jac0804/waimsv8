...
class po
{
  ...
  {
    $fields = ['radioprint', 'db', 'radiostatus', 'prepared', 'approved', 'received'];
  ...
  {
    $query = "select trno,dateid, docno, client, clientname, address,terms,rem,barcode,itemname,
                     revision,tel,email,contactperson,projectname, projectcode,subprojectname,
                     yourref, deladdress,notes,factor,uom,sum(qty) as qty, unitprice,disc, sum(ext) as ext,deldate
              from (select head.trno,date(head.dateid) as dateid, head.docno, client.client, client.clientname, 
                           head.address,head.terms,head.rem, item.barcode,item.itemname, 
                           round(stock.rrqty,6) as qty, stock.uom, round(stock.rrcost,7) as unitprice, 
                           stock.disc, round(stock.ext,2) as ext,m.model_name as model,item.sizeid,
                           head.revision, client.tel, client.email, client.contact as contactperson,
                           prj.name as projectname, prj.code as projectcode,sprj.subproject as subprojectname, 
                           head.yourref,left(head.deldate, 10) as deldate, head.deladdress, 
                           stock.rem as notes,uom.factor,stock.itemid
                    from pohead as head 
                    ...
                    where head.doc='po' and stock.itemid <> 10700 and head.trno='$trno' and stock.void=0
                    union all
                    select head.trno,date(head.dateid) as dateid, head.docno, client.client, client.clientname,
                           head.address, head.terms,head.rem, item.barcode,item.itemname, stock.rrqty as qty, 
                           stock.uom, stock.rrcost as unitprice, stock.disc, round(stock.ext,2) as ext,
                           m.model_name as model,item.sizeid, head.revision, client.tel, client.email, 
                           client.contact as contactperson,prj.name as projectname, prj.code as projectcode,
                           sprj.subproject as subprojectname, head.yourref,left(head.deldate, 10) as deldate, 
                           head.deladdress, stock.rem as notes,uom.factor,stock.itemid
                    ...
                    left join subproject as sprj on sprj.line = head.subproject
                    left join uom on uom.uom = stock.uom and uom.itemid = item.itemid
                    where head.doc='po' and stock.itemid <> 10700 and head.trno='$trno' and stock.void=0) as k
              group by trno,dateid, docno, client, clientname, address,terms,rem,barcode,itemname,revision,
                       tel,email,contactperson,projectname, projectcode,subprojectname,yourref, 
                       deladdress,notes,factor,uom, unitprice,disc,deldate
              union all
              select trno,dateid, docno, client, clientname, address,terms,rem,barcode,itemname,
                     revision,tel,email,contactperson,projectname, projectcode,subprojectname,
                     yourref, deladdress,notes,factor,uom,sum(qty) as qty, unitprice,disc, sum(ext) as ext,deldate
              from (select head.trno,date(head.dateid) as dateid, head.docno, client.client, client.clientname, 
                           head.address,head.terms,head.rem, item.barcode,item.itemname, 
  ...
  public function maxipro_layout_PDF($params, $data)
  {
    ...
    $extqry = $this->coreFunctions->opentable("select sum(round(ext,2)) as ext from (
            select ext
            from postock where trno=? and void=0
            union all
            select ext
            from hpostock where trno=? and void=0) as k", [$data[0]['trno'], $data[0]['trno']]);
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $counter++;
      $computedata = $this->othersClass->computestock($data[$i]['unitprice'], $data[$i]['disc'], $data[$i]['qty'], $data[$i]['factor'], 0, 'P', 0, 0, 0);

      $arr_ctr = $this->reporter->fixcolumn([$counter . ""], 10);
      $arr_itemname = $this->reporter->fixcolumn([$data[$i]['itemname']], 34); //35
      ...
      $arr_disc = $this->reporter->fixcolumn([$data[$i]['disc']], 10); //10
      $arr_unitprice = $this->reporter->fixcolumn([$peso . ' ' . number_format($unitprice, $decimalcurr)], 23); #21,23
      $arr_ext = $this->reporter->fixcolumn([$peso . ' ' . number_format($ext, $decimalcurr)], 25); #25
      ...
      if ($data[$i]['itemname'] != '') {
        for ($j = 0; $j < $maxrow; $j++) {
          ...
          PDF::MultiCell(200, 0, isset($arr_itemname[$j]) ? $arr_itemname[$j] : '', '', 'L', 0, 0, '', '', true, 0, true, false); //220
          ...
          PDF::MultiCell(65, 0, isset($arr_disc[$j]) ? $arr_disc[$j] : '', '', 'C', 0, 0, '', '', true, 0, true, false); //45
    ...
    PDF::MultiCell(640, 20, 'Total: ', 'TLB', 'R', false, 0, 20, PDF::getY());
    PDF::SetFont($font, '', $fontsize);

    PDF::MultiCell(30, 20, $tp, 'TB', 'L', false, 0);
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(90, 20,  number_format($totalext, 2) . '  ', 'RTB', 'R', false);

...
class jc
{
  ...
  {
    $query = "
      select head.docno, cl.client, cl.clientname, prj.name as projectname, coa.acno, coa.acnoname, 
            detail.db, detail.cr, sprj.subproject as subprojectname, date(head.dateid) as dateid,
  ...
  public function default_JC_header_PDF($params, $data, $data1)
  {
    ...
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(100, $maxh, "Subproject: ", '', 'L', false, 0, '',  '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(600, $maxh, (isset($data[0]['subprojectname']) ? $data[0]['subprojectname'] : ''), '', 'L', false, 0, '',  '');
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(600, $maxh, (isset($data[0]['workdesc']) ? $data[0]['workdesc'] : ''), '', 'L', false, 0, '',  '');
  ...
  public function JC_layout_PDF($params, $data)
  {
    ...
    if (!empty($data1)) {
      ...
      for ($i = 0; $i < count($data1); $i++) {
        $maxrow = 1;
        $arr_itemname = $this->reporter->fixcolumn([$data1[$i]['itemname']], '32', 0);
        $arr_rem = $this->reporter->fixcolumn([$data1[$i]['rem']], '21', 0);
        $arr_uom = $this->reporter->fixcolumn([$data1[$i]['uom']], '14', 0);
        ...
        $maxrow = $this->othersClass->getmaxcolumn([$arr_itemname, $arr_rem, $arr_qty, $arr_uom, $arr_ext]);
        $sign = '';
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(120, 0, (isset($arr_rem[$r]) ? $arr_rem[$r] : ''), '', 'L', false, 0, '', '', false, 1);
          ...
          PDF::MultiCell(30, 0, $sign, 'L', 'C', false, 0, '', '', false, 1, true);
          PDF::SetFont($font, '', 10);
    ...
    PDF::MultiCell(360, 20, "ACCOUNTING ENTRY", 'TL', 'C', false, 0);
    ...
    PDF::MultiCell(360, 0, '', 'TL', '', false, 0);
    ...
    if (!empty($data)) {
      ...
      for ($i = 0; $i < count($data); $i++) {
        ...
        // $arr_subproject = $this->reporter->fixcolumn([$data[$i]['subprojectname']], '30', 0);
        ...
        PDF::MultiCell(350, 20, $data[$i]['acnoname'], '', 'L', false, 0, '', '', false, 1);
        // PDF::MultiCell(160, 20, (isset($arr_subproject[0]) ? $arr_subproject[0] : ''), 'L', 'C', false, 0, '', '', false, 1);
    ...
    PDF::MultiCell(360, 0, '', 'BL', '', false, 0);

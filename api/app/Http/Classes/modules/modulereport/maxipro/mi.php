...
class mi
{
  ...
  public function report_default_query($config)
  {
    ...
    right(year(head.dateid),2) as year,left(head.dateid,10) as dateid, head.docno, 
    ...
    wh.client as whcode, wh.clientname as whname,stock.cost, proj.code as projcode, proj.name as projname
    from lahead as head
  ...
  public function default_MI_header_PDF($params, $data)
  {
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(620, 0, (isset($data[0]['projname']) ? $data[0]['projname'] : ''), 'B', 'L', false, 0, '',  '');
  ...
  public function default_MI_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $cost = number_format($data[$i]['amt'], $decimalprice);
        $disc = $data[$i]['disc'];
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(70, 0, (isset($arr_qty[$r]) ? $arr_qty[$r] : ''), '', 'R', false, 0, '', '', false, 1);
    ...
    if (!empty($chkposted)) {
      ...
      for ($k = 0; $k < count($detail); $k++) {
        ...
        if ($acname == $detail[$k]['acnoname']) {
          $acname = "";
        } else {
          $acname = $detail[$k]['acnoname'];
        }

        if ($accode == $detail[$k]['acno']) {
          $accode = "";
        } else {

...
class cv
{
  ...
  {
    $fields = ['radioprint', 'radioreporttype', 'prepared', 'approved', 'received', 'checked', 'issued', 'payor', 'tin', 'position', 'print'];
    ...
    data_set($col1, 'issued.label', 'Reviewed by');
  ...
  public function report_default_query($filters)
  {
    ...
    switch ($filters['params']['dataparams']['reporttype']) {
      case 0:
        ...
        detail.checkno, left(coa.alias,2) as alias, head2.yourref as invoiceno,proj.code as projcode,
        head.hacno,head.hacnoname,subproj.subproject,proj.name as projname,head.projectid,
        headproj.code as headprojcode,headproj.name as headprojname,detail.line
        ...
        where head.doc='cv' and head.trno ='$trno' and detail.db <> 0
        union all
        ...
        left join subproject as subproj on subproj.line=detail.subproject
        left join projectmasterfile as headproj on headproj.line=head.projectid
        where head.doc='cv' and head.trno ='$trno' and detail.cr <> 0
        union all
        select ifnull(DATE_FORMAT(cb.checkdate,'%Y-%m-%d'),DATE_FORMAT(head.dateid,'%Y-%m-%d')) as kdate, ifnull(head2.yourref,'') as dyourref,detail.rem as drem,
        DATE_FORMAT(left(detail.postdate,10),'%b %d %Y') as pdate,detail.ref,head.trno, head.docno, 
        date(head.dateid) as dateid, 
  ...
  public function PDF_maxipro_voucher_header($params, $data)
  {
    ...
    PDF::MultiCell(700, 5, 'CASH/CHECK VOUCHER', '', 'C', false);

    PDF::SetLineStyle(array('width' => 0.5, 'cap' => 'butt', 'join' => 'miter', 'dash' => 4));
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(80, 20, 'SUPPLIER #', '', 'L', false, 0);
    PDF::SetFont($fontbold, '', 11);
    PDF::MultiCell(330, 20, ':  ' . (isset($data[0]['client']) ? $data[0]['client'] : ''), 'B', 'L', false, 0, '',  '');
    PDF::MultiCell(30, 20, '', '', 'L', false, 0);
    PDF::SetFont($fontbold, '', 12);
    PDF::SetLineStyle(array('width' => 0.5, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0));
    PDF::MultiCell(90, 20, 'DOCUMENT #  :', '', 'R', false, 0);
    PDF::SetFont($fontbold, '', 11);
    PDF::SetTextColor(255, 0, 0);
    PDF::MultiCell(20, 20, '', '', 'R', false, 0);
    PDF::MultiCell(150, 20, (isset($data[0]['docno']) ? $data[0]['docno'] : ''), 'B', 'C', false);
    ...
    PDF::MultiCell(80, 20, 'SUPPLIER', '', 'L', false, 0);
    PDF::SetFont($fontbold, '', 11);
    PDF::MultiCell(330, 20, ':  ' . (isset($data[0]['clientname']) ? $data[0]['clientname'] : ''), 'B', 'L', false, 0, '',  '');
    PDF::MultiCell(30, 20, '', '', 'L', false, 0);
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(90, 20, 'DATE', '', 'L', false, 0);
    PDF::MultiCell(20, 20, '', '', 'R', false, 0);
    PDF::SetFont($fontbold, '', 11);
    PDF::MultiCell(150, 20, ':  ' . date('M-d-Y', strtotime($data[0]['dateid'])), 'B', 'L', false);

    $countarr = 0;

    for ($i = 0; $i < count($data[0]['address']); $i++) {
      $address = $this->reporter->fixcolumn([$data[0]['address']], '50');
      ...
      for ($r = 0; $r < $maxrow; $r++) {
        if ($r == 0) {
          $addrlabel = 'ADDRESS';
          $reflabel = 'REFERENCE #';
          $ref = ':  ' . (isset($data[0]['yourref']) ? $data[0]['yourref'] : '');
          $addr = ': ';
        } else {
          $addrlabel = '';
          $reflabel = '';
        ...
        PDF::SetFont($font, '', 11);
        PDF::MultiCell(80, 0, $addrlabel, '', 'L', false, 0);
        PDF::SetFont($fontbold, '', 11);
        PDF::MultiCell(330, 20, (isset($address[$r]) ? $addr . $address[$r] : ''), '', 'L', false, 0, '',  '');
        PDF::MultiCell(30, 20, '', '', 'L', false, 0);
        PDF::SetFont($font, '', 11);
        PDF::MultiCell(90, 20, $reflabel, '', 'L', false, 0);
        PDF::MultiCell(20, 20, '', '', 'R', false, 0);
        PDF::SetFont($fontbold, '', 11);
        PDF::MultiCell(150, 20, $ref, '', 'L', false);
      }
      // }
      ...
      PDF::MultiCell(150, 0, '', 'T', 'L', false);
    }

    PDF::SetFont($font, '', 11);
    PDF::MultiCell(80, 20, 'ACCT. NAME', '', 'L', false, 0);
    PDF::SetFont($fontbold, '', 11);
    PDF::MultiCell(330, 20, ':  ' . (isset($data[0]['hacnoname']) ? $data[0]['hacnoname'] : ''), 'B', 'L', false, 0, '',  '');
    PDF::MultiCell(30, 20, '', '', 'L', false, 0);
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(90, 20, 'ACCOUNT # ', '', 'L', false, 0);
    PDF::MultiCell(20, 20, '', '', 'R', false, 0);
    PDF::SetFont($fontbold, '', 11);
    PDF::MultiCell(150, 20, ':  ' . (isset($data[0]['hacno']) ? $data[0]['hacno'] : ''), 'B', 'L', false);
    ...
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(80, 20, 'NOTES', '', 'L', false, 0);
    PDF::SetFont($fontbold, '', 11);
    PDF::MultiCell(600, 20, ':  ' . (isset($data[0]['rem']) ? $data[0]['rem'] : ''), 'B', 'L', false);

    PDF::SetFont($font, '', 4);
    PDF::MultiCell(700, 0, '', '', 'L', false);

    PDF::SetFont($font, '', 11);
    PDF::MultiCell(80, 20, 'PROJECT', '', 'L', false, 0);
    PDF::SetFont($fontbold, '', 11);
    PDF::MultiCell(620, 20, ':  ' . (isset($data[0]['headprojcode']) ? $data[0]['headprojcode'] . ' - ' . $data[0]['headprojname'] : ''), 'B', 'L', false);
    ...
    PDF::MultiCell(710, 5, '', 'T');
    PDF::SetLineStyle(array('width' => 0.5, 'cap' => 'butt', 'join' => 'miter', 'dash' => 4));

    PDF::SetFont($fontbold, '', 9);
    PDF::MultiCell(50, 20, 'ACCT #', 'B', 'C', false, 0);
    PDF::MultiCell(150, 20, 'ACCOUNT NAME', 'B', 'C', false, 0);
    PDF::MultiCell(130, 20, 'PROJECT', 'B', 'C', false, 0);
    PDF::MultiCell(130, 20, 'SUBPROJECT', 'B', 'C', false, 0);
    PDF::MultiCell(80, 20, 'REMARKS', 'B', 'C', false, 0);
    PDF::MultiCell(80, 20, 'DEBIT', 'B', 'C', false, 0);
    PDF::MultiCell(80, 20, 'CREDIT', 'B', 'C', false);
  }
  ...
  public function PDF_maxipro_voucher_layout($data, $params)
  {
    ...
    if (!empty($data)) {
      ...
      for ($i = 0; $i < count($data); $i++) {
        $acnonamedescs = $this->reporter->fixcolumn([$data[$i]['acnoname']], 24, 0);
        $countarr = count($acnonamedescs);

        $project = $this->reporter->fixcolumn([$data[$i]['projcode']], 22, 0);
        $countarrp = count($project);

        $subproject = $this->reporter->fixcolumn([$data[$i]['subproject']], 20, 0);
        $countarrsp = count($subproject);

        $rem = $this->reporter->fixcolumn([$data[$i]['drem']], 10, 0);
        $countarrrem = count($rem);

        $maxrow = max($countarr, $countarrp, $countarrsp, $countarrrem);
        ...
        if ($data[$i]['acnoname'] == '') {
          ...
          for ($r = 0; $r < $maxrow; $r++) {
            if ($r == 0) {
              ...
              // $drem = $data[$i]['drem'];
              // $remlen = strlen($drem) / 15;
              $acclen = strlen($data[$i]['acnoname']) / 15;
            } else {
              $acno = '';
              $pdate = '';
              $debit = '';
            ...
            PDF::MultiCell(50, 15, $acno, '', 'C', false, 0);
            PDF::MultiCell(150, 15, isset($acnonamedescs[$r]) ? $acnonamedescs[$r] : '', '', 'L', false, 0);
            PDF::MultiCell(130, 15, isset($project[$r]) ? $project[$r] : '', '', 'L', false, 0);
            PDF::MultiCell(130, 15, isset($subproject[$r]) ? $subproject[$r] : '', '', 'L', false, 0);
            PDF::MultiCell(80, 15, isset($rem[$r]) ? ' ' . $rem[$r] : '', '', 'L', false, 0);
            PDF::MultiCell(80, 15, $debit, '', 'R', false, 0);
            PDF::MultiCell(80, 15, $credit, '', 'R', false);
          }
        }
        $totaldb += $data[$i]['db'];
        $totalcr += $data[$i]['cr'];
    ...
    PDF::SetFont($font, '', 9);
    PDF::MultiCell(70, 20, 'Check # : ', '', 'L', false, 0);
    PDF::SetFont('dejavusans', 'UI', 9, '', true);
    PDF::MultiCell(253, 20, $checkno, '', 'L', false);
    PDF::SetFont($font, '', 11);
    ...
    PDF::MultiCell(130, 20, '', '', 'L', false, 0);
    PDF::MultiCell(100, 20, '', '', 'L', false, 0);
    PDF::MultiCell(110, 20, 'GRAND TOTAL', '', 'L', false, 0);
    PDF::MultiCell(80, 20, number_format($totaldb, $decimal), '', 'R', false, 0);
    PDF::MultiCell(80, 20, number_format($totalcr, $decimal), '', 'R', false);
    ...
    PDF::MultiCell(350, 50, 'Prepared By: ', '', 'L', false, 0);
    PDF::MultiCell(350, 50, 'Checked By: ', '', 'L', false);

    PDF::SetFont($font, '', 10);
    PDF::MultiCell(23, 0, '', '', 'L', false, 0);
    PDF::MultiCell(220, 0, $params['params']['dataparams']['prepared'], 'B', 'C', false, 0);
    PDF::MultiCell(107, 0, '', '', 'L', false, 0);

    PDF::MultiCell(23, 0, '', '', 'L', false, 0);
    PDF::MultiCell(220, 0, $params['params']['dataparams']['checked'], 'B', 'C', false, 0);
    PDF::MultiCell(107, 0, '', '', 'L', false);


    PDF::MultiCell(0, 0, "\n\n");

    PDF::SetFont($fontbold, '', 10);
    PDF::MultiCell(350, 50, 'Reviewed By: ', '', 'L', false, 0);
    PDF::MultiCell(350, 50, 'Approved By: ', '', 'L', false);

    PDF::SetFont($font, '', 10);
    PDF::MultiCell(23, 0, '', '', 'L', false, 0);
    PDF::MultiCell(220, 0, $params['params']['dataparams']['issued'], 'B', 'C', false, 0);
    PDF::MultiCell(107, 0, '', '', 'L', false, 0);

    PDF::MultiCell(23, 0, '', '', 'L', false, 0);
    PDF::MultiCell(220, 0, $params['params']['dataparams']['approved'], 'B', 'C', false, 0);
    PDF::MultiCell(107, 0, '', '', 'L', false);

    PDF::MultiCell(0, 0, "\n\n\n\n\n\n");

    PDF::SetFont($fontbold, '', 10);
    PDF::MultiCell(350, 50, '', '', 'L', false, 0);
    PDF::MultiCell(350, 50, 'Received By: ', '', 'L', false);

    PDF::SetFont($font, '', 10);
    PDF::MultiCell(373, 0, '', '', 'L', false, 0);
    PDF::MultiCell(220, 0, $params['params']['dataparams']['received'], 'B', 'C', false, 0);
    PDF::MultiCell(10, 0, '', '', 'C', false, 0);
    PDF::MultiCell(74, 0, '', 'B', 'C', false);

    PDF::SetFont($font, '', 9);
    PDF::MultiCell(373, 0, '', '', 'L', false, 0);
    PDF::MultiCell(220, 0, 'Signature over Printed Name', '', 'C', false, 0);
    PDF::MultiCell(10, 0, '', '', 'C', false, 0);
    PDF::MultiCell(74, 0, 'Date', '', 'C', false);
  ...
  public function PDF_default_header($params, $data)
  {
    ...
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(470, 5, (isset($data[0]['clientname']) ? $data[0]['clientname'] : ''), '', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', 14);
    PDF::MultiCell(55, 5, 'DATE:', '', 'R', false, 0);
    PDF::SetFont($font, '', 11);
    ...
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(470, 5, '', 'T', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', 14);
    PDF::MultiCell(55, 5, '', '', 'R', false, 0);
    PDF::SetFont($font, '', 11);
    ...
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(420, 5, (isset($data[0]['address']) ? $data[0]['address'] : ''), '', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', 14);
    PDF::MultiCell(105, 5, 'REFERENCE # :', '', 'L', false, 0);
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(125, 5, (isset($data[0]['yourref']) ? $data[0]['yourref'] : ''), '', 'R', false);

    PDF::SetFont($fontbold, '', 14);
    PDF::MultiCell(70, 5, '', '', 'L', false, 0);
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(420, 5, '', 'T', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', 14);
    PDF::MultiCell(105, 5, '', '', 'L', false, 0);
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(125, 5, '', 'T', 'R', false);

    PDF::SetFont($fontbold, '', 14);
    PDF::MultiCell(70, 5, 'NOTES : ', '', 'L', false, 0);
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(650, 5, (isset($data[0]['rem']) ? $data[0]['rem'] : ''), '', 'L', false);

    PDF::SetFont($fontbold, '', 14);
    PDF::MultiCell(70, 5, '', '', 'L', false, 0);
    PDF::MultiCell(650, 5, '', 'T', 'L', false);

    PDF::MultiCell(720, 5, "\n", 'B', 'L', false);

    PDF::SetFont($fontbold, '', 11);
    PDF::MultiCell(90, 5, 'ACCT #', 'TB', 'C', false, 0);
    PDF::MultiCell(160, 5, 'ACCOUNT NAME', 'TB', 'C', false, 0);
    PDF::MultiCell(100, 5, 'CHECK DETAILS', 'TB', 'C', false, 0);
    PDF::MultiCell(75, 5, 'DATE', 'TB', 'C', false, 0);
    PDF::MultiCell(85, 5, 'DEBIT', 'TB', 'R', false, 0);
    PDF::MultiCell(85, 5, 'CREDIT', 'TB', 'R', false, 0);
    PDF::MultiCell(10, 5, '', 'TB', 'C', false, 0);
    PDF::MultiCell(110, 5, 'REMARKS', 'TB', 'C', false);
  }
  ...
  public function PDF_DEFAULT_CCVOUCHER_LAYOUT1($data, $params)
  {
      ...
      for ($i = 0; $i < count($data); $i++) {
        ...
        if ($data[$i]['acnoname'] == '') {
          ...
          for ($r = 0; $r < $maxrow; $r++) {
            ...
            PDF::MultiCell(160, 0, isset($acnonamedescs[$r]) ? $acnonamedescs[$r] : '', '', 'L', false, 0, '', '', false, 1);
            PDF::MultiCell(100, 0, " " . $checkno, '', 'L', false, 0, '', '', false, 1);
    ...
    PDF::MultiCell(0, 0, "\n");
    PDF::SetFont($fontbold, '', 12);
    PDF::MultiCell(75, 5, '', '', 'C', false, 0);
    PDF::MultiCell(135, 5, '', '', 'C', false, 0);
    PDF::MultiCell(200, 5, 'GRAND TOTAL : ', '', 'C', false, 0);
    PDF::MultiCell(100, 5, number_format($totaldb, $decimal), '', 'R', false, 0);
  ...
  public function PDF_DEFAULT_CCVOUCHER_LAYOUT2($data, $params)
  {
    ...
    PDF::SetMargins(45, 40);

    $qry = "select date(detail.postdate) as postdate,detail.checkno,coa.acno,detail.cr 
    ...
    UNION ALL
    select date(detail.postdate) as postdate,detail.checkno,coa.acno,detail.cr 
    ...
    for ($i = 0; $i < count($data2); $i++) {
      PDF::AddPage('p', [800, 1000]);
      $cc = $data2[$i]['cr'];
      $cdate = date('F d, Y', strtotime($data2[$i]['postdate']));

      PDF::SetFont($font, '', 5);
      PDF::MultiCell(775, 5, '', '', 'C', false);

      PDF::SetFont($font, '', 12);
      PDF::MultiCell(25, 5, '', '', 'C', false, 0);
      PDF::MultiCell(150, 5, "PAYEE'S ACCOUNT ONLY", '', 'C', false, 0, 70, 45);
      PDF::MultiCell(230, 5, '', '', 'C', false, 0);

      PDF::setFontSpacing(10);
      PDF::SetFont($font, '', 12);
      PDF::MultiCell(570, 0, date('m', strtotime($data2[$i]['postdate'])), '', 'L', false, 0, '550', '40'); //520,40
      PDF::setFontSpacing(11);
      PDF::MultiCell(50, 0, date('d', strtotime($data2[$i]['postdate'])), '', 'L', false, 0, '585', '40'); //565,40
      PDF::setFontSpacing(12);
      PDF::MultiCell(100, 0, date('Y', strtotime($data2[$i]['postdate'])), '', 'L', false, 0, '630', '40'); //605,40
      ...
      PDF::MultiCell(775, 5, '', '', 'C', false);
      PDF::SetFont($font, '', 12);
      PDF::MultiCell(100, 5, '', '', 'C', false, 0);
      PDF::MultiCell(420, 5, $params['params']['dataparams']['payor'], '', 'L', false, 0, 115, 67); //115,72
      PDF::MultiCell(20, 5, '', '', 'L', false, 0);
      PDF::MultiCell(235, 5, (isset($cc) ? number_format($cc, $decimal) : ''), '', 'L', false, 0, 530, 67);

      PDF::SetFont($font, '', 15);
      PDF::MultiCell(775, 0, '', '', 'C', false);

      $dd = number_format((float)$cc, 2, '.', '');
      PDF::SetFont($font, '', 12);
      PDF::MultiCell(45, 5, '', '', 'C', false, 0);
      PDF::MultiCell(730, 5, $this->ftNumberToWordsConverter($dd), '', 'L', false, 1, 90, 97); //90,102
  ...
  public function PDF_CV_WTAXREPORT($data, $params)
  {
    ...
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(200, 25, 'Tax Payer Identification Number (TIN)', '', 'C', false, 0);
    PDF::MultiCell(520, 18, (isset($data['head'][0]['payortin']) ? $data['head'][0]['payortin'] : ''), 'LTBR', 'C', false, 0);
    PDF::MultiCell(10, 25, '', '', 'C', false);
    ...
    foreach ($data['detail'] as $key => $value) {
      ...
      PDF::MultiCell(200, $ewt_height, $data['res'][$a]['ewtdesc'], 'LRB', '', false, 0);
      ...
      switch ($data['head'][0]['month']) {
        ...
        case '7':
        case '10':
          PDF::MultiCell(95, $ewt_height, number_format($data['detail'][$key]['oamt']), 'LRB', 'R', false, 0);
      ...
      PDF::MultiCell(120, $ewt_height, number_format($data['detail'][$key]['xamt']), 'LRB', 'R', false);
    ...
    //Row 27
    //ucwords($params['params']['dataparams']['payor'])
    //$params['params']['dataparams']['tin']
    //ucwords($params['params']['dataparams']['position'])

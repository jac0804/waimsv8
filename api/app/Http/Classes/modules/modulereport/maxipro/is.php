...
class is
{
  ...
  public function reportplotting($params, $data)
  {
    ...
    if ($params['params']['dataparams']['print'] == "default") {
      ...
      return $this->Maxipro_IS_PDF($params, $data);
    }
  }
  ...
  public function default_is_layout($params, $data)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $str .= $this->reporter->col(number_format($data[$i]['qty'], 2), '50px', null, false, '1px solid ', '', 'C', 'Century Gothic', '12', '', '', '2px');
  ...
  public function Maxipro_IS_header_PDF($params, $data)
  {
    $center = $params['params']['center'];
    $username = $params['params']['user'];
    ...
    PDF::MultiCell(470, 0, 'INVENTORY SET-UP', '', 'L', false, 0, '',  '140');
    ...
    PDF::MultiCell(400, 15, ':  ' . (isset($data[0]['whname']) ? $data[0]['whname'] : ''), 'B', 'L', false, 0, '',  '');
    // PDF::SetFont($fontbold, '', $fontsize);
  ...
  public function Maxipro_IS_PDF($params, $data)
  {
    ...
    $this->Maxipro_IS_header_PDF($params, $data);
    ...
    // PDF::MultiCell(20, 15, "<span>&#8369;</span>", 'TBL', 'R', false,0,true);
  ...
  public function default_IS_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $arr_gross = $this->reporter->fixcolumn([number_format($data[$i]['gross'], $decimalprice)], '13', 0);
        $arr_ext = $this->reporter->fixcolumn([number_format($data[$i]['ext'], $decimalcurr)], '13', 0);

        $maxrow = $this->othersClass->getmaxcolumn([$arr_barcode, $arr_itemname, $arr_qty, $arr_uom, $arr_gross, $arr_ext]);
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(125, 0, (isset($arr_gross[$r]) ? $arr_gross[$r] : ''), '', 'R', false, 0, '', '', false, 1);

...
class sj
{
  ...
  public function reportplotting($params, $data)
  {
    switch ($params['params']['dataparams']['reporttype']) {
      case '0':
        ...
        if ($params['params']['companyid'] == 41) { //labsol manila
          $str = $this->default_sjP_PDF($params, $data); // dr report
        } else {
  ...
  public function default_sjP_header_PDF($params, $data)
  {
    $center = $params['params']['center'];
    $username = $params['params']['user'];
    $companyid = $params['params']['companyid'];
  ...
  public function default_sjP_PDF($params, $data)
  {
    $decimalcurr = $this->companysetup->getdecimal('currency', $params['params']);
    $decimalqty = $this->companysetup->getdecimal('qty', $params['params']);
    $decimalprice = $this->companysetup->getdecimal('price', $params['params']);
    ...
    $this->default_sjP_header_PDF($params, $data);
    PDF::MultiCell(0, 40, "");
  ...
  public function default_siP_PDF($params, $data)
  {
    ...
    PDF::MultiCell(100, 0, number_format($totalext, $decimalcurr), '', 'R', false, 0, '658', '642', false, 1);
    PDF::MultiCell(100, 0, number_format($totalext, $decimalcurr), '', 'R', false, 1, '658', '695', false, 1);

    PDF::MultiCell(0, 0, "\n");
    PDF::SetFont($font, '', $fontsize);

    PDF::MultiCell(253, 0, $params['params']['dataparams']['prepared'], '', 'L', false, 0, 38, 750);
    PDF::MultiCell(253, 0, $params['params']['dataparams']['received'], '', 'L', false, 0, 210, 750); //Ordered
    PDF::MultiCell(253, 0, $params['params']['dataparams']['approved'], '', 'L', false, 0, 370, 750); //Delivered

...
class cv
{
  ...
  public function report_default_query($filters)
  {
    ...
    switch ($filters['params']['dataparams']['reporttype']) {
      ...
      default:
        $query = "select ifnull(DATE_FORMAT(cb.checkdate,'%Y-%m-%d'),DATE_FORMAT(head.dateid,'%Y-%m-%d')) as kdate, detail.rem as drem,
        ...
        ifnull((select group_concat(yourref) from glhead where trno=detail.refx),'') as pvref
        ...
        union all
        select ifnull(DATE_FORMAT(cb.checkdate,'%Y-%m-%d'),DATE_FORMAT(head.dateid,'%Y-%m-%d')) as kdate,detail.rem as drem,
        DATE_FORMAT(left(detail.postdate,10),'%b %d %Y') as pdate,detail.ref,head.trno, head.docno, 
        date(head.dateid) as dateid, 
        date(cntnum.postdate) as postdate,client.client, head.clientname, head.address,
        client.tin, '' as busstyle, head.terms, head.yourref, head.ourref, head.rem, coa.acno,
  ...
  public function PDF_default_header($params, $data)
  {
    ...
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(410, 5, (isset($data[0]['address']) ? $data[0]['address'] : ''), '', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', 14);
    PDF::MultiCell(100, 5, 'REFERENCE # :', '', 'L', false, 0);
    PDF::SetFont($font, '', 11);
    ...
    PDF::MultiCell(70, 5, '', '', 'L', false, 0);
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(410, 5, '', 'T', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', 14);
    ...
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(410, 5, (isset($data[0]['rem']) ? $data[0]['rem'] : ''), '', 'L', false, 0);
    PDF::SetFont($fontbold, '', 14);
    PDF::MultiCell(65, 5, 'OURREF : ', '', 'L', false, 0);
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(155, 5, (isset($data[0]['ourref']) ? $data[0]['ourref'] : ''), '', 'L', false);
    ...
    PDF::MultiCell(410, 5, '', 'T', 'L', false, 0);
    PDF::SetFont($fontbold, '', 14);
    PDF::MultiCell(65, 5, '', '', 'L', false, 0);
    PDF::SetFont($font, '', 11);
    PDF::MultiCell(155, 5, '', 'T', 'R', false);
    ...
    PDF::MultiCell(100, 5, 'CHECK DETAILS', '', 'C', false, 0);
    PDF::MultiCell(100, 5, 'REF NO.', '', 'C', false, 0);
    PDF::MultiCell(75, 5, 'DATE', '', 'C', false, 0);
    PDF::MultiCell(85, 5, 'DEBIT', '', 'R', false, 0);
    PDF::MultiCell(85, 5, 'CREDIT', '', 'R', false);
  ...
  public function PDF_DEFAULT_CCVOUCHER_LAYOUT1($data, $params)
  {
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $arr_checkno = $this->reporter->fixcolumn([$checkno], '28', 0);
        ...
        $arr_pvref = $this->reporter->fixcolumn([$pvref], '15', 0);
        // $arr_drem = $this->reporter->fixcolumn([$drem], '15', 0);

        $maxrow = $this->othersClass->getmaxcolumn([$arr_acno, $arr_acnonamedescs, $arr_checkno, $arr_pdate, $arr_debit, $arr_credit, $arr_pvref]);
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(100, 15, ' ' . (isset($arr_pvref[$r]) ? $arr_pvref[$r] : ''), '', 'L', false, 0, '', '', true, 0, false, true, 0, 'M', false);
    ...
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(530, 0, 'GRAND TOTAL: ', '', 'R', false, 0);
  ...
  public function PDF_DEFAULT_CCVOUCHER_LAYOUT2($data, $params)
  {
    ...
    for ($i = 0; $i < count($data2); $i++) {
      ...
      if (isset($params['params']['dataparams']['payor'])) {
        if ($params['params']['dataparams']['payor'] != '') {
          $clientname = $params['params']['dataparams']['payor'];
        }
      }
      PDF::MultiCell(120, 5, '', '', 'C', false);
      PDF::MultiCell(120, 5, '', '', 'C', false, 0);
      PDF::MultiCell(340, 5, $clientname, '', 'L', false, 0);
      PDF::MultiCell(260, 5, (isset($cc) ? number_format($cc, $decimal) : ''), '', 'L', false, 0);
  ...
  // on a chinabank check, adjusted date and amount to the left
  public function PDF_METROBANK_CHECK_LAYOUT($data, $filters)
  {
    ...
    for ($i = 0; $i < count($data2); $i++) {
      ...
      PDF::SetFont($font, '', 11);
      PDF::MultiCell(50, 0, $month, '', 'R', false, 0, '505', '40',true,0,true);
      PDF::MultiCell(50, 0, $day, '', 'C', false, 0, '565', '',true,0,true);
      PDF::MultiCell(50, 0, $year, '', 'C', false, 1, '', '',true,0,true);

      PDF::MultiCell(720, 5, "\n", '', 'L', false);

      $clientname = $data[0]['clientname'];
      if (isset($filters['params']['dataparams']['payor'])) {
        if ($filters['params']['dataparams']['payor'] != '') {
          $clientname = $filters['params']['dataparams']['payor'];
        }
      }

      // PDF::MultiCell(70, 15, "COST CENTER", 'TBL', 'C', false, 0,'40','180');
      PDF::MultiCell(80, 0, '', '', 'L', false, 0);
      PDF::MultiCell(450, 0, strtoupper($clientname), '', 'L', false, 0, '95', '57');

      PDF::MultiCell(70, 0, '', '', 'L', false, 0);
      PDF::MultiCell(220, 0, (isset($cc) ? number_format($cc, $decimal) : ''), '', 'L', false, 1, '520', '70');

      PDF::MultiCell(720, 5, "\n", '', 'L', false);

      $dd = number_format((float)$cc, 2, '.', '');
      PDF::MultiCell(620, 0, $this->ftNumberToWordsConverter($dd) . ' ONLY', '', 'L', false, 1, '70', '100');
    }

    return PDF::Output($this->modulename . '.pdf', 'S');
  } //end fn

  public function PDF_METROBANK_CHECK_LAYOUT_07_17_2024($data, $filters)
  {
    ...
    for ($i = 0; $i < count($data2); $i++) {
      ...
      // PDF::setFontSpacing(2);
      PDF::SetFont($font, '', 11);
      PDF::MultiCell(50, 0, date('m', strtotime($data2[$i]['postdate'])), '', 'R', false, 0, '570', '50');
      PDF::MultiCell(50, 0, date('d', strtotime($data2[$i]['postdate'])), '', 'C', false, 0, '', '');
      PDF::MultiCell(50, 0, date('Y', strtotime($data2[$i]['postdate'])), '', 'C', false, 1, '', '');
      ...
      // PDF::MultiCell(70, 15, "COST CENTER", 'TBL', 'C', false, 0,'40','180');
      PDF::MultiCell(80, 0, '', '', 'L', false, 0);
      PDF::MultiCell(450, 0, strtoupper($clientname), '', 'L', false, 0, '95', '');

      PDF::MultiCell(70, 0, '', '', 'L', false, 0);
      PDF::MultiCell(220, 0, (isset($cc) ? '***' . number_format($cc, $decimal) . '***' : ''), '', 'L', false, 1, '600', '');

      PDF::MultiCell(720, 5, "\n", '', 'L', false);

      $dd = number_format((float)$cc, 2, '.', '');
      PDF::MultiCell(620, 0, '***' . $this->ftNumberToWordsConverter($dd) . ' ONLY***', '', 'L', false, 1, '70', '110');
    }
  ...
  public function PDF_BPI_CHECK_LAYOUT($data, $filters)
  {
    ...
    for ($i = 0; $i < count($data2); $i++) {
      ...
      PDF::SetFont($font, '', 11);
      PDF::MultiCell(50, 0, date('m', strtotime($data2[$i]['postdate'])), '', 'R', false, 0, '505', '50');
      PDF::MultiCell(50, 0, date('d', strtotime($data2[$i]['postdate'])), '', 'C', false, 0, '565', '');
      PDF::MultiCell(100, 0, date('Y', strtotime($data2[$i]['postdate'])), '', 'C', false, 1, '605', '');
      ...
      PDF::MultiCell(470, 0, '**' . strtoupper($clientname) . '**', '', 'L', false, 0);
      // PDF::MultiCell(150, 0, '', 'TBLR', 'L', false, 0);

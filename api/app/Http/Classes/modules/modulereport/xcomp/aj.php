...
class aj
{
  ...
  public function report_default_query($filters)
  {
    ...
    head.ourref, head.yourref, prj.name as projectname,
    head.client, head.clientname as custname
    from lahead as head 
    ...
    stock.uom, stock.disc, stock.ext, stock.line,
    head.ourref, head.yourref, prj.name as projectname,
    cl.client, cl.clientname as custname
    from (glhead as head 
  ...
  public function default_AJ_header_PDF($params, $data)
  {
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(470, 0, (isset($data[0]['client']) ? $data[0]['client'].' - '.$data[0]['custname'] : ''), 'B', 'L', false, 0, '', '');
    PDF::SetFont($fontbold, '', $fontsize);
  ...
  public function default_AJ_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(100, 15, ' ' . ($viewcost == '1' ? (isset($arr_cost[$r]) ? $arr_cost[$r] : '') : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
          PDF::MultiCell(100, 15, ' ' . ($viewcost == '1' ? (isset($arr_ext[$r]) ? $arr_ext[$r] : '') : ''), '', 'R', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
        }
    ...
    PDF::MultiCell(100, 0, ($viewcost == '1' ? number_format($totalext, $decimalcurr) : ''), '', 'R');

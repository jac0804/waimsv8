...
class rr
{
  ...
  public function report_default_query($config)
  {
    ...
    $query = "select head.docno,head.trno, head.clientname, head.address, date(head.dateid) as dateid, head.terms, head.rem,
        item.barcode, if(ifnull(sit.itemdesc,'')='',item.itemname,sit.itemdesc) as itemname, stock.rrcost as gross, stock.cost as netamt, stock.rrqty as qty,
        stock.uom, stock.disc, stock.ext, stock.line,wh.client as wh,wh.clientname as whname,stock.loc,date(stock.expiry) as expiry,stock.rem as srem,item.sizeid,m.model_name as model,head.yourref,head.ourref
        from lahead as head
  ...
  public function default_RR_header_PDF($params, $data)
  {
    ...
    PDF::MultiCell(60, 12, "Docno #: ", '', 'L', false, 0);
    PDF::SetFont($font, '', 10);
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(470, 0, (isset($data[0]['ourref']) ? $data[0]['ourref'] : ''), 'TB', 'L', false, 0);
    PDF::SetFont($fontbold, '', $fontsize);
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          $wh = $data[$i]['wh'];
          $gross = number_format($data[$i]['gross'], 2);
          $disc = $data[$i]['disc'];
          ...
          $maxrow = $this->othersClass->getmaxcolumn([$arr_barcode, $arr_itemname, $arr_qty, $arr_uom, $arr_wh, $arr_gross, $arr_disc, $arr_ext]);
          ...
          for ($r = 0; $r < $maxrow; $r++) {
            ...
            PDF::MultiCell(105, 15, ' ' . (isset($arr_wh[$r]) ? $arr_wh[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);

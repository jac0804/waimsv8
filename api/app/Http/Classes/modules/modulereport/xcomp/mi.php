...
class mi
{
  ...
  public function report_default_query($config)
  {
    ...
    client.addr as address, head.terms, item.barcode, head.shipto, client.tin, head.yourref, head.ourref,
    item.itemname, stock.isqty as qty, stock.uom, stock.isamt as amt, stock.disc, stock.ext, head.agent,
    item.sizeid, ag.clientname as agname, item.brand, prj.name as project,
    (case when stockinfo.itemdesc != '' then ifnull(stockinfo.itemdesc,'') else item.itemname end) as itemdesc,
    ...
    left join stockinfo as stockinfo on stockinfo.line = stock.line and stockinfo.trno = stock.trno
  ...
  public function default_MI_header_PDF($params, $data)
  {
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(210, 0, (isset($data[0]['yourref']) ? $data[0]['yourref'] : ''), 'B', 'L', false, 0, '',  '');
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(50, 0, "Project: ", '', 'L', false, 0, '', '');
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(210, 0, (isset($data[0]['project']) ? $data[0]['project'] : ''), 'B', 'L', false, 0, '', '');
    PDF::SetFont($fontbold, '', $fontsize);
  ...
  public function default_MI_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $arr_itemdesc = $this->reporter->fixcolumn([$itemdesc], '40', 0);
        ...
        $maxrow = $this->othersClass->getmaxcolumn([$arr_barcode, $arr_itemdesc, $arr_qty, $arr_uom, $arr_amt, $arr_disc, $arr_ext]);
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(250, 15, ' ' . (isset($arr_itemdesc[$r]) ? $arr_itemdesc[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);

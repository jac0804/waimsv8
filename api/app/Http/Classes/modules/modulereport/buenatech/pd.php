...
class pd
{
    ...
    {
        $query = "select date(head.dateid) as dateid, head.docno, client.client, client.clientname,
         so.docno as sodocno,head.rem,  head.qty as hqty, head.uom as huom,head.itemid, item2.barcode as hbarcode,item2.itemname as hitemname,
         item.barcode,item.itemname,head.rem,warehouse.clientname as whname, 
          stock.rrqty as qty, stock.uom, stock.rrcost as netamt, stock.disc, stock.ext
        from pdhead as head 
        left join pdstock as stock on stock.trno=head.trno
        left join client on client.client=head.client
        left join item on item.itemid = stock.itemid
       
        left join item as item2 on  item2.itemid = head.itemid

        left join hsohead as so on so.trno=head.sotrno
        left join client as warehouse on warehouse.client=head.wh
        where head.doc='pd' and head.trno='$trno'
        union all
        ...
        left join hpdstock as stock on stock.trno=head.trno
        left join client on client.client=head.client
        left join item on item.itemid = stock.itemid
         left join item as item2 on item2.itemid = head.itemid
        left join hsohead as so on so.trno=head.sotrno
    ...
    public function default_PD_header_PDF($params, $data)
    {
        ...
        PDF::MultiCell(450, 20, (isset($data[0]['clientname']) ? $data[0]['clientname'] : ''), 'B', 'L', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
        PDF::SetFont($fontbold, '', $fontsize);
        ...
        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(80, 20, "Sales Order# : ", '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(450, 20, (isset($data[0]['sodocno']) ? $data[0]['sodocno'] : ''), 'B', 'L', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(70, 20, "Barcode : ", '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(100, 20, (isset($data[0]['hbarcode']) ? $data[0]['hbarcode'] : ''), 'B', 'L', false, 1, '',  '', true, 0, false, true, 0, 'B', true);


        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(80, 20, "Itemname : ", '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(450, 20, (isset($data[0]['hitemname']) ? $data[0]['hitemname'] : ''), 'B', 'L', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(70, 20, "QTY : ", '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(100, 20, (isset($data[0]['hqty']) ? $data[0]['hqty'] : ''), 'B', 'L', false, 1, '',  '', true, 0, false, true, 0, 'B', true);

        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(80, 20, "Warehouse : ", '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(450, 20, (isset($data[0]['whname']) ? $data[0]['whname'] : ''), 'B', 'L', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(70, 20, "UOM : ", '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(100, 20, (isset($data[0]['huom']) ? $data[0]['huom'] : ''), 'B', 'L', false, 1, '',  '', true, 0, false, true, 0, 'B', true);

        PDF::MultiCell(0, 0, "\n\n");

        PDF::SetFont($font, 'B', $fontsize);
        PDF::MultiCell(125, 25, "BARCODE", 'TB', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', true);
    ...
    public function default_PD_PDF($params, $data)
    {
        ...
        $this->default_PD_header_PDF($params, $data);

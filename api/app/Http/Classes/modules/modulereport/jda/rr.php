...
class rr
{
    ...
    public function createreportfilter($config)
    {
        ...
        data_set($col1, 'radioreporttype.options', [
            ['label' => 'Receiving Report', 'value' => 'PC', 'color' => 'red'],
            ['label' => 'Receiving Receipt', 'value' => 'IR', 'color' => 'red']
        ]);
    ...
    public function report_default_query($config)
    {
        ...
        $serial = ",ifnull((select ifnull(group_concat(concat('',rr.serial) separator '\\n\\r'),'') from serialin as rr where rr.trno=stock.trno and rr.line=stock.line),'') as serial";

        $trno = $config['params']['dataid'];
        $query = "select head.docno,head.trno, head.clientname, head.address, date(head.dateid) as dateid, head.terms, head.rem as remark,
    item.barcode, item.itemname, stock.rrcost as gross, stock.cost as netamt, stock.rrqty as qty, head.yourref, head.ourref,
    stock.uom, stock.disc, stock.ext, stock.line,wh.client as wh,wh.clientname as whname,stock.loc,date(stock.expiry) as expiry,
    stock.rem as srem,item.sizeid,m.model_name as model $serial
    from lahead as head
    ...
    union all
    select head.docno, head.trno, head.clientname, head.address, date(head.dateid) as dateid, head.terms, head.rem as remark,
    item.barcode, item.itemname, stock.rrcost as gross, stock.cost as netamt, stock.rrqty as qty, head.yourref, head.ourref,
    stock.uom, stock.disc, stock.ext, stock.line,wh.client as wh,wh.clientname as whname,stock.loc,date(stock.expiry) as expiry,
    stock.rem as srem,item.sizeid,m.model_name as model $serial
    from (glhead as head
    ...
    public function default_RR_header_PDF($params, $data)
    {
        ...
        PDF::MultiCell(520, 0, ' Receiving Report', '', 'L', false, 0);
        PDF::SetFont($fontbold, '', $fontsize);
        ...
        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(80, 20, "Supplier : ", '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
        ...
        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(80, 20, "Ourref : ", '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
        PDF::SetFont($font, '', $fontsize);
        PDF::MultiCell(470, 20, (isset($data[0]['ourref']) ? $data[0]['ourref'] : ''), 'B', 'L', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(50, 20, "Yourref : ", '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'B', true);
        PDF::SetFont($font, '', $fontsize);
        ...
        PDF::MultiCell(92, 0, "BARCODE", '', 'C', false, 0);
        PDF::MultiCell(42, 0, "QTY", '', 'C', false, 0);
        PDF::MultiCell(42, 0, "UNIT", '', 'C', false, 0);
        PDF::MultiCell(220, 0, "DESCRIPTION", '', 'L', false, 0);
        PDF::MultiCell(104, 0, "WH", '', 'C', false, 0);
        PDF::MultiCell(70, 0, "UNIT PRICE", '', 'R', false, 0);
        PDF::MultiCell(61, 0, "(+/-) %", '', 'R', false, 0);
        PDF::MultiCell(69, 0, "TOTAL", '', 'R', false);
    ...
    public function default_RR_PDF($params, $data)
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            $maxrow = $this->othersClass->getmaxcolumn([$arr_barcode, $arr_itemname, $arr_qty, $arr_uom, $arr_amt, $arr_disc, $arr_ext, $arr_wh]);
            for ($r = 0; $r < $maxrow; $r++) {

                PDF::SetFont($font, '', $fontsize);
                PDF::MultiCell(92, 15, ' ' . (isset($arr_barcode[$r]) ? $arr_barcode[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                PDF::MultiCell(42, 15, ' ' . (isset($arr_qty[$r]) ? $arr_qty[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                PDF::MultiCell(42, 15, ' ' . (isset($arr_uom[$r]) ? $arr_uom[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                PDF::MultiCell(220, 15, ' ' . (isset($arr_itemname[$r]) ? $arr_itemname[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                PDF::MultiCell(104, 15, ' ' . (isset($arr_wh[$r]) ? $arr_wh[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                PDF::MultiCell(70, 15, ' ' . (isset($arr_amt[$r]) ? $arr_amt[$r] : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                PDF::MultiCell(61, 15, ' ' . (isset($arr_disc[$r]) ? $arr_disc[$r] : ''), '', 'R', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                PDF::MultiCell(69, 15, ' ' . (isset($arr_ext[$r]) ? $arr_ext[$r] : ''), '', 'R', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
            }
        ...
        PDF::MultiCell(560, 0, $data[0]['remark'], '', 'L');
    ...
    public function default_RC_header_PDF($params, $data)
    {
        ...
        PDF::SetFont($fontbold, '', 18);
        PDF::MultiCell(520, 0, ' Receiving Receipt', '', 'L', false, 0);
        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(80, 0, "", '', 'L', false, 0, '',  '');
        PDF::SetFont($font, '', 10);
        PDF::MultiCell(100, 0, "", '', 'L', false);
        ...
        PDF::MultiCell(42, 0, "UNIT", '', 'C', false, 0);
        PDF::MultiCell(220, 0, "DESCRIPTION", '', 'L', false, 0);
        PDF::MultiCell(105, 0, "SERIAL NO#", '', 'C', false, 0);
        PDF::MultiCell(104, 0, "WH", '', 'C', false, 0);
        PDF::MultiCell(104, 0, "NOTES", '', 'L', false);
    ...
    public function default_RC_PDF($params, $data)
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            $arr_uom = $this->reporter->fixcolumn([$uom], '13', 0);
            $arr_wh = $this->reporter->fixcolumn([$wh], '15', 0);
            $arr_serial = $this->reporter->fixcolumn([$serial], '15', 0);
            $arr_notes = $this->reporter->fixcolumn([$notes], '35', 0);

            $maxrow = $this->othersClass->getmaxcolumn([$arr_barcode, $arr_itemname, $arr_qty, $arr_uom, $arr_wh, $arr_serial, $arr_notes]);
            for ($r = 0; $r < $maxrow; $r++) {
                ...
                PDF::MultiCell(42, 15, ' ' . (isset($arr_uom[$r]) ? $arr_uom[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                PDF::MultiCell(220, 15, ' ' . (isset($arr_itemname[$r]) ? $arr_itemname[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                PDF::MultiCell(104, 15, ' ' . (isset($arr_serial[$r]) ? $arr_serial[$r] : ''), '', 'L', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                PDF::MultiCell(104, 15, ' ' . (isset($arr_wh[$r]) ? $arr_wh[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);
                PDF::MultiCell(104, 15, ' ' . (isset($arr_notes[$r]) ? $arr_notes[$r] : ''), '', 'L', false, 1, '',  '', true, 0, false, true, 0, 'M', false);
            }

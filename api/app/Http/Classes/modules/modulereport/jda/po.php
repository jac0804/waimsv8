...
class po
{
  ...
  {
    $query = "select 0 as posted, date(head.dateid) as dateid, head.docno, client.client, client.clientname, head.address,
        head.terms,head.rem, item.partno, item.barcode,
        item.itemname, stock.rrqty as qty, stock.uom, stock.rrcost as netamt, stock.disc, stock.ext,m.model_name as model,item.sizeid, head.ourref, head.vattype, head.tax
        ...
        union all
        select 1 as posted, date(head.dateid) as dateid, head.docno, client.client, client.clientname,
  ...
  public function default_PO_header_PDF($params, $data)
  {
    ...
    if (isset($data[0]['posted']) && !$data[0]['posted']) {
      PDF::SetFont($fontbold, '', 20);
      PDF::MultiCell(300, 0, "", '', 'L', false, 0);
      PDF::MultiCell(100, 0, "DRAFT", '', 'C', false, 0, '',  '');
      PDF::MultiCell(300, 0, "", '', 'L', false);
    }

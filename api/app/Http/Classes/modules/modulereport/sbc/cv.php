...
class cv
{
    ...
    public function reportplotting($config, $data)
    {
        if ($config['params']['dataparams']['print'] == "default") {
            ...
            switch ($config['params']['dataparams']['reporttype']) {
                case '0': // VOUCHER
                    $str = $this->PDF_SBC_DEFAULT_CCVOUCHER_LAYOUT1($data, $config);
                    break;
                case '1':
    ...
    public function PDF_sbc_default_header($params, $data)
    {
        ...
        PDF::MultiCell(300, 5, $this->modulename, '', 'L', false, 1);
        ...
        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(120, 5, (isset($data[0]['dateid']) ? date('m/d/Y', strtotime($data[0]['dateid'])) : ''), '', 'R', false);
        ...
        PDF::MultiCell(60, 0, 'ACCT #', '', 'L', false, 0);
        PDF::MultiCell(155, 0, 'ACCOUNT NAME', '', 'C', false, 0);
        ...
        PDF::MultiCell(310, 0, 'REMARKS', '', 'C', false);

        PDF::SetFont($font, '', 5);
        PDF::MultiCell(700, 0, '', 'B');
    } // end fn

    public function PDF_SBC_DEFAULT_CCVOUCHER_LAYOUT1($data, $params)
    {
        ...
        $this->PDF_sbc_default_header($params, $data);
        ...
        if (!empty($data)) {
            ...
            for ($i = 0; $i < count($data); $i++) {
                ...
                if (!empty($checkno) && $checkno != '0') {
                    $acnoname_with_check .= ' - ' . $checkno;
                }

                $arr_acno = $this->reporter->fixcolumn([$acno], '15', 0);
                $arr_checkno = $this->reporter->fixcolumn([$checkno], '15', 0);
                $arr_acnonamedescs = $this->reporter->fixcolumn([$acnoname_with_check], '25', 0);
                ...
                $arr_drem = $this->reporter->fixcolumn([$drem], '55', 0);

                $maxrow = $this->othersClass->getmaxcolumn([$arr_acno, $arr_acnonamedescs, $arr_pdate, $arr_debit, $arr_credit, $arr_drem]);
        ...
        PDF::SetFont($fontbold, '', $fontsize);
        PDF::MultiCell(280, 5, 'GRAND TOTAL : ', '', 'R', false, 0);
        PDF::MultiCell(60, 5, $totaldb == 0 ? '-' : number_format($totaldb, $decimal), '', 'R', false, 0);
        PDF::MultiCell(60, 5, $totalcr == 0 ? '-' : number_format($totalcr, $decimal), '', 'R', false);
        ...
        PDF::MultiCell(253, 20, $params['params']['user'], '', 'L', false, 0);
        PDF::MultiCell(253, 20, $params['params']['dataparams']['approved'], '', 'L', false, 0);
        PDF::MultiCell(253, 20, $params['params']['dataparams']['received'], '', 'L');

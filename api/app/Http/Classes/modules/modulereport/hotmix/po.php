...
namespace App\Http\Classes\modules\modulereport\hotmix;
...
class po
{
  ...
  public function report_default_query($trno)
  {
    $query = "select date(head.dateid) as dateid, head.docno, client.client, client.clientname, head.address,
        ...
        stock.disc, stock.ext, m.model_name as model, item.sizeid, wh.clientname as whname, head.wh as whcode, head.ourref
  ...
  public function default_PO_header_PDF($params, $data)
  {
    ...
    $qry = "select trno from transnum where trno = $trno and postdate is null";
    $isdraft = $this->coreFunctions->opentable($qry);
    ...
    if ((isset($isdraft[0]->trno) ? $isdraft[0]->trno : '') != '') {
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(470, 0, (isset($data[0]['whcode']) ? $data[0]['whcode'] . ' ~ ' . $data[0]['whname'] : ''), 'B', 'L', false, 0, '', '');
    PDF::SetFont($fontbold, '', $fontsize);

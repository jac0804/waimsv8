...
class pv
{
  ...
  public function PDF_default_header_ccvoucher($params,$data)
  {
    ...
    PDF::MultiCell(300, 25, "#: ", '', 'R', false, 0, '',  '');
    PDF::SetFont($font, '', 11);
  ...
  public function PDF_DEFAULT_CCVOUCHER_LAYOUT1($data,$params)
  {
    ...
    for($i=0;$i<count($data);$i++){
      ...
      $arr_acnoname = $this->reporter->fixcolumn([$acnoname.' '.$project],'100',0);

      $maxrow = $this->othersClass->getmaxcolumn([$arr_debit, $arr_credit, $arr_acnoname]);

      for($r = 0; $r < $maxrow; $r++) {
        PDF::MultiCell(500, 25, (isset($arr_acnoname[$r]) ? $arr_acnoname[$r] : ''), 'TBL', 'L', false, 0);
        PDF::MultiCell(100, 25, (isset($arr_debit[$r]) ? $arr_debit[$r] : ''), 'TBL', 'R', false, 0);
        PDF::MultiCell(100, 25, (isset($arr_credit[$r]) ? $arr_credit[$r] : ''), 'TBLR', 'R', false);
      }
    }  

    PDF::MultiCell(253, 0, 'NOTES: ', '', 'L');
    PDF::MultiCell(500, 0, (isset($data[0]['rem']) ? $data[0]['rem'] : ''), '', 'L');
  ...
  public function PDF_CV_WTAXREPORT($data,$params)
  {
    ...
    //11th row -> 4 registered address
      ...
      PDF::MultiCell(410, 30, (isset($data['head'][0]['address'])? $data['head'][0]['address']:''), 'TBLR', 'C',false,0,165,348);
      PDF::SetFont($fontbold, '', 11);
    ...
    //41st Signature Line
      ...
      PDF::MultiCell(395, 20, ucwords($params['params']['dataparams']['payor']),'B', 'R',false,0);
      PDF::MultiCell(15, 20, '','', 'R',false,0);
      PDF::MultiCell(165, 20, $params['params']['dataparams']['tin'],'B', 'R',false,0);
      PDF::MultiCell(15, 20, '','', 'R',false,0);
      PDF::MultiCell(165, 20, ucwords($params['params']['dataparams']['position']),'B', 'R',false,0);
            ...
            //Payor name box
             $str .= $this->reporter->begintable('800');
              $str .= $this->reporter->startrow();
                ...
                $str .= $this->reporter->col((isset($data['head'][0]['payorcompname'])? $data['head'][0]['payorcompname']:''),'760',null,false,'2px solid','LRTB','L','Century Gothic','10','B','','3px');
            ...
            //address name box
             $str .= $this->reporter->begintable('800');
              $str .= $this->reporter->startrow();
                ...
                $str .= $this->reporter->col('','10',null,false,'2px solid ','R','C','Century Gothic','10','','','');
                $str .= $this->reporter->col((isset($data['res'][0]['payorzipcode'])? $data['res'][0]['payorzipcode']:''),'50',null,false,'2px solid ','LRTB','C','Century Gothic','10','B','','3px');

  ...
  public function default_KR_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      ...
      for ($i = 0; $i < count($data); $i++) {
        ...
        $arr_acnoname = $this->reporter->fixcolumn([$acnoname], '20', 0);
        $arr_ref = $this->reporter->fixcolumn([$ref], '15', 0);
        $arr_postdate = $this->reporter->fixcolumn([$postdate], '13', 0);
        $arr_debit = $this->reporter->fixcolumn([$debit], '10', 0);
        $arr_credit = $this->reporter->fixcolumn([$credit], '10', 0);
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(150, 15, ' ' . (isset($arr_acnoname[$r]) ? $arr_acnoname[$r] : ''), '', 'C', false, 0, '',  '', true, 0, false, true, 0, 'M', false);

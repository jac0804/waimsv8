...
class rr
{
  ...
  public function reportplotting($params, $data)
  {
    ...
    return $this->vitaline_RR_PDF($params, $data);
  }
  ...
  public function vitaline_RR_header_PDF($params, $data)
  {
    $center = $params['params']['center'];
    $username = $params['params']['user'];
    ...
    PDF::SetFont($font, '', $fontsize);
    PDF::MultiCell(140, 0, (isset($data[0]['terms']) ? $data[0]['terms'] : ''), 'B', 'L', false, 1, '',  '');

    PDF::MultiCell(0, 0, "\n\n");
    PDF::MultiCell(740, 0, '', 'T');

    PDF::SetFont($font, 'B', 12);
    PDF::MultiCell(50, 0, "QTY", '', 'C', false, 0);
    PDF::MultiCell(50, 0, "UNIT", '', 'C', false, 0);
    PDF::MultiCell(220, 0, "DESCRIPTION", '', 'L', false, 0);
    PDF::MultiCell(130, 0, "LOT", '', 'L', false, 0);
    PDF::MultiCell(80, 0, "EXPIRY", '', 'C', false, 0);
    PDF::MultiCell(80, 0, "UNIT PRICE", '', 'R', false, 0);
    PDF::MultiCell(50, 0, "(+/-) %", '', 'R', false, 0);
    PDF::MultiCell(80, 0, "TOTAL", '', 'R', false);

    PDF::MultiCell(740, 0, '', 'B');
  }

  public function vitaline_RR_PDF($params, $data)
  {
    ...
    $this->vitaline_RR_header_PDF($params, $data);
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $arr_lot = $this->reporter->fixcolumn([$lot], '19', 0);
        $arr_expiry = $this->reporter->fixcolumn([$expiry], '13', 0);
        $arr_gross = $this->reporter->fixcolumn([$gross], '13', 0);
        $arr_disc = $this->reporter->fixcolumn([$disc], '10', 0);
        $arr_ext = $this->reporter->fixcolumn([$ext], '13', 0);

        $maxrow = $this->othersClass->getmaxcolumn([$arr_itemname, $arr_qty, $arr_uom, $arr_lot, $arr_expiry, $arr_gross, $arr_disc, $arr_ext]);
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(220, 0, (isset($arr_itemname[$r]) ? $arr_itemname[$r] : ''), '', 'L', 0, 0, '', '', true, 0, true, false);
          PDF::MultiCell(130, 0, (isset($arr_lot[$r]) ? $arr_lot[$r] : ''), '', 'L', 0, 0, '', '', true, 0, true, false);
          PDF::MultiCell(80, 0, (isset($arr_expiry[$r]) ? $arr_expiry[$r] : ''), '', 'C', 0, 0, '', '', true, 0, true, false);
          PDF::MultiCell(80, 0, (isset($arr_gross[$r]) ? $arr_gross[$r] : ''), '', 'R', 0, 0, '', '', true, 0, true, false);
          PDF::MultiCell(50, 0, (isset($arr_disc[$r]) ? $arr_disc[$r] : ''), '', 'R', 0, 0, '', '', true, 0, true, false);
          PDF::MultiCell(80, 0, (isset($arr_ext[$r]) ? $arr_ext[$r] : ''), '', 'R', 0, 1, '', '', true, 0, true, false);
        }
    ...
    PDF::MultiCell(740, 0, "", "T");
    PDF::SetFont($fontbold, '', $fontsize);
    PDF::MultiCell(640, 0, 'GRAND TOTAL: ', '', 'R', false, 0);

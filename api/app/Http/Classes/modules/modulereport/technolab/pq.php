  ...
  public function default_PQ_PDF($params, $data)
  {
    ...
    if (!empty($data)) {
      for ($i = 0; $i < count($data); $i++) {
        ...
        $arr_remarks = $this->reporter->fixcolumn([$remarks], '25', 0);

        $maxrow = $this->othersClass->getmaxcolumn([$arr_acno, $arr_acnoname, $arr_postdate, $arr_amt, $arr_remarks]);

        for ($r = 0; $r < $maxrow; $r++) {
          PDF::SetFont($font, '', $fontsize);
          PDF::MultiCell(100, 0, (isset($arr_acno[$r]) ? $arr_acno[$r] : ''), '', 'C', 0, 0, '', '', true, 0, true, false);
          PDF::MultiCell(200, 0, (isset($arr_acnoname[$r]) ? $arr_acnoname[$r] : ''), '', 'L', 0, 0, '', '', true, 0, true, false);
          ...
          PDF::MultiCell(10, 0, '', '', 'L', 0, 0, '', '', true, 0, true, false);
          PDF::MultiCell(150, 0, (isset($arr_remarks[$r]) ? $arr_remarks[$r] : ''), '', 'L', 0, 0, '', '', true, 0, true, false);

...
class en_conductgrade
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'CONDUCT GRADE';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'en_conductgrade';
  private $othersClass;
  public $style = 'width:100%;';
  public $tablelogs = 'masterfile_log';
  public $tablelogs_del = 'del_masterfile_log';
  private $fields = ['conductenglish', 'conductchinese', 'lowgrade', 'highgrade', 'encodedby', 'encodeddate'];
  ...
  public function createTab($config)
  {
    $tab = [
      $this->gridname => [
        'gridcolumns' => ['action', 'conductenglish', 'conductchinese', 'highgrade', 'lowgrade', 'encodedby', 'encodeddate']
      ]
    ];
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['line'] == 0) {
          ...
          $this->logger->sbcmasterlog($data[$key]['line'], $config, ' CREATE - ' . $data[$key]['conductenglish']);
        } else {
  ...
  public function save($config)
  {
    ...
    if ($row['line'] == 0) {
      ...
      if ($line != 0) {
        ...
        $this->logger->sbcmasterlog($row['line'], $config, ' CREATE - ' . $data['conductenglish']);
  ...
  public function delete($config)
  {
    ...
    $this->logger->sbcdelmaster_log($row['line'], $config, 'REMOVE - ' . $row['conductenglish']);
  ...
  private function PDF_default_header($data, $filters)
  {
    ...
    PDF::MultiCell(760, 20, 'CONDUCT GRADE LIST', '', 'L', false);

    PDF::SetFont($font, '', 9);
    PDF::MultiCell(760, 20, "Page " . PDF::PageNo() . "  ", '', 'L', false);

    PDF::SetFont($fontbold, '', 11);
    PDF::MultiCell(190, 20, "CONDUCT ENGLISH", 'B', 'L', false, 0);
    PDF::MultiCell(190, 20, "CONDUCT CHINESE", 'B', 'L', false, 0);
    PDF::MultiCell(190, 20, "HIGH", 'B', 'L', false, 0);
    PDF::MultiCell(190, 20, "LOW", 'B', 'L', false);
  }
  ...
  private function PDF_DEFAULT_STATUS_MASTER_LAYOUT($data, $filters)
  {
    ...
    foreach ($data as $key => $value) {
      ...
      PDF::MultiCell(190, 0, $value->conductenglish, '', 'L', false, 0);
      PDF::MultiCell(190, 0, $value->conductchinese, '', 'L', false, 0);
      PDF::MultiCell(190, 0, $value->highgrade, '', 'L', false, 0);
      PDF::MultiCell(190, 0, $value->lowgrade, '', 'L', false);

...
class entryuomlist
{
  ...
  public function save($config)
  {
    ...
    if ($row['line'] == 0) {
      $uom = $this->coreFunctions->getfieldvalue("uomlist", "uom", "uom=? and isuom=1", [$data['uom']]);
      if (!empty($uom)) {
        ...
        $this->logger->sbcmasterlog($line, $config, ' CREATE - ' . $data['uom']);
  ...
  private function loaddataperrecord($line)
  {
    ...
    $qry = "select " . $select . " from " . $this->table . " where line=? and isuom=1 order by line";
  ...
  public function delete($config)
  {
    ...
    if ($this->checkuomtransaction($row['uom'])) {
      ...
      $this->logger->sbcmasterlog($tableid, $params, ' DELETE - LINE: ' . $row['line'] . '' . ', UOM: ' . $row['uom']);
  ...
  public function saveallentry($config)
  {
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['line'] == 0) {
          $uom = $this->coreFunctions->getfieldvalue("uomlist", "uom", "uom=? and isuom=1", [$data[$key]['uom']]);
          if (!empty($uom)) {
            ...
            $this->logger->sbcmasterlog($line, $config, ' CREATE - ' . $data[$key]['uom']);
          }
        } else {
          $olduom = $this->coreFunctions->getfieldvalue("uomlist", "uom", "line=? and isuom=1", [$data[$key]['line']]);
          $uom = $this->coreFunctions->getfieldvalue("uomlist", "uom", "uom=? and isuom=1", [$data[$key]['uom']]);
          if (!empty($uom) || $this->checkuomtransaction($olduom)) {
  ...
  public function loaddata($config)
  {
    $select = $this->selectqry();
    $select = $select . ",'' as bgcolor, line ";
    $qry = "select " . $select . " from " . $this->table . " where isuom=1 order by line";
  ...
  {
    $qry = "select stock.trno from cdstock as stock left join item on item.itemid=stock.itemid where stock.uom='" . $uom . "' and stock.itemid=0
         union all
         select stock.trno from hcdstock as stock left join item on item.itemid=stock.itemid where stock.uom='" . $uom . "' and stock.itemid=0";

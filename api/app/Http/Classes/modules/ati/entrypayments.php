...
class entrypayments
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'PAYMENTS MASTERFILE';
  ...
  public function createTab($config)
  {
    $tab = [
      $this->gridname => [
        'gridcolumns' => ['action', 'paymenttype']
      ]
    ];
    ...
    $obj[0][$this->gridname]['columns'][1]['style'] = "width:500px;whiteSpace: normal;min-width:500px;";
  ...
  public function saveallentry($config)
  {
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        $data2['ispaymenttype'] = 1;

        if ($data[$key]['line'] == 0) {
          $line = $this->coreFunctions->insertGetId($this->table, $data2);
          $this->logger->sbcmasterlog($line, $config, ' CREATE - ' . $data[$key]['paymenttype']);
        } else {
    ...
    return ['status' => true, 'msg' => 'Save All saved successfully.', 'data' => $returndata];
  } // end function
  ...
  public function save($config)
  {
    ...
    $data['ispaymenttype'] = 1;

    if ($row['line'] == 0) {
      $line = $this->coreFunctions->insertGetId($this->table, $data);
      if ($line != 0) {
        $returnrow = $this->loaddataperrecord($line);
        $this->logger->sbcmasterlog($line, $config, ' CREATE - ' . $data['paymenttype']);
  ...
  public function delete($config)
  {
    ...
    // $qry = "select purpose as value from vrstock where line = ?
    //         union all 
    //         select purpose as value from hvrstock where line = ?";
    // $count = $this->coreFunctions->datareader($qry, [$line, $line]);
    // 


    $qry = "delete from " . $this->table . " where line = ?";
    $this->coreFunctions->execqry($qry, 'delete', [$line]);
    $this->logger->sbcdelmaster_log($row['line'], $config, 'REMOVE - ' . $row['paymenttype']);
  ...
  private function loaddataperrecord($line)
  {
    ...
    $qry = "select " . $select . " from " . $this->table . " where line=? and ispaymenttype=1";
  ...
  public function loaddata($config)
  {
    ...
    $select = $this->selectqry();
    $select = $select . ",'' as bgcolor ";
    $qry = "select " . $select . " from " . $this->table . " where ispaymenttype=1 " . $filtersearch . " order by line limit " . $limit;
  ...
  private function report_default_query($config)
  {
    ...
    $query = "select line, paymenttype from " . $this->table . "
      order by line";
  ...
  public function reportdata($config)
  {
    ...
    $str = $this->rpt_PDF($data, $config);
    return ['status' => true, 'msg' => 'Generating report successfully.', 'report' => $str];
  }


  private function rpt_PDF_header_PDF($data, $filters)
  {
    ...
    PDF::SetSubject($this->modulename . ' Report');
    PDF::setPageUnit('px');
    ...
    PDF::SetFont($fontbold, '', 11);
    PDF::MultiCell(600, 20, "Payments", '', 'L', false, 0);
  ...
  private function rpt_PDF($data, $filters)
  {
    $companyid = $filters['params']['companyid'];
    $decimal = $this->companysetup->getdecimal('currency',  $filters['params']);
    ...
    $this->rpt_PDF_header_PDF($data, $filters);

    for ($i = 0; $i < count($data); $i++) {
      PDF::SetFont($font, '', $fontsize);
      // MultiCell($w, $h, $txt, $border=0, $align='J', $fill=0, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0)
      PDF::MultiCell(600, 10, $data[$i]['paymenttype'], '', 'L', 0, 0, '', '', true, 0, true, false);

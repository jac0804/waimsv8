...
class dm
{
  ...
  private $otherfields = ['trno', 'isapproved', 'isreturned', 'isrefunded', 'isdeductible', 'incidentid'];
  ...
  public function createTab($access, $config)
  {
    ...
    $obj[0]['inventory']['columns'][$isqty]['label'] = 'Return Qty';
    $obj[0]['inventory']['columns'][$isqty2]['label'] = 'Monitor Qty';
  ...
  public function createHeadField($config)
  {
    ...
    array_push($fields, ['returndate', 'refunddate'], 'incidentid', 'isapproved', 'isreturned', 'isrefunded', 'isdeductible');
    ...
    data_set($col3, 'isdeductible.label', 'Deductible in PO');
  ...
  public function loadheaddata($config)
  {
    ...
    $qry = $qryselect . " from $table as head
        ...
        left join trxstatus as tr on tr.line=info.incidentid
  ...
  public function updatehead($config, $isupdate)
  {
    ...
    if ($isupdate) {
      ...
      $incidents = [];
      $incidents['incidentid'] = $head['incidentid'];
      $incidents['isapproved'] = $head['isapproved'];
      $incidents['isreturned'] = $head['isreturned'];
      $incidents['isrefunded'] = $head['isrefunded'];
      $incidents['isdeductible'] = $head['isdeductible'];


      $this->coreFunctions->sbcupdate('cntnuminfo', $incidents, ['trno' => $head['trno']]);
    } else {
  ...
  public function posttrans($config)
  {
    ...
    if ($this->companysetup->isinvonly($config['params'])) {
      ...
      $inciden = $head['head'][0]->incidentid;
      $inciden2 = $head['head'][0]->incident;
      $inciden1 = $head['head'][0]->incident;


      $refunded = $head['head'][0]->isrefunded;
      $returned = $head['head'][0]->isreturned;
      $deductible = $head['head'][0]->isdeductible;
      ...
      switch ($inciden) {
        case '90':
          if ($refunded != 1 || $returned != 1) {
            return ['trno' => $trno, 'status' => false, 'msg' => 'Posting Failed!..Required checked refunded and item returned'];
          }
          break;
        case '91':
          if ($deductible != 1) {
            return ['trno' => $trno, 'status' => false, 'msg' => 'Required check Deducted in PO, item returned'];
          }
          break;
      }
      ...
      foreach ($stock as $i => $value) {
        if (floatval($value->isqty2) <> 0) {
          return ['trno' => $trno, 'status' => false, 'msg' => 'Not allowed to post if Monitor Qty has value.'];
        }
      }
  ...
  private function getstockselect($config)
  {
    ...
    item.subcode, item.partno, round(item.dqty, " . $this->companysetup->getdecimal('qty', $config['params']) . ") as boxcount,info.ctrlno";
    return $sqlselect;
  }
  ...
  public function openstock($trno, $config)
  {
    ...
    left join glstock as rrstock on rrstock.trno=stock.refx and rrstock.line=stock.linex
    left join hstockinfotrans as info on info.trno=rrstock.reqtrno and info.line=rrstock.reqline
  ...
  public function additem($action, $config)
  {
    ...
    $isqty3 = '';
    if (isset($config['params']['data']['isqty3'])) {
    ...
    if ($action == 'insert') {
      ...
      if ($this->coreFunctions->sbcinsert($this->stock, $data) == 1) {
        $this->coreFunctions->sbcinsert('stockinfo', $datainfo); //cntnum
      ...
      if ($isnoninv == 0) {
        if ($ispallet) {
          ...
          $cost = $this->othersClass->computecosting($itemid, $whid, $data['loc'], $data['expiry'], $trno, $line, $data['iss'], $config['params']['doc'], $config['params']['companyid'], $config['params']['companyid']);
        }
        if ($cost != -1) {
          $this->coreFunctions->sbcupdate($this->stock, ['cost' => $cost], ['trno' => $trno, 'line' => $line]);
        } else {
  ...
  public function deleteitem($config)
  {
    ...
    return ['status' => true, 'msg' => 'Delete Item successful'];
    //} else {
  ...
  public function getrrsummary($config)
  {
    ...
    foreach ($config['params']['rows'] as $key => $value) {
      $qry = "
        ...
        stock.projectid,stock.reqtrno,stock.reqline,info.ctrlno
  ...
  public function getrrdetails($config)
  {
    ...
    foreach ($config['params']['rows'] as $key => $value) {
      $qry = "select head.docno, item.itemid,stock.trno,stock.line, item.barcode,stock.uom, stock.cost,
                    ...
                    ifnull(location.loc,'') as location $addfield ,stock.reqtrno,stock.reqline,info.ctrlno
              FROM glhead as head 
              left join glstock as stock on stock.trno=head.trno
              left join hstockinfotrans as info on info.trno=stock.reqtrno and info.line=stock.reqline
              left join item on item.itemid=stock.itemid
  ...
  public function distribution($params, $config)
  {
    ...
    if (!$periodic) {
      // $amtdiff = (round($params['ext'] - $params['cost'],2)) ;

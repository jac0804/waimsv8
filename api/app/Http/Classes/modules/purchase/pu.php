...
class pu
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'MATERIAL PURCHASE ORDER';
  ...
  public function loaddoclisting($config)
  {
    ...
    switch ($itemfilter) {
      ...
      case 'pending':
        ...
        $condition = ' and stock.qty>stock.qa and stock.void=0';
        break;
    ...
    $qry = "select head.trno,head.docno,head.clientname,$dateid,'" . $ustatus . "' as stat,head.createby,head.editby,head.viewby,num.postedby, date(num.postdate) as postdate,head.yourref, head.ourref  
     ...
     group by head.trno, head.docno, head.clientname, head.dateid, stat.status,
          ...
          num.postdate, head.yourref, head.ourref,stat.line
     union all
     select head.trno,head.docno,head.clientname,$dateid," . $status . " as stat,head.createby,head.editby,head.viewby, num.postedby, date(num.postdate) as postdate,head.yourref, head.ourref  
     ...
     " . $hjoin . "
     where head.doc=? and num.center=? and convert(head.dateid,DATE)>=? and CONVERT(head.dateid,DATE)<=? " . $condition . $addparams . " $filtersearch
     group by head.trno, head.docno, head.clientname, head.dateid, stat.status, head.createby, head.editby, head.viewby, num.postedby, num.postdate, head.yourref, head.ourref,stat.line 
    $orderby $limit";
  ...
  public function loadheaddata($config)
  {
    ...
    $qryselect = "select
         ...
         head.revision, head.whreceiver, ifnull(whreceiver.clientname, '') as whreceivername,head.insurance
         " . $addedfield . "  ";
  ...
  public function posttrans($config)
  {
    ...
    $qry = "insert into " . $this->hhead . "(trno,doc,docno,client,clientname,address,shipto,dateid,
      ...
      head.revision, head.rqtrno, head.deldate, head.deladdress, head.whreceiver,head.insurance
  ...
  public function getprdetails($config)
  {
    ...
    foreach ($config['params']['rows'] as $key => $value) {
      $qry = "
        ...
        (stock.qty-stock.qa) as qty,stock.rrcost,
        round((stock.qty-stock.qa)/ case when ifnull(uom.factor,0)=0 then 1 else uom.factor end," . $this->companysetup->getdecimal('qty', $config['params']) . ") as rrqty,
        stock.disc,st.line as stageid,stock.rem
  ...
  public function reportsetup($config)
  {
    ...
    return ['status' => true, 'msg' => 'Loaded Success', 'modulename' => $modulename, 'data' => $data, 'txtfield' => $txtfield, 'txtdata' => $txtdata, 'style' => $style, 'directprint' => false, 'showemailbtn' => true];
  }

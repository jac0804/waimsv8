...
class on
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'OUTRIGHT INVOICE';
  ...
  public function paramsdatalisting($config)
  {
    ...
    $fields = [];
    $allownew = $this->othersClass->checkAccess($config['params']['user'], 5535);
  ...
  public function posttrans($config)
  {
    ...
    $checkForDrTagging = $this->coreFunctions->getfieldvalue('cntnum', "svnum", "svnum=? and doc=?", [$trno, 'SJ']);
    if (floatval($checkForDrTagging) == 0) {
      return ['trno' => $trno, 'status' => false, 'msg' => 'Posting failed. Please Tag DR'];
    }
  ...
  private function getstockselect($config)
  {
    ...
    round(case when (stock.Amt>0 and stock.iss>0 and stock.Cost>0) then (((((stock.Amt * stock.ISS) - (stock.Cost * stock.Iss)) / (stock.Amt * stock.Iss)))*100) else 0 end,2) markup,
    '' as bgcolor,
    '' as errcolor,
  ...
  public function openstock($trno, $config)
  {
    ...
    warehouse.clientname,stock.rem,uom.factor,stock.noprint,stock.isqty,head.doc
    UNION ALL
    " . $sqlselect . "
  ...
  public function getdrsummaryqry($config)
  {
    ...
    $qry = "select stock.trno,head.docno,left(head.dateid,10) as dateid,
            ...
            head.agentid,head.due,head.tax,ifnull(agent.client,'') as agent,head.projectid,ifnull(wh.client,'') as wh,head.contra
            from glhead as head
  ...
  public function createdistribution($config)
  {
    ...
    $qry = "select h.doc,h.dateid,client.client,h.tax,h.contra,h.cur,h.forex,stock.ext,wh.client as wh,ifnull(item.asset,'') as asset,ifnull(item.revenue,'') as revenue,
      item.expense,stock.isamt,stock.disc,stock.isqty,stock.cost,stock.iss,stock.fcost,h.projectid,client.rev,stock.rebate,h.taxdef,h.deldate

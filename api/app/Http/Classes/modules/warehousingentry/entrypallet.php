...
class entrypallet
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'PALLET MASTERFILE';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'pallet';
  private $othersClass;
  public $style = 'width:100%;';
  private $fields = ['name', 'type', 'status'];
  ...
  {
    $tab = [$this->gridname => ['gridcolumns' => ['action', 'name', 'type',  'status', 'location']]];
  ...
  {
    $qry = "p.line, p.locid, loc.loc as location";
  ...
  private function loaddataperrecord($line)
  {
    ...
    $qry = "select " . $select . " from " . $this->table . " as p left join location as loc on loc.line=p.locid where line=?";
  ...
  public function loaddata($config)
  {
    $select = $this->selectqry();
    $select = $select . ",'' as bgcolor ";
    $qry = "select " . $select . " from " . $this->table . " as p left join location as loc on loc.line=p.locid order by p.line";
  ...
  public function reportdata($config)
  {
    ...
    $str = $this->rpt_pallet_layout($data, $config);
  ...
  private function rpt_default_header($data, $filters)
  {
    ...
    $str .= $this->reporter->col('PALLET MASTERFILE', '800', null, false, '1px solid ', '', 'L', 'Century Gothic', '18', 'B', '', '');
    ...
    $str .= $this->reporter->begintable('800');
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('Pallet Name', '400', null, false, '1px solid ', 'B', 'L', 'Century Gothic', '12', 'B', '', '2px');
    $str .= $this->reporter->endrow();
    return $str;
  }

  private function rpt_pallet_layout($data, $filters)
  {
    $companyid = $filters['params']['companyid'];
    $decimal = $this->companysetup->getdecimal('currency', $filters['params']);

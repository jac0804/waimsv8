...
class entryshowproductinquirybalance
{
    ...
    public $modulename = 'SHOW PRODUCT INQUIRY BALANCE';
    ...
    {
        $this->modulename = 'SHOW PRODUCT INQUIRY BALANCE-' . $config['params']['row']['itemname'];
        $column = ['action', 'wh', 'whname', 'loc', 'expiry', 'bal', 'uom', 'barcode'];
        ...
        if ($company == 40) { //cdo
            $stockbuttons = ['listingshowcolorbalance'];
        }
    ...
    public function loaddata($config)
    {
        ...
        ifnull(pallet.`name`,'') as pallet, ifnull(location.loc,'') as location,item.barcode
        from rrstatus
        ...
        group by wh.client,wh.clientname,rrstatus.loc,rrstatus.expiry,item.uom,il.min,il.max,pallet.`name`,location.loc,item.barcode";

...
class entrywhrem
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'Warehouse Remarks';
  ...
  {
    $attrib = array('load' => 2402, 'save' => 2403);
    return $attrib;
  }

  public function createTab($config)
  {
    $tab = [$this->gridname => ['gridcolumns' => ['action', 'rem']]];
  ...
  {
    $qry = "line, rem, (case when forreturn = 1 then 'true' else 'false' end) as forreturn ";
    return $qry;
  }
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['line'] == 0) {
          ...
          $this->logger->sbcmasterlog($line, $config, ' CREATE - ' . $data[$key]['rem']);
        } else {
  ...
  public function save($config)
  {
    ...
    if ($row['line'] == 0) {
      ...
      if ($line != 0) {
        ...
        $this->logger->sbcmasterlog($line, $config, ' CREATE - ' . $data['rem']);
  ...
  public function delete($config)
  {
    ...
    $this->logger->sbcdelmaster_log($row['line'], $config, 'REMOVE - ' . $row['rem']);
  ...
  private function report_default_query($config)
  {
    ...
    $query = "select rem as name from " . $this->table . "
    order by line";
  ...
  public function reportdata($config)
  {
    ...
    $str = $this->rpt_deliverytype_layout($data, $config);
  ...
  private function rpt_default_header($data, $filters)
  {
    ...
    $str .= $this->reporter->col('WAREHOUSE REMARKS MASTERFILE', '800', null, false, '1px solid ', '', 'L', 'Century Gothic', '18', 'B', '', '');
    ...
    $str .= $this->reporter->begintable('800');
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('Remarks', '400', null, false, '1px solid ', 'B', 'L', 'Century Gothic', '12', 'B', '', '2px');
    $str .= $this->reporter->endrow();
    return $str;
  }

  private function rpt_deliverytype_layout($data, $filters)
  {
    $companyid = $filters['params']['companyid'];
    $decimal = $this->companysetup->getdecimal('currency', $filters['params']);

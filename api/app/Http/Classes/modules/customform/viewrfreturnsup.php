...
class viewrfreturnsup
{
  ...
  public $modulename = 'Return to Supplier';
  ...
  public function createHeadField($config)
  {
    ...
    $fields = [
      ['client', 'returndate_sup'], 'clientname',
      ['lblshipping', ''], ['awb'], 'action'
    ];
    ...
    data_set($col1, 'clientname.label', 'Supplier Name');
    data_set($col1, 'client.lookupclass', 'rfsupplier');
    data_set($col1, 'lblshipping.label', 'Additional Details Supplier');
    data_set($col1, 'awb.type', 'cinput');
    data_set($col1, 'awb.maxlength', '30');
    data_set($col1, 'action.type', 'ctextarea');
    data_set($col1, 'action.maxlength', '200');

    if ($check_dateclose != "") {
      data_set($col1, 'refresh.label', 'Save');
      data_set($col1, 'returndate_sup.type', 'input');
      data_set($col1, 'returndate_cust.type', 'input');
      data_set($col1, 'returndate_sup.readonly', true);
      data_set($col1, 'returndate_cust.readonly', true);
      data_set($col1, 'returndate_supby.readonly', true);
      data_set($col1, 'returndate_custby.readonly', true);
      data_set($col1, 'awb.readonly', true);
      data_set($col1, 'dateclose.readonly', true);
      data_set($col1, 'action.readonly', true);
    }
  ...
  public function getheaddata($config)
  {
    ...
    $select = "
    select ifnull(rf.trno, 0) as trno, ifnull(client.client,'') as client, ifnull(rf.supplierid,0) as clientid, 
    ifnull(client.clientname,'') as clientname,
    ifnull(date(returndate_sup),'') as returndate_sup, 
    ifnull(rf.returndate_supby, '') as returndate_supby,
    ifnull(rf.awb, '') as awb, ifnull(rf.action, '') as action, ifnull(date(dateclose),'') as dateclose";
    ...
    switch ($config['params']['action2']) {
      ...
      default:
        $data = [
          ...
          'action' => $config['params']['dataparams']['action'],
        ];

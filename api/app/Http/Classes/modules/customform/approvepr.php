...
class approvepr
{
  ...
  {
    $allowapprove = $this->othersClass->checkAccess($config['params']['user'], 2254);
    $tab = [
      $this->gridname => [
        'gridcolumns' => ['radiostatus', 'rqty', 'rrqty', 'uom', 'itemname', 'subactid', 'subactivity', 'rem']
      ]
    ];
    ...
    $obj[0][$this->gridname]['columns'][5]['style'] = "text-align:left;width:100px;whiteSpace: normal;min-width:100px;";
    $obj[0][$this->gridname]['columns'][5]['readonly'] = true;
    $obj[0][$this->gridname]['columns'][6]['style'] = "text-align:left;width:300px;whiteSpace: normal;min-width:300px;";
    $obj[0][$this->gridname]['columns'][6]['readonly'] = true;
    $obj[0][$this->gridname]['columns'][7]['style'] = "text-align:left;width:200px;whiteSpace: normal;min-width:200px;";
  ...
  public function data($config)
  {
    ...
    $qry = "select head.trno,stock.line,head.dateid,head.docno,p.name as projectname,head.doc,
    ...
    stock.refx,stock.linex,stock.status, stock.stageid,sa.subactivity,stock.rem, bq.subactid
    from prhead as head 
    ...
    where head.doc='RQ' and head.lockdate is not null and stock.status = 0 and 
  ...
  public function loaddata($config)
  {
    ...
    foreach ($rows as $key) {
      ...
      if ($config['params']['companyid'] == 8) { //maxipro
        $path = 'App\Http\Classes\modules\construction\\rq';
  ...
  private function setserveditems($refx, $linex)
  {
    $qry1 = "select stock.qty from prhead as head left join prstock as 
      stock on stock.trno=head.trno where head.doc='RQ' and stock.refx=" . $refx . " and stock.linex=" . $linex;

    $qry1 = $qry1 . " union all select hprstock.qty from hprhead left join hprstock on hprstock.trno=
      hprhead.trno where hprhead.doc='RQ' and hprstock.refx=" . $refx . " and hprstock.linex=" . $linex;

...
class entryreassignmentcategory
{
    private $fieldClass;
    private $tabClass;
    public $modulename = 'Employee Category';
    ...
    private $fields = ['category', 'iscat', 'isreassigned'];
    ...
    {
        $qry = "head.line, head.category, head.iscat,head.isreassigned";
        return $qry;
    }
    ...
    public function delete($config)
    {
        ...
        $used = $this->coreFunctions->opentable("select docno from rashead where category=?
                union all
                select docno from hrashead where category=? limit 1", [$row['line'], $row['line']]);
    ...
    public function loaddata($config)
    {
        ...
        $qry = "select " . $select . " 
        from " . $this->table . " as head
        where isreassigned = 1  " . $filtersearch . "
    ...
    private function report_default_query($config)
    {
        ...
        $query = " select category from reqcategory";
    ...
    public function reportdata($config)
    {
        ...
        if ($config['params']['dataparams']['print'] == "default") {
            $str = $this->rpt_DEFAULT_reassignment_LAYOUT($data, $config);
        } else if ($config['params']['dataparams']['print'] == "PDFM") {
            $str = $this->rpt_reassignment_PDF($data, $config);
        }
    ...
    private function rpt_default_header($data, $filters)
    {
        ...
        $str .= $this->reporter->begintable('800');
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col('CATEGORY LIST', '600', null, false, $border, '', 'L', $font, '18', 'B', '', '');
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();
        ...
        $str .= $this->reporter->col('CATEGORY NAME', '100', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '30px', '8px');
        return $str;
    }

    private function rpt_DEFAULT_reassignment_LAYOUT($data, $filters)
    {
        $companyid = $filters['params']['companyid'];
        $decimal = $this->companysetup->getdecimal('currency', $filters['params']);
        ...
        foreach ($data as $key => $value) {
            ...
            $str .= $this->reporter->col($value->category, '50px', null, false, $border, '', 'L', $font, $fontsize, '', '', '2px');
    ...
    private function rpt_reassignment_PDF_header_PDF($data, $filters)
    {
        $companyid = $filters['params']['companyid'];
        $decimal = $this->companysetup->getdecimal('currency', $filters['params']);
        ...
        PDF::MultiCell(800, 20, "CATEGORY LIST", '', 'L', false);
    ...
    private function rpt_reassignment_PDF($data, $filters)
    {
        $companyid = $filters['params']['companyid'];
        $decimal = $this->companysetup->getdecimal('currency', $filters['params']);
        ...
        $this->rpt_reassignment_PDF_header_PDF($data, $filters);
        $i = 0;
        foreach ($data as $key => $value) {

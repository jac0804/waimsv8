...
class entryduration
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'Duration Setup';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'duration';
  private $othersClass;
  public $style = 'width:100%;';
  public $tablelogs = 'masterfile_log';
  public $tablelogs_del = 'del_masterfile_log';
  private $fields = ['duration', 'days'];
  ...
  public function createTab($config)
  {
    ...
    $tab = [
      $this->gridname => [
        'gridcolumns' => ['action', 'duration', 'days']
      ]
    ];
  ...
  public function saveallentry($config)
  {
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        $data2['duration'] = $data[$key]['duration'];
        if ($data[$key]['line'] == 0) {
          $line = $this->coreFunctions->insertGetId($this->table, $data2);
          $this->logger->sbcmasterlog($line, $config, ' CREATE - ' . $data[$key]['duration']);
        } else {
  ...
  public function save($config)
  {
    ...
    if ($row['line'] == 0) {
      ...
      if ($line != 0) {
        ...
        $this->logger->sbcmasterlog($line, $config, ' CREATE - ' . $data['duration']);
  ...
  public function delete($config)
  {
    ...
    $this->logger->sbcdelmaster_log($row['line'], $config, 'REMOVE - ' . $row['duration']);

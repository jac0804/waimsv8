...
class sj
{
  ...
  public function loaddoclisting($config)
  {
    ...
    head.createby,head.editby,head.viewby,num.postedby,
     head.yourref, head.ourref,format(sum(stock.ext),2) as total  $addrem
     ...
     group by head.trno,head.docno,head.clientname,head.dateid,
     head.createby,head.editby,head.viewby,num.postedby,
     head.yourref, head.ourref,head.lockdate  $addrem
  ...
  public function createtab2($access, $config)
  {
    ...
    if ($config['params']['companyid'] == 60) { //transpower      
      $changecode = $this->othersClass->checkAccess($config['params']['user'], 5487);
      if ($changecode) {
  ...
  public function createtabbutton($config)
  {
    ...
    if ($isserial) {
      ...
      $tbuttons = ['pendingpo', 'additem', 'quickadd', 'saveitem', 'deleteallitem', 'pendingso'];
    }
  ...
  public function createHeadField($config)
  {
    ...
    data_set($col4, 'lblgrandtotal.label', 'Agent Total');
    data_set($col4, 'lblgrandtotal.style', 'font-weight:bold; font-size:15px; display:inline-block; width:100%; text-align:center;padding:10px;');
    data_set($col4, 'totalcash.label', '');
    data_set($col4, 'creditinfo.style', 'height:8em;');
  ...
  public function loadheaddata($config)
  {
    ...
    $qryselect = "select
         ...
         hinfo.interestrate,hinfo.downpayment,  head.phaseid, ps.code as phase,  head.modelid, hm.model as housemodel, head.blklotid, 
           ...
           head.bpo, head.ctnsno, head.invoiceno,head.cmtrno,cmref.docno as cmdocno
         ";
  ...
  private function getstockselect($config)
  {
    ...
    '' as bgcolor,
    '' as errcolor,
    case when item.namt6>stock.amt then 'bg-red-2' else '' end as qacolor,
    ...
    format(stock.agentamt,2) as agentamt, stock.startwire, stock.endwire, stock.porefx, stock.polinex ,ifnull(group_concat(concat('PNP#: ',rr.pnp,' / CSR#: ',rr.csr) separator '\\n\\r'),'') as pnp,stock.color";
  ...
  public function openstock($trno, $config)
  {
    ...
    stock.refx,stock.linex,item.barcode,item.itemname, stock.agentamt, stock.startwire, stock.endwire, stock.porefx, stock.polinex, stock.uom,stock.kgs,
  ...
  public function additem($action, $config, $setlog = false)
  {
    ...
    if ($action == 'insert') {
      ...
      $agentamt = $this->othersClass->sanitizekeyfieldFast('qty', $agentamt, $lookups);
    ...
    switch ($pricetype) {
      ...
      default:
        ...
        $qry = "select docno,left(dateid,10) as dateid,round(amt," . $this->companysetup->getdecimal('price', $config['params']) . ") as amt,
              round(amt," . $this->companysetup->getdecimal('price', $config['params']) . ") as defamt,disc,uom,rem from
              (select '' as docno,left(now(),10) as dateid,namt5 as amt,uom,'' as disc,'' as rem
  ...
  public function getsosummaryqry($config)
  {
    ...
    return "
        ...
        info.driverid,info.helperid,info.checkerid,info.plateno,info.truckid,sinfo.itemdesc,head.sano,head.pono,head.wh,head.salestype,head.due,stock.agentamt,stock.rem as srem
  ...
  public function getsodetails($config)
  {
    ...
    foreach ($config['params']['rows'] as $key => $value) {
      $qry = "
        ...
        round((stock.iss-stock.qa)/ case when ifnull(uom.factor,0)=0 then 1 else uom.factor end," . $this->companysetup->getdecimal('qty', $config['params']) . ") as isqty,
        stock.disc,stock.loc,stock.expiry,stock.projectid,wh.client as swh,info.driverid,info.helperid,info.checkerid,info.plateno,stock.weight,
        sinfo.itemdesc,head.sano,head.pono,head.wh,head.due, stock.agentamt,stock.rem as srem
        FROM hsohead as head left join hsostock as stock on stock.trno=head.trno left join item on item.itemid=
  ...
  public function distribution($params, $config)
  {
    ...
    if (floatval($params['ewt']) != 0) {
      ...
      $entry = ['acnoid' => $acnoid, 'client' => $params['client'], 'db' => $ewtamt * $forex, 'cr' => 0, 'postdate' => $params['date'], 'cur' => $cur, 'forex' => $forex, 'fdb' => floatval($forex) == 1 ? 0 : $ewtamt, 'fcr' => 0, 'projectid' => $params['projectid']];
      $this->acctg = $this->othersClass->upsertdetail($this->acctg, $entry, $config);
    }
  ...
  public function getposummary($config)
  {
    ...
    foreach ($config['params']['rows'] as $key => $value) {
      ...
      if (!empty($data)) {
        //$this->coreFunctions->sbcupdate($this->head, ["rem" => $data[0]->hrem], ['trno' => $trno]);
        foreach ($data as $key2 => $value) {
          $config['params']['data']['uom'] = $data[$key2]->uom;
          $config['params']['data']['itemid'] = $data[$key2]->itemid;
          $config['params']['trno'] = $trno;
          ...
          if ($return['status']) {
            if ($this->setservedpoitems($data[$key2]->trno, $data[$key2]->line) == 0) {
              ...
              $row = $this->openstockline($config);
              $return = ['row' => $row, 'status' => true, 'msg' => 'Added'];
            }
            array_push($rows, $return['row'][0]);
          }
        } // end foreach

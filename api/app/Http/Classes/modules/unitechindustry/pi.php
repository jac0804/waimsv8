...
class pi
{
    ...
    public function loaddoclisting($config)
    {
        ...
        if (isset($config['params']['search'])) {
            $searchfield = ['head.docno', 'item.barcode', 'item.itemname', 'head.createby', 'head.editby', 'head.viewby', 'num.postedby', 'head.yourref', 'head.ourref', 'item.itemname'];
        ...
        $qry = "select head.trno, head.docno, $dateid, case ifnull(head.lockdate,'') when '' then '" . $ustatus . "' else 'Locked' end as stat, head.createby, 
                head.editby, head.viewby, num.postedby, date(num.postdate) as postdate, head.yourref, head.ourref,item.barcode, item.itemname as itemdesc
                ...
                date(num.postdate) as postdate, head.yourref, head.ourref,item.barcode, item.itemname as itemdesc
                ...
                " . $leftjoin_posted . "
                where head.doc=? and num.center=? and convert(head.dateid,DATE)>=? and CONVERT(head.dateid,DATE)<=? " . $condition . " $filtersearch
                group by head.trno, head.docno,  head.dateid, stat.status, head.createby, head.editby, head.viewby, num.postedby, num.postdate, head.yourref, 
                head.ourref,item.barcode, item.itemname 
    ...
    public function createHeadbutton($config)
    {
        ...
        $step3 = $this->helpClass->getFields(['btnadditem', 'btnquickadd', 'rrqty', 'uom', 'rrcost', 'disc', 'wh', 'rem', 'maxqty', 'btnstocksave', 'btnsaveitem']);
        $step4 = $this->helpClass->getFields(['rrqty', 'uom', 'rrcost', 'disc', 'wh', 'rem', 'maxqty', 'btnstocksave', 'btnsaveitem']);
        ...
        $buttons['others']['items']['downloadexcel'] = ['label' => 'Download PI Template', 'todo' => ['type' => 'downloadexcel', 'action' => 'downloadexcel', 'lookupclass' => 'downloadexcel', 'access' => 'view']];
        return $buttons;
    } // createHeadbutton
    ...
    public function createTab($access, $config)
    {
        ...
        $obj[0]['inventory']['columns'][$rem]['style'] = 'text-align: right; width:180px;whiteSpace: normal;min-width:150px;';
        $obj[0]['inventory']['columns'][$sku]['style'] = 'text-align: right; width:180px;whiteSpace: normal;min-width:150px;';
    ...
    public function createHeadField($config)
    {
        ...
        data_set($col1, 'ditemname.lookupclass', 'lookupitemprod');
        data_set($col1, 'luom.lookupclass', 'piuom');
        data_set($col1, 'luom.addedparams', ['itemid']);

        $fields = ['dateid', ['yourref', 'ourref'], ['totalweight', 'weight']];
    ...
    public function loadheaddata($config)
    {
        ...
        $qryselect = "select num.center,head.trno,head.docno,head.ourref,left(head.dateid,10) as dateid,
                    date_format(head.createdate,'%Y-%m-%d') as createdate,head.rem,head.yourref,
                    ifnull(i.barcode,'') as barcode,ifnull(head.qty,'') as qty,ifnull(head.uom,'') as uom, ifnull(i.itemname,'') as itemname, 
                    head.itemid, left(head.voiddate,10) as voiddate,head.weight,info.weight as totalweight";
        ...
        foreach ($rawdata as $key => $value) {
            ...
            try {
                if (floatval($rawdata[$key]['qty']) > 0) {
                    $config['params']['data']['barcode'] = $rawdata[$key][$uniquefield];
                    ...
                    if ($config['params']['data']['itemid'] == '') {
                        ...
                        if ($rawdata[$key]['uom'] == '') {
                            ...
                            $config['params']['data']['maxqty'] = $rawdata[$key]['qty'];
                            $config['params']['data']['rem'] =   isset($rawdata[$key]['notes']) ?  $rawdata[$key]['notes'] : "";
                            $config['params']['data']['rem2'] =   isset($rawdata[$key]['addnotes']) ?  $rawdata[$key]['addnotes'] : "";
    ...
    public function posttrans($config)
    {
        ...
        //for hcihead
        $qry = "insert into " . $this->hhead . "(trno,doc,docno,dateid,rem,yourref,ourref,itemid,uom,qty,
        createdate,createby,editby,editdate,lockdate,lockuser,weight)
        SELECT head.trno,head.doc, head.docno,head.dateid as dateid, head.rem,head.yourref, head.ourref,head.itemid,
        head.uom,head.qty,head.createdate,head.createby,head.editby,head.editdate, head.lockdate,head.lockuser,head.weight
        ...
        if ($posthead) {
            ...
            $qry = "insert into " . $this->hstock . "(trno,line,barcode,itemname,
                    ...
                    encodeddate,refx,linex,amenity,subamenity,maxqty,sku)
    ...
    public function unposttrans($config)
    {
        ...
        $qry = "insert into " . $this->head . "(trno,doc,docno,dateid,rem,yourref,ourref,itemid,uom,qty,
        createdate,createby,editby,editdate,lockdate,lockuser,weight)
        SELECT head.trno,head.doc, head.docno,head.dateid as dateid, head.rem,head.yourref, head.ourref,head.itemid,
        head.uom,head.qty,head.createdate,head.createby,head.editby,head.editdate, head.lockdate,head.lockuser,head.weight
    ...
    private function getstockselect($config)
    {
        ...
        ifnull(uom.factor,1) as uomfactor,stock.sku,
        '' as bgcolor";
    ...
    public function additem($action, $config)
    {
        ...
        if (isset($config['params']['data']['rem2'])) {
            $rem2 = $config['params']['data']['rem2'];
        }
        ...
        if ($config['params']['companyid'] == 50) { //unitech
            if (isset($config['params']['data']['sku'])) {
                $rem2 = $config['params']['data']['sku'];
            }
        }
        ...
        $maxqty = round($maxqty, $this->companysetup->getdecimal('maxqty', $config['params']));

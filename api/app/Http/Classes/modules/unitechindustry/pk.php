...
class pk
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'PRODUCTION RETURN';
  ...
  public function loaddoclisting($config)
  {
    ...
    if (empty($search)) $limit = 'limit 150';
  ...
  public function createHeadbutton($config)
  {
    ...
    if ($this->companysetup->getisshowmanual($config['params'])) {
      $buttons['others']['items']['manual'] = ['label' => 'View Manual', 'todo' => ['lookupclass' => 'pK', 'title' => 'Purchase Return Manual', 'action' => 'viewpdf',  'access' => 'view', 'type' => 'viewmanual']];
    }
  ...
  public function createTab($access, $config)
  {
    ...
    $obj[0]['inventory']['columns'][$ref]['lookupclass'] = 'refpK';
  ...
  {
    $tbuttons = ['additem', 'quickadd', 'saveitem', 'deleteallitem', 'pendingpn'];
  ...
  public function stockstatus($config)
  {
    switch ($config['params']['action']) {
      ...
      case 'saveperitem':
        ...
        break;
      case 'getpnsummary':
        return $this->getpnsummary($config);
        break;
      case 'getpndetails':
        return $this->getpndetails($config);
        break;
  ...
  public function diagram($config)
  {
    ...
    $qry = "
      ...
      CAST(concat('Total PK Amt: ',round(sum(stock.ext),2)) as CHAR) as rem, stock.refx
      from glhead as head
  ...
  public function setserveditems($refx, $linex)
  {
    ...
    stock on stock.trno=head.trno where head.doc='PK' and stock.refx=" . $refx . " and stock.linex=" . $linex;

    $qry1 = $qry1 . " union all select glstock." . $this->hqty . " from glhead left join glstock on glstock.trno=
    glhead.trno where glhead.doc='PK' and glstock.refx=" . $refx . " and glstock.linex=" . $linex;
  ...
  public function getpnsummary($config)
  {
    $trno = $config['params']['trno'];
    $wh = $config['params']['wh'];
    $rows = [];
  ...
  public function getpndetails($config)
  {
    $trno = $config['params']['trno'];
    $wh = $config['params']['wh'];

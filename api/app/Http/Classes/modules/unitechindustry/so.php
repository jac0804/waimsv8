...
class so
{
    ...
    public function createTab($access, $config)
    {
        ...
        $column = ['action', 'isqty', 'uom', 'kgs', 'weight', 'isamt', 'disc', 'ext', 'fstatus', 'wh', 'rem', 'loc', 'qa', 'roqa', 'void', 'ref', 'itemname', 'noprint', 'barcode'];
        $sortcolumn = ['action', 'isqty', 'uom', 'kgs', 'weight', 'isamt', 'disc', 'ext', 'fstatus', 'wh', 'rem', 'loc', 'qa', 'roqa', 'void', 'ref', 'itemname', 'noprint', 'barcode'];
    ...
    public function getlatestprice($config)
    {
        ...
        switch ($pricetype) {
            ...
            case 'CustomerGroupLatest':
                ...
                if ($pricegrp != '') {
                    ...
                    $data = $this->coreFunctions->opentable("select docno,left(dateid,10) as dateid,round(amt,2) as amt,round(amt,2) as defamt,disc,uom,factor from(
                    select 'A' as g,'" . $pricefield['label'] . "' as docno,left(now(),10) as dateid, " . $pricefield['amt'] . " as amt," . $pricefield['amt'] . " as defamt, " . $pricefield['disc'] . " as disc, uom, itemid,1 as factor from item where barcode=?
                    union all
                    select 'Z' as g,head.docno,head.dateid,
                            stock.isamt as amt,stock.isamt as defamt,stock.disc,stock.uom,item.itemid,uom.factor
                            from lahead as head
                            left join lastock as stock on stock.trno = head.trno
                            ...
                            and stock.isamt <> 0
                            UNION ALL
                            select 'Z' as g,head.docno,head.dateid,stock.isamt as amt,stock.isamt as defamt,
                            stock.disc,stock.uom,item.itemid,uom.factor from glhead as head
                            left join glstock as stock on stock.trno = head.trno
                            left join item on item.itemid = stock.itemid
            ...
            default:
                ...
                $qry = "select docno,left(dateid,10) as dateid,round(amt,2) as amt,round(amt,2) as defamt,disc,uom,factor from(select head.docno,head.dateid,
                            stock.isamt as amt,stock.uom,stock.disc,uom.factor
                            from lahead as head
                            left join lastock as stock on stock.trno = head.trno
                            left join cntnum on cntnum.trno=head.trno
                            left join item on item.itemid = stock.itemid
                            ...
                            stock.uom,stock.disc,uom.factor from glhead as head
                            left join glstock as stock on stock.trno = head.trno
                            left join item on item.itemid = stock.itemid
                            left join client on client.clientid = head.clientid
                            left join cntnum on cntnum.trno=head.trno 
        ...
        if (!empty($data)) {
            ...
            itempricehere:
            $qry = "select 'Retail Price' as docno, amt,amt as defamt,disc,uom,1 as factor from item where barcode=?";

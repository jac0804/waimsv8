...
namespace App\Http\Classes\modules\brgyentry;
...
class brgyofficial
{
    private $fieldClass;
    private $tabClass;
    public $modulename = 'BARANGAY OFFICIAL SETUP';
    ...
    private $fields = ['position', 'category', 'isbrgyoff'];
    ...
    {
        $columns = ['action', 'position', 'category'];
        ...
        $obj[0][$this->gridname]['columns'][$category]['style'] = "width:720px;whiteSpace: normal;min-width:720px;";
        $obj[0][$this->gridname]['columns'][$category]['label'] = "Name of Official";
    ...
    public function save($config)
    {
        ...
        if (trim($row['category'] == '')) {
            return ['status' => false, 'msg' => 'Name of official is empty.'];
        }

        if (trim($row['position'] == '')) {
            return ['status' => false, 'msg' => 'Position is empty.'];
        }


        if ($row['line'] == 0) {
            $qry = "select position as value from " . $this->table . " where position = '" . $data['position'] . "'";
            $checking = $this->coreFunctions->datareader($qry);

            if (!empty($checking)) {
                return ['status' => false, 'msg' => 'Position already exists. - ' . $data['position']];
            }
            ...
            if ($line != 0) {
                ...
                $this->logger->sbcmasterlog(
                    $line,
                    $config,
                    'CREATE' . ' - ' . $data['position'] . ' - ' . $data['category']
                );
            ...
            $qry = "select position as value from " . $this->table . " where position = '" . $data['position'] . "' and line = '" . $row['line'] . "'";
            $checking = $this->coreFunctions->datareader($qry);
            ...
            if (!empty($checking)) {
                ...
                if (!empty($checking1)) {
                    ...
                    return ['status' => false, 'msg' => 'Position already exists. - ' . $data['position'], 'data' => $data];
                }
            }
    ...
    public function saveallentry($config)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if ($data[$key]['bgcolor'] != '') {
                ...
                if (trim($data[$key]['position'] == '')) {
                    return ['status' => false, 'msg' => 'Position is empty.'];
                }

                if ($data[$key]['line'] == 0) {
                    $qry = "select position as value from " . $this->table . " where position = '" . $data[$key]['position'] . "'";
                    $checking = $this->coreFunctions->datareader($qry);

                    if (!empty($checking)) {
                        return ['status' => false, 'msg' => 'Position already exists. - ' . $data[$key]['position'], 'data' => $data];
                    }
                    ...
                    $this->logger->sbcmasterlog(
                        $line,
                        $config,
                        'CREATE' . ' - ' . $data[$key]['position'] . ' : ' . $data[$key]['category']
                    );
                } else {
                    $qry = "select position as value from " . $this->table . " where position = '" . $data[$key]['position'] . "' and line = '" . $data[$key]['line'] . "'";
                    $checking = $this->coreFunctions->datareader($qry);

                    if (!empty($checking)) {
                        unset($data2[$key]["position"]);
                    } else {
                        $qry = "select position as value from " . $this->table . " where position = '" . $data[$key]['position'] . "'";
                        $checking1 = $this->coreFunctions->datareader($qry);
    ...
    public function delete($config)
    {
        ...
        $this->logger->sbcdelmaster_log($row['line'], $config, 'REMOVE - ' . $row['position']);
    ...
    private function loaddataperrecord($line)
    {
        ...
        $qry = "select " . $select . " from " . $this->table . " where isbrgyoff =1 and  line=?";
    ...
    public function loaddata($config)
    {
        ...
        $select = $this->selectqry();
        $select = $select . ",'' as bgcolor ";
        $qry = "select " . $select . " from " . $this->table . "  where isbrgyoff =1   order by line";
    ...
    private function report_default_query($config)
    {
        ...
        $query = "select line, category, position from reqcategory where isbrgyoff =1  order by line";

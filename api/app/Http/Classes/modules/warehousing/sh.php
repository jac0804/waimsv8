...
class sh
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'SPECIAL PARTS ISSUANCE';
  ...
  {
    $postdate = 6;
    $getcols = ['action', 'liststatus', 'listdocument', 'listdate', 'listclientname', 'lockdate', 'postdate', 'listpostedby', 'listcreateby', 'listeditby', 'listviewby'];
    $stockbuttons = ['view'];
  ...
  public function loaddoclisting($config)
  {
    ...
    if(num.status=''," . $status . ",num.status) as status,head.createby,head.editby,head.viewby,num.postedby, date(head.lockdate) as lockdate, 
    date(num.postdate) as postdate
     ...
     num.status,head.createby,head.editby,head.viewby, num.postedby, date(head.lockdate) as lockdate, 
     date(num.postdate) as postdate
  ...
  public function createtabbutton($config)
  {
    ...
    if ($isexpiry) {
      ...
      $tbuttons = ['pendingso', 'saveitem', 'deleteallitem'];
    } else {
  ...
  public function createHeadField($config)
  {
    ...
    data_set($col1, 'client.lookupclass', 'shcustomer');
    ...
    $fields = [['yourref', 'ourref'], ['cur', 'forex'], 'partreqtype', 'deliverytype'];
    $col3 = $this->fieldClass->create($fields);
    data_set($col3, 'deliverytype.action', 'lookupdeliverytypename');
    data_set($col3, 'deliverytype.name', 'deliverytypename');

    data_set($col3, 'partreqtype.type', 'input');
    data_set($col3, 'deliverytype.type', 'input');

    data_set($col3, 'partreqtype.class', 'sbccsreadonly');
    data_set($col3, 'deliverytype.class', 'sbccsreadonly');

    data_set($col3, 'partreqtype.required', false);
    data_set($col3, 'deliverytype.required', false);
  ...
  private function getstockselect($config)
  {
    ...
    FORMAT(stock." . $this->damt . "," . $amtdec . ") as isamt,
    (case when stock.void=0 then FORMAT(stock." . $this->dqty . "," . $qtydec . ") else FORMAT(stock." . $this->dqty . " * -1," . $qtydec . ") end) as isqty,
    FORMAT(stock.ext," . $extdec . ") as ext,
  ...
  left join location on location.line=stock.locid
  left join uom on uom.itemid=item.itemid and uom.uom=stock.uom 
  left join client as warehouse on warehouse.clientid=stock.whid where stock.trno = ? and stock.line = ? ";
  ...
  public function autoadditem($config)
  {
    ...
    if ($qty != 0) {
      $qry = "select sum(rrstatus.bal) as bal,rrstatus.locid,rrstatus.palletid,client.client as wh from rrstatus left join client on client.clientid=rrstatus.whid where client.client='" . $wh . "' and rrstatus.itemid=" . $config['params']['data']['itemid'] . " and rrstatus.bal>0 and rrstatus.bal>=" . $qty . " group by rrstatus.locid,rrstatus.palletid,client.client";
      $rrstatus = $this->coreFunctions->opentable($qry);
      if (empty($rrstatus)) {
        $qry = "select sum(rrstatus.bal) as bal,rrstatus.locid,rrstatus.palletid,client.client as wh from rrstatus left join client on client.clientid=rrstatus.whid where client.client='" . $wh . "' and rrstatus.itemid=" . $config['params']['data']['itemid'] . " and rrstatus.bal>0 group by rrstatus.locid,rrstatus.palletid,client.client";
        $rrstatus = $this->coreFunctions->opentable($qry);
      }
      $rows = [];
      if (!empty($rrstatus)) {
        ...
        if ($isnoninv == 0) {
          ...
          if ($cost != -1) {
            ...
            if ($belowcost == 1) {
              ...
              $this->logger->sbcwritelog($trno, $config, 'STOCK', 'BELOW COST - Line:' . $line . ' barcode:' . $item[0]->barcode . ' Qty' . $qty . ' Amt:' . $amt . ' Disc:' . $disc . ' wh:' . $wh . ' ext:0.0' . ' Uom:' . $uom);
            ...
            $this->coreFunctions->execqry('delete from costing where trno=? and line=?', 'delete', [$trno, $line]);
            $this->logger->sbcwritelog($trno, $config, 'STOCK', 'OUT OF STOCK - Line:' . $line . ' barcode:' . $item[0]->barcode . ' Qty' . $qty . ' Amt:' . $amt . ' Disc:' . $disc . ' wh:' . $wh . ' ext:0.0' . ' Uom:' . $uom);
          }
        }
  ...
  public function setserveditems($refx, $linex)
  {
    ...
    stock on stock.trno=head.trno where head.doc='SH' and stock.refx=" . $refx . " and stock.linex=" . $linex;

    $qry1 .= " union all select glstock." . $this->hqty . " from glhead left join glstock on glstock.trno=
    glhead.trno where glhead.doc='SH' and glstock.refx=" . $refx . " and glstock.linex=" . $linex;
    ...
    $result = $this->coreFunctions->execqry("update hsgstock set qa=" . $qty . " where trno=" . $refx . " and line=" . $linex, 'update');

    $status = $this->coreFunctions->datareader("select ifnull(count(trno),0) as value from hsgstock where trno=? and iss>qa", [$refx]);

    if ($status) {
      $status = $this->coreFunctions->datareader("select ifnull(count(trno),0) as value from hsgstock where trno=? and qa<>0", [$refx]);
      if ($status) {
  ...
  public function getsosummary($config)
  {
    ...
    foreach ($config['params']['rows'] as $key => $value) {
      $qry = "
        ...
        stock.disc,stock.loc,stock.expiry, head.partreqtypeid, head.deliverytype
    ...
    ";
      ...
      if (!empty($data)) {
        ...
        $this->coreFunctions->execqry("update lahead set partreqtypeid=" . $data[0]->partreqtypeid . ", deliverytype=" . $data[0]->deliverytype . " where trno=" . $trno);

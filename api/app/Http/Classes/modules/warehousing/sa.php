...
class sa
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'SALES ORDER DEALER';
  ...
  public $head = 'sahead';
  public $hhead = 'hsahead';
  public $stock = 'sastock';
  public $hstock = 'hsastock';
  ...
  public $damt = 'isamt';
  public $hamt = 'amt';
  private $fields = ['trno', 'docno', 'dateid', 'due', 'client', 'clientname', 'yourref', 'ourref', 'rem', 'terms', 'forex', 'cur', 'wh', 'address', 'agent', 'creditinfo', 'deliverytype', 'truckid'];
  ...
  public function loaddoclisting($config)
  {
    ...
    $admin = $this->othersClass->checkAccess($config['params']['user'], 2216);
    ...
    $qry = "select head.trno,head.docno,head.clientname,left(head.dateid,10) as dateid, stat.status as stat,
    head.createby,head.editby,head.viewby,num.postedby,head.lockdate, num.postdate
     ...
     union all
     select head.trno,head.docno,head.clientname,left(head.dateid,10) as dateid,stat.status as stat,
     head.createby,head.editby,head.viewby, num.postedby, head.lockdate, num.postdate
  ...
  public function createTab($access, $config)
  {
    ...
    $sa_btnvoid_access = $this->othersClass->checkAccess($config['params']['user'], 3597);
    ...
    $tab = [
      ...
      'singleinput' => ['inputcolumn' => ['col1' => $col1], 'label' => 'CREDIT INFO'],
    ];
  ...
  {
    $isAdmin = $this->othersClass->checkAccess($config['params']['user'], 2216);

    $fields = ['docno', 'client', 'clientname', 'address', 'forwarder'];

    $col1 = $this->fieldClass->create($fields);
    if ($isAdmin) {
      data_set($col1, 'client.lookupclass', 'sacustomer');
    } else {
      data_set($col1, 'client.lookupclass', 'agentcustomer');
    }
  ...
  public function createnewtransaction($docno, $params)
  {
    ...
    $data[0]['ourref'] = '';
    $data[0]['rem'] = '';
    $data[0]['agent'] = $this->coreFunctions->datareader("select client as value from client where clientid='" . $params['adminid'] . "'");
    $data[0]['agentname'] = $this->coreFunctions->datareader("select clientname as value from client where clientid='" . $params['adminid'] . "'");
    $data[0]['dagentname'] = '';
    $data[0]['terms'] = '';
    $data[0]['forex'] = 1;
  ...
  public function additem($action, $config)
  {
    ...
    if (!empty($item)) {
      ...
      if ($action == 'insert') {
        ...
        if ($amt5 < $amt4) {
          if ($item[0]->markup > 0) {
            $config['params']['pricedate'] = $item[0]->pdate;
            $markupapplied = $this->ismarkupapplied($config);
            if (!$markupapplied) {
              $amt = $amt4 + $item[0]->markup;
            } else {
              if ($latestprice['status']) {
                $amt = $latestprice['data'][0]->amt;
              }
            }
          }
        }
  ...
  public function getlatestprice($config)
  {
    ...
    $qry = "select docno,left(dateid,10) as dateid,round(amt,2) as amt,disc,uom from(select head.docno,head.dateid,
          stock.isamt as amt,stock.uom,stock.disc,head.trno
          from lahead as head
          ...
          stock.uom,stock.disc,head.trno from glhead as head
          ...
          and stock.isamt <> 0
          order by dateid desc limit 5) as tbl order by dateid desc, trno desc limit 1";
  ...
  public function ismarkupapplied($config)
  {
    $barcode = $config['params']['barcode'];
    $client = $config['params']['client'];
    $center = $config['params']['center'];
    $trno = $config['params']['trno'];
    $pricedate = $config['params']['pricedate'];
    ...
    left join item on item.itemid = stock.itemid
    where head.doc = 'SD' and cntnum.center = ?
    and item.barcode = ? and head.client = ? and head.createdate>=?
    and stock.isamt <> 0
    UNION ALL
    select head.docno,head.dateid,stock.isamt as amt,
    ...
    left join cntnum on cntnum.trno=head.trno
    where head.doc = 'SD' and cntnum.center = ?
    and item.barcode = ? and client.client = ? and head.createdate>=?
    and stock.isamt <> 0
    order by dateid desc limit 5) as tbl order by dateid desc limit 1";
    $data = $this->coreFunctions->opentable($qry, [$center, $barcode, $client, $pricedate, $center, $barcode, $client, $pricedate]);
    if (!empty($data)) {
      return true;
    } else {
      return false;
  ...
  public function diagram($config)
  {
    ...
    $qry = "select so.trno,so.docno,left(so.dateid,10) as dateid,
     ...
     from hsahead as so 
     left join hsastock as s on s.trno = so.trno
     where so.trno = ? 
  ...
  private function report_default_query($trno)
  {
    ...
    $query = "select head.docno,head.trno, head.clientname, head.address,
      ...
      stock.uom, stock.disc, stock.ext, stock.line,item.brand,client.clientname as whname,
      item.sizeid,m.model_name as model, item.subcode, item.partno, stock.rem as stockrem
      from hsahead as head
      left join hsastock as stock on stock.trno=head.trno
      left join item on item.itemid=stock.itemid
      left join model_masterfile as m on m.model_id = item.model
      left join client on client.client=head.wh
      left join client as cust on cust.client = head.client
  ...
  public function reportplotting($params, $data)
  {
    ...
    $str .= $this->reporter->col('&nbsp&nbsp&nbsp' . 'DOCUMENT # :', '100', null, false, '1px solid ', '', 'L', 'Century Gothic', '13', 'B', '', '');
    ...
    $str .= $this->reporter->col('GRAND TOTAL :', '125px', null, false, '1px dotted ', 'T', 'R', 'Century Gothic', '12', 'B', '', '');
    $str .= $this->reporter->col('', '50px', null, false, '1px dotted ', 'T', 'R', 'Century Gothic', '12', 'B', '', '');

...
class sc
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'SALES ORDER ONLINE';
  ...
  public $head = 'schead';
  public $hhead = 'hschead';
  public $stock = 'scstock';
  public $hstock = 'hscstock';
  ...
  public $damt = 'isamt';
  public $hamt = 'amt';
  private $fields = ['trno', 'docno', 'dateid', 'due', 'client', 'clientname', 'yourref', 'ourref', 'rem', 'terms', 'tel', 'forex', 'cur', 'wh', 'address', 'agent', 'creditinfo', 'deliverytype', 'customername', 'truckid'];
  ...
  public function loaddoclisting($config)
  {
    ...
    $admin = $this->othersClass->checkAccess($config['params']['user'], 2218);
  ...
  public function createTab($access, $config)
  {
    ...
    $sc_btnvoid_access = $this->othersClass->checkAccess($config['params']['user'], 3599);
  ...
  public function createHeadField($config)
  {
    ...
    $col1 = $this->fieldClass->create($fields);
    data_set($col1, 'client.lookupclass', 'sccustomer');
    ...
    $fields = ['customername', 'tel', 'rem'];
    $col4 = $this->fieldClass->create($fields);
    data_set($col4, 'rem.required', false);
    data_set($col4, 'tel.required', true);
    data_set($col4, 'tel.label', 'Contact No.');
  ...
  public function posttrans($config)
  {
      ...
      if ($override == '0') {
        if (floatval($overdue) <> 0) {
          if (floatval($crline) < floatval($totalso) || $cstatus <> 'ACTIVE') {
            ...
            return ['status' => false, 'msg' => 'Posting failed. Due to SO disapproval, transaction cannot be posted.. Overddue: ' . $overdue . ' - Pending SO: ' . $totalso . ' - Status: ' . $cstatus];
          }
        }
      }
    }
    ...
    $qry = "insert into " . $this->hhead . "(trno,doc,docno,client,clientname,address,shipto,dateid,
      terms,rem,forex,yourref,ourref,createdate,createby,editby,editdate,lockdate,lockuser,agent,wh,due,cur,creditinfo,customername,deliverytype,tel, truckid)
      ...
      head.due,head.cur,head.creditinfo,head.customername,head.deliverytype,head.tel, head.truckid FROM " . $this->head . " as head left join cntnum on cntnum.trno=head.trno
  ...
  yourref,ourref,createdate,createby,editby,editdate,lockdate,lockuser,wh,due,cur,creditinfo,agent,customername,deliverytype,tel, truckid)
  ...
  head.customername,head.deliverytype,head.tel, head.truckid
  ...
  public function diagram($config)
  {
    ...
    $qry = "select so.trno,so.docno,left(so.dateid,10) as dateid,
     ...
     from hschead as so 
     left join hscstock as s on s.trno = so.trno
     where so.trno = ? 
  ...
  private function report_default_query($trno)
  {
    ...
    $query = "select head.docno,head.trno, head.clientname, head.address,
      ...
      head.customername, cust.bstyle, head.client
      ...
      item.sizeid,m.model_name as model, item.subcode, item.partno,stock.rem as stockrem, 
      head.customername, cust.bstyle, head.client
      from hschead as head
      left join hscstock as stock on stock.trno=head.trno
      left join item on item.itemid=stock.itemid
      left join model_masterfile as m on m.model_id = item.model
      left join client on client.client=head.wh
      left join client as cust on cust.client = head.client

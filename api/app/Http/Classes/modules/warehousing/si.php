...
class si
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'SPECIAL PARTS RETURN';
  ...
  private $fields = ['trno', 'docno', 'dateid', 'due', 'client', 'clientname', 'yourref', 'ourref', 'rem', 'terms', 'forex', 'cur', 'wh', 'address', 'contra', 'tax', 'vattype', 'projectid', 'agent'];
  ...
  public function createTab($access, $config)
  {
    ...
    $column = ['action', 'rrqty', 'uom', 'isamt', 'disc', 'ext', 'wh', 'ref', 'loc', 'expiry', 'rem', 'pallet', 'location', 'itemname', 'barcode'];
    $sortcolumn = ['action', 'rrqty', 'uom', 'isamt', 'disc', 'ext', 'wh', 'ref', 'loc', 'expiry', 'rem', 'pallet', 'location', 'itemname', 'barcode'];
  ...
  public function createtabbutton($config)
  {
    ...
    $obj[0]['label'] = 'Issuance';
    $obj[0]['lookupclass'] = 'pendingshsummary';
    $obj[0]['action'] = 'pendingshsummary';
  ...
  public function createHeadField($config)
  {
    ...
    data_set($col1, 'client.lookupclass', 'sicustomer');
  ...
  public function loadheaddata($config)
  {
    ...
    $qryselect = "select
         ...
         client.groupid,ifnull(agent.client, '') as agent,ifnull(agent.clientname, '') as agentname,'' as dagentname,ifnull(project.code,'') as projectcode  ";
  ...
  public function setserveditems($refx, $linex)
  {
    ...
    stock on stock.trno=head.trno where head.doc='SI' and stock.refx=" . $refx . " and stock.linex=" . $linex;

    $qry1 = $qry1 . " union all select glstock." . $this->hqty . " from glhead left join glstock on glstock.trno=
    glhead.trno where glhead.doc='SI' and glstock.refx=" . $refx . " and glstock.linex=" . $linex;
  ...
  public function getsjsummary($config)
  {
    ...
    foreach ($config['params']['rows'] as $key => $value) {
      $qry = "
        ...
        stock.disc,stock.fcost,stock.loc,stock.expiry
  ...
  public function createdistribution($config)
  {
    ...
    $qry = 'select head.dateid,head.client,head.tax,head.contra,head.cur,head.forex,stock.ext,wh.client as wh,ifnull(item.asset,"") as asset,ifnull(item.revenue,"") as revenue,stock.isamt,stock.cost,stock.amt,stock.disc,stock.rrqty,stock.qty,stock.fcost,head.projectid

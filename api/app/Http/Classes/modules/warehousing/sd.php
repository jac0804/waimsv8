...
class sd
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'SALES JOURNAL DEALER';
  ...
  private $stockselect;
  private $fields = ['trno', 'docno', 'dateid', 'due', 'client', 'clientname', 'yourref', 'ourref', 'rem', 'terms', 'forex', 'cur', 'wh', 'address', 'contra', 'tax', 'vattype', 'agent', 'projectid', 'creditinfo', 'deliverytype', 'customername', 'statid'];
  private $except = ['trno', 'dateid', 'due', 'creditinfo', 'truckid'];
  private $acctg = [];
  ...
  public function createdoclisting($config)
  {
    ...
    $cols[$agentname]['style'] = 'width:150px;whiteSpace: normal;min-width:150px;';
  ...
  public function loaddoclisting($config)
  {
    ...
    if (isset($config['params']['search'])) {
      $searchfield = ['head.docno', 'head.clientname', 'head.rem', 'ag.clientname', 'num.postedby', 'head.createby', 'head.editby', 'head.viewby'];
    ...
    $qry = "select head.trno,head.docno,head.clientname,left(head.dateid,10) as dateid, head.rem,
      if(num.status=''," . $status . ",num.status) as status,head.createby,head.editby,head.viewby,num.postedby,
      ifnull(ag.clientname, '') as agentname,
      (select group_concat(distinct ref SEPARATOR ', ') from lastock where trno = head.trno) as sonumber, head.lockdate, num.postdate
      ...
      union all
      select head.trno,head.docno,head.clientname,left(head.dateid,10) as dateid, head.rem,
      num.status,head.createby,head.editby,head.viewby, num.postedby,
      ifnull(ag.clientname, '') as agentname,
      (select group_concat(distinct ref SEPARATOR ', ') from glstock where trno = head.trno) as sonumber, head.lockdate, num.postdate
  ...
  public function createTab($access, $config)
  {
    ...
    $column = ['action', 'isqty', 'dqty', 'uom', 'partno', 'subcode', 'model', 'boxcount', 'amt1', 'amt2', 'amt3', 'amt4', 'amt5', 'amtstatus', 'isapprove', 'isamt', 'disc', 'ext', 'markup', 'rebate', 'wh', 'ref', 'loc', 'expiry', 'pallet', 'location', 'void', 'barcode', 'rem', 'itemname'];
    ...
    $stockbuttons = $this->companysetup->getserial($config['params']) ?  ['save', 'delete', 'serialout'] : ['save', 'delete', 'showbalance'];

    $obj = $this->tabClass->createtab($tab, $stockbuttons);

    $obj[0]['inventory']['columns'][$dqty]['label'] = 'Per Box/Pack/Set';
    $obj[0]['inventory']['columns'][$dqty]['type'] = 'label';
    $obj[0]['inventory']['columns'][$dqty]['style'] = 'text-align:center';
    ...
    $obj[0]['inventory']['columns'][$location]['action'] = 'lookuppalletbalance';
    $obj[0]['inventory']['columns'][$location]['lookupclass'] = 'palletstock';
    ...
    $obj[0][$this->gridname]['columns'][$isqty]['checkfield'] = 'ispicked';
    $obj[0][$this->gridname]['columns'][$isamt]['checkfield'] = 'ispicked';
    $obj[0][$this->gridname]['columns'][$disc]['checkfield'] = 'ispicked';
    $obj[0][$this->gridname]['columns'][$location]['checkfield'] = 'ispicked';
    $obj[0][$this->gridname]['columns'][$ref]['checkfield'] = 'ispicked';
    $obj[0][$this->gridname]['columns'][$wh]['checkfield'] = 'ispicked';
    $obj[0][$this->gridname]['columns'][$rem]['checkfield'] = 'ispicked';

    $obj[0][$this->gridname]['columns'][$action]['btns']['save']['checkfield'] = "ispicked";
    $obj[0][$this->gridname]['columns'][$action]['btns']['delete']['checkfield'] = "ispicked";
  ...
  public function createtabbutton($config)
  {
    ...
    if ($isexpiry) {
      ...
      $tbuttons = ['pendingso', 'additem', 'quickadd', 'saveitem', 'deleteallitem'];
    }
  ...
  {
    $admin = $this->othersClass->checkAccess($config['params']['user'], 2725);

    $fields = ['docno', 'client', 'clientname', 'address', 'dprojectname', ''];

    $col1 = $this->fieldClass->create($fields);
    data_set($col1, 'client.lookupclass', 'sdcustomer');
    data_set($col1, 'docno.label', 'Transaction#');
    data_set($col1, 'address.type', 'textarea');

    $fields = [['dateid', 'terms'], ['due', 'dvattype'], 'dacnoname', 'dwhname', 'forwarder'];
    $col2 = $this->fieldClass->create($fields);
    data_set($col2, 'dacnoname.label', 'AR Account');
    data_set($col2, 'dacnoname.lookupclass', 'AR');

    $fields = [['yourref', 'ourref'], ['cur', 'forex'], 'dagentname', 'deliverytype', 'statname'];
  ...
  public function loadheaddata($config)
  {
    ...
    $qryselect = "select
         ...
         head.deliverytype,head.customername,deliverytype.name as deliverytypename,
  ...
  public function deletetrans($config)
  {
    ...
    $this->coreFunctions->execqry('delete from splitstock where trno=?', 'delete', [$trno]);
  ...
  private function getstockselect($config)
  {
    ...
    FORMAT(case when stock.void=0 then stock.ext else 0 end," . $this->companysetup->getdecimal('currency', $config['params']) . ") as ext,
    ...
    item.brand,
    stock.rem,
    FORMAT(item.dqty," . $this->companysetup->getdecimal('qty', $config['params']) . ")  as dqty,
    stock.palletid,
    stock.locid,
    ...
    CASE
      ...
      ELSE ''
    END as amtstatus, IF(stock.pickerstart is not null,'true','false') as ispicked,
    IF(stock.isapprove=1,'true','false') as isapprove
    ";
    return $sqlselect;
  }
  ...
  public function openstock($trno, $config)
  {
    ...
    left join client as warehouse on warehouse.clientid=stock.whid where stock.trno =?
    UNION ALL
    " . $sqlselect . "
  ...
  public function autoadditem($config)
  {
    ...
    if ($qty != 0) {
      ...
      $rrstatus = $this->coreFunctions->opentable($qry);
      if (empty($rrstatus)) {
        $qry = "select sum(rrstatus.bal) as bal,rrstatus.locid,client.client as wh from rrstatus left join client on client.clientid=rrstatus.whid where client.client='" . $wh . "' and rrstatus.itemid=" . $config['params']['data']['itemid'] . " and rrstatus.bal>0 group by rrstatus.locid,client.client";
        $rrstatus = $this->coreFunctions->opentable($qry);
      }
      $rows = [];
      if (!empty($rrstatus)) {
        foreach ($rrstatus as $key => $value) {
          if (floatval($value->bal) >= floatval($qty)) {
            $config['params']['data']['wh'] = $value->wh;
          ...
          } elseif (floatval($value->bal) < floatval($qty)) {
            $config['params']['data']['wh'] = $value->wh;
            $config['params']['data']['locid'] = $value->locid;
            $config['params']['data']['qty'] = $value->bal;
            $return =  $this->additem('insert', $config);
            if ($return['status'] == true) {
              $qty = $qty - $value->bal;
              array_push($rows, $return['row'][0]);
        ...
        exitauto:
        return ['status' => true, 'msg' => 'Success', 'rows' => $rows, 'qty' => $qty];
      } else {
        return ['status' => false, 'msg' => 'No available Stock'];
      }
    } else {
      return ['status' => false, 'msg' => 'Zero Quantity'];
    }
  } //end function
  ...
  public function stockstatus($config)
  {
    switch ($config['params']['action']) {
      case 'autoadditem':
        return $this->autoadditem($config);
        break;
      case 'additem':
        $return =  $this->additem('insert', $config);
        if ($return['status'] == true) {
  ...
  public function additem($action, $config)
  {
    ...
    $isapprove = isset($config['params']['data']['isapprove']) ? $config['params']['data']['isapprove'] : 0;
    ...
    if ($action == 'insert') {
      ...
      if ($this->coreFunctions->sbcinsert($this->stock, $data) == 1) {
        ...
        $this->coreFunctions->sbcinsert("stockinfo", ['trno' => $trno, 'line' => $line, 'amt1' => $amt1, 'amt2' => $amt2, 'amt3' => $amt3, 'amt4' => $amt4, 'amt5' => $amt5]);
  ...
  public function deleteallitem($config)
  {
    ...
    $this->coreFunctions->execqry('delete from splitstock where trno=?', 'delete', [$trno]);
    $this->coreFunctions->execqry('delete from voidstock where trno=?', 'delete', [$trno]);
    $this->coreFunctions->execqry('delete from replacestock where trno=?', 'delete', [$trno]);
    $this->coreFunctions->execqry('delete from stockinfo where trno=?', 'delete', [$trno]);
    foreach ($data as $key => $value) {
  ...
  public function setserveditems($refx, $linex)
  {
    ...
    stock on stock.trno=head.trno where head.doc='SD' and stock.refx=" . $refx . " and stock.linex=" . $linex;

    $qry1 = $qry1 . " union all select glstock." . $this->hqty . " from glhead left join glstock on glstock.trno=
    glhead.trno where glhead.doc='SD' and glstock.refx=" . $refx . " and glstock.linex=" . $linex;
    ...
    $result = $this->coreFunctions->execqry("update hsastock set qa=" . $qty . " where trno=" . $refx . " and line=" . $linex, 'update');

    $status = $this->coreFunctions->datareader("select ifnull(count(trno),0) as value from hsastock where trno=? and iss>qa", [$refx]);
    if ($status) {
      $status = $this->coreFunctions->datareader("select ifnull(count(trno),0) as value from hsastock where trno=? and qa<>0", [$refx]);
      if ($status) {
  ...
  public function deleteitem($config)
  {
    ...
    $this->coreFunctions->execqry('delete from costing where trno=? and line=?', 'delete', [$trno, $line]);
    $this->coreFunctions->execqry('delete from splitstock where trno=? and line=?', 'delete', [$trno, $line]);
    $this->coreFunctions->execqry('delete from voidstock where trno=? and line=?', 'delete', [$trno, $line]);
  ...
  private function report_default_query($trno)
  {
    ...
    wh.client as whcode, wh.clientname as whname, item.subcode, item.partno, model.model_name
    from lahead as head
    left join lastock as stock on stock.trno=head.trno
    ...
    left join model_masterfile as model on item.model = model.model_id
  ...
  public function reportplotting($params, $data)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $str .= $this->reporter->col($data[$i]['barcode'], '50px', null, false, $border, '', 'L', $font, $fontsize, '', '', '2px');
      $str .= $this->reporter->col($data[$i]['subcode'], '50px', null, false, $border, '', 'L', $font, $fontsize, '', '', '2px');
      $str .= $this->reporter->col($data[$i]['partno'], '50px', null, false, $border, '', 'L', $font, $fontsize, '', '', '2px');
      $str .= $this->reporter->col($data[$i]['itemname'], '500px', null, false, $border, '', 'L', $font, $fontsize, '', '', '2px');
      $str .= $this->reporter->col($data[$i]['model_name'], '50px', null, false, $border, '', 'L', $font, $fontsize, '', '', '2px');
      $str .= $this->reporter->col($data[$i]['srem'], '100px', null, false, $border, '', 'L', $font, $fontsize, '', '', '2px');
    ...
    $str .= $this->reporter->col('', '50px', null, false, '1px dotted ', 'T', 'C', $font, '12', 'B', '', '');
    $str .= $this->reporter->col('', '100px', null, false, '1px dotted ', 'T', 'C', $font, '12', 'B', '', '');
    $str .= $this->reporter->col('GRAND TOTAL :', '125px', null, false, '1px dotted ', 'T', 'R', $font, '12', 'B', '', '');
    $str .= $this->reporter->col('', '50px', null, false, '1px dotted ', 'T', 'R', $font, '12', 'B', '', '');
  ...
  private function report_default_header($params, $data)
  {
    ...
    $str .= $this->reporter->col('&nbsp&nbsp&nbsp' . 'DOCUMENT # :', '100', null, false, $border, '', 'L', $font, '13', 'B', '', '');
    ...
    $str .= $this->reporter->col('TERMS : ', '40', null, false, $border, '', 'L', $font, '12', 'B', '', '');
    $str .= $this->reporter->col((isset($data[0]['terms']) ? $data[0]['terms'] : ''), '160', null, false, $border, 'B', 'R', $font, '12', '', '', '');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();
    ...
    $str .= $this->reporter->col('SKU', '50px', null, false, $border, 'B', 'C', $font, '12', 'B', '30px', '8px');
    $str .= $this->reporter->col('OLD SKU', '50px', null, false, $border, 'B', 'C', $font, '12', 'B', '30px', '8px');
    $str .= $this->reporter->col('PART-NO', '50px', null, false, $border, 'B', 'C', $font, '12', 'B', '30px', '8px');
    $str .= $this->reporter->col('D E S C R I P T I O N', '500px', null, false, $border, 'B', 'C', $font, '12', 'B', '30px', '8px');
    $str .= $this->reporter->col('MODEL', '50px', null, false, $border, 'B', 'C', $font, '12', 'B', '30px', '8px');
    $str .= $this->reporter->col('REMARKS', '100px', null, false, $border, 'B', 'C', $font, '12', 'B', '30px', '8px');
  ...
  private function vitaline_sireport($params, $data)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      if ($x > $i) {
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->addline();
        $str .= $this->reporter->col("<div style='margin-top: -20px;'>" . $this->reporter->line . "-" . number_format($data[$i]['qty'], $this->companysetup->getdecimal('qty', $params['params'])) . "</div>", '100', null, false, $border, '', 'C', $font, $fontsize, '', '', '2px');
        $str .= $this->reporter->col("<div style='margin-top: -20px;'>" . $data[$i]['uom'] . "</div>", '75', null, false, $border, '', 'C', $font, $fontsize, '', '', '2px');
        $str .= $this->reporter->col($data[$i]['itemname'], '450', null, false, $border, '', 'L', $font, $fontsize, '', '', '2px');
    ...
    $str .= $this->reporter->begintable();
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('&nbsp' . $this->reporter->line . "-", '100', null, false, '1px dotted ', '', 'C', $font, $fontsize, 'B', '', '');

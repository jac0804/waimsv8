...
class se
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'SALES JOURNAL BRANCH';
  ...
  public function loaddoclisting($config)
  {
    ...
    if ($userid == 0) {
      return ['data' => [], 'status' => false, 'msg' => 'Sorry, you`re not allowed to create transaction. Please setup first your Customer Code.'];
    }
    ...
    if (isset($config['params']['search'])) {
      $searchfield = ['head.docno', 'head.clientname', 'ag.clientname', 'head.rem', 'num.postedby', 'head.createby', 'head.editby', 'head.viewby'];
    ...
    (select group_concat(distinct ref SEPARATOR ', ') from lastock where trno = head.trno) as sonumber,
    head.lockdate, num.postdate,
    ifnull(ag.clientname, '') as agentname
     ...
     (select group_concat(distinct ref SEPARATOR ', ') from glstock where trno = head.trno) as sonumber,
     head.lockdate, num.postdate,
     ifnull(ag.clientname, '') as agentname
  ...
  public function createTab($access, $config)
  {
    ...
    $column = ['action', 'isqty', 'dqty', 'uom', 'partno', 'subcode', 'model', 'boxcount', 'amt1', 'amt2', 'amt3', 'amt4', 'amt5', 'amtstatus', 'isamt', 'disc', 'ext', 'markup', 'rebate', 'wh', 'ref', 'loc', 'expiry', 'pallet', 'location', 'void', 'itemname', 'barcode', 'rem'];
    $sortcolumn = ['action', 'isqty', 'dqty', 'uom', 'partno', 'subcode', 'model', 'boxcount', 'amt1', 'amt2', 'amt3', 'amt4', 'amt5', 'amtstatus', 'isamt', 'disc', 'ext', 'markup', 'rebate', 'wh', 'ref', 'loc', 'expiry', 'pallet', 'location', 'void', 'itemname', 'barcode', 'rem'];
    ...
    if ($viewcost == '0') {
      $obj[0]['inventory']['columns'][$markup]['type'] = 'coldel';  //markup
    }

    $obj[0]['inventory']['columns'][$ref]['lookupclass'] = 'refrr';
    $obj[0]['inventory']['columns'][$rebate]['type'] = 'coldel';  //rebate

    $obj[0]['inventory']['columns'][$loc]['type'] = 'coldel';  //loc
    $obj[0]['inventory']['columns'][$expiry]['type'] = 'coldel';  //expiry
  ...
  public function createtabbutton($config)
  {
    ...
    if ($isexpiry) {
      ...
      $tbuttons = ['poserial', 'saveitem', 'deleteallitem'];
    } else {
  ...
  public function createHeadField($config)
  {
    ...
    data_set($col1, 'client.lookupclass', 'secustomer');
    data_set($col1, 'docno.label', 'Transaction#');
    data_set($col1, 'address.type', 'textarea');

    $fields = [['dateid', 'terms'], ['due', 'dvattype'], 'dacnoname', 'dprojectname', 'dwhname', 'customername'];
    $col2 = $this->fieldClass->create($fields);
    data_set($col2, 'dacnoname.label', 'AR Account');
    data_set($col2, 'dacnoname.lookupclass', 'AR');

    $fields = [['yourref', 'ourref'], ['cur', 'forex'], 'dagentname', 'deliverytype', 'forwarder', 'statname'];
  ...
  public function setserveditems($refx, $linex)
  {
    ...
    stock on stock.trno=head.trno where head.doc='SE' and stock.refx=" . $refx . " and stock.linex=" . $linex;

    $qry1 = $qry1 . " union all select glstock." . $this->hqty . " from glhead left join glstock on glstock.trno=
    glhead.trno where glhead.doc='SE' and glstock.refx=" . $refx . " and glstock.linex=" . $linex;
    ...
    $result = $this->coreFunctions->execqry("update hsbstock set qa=" . $qty . " where trno=" . $refx . " and line=" . $linex, 'update');

    $status = $this->coreFunctions->datareader("select ifnull(count(trno),0) as value from hsbstock where trno=? and iss>qa", [$refx]);
    if ($status) {
      $status = $this->coreFunctions->datareader("select ifnull(count(trno),0) as value from hsbstock where trno=? and qa<>0", [$refx]);
      if ($status) {
  ...
  private function report_default_header($params, $data)
  {
    ...
    $str .= $this->reporter->col('&nbsp&nbsp' . 'DOCUMENT # :', '100', null, false, $border, '', 'L', $font, '13', 'B', '', '');

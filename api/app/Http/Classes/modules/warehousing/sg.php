...
class sg
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'SPECIAL PARTS REQUEST';
  ...
  public $head = 'sghead';
  public $hhead = 'hsghead';
  public $stock = 'sgstock';
  public $hstock = 'hsgstock';
  ...
  public function createdoclisting($config)
  {
    ...
    $postdate = 6;
    $getcols = ['action', 'liststatus', 'listdocument', 'listdate', 'listclientname', 'partreqtype', 'lockdate', 'postdate', 'listpostedby', 'listcreateby', 'listeditby', 'listviewby'];
    $stockbuttons = ['view'];
  ...
  public function loaddoclisting($config)
  {
    ...
    if (isset($config['params']['search'])) {
      $searchfield = ['head.docno', 'head.clientname', 'partrequest.name', 'num.postedby', 'head.createby', 'head.editby', 'head.viewby'];
    ...
    $admin = $this->othersClass->checkAccess($config['params']['user'], 2219);
    ...
    $qry = "select head.trno,head.docno,head.clientname,left(head.dateid,10) as dateid, head.partreqtypeid, partrequest.name as partreqtype,
    (case when head.lockdate is not null then 'LOCKED' else 'DRAFT' end) as status, date(head.lockdate), date(num.postdate) as postdate, 
    head.createby,head.editby,head.viewby,num.postedby  
     from " . $this->head . " as head left join " . $this->tablenum . " as num 
     on num.trno=head.trno left join client as agent on agent.client=head.agent 
     left join partrequest on partrequest.line = head.partreqtypeid
     where head.doc=? and num.center=? and CONVERT(head.dateid,DATE)>=? and CONVERT(head.dateid,DATE)<=? " . $condition . $filteruser .  "  " . $filtersearch . "
     union all
     select head.trno,head.docno,head.clientname,left(head.dateid,10) as dateid, head.partreqtypeid, partrequest.name as partreqtype,
     'POSTED' as status,date(head.lockdate), date(num.postdate) as postdate, head.createby,head.editby,head.viewby, num.postedby  
  ...
  {
    $sg_btnvoid_access = $this->othersClass->checkAccess($config['params']['user'], 3622);
    $fields = ['creditinfo'];
  ...
  public function createHeadField($config)
  {
    ...
    $fields = [['dateid', 'terms'], 'due', 'dwhname', 'partreqtype'];
  ...
  public function createnewtransaction($docno, $params)
  {
    ...
    select client.client, client.clientname, client.addr, client.terms, ifnull(terms.days,0) as days 
    from client left join terms on terms.terms = client.terms where client.clientid=" . $params['adminid']);
    ...
    $data[0]['due'] = $this->othersClass->getDueDate(date('Y-m-d'), $client[0]->days);
    $data[0]['yourref'] = '';
    $data[0]['shipto'] = '';
    $data[0]['ourref'] = '';
    $data[0]['rem'] = '';
  ...
  public function loadheaddata($config)
  {
    ...
    $qryselect = "select 
         ...
         head.partreqtypeid, partrequest.name as partreqtype,
  ...
  public function posttrans($config)
  {
    ...
    $qry = "insert into " . $this->hhead . "(trno,doc,docno,client,clientname,address,shipto,dateid,
      terms,rem,forex,yourref,ourref,createdate,createby,editby,editdate,lockdate,lockuser,agent,wh,
      due,cur,creditinfo,customername,deliverytype,partreqtypeid)
      ...
      head.due,head.cur,head.creditinfo,head.customername,head.deliverytype,head.partreqtypeid
  ...
  yourref,ourref,createdate,createby,editby,editdate,lockdate,lockuser,wh,due,cur,creditinfo,agent,customername,deliverytype,partreqtypeid)
  ...
  head.customername,head.deliverytype,head.partreqtypeid
  ...
  public function diagram($config)
  {
    ...
    $qry = "select so.trno,so.docno,left(so.dateid,10) as dateid,
     CAST(concat('Total SG Amt: ',round(sum(s.ext),2)) as CHAR) as rem
     from hsghead as so 
     left join hsgstock as s on s.trno = so.trno
     where so.trno = ? 
    ...
    if (!empty($t)) {
      ...
      foreach ($t as $key => $value) {
        ...
        //WA
        $qry = "select wahead.trno,wahead.docno,left(wahead.dateid,10) as dateid,
        CAST(concat('Total WA Amt: ',round(sum(s.ext),2)) as CHAR) as rem, wahead.trno
        from wahead as wahead 
        left join wastock as s on s.trno = wahead.trno
        where s.refx = ? 
        group by wahead.trno,wahead.docno,wahead.dateid, wahead.trno
        union all
        select wahead.trno,wahead.docno,left(wahead.dateid,10) as dateid,
        CAST(concat('Total WA Amt: ',round(sum(s.ext),2)) as CHAR) as rem, wahead.trno
        from hwahead as wahead 
        left join hwastock as s on s.trno = wahead.trno
        where s.refx = ? 
        group by wahead.trno,wahead.docno,wahead.dateid, wahead.trno";
        $wadata = $this->coreFunctions->opentable($qry, [$config['params']['trno'], $config['params']['trno']]);
        if (!empty($wadata)) {
          foreach ($wadata as $wkey => $wvalue) {
            data_set(
              $nodes,
              $wadata[$wkey]->docno,
              [
                ...
                'w' => 250,
                'h' => 80,
                'type' => $wadata[$wkey]->docno,
                'label' => $wadata[$wkey]->rem,
                'color' => 'orange',
                'details' => [$wadata[$wkey]->dateid]
              ]
            );
            array_push($links, ['from' => $wadata[$wkey]->docno, 'to' => $t[$key]->docno]);

            //WB
            $qry = "select head.trno,head.docno,left(head.dateid,10) as dateid,
            CAST(concat('Total WB Amt: ',round(sum(s.ext),2)) as CHAR) as rem, head.trno
            from lahead as head 
            left join lastock as s on s.trno = head.trno
            where s.refx = ? and head.doc = 'WB'
            group by head.trno,head.docno,head.dateid, head.trno
            union all
            ...
            where s.refx = ? and head.doc = 'WB'
            group by head.trno,head.docno,head.dateid, head.trno";
            $wbdata = $this->coreFunctions->opentable($qry, [$wadata[$wkey]->trno, $wadata[$wkey]->trno]);
            if (!empty($wbdata)) {
              foreach ($t as $wbkey => $wbvalue) {
                data_set(
                  $nodes,
                  $wbdata[$wbkey]->docno,
                  [
                ...
                array_push($links, ['from' => $wbdata[$wbkey]->docno, 'to' => $wadata[$wkey]->docno]);
              }
            }
          }
        }
    ...
    CAST(concat('Total SH Amt: ', round(sum(stock.ext),2), ' - ', 'Balance: ', round(ar.bal, 2)) as CHAR) as rem, 
    head.trno
    from glhead as head
    ...
    select head.docno,
    date(head.dateid) as dateid,
    CAST(concat('Total SH Amt: ', round(sum(stock.ext),2), ' - ', 'Balance: ', round(sum(stock.ext),2)) as CHAR) as rem, 
    head.trno
    from lahead as head
  ...
  {
    $trno = $config['params']['trno'];
    $query = "select createby,lockuser from sghead
              where trno = $trno
              union
              select createby,lockuser from hsghead
              where trno = $trno";
    $data = json_decode(json_encode($this->coreFunctions->opentable($query)), true);
    $createby = $data[0]['createby'];
    $lockedby = $data[0]['lockuser'];
    return $this->coreFunctions->opentable(
      "select 
      'default' as print,
      '$createby' as prepared,
      '$lockedby' as approved,
      '' as received
      "
    );
  }
  ...
  private function report_default_query($trno)
  {
    ...
    $query = "select cust.tel2,cust.email,head.docno,head.trno, head.clientname, head.address, 
      ...
      item.sizeid,m.model_name as model,partreq.name as partreqname
      ...
      left join partrequest as partreq on partreq.line = head.partreqtypeid
  ...
  public function reportplotting($params, $data)
  {
    ...
    $str .= $this->reporter->col('SPECIAL PARTS REQUEST ' . '(' . $data[0]['partreqname'] . ')', '600', null, false, '1px solid ', '', 'L', 'Century Gothic', '18', 'B', '', '');

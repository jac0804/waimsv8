...
class sf
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'SALES JOURNAL ONLINE';
  ...
  private $stockselect;
  private $fields = ['trno', 'docno', 'dateid', 'due', 'client', 'clientname', 'yourref', 'ourref', 'rem', 'tel', 'terms', 'forex', 'cur', 'wh', 'address', 'contra', 'tax', 'vattype', 'agent', 'projectid', 'creditinfo', 'deliverytype', 'customername', 'statid'];
  ...
  public function loaddoclisting($config)
  {
    ...
    if(num.status=''," . $status . ",num.status) as status,head.createby,head.editby,head.viewby,num.postedby, head.lockdate,head.rem,num.postdate,
    ifnull(ag.clientname, '') as agentname
     ...
     num.status,head.createby,head.editby,head.viewby, num.postedby, head.lockdate,head.rem, num.postdate,
     ifnull(ag.clientname, '') as agentname
  ...
  public function createHeadField($config)
  {
    ...
    data_set($col1, 'client.lookupclass', 'sfcustomer');
    ...
    $fields = [['yourref', 'ourref'], ['cur', 'forex'], 'dagentname', 'deliverytype', 'tel', 'statname'];
    ...
    data_set($col3, 'tel.required', true);
    data_set($col3, 'tel.label', 'Contact No.');
  ...
  public function setserveditems($refx, $linex)
  {
    ...
    stock on stock.trno=head.trno where head.doc='SF' and stock.refx=" . $refx . " and stock.linex=" . $linex;

    $qry1 = $qry1 . " union all select glstock." . $this->hqty . " from glhead left join glstock on glstock.trno=
    glhead.trno where glhead.doc='SF' and glstock.refx=" . $refx . " and glstock.linex=" . $linex;
    ...
    $result = $this->coreFunctions->execqry("update hscstock set qa=" . $qty . " where trno=" . $refx . " and line=" . $linex, 'update');

    $status = $this->coreFunctions->datareader("select ifnull(count(trno),0) as value from hscstock where trno=? and iss>qa", [$refx]);
    if ($status) {
      $status = $this->coreFunctions->datareader("select ifnull(count(trno),0) as value from hscstock where trno=? and qa<>0", [$refx]);
      if ($status) {
  ...
  public function getlatestprice($config)
  {
    ...
    $qry = "select docno,left(dateid,10) as dateid,round(amt,2) as amt,disc,uom from(select head.docno,head.dateid,
          ...
          left join item on item.itemid = stock.itemid
          where head.doc = 'SF' and cntnum.center = ?
  ...
  private function report_default_query($trno)
  {
    ...
    wh.client as whcode, wh.clientname as whname, item.subcode, item.partno, model.model_name, client.bstyle,head.customername
    from lahead as head
    left join lastock as stock on stock.trno=head.trno
  ...
  private function report_default_header($params, $data)
  {
    ...
    if ($data[0]['bstyle'] == "Online Sale") {
      $str .= $this->reporter->col((isset($data[0]['customername']) ? $data[0]['customername'] : ''), '520', null, false, '1px solid ', 'B', 'L', 'Century Gothic', '12', '', '30px', '4px');
    } else {

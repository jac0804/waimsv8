...
class py
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'Payment Listing';
  ...
  private $fields = ['trno', 'docno', 'dateid', 'client', 'clientname', 'yourref', 'ourref', 'rem', 'address'];
  ...
  public function createTab($access, $config)
  {
    ...
    $obj[0]['accounting']['columns'][5]['label'] = 'Supplier';
  ...
  {
    $tbuttons = ['unpaidpy', 'deleteallitem'];
  ...
  {
    $selectqry = "ctbl.client,ledger.docno as ref,ledger.trno as artrno, ledger.py as trno,
        ledger.line,ledger.acnoid,coa.acno,coa.acnoname,cntnum.center,ledger.clientid,
          ...
          ifnull(head.ourref,'') as yourref ,detail.rem,'' as bgcolor,'' as errcolor from apledger as ledger";
    ...
    $qry = "select " . $selectqry . "
          ...
          where ledger.py = ? order by postdate";
  ...
  {
    $trno = $config['params']['trno'];
    $data = $this->coreFunctions->opentable('select trno,line from apledger where py=? ', [$trno]);

    foreach ($data as $key => $value) {
      $this->coreFunctions->execqry('update apledger set py =0  where py=?', 'update', [$trno]);
    }
  ...
  public function deleteitem($config)
  {
    ...
    $krtrno = $this->coreFunctions->getfieldvalue("apledger", "py", "trno=? and line=?", [$trno, $line]);
    $data = $this->opendetailline($trno, $line, $config);
    $qry = "update apledger set py =0 where trno = ? and line =?";
    $this->coreFunctions->execqry($qry, 'update', [$trno, $line]);
    $data = json_decode(json_encode($data), true);
    $this->logger->sbcwritelog($krtrno, $config, 'ACCTG', 'REMOVED PL- Line:' . $line . ' code:' . $data[0]['acno'] . ' db:' . $data[0]['db'] . ' cr:' . $data[0]['cr'] . ' client:' . $data[0]['client'] . ' date:' . $data[0]['postdate'] . ' ref:' . $data[0]['ref']);

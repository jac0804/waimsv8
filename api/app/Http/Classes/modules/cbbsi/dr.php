...
class dr
{
  ...
  public function createHeadField($config)
  {
    ...
    $fields = [['dateid', 'terms'], 'due', 'dwhname', 'trnxtype'];
    $col2 = $this->fieldClass->create($fields);
      ...
      if (floatval($islimit) == 0) {
        if ($override == '0') {
          ...
          if (floatval($crline) < floatval($totalso)) {
            ...
            return ['status' => false, 'msg' => 'Posting failed. Account Overdue / Credit Limit Exceeded.'];
          }
        }
      }
  ...
  public function setserveditems($refx, $linex)
  {
    ...
    stock on stock.trno=head.trno where head.doc in ('SJ','BO','DR') and stock.refx=" . $refx . " and stock.linex=" . $linex;

    $qry1 = $qry1 . " union all select glstock." . $this->hqty . " from glhead left join glstock on glstock.trno=
    glhead.trno where glhead.doc in ('SJ','BO','DR') and glstock.refx=" . $refx . " and glstock.linex=" . $linex;
  ...
  public function getposummaryqry($config)
  {
    return "
        ...
        info.driverid,info.helperid,info.checkerid,info.plateno,info.truckid,sinfo.itemdesc,head.sano,head.pono,head.wh,head.salestype,head.due,info.trnxtype

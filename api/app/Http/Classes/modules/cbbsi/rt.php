...
class rt
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'TEMPORARY RR';
  ...
  public function createHeadField($config)
  {
    ...
    $fields = [['dateid', 'terms'], ['due', 'dvattype'], 'whname', 'waybill'];
    ...
    data_set($col2, 'waybill.type', 'cinput');
  ...
  public function unposttrans($config)
  {
    $trno = $config['params']['trno'];
    $user = $config['params']['user'];
    $qry = "select trno from " . $this->hstock . " where trno=? and qa>0";
    ...
    if ($this->coreFunctions->execqry($qry, 'insert', [$trno])) {
      ...
      cost,qty,rrcost,rrqty,ext,encodeddate,qa,encodedby,editdate,editby,sku,refx,linex)
      select trno, line, itemid, uom,whid,loc,ref,disc,cost, qty, rrcost, rrqty,
      ext, encodeddate, qa, encodedby, editdate, editby,sku,refx,linex
  ...
  private function getstockselect($config)
  {
    ...
    stock.uom,
    stock.cost,
    stock.qty as qty,item.itemname,
    ...
    warehouse.clientname as whname,
    stock.loc,
    item.brand, stock.stageid,st.stage,

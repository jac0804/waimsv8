...
class ka
{
  ...
  public function createTab($access, $config)
  {
    ...
    $obj[0]['accounting']['columns'][3]['readonly'] = true;
    $obj[0]['accounting']['columns'][4]['readonly'] = true;
    $obj[0]['accounting']['columns'][6]['type'] = 'input';
    $obj[0]['accounting']['columns'][5]['label'] = 'Customer';
    $obj[0]['accounting']['columns'][5]['type'] = 'input';
  ...
  {
    $tbuttons = ['unpaidka', 'deleteallitem'];
  ...
  public function posttrans($config)
  {
    ...
    $qry = "insert into " . $this->hhead . "(trno,doc,docno,shipto,dateid,rem,forex,createdate,createby,editby,editdate,lockdate,lockuser,agent,wh,due,cur)
                    SELECT head.trno,head.doc, head.docno,head.shipto,head.dateid as dateid,  head.rem, head.forex,
                    head.createdate,head.createby,head.editby,head.editdate, head.lockdate,head.lockuser,head.agent,head.wh,
                    head.due,head.cur FROM " . $this->head . " as head 
  ...
  public function unposttrans($config)
  {
    ...
    $user = $config['params']['user'];
    $docno = $this->coreFunctions->datareader('select docno as value from ' . $this->tablenum . ' where trno=?', [$trno]);
    $qry = "insert into " . $this->head . "(trno,doc,docno,shipto,dateid,rem,forex,createdate,createby,editby,editdate,lockdate,lockuser,agent,wh,due,cur)
                    SELECT head.trno,head.doc, head.docno,head.shipto, head.dateid as dateid, head.rem, head.forex,
                    head.createdate,head.createby,head.editby,head.editdate, head.lockdate,head.lockuser,head.agent,head.wh,
                    head.due,head.cur FROM " . $this->hhead . " as head 
                    where head.trno=? limit 1";
    $posthead = $this->coreFunctions->execqry($qry, 'insert', [$trno]);
  ...
  public function opendetail($trno, $config)
  {
    $selectqry = "ctbl.client,ledger.docno as ref,ledger.trno as artrno, ledger.kr as trno,
        ledger.line,ledger.acnoid,coa.acno,coa.acnoname,cntnum.center,ledger.clientid,
          ...
          ifnull(head.ourref,'') as yourref ,head.rem,'' as bgcolor,'' as errcolor from arledger as ledger";

    $qry = "select " . $selectqry . "
          ...
          where ledger.ka = ? order by postdate";
  ...
  {
    $trno = $config['params']['trno'];
    $data = $this->coreFunctions->opentable('select trno,line from arledger where ka=? ', [$trno]);

    foreach ($data as $key => $value) {
      $this->coreFunctions->execqry('update arledger set ka =0  where ka=?', 'update', [$trno]);
    }
  ...
  public function deleteitem($config)
  {
    ...
    $krtrno = $this->coreFunctions->getfieldvalue("arledger", "ka", "trno=? and line=?", [$trno, $line]);
    $data = $this->opendetailline($trno, $line, $config);
    $qry = "update arledger set ka =0 where trno = ? and line =?";
    $this->coreFunctions->execqry($qry, 'update', [$trno, $line]);
    $data = json_decode(json_encode($data), true);
    $this->logger->sbcwritelog($krtrno, $config, 'ACCTG', 'REMOVED KA- Line:' . $line . ' code:' . $data[0]['acno'] . ' db:' . $data[0]['db'] . ' cr:' . $data[0]['cr'] . ' client:' . $data[0]['client'] . ' date:' . $data[0]['postdate'] . ' ref:' . $data[0]['ref']);
  ...
  public function getunpaidselected($config)
  {
    ...
    foreach ($data as $key => $value) {
      $qry = "update arledger set ka = " . $trno . " where trno = ? and line =?";

...
class ck
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'REQUEST FOR SALES RETURN';
  ...
  public $head = 'ckhead';
  public $hhead = 'hckhead';
  public $stock = 'ckstock';
  public $hstock = 'hckstock';
  ...
  public $damt = 'isamt';
  public $hamt = 'amt';
  public $fields = ['trno', 'docno', 'dateid', 'due', 'client', 'clientname', 'yourref', 'ourref', 'rem', 'contra', 'terms', 'forex', 'cur', 'wh', 'address', 'agent', 'creditinfo', 'projectid', 'ms_freight', 'mlcp_freight', 'shipto', 'sano', 'pono', 'salestype', 'statid'];
  ...
  public function loaddoclisting($config)
  {
    ...
    $qry = "select head.trno,head.docno,head.clientname,left(head.dateid,10) as dateid, case ifnull(head.lockdate,'') when '' then  'DRAFT' else 'LOCKED' end as status,
    head.createby,head.editby,head.viewby,num.postedby,head.createdate,num.postdate,
     head.yourref, head.ourref,case ifnull(head.lockdate,'') when '' then 'red' else 'green' end as statuscolor  $lfield
     ...
     head.createby,head.editby,head.viewby, num.postedby,head.createdate,num.postdate,
      head.yourref, head.ourref,'grey' as statuscolor $gfield
  ...
  public function paramsdatalisting($config)
  {
    ...
    $fields = [];
    $allownew = $this->othersClass->checkAccess($config['params']['user'], 4268);
    if ($allownew == '1') $fields = ['pickpo'];
    $col1 = $this->fieldClass->create($fields);
    data_set($col1, 'pickpo.label', 'pick dr');
    data_set($col1, 'pickpo.lookupclass', 'drrfshortcut');
  ...
  public function createTab($access, $config)
  {
    ...
    $column = ['action', 'isqty', 'uom', 'kgs', 'weight', 'isamt', 'disc', 'ext', 'fstatus', 'wh', 'rem', 'loc', 'qa', 'void', 'ref', 'itemname', 'barcode'];
    $sortcolumn = ['action', 'isqty', 'uom', 'kgs', 'weight', 'isamt', 'disc', 'ext', 'fstatus', 'wh', 'rem', 'loc', 'qa', 'void', 'ref', 'itemname', 'barcode'];
  ...
  public function createHeadField($config)
  {
    ...
    // col 2
    $fields = [['dateid', 'terms'], 'due', 'dacnoname', 'dagentname', 'dwhname'];
    $col2 = $this->fieldClass->create($fields);
  ...
  public function posttrans($config)
  {
    ...
    $qry = "insert into " . $this->hhead . "(trno,doc,docno,client,clientname,address,shipto,dateid,
      terms,rem,forex,yourref,ourref,createdate,createby,editby,editdate,lockdate,lockuser,agent,wh,due,cur,creditinfo,crline,overdue, projectid,mlcp_freight,ms_freight,sano,pono,statid,contra" . $addfield . ")
      ...
      head.mlcp_freight,head.ms_freight,head.sano,head.pono,head.statid,head.contra" . $addfieldfilter . "
    ...
    if ($posthead) {
      ...
      $qry = "insert into " . $this->hstock . "(trno,line,itemid,uom,
        ...
        encodeddate,encodedby,editdate,editby,refx,linex,rem,ref,weight,weight2" . $addsfield . ")
        SELECT trno, line, itemid, uom,whid,loc,expiry,disc, iss,void,isamt,amt, isqty, ext,kgs,
        encodeddate, encodedby,editdate,editby,refx,linex,rem,ref,weight,weight2 " . $addsfield . " FROM " . $this->stock . " where trno =?";
  ...
  public function unposttrans($config)
  {
    ...
    yourref,ourref,createdate,createby,editby,editdate,lockdate,lockuser,wh,due,cur,creditinfo,crline,overdue,agent, projectid,mlcp_freight,ms_freight,sano,pono,statid,contra " . $addfield . ")
    ...
    head.projectid,head.mlcp_freight,head.ms_freight,head.sano,head.pono,head.statid,head.contra " . $addfieldfilter . "
    ...
    if ($this->coreFunctions->execqry($qry, 'insert', [$trno])) {
      ...
      amt,iss,void,isamt,isqty,ext,kgs,rem,encodeddate,encodedby,editdate,editby,refx,linex,ref,weight,weight2 " . $addsfield . ")
      select trno, line, itemid, uom,whid,loc,expiry,disc,amt, iss,void, isamt, isqty,
      ext,kgs,ifnull(rem,''), encodeddate,encodedby, editdate, editby,refx,linex,ref,weight,weight2" . $addsfield . "
    ...
    foreach ($rawdata as $key => $value) {
      try {
        ...
        if (isset($rawdata[$key]['helper'])) {
          ...
          if ($helperid == '') {
            $status = false;
            $msg .= 'Failed to upload. Helter ' . $rawdata[$key]['helper'] . ' does not exist. ';
            continue;
          }
        } else {
  ...
  public function additem($action, $config, $setlog = false)
  {
    ...
    if ($action == 'insert') {
      ...
      $return = true;
      $msg = '';
      $this->coreFunctions->execqry("update " . $this->stock . " set itemid='" . $data['itemid'] . "', isamt='" . $data['isamt'] . "', amt='" . $data['amt'] . "', isqty='" . $data['isqty'] . "', iss='" . $data['iss'] . "', ext='" . $data['ext'] . "', kgs='" . $data['kgs'] . "', disc='" . $data['disc'] . "', whid='" . $data['whid'] . "', loc='" . $data['loc'] . "', void='" . $data['void'] . "', uom='" . $data['uom'] . "', rem='" . $data['rem'] . "', refx='" . $data['refx'] . "', linex='" . $data['linex'] . "', expiry='" . $data['expiry'] . "', ref='" . $data['ref'] . "', fstatus='" . $data['fstatus'] . "' where trno='" . $data['trno'] . "' and line='" . $data['line'] . "'", 'update');
  ...
  public function getdrsummary($config)
  {
    ...
    foreach ($config['params']['rows'] as $key => $value) {
      $qry = "
        ...
        (stock.iss-stock.qa) as iss,stock.isamt,
        round((stock.iss-(stock.ckqa+stock.qa))/ case when ifnull(uom.factor,0)=0 then 1 else uom.factor end," . $this->companysetup->getdecimal('qty', $config['params']) . ") as isqty,

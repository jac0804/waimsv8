...
class sk
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'SALES INVOICE';
  ...
  private $stockselect;
  private $fields = ['trno', 'docno', 'dateid', 'due', 'client', 'clientname', 'yourref', 'ourref', 'rem', 'terms', 'forex', 'cur', 'wh', 'address', 'contra', 'tax', 'vattype', 'agent', 'projectid', 'creditinfo', 'billid', 'shipid', 'branch', 'deptid', 'taxdef', 'billcontactid', 'shipcontactid', 'ms_freight', 'mlcp_freight', 'shipto', 'salestype', 'sotrno', 'statid', 'deldate', 'crref', 'trnxtype'];
  ...
  public function paramsdatalisting($config)
  {
    ...
    $fields = [];
    $allownew = $this->othersClass->checkAccess($config['params']['user'], 4314);
    if ($allownew == '1') $fields = ['pickpo'];
    $col1 = $this->fieldClass->create($fields);
    data_set($col1, 'pickpo.label', 'pick dr');
    data_set($col1, 'pickpo.lookupclass', 'drtaggedshortcut');
    ...
    $prefix = $this->coreFunctions->getfieldvalue('profile', 'pvalue', 'doc=? and psection=?', ['SED', 'SK']);
    if ($prefix != '') {
  ...
  public function createTab($access, $config)
  {
    ...
    $column = ['action', 'itemdescription', 'serialno', 'isqty', 'uom', 'kgs', 'isamt', 'disc', 'ext', 'itemstatus', 'cost', 'markup', 'rebate', 'gprofit', 'wh', 'whname', 'ref', 'loc', 'expiry', 'rem', 'itemname', 'stock_projectname', 'noprint', 'barcode'];
    $sortcolumn = ['action', 'itemdescription', 'serialno', 'isqty', 'uom', 'kgs', 'isamt', 'disc', 'ext', 'itemstatus', 'cost', 'markup', 'rebate', 'gprofit', 'wh', 'whname', 'ref', 'loc', 'expiry', 'rem', 'itemname', 'stock_projectname', 'noprint', 'barcode'];
    ...
    $tab['multigrid2'] = ['action' => 'tableentry', 'lookupclass' => 'viewdrstockinfo', 'label' => 'DR'];
    $tab['multigrid'] = ['action' => 'tableentry', 'lookupclass' => 'viewdnstockinfo', 'label' => 'DR Return'];
    $tab['multigrid3'] = ['action' => 'tableentry', 'lookupclass' => 'viewsostockinfo', 'label' => 'SO'];
  ...
  public function createtabbutton($config)
  {
    ...
    $tbuttons = [];
    array_push($tbuttons, 'drtagged', 'sotagged');
  ...
  public function createHeadField($config)
  {
    ...
    if ($inv) {
      ...
      $fields = [['dateid', 'terms'], 'due', 'dacnoname', 'dwhname', 'trnxtype'];
    }
  ...
  public function deletetrans($config)
  {
    ...
    $this->coreFunctions->sbcupdate('cntnum', ['svnum' => 0], ['svnum' => $trno]);
    $this->coreFunctions->sbcupdate('transnum', ['sitagging' => 0], ['sitagging' => $trno]);
  ...
  public function posttrans($config)
  {
    ...
    $checkForDrTagging = $this->coreFunctions->getfieldvalue('cntnum', "svnum", "svnum=? and doc=?", [$trno, 'DR']);
    $checkForSoTagging = $this->coreFunctions->getfieldvalue('transnum', "sitagging", "sitagging=?", [$trno]);
    if (floatval($checkForDrTagging) == 0 && floatval($checkForSoTagging) == 0) {
      return ['trno' => $trno, 'status' => false, 'msg' => 'Posting failed. Please Tag DR or SO.'];
    }
  ...
  private function getstockselect($config)
  {
    ...
    stock.kgs,
    stock." . $this->hamt . ",
    case head.doc when 'DR' then stock." . $this->hqty . " else stock.qty*-1 end as iss,
    FORMAT(stock." . $this->damt . "," . $this->companysetup->getdecimal('price', $config['params']) . ") as isamt,
    FORMAT(case head.doc when 'DR' then stock." . $this->dqty . " else stock.rrqty*-1 end," . $qty_dec . ")  as isqty,
    FORMAT(stock." . $this->dqty . "," . $qty_dec . ")  as qty,
    FORMAT(case head.doc when 'DR' then stock.ext else stock.ext*-1 end," . $this->companysetup->getdecimal('currency', $config['params']) . ") as ext,
  ...
  public function openstock($trno, $config)
  {
    ...
    stock.ext ,
    stock.encodeddate,stock.disc,stock.void,head.docno,stock.whid,warehouse.client,
    ...
    prj.name,stock.projectid,stock.sgdrate,stock.noprint,brand.brand_desc,i.itemdescription,stock.itemstatus, stock.isqty,head.doc,stock.qty,stock.rrqty
    UNION ALL
    " . $sqlselect . "
    ...
    '' as location,
    ifnull(uom.factor,1) as uomfactor,
    0 as  markup,0 as rebate,
    0 as gprofit,
    '' as bgcolor,
    '' as errcolor,
    prj.name as stock_projectname,
    stock.projectid as projectid,0 as sgdrate,'' as itemstatus,
    'false' as noprint,
    concat(item.itemname,'\\n',ifnull(brand.brand_desc,''),'\\r\\n',ifnull(mm.model_name,''),'\\r\\n',ifnull(i.itemdescription,'')) as itemdescription
    ...
    warehouse.clientname,stock.loc,stock.expiry,stock.rem,
    uom.factor,head.forex,
    prj.name,stock.projectid,brand.brand_desc,i.itemdescription, stock.isqty order by sortline, line";

    $this->coreFunctions->LogConsole('QRY ' . $this->othersClass->getCurrentTimeStamp() . '; ');
  ...
  public function openstockline($config)
  {
    ...
    warehouse.clientname,stock.loc,stock.expiry,stock.rem,stock.palletid,stock.locid,
    pallet.name,location.loc,uom.factor,head.forex,stock.rebate,
    prj.name,stock.projectid,stock.sgdrate,stock.noprint,brand.brand_desc,i.itemdescription,stock.itemstatus, stock.isqty";
  ...
  public function stockstatus($config)
  {
    switch ($config['params']['action']) {
      case 'tagdr':
        return $this->tagdr($config);
        break;
      case 'tagdn':
        return $this->tagdn($config);
        break;
      case 'tagso':
        return $this->tagso($config);
        break;
      default:
  ...
  public function tagdr($config)
  {

    $sktrno = $config['params']['trno'];
    foreach ($config['params']['rows'] as $key => $value) {
      $drtrno = $value['keyid'];
      $this->coreFunctions->sbcupdate('cntnum', ['svnum' => $sktrno], ['trno' => $drtrno]);
      $dn = $this->coreFunctions->opentable("select distinct trno from glstock where refx = " . $drtrno);
      if (!empty($dn)) {
        foreach ($dn as $k => $v) {
          $this->coreFunctions->sbcupdate('cntnum', ['svnum' => $sktrno], ['trno' => $dn[$k]->trno]);
        }
      }
    }
    return ['status' => true, 'msg' => 'Tagged DR Successfully...', 'reloadhead' => true];
  }

  public function tagdn($config)
  {
    $sktrno = $config['params']['trno'];
    foreach ($config['params']['rows'] as $key => $value) {
      ...
      $this->coreFunctions->sbcupdate('cntnum', ['svnum' => $sktrno], ['trno' => $dntrno]);
    }
    $stock = $this->openstock($sktrno, $config);
    return ['status' => true, 'msg' => 'Tagged DN Successfully...', 'inventory' => $stock, 'reloadhead' => true];
  }

  public function tagso($config)
  {
    ...
    foreach ($config['params']['rows'] as $key => $value) {
      ...
      $this->coreFunctions->sbcupdate('transnum', ['sitagging' => $sktrno], ['trno' => $sotrno]);
    }
    return ['status' => true, 'msg' => 'Tagged SO Successfully...', 'reloadhead' => true];
  }
  ...
  public function getdrsummaryqry($config)
  {
    ...
    $qry = "select stock.trno,head.docno,left(head.dateid,10) as dateid,
            client.client,client.clientname,head.address,head.shipto,ifnull(p.name,'') as projectname,
            head.yourref,head.ourref,head.terms,head.trnxtype,head.cur,head.forex,head.rem,head.vattype,
            head.agentid,head.due,head.tax,agent.client as agent,head.projectid,wh.client as wh,head.contra
            from glhead as head
            ...
            left join " . $this->hinfohead . " as info on info.trno = head.trno
            where head.doc in ('DR') and cntnum.svnum=0
            and stock.void = 0 and head.trno =?
            group by stock.trno,head.docno,head.dateid,client.client,client.clientname,head.address,head.shipto,p.name,head.yourref,head.ourref,head.terms,head.trnxtype,
            head.cur,head.forex,head.rem,head.vattype,head.agentid,head.due,head.tax,agent.client,head.projectid,wh.client,head.contra";
    return $qry;
  }
  ...
  public function additem($action, $config, $setlog = false)
  {
    ...
    if ($action == 'insert') {
      ...
      if ($this->coreFunctions->sbcinsert($this->stock, $data) == 1) {
        ...
        $this->logger->sbcwritelog($trno, $config, 'STOCK', 'ADD - Line:' . $line . ' barcode:' . $item[0]->barcode . ' Uom:' . $uom . ' Qty' . $qty . ' Amt:' . $amt . ' Disc:' . $disc . ' wh:' . $wh . ' ext:' . $computedata['ext'], $setlog ? $this->tablelogs : '');
        if ($isnoninv == 0) {
          if ($ispallet) {
  ...
  public function createdistribution($config)
  {
    ...
    $qry = "select h.doc,h.dateid,client.client,h.tax,h.contra,h.cur,h.forex,case head.doc when 'DR' then stock.ext else stock.ext*-1 end as ext,wh.client as wh,ifnull(item.asset,'') as asset,ifnull(item.revenue,'') as revenue,
      item.expense,stock.isamt,stock.disc,case head.doc when 'DR' then stock.isqty else stock.rrqty*-1 end as isqty,stock.cost,stock.iss,stock.fcost,h.projectid,client.rev,stock.rebate,h.taxdef,h.deldate
      from glhead as head  left join glstock as stock on stock.trno=head.trno left join cntnum as num on num.trno = head.trno left join lahead as h on h.trno = num.svnum
      left join item on item.itemid=stock.itemid left join client on client.clientid = head.clientid left join client as wh on wh.clientid = stock.whid where num.svnum=" . $trno . "
      union all
      select h.doc,h.dateid,client.client,h.tax,h.contra,h.cur,h.forex,stock.ext,wh.client as wh,ifnull(item.asset,'') as asset,ifnull(item.revenue,'') as revenue,
      item.expense,stock.isamt,stock.disc,stock.isqty,0 as cost,stock.iss,0 as fcost,h.projectid,client.rev,0 as rebate,h.taxdef,h.deldate
      from hsohead as head left join hsostock as stock on stock.trno=head.trno left join transnum as num on num.trno = head.trno left join lahead as h on h.trno = num.sitagging
      left join item on item.itemid=stock.itemid left join client on client.client = head.client left join client as wh on wh.clientid = stock.whid where num.sitagging=" . $trno;

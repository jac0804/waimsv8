...
class aj
{
  ...
  private $fields = ['trno', 'docno', 'dateid', 'wh', 'yourref', 'ourref', 'rem', 'contra', 'projectid', 'branch', 'deptid', 'whref', 'client', 'clientname', 'trnxtype'];
  ...
  public function createTab($access, $config)
  {
    ...
    $column =  ['action', 'itemdescription', 'serialno', 'rrqty', 'uom', 'kgs', 'rrcost', 'ext', 'wh', 'whname', 'ref', 'loc', 'expiry', 'rem', 'pallet', 'location', 'itemname', 'stock_projectname', 'subcode', 'partno', 'boxcount', 'barcode'];
    $sortcolumn =  ['action', 'itemdescription', 'serialno', 'rrqty', 'uom', 'kgs', 'rrcost', 'ext', 'wh', 'whname', 'ref', 'loc', 'expiry', 'rem', 'pallet', 'location', 'itemname', 'stock_projectname', 'subcode', 'partno', 'boxcount', 'barcode'];
  ...
  public function createHeadField($config)
  {
    ...
    $fields = [['yourref', 'ourref'], 'trnxtype'];
  ...
  public function createnewtransaction($docno, $params)
  {
    ...
    if ($params['companyid'] == 39) $data[0]['contra'] = $this->coreFunctions->getfieldvalue('coa', 'acno', 'alias=?', ['CG1']);
  ...
  public function loadheaddata($config)
  {
    ...
    $qryselect = "select
         ...
         ifnull(d.clientname,'') as deptname,head.deptid,'' as ddeptname,'' as dwhref, whref.clientname as whnameref, head.whref, head.trnxtype ";
  ...
  public function posttrans($config)
  {
    ...
    if ($return['status']) {
      $trnxtype = $this->coreFunctions->getfieldvalue($this->hhead, 'trnxtype', ['trno' => $trno]);
      if ($trnxtype == 'Recosting') {
        $hstock = $this->coreFunctions->opentable("select itemid, cost from glstock where trno=" . $trno);
        if (!empty($hstock)) {
          foreach ($hstock as $s) {
            $this->coreFunctions->execqry("update item set amt8=amt9, amt9='" . $s->cost . "' where itemid='" . $s->itemid . "'", 'update');
          }
        }
      }
    }
  ...
  private function getstockselect($config)
  {
    ...
    concat(item.itemname,'\\n',ifnull(brand.brand_desc,''),'\\r\\n',ifnull(mm.model_name,''),'\\r\\n',ifnull(i.itemdescription,'')) as itemdescription,case stock.iss when 0 then ifnull(group_concat(rr.serial separator '\\r\\n'),'') else ifnull(group_concat(rr2.serial separator '\\r\\n'),'') end as serialno";
    return $sqlselect;
  }
  ...
  public function openstock($trno, $config)
  {
    ...
    stock.disc,stock.void,stock.iss,stock.qa,stock.ref,stock.whid,warehouse.client,warehouse.clientname,stock.loc,
    ...
    stock.disc,stock.void,stock.iss,stock.qa,stock.ref,stock.whid,warehouse.client,warehouse.clientname,stock.loc,
    stock.expiry,stock.palletid,stock.locid,pallet.name,location.loc,stock.rem,stock.loc,uom.factor,
    prj.name,stock.projectid, item.subcode, item.partno, item.dqty,brand.brand_desc,i.itemdescription order by sortline,line";
  ...
  public function openstockline($config)
  {
   ...
   FROM $this->stock as stock
    ...
    stock.disc,stock.void,stock.iss,stock.qa,stock.ref,stock.whid,warehouse.client,warehouse.clientname,stock.loc,
    stock.expiry,stock.palletid,stock.locid,pallet.name,location.loc,stock.rem,stock.loc,uom.factor,
    prj.name,stock.projectid, item.subcode, item.partno, item.dqty,brand.brand_desc,i.itemdescription,stock.sortline";
  ...
  public function createdistribution($config)
  {
    ...
    $qry = 'select head.dateid,head.client,head.tax,head.contra,head.cur,head.forex,((stock.qty-stock.iss) * stock.cost) as ext,wh.client as wh,ifnull(item.asset,"") as asset,ifnull(item.revenue,"") as revenue,stock.rrcost,stock.disc,stock.rrqty,head.projectid
  ...
  public function distribution($params, $config)
  {
    ...
    $cur = $params['cur'];
    $invamt = ($params['ext'] - $params['tax']); //+ $params['discamt'];
    //AP

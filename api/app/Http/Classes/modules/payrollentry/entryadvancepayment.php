...
class entryadvancepayment
{
  ...
  private $tablesetup = 'standardsetupadv';
  ...
  public function save($config)
  {
    ...
    $bal = $this->coreFunctions->datareader("select balance as value from standardsetupadv where trno=" . $data['trno']);
    ...
    if ($data['cr'] <= $bal) {
      if ($row['line'] == 0) {
        ...
        if ($line) {
          $this->coreFunctions->execqry("update standardsetupadv set balance = balance - " . $data['cr'] . " where trno=" . $data['trno']);
        ...
        $prevamt = $this->coreFunctions->getfieldvalue($this->table, "cr", "trno=? and line=?", [$row['trno'], $row['line']]);
        $this->coreFunctions->execqry("update standardsetupadv set balance = balance + " . $prevamt . " where trno=" . $row['trno']);

        $data['editdate'] = $this->othersClass->getCurrentTimeStamp();
        $data['editby'] = $config['params']['user'];
        if ($this->coreFunctions->sbcupdate($this->table, $data, ['trno' => $row['trno'], 'line' => $row['line']]) == 1) {
          $this->coreFunctions->execqry("update standardsetupadv set balance = balance - " . $row['cr'] . " 
                  where trno=" . $row['trno'] . "");
          ...
          //   $data['trno'],
          //   $config,
          //   "UPDATE MANUAL PAYMENT - LINE: ". $data['line'] . " DOCUMENT#: ". $data['docno'] . ", DATE: " . $data['dateid']. ", PAYMENT: " . $data['cr'],
          //   1 // isedit
          // );
  ...
  public function saveallentry($config)
  {
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        $bal = $this->coreFunctions->datareader("select balance as value from standardsetupadv where trno=" . $data[$key]['trno']);
        ...
        if ($data[$key]['cr'] <= $bal) {
          if ($data[$key]['line'] == 0) {
            ...
            if ($line) {
              $this->coreFunctions->execqry("update standardsetupadv set balance = balance - " . $data[$key]['cr'] . " where trno=" . $data[$key]['trno']);
            ...
            $this->coreFunctions->execqry("update standardsetupadv set balance = balance + " . $prevamt . " where trno=" . $data[$key]['trno']);

            $data2['editdate'] = $this->othersClass->getCurrentTimeStamp();
            $data2['editby'] = $config['params']['user'];
            if ($this->coreFunctions->sbcupdate($this->table, $data2, ['line' => $data[$key]['line']])) {
              $this->coreFunctions->execqry("update standardsetupadv set balance = balance - " . $data[$key]['cr'] . " 
  ...
  public function delete($config)
  {
    ...
    $this->coreFunctions->execqry("update standardsetupadv set balance = balance + " . $prevamt . " where trno=" . $trno);
  ...
  private function loaddataperrecord($trno, $line, $config)
  {
    ...
    $qry = "select " . $select . " 
        from " . $this->table . " 
        where trno=? and line=? and ismanual = ?";

...
class viewemploans
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'LOANS';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'standardsetup';
  private $othersClass;
  ...
  public function createTab($config)
  {
    ...
    switch (strtolower($config['params']['doc'])) {
      ...
      case 'myinfo':
        ...
        $obj[0][$this->gridname]['columns'][$balance]['align'] = "right";
        $obj[0][$this->gridname]['columns'][$amortization]['align'] = "right";
        ...
        $obj[0][$this->gridname]['columns'][$effdate]['style'] = "width:100px;whiteSpace: normal;min-width:100px;";
        // $obj[0][$this->gridname]['columns'][$paymentdate]['style'] = "width:100px;whiteSpace: normal;min-width:100px;";    
  ...
  public function loaddata($config){
    ...
    $qry = "select empid,trno,docno,dateid,effdate,amt,cr,amortization,acnoname, (amt - ifnull(cr,0)) as balance from (
            select ss.empid,ss.trno, ss.docno, date(ss.dateid) as dateid,date(ss.effdate) as effdate,
                   ss.amt, sum(strans.cr) as cr,ss.amortization, p.codename acnoname
            from ".$this->table." as ss 
            left join standardtrans as strans on ss.trno = strans.trno
            left join paccount as p on p.line=ss.acnoid 
            where ss.empid=? and p.alias in ('LOAN', 'HDMFLOAN', 'SSSLOAN', 'DEDUCTION')

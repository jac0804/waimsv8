...
class entrypurpose
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'PURPOSE MASTERFILE';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'purpose_masterfile';
  private $othersClass;
  public $style = 'width:100%;';
  public $tablelogs = 'masterfile_log';
  public $tablelogs_del = 'del_masterfile_log';
  private $fields = ['purpose'];
  ...
  public function createTab($config)
  {
    ...
    $obj[0][$this->gridname]['columns'][1]['style'] = "width:800px;whiteSpace: normal;min-width:800px;";
    return $obj;
  }
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['line'] == 0) {
          ...
          $this->logger->sbcmasterlog($modelid, $config, ' CREATE - ' . $data[$key]['purpose']);
        } else {
  ...
  public function save($config)
  {
    ...
    if ($row['line'] == 0) {
      ...
      if ($line != 0) {
        ...
        $this->logger->sbcmasterlog($line, $config, ' CREATE - ' . $data['purpose']);
  ...
  public function delete($config)
  {
    ...
    if ($count != '') {
      return ['clientid' => $row['purpose'], 'status' => false, 'msg' => 'Already have transaction...'];
    }

    $qry = "delete from " . $this->table . " where line = ?";
    $this->coreFunctions->execqry($qry, 'delete', [$line]);
    $this->logger->sbcdelmaster_log($row['line'], $config, 'REMOVE - ' . $row['purpose']);
  ...
  public function loaddata($config)
  {
    ...
    $select = $this->selectqry();
    $select = $select . ",'' as bgcolor ";
    $qry = "select " . $select . " from " . $this->table . " where 1=1 " . $filtersearch . " order by line limit " . $limit;
  ...
  private function report_default_query($config)
  {
    ...
    $query = "select line, purpose from " . $this->table . "
      order by line";
  ...
  private function rpt_PDF_header_PDF($data, $filters)
  {
    ...
    PDF::SetFont($fontbold, '', 11);
    PDF::MultiCell(600, 20, "Purpose", '', 'L', false, 0);
  ...
  private function rpt_PDF($data, $filters)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      PDF::MultiCell(600, 10, $data[$i]['purpose'], '', 'L', 0, 0, '', '', true, 0, true, false);

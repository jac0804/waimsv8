...
class passenger
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'PASSENGER LEDGER';
  ...
  public $head = 'client';
  public $prefix = 'PL'; // driver ledger
  ...
  private $fields = [
    ...
    'contact', 'rem', 'type',
    'iscustomer', 'issupplier', 'isagent', 'iswarehouse', 'isinactive', 'isemployee', 'isdepartment', 'picture', 'isdriver', 'ispassenger',
    'isadmin', 'uv_ischecker', 'uv_ispicker', 'deptid', 'position', 'empid', 'salesgroupid', 'branchid'
  ];
  private $except = ['clientid'];
  private $blnfields = ['iscustomer', 'issupplier', 'isagent', 'iswarehouse', 'isinactive', 'isdriver', 'ispassenger', 'isemployee', 'isdepartment', 'isadmin', 'uv_ischecker', 'uv_ispicker'];
  private $acctg = [];
  ...
  public function loaddoclisting($config)
  {
    ...
    $qry = "select client.clientid,client.client,client.clientname,client.addr from client where ispassenger =1 $filtersearch
     order by client";
  ...
  public function createHeadField($config)
  {
    ...
    $fields = ['picture', 'ispassenger', 'isdriver', 'iswarehouse', 'issupplier', 'iscustomer', 'isagent', 'isinactive', 'isdepartment', 'isadmin'];

    $col3 = $this->fieldClass->create($fields);
    data_set($col3, 'ispassenger.class', 'csissupplier sbccsreadonly');
  ...
  public function loadheaddata($config)
  {
    ...
    if ($clientid == 0) {
      ...
      if ($clientid == 0) {
        $clientid = $this->coreFunctions->datareader("select clientid as value from client where ispassenger=1 and center=? order by clientid desc limit 1", [$center]);
      }
    ...
    $qry = $qryselect . ", dept.client as dept, dept.clientname as deptname,
      ...
      where client.clientid = ? and client.ispassenger = 1";
  ...
  public function getlastclient($pref)
  {
    ...
    $return = '';
    if ($length == 0) {
      $return = $this->coreFunctions->datareader('select client as value from client where  ispassenger=1 order by client desc limit 1');
    } else {
      $return = $this->coreFunctions->datareader('select client as value from client where  ispassenger=1 and left(client,?)=? order by client desc limit 1', [$length, $pref]);
    }
    return $return;
  }
  ...
  public function deletetrans($config)
  {
    ...
    $qry = "select vrpassenger.trno as value from vrpassenger where passengerid=?
            union all
            select hvrpassenger.trno as value from hvrpassenger where passengerid=? limit 1";
    ...
    $qry = "select clientid as value from client where clientid<? and ispassenger=1 order by clientid desc limit 1 ";
  ...
  public function generateResult($config)
  {
    ...
    client.isagent,client.isemployee,client.ispassenger

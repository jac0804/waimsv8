...
class waterreading
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'WATER READING';
  public $gridname = 'entrygrid';
  public $head = 'waterreading';
  ...
  public function createTab($config)
  {
    ...
    $wend = 3;
    $consump = 4;
    $editprev = $this->othersClass->checkAccess($config['params']['user'], 4202);
    ...
    if ($editprev) {
      $obj[0][$this->gridname]['columns'][$wstart]['type'] = 'input';
    } else {
      $obj[0][$this->gridname]['columns'][$wstart]['type'] = 'label';
    }
  ...
  {
    $qry = "water.line, water.clientid, water.wstart, water.wend, water.wrate, water.bmonth, 
      water.byear, water.center, water.readstart, water.readend, water.consump,
      cl.client, cl.clientname,
      case 
        when water.isposted=0 then 'false' 
        else 'true' 
      end as isposted,
      '' as errcolor";
    ...
    switch ($action) {
      case "create":
        ...
        if ($head['bmonth'] != "" || $head['year'] != "") {
          ...
          $waterrate = $this->coreFunctions->datareader("select amt as value from waterrate order by dateid desc limit 1");
          ...
          if ($checking[0]->clientid !== NULL) {
            $qry = "
              ...
              where cl.isinactive = 0 and cl.istenant = 1 and  loc.wmeter is not null and tinfo.clientid not in (" . $checking[0]->clientid . ")
              ";
            ...
            $qry = "
                ...
                left join client as cl on cl.clientid = tinfo.clientid
                left join loc as loc on loc.line = cl.locid
                where cl.isinactive = 0 and cl.istenant = 1 and  loc.wmeter is not null";
  ...
  private function savechanges($config)
  {
    ...
    foreach ($rows as $k => $val) {
      if ($val["bgcolor"] != "") {
        ...
        $val["consump"] = $val["wend"] - $val['wstart'];
  ...
  public function default_query($config)
  {
    ...
    $select = "
      ...
      water.byear, water.center, water.readstart, water.readend, water.consump,
      cl.client, cl.clientname, loc.name as locname, loc.wmeter,t.wmulti";
  ...
  public function default_waterreading_header_PDF($params, $data)
  {
    ...
    PDF::MultiCell(0, 0, "Water Consumption Proof Sheet", '', 'C');
  ...
  public function reportdata($params)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      PDF::MultiCell(75, 0, $data[$i]->wmeter, '', 'L', 0, 0, '', '');
      PDF::MultiCell(50, 0, number_format($data[$i]->wstart, $decimalqty), '', 'R', 0, 0, '', '');
      PDF::MultiCell(50, 0, number_format($data[$i]->wend, $decimalqty), '', 'R', 0, 0, '', '');
      PDF::MultiCell(75, 0, number_format($data[$i]->consump, $decimalqty), '', 'R', 0, 0, '', '');
      PDF::MultiCell(50, 0, number_format($data[$i]->wrate, $decimalcurr), '', 'R', 0, 0, '', '');
      PDF::MultiCell(50, 0, number_format(($data[$i]->wrate * $data[$i]->wmulti) * $data[$i]->consump, $decimalcurr), '', 'R', 0, 0, '', '');
      PDF::MultiCell(100, 0, '', '', 'L', 0, 1, '', '');
      if (intVal($i) + 1 == $page) {
        $this->default_waterreading_header_PDF($params, $data);
        $page += $count;
      }
      $gamt += (($data[$i]->wrate * $data[$i]->wmulti) * $data[$i]->consump);
    }
    PDF::MultiCell(700, 0, "", "B");

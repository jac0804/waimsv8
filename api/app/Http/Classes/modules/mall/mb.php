...
class mb
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'BILLING ENTRY';
  ...
  public function loaddoclisting($config)
  {
    ...
    $qry = "select head.trno,head.docno,head.clientname,$dateid,  head.dateid as date2,
    'DRAFT' as status,head.createby,head.editby,head.viewby,num.postedby,
     ...
     union all
     select head.trno,head.docno,head.clientname,$dateid, head.dateid as date2,
  ...
  public function createHeadField($config)
  {
    ...
    data_set($col1, 'client.lookupclass', 'lookuptenant');
    data_set($col1, 'docno.label', 'Transaction#');


    $fields = [['dateid', 'terms'], 'address'];
  ...
  public function deletetrans($config)
  {
    ...
    $bmonth = $this->coreFunctions->datareader("select month(dateid) as value from " . $this->head . ' where trno=?', [$trno]);
    $byear = $this->coreFunctions->datareader("select year(dateid) as value from " . $this->head . ' where trno=?', [$trno]);
    $clientid = $this->coreFunctions->datareader("select c.clientid as value from " . $this->head . ' as h left join client as c on c.client = h.client where h.trno=?', [$trno]);
    ...
    $this->coreFunctions->execqry('delete from tenantbal where clientid=? and bmonth = ? and byear=?', 'delete', [$clientid, $bmonth, $byear]);

    $qry = "insert into waterreading (line, clientid, wstart, wend, wrate, bmonth, byear, center, readstart, readend, isposted, consump)
    select line, clientid, wstart, wend, wrate, bmonth, byear, center, readstart, readend, isposted, consump
    from hwaterreading where clientid = " . $clientid . " and bmonth = '" . $bmonth . "' and byear = '" . $byear . "' and center = '" . $center . "'";
    $this->coreFunctions->execqry($qry, 'insert');

    $qry = "insert into electricreading (line, clientid, estart, eend, erate, bmonth, byear, center, readstart, readend, isposted, consump)
    select line, clientid, estart, eend, erate, bmonth, byear, center, readstart, readend, isposted, consump
    from helectricreading where clientid = " . $clientid . " and bmonth = '" . $bmonth . "' and byear = '" . $byear . "' and center = '" . $center . "'";
    $this->coreFunctions->execqry($qry, 'insert');

    $qry = "insert into chargesbilling (line, cline, amt, bmonth, byear,clientid, center,rem, isposted)
    select line, cline, amt, bmonth, byear,clientid, center,rem, isposted
    from hchargesbilling where clientid = " . $clientid . " and bmonth = '" . $bmonth . "' and byear = '" . $byear . "' and center = '" . $center . "'";
    $this->coreFunctions->execqry($qry, 'insert');

    $this->coreFunctions->execqry("delete from hwaterreading where clientid = " . $clientid . " and bmonth = '" . $bmonth . "' and byear = '" . $byear . "' and center = '" . $center . "'", 'delete');
    $this->coreFunctions->execqry("delete from helectricreading where clientid = " . $clientid . " and bmonth = '" . $bmonth . "' and byear = '" . $byear . "' and center = '" . $center . "'", 'delete');
    $this->coreFunctions->execqry("delete from hchargesbilling where clientid = " . $clientid . " and bmonth = '" . $bmonth . "' and byear = '" . $byear . "' and center = '" . $center . "'", 'delete');

    $oldrate = $this->coreFunctions->datareader("select ifnull(oldrate,0) as value from escalation where dateapplied = '" . $dateid . "' and clientid =" . $clientid . " order by dateapplied desc limit 1");
    if ($oldrate <> 0) {
      $this->coreFunctions->execqry("update tenantinfo set leaserate = " . $oldrate . " where clientid =" . $clientid, 'update');
      $this->coreFunctions->execqry("update escalation set oldrate =0,dateapplied = null,rem ='',isapplied =0 where dateapplied = '" . $dateid . "' and clientid =" . $clientid, 'update');
    }

...
class entryreasonforhiring
{
    private $fieldClass;
    private $tabClass;
    public $modulename = 'Reason For Hiring Setup';
    ...
    private $fields = ['isreasonhiring', 'category'];
    ...
    public function createTab($config)
    {
        ...
        $obj[0][$this->gridname]['columns'][$category]['label'] = 'Description';
        $obj[0][$this->gridname]['columns'][$category]['style'] = "width:300px;whiteSpace: normal;min-width:300px;";
        ...
        public function loaddata($config)
    {
        ...
        $qry    = "select " . $select . " from " . $this->table .
            " where isreasonhiring = 1 " . $filtersearch . " order by line";
    ...
    public function saveallentry($config)
{
    ...
    foreach ($data as $key => $value) {
        ...
        if ($data[$key]['bgcolor'] != '') {
            ...
            if (empty(trim($data2['category']))) {
                return ['status' => false, 'msg' => 'Saving failed. Please input category.'];
            }
...
public function save($config)
{
     ...
     public function delete($config)
    {
        ...
        $this->logger->sbcdelmaster_log($row['line'], $config, 'REMOVE LINE: ' . $row['line'] . ' - ' . $row['category']);

...
class paygroup
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'PAYROLL GROUP';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'paygroup';
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['line'] == 0) {
          ...
          // $data[$key]['line'],
          // $config,
          // 'UPDATE' . ' - ' .$data[$key]['code'].' - '.$data[$key]['paygroup'] .' - LINE'.$data[$key]['line']); 
        }
      } // end if
    } // foreach
    $returndata = $this->loaddata($config);
  ...
  public function delete($config)
  {
    ...
    $qry1 = "select paygroup as value from employee where paygroup=?";
    ...
    $this->logger->sbcdelmaster_log($row['line'], $config, 'REMOVE - ' . $row['code'] . ' - ' . $row['paygroup']);
  ...
  private function report_default_query($config)
  {
    ...
    $query = "
      select line, code, paygroup from paygroup
    ";
  ...
  public function reportdata($config)
  {
    ...
    if ($config['params']['dataparams']['print'] == "default") {
      $str = $this->rpt_DEFAULT_paygroup_MASTER_LAYOUT($data, $config);
    } else if ($config['params']['dataparams']['print'] == "PDFM") {
      $str = $this->rpt_paygroup_PDF($data, $config);
    }
  ...
  private function rpt_default_header($data, $filters)
  {
    ...
    $str .= $this->reporter->col('PAYGROUP LIST', '600', null, false, $border, '', 'L', $font, '18', 'B', '', '');
    ...
    $str .= $this->reporter->col('PAYROLL GROUP', '100', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '30px', '8px');
    return $str;
  }

  private function rpt_DEFAULT_paygroup_MASTER_LAYOUT($data, $filters)
  {
    $companyid = $filters['params']['companyid'];
    $decimal = $this->companysetup->getdecimal('currency', $filters['params']);
    ...
    foreach ($data as $key => $value) {
      ...
      $str .= $this->reporter->col($value->paygroup, '50px', null, false, $border, '', 'C', $font, $fontsize, '', '', '2px');
  ...
  private function rpt_paygroup_PDF_header_PDF($data, $filters)
  {
    $companyid = $filters['params']['companyid'];
    $decimal = $this->companysetup->getdecimal('currency', $filters['params']);
    ...
    PDF::MultiCell(800, 20, "PAYGROUP LIST", '', 'L', false);
    ...
    PDF::MultiCell(300, 20, "Code", '', 'L', false, 0);
    PDF::MultiCell(300, 20, "Payroll Group", '', 'L', false, 0);
  ...
  private function rpt_paygroup_PDF($data, $filters)
  {
    $companyid = $filters['params']['companyid'];
    $decimal = $this->companysetup->getdecimal('currency', $filters['params']);
    ...
    $this->rpt_paygroup_PDF_header_PDF($data, $filters);
    $i = 0;
    foreach ($data as $key => $value) {
      ...
      PDF::MultiCell(300, 10, $value->paygroup, '', 'L', 0, 0, '', '', true, 0, true, false);

...
class philhealth
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'PHIC TABLE';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'phictab';
  public $tablelogs = 'masterfile_log';
  public $tablelogs_del = 'del_masterfile_log';
  private $othersClass;
  public $style = 'width:100%;';
  private $fields = ['bracket', 'range1', 'range2', 'phicee', 'phicer', 'phictotal'];
  ...
  {
    $columns = ['action', 'bracket', 'range1', 'range2', 'phicee', 'phicer', 'phictotal'];
    ...
    $obj[0][$this->gridname]['columns'][$phicee]['style'] = "width:150px;whiteSpace: normal;min-width:150px;";
    $obj[0][$this->gridname]['columns'][$phicer]['style'] = "width:150px;whiteSpace: normal;min-width:150px;";
    $obj[0][$this->gridname]['columns'][$phictotal]['style'] = "width:150px;whiteSpace: normal;min-width:150px;";
    return $obj;
  }
    ...
    if ($row['line'] == 0) {
      ...
      if ($line != 0) {
        ...
        $this->logger->sbcmasterlog(
          $line,
          $config,
          'CREATE -' . ' BRACKET: ' . $data['bracket'] . ' EE: ' . $data['phicee'] . ' ER: ' . $data['phicer'] . ' MULTIPLIER: ' . $data['phictotal']
        );
      ...
      if ($this->coreFunctions->sbcupdate($this->table, $data, ['line' => $row['line']]) == 1) {
        ...
        //   $row['line'],
        //   $config,
        //   'UPDATE -' . ' BRACKET: ' .$data['bracket'].' EE: '.$data['phicee']. ' ER: '.$data['phicer']. ' MULTIPLIER: '.$data['phictotal']); 
        return ['status' => true, 'msg' => 'Successfully saved.', 'row' => $returnrow];
      } else {
        return ['status' => false, 'msg' => 'Saving failed.'];
      }
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['line'] == 0) {
          ...
          $this->logger->sbcmasterlog(
            $line,
            $config,
            'CREATE -' . ' BRACKET: ' . $data[$key]['bracket'] . ' EE: ' . $data[$key]['phicee'] . ' ER: ' . $data[$key]['phicer'] . ' MULTIPLIER: ' . $data[$key]['phictotal']
          );
        } else {
          ...
          // $data[$key]['line'],
          // $config,
          // 'UPDATE -' . ' BRACKET: ' .$data[$key]['bracket'].' AMT: '.$data[$key]['amt']. ' PERCENT: '.$data[$key]['percentage']); 
        }
      } // end if
    } // foreach
  ...
  public function delete($config)
  {
    ...
    $this->logger->sbcdelmaster_log(
      $row['line'],
      $config,
      'REMOVE -' . ' BRACKET: ' . $row['bracket'] . ' EE: ' . $row['phicee'] . ' ER: ' . $row['phicer'] . ' MULTIPLIER: ' . $row['phictotal']
    );
  ...
  {
    $qry = "INSERT INTO `phictab` (`line`,`bracket`,`range1`,`range2`,`phicee`,`phicer`,`phictotal`) VALUES 
 (1,1,'0.00','10000.00','175.00','175.00','3.50'),
 (2,2,'70000.00','999999.00','1225.00','1225.00','3.50');";

    $exist = $this->coreFunctions->datareader("select bracket as value from phictab limit 1");

    if (!empty($exist)) {
      $data = app('App\Http\Classes\modules\payrollsetup\philhealth')->loaddata($config);
  ...
  private function report_default_query($config)
  {
    ...
    $query = "
      select bracket, range1, range2, phicee, phicer, phictotal from phictab
    ";
  ...
  public function reportdata($config)
  {
    ...
    if ($config['params']['dataparams']['print'] == "default") {
      $str = $this->rpt_DEFAULT_philhealth_MASTER_LAYOUT($data, $config);
    } else if ($config['params']['dataparams']['print'] == "PDFM") {
      $str = $this->rpt_philhealth_PDF($data, $config);
    }
  ...
  private function rpt_default_header($data, $filters)
  {
    ...
    $str .= $this->reporter->col('PHIC TABLE LIST', '600', null, false, $border, '', 'L', $font, '18', 'B', '', '');
    ...
    $str .= $this->reporter->col('RANGE 1', '100', null, false, $border, 'B', 'R', $font, $fontsize, 'B', '30px', '8px');
    $str .= $this->reporter->col('RANGE 2', '100', null, false, $border, 'B', 'R', $font, $fontsize, 'B', '30px', '8px');
    $str .= $this->reporter->col('PHIC EE', '100', null, false, $border, 'B', 'R', $font, $fontsize, 'B', '30px', '8px');
    $str .= $this->reporter->col('PHIC ER', '100', null, false, $border, 'B', 'R', $font, $fontsize, 'B', '30px', '8px');
    $str .= $this->reporter->col('MULTIPLIER(%)', '100', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '30px', '8px');
    return $str;
  }

  private function rpt_DEFAULT_philhealth_MASTER_LAYOUT($data, $filters)
  {
    $companyid = $filters['params']['companyid'];
    $decimal = $this->companysetup->getdecimal('currency', $filters['params']);
    ...
    foreach ($data as $key => $value) {
      ...
      $str .= $this->reporter->col($value->range1, '100', null, false, $border, '', 'R', $font, $fontsize, '', '', '2px');
      $str .= $this->reporter->col($value->range2, '100', null, false, $border, '', 'R', $font, $fontsize, '', '', '2px');
      $str .= $this->reporter->col($value->phicee, '100', null, false, $border, '', 'R', $font, $fontsize, '', '', '2px');
      $str .= $this->reporter->col($value->phicer, '100', null, false, $border, '', 'R', $font, $fontsize, '', '', '2px');
      $str .= $this->reporter->col($value->phictotal, '100', null, false, $border, '', 'C', $font, $fontsize, '', '', '2px');
  ...
  private function rpt_philhealth_PDF_header_PDF($data, $filters)
  {
    $companyid = $filters['params']['companyid'];
    $decimal = $this->companysetup->getdecimal('currency', $filters['params']);
    ...
    PDF::MultiCell(800, 20, "PHIC TABLE LIST", '', 'L', false);
    ...
    PDF::MultiCell(100, 20, "PHIC EE", '', 'C', false, 0);
    PDF::MultiCell(100, 20, "PHIC ER", '', 'C', false, 0);
  ...
  private function rpt_philhealth_PDF($data, $filters)
  {
    $companyid = $filters['params']['companyid'];
    $decimal = $this->companysetup->getdecimal('currency', $filters['params']);
    ...
    $this->rpt_philhealth_PDF_header_PDF($data, $filters);
    $i = 0;
    foreach ($data as $key => $value) {
      PDF::SetFont($font, '', $fontsize);
      // MultiCell($w, $h, $txt, $border=0, $align='J', $fill=0, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0)
      // PDF::MultiCell(300, 10, $data[$i]['stockgrp_code'], '', 'L', 0, 0, '', '', true, 0, true, false);
      PDF::MultiCell(200, 10, $value->bracket, '', 'L', 0, 0, '', '', true, 0, true, false);
      PDF::MultiCell(100, 10, $value->range1, '', 'R', 0, 0, '', '', true, 0, true, false);
      PDF::MultiCell(100, 10, $value->range2, '', 'R', 0, 0, '', '', true, 0, true, false);
      PDF::MultiCell(100, 10, number_format($value->phicee, $decimal), '', 'R', 0, 0, '', '', true, 0, true, false);
      PDF::MultiCell(100, 10, number_format($value->phicer, $decimal), '', 'R', 0, 0, '', '', true, 0, true, false);
      PDF::MultiCell(100, 10, $value->phictotal, '', 'C', 0, 0, '', '', true, 0, true, false);

...
class earningdeductionsetup
{
  private $btnClass;
  private $fieldClass;
  private $tabClass;
  public $modulename = 'EARNING AND DEDUCTION SETUP';
  ...
  public function createdoclisting($config)
  {
    ...
    $cols[$codename]['style'] = 'width:150px;whiteSpace: normal;min-width:150px;';
    $cols[$empname]['style'] = 'width:200px;whiteSpace: normal;min-width:200px;';
    $cols[$codename]['label'] = 'Earning/Deduction Type';
  ...
  public function loaddoclisting($config)
  {
    ...
    if ($option == 'draft') {
      $filteroption = " if(p.alias='DEDUCTION',s.balance<>0,s.halt=0)";
    } else {
      $filteroption = " if(p.alias='DEDUCTION',s.balance=0,s.halt=1)";
    }
    $filtersearch = "";
    if (isset($config['params']['search'])) {
      $searchfield = ['s.docno', 'e.empid', 'client.client', 'e.emplast', 'e.empfirst', 'e.empmiddle', 'p.codename'];
    ...
    $qry = "select s.trno as clientid, s.docno, date(s.dateid) as dateid, e.empid, client.client as empcode,
                CONCAT(emplast,', ',empfirst,' ',empmiddle) as empname, p.codename, FORMAT(s.balance,2) as balance
                from standardsetup as s left join employee as e ON e.empid = s.empid
                left join client on client.clientid=e.empid left join paccount as p on p.line=s.acnoid
  ...
  public function createHeadField($config)
  {
    ...
    if ($config['params']['companyid'] == 58) { //cdo
      data_set($col2, 'acnoname.label', 'Loan Type');
    }
  ...
  {
    $clientid = isset($config['params']['clientid']) ? $config['params']['clientid'] : $config['params']['trno'];
  ...
  public function deletetrans($config)
  {
    ...
    $qry = "select trno as value from standardtrans where trno=?";
  ...
  //   $query = "select ss.trno, ss.docno,st.docno as transdoc, date(ss.dateid) as ssdate, ss.remarks,

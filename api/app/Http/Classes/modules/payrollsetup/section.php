...
class section
{
  ...
  private $fields = ['sectcode', 'sectname'];
  ...
  public function createTab($config)
  {
    $tab = [
      $this->gridname => [
        'gridcolumns' => ['action', 'sectcode', 'sectname']
      ]
    ];
    ...
    $obj[0][$this->gridname]['columns'][2]['style'] = "width:700px;whiteSpace: normal;min-width:700px;";
    return $obj;
  }
  ...
  public function save($config)
  {
    ...
    if ($row['sectid'] == 0) {
      $sectid = $this->coreFunctions->insertGetId($this->table, $data);
      if ($sectid != 0) {
        $returnrow = $this->loaddataperrecord($sectid);

        $this->logger->sbcmasterlog(
          $sectid,
          $config,
          'CREATE' . ' - ' . $data['sectcode'] . ' - ' . $data['sectname'] . ' - LINE' . $sectid
        );
      ...
      if ($this->coreFunctions->sbcupdate($this->table, $data, ['sectid' => $row['sectid']]) == 1) {
        $returnrow = $this->loaddataperrecord($row['sectid']);
        return ['status' => true, 'msg' => 'Successfully saved.', 'row' => $returnrow];
      } else {
        return ['status' => false, 'msg' => 'Saving failed.'];
      }
  ...
  public function saveallentry($config)
  {
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['sectid'] == 0) {
          $sectid = $this->coreFunctions->insertGetId($this->table, $data2);

          $this->logger->sbcmasterlog(
            $sectid,
            $config,
            'CREATE' . ' - ' . $data[$key]['sectcode'] . ' - ' . $data[$key]['sectname'] . ' - LINE' . $sectid
          );
        } else {
          $data2['editdate'] = $this->othersClass->getCurrentTimeStamp();
          $data2['editby'] = $config['params']['user'];
          $this->coreFunctions->sbcupdate($this->table, $data2, ['sectid' => $data[$key]['sectid']]);

          // $this->logger->sbcmasterlog(
          // $data[$key]['sectid'],
          // $config,
          // 'UPDATE' . ' - ' .$data[$key]['sectcode'].' - '.$data[$key]['sectname'] .' - LINE'.$data[$key]['sectid']); 
        }
      } // end if
    } // foreach
  ...
  public function delete($config)
  {
    ...
    $qry1 = "select sectid as value from employee where sectid=?";
    $count = $this->coreFunctions->datareader($qry1, [$row['sectid']]);

    if ($count != '') {
      return ['clientid' => $row['sectid'], 'status' => false, 'msg' => 'Already have transaction...'];
    }

    $qry = "delete from " . $this->table . " where sectid=?";
    $this->coreFunctions->execqry($qry, 'delete', [$row['sectid']]);
    $this->logger->sbcdelmaster_log($row['sectid'], $config, 'REMOVE - ' . $row['sectcode'] . ' - ' . $row['sectname']);
    return ['status' => true, 'msg' => 'Successfully deleted.'];
  }


  private function loaddataperrecord($sectid)
  {
    $select = $this->selectqry();
    $select = $select . ",'' as bgcolor ";
    $qry = "select " . $select . " from " . $this->table . " where sectid=?";
    $data = $this->coreFunctions->opentable($qry, [$sectid]);
    return $data;
  }
  ...
  public function loaddata($config)
  {
    $select = $this->selectqry();
    $select = $select . ",'' as bgcolor ";
    $qry = "select " . $select . " from " . $this->table . " order by sectid";
  ...
  private function report_default_query($config)
  {
    ...
    $query = "
      select sectid, sectcode, sectname from section
    ";
  ...
  public function reportdata($config)
  {
    ...
    if ($config['params']['dataparams']['print'] == "default") {
      $str = $this->rpt_DEFAULT_section_MASTER_LAYOUT($data, $config);
  ...
  private function rpt_default_header($data, $filters)
  {
    ...
    $str .= $this->reporter->begintable('800');
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('SECTION LIST', '600', null, false, $border, '', 'L', $font, '18', 'B', '', '');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();
    ...
    $str .= $this->reporter->col('SECTION NAME', '100', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '30px', '8px');
    return $str;
  }

  private function rpt_DEFAULT_section_MASTER_LAYOUT($data, $filters)
  {
    $companyid = $filters['params']['companyid'];
    $decimal = $this->companysetup->getdecimal('currency', $filters['params']);
    ...
    foreach ($data as $key => $value) {
      $str .= $this->reporter->startrow();
      $str .= $this->reporter->col($value->sectcode, '50px', null, false, $border, '', 'C', $font, $fontsize, '', '', '2px');
      $str .= $this->reporter->col($value->sectname, '50px', null, false, $border, '', 'L', $font, $fontsize, '', '', '2px');
  ...
  private function rpt_division_PDF_header_PDF($data, $filters)
  {
    ...
    PDF::MultiCell(800, 20, "SECTION LIST", '', 'L', false);
    ...
    PDF::MultiCell(300, 20, "Code", '', 'L', false, 0);
    PDF::MultiCell(300, 20, "Section Name", '', 'L', false, 0);
  ...
  private function rpt_division_PDF($data, $filters)
  {
    ...
    foreach ($data as $key => $value) {
      ...
      PDF::MultiCell(300, 10, $value->sectcode, '', 'L', 0, 0, '', '', true, 0, true, false);
      PDF::MultiCell(300, 10, $value->sectname, '', 'L', 0, 0, '', '', true, 0, true, false);

...
class division
{
  ...
  private $fields = ['divcode', 'divname', 'address'];
  ...
  public function createTab($config)
  {
    ...
    $column = ['action', 'divid', 'divcode', 'divname', 'address'];
    ...
    $obj[0][$this->gridname]['columns'][$divcode]['style'] = "width:100px;whiteSpace: normal;min-width:100px;";
    $obj[0][$this->gridname]['columns'][$divname]['style'] = "width:300px;whiteSpace: normal;min-width:300px;";
    $obj[0][$this->gridname]['columns'][$address]['style'] = "width:500px;whiteSpace: normal;min-width:500px;";
    $obj[0][$this->gridname]['columns'][$divcode]['label'] = "Company Code";
    $obj[0][$this->gridname]['columns'][$divname]['label'] = "Company Name";

    if ($companyid != 62) { //onesky
      $obj[0][$this->gridname]['columns'][$divid]['type'] = "coldel";
    }
  ...
  public function save($config)
  {
    ...
    if ($row['divid'] == 0) {
      $divid = $this->coreFunctions->insertGetId($this->table, $data);
      if ($divid != 0) {
        $returnrow = $this->loaddataperrecord($divid);
        $this->logger->sbcmasterlog(
          $divid,
          $config,
          'CREATE' . ' - ' . $data['divcode'] . ' - ' . $data['divname'] . 'LINE' . $divid . "Address: " . $data['address']
        );
      ...
      if ($this->coreFunctions->sbcupdate($this->table, $data, ['divid' => $row['divid']]) == 1) {
        $returnrow = $this->loaddataperrecord($row['divid']);
        // $this->logger->sbcmasterlog(
        //   $row['divid'],
        //   $config,
  ...
  public function saveallentry($config)
  {
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['divid'] == 0) {
          $divid = $this->coreFunctions->insertGetId($this->table, $data2);

          $this->logger->sbcmasterlog(
            $divid,
            $config,
            'CREATE' . ' - ' . $data[$key]['divcode'] . ' - ' . $data[$key]['divname'] . ' - LINE' . $divid . "Address: " . $data[$key]['address']
          );
        } else {
          $data2['editdate'] = $this->othersClass->getCurrentTimeStamp();
          $data2['editby'] = $config['params']['user'];
          $this->coreFunctions->sbcupdate($this->table, $data2, ['divid' => $data[$key]['divid']]);
  ...
  public function delete($config)
  {
    ...
    $qry1 = "select divid as value from employee where divid=?";
    $count = $this->coreFunctions->datareader($qry1, [$divid], '', true);

    if ($count != 0) {
      return ['clientid' => $divid, 'status' => false, 'msg' => 'Already have transaction...'];
    }

    $qry = "delete from " . $this->table . " where divid=?";
    $this->coreFunctions->execqry($qry, 'delete', [$row['divid']]);
    $this->logger->sbcdelmaster_log($row['divid'], $config, 'REMOVE - ' . $row['divcode'] . ' - ' . $row['divname']) . "Address: " . $row['address'];
    return ['status' => true, 'msg' => 'Successfully deleted.'];
  }


  private function loaddataperrecord($divid)
  {
    $select = $this->selectqry();
    $select = $select . ",'' as bgcolor ";
    $qry = "select " . $select . " from " . $this->table . " where divid=?";
    $data = $this->coreFunctions->opentable($qry, [$divid]);
    return $data;
  }
  ...
  public function loaddata($config)
  {
    $select = $this->selectqry();
    $select = $select . ",'' as bgcolor ";
    $qry = "select " . $select . " from " . $this->table . " order by divid";

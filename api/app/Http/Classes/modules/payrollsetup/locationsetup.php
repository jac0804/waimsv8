...
class locationsetup
{
    private $fieldClass;
    private $tabClass;
    public $modulename = 'Location Setup';
    public $gridname = 'inventory';
    private $companysetup;
    private $coreFunctions;
    private $table = 'emploc';
    public $tablelogs = 'masterfile_log';
    public $tablelogs_del = 'del_masterfile_log';
    private $othersClass;
    public $style = 'width:100%;';
    private $fields = ['locname'];
    ...
    {
        $attrib = array('load' => 5362);
        return $attrib;
    }

    public function createTab($config)
    {
        $columns = ['action', 'locname'];
        ...
        $obj[0][$this->gridname]['columns'][$locname]['style'] = "width:800px;whiteSpace: normal;min-width:800px;";
        return $obj;
    }
    ...
    public function save($config)
    {
        ...
        if ($row['line'] == 0) {
            ...
            if (!empty($checking)) {
                return ['status' => false, 'msg' => 'Location already exist. - ' . $data['locname']];
            }
            ...
            $qry = "select locname as value from " . $this->table . " where locname = '" . $data['locname'] . "' and line = '" . $row['line'] . "'";
            $checking = $this->coreFunctions->datareader($qry);
            ...
            if (!empty($checking)) {
                ...
                if (!empty($checking1)) {
                    ...
                    return ['status' => false, 'msg' => 'Location already exist. - ' . $data['locname'], 'data' => $data];
                }
            }
        ...
        foreach ($data as $key => $value) {
            ...
            if ($data[$key]['bgcolor'] != '') {
                ...
                if ($data[$key]['line'] == 0) {
                    ...
                    if (!empty($checking)) {
                        return ['status' => false, 'msg' => 'Location already exist. - ' . $data[$key]['locname'], 'data' => $data];
                    }
                    ...
                    $this->logger->sbcmasterlog(
                        $line,
                        $config,
                        'CREATE' . ' - ' . $data[$key]['locname'] . ' - LINE' . $line
                    );
                } else {
                    $qry = "select locname as value from " . $this->table . " where locname = '" . $data[$key]['locname'] . "' and line = '" . $data[$key]['line'] . "'";
                    $checking = $this->coreFunctions->datareader($qry);

                    if (!empty($checking)) {
                        unset($data2[$key]["locname"]);
                    } else {
                        $qry = "select locname as value from " . $this->table . " where locname = '" . $data[$key]['locname'] . "'";
                        $checking1 = $this->coreFunctions->datareader($qry);
    ...
    public function delete($config)
    {
        ...
        $line = $row['line'];
        $loc = $config['params']['row']['locname'];
        $qry = "select emploc as value from employee where emploc=?";
        $exist = $this->coreFunctions->opentable($qry, [$loc]);
        if (!empty($exist)) {
            return ['line' => $line, 'status' => false, 'msg' => 'Unable to delete, it was already picked up in Employee/Holiday Location.'];
        } else {
            $qry = "delete from " . $this->table . " where line=?";
            $this->coreFunctions->execqry($qry, 'delete', [$row['line']]);
            $this->logger->sbcdelmaster_log($row['line'], $config, 'REMOVE - ' . $row['locname']);
    ...
    private function report_default_query($config)
    {
        ...
        $query = "
      select line, locname from emploc
    ";
    ...
    public function reportdata($config)
    {
        ...
        if ($config['params']['dataparams']['print'] == "default") {
            $str = $this->rpt_DEFAULT_LOCATION_LAYOUT($data, $config);
    ...
    private function rpt_default_header($data, $filters)
    {
        ...
        $str .= $this->reporter->col('Location List', '600', null, false, $border, '', 'L', $font, '18', 'B', '', '');
        ...
        $str .= $this->reporter->begintable('800');
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col('LOCATION', '100', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '30px', '8px');
        return $str;
    }

    private function rpt_DEFAULT_LOCATION_LAYOUT($data, $filters)
    {
        $companyid = $filters['params']['companyid'];
        $decimal = $this->companysetup->getdecimal('currency', $filters['params']);
        ...
        foreach ($data as $key => $value) {
            $str .= $this->reporter->startrow();
            $str .= $this->reporter->col($value->locname, '50px', null, false, $border, '', 'C', $font, $fontsize, '', '', '2px');

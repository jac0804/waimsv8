...
class cr
{
  ...
  public function paramsdatalisting($config)
  {
    ...
    //   data_set($col1, 'pickpo.label', 'ISSUED LOANS');
  ...
  public function loaddoclisting($config)
  {
    ...
    head.createby,head.editby,head.viewby,num.postedby, date(num.postdate) as postdate,
      ...
      case ifnull(head.lockdate,'') when '' then (case ifnull(head.voiddate,'') when '' then 'red' else 'grey' end) else (case ifnull(head.voiddate,'') when '' then 'green' else 'grey' end) end as statuscolor
     ...
     left join heainfo as info on info.trno = ea.trno  $leftjoin   
    ...
    head.createby,head.editby,head.viewby,num.postedby, date(num.postdate),
     ...
     head.createby,head.editby,head.viewby, num.postedby, date(num.postdate) as postdate,
       ...
       case ifnull(head.voiddate,'') when '' then '" . $gstatuscolor . "' else 'grey' end  as statuscolor     
  ...
  {
    $companyid = $config['params']['companyid'];
    $tbuttons = ['generateautoentry', 'additem', 'saveitem', 'deleteallitem', 'unpaid'];
  ...
  public function createHeadField($config)
  {
    ...
    data_set($col2, 'appref.type', 'lookup');
    data_set($col2, 'appref.class', 'sbccsreadonly');
    data_set($col2, 'appref.lookupclass', 'lookupappdocno');
    data_set($col2, 'appref.action', 'lookupappdocno');
    data_set($col2, 'appref.addedparams', ['clientid']);
    data_set($col2, 'yourref.label', 'AR#');
    ...
    $fields = [['checkno', 'checkdate'], 'amount'];
    ...
    data_set($col3, 'dacnoname.lookupclass', 'lookupdepositto');
    ...
    data_set($col3, 'checkno.addedparams', ['client']);
  ...
  public function loadheaddata($config)
  {
    ...
    $qryselect = "select 
        ...
        head.crref,head.checkno,left(head.checkdate,10) as checkdate,format(head.amount,2)as amount ,
        head.aftrno,ifnull(head.voiddate,'') as voiddate,ifnull(ea.docno,'') as appref,head.rctrno,head.rcline,head.purposeid ";
    ...
    $qry = $qryselect . " from $table as head
        ...
        left join arledger as ea on ea.trno = head.aftrno
    ...
    if (!empty($head)) {
      $head[0]->radiopaytype = $head[0]->purposeid;
  ...
  public function updatehead($config, $isupdate)
  {
    ...
    $data['purposeid'] = $head['radiopaytype'];
    if ($isupdate) {
  ...
  private function getdetailselect($config)
  {
    $qry = " head.trno,left(head.dateid,10) as dateid,d.ref,d.line,d.sortline,coa.acno,coa.acnoname,
      ...
      d.ewtcode,d.ewtrate,d.damt,'' as bgcolor,case d.void when 1 then 'bg-red-2' else '' end as  errcolor,num.dptrno as letrno,case d.void when 0 then 'false' else 'true' end as void
...
left join cntnum as num on num.trno = head.trno
  ...
  public function deleteitem($config)
  {
    ...
    if ($data[0]->refx != 0) {
      ...
      $letrno = $this->coreFunctions->getfieldvalue($this->tablenum, "dptrno", "trno=?", [$trno], '', true);

      if ($letrno != 0) {
        $this->coreFunctions->sbcupdate("htempdetailinfo", ['payment' => 0], ['trno' => $letrno, 'dateid' => $data[0]['postdate']]);
      }
    }
  ...
  public function getunpaidselected($config)
  {
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['db'] != 0) {
        ...
        // if($data[$key]['alias'] =='AR3'){
        //   $takeout = $takeout +  $data[$key]['tobal'];
        //   if($apptrno !=""){
        //     $apptrno = $apptrno.",".$this->coreFunctions->getfieldvalue("glhead","aftrno","trno = ?",[$data[$key]['trno']]);
        //   }else{
    ...
    //     $config['params']['data']['fdb'] = 0;
    //     $config['params']['data']['fcr'] = 0;
    //     $config['params']['data']['postdate'] = $crdate;
    //     $config['params']['data']['project'] = 0;
  ...
  public function applytoar($config, $isdb = false)
  {
    ...
    where head.trno = " . $aptrno . " and ar.bal<>0 " . $alias;
    ...
    if ($aptrno != 0) {
      ...
      $qry = "select cl.client,cl.clientid, head.dateid,head.ourref,head.amount,head.checkno,head.checkdate,coa.acnoid from " . $this->head . " as head 
      left join client as cl on cl.client = head.client left join coa on coa.acno = head.contra where head.trno =?";
      ...
      $ueint = $this->coreFunctions->getfieldvalue("coa", "acnoid", "alias ='UE1'");
      $uepf = $this->coreFunctions->getfieldvalue("coa", "acnoid", "alias ='UE2'");
      $saint = $this->coreFunctions->getfieldvalue("coa", "acnoid", "alias ='SA2'");
      $sapf = $this->coreFunctions->getfieldvalue("coa", "acnoid", "alias ='SA3'");
      $pt = $this->coreFunctions->getfieldvalue("coa", "acnoid", "alias ='SA6'");

      if (!empty($head)) {
        $cashacnoid = $head[0]->acnoid;
        $checkdate = $head[0]->checkdate;
        $dateid = $head[0]->dateid;

        $qry = "select ar.trno,ar.line, ar.bal,ar.db,ar.cr,ar.dateid,ar.docno,ar.acnoid,detail.rem,coa.alias,num.dptrno,i.penalty,ifnull(ar.lpaydate,'') as lpaydate from glhead as head 
        ...
        where head.trno = " . $aptrno . " and ar.bal<>0 " . $alias . "  order by  dateid asc,trno,line desc";
        ...
        if (!empty($ar)) {
          ...
          $dayselapse = 0;
          $ptotal = 0;
          $pr = round($ar[0]->penalty / 100, 2);

          //penalty computation
          foreach ($ar as $m => $n) {
            //penalty
            if ($ar[$m]->lpaydate != "") {
              $dateid = $ar[$m]->lpaydate;
            }

            if ($dateid > $ar[$m]->dateid) {
              if ($ar[$m]->penalty != 0) {
                $dayselapse = date_diff(date_create($dateid), date_create($ar[$m]->dateid));
                $dayselapse = intval($dayselapse->format("%a"));
                if ($dayselapse > 0) {
                  $penalty = $ar[$m]->bal * $pr * ($dayselapse / 30);
                  if ($penalty != 0) {
                    $ptotal = $ptotal + $penalty;
                  }
                }
              }
            }
          ...
          foreach ($detail as $key => $value) {
            ...
            if ($this->coreFunctions->sbcinsert($this->detail, $detail[$key]) == 1) {
              ...
              if ($detail[$key]['refx'] != 0) {
                ...
                if (!$this->sqlquery->setupdatebal($detail[$key]['refx'], $detail[$key]['linex'], $acno, $config, 0, 1, $dateid)) {
                ...
                $this->coreFunctions->LogConsole($letrno . 'app');

                if ($letrno != 0) {
                  $this->coreFunctions->sbcupdate("htempdetailinfo", ['payment' => $colamt], ['trno' => $letrno, 'dateid' => $detail[$key]['postdate']]);
                }
              }
    ...
    $data = $this->opendetail($trno, $config);
    return ['accounting' => $data, 'status' => $status, 'msg' => $msg, 'reloadhead' => true, 'reloadgriddata' => true];
  }
  ...
  public function applyardiminish($config, $isdb = false)
  {
    ...
    if ($aptrno != 0) {
      ...
      if (!empty($head)) {
        ...
        $firstdue = $this->coreFunctions->datareader("select left(ar.dateid,10) as value from glhead as head
        left join arledger as ar on head.trno = ar.trno
        where head.trno = 99 and ar.bal<>0 order by value asc,head.trno,ar.line desc limit 1");

...
class entryamenities
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'AMENITIES MASTERFILE';
  ...
  public function createTab($config)
  {
    ...
    $tab = [$this->gridname => ['gridcolumns' => ['action', 'code', 'description', 'isdisable']]];
    $stockbuttons = ['save', 'delete', 'addsubamenity'];

    $obj = $this->tabClass->createtab($tab, $stockbuttons);
    // action
    $obj[0][$this->gridname]['columns'][$action]['style'] = "width:100px;whiteSpace: normal;min-width:100px;";
    $obj[0][$this->gridname]['columns'][$action]['btns']['addsubamenity']['checkfield'] = 'isdisable';
    $obj[0][$this->gridname]['columns'][$code]['style'] = "width:200px;whiteSpace: normal;min-width:200px;";
    $obj[0][$this->gridname]['columns'][$disabled]['type'] = "hidden";
    $obj[0][$this->gridname]['columns'][$disabled]['label'] = "";
    $obj[0][$this->gridname]['columns'][$disabled]['style'] = "width:2px;";
  ...
  {
    return "head.line, head.code, head.description, case when head.code='' then 'true' else 'false' end as isdisable";
  }
  ...
  public function saveallentry($config)
  {
    ...
    $config['params']['doc'] = 'ENTRYAMENITIES';

    $dateTables = ['amenities'];
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['line'] == 0) {
          $amenities = $this->coreFunctions->insertGetId($this->table, $data2);
          $this->logger->sbcmasterlog($amenities, $config, ' CREATE - ' . $data[$key]['code'] . ' - ' . $data[$key]['description']);
        } else {
  ...
  public function delete($config)
  {
    ...
    $this->coreFunctions->execqry("delete from amenities where line=" . $row['line'], 'delete');
  ...
  public function lookuplogs($config)
  {
    ...
    $lookupsetup = array(
      'type' => 'show',
      'title' => 'Amenities Masterfile Logs',
  ...
  private function report_default_query($config)
  {
    ...
    $query = "select line, code, description from amenities
    order by line";
  ...
  public function reportdata($config)
  {
    ...
    if ($config['params']['dataparams']['print'] == "default") {
      ...
      $str = $this->amenities_PDF($data, $config);
    }
  ...
  private function amenities_PDF($data, $filters)
  {
    $companyid = $filters['params']['companyid'];
    $decimal = $this->companysetup->getdecimal('currency', $filters['params']);

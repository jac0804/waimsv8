...
class pr
{
  ...
  public function createHeadField($config)
  {
    ...
    if ($companyid == 10 || $companyid == 12) { //afti, afti usd
      ...
      if ($systemtype == 'REALESTATE') {
        array_push($fields, 'dprojectname', 'phase', 'housemodel', ['blklot', 'lot'], 'amenityname', 'subamenityname');
        $col4 = $this->fieldClass->create($fields);
  ...
  public function getmrsummaryqry($config)
  {
    return "select concat(stock.trno,stock.line) as keyid,stock.trno,stock.line,item.itemid,item.itemname,head.docno,left(head.dateid,10) as dateid,item.barcode,
              ...
              client.addr as address, client.terms, head.rem, 'P' as cur, 1 as forex,head.ourref,stock.uom,stock.prqty,
              ...
              stock.loc,head.yourref,head.agent,head.projectid,head.phaseid,head.modelid,head.blklotid,stock.amenity,stock.subamenity
              ...
              where head.trno = ? and stock.iss>(stock.qa+stock.prqa) and stock.prqty<>0
              and stock.void = 0 ";
  }
  ...
  public function updateperitem($config)
  {
    ...
    foreach ($data2 as $key => $value) {
      if ($data2[$key][$this->dqty] == 0) {
        ...
        $isupdate = false;
        if ($data[$key]->refx != 0) {
          $msg1 = ' Qty is Greater than request Qty ';
        }
      }
    }

    if (!$isupdate) {
      return ['row' => $data, 'status' => true, 'msg' => $msg1];
    } else {
  ...
  public function updateitem($config)
  {
    ...
    foreach ($data2 as $key => $value) {
      if ($data2[$key][$this->dqty] == 0) {
        ...
        $isupdate = false;
        if ($data[$key]->refx != 0) {
          $msg = 'Qty is Greater than Request Qty ';
        }
      }
    }
  ...
  public function additem($action, $config)
  {
    ...
    if (isset($config['params']['data']['amenity'])) {
      $amenityid = $config['params']['data']['amenity'];
    }

    if (isset($config['params']['data']['subamenity'])) {
      $subamenityid = $config['params']['data']['subamenity'];
    }
  ...
  public function setserveditems($refx, $linex)
  {
    ...
    return $this->coreFunctions->execqry("update hmrstock set prqa=" . $qty . " where trno=" . $refx . " and line=" . $linex, 'update');
  }

...
class entrysubamenities
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'Sub Amenities MASTERFILE';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'subamenities';
  private $othersClass;
  public $style = 'width:100%;';
  public $tablelogs = 'masterfile_log';
  public $tablelogs_del = 'del_masterfile_log';
  private $fields = ['amenityid', 'code', 'description'];
  ...
  public function add($config)
  {
    ...
    $data = [];
    $data['line'] = 0;
    $data['amenityid'] = $config['params']['sourcerow']['line'];
    $data['code'] = '';
    $data['description'] = '';
  ...
  {
    return "head.line, head.code, head.amenityid, head.description";
  }
  ...
  public function saveallentry($config)
  {
    ...
    $config['params']['doc'] = 'ENTRYSUBAMENITIES';

    $dateTables = ['subamenities'];
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['line'] == 0) {
          $subamenities = $this->coreFunctions->insertGetId($this->table, $data2);
          $this->logger->sbcmasterlog($subamenities, $config, ' CREATE - ' . $data[$key]['code'] . ' - ' . $data[$key]['description']);
        } else {
  ...
  public function save($config)
  {
    ...
    $amenity_id = $config['params']['row']['line'];
    ...
    if ($row['line'] == 0) {
      ...
      if ($line != 0) {
        $returnrow = $this->loaddataperrecord($line, $amenity_id);
        $this->logger->sbcmasterlog($line, $config, ' CREATE - ' . $data['code'] . ' - ' . $data['description']);
      ...
      if ($this->coreFunctions->sbcupdate($this->table, $data, ['line' => $row['line']]) == 1) {
        $returnrow = $this->loaddataperrecord($row['line'], $amenity_id);
        return ['status' => true, 'msg' => 'Successfully saved.', 'row' => $returnrow];
      } else {
        return ['status' => false, 'msg' => 'Saving failed.'];
      }
  ...
  public function delete($config)
  {
    ...
    $this->coreFunctions->execqry("delete from subamenities where line=" . $row['line'], 'delete');
    $this->logger->sbcdelmaster_log($row['line'], $config, 'REMOVE - ' . $row['code'] . ' - ' . $row['description']);
  ...
  {
    $filtersearch = "";
    $searcfield = ['head.code'];
    $limit = "1000";


    $amenity_id = isset($config['params']['row']['line']) ? $config['params']['row']['line'] : $config['params']['sourcerow']['line'];
    ...
    $qry = "select " . $select . " 
    from " . $this->table . " as head
    where 1=1 and head.amenityid = $amenity_id " . $filtersearch . "
  ...
  {
    $doc = 'ENTRYSUBAMENITIES';
    $lookupsetup = array(
      'type' => 'show',
      'title' => 'SubAmenities Masterfile Logs',
  ...
  private function report_default_query($config)
  {
    ...
    $query = "select line, code, description from subamenities
    order by line";
  ...
  public function reportdata($config)
  {
    ...
    if ($config['params']['dataparams']['print'] == "default") {
      $str = $this->old_masterfile_layout($data, $config);
    } else if ($config['params']['dataparams']['print'] == "PDFM") {
      $str = $this->subamenities_PDF($data, $config);
    }
  ...
  private function rpt_default_header($data, $filters)
  {
    ...
    $str .= $this->reporter->col('Description', '400', null, false, '1px solid ', 'B', 'L', 'Century Gothic', '12', 'B', '', '2px');
    $str .= $this->reporter->endrow();
    return $str;
  }

  private function old_masterfile_layout($data, $filters)
  {
    $companyid = $filters['params']['companyid'];
    $decimal = $this->companysetup->getdecimal('currency', $filters['params']);
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $str .= $this->reporter->col($data[$i]['description'], '400', null, false, '1px solid ', '', 'L', 'Century Gothic', '11', '', '', '3px');
      $str .= $this->reporter->endrow();
  ...
  private function header_PDF($data, $filters)
  {
    $companyid = $filters['params']['companyid'];
    $decimal = $this->companysetup->getdecimal('currency', $filters['params']);
  ...
  private function subamenities_PDF($data, $filters)
  {
    $companyid = $filters['params']['companyid'];
    $decimal = $this->companysetup->getdecimal('currency', $filters['params']);

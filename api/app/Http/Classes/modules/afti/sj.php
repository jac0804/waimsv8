...
namespace App\Http\Classes\modules\afti;
...
class sj
{
  ...
  {
    $companyid = $config['params']['companyid'];
    $viewexp = $this->othersClass->checkAccess($config['params']['user'], 5482);
  ...
  public function createTab($access, $config)
  {
    ...
    switch ($companyid) {
      case 40: //cdo
        ...
        break;
      case 60: //transpower
        array_push($column, 'agentamt', 'startwire', 'endwire');
        array_push($sortcolumn, 'agentamt', 'startwire', 'endwire');
        break;
    }
  ...
  private function getstockselect($config)
  {
    ...
    switch ($companyid) {
      ...
      case 41: //labsol manila
        ...
        break;
      case 60: //transpower
        $serialfield = ",stock.agentamt, stock.startwire, stock.endwire, stock.porefx, polinex ";
        break;
    }
  ...
  public function additem($action, $config, $setlog = false)
  {
    ...
    $kgs = $this->othersClass->sanitizekeyfieldFast('kgs', $kgs, $lookups);
    ...
    switch ($pricetype) {
      ...
      case 'CustomerGroupLatest':
        ...
        if ($pricegrp != '') {
          ...
          $qry = "select '" . $pricefield['label'] . "' as docno, left(now(),10) as dateid," . $pricefield['amt'] . " as amt," . $pricefield['amt'] . " as defamt, " . $pricefield['disc'] . " as disc, uom, itemid from item where barcode=? 
          union all
          select docno,left(dateid,10) as dateid,round(amt," . $this->companysetup->getdecimal('price', $config['params']) . ") as amt,round(amt," . $this->companysetup->getdecimal('price', $config['params']) . ") as defamt,disc,uom,itemid from(select head.docno,head.dateid,
            ...
            stock.uom,stock.disc,stock.itemid from glhead as head
            left join glstock as stock on stock.trno = head.trno
            left join item on item.itemid = stock.itemid
  ...
  public function createdistribution($config)
  {
    ...
    if ($commexp != 0) {
      ...
      $entry = ['acnoid' => $acnoid, 'client' => $d[0]->client, 'db' => $commexp * $d[0]->forex, 'cr' => 0, 'postdate' => $d[0]->dateid, 'cur' => $d[0]->cur, 'forex' => $d[0]->forex, 'fcr' => floatval($d[0]->forex) == 1 ? 0 : $delcharge, 'fdb' => 0];
      $this->acctg = $this->othersClass->upsertdetail($this->acctg, $entry, $config);

      $acnoid = $this->coreFunctions->getfieldvalue('coa', 'acnoid', 'acno=?', [$params['acno']]);
      $entry = ['acnoid' => $acnoid, 'client' => $d[0]->client, 'db' => ($commexp * $d[0]->forex) * -1, 'cr' => 0, 'postdate' => $d[0]->dateid, 'cur' => $d[0]->cur, 'forex' => $d[0]->forex, 'fdb' => floatval($d[0]->forex) == 1 ? 0 : $d[0]->dateid, 'fcr' => 0];
      $this->acctg = $this->othersClass->upsertdetail($this->acctg, $entry, $config);
    }
  ...
  public function getpaysummaryqry($config)
  {
    ...
    abs(arledger.fdb-arledger.fcr) as fdb,glhead.yourref,gldetail.rem as drem,glhead.rem as hrem,glhead.rem,gldetail.projectid,gldetail.subproject,
    gldetail.stageid,gldetail.branch,gldetail.deptid,gldetail.poref,gldetail.podate,coa.alias,gldetail.postdate,glhead.tax,glhead.vattype,glhead.ewt,glhead.ewtrate,a.client as agent,glhead.address from (arledger
  ...
  public function reportsetup($config)
  {
    ...
    switch ($companyid) {
      ...
      case 60: //transpower
        ...
        $stocks = $this->coreFunctions->datareader("select stock.line as value from " . $tbl . " as stock left join item on item.itemid=stock.itemid where stock.trno=? and stock.isamt<item.namt", [$config['params']['trno']], '', true);
        if ($stocks != 0) {
          return ['status' => false, 'msg' => 'Print failed, amount less than net amount.'];
        }
        break;
    }
    ...
    foreach ($config['params']['rows'] as $key => $value) {
      ...
      if (!empty($data)) {
        foreach ($data as $key2 => $value) {
          ...
          $idata = $this->getlatestprice($config);
          if (!empty($idata)) $config['params']['data']['amt'] = $idata['data'][0]->amt;

...
class sj
{
    ...
    public function loaddoclisting($config)
    {
        ...
        $qry = "select head.dateid as date2,head.trno,head.docno,head.clientname,left(head.dateid,10) as dateid, case ifnull(head.lockdate,'') when '' then 'DRAFT' else 'LOCKED' end as status, case ifnull(head.lockdate,'') when '' then 'red' else 'green' end as statuscolor,$rem
    head.createby,head.editby,head.viewby,num.postedby,
     ...
     union all
     select head.dateid as date2,head.trno,head.docno,head.clientname,left(head.dateid,10) as dateid,'POSTED' as status,'grey' as statuscolor,$rem
     head.createby,head.editby,head.viewby, num.postedby,
      head.yourref, head.ourref,head.shipto 
    ...
    order by dateid desc, docno desc $limit";
        $this->coreFunctions->LogConsole($condition);
    ...
    public function createTab($access, $config)
    {
        ...
        $column = ['action', 'barcode', 'isqty',  'uom', 'itemname', 'isamt',  'disc', 'ext', 'cost', 'markup', 'ref', 'rem'];
        $sortcolumn = ['action', 'barcode',  'isqty', 'uom', 'itemname', 'isamt',  'disc', 'ext', 'cost', 'markup', 'ref', 'rem'];
    ...
    public function createHeadField($config)
    {
        ...
        $fields = ['docno', 'client', 'clientname', 'address', 'dprojectname', 'voyage', 'orderno'];

        $col1 = $this->fieldClass->create($fields);
        data_set($col1, 'client.lookupclass', 'customer');
        data_set($col1, 'docno.label', 'Transaction#');
        data_set($col1, 'orderno.readonly', false);
        data_set($col1, 'orderno.label', 'B.L. No.');

        $fields = [['dateid', 'terms'], 'due', 'dacnoname', 'dwhname', 'swsno', 'rem2'];

        $col2 = $this->fieldClass->create($fields);
        data_set($col2, 'dacnoname.label', 'AR Account');
        data_set($col2, 'dacnoname.lookupclass', 'AR');
        data_set($col2, 'swsno.label', 'Ship Via');
        data_set($col2, 'swsno.maxlength', 100);
        data_set($col2, 'rem2.maxlength', 100);
        data_set($col2, 'rem2.label', 'M / V');
        data_set($col2, 'rem2.type', 'input');

        $fields = [['yourref', 'ourref'], ['cur', 'forex'], 'dvattype', 'dagentname', 'bpo', 'ctnsno', 'isreported'];
    ...
    public function defaultheaddata($params)
    {
        ...
        $data[0]['swsno'] = ''; //ship via
        $data[0]['voyage'] = '';
        $data[0]['rem2'] = ''; //M / V
        $data[0]['orderno'] = ''; //B.L. No.
    ...
    public function loadheaddata($config)
    {
        ...
        $qryselect = "select
         ...
         hinfo.interestrate,hinfo.downpayment,
         head.isreported,
         head.bpo, head.ctnsno, head.invoiceno,ifnull(head.swsno,'') as swsno,ifnull(head.voyage,'') as voyage,ifnull(head.rem2,'') as rem2,
    ...
    public function createdistribution($config)
    {
        ...
        $qry = '
        select head.dateid,head.client,head.tax,head.contra,head.cur,head.forex,stock.ext,wh.client as wh,ifnull(item.asset,"") as asset,ifnull(item.revenue,"") as revenue,
        item.expense,stock.isamt,stock.disc,stock.isqty,stock.cost,stock.iss,stock.fcost,head.projectid,client.rev,stock.rebate,head.taxdef,head.deldate,head.ewt,
        head.ewtrate ' . $fields . '

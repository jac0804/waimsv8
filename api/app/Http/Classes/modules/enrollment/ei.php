...
class ei
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'Grades School Assessment';
  ...
  private $stockselect;
  private $fields = ['trno', 'docno', 'dateid', 'client', 'clientname', 'periodid', 'syid', 'courseid', 'schedcode', 'rem', 'deptid', 'levelid', 'semid', 'yr', 'sectionid', 'contra', 'modeofpayment', 'curriculumtrno', 'disc', 'ischinese'];
  ...
  {
    $getcols = ['action', 'liststatus', 'listdocument', 'listclientname', 'listdate', 'listcourse', 'listsection', 'listsy', 'listperiod'];
    $stockbuttons = ['view'];
  ...
  public function loaddoclisting($config)
  {
    ...
    if (isset($config['params']['search'])) {
      $searchfield = ['head.docno', 'client.clientname', 'sy.sy', 'p.code'];
    ...
    $qry = "select head.trno, head.docno, c.coursecode, left(head.dateid,10) as dateid, 'DRAFT' as status, c.coursename, head.curriculumname, sc.section, t.term as terms, p.code as period, head.yr, head.curriculumcode,sy.sy as sy,client.client,client.clientname, head.sectionid
      ...
      left join en_section as sc on sc.line=head.sectionid
      where head.doc=? and num.center=? and CONVERT(head.dateid,DATE)>=? and CONVERT(head.dateid,DATE)<=? " . $condition . " " . $filtersearch . "
      union all
      select head.trno, head.docno, c.coursecode, left(head.dateid,10) as dateid, 'POSTED' as status, c.coursename, head.curriculumname, sc.section, t.term as terms, p.code as period, head.yr, head.curriculumcode,sy.sy as sy,client.client,client.clientname, head.sectionid
  ...
  public function createTab($access, $config)
  {
    ...
    $tab = [
      $this->gridname => ['gridcolumns' => ['action', 'subjectcode', 'subjectname', 'units', 'lecture', 'laboratory', 'hours', 'term', 'eainstructorcode', 'instructorname', 'lbldgcode', 'roomcode', 'schedday', 'schedstarttime', 'schedendtime', 'availableslot'], 'headgridbtns' => ['viewotherfees', 'viewcredentials', 'viewsosummary', 'generateenbilling', 'viewbooks']],
    ];
    ...
    $obj[0][$this->gridname]['columns'][7]['label'] = 'Semester';
    ...
    $obj[0][$this->gridname]['columns'][11]['style'] = 'width:100px;whiteSpace:normal;min-width:100px;';
    $obj[0][$this->gridname]['columns'][12]['readonly'] = true;
    $obj[0][$this->gridname]['columns'][12]['style'] = 'width:100px;whiteSpace:normal;min-width:100px;';
    $obj[0][$this->gridname]['columns'][13]['readonly'] = true;
    $obj[0][$this->gridname]['columns'][15]['style'] = 'width:100px;whiteSpace:normal;min-width:100px;';
  ...
  {
    $tbuttons = ['generatestudcurriculum', 'deleteallsched', 'otherfees', 'credentials', 'assummary', 'addbooks'];
    $obj = $this->tabClass->createtabbutton($tbuttons);
    $obj[0]['label'] = 'Extract Curriculum';
    $obj[0]['lookupclass'] = 'stockstatus';
    $obj[1]['label'] = 'Delete Curriculum';
    $obj[1]['access'] = 'edit';
    $obj[5]['label'] = 'View Books';
    $obj[5]['icon'] = 'books';
    $obj[5]['access'] = 'edit';
    $obj[5]['lookupclass'] = 'otherfees';
    return $obj;
  }
  ...
  public function createHeadField($config)
  {
    ...
    data_set($col1, 'client.addedparams', ['ischinese',]);
    ...
    data_set($col3, 'levels.type', 'lookup');
  ...
  public function loadheaddata($config)
  {
    ...
    $qryselect = "select head.trno,head.doc,head.docno,client.client,client.clientname,head.dateid,head.courseid,course.coursecode,course.coursename,head.periodid,
      ...
      head.curriculumtrno,head.disc, head.ischinese ";
  ...
  public function postingTrans($config)
  {
    ...
    $qry = "
      insert into " . $this->hhead . "(trno, doc, docno, dateid, periodid, syid, deptid, levelid, yr, semid, sectionid, clientid, terms, courseid,
        contra, editdate, editby, createdate, createby, encodeddate, encodedby, viewdate, viewby, lockdate, rem, modeofpayment, curriculumtrno, disc, ischinese)
    select head.trno, head.doc, head.docno, head.dateid, head.periodid, head.syid, head.deptid, head.levelid, head.yr, head.semid, head.sectionid,
      ...
      head.encodedby, head.viewdate, head.viewby, head.lockdate, head.rem, head.modeofpayment, head.curriculumtrno,head.disc, head.ischinese  FROM " . $this->head . " as head
    ...
    if ($posthead) {
      // for glstock
      $qry = "insert into " . $this->hstock . "(trno,line,schedday,units,lecture,laboratory,hours,subjectid,roomid,bldgid,instructorid,schedstarttime,schedendtime,refx,linex,ctrno,cline,scline,semid)
        select stock.trno,stock.line,stock.schedday,stock.units,stock.lecture,stock.laboratory,stock.hours,stock.subjectid,stock.roomid,stock.bldgid,stock.instructorid,null,null,stock.refx,stock.linex,stock.ctrno,stock.cline,stock.scline,stock.semid 
        ...
        if ($this->coreFunctions->execqry($qry, 'insert', [$trno])) {
          ...
          if ($this->coreFunctions->execqry($qry, 'insert', [$trno])) {
            ...
            if ($this->coreFunctions->execqry($qry, 'insert', [$trno])) {
              ...
              $this->coreFunctions->execqry("delete from " . $this->assessment . " where trno=?", "delete", [$trno]);
              $this->coreFunctions->execqry("delete from " . $this->head . " where trno=?", "delete", [$trno]);
              $this->coreFunctions->execqry("update en_studentinfo set assesstrno=? where clientid=?", 'update', [$trno, $client]);
  ...
  public function unposttrans($config)
  {
    ...
    $docno = $this->coreFunctions->datareader('select docno as value from ' . $this->tablenum . ' where trno=?', [$trno]);
    $client = $this->coreFunctions->datareader('select clientid as value from ' . $this->hhead . ' where trno=?', [$trno]);

    $qry = "insert into " . $this->head . "(trno, doc, docno, dateid, periodid, syid, deptid, levelid, yr, semid, section, client, terms, courseid,
      contra, editdate, editby, createdate, createby, encodeddate, encodedby, viewdate, viewby, lockdate, rem, modeofpayment, curriculumtrno, sectionid, disc, ischinese)
      ...
      head.encodedby, head.viewdate, head.viewby, head.lockdate, head.rem, head.modeofpayment, head.curriculumtrno, head.sectionid, head.disc, head.ischinese FROM " . $this->hhead . " as head
    ...
    //head
    if ($this->coreFunctions->execqry($qry, 'insert', [$trno])) {
      $qry = "insert into " . $this->stock . "(trno,line,schedday,units,lecture,laboratory,hours,subjectid,roomid,bldgid,instructorid,schedstarttime,schedendtime,refx,linex,ctrno,cline,scline,semid)
        select stock.trno,stock.line,stock.schedday,stock.units,stock.lecture,stock.laboratory,stock.hours,stock.subjectid,stock.roomid,stock.bldgid,stock.instructorid,stock.schedstarttime,stock.schedendtime,stock.refx,stock.linex,stock.ctrno,stock.cline,stock.scline,stock.semid
        ...
        if ($this->coreFunctions->execqry($qry, 'insert', [$trno])) {
          ...
          if ($this->coreFunctions->execqry($qry, 'insert', [$trno])) {
            ...
            if ($this->coreFunctions->execqry($qry, 'insert', [$trno])) {
              ...
              $this->coreFunctions->execqry("update en_studentinfo set assesstrno=0 where clientid=?", 'update', [$client]);
  ...
  private function getstockselect($config)
  {
    $sqlselect = "select stock.trno,stock.line,stock.subjectid,sub.subjectcode,sub.subjectname,stock.units,stock.laboratory,stock.lecture,stock.hours,stock.instructorid,i.client as eainstructorcode,i.clientname as instructorname,stock.schedday,stock.schedstarttime,stock.schedendtime, stock.roomid,r.roomcode as roomcode,stock.bldgid, b.bldgcode as lbldgcode, stock.schedref,stock.origdocno,stock.origsubjectcode, stock.refx,stock.linex,
      ...
      '' as errcolor,s.maxslot-(select count(ea.trno) from (
        ...
        where head.doc='EA') as ea where ea.refx=stock.refx and ea.linex=stock.linex) as availableslot,sem.term ";
    return $sqlselect;
  }
  ...
  public function openstock($trno, $config)
  {
    ...
    $qry = $sqlselect . " 
      ...
      left join en_glsubject as s on s.trno=stock.refx and s.line=stock.linex left join en_term as sem on sem.line=stock.semid
  ...
  public function generatefeessummary($config)
  {
    ...
    if (!empty($subjectdetail)) {
      ...
      foreach ($otherfeesdetail as $key2 => $value) {
              ...
              if ($strSubject > 0) {
                foreach ($dtsubject as $key2 => $value) {
                  if ($dtsubject[$key2]->subjectid > 0) {
                    $dcFess =  $this->computefees($strFeesCode, $strSCheme, $dcRate, $strFeesType, $config, $dtsubject[$key2]->subjectid);
                  }
                }
              }
            }
        ...
        if ($strFeesType != 'OTHERS' && $strFeesType != 'LAB') {
  ...
  public function computefees($FeesCode, $scheme, $dcRate, $strFeesType, $config, $strSubject)
  {
    ...
    if ($strSubject > 0) {
      ...
      $subjectlist = $this->coreFunctions->opentable($qry, [$trno, $strSubject]);
    } else {
      $qry = "select stock.lecture,stock.hours,stock.units,stock.laboratory from en_sosubject as stock where stock.trno= ? ";
  ...
  public function additem($action, $config)
  {
    ...
    if ($action == 'insert') {
      if ($this->coreFunctions->sbcinsert($this->stock, $data) == 1) {
        $this->logger->sbcwritelog($trno, $config, 'SUBJECT', 'ADD - Line:' . $line . ' Subject:' . $data['origsubjectcode']);
  ...
  public function deleteallitem($config)
  {
    ...
    $this->logger->sbcwritelog($trno, $config, 'STOCK', 'DELETED ALL SUBJECTS');
  ...
  public function reportdata($config)
  {
    ...
    $eiwithbooks = $config['params']['dataparams']['eiwithbooks'];
    ...
    switch ($eiwithbooks) {
      case 'booksonly':
        $books = app($this->companysetup->getreportpath($config['params']))->withbooks_query($config);
        $str = app($this->companysetup->getreportpath($config['params']))->booksonly_plotting($config, $data, $books);
        break;
      case 'assessmentonly':
        $assess = app($this->companysetup->getreportpath($config['params']))->assessment_query($config);
        $str = app($this->companysetup->getreportpath($config['params']))->assessment_report_plotting($config, $data, $assess, $books = []);
        break;
      default:
        switch ($eiassessment) {
          case 'withassess':
            switch ($eischedule) {
              ...
              default:
                $assess = app($this->companysetup->getreportpath($config['params']))->assessment_query($config);
                $books = app($this->companysetup->getreportpath($config['params']))->withbooks_query($config);
                $str = app($this->companysetup->getreportpath($config['params']))->assessment2_report_plotting($config, $data, $assess, $books);
                break;
            }
            break;
          default:
            switch ($eischedule) {
              case 'withsched':
                $books = app($this->companysetup->getreportpath($config['params']))->withbooks_query($config);
                $str = app($this->companysetup->getreportpath($config['params']))->withschedplotting($config, $data, $books);
                break;
              default:
                $books = app($this->companysetup->getreportpath($config['params']))->withbooks_query($config);
  ...
  {
    $trno = $config['params']['trno'];
    $head = $this->coreFunctions->opentable("select curriculumtrno, yr, semid, levelid from en_sohead where trno=?", [$trno]);
    ...
    $ishs = $this->coreFunctions->datareader("select ishighschool as value from en_levels where line=$levelid limit 1");

    if ($ishs == 0) {
      $qry = "select stock.trno, stock.line, stock.cline, stock.subjectid, yr.year, yr.semid, stock.units, stock.lecture, stock.laboratory, stock.hours
      from en_glhead as head left join en_glsubject as stock on stock.trno=head.trno
      left join en_glyear as yr on yr.trno=stock.trno and yr.line=stock.cline
        where head.trno=? and yr.year=? and yr.semid=?";
      $subjects = $this->coreFunctions->opentable($qry, [$curriculumtrno, $yr, $semid]);
    } else {
      $qry = "select stock.trno, stock.line, stock.cline, stock.subjectid, yr.year, yr.semid, stock.units, stock.lecture, stock.laboratory, stock.hours
      from en_glhead as head left join en_glsubject as stock on stock.trno=head.trno
      left join en_glyear as yr on yr.trno=stock.trno and yr.line=stock.cline
        where head.trno=? and yr.year=? ";
      $subjects = $this->coreFunctions->opentable($qry, [$curriculumtrno, $yr]);
    }
    ...
    foreach ($subjects as $sub) {
      $c = $this->coreFunctions->opentable("select line from $this->stock where cline=? and scline=? and trno=?", [$sub->cline, $sub->line, $sub->trno]);
      if (empty($c)) {
        $line = $this->coreFunctions->datareader("select line as value from $this->stock where trno=$trno order by line desc limit 1");
        if ($line == '') $line = 0;
        $line += 1;
        $qry = "insert into $this->stock(trno, line, subjectid, ctrno, cline, scline, units, lecture, laboratory, hours, semid) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $this->coreFunctions->execqry($qry, 'insert', [$trno, $line, $sub->subjectid, $sub->trno, $sub->cline, $sub->line, $sub->units, $sub->lecture, $sub->laboratory, $sub->hours, $sub->semid]);
      }
    }

...
class en_levelup
{
  ...
  {
    $schoolyear  = $this->coreFunctions->getfieldvalue('en_schoolyear', 'line', 'issy=1');
    $periodid = $this->coreFunctions->getfieldvalue('en_period', 'line', 'isactive=1');
    $limit = "limit 50";
    $condition = "";
    ...
    $qry = "select head.trno,head.docno,head.clientname,left(head.dateid,10) as dateid,'PENDING' as status,head.createby,head.editby,head.viewby, num.postedby , head.approveddate 
       from " . $this->hhead . " as head left join " . $this->tablenum . " as num 
       on num.trno=head.trno where head.doc='ES' and head.syid=?  and head.periodid=? and head.approveddate is null and num.postdate is not null " . $condition . " " . $filtersearch . "
       order by dateid desc, docno desc " . $limit;


    $data = $this->coreFunctions->opentable($qry, [$schoolyear, $periodid]);
  ...
  {
    // $tab = [
    //   'tableentry' => ['action' => 'tableentry', 'lookupclass' => 'entryapprovetrqty', 'label' => 'STUDENT']
    // ];
    $tab = [
  ...
  {
    $fields = ['docno', 'coursecode', 'coursename', 'advisername'];
    $col1 = $this->fieldClass->create($fields);
    data_set($col1, 'docno.label', 'Transaction#');
    data_set($col1, 'docno.type', 'input');

    $fields = ['dateid', 'curriculumdocno', 'curriculumcode', 'sy'];
    $col2 = $this->fieldClass->create($fields);
    data_set($col2, 'curriculumdocno.type', 'input');
    data_set($col2, 'curriculumcode.type', 'input');
    data_set($col2, 'sy.type', 'input');

    $fields = ['period', 'yr', 'semester', 'section'];
    $col3 = $this->fieldClass->create($fields);
    data_set($col3, 'period.type', 'input');
    data_set($col3, 'yr.type', 'input');
    data_set($col3, 'semester.type', 'input');
    data_set($col3, 'section.type', 'input');

    $fields = ['levelup'];
    $col4 = $this->fieldClass->create($fields);
    data_set($col4, 'levelup.type', 'input');
  ...
  public function loadheaddata($config)
  {
    ...
    if ($trno == 0) {
      ...
      if ($trno == '') {
        $trno = $this->coreFunctions->datareader("select trno as value from " . $this->tablenum . " where doc='ES' and center=? order by trno desc limit 1", [$doc, $center]);
      }
    ...
    $center = $config['params']['center'];
    $head = [];
    $islocked = false; //$this->othersClass->islocked($config);
    $isposted = false; //$this->othersClass->isposted($config);
    $table = $this->head;
    $htable = $this->hhead;
    $tablenum = $this->tablenum;
    $qryselect = "select 
    head.trno,head.doc,head.docno,head.dateid,head.rem,head.curriculumcode,head.curriculumname,head.curriculumdocno,
    i.clientname as advisername,c.coursecode,p.code as period,sy.sy,sem.term as terms,sec.section,c.coursename,head.yr,y.levelup ";

    $qry = $qryselect . " from " . $htable . " as head left join client as i on i.clientid=head.adviserid left join en_course as c on c.line=courseid
    left join en_period as p on p.line=head.periodid left join en_schoolyear as sy on sy.line=head.syid left join en_term as sem on sem.line=head.semid
    left join en_section as sec on sec.line=head.sectionid left join en_glhead as h on h.docno=head.curriculumdocno  left join en_glyear as y on y.trno=h.trno 
    where head.doc='ES' and head.trno=? ";
  ...
  private function report_default_query($trno)
  {
    ...
    item.barcode, item.itemname, stock.uom, stock.wh as stockwh,

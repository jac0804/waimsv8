...
class entrytransnumpicture
{
  ...
  private $table = 'transnum_picture';
  private $othersClass;
  ...
  public function createTab($config)
  {
    ...
    if ($doc == 'RG') { //main
      $viewallow = $this->othersClass->checkAccess($config['params']['user'], 4983);
      $downloadallow = $this->othersClass->checkAccess($config['params']['user'], 4984);
      $delallow = $this->othersClass->checkAccess($config['params']['user'], 4985);
    } else {
  ...
  public function createtabbutton($config)
  {
    ...
    if ($doc == 'RG') { //main
      $allow = $this->othersClass->checkAccess($config['params']['user'], 4982);
    } else {
  ...
  {
    $qry = "'tp' as type,md5(trno) as trno2,md5(line) as line2,trno,line,title,picture,

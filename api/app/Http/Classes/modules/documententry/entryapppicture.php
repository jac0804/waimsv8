...
class entryapppicture
{
  ...
  public function createTab($config)
  {
    ...
    $customername = $this->coreFunctions->datareader("select concat(emplast, ', ',empfirst,' ',empmiddle) as value from app where empid = ? ", [$clientid]);
    ...
    $obj[0]['inventory']['columns'][$ext]['label'] = 'FileType';
    $obj[0]['inventory']['columns'][$ext]['align'] = 'left';
    $obj[0]['inventory']['columns'][$ext]['style'] = 'width: 40px;whiteSpace: normal;min-width:40px;max-width:40px;';
    $obj[0]['inventory']['columns'][$action]['style'] = 'width: 20px;whiteSpace: normal;min-width:20px;max-width:20px;';
    return $obj;
  }
  ...
  {
    $qry = "'app' as type,md5(trno) as trno2,md5(line) as line2,trno,line,title,picture,left(encodeddate, 10) as encodeddate,encodedby,right(picture,3) as ext";

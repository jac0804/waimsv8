...
class cc
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'TICKET HISTORY';
  ...
  {
    $getcols = ['action', 'listdocument', 'listdate', 'listclientname', 'listpostedby', 'listcreateby', 'listeditby', 'listviewby'];
    $stockbuttons = ['view'];
  ...
  public function loaddoclisting($config)
  {
    ...
    if (isset($config['params']['search'])) {
      $searchfield = ['head.docno', 'client.clientname', 'num.postedby', 'head.createby', 'head.editby', 'head.viewby'];
    ...
    $qry = "
     select head.trno,head.docno,client.clientname,left(head.dateid,10) as dateid,head.ourref as status,head.createby,head.editby,head.viewby, num.postedby  
     from " . $this->hhead . " as head left join " . $this->tablenum . " as num 
     on num.trno=head.trno left join client on client.clientid=head.clientid where head.doc=? and num.center=? and convert(head.dateid,DATE)>=? and CONVERT(head.dateid,DATE)<=? " . $condition . " " . $filtersearch . "

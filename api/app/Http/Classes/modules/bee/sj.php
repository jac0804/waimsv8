...
namespace App\Http\Classes\modules\bee;
...
class sj
{
  ...
  public function createTab($access, $config)
  {
    ...
    $columns = ['acno', 'acnoname', 'db', 'cr', 'postdate',  'client', 'ref', 'rem'];
    ...
    $obj[0]['accounting']['columns'][$acnoname]['type'] = 'input';
    $obj[0]['accounting']['columns'][$client]['type'] = 'input';
    $obj[0]['accounting']['columns'][$postdate]['type'] = 'input';
    $obj[0]['accounting']['columns'][$ref]['type'] = 'input';

    $obj[0]['accounting']['columns'][$acnoname]['label'] = 'Account Name';
    $obj[0]['accounting']['columns'][$client]['label'] = 'Customer';

    $obj[0]['accounting']['columns'][$client]['style'] = 'text-align:left;width:150px;whiteSpace: normal;min-width:150px;';
    $obj[0]['accounting']['columns'][$acnoname]['style'] = 'text-align:left;width:250px;whiteSpace: normal;min-width:250px;';
    $obj[0]['accounting']['columns'][$postdate]['style'] = 'text-align:left;width:120px;whiteSpace: normal;min-width:120px;';
    ...
    $obj[0]['accounting']['columns'][$ref]['readonly'] = true;
    $obj[0]['accounting']['columns'][$acno]['readonly'] = true;
  ...
  {
    $allowcancel = $this->othersClass->checkAccess($config['params']['user'], 3886);
    ...
    $fields = [['dateid', 'terms'], 'dprojectname', 'address'];
  ...
  {
    $trno = $config['params']['trno'];
    $qry  = "select (db-bal) as value from arledger where trno = ?";
    $ref  =  $this->coreFunctions->datareader("select reftrno as value from temptrans where trno = ? ", [$trno]);

    $archeck = $this->coreFunctions->datareader($qry, [$trno]);
    if ($archeck == 0) {
      ...
      $this->coreFunctions->execqry('update temptrans set iscancel = 1 where trno =? and reftrno=?', 'update', [$trno, $ref]);
      $this->logger->sbcwritelog($trno, $config, 'ACCTG', 'CANCEL TRANSACTION: DELETED ALL ACCTG ENTRIES');
      $msg = "Cancel Transaction Success";
      $status = true;
      $load = true;
    } else {
      $qry  = "select group_concat(docno) as value from (
                select docno from ladetail as detail
                left join lahead as head on detail.trno = head.trno
                where refx = $trno
                union all
                select docno from gldetail as detail
                left join glhead as head on detail.trno = head.trno
                where refx = $trno) as xx
              ";
      $docno = $this->coreFunctions->datareader($qry, [$trno, $trno]);

      $msg = "This Transaction cannot be CANCEL, Already have a payment reference ( $docno ) ";
      $status = false;
      $load = false;
    }

    return ['status' => $status, 'msg' => $msg, 'reloadhead' => $load];
  }

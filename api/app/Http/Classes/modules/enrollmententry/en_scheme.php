...
class en_scheme
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'SCHEME';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'en_scheme';
  private $othersClass;
  public $style = 'width:100%;';
  private $fields = ['scheme', 'orderscheme'];
  ...
  {
    $tab = [$this->gridname => ['gridcolumns' => ['scheme', 'orderscheme']]];
    $stockbuttons = [];
  ...
  {
    $exist = $this->coreFunctions->datareader("select line as value from en_scheme limit 1");
    if (!empty($exist)) {
      $data = $this->loaddata($config);
      return ['status' => false, 'msg' => 'Already have setup.', 'data' => $data];
    }
    $qry = "insert into en_scheme(scheme, orderscheme) values('PER UNIT', 0), ('PER SUBJECT', 0), ('FIXED', 0)";
  ...
  private function report_default_query($config)
  {
    ...
    $query = "
      select scheme, orderscheme from en_scheme
    ";
  ...
  //   $str = $this->rpt_scheme_LAYOUT($data,$config);
  ...
  public function reportdata($config)
  {
    ...
    if ($config['params']['dataparams']['print'] == "default") {
      ...
      $str = $this->PDF_scheme_LAYOUT($data, $config);
    }
    return ['status' => true, 'msg' => 'Generating report successfully.', 'report' => $str];
  }
  ...
  private function PDF_default_header($data, $filters)
  {
    ...
    PDF::MultiCell(800, 20, 'SCHEME LIST', '', 'L', false);

    PDF::SetFont($font, '', 9);
    PDF::MultiCell(800, 20, "Page " . PDF::PageNo() . "  ", '', 'L', false);

    PDF::SetFont($fontbold, '', 11);
    PDF::MultiCell(300, 20, "SCHEME", '', 'L', false, 0);
  ...
  private function PDF_scheme_LAYOUT($data, $filters)
  {
    $companyid = $filters['params']['companyid'];
    $decimal = $this->companysetup->getdecimal('currency',  $filters['params']);
    ...
    foreach ($data as $key => $value) {
      ...
      PDF::MultiCell(300, 10, $value->scheme, '', 'L', 0, 0, '', '', true, 0, true, false);
  ...
  private function rpt_default_header($data, $filters)
  {
    ...
    $str .= $this->reporter->col('SCHEME LIST', '600', null, false, $border, '', 'L', $font, '18', 'B', '', '');
    ...
    $str .= $this->reporter->begintable('800');
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('SCHEME', '100', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '30px', '8px');
    $str .= $this->reporter->col('ORDER', '100', null, false, $border, 'B', 'C', $font, $fontsize, 'B', '30px', '8px');
    return $str;
  }

  private function rpt_scheme_LAYOUT($data, $filters)
  {
    $companyid = $filters['params']['companyid'];
    $decimal = $this->companysetup->getdecimal('currency',  $filters['params']);
    ...
    foreach ($data as $key => $value) {
      $str .= $this->reporter->startrow();
      $str .= $this->reporter->col($value->scheme, '100', null, false, $border, '', 'C', $font, $fontsize, '', '', '2px');
      $str .= $this->reporter->col($value->orderscheme, '100', null, false, $border, '', 'C', $font, $fontsize, '', '', '2px');

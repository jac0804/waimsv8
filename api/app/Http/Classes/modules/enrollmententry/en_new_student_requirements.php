...
use PDF;
use TCPDF_FONTS;
class en_new_student_requirements
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'NEW STUDENT REQUIREMENTS';
  ...
  public function checkReq($data, $type = 'uppdate') {
  ...
  public function lookuplogs($config)
  {
    ...
    $lookupsetup = array(
      'type' => 'show',
      'title' => 'New Student Requirement Logs',
    ...
    where log.doc = 'en_transferee_requirements'
    union all
    select trno, doc, task, log.user, dateid, 
    if(pic='','blank_user.png',pic) as pic
  ...
  private function loaddataperrecord($line)
  {
    ...
    $qry = "select " . $select . " from " . $this->table . " where studenttype = '0' and line=?";
  ...
  public function loaddata($config)
  {
    $select = $this->selectqry();
    $select = $select . ",'' as bgcolor ";
    $qry = "select " . $select . " from " . $this->table . " where studenttype = '0' order by line";
    $data = $this->coreFunctions->opentable($qry);
    return $data;
  }

...
class viewgradeentry
{
  ...
  private $fields = ['trno', 'line', 'name', 'gcsubcode', 'gcsubtopic', 'gcsubnoofitems', 'points'];
  ...
  {
    $tab = [$this->gridname => ['gridcolumns' => ['name', 'gcsubcode', 'gcsubtopic', 'gcsubnoofitems', 'points']]];
    $stockbuttons = [];
    $obj = $this->tabClass->createtab($tab, $stockbuttons);

    $obj[0][$this->gridname]['columns'][0]['type'] = "label";
    $obj[0][$this->gridname]['columns'][0]['style'] = 'width:60px;whiteSpace:normal;min-width:180px;';
    $obj[0][$this->gridname]['columns'][1]['type'] = "label";
    $obj[0][$this->gridname]['columns'][1]['style'] = 'width:60px;whiteSpace:normal;min-width:80px;';
    $obj[0][$this->gridname]['columns'][2]['type'] = "label";
    $obj[0][$this->gridname]['columns'][3]['type'] = "label";
    $obj[0][$this->gridname]['columns'][3]['style'] = 'width:60px;whiteSpace:normal;min-width:80px;';
  ...
  {
    return "select client.clientname as name,stock.trno, stock.line,stock.gcsubcode, stock.topic as gcsubtopic, stock.noofitems as gcsubnoofitems, stock.points ";
  }
  ...
  private function loaddataperrecord($trno, $compid, $line)
  {
    ...
    $qry = $select . " from en_glgrades  as stock left join client on client.clientid=stock.clientid  where stock.trno=?";
  ...
  public function loaddata($config)
  {
    ...
    $select = $this->selectqry();
    $select = $select . ",'' as bgcolor ";
    $qry = $select . " from en_glgrades as stock left join client on client.clientid=stock.clientid where stock.trno=? order by client.clientname,stock.gcsubcode";

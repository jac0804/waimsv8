...
class dr
{
    ...
    public $head = 'drhead';
    public $hhead = 'hdrhead';
    public $stock = 'drstock';
    public $hstock = 'hdrstock';
    ...
    public $damt = 'isamt';
    public $hamt = 'amt';
    public $fields = ['trno', 'docno', 'dateid', 'due', 'client', 'clientname', 'yourref', 'ourref', 'rem', 'terms', 'forex', 'cur', 'wh', 'address'];
    ...
    public function loaddoclisting($config)
    {
        ...
        $lscolor = " case when num.postdate is null and head.lockdate is null then 'red' when head.lockdate is not null then 'green' else 'grey' end ";
        ...
        $qry = "select head.trno,head.docno,head.clientname,left(head.dateid,10) as dateid, " . $lstatus . " as status,num.postedby,
     head.yourref, head.ourref,case ifnull(head.lockdate,'') when '' 
                  then $lscolor else 'green' end as statuscolor, head.createby, head.editby, head.viewby  $lfield
     ...
     union all
     select head.trno,head.docno,head.clientname,left(head.dateid,10) as dateid,'POSTED' as status, num.postedby,
      head.yourref, head.ourref,'grey' as statuscolor,head.createby, head.editby, head.viewby  $gfield
    ...
    public function createTab($access, $config)
    {
        ...
        $column = ['action', 'isqty', 'uom',  'isamt', 'disc', 'ext', 'rem', 'ref', 'wh'];
    ...
    public function createHeadField($config)
    {
        ...
        data_set($col2, 'dwhname.class', 'sbcreadonly');

        $fields = [['yourref', 'ourref'], ['cur', 'forex'], 'vat'];
    ...
    public function loadheaddata($config)
    {
        ...
        left join client as warehouse on warehouse.client = head.wh
          where head.trno = ? and num.center=? ";
        // var_dump($qry, [$trno, $center, $trno, $center]);
    ...
    public function posttrans($config)
    {
      ...
      terms,rem,forex,yourref,ourref,createdate,createby,editby,editdate,lockdate,lockuser,agent,wh,due,cur,projectid" . $addfield . ")
      ...
      head.due,head.cur,head.projectid " . $addfieldfilter . "
      ...
      where head.trno=? limit 1";
        ...
        whid,disc,iss,void,isamt,amt,isqty,ext,
        encodeddate,encodedby,editdate,editby,refx,linex,rem,ref" . $addsfield . ")
        SELECT trno, line, itemid, uom,whid,disc, iss,void,isamt,amt, isqty, ext,
    ...
    public function unposttrans($config)
    {
        ...
        $qry = "insert into " . $this->head . "(trno,doc,docno,client,clientname,address,dateid,terms,rem,forex,
        yourref,ourref,createdate,createby,editby,editdate,lockdate,lockuser,wh,due,cur,projectid " . $addfield . ")
        ...
        head.createby, head.editby, head.editdate, head.lockdate, head.lockuser,head.wh,head.due,head.cur,head.projectid" . $addfieldfilter . "
        ...
        if ($this->coreFunctions->execqry($qry, 'insert', [$trno])) {
            ...
            trno,line,itemid,uom,whid,disc,
            amt,iss,void,isamt,isqty,ext,rem,encodeddate,encodedby,editdate,editby,refx,linex,ref " . $addsfield . ")
            select trno, line, itemid, uom,whid,disc,amt, iss,void, isamt, isqty,
            ext,ifnull(rem,''), encodeddate,encodedby, editdate, editby,refx,linex,ref" . $addsfield . "
    ...
    round(stock.iss/ case when ifnull(uom.factor,0)=0 then 1 else uom.factor end," . $this->companysetup->getdecimal('qty', $config['params']) . ") as qa,
    stock.whid,
    ...
    public function getsosummary($config)
    {
        ...
        $sj_url = 'App\Http\Classes\modules\sales\\' . 'sj';

        foreach ($config['params']['rows'] as $key => $value) {
            $qry = app($sj_url)->getsosummaryqry($config);
            ...
            if (!empty($data)) {
                ...
                foreach ($data as $key2 => $value) {
                    ...
                    if ($return['status']) {
                        if (app($sj_url)->setserveditems($data[$key2]->trno, $data[$key2]->line, $companyid) == 0) {
                            $data2 = [$this->dqty => 0, $this->hqty => 0, 'ext' => 0];
                            $line = $return['row'][0]->line;
                            $config['params']['trno'] = $trno;
                            $config['params']['line'] = $line;
                            $this->coreFunctions->sbcupdate($this->stock, $data2, ['trno' => $trno, 'line' => $line]);
                            app($sj_url)->setserveditems($data[$key2]->trno, $data[$key2]->line, $companyid);
        ...
        foreach ($config['params']['rows'] as $key => $value) {
            ...
            if (!empty($data)) {
                ...
                foreach ($data as $key2 => $value) {
                    ...
                    if ($return['status']) {
                        if (app($sj_url)->setserveditems($data[$key2]->trno, $data[$key2]->line, $config['params']['companyid']) == 0) {
                            $data2 = [$this->dqty => 0, $this->hqty => 0, 'ext' => 0];
                            $line = $return['row'][0]->line;
                            $config['params']['trno'] = $trno;
                            $config['params']['line'] = $line;
                            $this->coreFunctions->sbcupdate($this->stock, $data2, ['trno' => $trno, 'line' => $line]);
                            app($sj_url)->setserveditems($data[$key2]->trno, $data[$key2]->line, $config['params']['companyid']);
                            $row = $this->openstockline($config);
                            $return = ['row' => $row, 'status' => true, 'msg' => $msg];
                        }
                        array_push($rows, $return['row'][0]);
    ...
    public function diagram($config)
    {
     ...
     from hdrhead as so 
     left join hdrstock as s on s.trno = so.trno
     where so.trno = ? 
    ...
    public function additem($action, $config, $setlog = false)
    {
        ...
        $computedata = $this->othersClass->computestock($amt, $disc, $qty, $factor, 0, 'P');
        ...
        if ($action == 'insert') {
            ...
            if ($this->coreFunctions->sbcinsert($this->stock, $data) == 1) {
                ...
                if ($refx != 0) {
                    if (app($sj_url)->setserveditems($refx, $linex, $config['params']['companyid']) == 0) {
                        $data2 = [$this->dqty => 0, $this->hqty => 0, 'ext' => 0];
                        $this->coreFunctions->sbcupdate($this->stock, $data2, ['trno' => $trno, 'line' => $line]);
                        app($sj_url)->setserveditems($refx, $linex, $config['params']['companyid']);
                        $return = false;
    ...
    public function deleteallitem($config)
    {
        ...
        // THIS FUNCTION WILL DELETE ALL ITEMS IN THE TRANSACTION, IT WILL NOT DELETE THE HEAD, SO YOU CAN STILL USE THE SAME TRNO TO RE-ADD ITEMS
        $this->coreFunctions->execqry('delete from ' . $this->stock . ' where trno=?', 'delete', [$trno]);
        $this->coreFunctions->execqry('delete from stockinfotrans where trno=?', 'delete', [$trno]);
        ...
        $sj_url = 'App\Http\Classes\modules\sales\\' . 'sj';
        foreach ($data as $key => $value) {
            app($sj_url)->setserveditems($data[0]->refx, $data[0]->linex, $config['params']['companyid']);
        }
    ...
    {
        // THIS FUNCTION WILL DELETE THE SELECTED LINE, IT WILL NOT DELETE THE HEAD, SO YOU CAN STILL USE THE SAME TRNO TO RE-ADD ITEM IN THE SAME LINE
    ...
    public function getlatestprice($config)
    {
        ...
        $qry = "select docno,left(dateid,10) as dateid,round(amt,2) as amt,disc,uom from(select head.docno,head.dateid,
          ...
          from drhead as head
          left join drstock as stock on stock.trno = head.trno
          ...
          stock.uom,stock.disc from hdrhead as head

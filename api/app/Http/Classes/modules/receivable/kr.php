...
class kr
{
  ...
  public function createtab2($access, $config)
  {
    ...
    if ($config['params']['companyid'] == 60) { //transpower      
      $changecode = $this->othersClass->checkAccess($config['params']['user'], 5503);
      if ($changecode) {
  ...
  public function createTab($access, $config)
  {
    ...
    $obj[0]['accounting']['columns'][$action]['style'] = 'width:40px;whiteSpace: normal;min-width:40px;max-width:40px;';
    $obj[0]['accounting']['columns'][$db]['style'] = 'width:380px;whiteSpace: normal;min-width:380px;max-width:380px;';
    $obj[0]['accounting']['columns'][$cr]['style'] = 'width:380px;whiteSpace: normal;min-width:380px;max-width:380px;';
    $obj[0]['accounting']['columns'][$postdate]['style'] = 'width:200px;whiteSpace: normal;min-width:200px;max-width:200px;';
    $obj[0]['accounting']['columns'][$rem]['style'] = 'width:200px;whiteSpace: normal;min-width:200px;max-width:200px;';
    $obj[0]['accounting']['columns'][$client]['style'] = 'width:200px;whiteSpace: normal;min-width:200px;max-width:200px;';
    $obj[0]['accounting']['columns'][$ref]['style'] = 'width:200px;whiteSpace: normal;min-width:200px;max-width:200px;';
  ...
  {
    $tbuttons = ['unpaidkr', 'deleteallitem'];
    // if ($config['params']['companyid'] == 50) { //unitech
    //   array_push($tbuttons, 'unpaidkrall');
    // }
  ...
  public function loadheaddata($config)
  {
    ...
    $qryselect = "select 
        ...
        subamh.description as subamenityname,head.disc";
  ...
  public function posttrans($config)
  {
    ...
    $qry = "insert into " . $this->hhead . "(trno,doc,docno,client,clientname,address,shipto,dateid,
                    ...
                    projectid,phaseid,modelid,blklotid,amenityid,subamenityid,disc)
                    ...
                    head.due,head.cur, 
                    head.projectid,head.phaseid,head.modelid,head.blklotid,head.amenityid,head.subamenityid,head.disc
  ...
  public function opendetail($trno, $config)
  {
    ...
    $joins = '';
    $selectqry = "ctbl.client,ledger.docno as ref,ledger.trno as artrno, ledger.kr as trno,
    ...
    if ($companyid == 17) { //unihome
      $selectqry = "ctbl.client,ledger.docno as ref,ledger.trno as artrno, ledger.kr as trno,
        ledger.line,ledger.acnoid,coa.acno,coa.acnoname,cntnum.center,ledger.clientid,
          case when (ledger.db > 0) then format(ledger.bal,2) else FORMAT(ledger.db,2) end as db,
          case when (ledger.cr > 0) then format(ledger.bal,2) else FORMAT(ledger.cr,2) end as cr,left(ledger.dateid,10) as postdate,
          ifnull(head.ourref,'') as yourref ,head.rem,'' as bgcolor,'' as errcolor ";
    }
    ...
    switch ($systype) {
      case 'REALESTATE':
        $selectqry .= ",
            detail.projectid, ifnull(proj.name,'') as projectname,
            proj.code as project,
            detail.phaseid, ph.code as phasename,
            detail.modelid, hm.model as housemodel, 
            detail.blklotid, bl.blk, bl.lot,
            am.line as amenity,
            am.description as amenityname,
            subam.line as subamenity,
            subam.description as subamenityname 
            ,detail.projectid,detail.phaseid,detail.modelid,detail.blklotid,detail.amenityid,detail.subamenityid";
        $joins = "
            left join projectmasterfile as proj on proj.line = detail.projectid    
            left join phase as ph on ph.line = detail.phaseid
            left join housemodel as hm on hm.line = detail.modelid
            left join blklot as bl on bl.line = detail.blklotid
            left join amenities as am on am.line= detail.amenityid
            left join subamenities as subam on subam.line=detail.subamenityid and subam.amenityid=detail.amenityid";

        break;
    }

    $qry = "select " . $selectqry . " from arledger as ledger
          ...
          $joins
          where ledger.kr = ? order by postdate";
  ...
  public function opendetailline($trno, $line, $config)
  {
    ...
    if ($companyid == 17) { //unihome
      $selectqry = "ctbl.client,ledger.docno as ref,ledger.trno as artrno, ledger.kr as trno,
        ledger.line,ledger.acnoid,coa.acno,coa.acnoname,cntnum.center,ledger.clientid,
          case when (ledger.db > 0) then format(ledger.bal,2) else FORMAT(ledger.db,2) end as db ,
          case when (ledger.cr > 0) then format(ledger.bal,2) else FORMAT(ledger.cr,2) end as cr,
          left(ledger.dateid,10) as postdate,
          ifnull(head.ourref,'') as yourref ,head.rem,'' as bgcolor,'' as errcolor";
    }
  ...
  {
    $trno = $config['params']['trno'];
    $data = $this->coreFunctions->opentable('select trno,line from arledger where kr=? ', [$trno]);

    foreach ($data as $key => $value) {
      $this->coreFunctions->execqry('update arledger set kr =0  where kr=?', 'update', [$trno]);
    }
  ...
  {
    $config['params']['trno'] = $config['params']['row']['artrno'];
    $config['params']['line'] = $config['params']['row']['line'];

    $trno = $config['params']['trno'];
    $line = $config['params']['line'];
    $krtrno = $this->coreFunctions->getfieldvalue("arledger", "kr", "trno=? and line=?", [$trno, $line]);
    $data = $this->opendetailline($trno, $line, $config);
    $qry = "update arledger set kr =0 where trno = ? and line =?";
    $this->coreFunctions->execqry($qry, 'update', [$trno, $line]);
    $data = json_decode(json_encode($data), true);
    $this->logger->sbcwritelog($krtrno, $config, 'ACCTG', 'REMOVED KR- Line:' . $line . ' code:' . $data[0]['acno'] . ' db:' . $data[0]['db'] . ' cr:' . $data[0]['cr'] . ' client:' . $data[0]['client'] . ' date:' . $data[0]['postdate'] . ' ref:' . $data[0]['ref']);
  ...
  public function getunpaidselected($config)
  {
    ...
    foreach ($data as $key => $value) {
      $qry = "update arledger set kr = " . $trno . " where trno = ? and line =?";

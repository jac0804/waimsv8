...
class cr
{
  ...
  public function createdoclisting($config)
  {
    ...
    switch ($companyid) {
      case 34: //evergreen
        $this->showfilterlabel = [
          ...
          ['val' => 'void', 'label' => 'Void', 'color' => 'grey'],
          ['val' => 'all', 'label' => 'All', 'color' => 'blue']
        ];

        $getcols = ['action', 'liststatus', 'listdocument', 'listdate', 'yourref', 'listplanholder', 'listclientname', 'amount', 'ourref', 'rem', 'db', 'postdate', 'listpostedby', 'listcreateby', 'listeditby', 'listviewby'];
        break;
      case 56: //homeworks
        $getcols = ['action', 'liststatus', 'listdocument', 'listdate', 'client', 'listclientname', 'listprojectname', 'yourref', 'ourref', 'rem', 'db', 'postdate', 'listpostedby', 'listcreateby', 'listeditby', 'listviewby'];
        break;

      case 29: //sbc
        $getcols = ['action', 'liststatus', 'listdocument', 'listdate', 'listclientname', 'rem', 'listprojectname', 'yourref', 'ourref', 'db', 'postdate', 'listpostedby', 'listcreateby', 'listeditby', 'listviewby'];
        break;
      default:
    ...
    switch ($config['params']['companyid']) {
      ...
      case 28: //xcomp
        break;
      case 8: //maxipro
        $cols[$listprojectname]['style'] = 'width:600px;whiteSpace: normal;min-width:600px;';
        $cols[$db]['type'] = 'coldel';
        break;
      case 34: //evergreen
        ...
        $cols[$yourref]['style'] = 'width:125px;whiteSpace: normal;min-width:150px;';
        $cols[$amount]['style'] = 'width:80px;whiteSpace: normal;min-width:80px;';
        $cols[$db]['type'] = 'coldel';
        $cols[$yourref]['label'] = 'Contract #';
        $cols[$listclientname]['label'] = 'Payor';
        $cols[$ourref]['label'] = 'Mode of Payment';
  ...
  public function paramsdatalisting($config)
  {
    ...
    if ($companyid == 34) { //evergreen
      $allownew = $this->othersClass->checkAccess($config['params']['user'], 226);
      if ($allownew == '1') {
        array_push($fields, 'pickpo');
      }

      $col1 = $this->fieldClass->create($fields);
      data_set($col1, 'pickpo.label', 'CONTRACTS');
      data_set($col1, 'pickpo.action', 'lookupclient');
      data_set($col1, 'pickpo.lookupclass', 'customereappsc');
      data_set($col1, 'pickpo.confirmlabel', 'Proceed to Create Received Payment?');
    } else {
      $col1 = [];
    }
  ...
  public function loaddoclisting($config)
  {
    ...
    if ($companyid == 34) { //evergreen
      $lstatus = "case ifnull(head.lockdate,'') when '' then (case ifnull(head.voiddate,'') when '' then 'DRAFT' else 'VOID' end) else (case ifnull(head.voiddate,'') when '' then 'FOR REVIEW' else 'VOID' end) end";
      $gstatus = "case ifnull(head.voiddate,'') when '' then 'APPROVED' else 'VOID' end";
      $gstatuscolor = 'blue';
    }
    $join = "";
    $hjoin = "";
    ...
    switch ($companyid) {
      ...
      case 60: //transpower
        ...
        $ladb = ", (select FORMAT(sum(d.db)," . $this->companysetup->getdecimal('currency', $config['params']) . ") 
       from ladetail as d left join coa on coa.acnoid = d.acnoid
        where d.trno= head.trno) as db ";
        $gldb = ", (select FORMAT(sum(d.db)," . $this->companysetup->getdecimal('currency', $config['params']) . ") 
      ...
      case 26: //bee healthy
        $dateid = "left(head.dateid,10) as dateid";
        $yourref = 'head.yourref';
        if ($searchfilter == "") $limit = 'limit 300';
        $orderby =  "order by  dateid desc, docno desc";
        break;
      case 8: //maxipro
        $dateid = "left(head.dateid,10) as dateid,concat(p.code,' - ',p.name) as projectname";
        $yourref = 'head.yourref';
        if ($searchfilter == "") $limit = 'limit 150';
        $orderby =  "order by  dateid desc, docno desc";
        $leftjoin = "left join projectmasterfile as p on p.line = head.projectid";
  ...
  public function createHeadbutton($config)
  {
    ...
    if ($config['params']['companyid'] == 34) { //evergreen
      ...
      $buttons['unlock']['label'] = 'RETURN TO CASHIER';

      $this->labellocked = "FOR REVIEW";
      $this->labelposted = "APPROVED!";
    }
  ...
  public function createtab2($access, $config)
  {
    ...
    if ($config['params']['companyid'] == 60) { //transpower      
      $changecode = $this->othersClass->checkAccess($config['params']['user'], 5502);
      if ($changecode) {
  ...
  {
    $companyid = $config['params']['companyid'];
    $tbuttons = ['additem', 'saveitem', 'deleteallitem', 'unpaid', 'unpaidall'];
    ...
    switch ($companyid) {
      case 11: //summit
        ...
        break;
      case 26: //bee healthy
        $tbuttons = ['saveitem', 'deleteallitem', 'unpaid', 'unpaidall'];
        break;
    }

    if ($this->companysetup->getisconsign($config['params'])) {
      array_push($tbuttons, 'unpaidchild');
    }

    if ($companyid == 34) { //evergreen
      $tbuttons = ['generateautoentry', 'deleteallitem'];
    }

    if ($this->companysetup->getsystemtype($config['params']) == 'REALESTATE') {
      $tbuttons = ['additem', 'saveitem', 'deleteallitem',  'generateautoentry', 'unpaid'];
    }
    if ($companyid == 40) { //cdo
      $tbuttons = ['additem', 'saveitem', 'deleteallitem', 'mccollection'];
    }

    if ($companyid == 59) { //roosevelt
      array_push($tbuttons, 'rchecks');
    }
    ...
    switch ($companyid) {
      case 26: //bee healthy
        $obj[0]['label'] = "SAVE ACCOUNT";
        $obj[1]['label'] = "DELETE ACCOUNT";
        break;
      case 34: //evergreen
        $obj[0]['label'] = "APPLY TO AR";
        $obj[0]['action'] = "applytoar";
        $obj[1]['label'] = "DELETE ACCOUNT";
        break;
      case 59: //roosevelt
        ...
        $obj[5]['lookupclass'] = "lookuprchecks";
        $obj[5]['action'] = "lookuprchecks";
        break;
      case 60: //transpower
        $obj[0]['label'] = "ADD ACCOUNT";
        $obj[0]['action'] = "adddetail";
        $obj[0]['lookupclass'] = 'adddetail';
        $obj[1]['label'] = "SAVE ACCOUNT";
        $obj[2]['label'] = "DELETE ACCOUNT";
        break;
      default:
    ...
    if ($this->companysetup->getsystemtype($config['params']) == 'REALESTATE') {
      $obj[4]['lookupclass'] = "funpaid";
      $obj[4]['action'] = "funpaid";
      $obj[4]['addedparams'] = ['projectid', 'phaseid', 'modelid', 'blklotid'];
    }
    return $obj;
  }
  ...
  public function createHeadField($config)
  {
    ...
    switch ($companyid) {
      ...
      case 26: //bee healthy
        ...
        data_set($col1, 'client2.plotting', ['clientid' => 'clientid', 'client2' => 'label', 'client' => 'client', 'clientname' => 'label', 'address' => 'description2', 'tel' => 'description']);
        data_set($col1, 'client2.selectaction', 'computeterms');
        data_set($col1, 'client2.plottype', 'callback');
        break;
      case 34: //evergreen
        data_set($col1, 'client.lookupclass', 'customereapp');
        data_set($col1, 'idno.label', 'ID Number');
        data_set($col1, 'idno.class', 'sbccsreadonly');
        break;
      case 47: //kitchenstar
        data_set($col1, 'dacnoname.label', 'Deposited To');
        data_set($col1, 'dacnoname.lookupclass', 'lookupdepositto');
        break;
    }
      ...
      default:
        switch ($companyid) {
          ...
          case 34: //evergreen
            ...
            break;
          case 40: //cdo
            $fields = ['dateid', 'amount', ['yourref', 'ourref']];
            break;
        }
        break;
    }
    ...
    if ($companyid == 40) { //cdo
      array_push($fields, ['checkno', 'checkdate']);
    }
    ...
    if ($companyid == 26) { //bee healthy
      ...
      data_set($col2, 'dprojectname.type', 'qselectedit');
      $qry = "select line as projectid, code as projectcode, name as projectname, concat(code,'~',name) as label, line as value from projectmasterfile order by line";
      $bunits = $this->coreFunctions->opentable($qry);
      if (!empty($bunits)) $bunits = json_decode(json_encode($bunits), true);
      data_set($col2, 'dprojectname.options', $bunits);
      ...
      data_set($col2, 'yourref.label', 'Your Ref.');
      data_set($col2, 'ourref.label', 'Our Ref.');
    }

    if ($companyid == 6) { //mitsukoshi
      data_set($col2, 'yourref.label', 'ARC#');
      data_set($col2, 'ourref.label', 'IOR#');
    }

    if ($companyid == 34) { //evergreen
      data_set($col2, 'amount.label', 'Amount Received');
      data_set($col2, 'ourref.label', 'Mode of Payment');
      data_set($col2, 'ourref.type', 'lookup');
      data_set($col2, 'ourref.required', true);
      data_set($col2, 'ourref.class', 'sbccsreadonly');
      data_set($col2, 'ourref.lookupclass', 'paymentmode');
      data_set($col2, 'ourref.action', 'lookuprandom');
      data_set($col2, 'yourref.label', 'Contract #');
    ...
    if ($this->companysetup->getsystemtype($config['params']) == "REALESTATE") {
      data_set($col3, 'ourref.label', 'Apply To');
      data_set($col3, 'ourref.type', 'lookup');
      data_set($col3, 'ourref.class', 'sbccsreadonly');
      data_set($col3, 'ourref.lookupclass', 'fpaymenttype');
      data_set($col3, 'ourref.action', 'lookuprandom');
      data_set($col3, 'checkno.type', 'lookup');
      data_set($col3, 'checkno.lookupclass', 'lookupdcchecks');
      data_set($col3, 'checkno.action', 'lookupdcchecks');
      data_set($col3, 'checkno.addedparams', ['client', 'projectid', 'phaseid', 'blklotid']);
    }
    ...
    if ($companyid == 34) { //evergreen
      data_set($col3, 'checkno.label', 'Check No.');
      data_set($col3, 'checkdate.label', 'Check Date');
      data_set($col3, 'rem.type', 'textarea');
      data_set($col3, 'plantype.type', 'input');
      data_set($col3, 'plantype.class', 'sbccsreadonly');
    }

    if ($companyid == 34) { //evergreen
      $fields = ['lblattached', 'lblpaid'];
      ...
      if ($this->companysetup->getsystemtype($config['params']) == "REALESTATE") {
        ...
        $fields = ['dprojectname', ['phase', 'housemodel'], ['blklot', 'lot'], 'amenityname', 'subamenityname', 'lblpaid'];
  ...
  public function loadheaddata($config)
  {
    ...
    $qryselect = "select 
        ...
        ifnull(project.code,'') as projectcode,head.qttrno,
        head.crref,head.checkno,left(head.checkdate,10) as checkdate,format(head.amount,2)as amount ,head.phaseid, ph.code as phase,
        head.modelid, hm.model as housemodel, 
        head.blklotid, bl.blk as blklot, bl.lot,head.rctrno,head.rcline,ifnull(ci.cptrno,0) as cptrno,ifnull(i.clientname,'')  as planholder,
  ...
  public function posttrans($config)
  {
    switch ($config['params']['companyid']) {
      case 21: //kinggeorge
        $this->logger->sbcwritelog($config['params']['trno'], $config, 'POST', 'Posting ' . $this->modulename);
        $result = $this->othersClass->posttransacctg($config);
        if ($result['status']) {
          $config['params']['lookupclass'] = 'next';
          $config['params']['docno'] = $this->coreFunctions->getfieldvalue($this->tablenum, "docno", "trno=?", [$config['params']['trno']]);
        ...
        break;
      case 34: //evergreen
        $ourref = $this->coreFunctions->datareader("select ourref as value from " . $this->head . " where trno = ?", [$config['params']['trno']]);
        if (strtoupper($ourref) != 'CASH') {
          $checkattachments = $this->coreFunctions->opentable("select count(*) as acount from cntnum_picture where trno=?", [$config['params']['trno']]);
          if ($checkattachments[0]->acount <= 0) {
            return ['msg' => 'Cannot Post, Attachments empty', 'status' => false, 'islocked' => false, 'isposted' => false];
          } else {
  ...
  public function stockstatus($config)
  {
    switch ($config['params']['action']) {
      ...
      case 'getunpaidselected':
        ...
        break;
      case 'generateautoentry':
        return $this->generateautoentry($config);
        break;
      case 'applytoar':
        return $this->applytoar($config);
        break;
      case 'getmccollection':
        return $this->getmccollection($config);
        break;
      case 'getrc':
  ...
  public function additem($action, $config)
  {
    ...
    switch ($companyid) {
      case 6: //mitsukoshi
        $alias = $this->coreFunctions->getfieldvalue('coa', 'alias', 'acno=?', [$acno]);
        if (substr($alias, 0, 2) == 'CR') {
          if ($rem == '') {
            return ['status' => false, 'msg' => 'Add Account Failed, notes is required for ' . $acnoname];
          }
        }
        break;
    }
    ...
    if ($action == 'insert') {
      ...
      if ($this->coreFunctions->sbcinsert($this->detail, $data) == 1) {
        ...
        if ($refx != 0) {
          $rcdoc = $this->coreFunctions->getfieldvalue('transnum', 'doc', 'trno=?', [$refx]);
          switch ($rcdoc) {
            case 'RH':
            case 'RC':
              $this->coreFunctions->sbcupdate('h' . strtolower($rcdoc) . 'detail', ['ortrno' => $trno, 'orline' => $line], ['trno' => $refx, 'line' => $linex]);
              break;
            default:
          ...
          // if ($companyid == 59) { //roosevelt
          //   $this->coreFunctions->sbcupdate('hrcdetail', ['ortrno' => $trno, 'orline' => $line], ['trno' => $refx, 'line' => $linex]);
          // } else {
          //   if (!$this->sqlquery->setupdatebal($refx, $linex, $acno, $config)) {
          //     $this->coreFunctions->sbcupdate($this->detail, ['db' => 0, 'cr' => 0, 'fdb' => 0, 'fcr' => 0], ['trno' => $trno, 'line' => $line]);
          //     $this->sqlquery->setupdatebal($refx, $linex, $acno, $config);
        ...
        if ($companyid == 19) { //housegem
          ...
          if ($refx != 0) {
            $this->coreFunctions->sbcupdate('ladetail', ['isar' => ($bal != 0) ? 1 : 0], ['trno' => $trno, 'line' => $line]);
          } else {
            $this->coreFunctions->sbcupdate('ladetail', ['isar' => ($bal != 0) ? 1 : 0], ['trno' => $trno]);
            $this->isexept($config, $trno, $line, $data['postdate']);
          }
        }
      ...
      if ($this->coreFunctions->sbcupdate($this->detail, $data, ['trno' => $trno, 'line' => $line]) == 1) {
        if ($refx != 0) {
          ...
          // $doc = $this->coreFunctions->getfieldvalue('hrchead', 'doc', 'trno=?', [$refx]);
          // if ($companyid == 59 && $doc == 'RC') { //roosevelt
          //   $this->coreFunctions->sbcupdate('hrcdetail', ['ortrno' => $trno, 'orline' => $line], ['trno' => $refx, 'line' => $linex]);
          // } else {
          //   if (!$this->sqlquery->setupdatebal($refx, $linex, $acno, $config)) {
          //     $this->coreFunctions->sbcupdate($this->detail, ['db' => 0, 'cr' => 0, 'fdb' => 0, 'fcr' => 0], ['trno' => $trno, 'line' => $line]);
  ...
  public function deleteallitem($config)
  {
    ...
    if ($companyid == 40) { //cdo
      $this->coreFunctions->sbcupdate('hmchead', ['crtrno' => 0, 'isok' => 0], ['crtrno' => $trno]);
    }
  ...
  public function deleteitem($config)
  {
    ...
    if ($data[0]->refx != 0) {
      ...
      if ($config['params']['companyid'] == 59) { //roosevelt
        $this->coreFunctions->sbcupdate('hrcdetail', ['ortrno' => 0, 'orline' => 0], ['trno' => $data[0]->refx, 'line' => $data[0]->linex]);
    ...
    if ($this->companysetup->getsystemtype($config['params']) == 'REALESTATE') {
      $this->coreFunctions->sbcupdate('hdetailinfo', ['ortrno' => 0], ['ortrno' => $trno]);
    }

    if ($config['params']['companyid'] == 40) { //cdo
      $this->coreFunctions->sbcupdate('hmchead', ['crtrno' => 0], ['crtrno' => $trno]);
    }
  ...
  public function getunpaidselected($config)
  {
    ...
    $fb = $this->coreFunctions->getfieldvalue($this->head, "contra", "trno=?", [$trno]);
    if ($config['params']['companyid'] == 34) { //evergreen
      $fb = $this->coreFunctions->getfieldvalue("coa", "acno", "alias='CA1'");
    }
    ...
    foreach ($data as $key => $value) {
      ...
      switch ($config['params']['companyid']) {
        case 28: //xcomp
          $config['params']['data']['rem'] = $data[$key]['ourref'];
          break;
      ...
      $config['params']['data']['poref'] = $data[$key]['poref'];
      $config['params']['data']['podate'] = $data[$key]['podate'];
  ...
  public function applytoar($config)
  {
    ...
    if ($cptrno != 0) {
      ...
      $bal = 0;
      $pf = 0;
      $overacnoid = $this->coreFunctions->getfieldvalue("coa", "acnoid", "alias = 'SA3'");
      if (!empty($head)) {
        switch (strtoupper($head[0]->ourref)) {
          case 'CASH':

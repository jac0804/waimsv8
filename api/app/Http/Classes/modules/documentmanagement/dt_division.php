...
class dt_division
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'DIVISION LIST';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'dt_division';
  private $othersClass;
  public $style = 'width:100%;';
  public $tablelogs = 'masterfile_log';
  public $tablelogs_del = 'del_masterfile_log';
  private $fields = ['divisionname'];
  ...
  {
    $tab = [$this->gridname => ['gridcolumns' => ['action', 'divisionname']]];
  ...
  public function save($config)
  {
    ...
    $dateTables = ['dt_division'];
    $lookups = $this->othersClass->buildSanitizeLookups($config['params']['doc'], $companyid, [], false, $dateTables);

    foreach ($this->fields as $key => $value) {
      $data[$value] = $this->othersClass->sanitizekeyfieldFast($value, $row[$value], $lookups);
    }
    $data2['division'] = $data['divisionname'];
    if ($row['id'] == 0) {
      $id = $this->coreFunctions->insertGetId($this->table, $data2);
      $this->logger->sbcmasterlog($row['id'], $config, ' CREATE - ' . $data2['division']);
      if ($id != 0) {
  ...
  public function delete($config)
  {
    ...
    $this->logger->sbcdelmaster_log($row['id'], $config, 'REMOVE - ' . $row['divisionname']);
  ...
  {
    $qry = "select id, division as divisionname, '' as bgcolor from $this->table where id=?";
  ...
  {
    $qry = "select id, division as divisionname, '' as bgcolor from $this->table order by id";
    $data = $this->coreFunctions->opentable($qry);
    return $data;
  }
  ...
  public function saveallentry($config)
  {
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        $data3['division'] = $data2['divisionname'];
        if ($data[$key]['id'] == 0) {
          $id = $this->coreFunctions->insertGetId($this->table, $data3);
          $this->logger->sbcmasterlog($data[$key]['id'], $config, ' CREATE - ' . $data3['division']);
        } else {
          $data['editdate'] = $this->othersClass->getCurrentTimeStamp();
          $data['editby'] = $config['params']['user'];

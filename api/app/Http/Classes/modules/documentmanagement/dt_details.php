...
class dt_details
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'DETAILS LIST';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'dt_details';
  private $othersClass;
  public $style = 'width:100%;';
  public $tablelogs = 'masterfile_log';
  public $tablelogs_del = 'del_masterfile_log';
  private $fields = ['details'];
  ...
  {
    $tab = [$this->gridname => ['gridcolumns' => ['action', 'details']]];
  ...
  public function save($config)
  {
    ...
    if ($row['id'] == 0) {
      ...
      $this->logger->sbcmasterlog($row['id'], $config, ' CREATE - ' . $data['details']);
      if ($id != 0) {
  ...
  public function delete($config)
  {
    ...
    $this->logger->sbcdelmaster_log($row['id'], $config, 'REMOVE - ' . $row['details']);
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['id'] == 0) {
          ...
          $this->logger->sbcmasterlog($data[$key]['id'], $config, ' CREATE - ' . $data[$key]['details']);
        } else {
          $data['editdate'] = $this->othersClass->getCurrentTimeStamp();
          $data['editby'] = $config['params']['user'];
          $this->coreFunctions->sbcupdate($this->table, $data2, ['id' => $data[$key]['details']]);
        }
      } // end if
    } // foreach

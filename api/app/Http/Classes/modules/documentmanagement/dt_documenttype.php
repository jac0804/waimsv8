...
class dt_documenttype
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'DOCUMENT TYPE LIST';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'dt_documenttype';
  private $othersClass;
  public $style = 'width:100%;';
  public $tablelogs = 'masterfile_log';
  public $tablelogs_del = 'del_masterfile_log';
  private $fields = ['documenttype'];
  ...
  {
    $tab = [$this->gridname => ['gridcolumns' => ['action', 'documenttype']]];
  ...
  public function save($config)
  {
    ...
    if ($row['id'] == 0) {
      ...
      if ($id != 0) {
        ...
        $this->logger->sbcmasterlog($row['id'], $config, ' CREATE - ' . $data['documenttype']);
      ...
      if ($this->coreFunctions->sbcupdate($this->table, $data, ['id' => $row['id']]) == 1) {
        ...
        return ['status' => false, 'msg' => 'SSaving failed.'];
      }
    }
  } //end function
  ...
  public function delete($config)
  {
    ...
    $this->logger->sbcdelmaster_log($row['id'], $config, 'REMOVE - ' . $row['documenttype']);
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['id'] == 0) {
          ...
          $this->logger->sbcmasterlog($data[$key]['id'], $config, ' CREATE - ' . $data[$key]['documenttype']);
        } else {
          $data['editdate'] = $this->othersClass->getCurrentTimeStamp();
          $data['editby'] = $config['params']['user'];

...
class jp
{
  ...
  private $logger;
  private $sqlquery;
  public $expirystatus = ['readonly' => false, 'show' => true, 'showstage' => true];
  ...
  public $damt = 'isamt';
  public $hamt = 'amt';
  public $defaultContra = 'CG1';
  private $stockselect;
  private $fields = ['trno', 'docno', 'dateid', 'client', 'clientname', 'yourref', 'ourref', 'rem', 'wh', 'contra', 'projectid', 'subproject'];
  private $fieldsOther = ['trno', 'lotno', 'batchsize', 'yield', 'expirydate'];
  ...
  public function createdoclisting($config)
  {
    ...
    $getcols = ['action', 'liststatus', 'listdocument', 'listdate', 'barcode', 'itemname', 'listclientname', 'lotno', 'listpostedby', 'listcreateby', 'listeditby', 'listviewby'];
    $stockbuttons = ['view'];
    $cols = $this->tabClass->createdoclisting($getcols, $stockbuttons);
    $cols[$action]['style'] = 'width:40px;whiteSpace: normal;min-width:40px;';
    $cols[$clientname]['style'] = 'width:300px;whiteSpace: normal;min-width:300px;';
    $cols[$barcode]['type'] = 'label';
    $cols[$itemname]['type'] = 'label';
    $cols[$itemname]['label'] = 'Item Name';
    if ($companyid != 36 && $companyid != 27) { //not rozlab & not NTE
      $cols[$barcode]['type'] = 'coldel';
      $cols[$itemname]['type'] = 'coldel';
    }
    return $cols;
  }
  ...
  public function loaddoclisting($config)
  {
    ...
    if (isset($config['params']['search'])) {
      $searchfield = ['head.docno', 'head.clientname', 'head.createby', 'head.editby', 'head.viewby', 'num.postedby', 'info.lotno', 'item.barcode', 'item.itemname'];
    ...
    $qry = "select head.trno,head.docno,head.clientname,left(head.dateid,10) as dateid, 'DRAFT' as status,head.createby,head.editby,head.viewby,num.postedby,info.lotno,item.barcode,item.itemname
     from " . $this->head . " as head 
     left join " . $this->tablenum . " as num on num.trno=head.trno 
     left join cntnuminfo as info on info.trno=head.trno
     left join item on item.itemid=info.itemid
     where head.doc=? and num.center = ? and CONVERT(head.dateid,DATE)>=? and CONVERT(head.dateid,DATE)<=? " . $condition . $projectfilter . " $filtersearch
     union all
     select head.trno,head.docno,head.clientname,left(head.dateid,10) as dateid,'POSTED' as status,head.createby,head.editby,head.viewby, num.postedby,info.lotno,item.barcode,item.itemname
  ...
  public function paramsdatalisting($config)
  {
    ...
    data_set($col1, 'pickpo.label', 'PICK BOM');
    data_set($col1, 'pickpo.lookupclass', 'getbomshortcut');
    data_set($col1, 'pickpo.action', 'pendingbom');
    data_set($col1, 'pickpo.confirmlabel', 'Proceed to pick BOM?');
    ...
    if ($companyid == 36) { //rozlab
      data_set($col2, 'yield.label', 'Theoretical Yield');
    } else {
  ...
  public function createTab($access, $config)
  {
    ...
    $loc = 10;
    $itemname = 11;
    $column = ['action', 'isqty', 'isqty2', 'uom', 'cost', 'isamt', 'disc', 'ext', 'wh', 'expiry', 'loc', 'itemname'];

    $tab = [
      $this->gridname => [
        'gridcolumns' => $column,
        'computefield' => ['dqty' => $this->dqty, 'hqty' => $this->hqty, 'damt' => $this->damt, 'hamt' => $this->hamt, 'disc' => 'disc', 'total' => 'ext'],
        'headgridbtns' => ['viewdistribution', 'viewref', 'viewdiagram', 'showheaditembalance']
      ],
    ...
    $obj[0]['inventory']['columns'][$isamt]['label'] = 'Cost';
    $obj[0]['inventory']['columns'][$isamt]['readonly'] = true;

    if ($companyid != 36) { //not rozlab
      $obj[0]['inventory']['columns'][$isqty2]['type'] = 'coldel';
    } else {
      $obj[0]['inventory']['columns'][$isqty2]['label'] = 'JO Qty';
      $obj[0]['inventory']['columns'][$isqty2]['readonly'] = true;
      $obj[0]['inventory']['columns'][$isqty2]['style'] = 'text-align:right;width:170px;whiteSpace: normal;min-width:170px;';
    }
    ...
    $obj[0]['inventory']['columns'][$disc]['style'] = 'text-align:right;width:150px;whiteSpace: normal;min-width:150px;';
    $obj[0]['inventory']['columns'][$isamt]['style'] = 'text-align:right;width:170px;whiteSpace: normal;min-width:170px;';
    $obj[0]['inventory']['columns'][$isqty]['style'] = 'text-align:right;width:170px;whiteSpace: normal;min-width:170px;';
  ...
  public function createtabbutton($config)
  {
    ...
    switch ($systemtype) {
      case 'CAIMS':
        $tbuttons = ['pendingboq_mi', 'saveitem', 'deleteallitem'];
        break;
    }
  ...
  {
    $companyid =  $config['params']['companyid'];
    $fields = ['docno', 'client', 'clientname', 'dacnoname', 'dprojectname'];
    $col1 = $this->fieldClass->create($fields);
    ...
    $fields = ['dateid', 'barcode', 'itemname', 'uom', 'lotno'];
    $col2 = $this->fieldClass->create($fields);
    data_set($col2, 'itemname.class', 'sbccsreadonly');
    data_set($col2, 'uom.class', 'sbccsreadonly');
    data_set($col2, 'barcode.class', 'sbccsreadonly');
    data_set($col2, 'barcode.type', 'input');


    $fields = ['dwhname', 'batchsize', 'uom2', 'yield'];
    if ($companyid == 36) { //rozlab
      array_push($fields, 'expiry1');
    }
    $col3 = $this->fieldClass->create($fields);
    data_set($col3, 'dwhname.required', true);

    data_set($col3, 'uom2.class', 'sbccsreadonly');

    data_set($col3, 'uom2.type', 'input');
    if ($companyid == 36) { //rozlab
      data_set($col3, 'expiry1.name', 'expirydate');
      data_set($col3, 'expiry1.label', 'Expiry Date');
      data_set($col3, 'expiry1.class', 'csexpirydate');
    }

    $fields = ['rem', ['lblcostuom', 'costuom']];
    $col4 = $this->fieldClass->create($fields);
    data_set($col4, 'costuom.label', '');
    data_set($col4, 'costuom.style', 'font-weight:bold;font-size:30px;');
    data_set($col4, 'lblcostuom.style', 'font-weight:bold;font-size:25px;');
  ...
  public function createnewtransaction($docno, $params)
  {
    ...
    $data[0]['expirydate'] = $this->othersClass->getCurrentDate();
    return $data;
  }
  ...
  public function loadheaddata($config)
  {
    ...
    $qryselect = "select 
         ...
         info.itemid, item.barcode, item.itemname, item.uom, info.batchsize, info.yield, info.lotno, info.uom2, info.expirydate ";

    $qry = $qryselect . ", FORMAT(ifnull((select sum(ext)/ifnull(info.yield,0) from " . $this->stock . " where trno=head.trno),0),6) as costuom 
        from $table as head
        ...
        union all " . $qryselect . ", FORMAT(ifnull((select sum(ext)/ifnull(info.yield,0) from " . $this->hstock . " where trno=head.trno),0),6) as costuom
        from $htable as head
        left join $tablenum as num on num.trno = head.trno
  ...
  public function posttrans($config)
  {
    ...
    if ($this->companysetup->isinvonly($config['params'])) {
      ...
      $pending = $this->coreFunctions->opentable("select trno from " . $this->stock . " where trno=? and isqty<>isqty2 and isqty2<>0", [$trno]);
      if (!empty($pending)) {
        return ['trno' => $trno, 'status' => false, 'msg' => "Posting failed. Quantity is not equal to JO quantity."];
      }
  ...
  public function additem($action, $config, $setlog = false)
  {
    ...
    $isqty2 = isset($config['params']['data']['qty2']) ? $config['params']['data']['qty2'] : '';
    ...
    $isqty2 = round($isqty2, $this->companysetup->getdecimal('qty', $config['params']));
    ...
    if ($action == 'insert') {
      ...
      if ($this->coreFunctions->sbcinsert($this->stock, $data) == 1) {
        ...
        $this->logger->sbcwritelog($trno, $config, 'STOCK', 'ADD - Line:' . $line . ' barcode:' . $item[0]->barcode . ' Qty' . $qty . ' Amt:' . $amt . ' wh:' . $wh . ' ext:' . $computedata['ext'], $setlog ? $this->tablelogs : '');

        $cost = number_format($this->othersClass->computecosting($data['itemid'], $data['whid'], $data['loc'], $expiry, $trno, $line, $data['iss'], $config['params']['doc'], $config['params']['companyid']), 6, '.', '');

        if ($cost != -1) {


          $computedata = $this->othersClass->computestock($cost * $factor, $disc, $qty, $factor);
          $isamt = round($cost * $factor, $this->companysetup->getdecimal('price', $config['params']));
          $this->coreFunctions->sbcupdate($this->stock, ['cost' => $cost, 'amt' => $computedata['amt'], 'isamt' => $isamt, 'ext' => $computedata['ext']], ['trno' => $trno, 'line' => $line]);
        } else {
          $havestock = false;
          ...
          $this->logger->sbcwritelog($trno, $config, 'STOCK', 'OUT OF STOCK - Line:' . $line . ' Barcode:' . $item[0]->barcode . ' Qty' . $qty . ' Amt:' . $amt . ' WH:' . $wh . ' Ext:0.0', $setlog ? $this->tablelogs : '');
        }
      ...
      if ($cost != -1) {
        ...
        $this->coreFunctions->sbcupdate($this->stock, [$this->dqty => 0, $this->hqty => 0, 'ext' => 0, 'editby' => 'OUT_STOCK', 'editdate' => $current_timestamp], ['trno' => $trno, 'line' => $line]);
        $this->coreFunctions->execqry('delete from costing where trno=? and line=?', 'delete', [$trno, $line]);
        $this->logger->sbcwritelog($trno, $config, 'STOCK', 'OUT OF STOCK - Line:' . $line . ' Barcode:' . $item[0]->barcode . ' Amt:' . $amt . ' WH:' . $wh . ' Ext:0.0', $setlog ? $this->tablelogs : '');
        $return = false;
        $msg = "(" . $item[0]->barcode . ") Out of Stock.";
      }
  ...
  public function getposummaryqry($config)
  {
    ...
    return "select itemc.itemid, item.itemid as bitemid, item.barcode as bbarcode, item.itemname as bitemname, item.uom as buom, client.client, bom.bclientname as clientname , bom.batchsize, bom.yield, bom.uom2, bom.rem, client.addr as address, '" . $this->companysetup->getwh($config['params']) . "' as swh,
          comp.barcode, comp.itemname, if(" . $yield . "<>0,((" . $yield . "/bom.yield)*comp.isqty),comp.isqty) as isqty, comp.uom, comp.cost as isamt, '' as ref, 0 as refx, 0 as linex, '' as yourref, '' as disc, '' as docno, 0 as trno, 0 as line, 0 as stageid, 0 as projectid
          from item left join bom on bom.itemid=item.itemid left join client on client.clientid=bom.bclientid 
          left join component as comp on comp.itemid=item.itemid left join item as itemc on itemc.barcode=comp.barcode
          where item.itemid=? and item.fg_isfinishedgood=1 and item.isinactive=0 and bom.itemid is not null order by item.itemname";
  }
  ...
  public function insertfifoexpiration($config, $value, $wh, $setlog = false)
  {
    ...
    foreach ($invdata as $key => $val) {
      ...
      if ($running_qty > 0) {
        ...
        if ($running_qty > 0 && $val->bal <= $qty) {
          $config['params']['data']['qty2'] = $running_qty +  $qty;
        } else {
          $config['params']['data']['qty2'] = $qty;
        }

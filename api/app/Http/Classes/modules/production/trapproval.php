...
class trapproval
{
  ...
  public function loaddoclisting($config)
  {
    ...
    $qry = "select head.trno,head.docno,head.clientname,left(head.dateid,10) as dateid,'PENDING' as status,head.createby,head.editby,head.viewby, num.postedby , head.approvedate 
       ...
       on num.trno=head.trno where head.doc='TR' and head.approvedate is null and num.postdate is not null and num.center = ? and CONVERT(head.dateid,DATE)>=? and CONVERT(head.dateid,DATE)<=? " . $condition . " " . $filtersearch . " 
  ...
  public function loadheaddata($config)
  {
    ...
    $qry = $qryselect . " from $htable as head
          ...
          where head.trno = ? and num.doc='TR' and num.center=? ";

...
class rm
{
  ...
  private $fieldClass;
  private $tabClass;
  public $modulename = 'RAW MATERIAL USAGE';
  ...
  public function createTab($access, $config)
  {
    ...
    $column = ['action', 'itemdescription', 'isqty', 'uom', 'isamt', 'ext', 'stage', 'void', 'barcode', 'disc', 'wh', 'assetaccount', 'revenueaccount'];
    $sortcolumn =  ['action', 'itemdescription', 'isqty', 'uom', 'isamt', 'ext', 'stage', 'void', 'barcode', 'disc', 'wh', 'assetaccount', 'revenueaccount'];
    ...
    $obj[0][$this->gridname]['columns'][$isqty]['style'] =  'width:200px;whiteSpace: normal;min-width:200px;';
    ...
    $obj[0][$this->gridname]['columns'][$assetaccount]['label'] = 'Inventory Account';
    $obj[0][$this->gridname]['columns'][$assetaccount]['action'] = 'lookupcontra';
    $obj[0][$this->gridname]['columns'][$assetaccount]['lookupclass'] = 'IN';
    $obj[0][$this->gridname]['columns'][$assetaccount]['style'] =  'width:200px;whiteSpace: normal;min-width:200px;';
  ...
  public function updatehead($config, $isupdate)
  {
    ...
    $dateTables = ['lahead', 'lastock', 'stockinfo'];
  ...
  public function posttrans($config)
  {
    ...
    if ($this->companysetup->isinvonly($config['params'])) {
      ...
      if ($companyid != 24 && $companyid != 69) { // not goodfound, not cemphil
        ...
        $missinginv = $this->coreFunctions->opentable("select trno from " . $this->stock . " where (expid=0 or invid=0) and trno=?", [$trno]);
        if (!empty($missinginv)) {
          return ['trno' => $trno, 'status' => false, 'msg' => 'Please setup inventory/expense account for all items'];
        }
      }
  ...
  public function createdistribution($config)
  {
    ...
    ifnull(" . $fieldasset . ",'') as asset,ifnull(" . $fieldrevenue . ",'') as revenue,stock.isamt,stock.disc,stock.isqty,stock.cost,stock.iss,stock.fcost,head.projectid,client.rev,stock.rebate,head.subproject,stock.stageid   
          ...
          left join coa as inv on inv.acnoid=stock.invid left join coa as exp on exp.acnoid=stock.expid
          where head.trno=?";

    $stock = $this->coreFunctions->opentable($qry, [$trno]);
    $tax = 0;
    if (!empty($stock)) {
      $invacct = $this->coreFunctions->getfieldvalue('coa', 'acno', 'alias=?', ['RM1']);
  ...
  {
    $sqlselect = "select stock.trno, stock.line, stock.itemid, item.barcode, item.itemname, stock.uom, stock.disc, stock.rem, stock.amt, 
      ...
      stock.void, stock.refx, stock.linex, stock.ref, stock.stageid, stage.stage as stage, stock.whid, warehouse.client as wh, warehouse.clientname as whname, '' as bgcolor, '' as errcolor, 0 as qa,
      stock.invid, inv.acnoname as assetaccount, stock.expid, exp.acnoname as revenueaccount";
    return $sqlselect;
  }
  ...
  public function openstock($trno, $config)
  {
    ...
    left join client as warehouse on warehouse.clientid=stock.whid left join coa as inv on inv.acnoid=stock.invid left join coa as exp on exp.acnoid=stock.expid
    where stock.trno =? 
  ...
  public function additem($action, $config, $setlog = false)
  {
    ...
    $invid = isset($config['params']['data']['invid']) ? $config['params']['data']['invid'] : 0;

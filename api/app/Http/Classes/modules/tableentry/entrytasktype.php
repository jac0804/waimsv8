...
class entrytasktype
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'TYPE MASTERFILE';
  ...
  private $fields = ['category','isdailytask'];
  ...
  {
    $columns = ['action', 'category','isdailytask'];
    $tab = [
      $this->gridname => [
        'gridcolumns' => $columns
      ]
    ...
    $obj[0][$this->gridname]['columns'][1]['label'] = 'Task Type';
  ...
  public function saveallentry($config)
  {
    ...
    foreach ($data as $key => $value) {
      if ($data[$key]['isdailytask'] == 'true') {
        $data[$key]['isdailytask'] = 1;
      } else {
        $data[$key]['isdailytask'] = 0;
      }
    ...
    if ($row['line'] == 0 && $row['category'] != '') {
      ...
      if (!empty($resultdata[0]['category'])) {
        if (trim($resultdata[0]['category']) == trim($row['category'])) {
          return ['status' => false, 'msg' => ' Task ( ' . $resultdata[0]['category'] . ' )' . ' is already exist', 'data' => [$resultdata]];
        }
      }
    }
  ...
  private function loaddataperrecord($line)
  {
    ...
    $qry = "select line,category,case when isdailytask =1 then 'true' else 'false' end as isdailytask,'' as bgcolor from " . $this->table . " as r where r.istasktype =1 and r.line=?";
  ...
  public function loaddata($config)
  {
    ...
    $qry = "select line,category,case when isdailytask =1 then 'true' else 'false' end as isdailytask,'' as bgcolor from " . $this->table . " as r  where r.istasktype =1  " . $filtersearch . " order by line $l";

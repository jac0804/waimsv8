...
class  tabcustomer
{
    ...
    public function createTab($config)
    {
        ...
        $columns = ['action', 'clientname', 'address', 'rem'];
        ...
        $obj[0][$this->gridname]['columns'][$rem]['style'] = 'width:350px;whiteSpace: normal;min-width:350px;';
    ...
    public function lookupsetup($config)
    {
        ...
        switch ($lookupclass2) {
            case 'additem':
                return $this->lookupaddcustomer($config);
                break;
            default:
    ...
    public function lookupaddcustomer($config)
    {
        $trno = $config['params']['tableid'];
        ...
        $qry = "select " . $trno . " as trno, '0' as line, clientid, 
                ...
                from client where iscustomer = 1 and isinactive = 0";
    ...
    public function save($action, $config)
    {
        ...
        $existclient = $this->coreFunctions->getfieldvalue("cntclient", "clientid", "clientid=? and trno=?", [$data['clientid'], $trno]);
        ...
        if ($existclient == 0) {
            if ($action == 'insert') {
                ...
                if ($this->coreFunctions->sbcinsert($this->table, $data)) {
                    ...
                    $this->logger->sbcwritelog($trno, $config, ' CREATE - CUSTOMER', $row['clientname']);
            ...
            return ['status' => false, 'msg' => 'There was already a customer named ' . $row['clientname'] . '.', 'data' => []];
        }
    } //end function
    ...
    public function delete($config)
    {
        ...
        $this->logger->sbcdel_log($row['trno'], $config, 'REMOVE - CUSTOMER', $row['clientname']);
    ...
    private function loaddataperrecord($config, $line)
    {
        ...
        $qry = $sqlselect . ",'' as bgcolor from " . $this->table . " as head
                ...
                where head.trno = $trno and head.line = $line and head.ishelper=0 order by line";
    ...
    public function loaddata($config)
    {
        ...
        $qry = $sqlselect . ",'' as bgcolor from " . $this->table . " as head
                left join client as c on c.clientid = head.clientid
                where head.trno = ? and head.ishelper=0 
                union all ";

...
class entryservice
{
  ...
  private $fields = ['isservice', 'code', 'description', 'color', 'filename', 'groupname'];
  ...
  {
    $attrib = array('load' => 5580);
    return $attrib;
  }

  public function createTab($config)
  {
    $columns = ['action', 'code', 'description', 'color', 'filename', 'groupname'];
    $tab = [
      $this->gridname => [
        'gridcolumns' => $columns
      ]
    ...
    $stockbuttons = ['save', 'delete', 'addpic'];
    ...
    $obj[0][$this->gridname]['columns'][$code]['label'] = 'Services';
    $obj[0][$this->gridname]['columns'][$code]['style'] = "width:150px;whiteSpace: normal;min-width:150px;";
    $obj[0][$this->gridname]['columns'][$description]['label'] = 'Short Description';
    ...
    $obj[0][$this->gridname]['columns'][$groupname]['label'] = 'Group';
    $obj[0][$this->gridname]['columns'][$action]['style'] = "width:100px;whiteSpace: normal;min-width:100px;";
    $obj[0][$this->gridname]['columns'][0]['btns']['addpic']['lookupclass'] = ['table' => $this->table, 'field' => 'picpath', 'fieldid' => 'line', 'folder' => 'services'];
    $obj[0][$this->gridname]['columns'][0]['btns']['addpic']['checkfield'] = 'isnew';
  ...
  public function loaddata($config)
  {
    ...
    $select = $this->selectqry() . ",'false' as isnew, '' as bgcolor";
    $qry    = "select " . $select . " from " . $this->table .
      " where isservice = 1 " . $filtersearch . " order by line";
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if (empty(trim($data2['code']))) {
          return ['status' => false, 'msg' => 'Saving failed. Please input code.'];
        }
  ...
  private function loaddataperrecord($line)
  {
    ...
    $select = $select . ",'false' as isnew,'' as bgcolor ";
  ...
  public function delete($config)
  {
    ...
    $qry = "select serviceline as value from (select serviceline from currentservice where serviceline=? union all select serviceline from hcurrentservice where serviceline=?) as a limit 1";

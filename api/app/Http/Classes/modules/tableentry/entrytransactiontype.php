...
class entrytransactiontype
{
    private $fieldClass;
    private $tabClass;
    public $modulename = 'TRANSACTION TYPE MASTERFILE';
    ...
    private $fields = ['category', 'inactive','isttype'];
    ...
    public function createTab($config)
    {
        ...
        $obj[0][$this->gridname]['columns'][$category]['label'] = 'Type';
    ...
    {
        $qry = " r.line,r.category,r.reqtype, case when r.inactive=0 then 'false' else 'true' end as inactive,r.isttype";
        return $qry;
    }
        ...
        foreach ($data as $key => $value) {
            ...
            if ($data[$key]['bgcolor'] != '') {
                ...
                if ($data[$key]['line'] == 0 && $data[$key]['category'] != '') {
                    $qry = "select category from reqcategory where category = '" . $data[$key]['category'] . "' and isttype=1 limit 1";
                ...
                if (trim($data[$key]['category'] == '')) {
                    return ['status' => false, 'msg' => 'Type is empty'];
                }
                ...
                if ($data[$key]['line'] == 0) {
                    ...
                    if ($data[$key]['line'] != 0 && $data[$key]['category'] != '') {
                        $qry = "select category,line from reqcategory where category = '" . $data[$key]['category'] . "'  and isttype=1 and line <> ".$data[$key]['line']." limit 1";
                        $opendata = $this->coreFunctions->opentable($qry);
                        $resultdata =  json_decode(json_encode($opendata), true);
                        if (!empty($resultdata)) {
    ...
    public function save($config)
    {
        ...
        if ($row['line'] == 0 && $row['category'] != '') {
            $qry = "select category from reqcategory where category = '" . $row['category'] . "'  and isttype=1 limit 1";
            $opendata = $this->coreFunctions->opentable($qry);
            $resultdata =  json_decode(json_encode($opendata), true);
            if (!empty($resultdata[0]['category'])) {
        ...
        if ($row['line'] == 0) {
            ...
            if ($row['category'] != '') {
                $qry = "select line,category,reqtype,case when inactive=0 then 'false' else 'true' end as inactive from  reqcategory where category = '" . $row['category'] . "' and line <> ".$row['line']. " and isttype=1 limit 1";
                $opendata = $this->coreFunctions->opentable($qry);
                $resultdata =  json_decode(json_encode($opendata), true);
                if (!empty($resultdata)) {
    ...
    private function loaddataperrecord($line)
    {
        ...
        $qry = "select " . $select . " from " . $this->table . " as r  where  r.isttype = 1  and line=?";
    ...
    public function loaddata($config)
    {
        ...
        $qry = "select " . $select . " from " . $this->table . " as r  where r.isttype = 1  and 1=1 " . $filtersearch . " order by line $l";
    ...
    private function rpt_default_header($data, $filters)
    {
        ...
        $str .= $this->reporter->begintable('800');
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col('TRANSACTION TYPE MASTERFILE', '800', null, false, '1px solid ', '', 'L', 'Century Gothic', '18', 'B', '', '');
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();

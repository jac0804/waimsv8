...
class entryloantype
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'LOAN TYPE MASTERFILE';
  ...
  private $fields = ['category', 'reqtype', 'acnoid', 'isloantype','isdiminishing'];
  ...
  {
    $columns = ['action', 'category', 'reqtype', 'acno','isdiminishing'];
    $tab = [
      $this->gridname => [
        'gridcolumns' => $columns
      ]
    ...
    $obj[0][$this->gridname]['columns'][0]['btns']['addsub']['action'] = 'tableentry';
    $obj[0][$this->gridname]['columns'][0]['btns']['addsub']['lookupclass'] = 'loanpurpose';
    // action
    $obj[0][$this->gridname]['columns'][0]['style'] = 'width:50px;whiteSpace: normal;min-width:50px;';
    $obj[0][$this->gridname]['columns'][1]['style'] = 'width:125px;whiteSpace: normal;min-width:125px;';
    $obj[0][$this->gridname]['columns'][2]['style'] = 'width:175px;whiteSpace: normal;min-width:175px;';

    $obj[0][$this->gridname]['columns'][1]['label'] = 'Code';
    $obj[0][$this->gridname]['columns'][2]['label'] = 'Name';

    $obj[0][$this->gridname]['columns'][3]['label'] = "Receivable Account";
    $obj[0][$this->gridname]['columns'][3]['type'] = "lookup";
    $obj[0][$this->gridname]['columns'][3]['lookupclass'] = "lookupcoa";
        ...
        if ($data[$key]['line'] == 0 && $data[$key]['category'] != '') {
          ...
          if (!empty($resultdata[0]['category'])) {
            if (trim($resultdata[0]['category']) == trim($data[$key]['category'])) {
              return ['status' => false, 'msg' => ' Code ( ' . $resultdata[0]['category'] . ' )' . '  already exist', 'data' => [$resultdata]];
            }
          }
        }
          ...
          if ($data[$key]['line'] != 0 && $data[$key]['category'] != '') {
            ...
            if (!empty($resultdata[0]['category'])) {
              if (trim($resultdata[0]['category']) == trim($data[$key]['category'])) {
                ...
                return ['status' => false, 'msg' => ' Code ( ' . $resultdata[0]['category'] . ' )' . ' is already exist', 'data' => [$resultdata], 'rowid' => [$data[$key]['line']  . ' -- ' . $resultdata[0]['line']]];
              } else {
                update:
  ...
  public function save($config)
  {
    ...
    if ($row['line'] == 0 && $row['category'] != '') {
      $qry = "select category from reqcategory where category = '" . $row['category'] . "' limit 1";
      $opendata = $this->coreFunctions->opentable($qry);
      $resultdata =  json_decode(json_encode($opendata), true);
      if (!empty($resultdata[0]['category'])) {
        if (trim($resultdata[0]['category']) == trim($row['category'])) {
          return ['status' => false, 'msg' => ' Code ( ' . $resultdata[0]['category'] . ' )' . ' is already exist', 'data' => [$resultdata]];
        }
      }
    }
    ...
    if ($row['line'] == 0) {
      ...
      if ($row['category'] != 0 || $row['category'] != '') {
        $qry = "select line,category,reqtype from  reqcategory where category = '" . $row['category'] . "' limit 1";
        $opendata = $this->coreFunctions->opentable($qry);
        $resultdata =  json_decode(json_encode($opendata), true);
        if (!empty($resultdata[0]['category'])) {
          if (trim($resultdata[0]['category']) == trim($row['category'])) {
            ...
            return ['status' => false, 'msg' => ' Code ( ' . $resultdata[0]['category'] . ' )' . ' is already exist', 'data' => [$resultdata], 'rowid' => [$row['line']  . ' -- ' . $resultdata[0]['line']]];
          } else {
            update:
  ...
  private function loaddataperrecord($line)
  {
    ...
    $qry = "select " . $select . ",coa.acno from " . $this->table . " as r left join coa on coa.acnoid = r.acnoid where r.isloantype =1 and r.line=?";
  ...
  public function loaddata($config)
  {
    ...
    $qry = "select " . $select . ",coa.acno,case when r.isdiminishing =1 then 'true' else 'false' end as isdiminishing from " . $this->table . " as r left join coa on coa.acnoid = r.acnoid where r.isloantype =1  " . $filtersearch . " order by line $l";
  ...
  public function lookupcoa($config)
  {
    ...
    $qry = "select acnoid,acno,acnoname from coa where left(alias,2)='AR' order by acno";
    $data = $this->coreFunctions->opentable($qry);
  ...
  private function rpt_default_header($data, $filters)
  {
    ...
    $str .= $this->reporter->begintable('800');
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('LOAN TYPE MASTERFILE', '800', null, false, '1px solid ', '', 'L', 'Century Gothic', '18', 'B', '', '');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();

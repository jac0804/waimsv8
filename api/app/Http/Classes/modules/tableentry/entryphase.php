...
class entryphase
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'PHASE MASTERFILE';
  ...
  public function createTab($config)
  {
    ...
    $tab = [$this->gridname => ['gridcolumns' => ['action', 'code']]];
    $stockbuttons = ['save', 'delete', 'addblklot'];

    $obj = $this->tabClass->createtab($tab, $stockbuttons);
    // action
    $obj[0][$this->gridname]['columns'][$action]['style'] = "width:100px;whiteSpace: normal;min-width:100px;";
    $obj[0][$this->gridname]['columns'][$action]['btns']['addblklot']['checkfield'] = 'newtrans';
  ...
  {
    return "head.line, head.code, head.projectid";
  }

  public function saveallentry($config)
  {
    $data = $config['params']['data'];
    $config['params']['doc'] = 'ENTRYPHASE';
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['line'] == 0) {
          ...
          $this->logger->sbcmasterlog($project_id, $config, ' CREATE - ' . $data[$key]['code'] . ' - ' . $data[$key]['projectid']);
        } else {
  ...
  public function save($config)
  {
    ...
    if ($row['line'] == 0) {
      ...
      if ($line != 0) {
        ...
        $this->logger->sbcmasterlog($line, $config, ' CREATE - ' . $data['code'] . ' - ' . $data['projectid']);
  ...
  public function delete($config)
  {
    ...
    $this->coreFunctions->execqry("delete from blklot where phaseid=" . $row['line'] . " and projectid=" . $row['projectid'], 'delete');
    $this->logger->sbcdelmaster_log($row['line'], $config, 'REMOVE - ' . $row['code'] . ' - ' . $row['projectid']);
  ...
  public function loaddata($config)
  {
    ...
    $qry = "select " . $select . " 
    from " . $this->table . " as head
    where head.projectid=" . $projectid . " " . $filtersearch . "

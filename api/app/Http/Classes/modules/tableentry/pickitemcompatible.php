...
class pickitemcompatible
{
  ...
  public function createTab($config)
  {
    $action = 0;
    $itemname = 1;
    $tab = [$this->gridname => ['gridcolumns' => ['action', 'itemname']]];
  ...
  {
    $tbuttons = ['pickitemcompatible']; //'pickitemcompatible'
  ...
  {
    $select = "i.line, i.itemid, c.line as cmodelid,
      c.brand, c.model, c.classification, item.itemname";
    $select = $select . ",'' as bgcolor ";
    $qry = "select " . $select . "
    from itemcmodels as i
    left join cmodels as c on i.cmodelid = c.line
    left join item as item on item.itemid = i.itemid
  ...
  public function loaddata($config)
  {
    ...
    left join cmodels as c on i.cmodelid = c.line
    left join item as item on item.itemid = i.itemid
    where c.line=? order by i.line";
    $data = $this->coreFunctions->opentable($qry, [$row['line']]);
    return $data;
  }

  public function lookupsetup($config)
  {
    return $this->lookupclass->pickitemcompatible($config);
  }

...
class entrycategories
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'SUPP/CUST. CATEGORIES';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'category_masterfile';
  private $othersClass;
  public $style = 'width:100%;';
  public $tablelogs = 'masterfile_log';
  public $tablelogs_del = 'del_masterfile_log';
  private $fields = ['cat_name'];
  ...
  public function createTab($config)
  {
    ...
    if ($companyid == 22) { //eipi
      $this->modulename = 'BUSINESS STYLE';
    }
    $tab = [
      $this->gridname => [
        'gridcolumns' => ['action', 'cat_name']
      ]
    ];
    ...
    switch ($companyid) {
      ...
      case 52: //technolab
        $obj[0][$this->gridname]['columns'][1]['label'] = 'Business Style';
        break;
    }
  ...
  public function saveallentry($config)
  {
    ...
    $dateTables = ['category_masterfile'];
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['cat_id'] == 0 && $data[$key]['cat_name'] != '') {
          $qry = "select cat_name from category_masterfile where cat_name = '" . $data[$key]['cat_name'] . "' limit 1";
          $opendata = $this->coreFunctions->opentable($qry);
          $resultdata =  json_decode(json_encode($opendata), true);
          if (!empty($resultdata[0]['cat_name'])) {
            if (trim($resultdata[0]['cat_name']) == trim($data[$key]['cat_name'])) {
              return ['status' => false, 'msg' => ' Category Name ( ' . $resultdata[0]['cat_name'] . ' )' . ' is already exist', 'data' => [$resultdata]];
            }
          }
        }
        if (trim($data[$key]['cat_name'] == '')) {
          return ['status' => false, 'msg' => 'Category Name is empty'];
        }
        if ($data[$key]['cat_id'] == 0) {
          $cat_id  = $this->coreFunctions->insertGetId($this->table, $data2);
          $this->logger->sbcmasterlog($cat_id, $config, ' CREATE - ' . $data[$key]['cat_name']);
        } else {

          if ($data[$key]['cat_id'] != 0 && $data[$key]['cat_name'] != '') {
            $qry = "select cat_name,cat_id from category_masterfile where cat_name = '" . $data[$key]['cat_name'] . "' limit 1";
            $opendata = $this->coreFunctions->opentable($qry);
            $resultdata =  json_decode(json_encode($opendata), true);
            if (!empty($resultdata[0]['cat_name'])) {
              if (trim($resultdata[0]['cat_name']) == trim($data[$key]['cat_name'])) {
                if ($data[$key]['cat_id'] == $resultdata[0]['cat_id']) {
                  goto update;
                }
                return ['status' => false, 'msg' => ' Category Name ( ' . $resultdata[0]['cat_name'] . ' )' . ' is already exist', 'data' => [$resultdata], 'rowid' => [$data[$key]['cat_id']  . ' -- ' . $resultdata[0]['cat_id']]];
              } else {
                update:
                ...
                $this->coreFunctions->sbcupdate($this->table, $data2, ['cat_id' => $data[$key]['cat_id']]);
                $this->logger->sbcmasterlog($data[$key]['cat_id'], $config, ' UPDATE - ' . $data[$key]['cat_name']);
              }
            } else {
              goto update;
            }
  ...
  public function save($config)
  {
    ...
    if ($row['cat_id'] == 0 && $row['cat_name'] != '') {
      $qry = "select cat_name from category_masterfile where cat_name = '" . $row['cat_name'] . "' limit 1";
      $opendata = $this->coreFunctions->opentable($qry);
      $resultdata =  json_decode(json_encode($opendata), true);
      if (!empty($resultdata[0]['cat_name'])) {
        if (trim($resultdata[0]['cat_name']) == trim($row['cat_name'])) {
          return ['status' => false, 'msg' => ' Category Name ( ' . $resultdata[0]['cat_name'] . ' )' . ' is already exist', 'data' => [$resultdata]];
        }
      }
    }
    ...
    if ($row['cat_id'] == 0) {
      ...
      if ($cat_id != 0) {
        ...
        $this->logger->sbcmasterlog($cat_id, $config, ' CREATE - ' . $data['cat_name']);
      ...
      if ($row['cat_id'] != 0 && $row['cat_name'] != '') {
        $qry = "select cat_name,cat_id from category_masterfile where cat_name = '" . $row['cat_name'] . "' limit 1";
        $opendata = $this->coreFunctions->opentable($qry);
        $resultdata =  json_decode(json_encode($opendata), true);
        if (!empty($resultdata[0]['cat_name'])) {
          if (trim($resultdata[0]['cat_name']) == trim($row['cat_name'])) {
            if ($row['cat_id'] == $resultdata[0]['cat_id']) {
              goto update;
            }
            return ['status' => false, 'msg' => ' Class Name ( ' . $resultdata[0]['cat_name'] . ' )' . ' is already exist', 'data' => [$resultdata], 'rowid' => [$row['cat_id']  . ' -- ' . $resultdata[0]['cat_id']]];
          } else {
            update:
            ...
            if ($this->coreFunctions->sbcupdate($this->table, $data, ['cat_id' => $row['cat_id']]) == 1) {
              $returnrow = $this->loaddataperrecord($row['cat_id']);
              $this->logger->sbcmasterlog($row['cat_id'], $config, ' UPDATE - ' . $data['cat_name']);
              return ['status' => true, 'msg' => 'Successfully saved.', 'row' => $returnrow];
            } else {
              return ['status' => false, 'msg' => 'Saving failed.'];
            }
  ...
  public function delete($config)
  {
    ...
    $qry = "select clientname from client where category=?";
    $count = $this->coreFunctions->opentable($qry, [$row['cat_id']]);

    if ($count) {
      return ['clientid' => $row['cat_id'], 'status' => false, 'msg' => 'Unable to delete. Already used by ' . $count[0]->clientname];
    }

    $qry = "select itemid from pricebracket where groupid=?";
    $count = $this->coreFunctions->opentable($qry, [$row['cat_id']]);
    if ($count) {
      return ['clientid' => $row['cat_id'], 'status' => false, 'msg' => 'Unable to delete. Already used by price list'];
    }

    $qry = "delete from " . $this->table . " where cat_id=?";
    $this->coreFunctions->execqry($qry, 'delete', [$row['cat_id']]);
    $this->logger->sbcdelmaster_log($row['cat_id'], $config, 'REMOVE - ' . $row['cat_name']);
  ...
  private function loaddataperrecord($cat_id)
  {
    ...
    $qry = "select " . $select . " from " . $this->table . " where cat_id=?";
  ...
  public function loaddata($config)
  {
    ...
    $select = $this->selectqry();
    $select = $select . ",'' as bgcolor ";
    $qry = "select " . $select . " from " . $this->table . " where 1=1 " . $filtersearch . " order by cat_id limit " . $limit;
  ...
  public function lookuplogs($config)
  {
    ...
    $lookupsetup = array(
      'type' => 'show',
      'title' => 'Supplier/Customer Categories Master Logs',
  ...
  private function report_default_query($config)
  {
    ...
    $query = "select cat_id, cat_code, cat_name from category_masterfile
      order by cat_id";
  ...
  private function rpt_class_PDF_header_PDF($data, $filters)
  {
    ...
    PDF::MultiCell(800, 20, "SUPPLIER/CUSTOMER CATEGORIES MASTERFILE", '', 'L', false);
  ...
  private function rpt_class_PDF($data, $filters)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      PDF::MultiCell(600, 10, $data[$i]['cat_name'], '', 'L', 0, 0, '', '', true, 0, true, false);

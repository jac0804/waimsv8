...
class entrysbu
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'SBU MASTERFILE';
  ...
  private $fields = ['category', 'reqtype', 'issbu'];
  ...
  public function createTab($config)
  {
    $tab = [
      $this->gridname => [
        'gridcolumns' => ['action', 'category', 'reqtype']
      ]
    ];
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['line'] == 0) {
          ...
          if ($data[$key]['line'] != 0 && $data[$key]['category'] != '') {
            if (!empty($resultdata[0]['category'])) {
              if (trim($resultdata[0]['category']) == trim($data[$key]['category'])) {
                $resultdata[0]['category'] == $data[$key]['category'];
  ...
  private function loaddataperrecord($line)
  {
    ...
    $qry = "select " . $select . " from " . $this->table . " as r where line=? and issbu = 1";
  ...
  public function loaddata($config)
  {
    ...
    $qry = "select " . $select . " from " . $this->table . " as r where 1=1 and issbu = 1 " . $filtersearch . " order by line $l";
  ...
  private function rpt_class_PDF_header_PDF($data, $filters)
  {
    ...
    PDF::MultiCell(100, 20, "Code", '', 'L', false, 0);
    PDF::MultiCell(500, 20, "Name", '', 'L', false, 0);
  ...
  private function rpt_loantype_PDF($data, $filters)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      PDF::MultiCell(100, 10, $data[$i]['category'], '', 'L', 0, 0, '', '', true, 0, true, false);
      PDF::MultiCell(500, 10, $data[$i]['reqtype'], '', 'L', 0, 0, '', '', true, 0, true, false);

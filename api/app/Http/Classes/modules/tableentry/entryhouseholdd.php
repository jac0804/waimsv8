...
class entryhouseholdd
{
    private $fieldClass;
    private $tabClass;
    public $modulename = 'HOUSEHOLD MEMBER';
    ...
    private $fields = ['ownertype', 'ownername', 'addr2', 'empid', 'ishouseholdmm'];
                    ...
                    if ($data[$key]['line'] != 0 && $data[$key]['ownername'] != '') {
                        ...
                        if (!empty($resultdata[0]['ownername'])) {
                            if (trim($resultdata[0]['ownername']) == trim($data[$key]['ownername'])) {
                                ...
                                return ['status' => false, 'msg' => ' Category Name ( ' . $resultdata[0]['ownername'] . ' )' . ' is already exist', 'data' => [$resultdata], 'rowid' => [$data[$key]['line']  . ' -- ' . $resultdata[0]['line']]];
                            } else {
                                update:
    ...
    private function loaddataperrecord($line)
    {
        ...
        $qry = "select " . $select . " from " . $this->table . " where ishouseholdmm =1 and line=?";
    ...
    public function loaddata($config)
    {
        ...
        $select = $this->selectqry();
        $select = $select . ",'' as bgcolor ";
        $qry = "select " . $select . " from " . $this->table . " where ishouseholdmm =1 " . $filtersearch . " order by line limit " . $limit;
    ...
    public function lookuplogs($config)
    {
        ...
        $qry = "
            ...
            where log.doc = '" . $doc . "' and  cn.ishouseholdmm=1
            union all
            select trno, doc, task, log.user, dateid, 
            if(pic='','blank_user.png',pic) as pic

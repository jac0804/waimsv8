...
class entrymodeoftransaction
{
    private $fieldClass;
    private $tabClass;
    public $modulename = 'MODE OF TRANSACTION';
    public $gridname = 'inventory';
    private $companysetup;
    private $coreFunctions;
    private $table = 'mode_masterfile';
    private $othersClass;
    public $style = 'width:100%;';
    public $tablelogs = 'masterfile_log';
    public $tablelogs_del = 'del_masterfile_log';
    private $fields = ['name', 'issp', 'ismc', 'isactive', 'clientid'];
    ...
    public function createTab($config)
    {
        $tab = [
            $this->gridname => [
                'gridcolumns' => ['action', 'name', 'issp', 'ismc', 'client', 'isactive']
            ]
        ];
        ...
        $obj[0][$this->gridname]['columns'][5]['label'] = "Isinactive";
        $obj[0][$this->gridname]['columns'][4]['label'] = "Financing Partner";
        $obj[0][$this->gridname]['columns'][4]['lookupclass'] = "lookupclient";
    ...
    {
        $qry = "m.name,m.line,
        (case when m.issp = 0 then 'false' else 'true' end) as issp,
        (case when m.isactive = 1 then 'true' else 'false' end) as isactive,
        (case when m.ismc = 0 then 'false' else 'true' end) as ismc,m.clientid,f.client";
        return $qry;
    }
        ...
        foreach ($data as $key => $value) {
            ...
            if ($data[$key]['bgcolor'] != '') {
                ...
                if ($data[$key]['line'] == 0 && $data[$key]['name'] != '') {
                    ...
                    if (!empty($resultdata[0]['name'])) {
                        if (trim($resultdata[0]['name']) == trim($data[$key]['name'])) {
                            return ['status' => false, 'msg' => ' Mode Name ( ' . $resultdata[0]['name'] . ' )' . ' is already exist', 'data' => [$resultdata]];
                        }
                    }
                }
                ...
                if ($data[$key]['line'] == 0) {
                    ...
                    if ($data[$key]['line'] != 0 && $data[$key]['name'] != '') {
                        $qry = "select name,line from mode_masterfile where name = '" . $data[$key]['name'] . "' limit 1";
                        $opendata = $this->coreFunctions->opentable($qry);
                        $resultdata =  json_decode(json_encode($opendata), true);
                        if (!empty($resultdata[0]['name'])) {
                            if (trim($resultdata[0]['name']) == trim($data[$key]['name'])) {
                                ...
                                return ['status' => false, 'msg' => 'Mode Name ( ' . $resultdata[0]['name'] . ' )' . ' is already exist', 'data' => [$resultdata], 'rowid' => [$data[$key]['line']  . ' -- ' . $resultdata[0]['line']]];
                            } else {
                                update:
    ...
    public function save($config)
    {
        ...
        if ($row['line'] == 0) {
            ...
            if ($row['line'] != 0 && $row['name'] != '') {
                $qry = "select name,line from  mode_masterfile where name = '" . $row['name'] . "' limit 1";
                $opendata = $this->coreFunctions->opentable($qry);
                $resultdata =  json_decode(json_encode($opendata), true);
                if (!empty($resultdata[0]['name'])) {
                    if (trim($resultdata[0]['name']) == trim($row['name'])) {
                        ...
                        return ['status' => false, 'msg' => 'Mode Name ( ' . $resultdata[0]['name'] . ' )' . 'is already exist', 'data' => [$resultdata], 'rowid' => [$row['line']  . ' -- ' . $resultdata[0]['line']]];
                    } else {
                        update:
                        $data2['editdate'] = $this->othersClass->getCurrentTimeStamp();
                        $data2['editby'] = $config['params']['user'];
    ...
    {
        $row = $config['params']['row'];
        $qry = "select name as value from item where name=?";
    ...
    private function loaddataperrecord($line)
    {
        ...
        $qry = "select " . $select . " from " . $this->table . " as m left join client as f on f.clientid = m.clientid where line=?";
    ...
    public function loaddata($config)
    {
        ...
        $select = $this->selectqry();
        $select = $select . ",'' as bgcolor ";
        $qry = "select " . $select . " from " . $this->table . " as m left join client as f on f.clientid = m.clientid where 1=1 " . $filtersearch . " order by name " . $l;
    ...
    {
        $title = 'List of Financing Partners';
        ...
        $qry = "select client.clientid,client.client,client.clientname 
                from client    
                where isfp=1 and isinactive=0 order by client";
        $data = $this->coreFunctions->opentable($qry);
    ...
    private function report_default_query($config)
    {
        ...
        $query = "select name from mode_masterfile
      order by line";
    ...
    private function rpt_mode_PDF_header_PDF($data, $filters)
    {
        ...
        PDF::MultiCell(600, 20, "Mode Name", '', 'L', false, 0);

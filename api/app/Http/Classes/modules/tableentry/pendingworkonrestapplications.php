...
class pendingworkonrestapplications
{
    private $fieldClass;
    private $tabClass;
    private $logger;
    public $modulename = 'PENDING WORK ON REST DAY APPLICATIONS';
    ...
    public function createTab($config)
    {
        ...
        $approversetup = $this->coreFunctions->datareader("select approverseq as value from moduleapproval where modulename='WORKONRESTDAY'");
        if ($approversetup == '') {
        ...
        // $cols = ['action', 'clientname', 'dateid', 'type', 'rem', 'rem1', 'rem2'];
        ...
        $obj[0][$this->gridname]['columns'][$daytype]['style'] = 'width:150px;min-width:150px;';
        $obj[0][$this->gridname]['columns'][$daytype]['type'] = 'coldel';
        $obj[0][$this->gridname]['columns'][$daytype]['label'] = 'Type';
    ...
    {
        $qry = "select cs.line, client.clientname, date(cs.dateid) as dateid,cs.empid,date(cs.createdate) as createdate,
            cs.orgdaytype as daytype,cs.approvedby2, '' as rem2,cs.disapproved_remarks2 as rem1,
            ...
            left join pendingapp as p on p.line=cs.line and p.doc='WORKONRESTDAY'
            left join moduleapproval as m on m.modulename=p.doc
            where cs.status = 0 and isword = 1 and p.clientid=" . $config['params']['adminid'] . " order by cs.dateid, client.clientname";
    ...
    public function updateapp($config, $status)
    {
        ...
        $isapp = $row['approver'];
        if ($isapp == '' || $isapp == null) $isapp = $this->coreFunctions->datareader("select approver as value from pendingapp where doc='WORKONRESTDAY' and line=" . $row['line']);
                ...
                if (count($approversetup) > 1) {
                    ...
                    if (count($approversetup) == 1) {
                        ...
                        if ($cstatus == 1) {
                            ...
                            if ($row['rem2'] == '') return ['status' => false, 'msg' => 'Last Approver Remarks is empty.', 'data' => []];
        ...
        $pendingdata = $this->coreFunctions->opentable("select * from pendingapp where line=" . $line . " and doc='WORKONRESTDAY'");
        $this->coreFunctions->execqry("delete from pendingapp where doc='WORKONRESTDAY' and line=" . $line, 'delete');
        $appstatus = ['status' => true];
        $tempdata['empid'] = $row['empid'];
        if (!$lastapp && $cstatus == 1) $appstatus = $this->othersClass->insertUpdatePendingapp(0, $line, 'WORKONRESTDAY', $tempdata, $url, $config, 0, true);
        if (!$appstatus['status']) {

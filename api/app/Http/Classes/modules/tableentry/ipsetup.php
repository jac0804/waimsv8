...
class ipsetup
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'IP SETUP';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'ipsetup';
  private $othersClass;
  public $style = 'width:100%;';
  public $tablelogs = 'masterfile_log';
  public $tablelogs_del = 'del_masterfile_log';
  private $fields = ['line', 'ipaddress'];
  ...
  public function saveallentry($config)
  {
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if (!empty($checking)) {
          return ['status' => false, 'msg' => 'IP Address Already Exists. - ' . $data2['ipaddress'], 'data' => $data];
        }
        ...
        if ($data[$key]['line'] == 0) {
          ...
          $this->logger->sbcmasterlog($data[$key]['line'], $config, ' CREATE - ' . $data[$key]['ipaddress']);
        }
      } // end if
    } // foreach
  ...
  public function save($config)
  {
    ...
    $checking = $this->coreFunctions->datareader($qry);
    if (!empty($checking)) {
      return ['status' => false, 'msg' => 'IP Address Already Exists. - ' . $data['ipaddress'], 'data' => $data];
    }
    ...
    if ($row['line'] == 0) {
      ...
      if ($line != 0) {
        ...
        $this->logger->sbcmasterlog($row['line'], $config, ' CREATE - ' . $data['ipaddress']);
  ...
  public function lookuplogs($config)
  {
    ...
    $cols = array(
      ...
      array('ipaddress' => 'doc', 'label' => 'Doc', 'align' => 'left', 'field' => 'doc', 'sortable' => true, 'style' => 'font-size:16px;'),
  ...
  public function delete($config)
  {
    ...
    $this->logger->sbcdelmaster_log($row['line'], $config, 'REMOVE - ' . $row['ipaddress']);

...
class entryaccaddons
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'ACCESSORIES ADDONS';
  ...
  public function createTab($config)
  {
    ...
    $gridcolumns = ['action', 'qty', 'addons'];
    $stockbuttons = ['save', 'delete'];
    if ($isposted || $islocked) {
      $gridcolumns = ['qty', 'addons'];
      $stockbuttons = [];
    }
    ...
    if ($isposted || $islocked) {
      ...
      $obj[0][$this->gridname]['columns'][1]['label'] = 'Qty';
      $obj[0][$this->gridname]['columns'][1]['type'] = 'editlookup';
      $obj[0][$this->gridname]['columns'][1]['action'] = 'lookupsetup';
      $obj[0][$this->gridname]['columns'][1]['lookupclass'] = 'addonqty';
    }
    return $obj;
  }
  ...
  {
    $qry = "select line, trno, addons, qty, '' as bgcolor from qtaddons where trno=? and line=? and addontype=2 union all select line, trno, addons, qty, '' as bgcolor from hqtaddons where trno=? and line=? and addontype=2";
  ...
  {
    $tableid = $config['params']['tableid'];
    $qry = "select line, trno, addons, qty, '' as bgcolor from qtaddons where trno=? and addontype=2 union all select line, trno, addons, qty, '' as bgcolor from hqtaddons where trno=? and addontype=2 order by line";
  ...
  public function lookupsetup($config)
  {
    ...
    switch ($lookupclass2) {
      case 'addons':
        ...
        $qry = "select distinct addons from qtaddons where addontype=2 order by addons";
      ...
      case 'addonqty':
        ...
        $data = $this->coreFunctions->opentable("select distinct qty from qtaddons where addontype=2 order by qty");
        return ['status' => true, 'msg' => 'ok', 'data' => $data, 'lookupsetup' => $lookupsetup, 'cols' => $cols, 'plotsetup' => $plotsetup, 'index' => $rowindex];
        break;
      default:

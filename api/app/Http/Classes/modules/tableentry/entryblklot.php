...
class entryblklot
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'BLK/LOT MASTERFILE';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'blklot';
  private $othersClass;
  public $style = 'width:100%;';
  public $tablelogs = 'masterfile_log';
  public $tablelogs_del = 'del_masterfile_log';
  private $fields = ['phaseid', 'projectid', 'blk', 'lot', 'sqm', 'isinactive'];
  ...
  public function createTab($config)
  {
    ...
    $tab = [$this->gridname => ['gridcolumns' => ['action', 'blk', 'lot', 'sqm', 'isinactive']]];
    $stockbuttons = ['save', 'delete'];

    $obj = $this->tabClass->createtab($tab, $stockbuttons);
    $obj[0][$this->gridname]['columns'][$action]['style'] = "width:100px;whiteSpace: normal;min-width:100px;";
    $obj[0][$this->gridname]['columns'][$blk]['style'] = "width:200px;whiteSpace: normal;min-width:200px;";
    $obj[0][$this->gridname]['columns'][$lot]['style'] = "width:200px;whiteSpace: normal;min-width:200px;";
    $obj[0][$this->gridname]['columns'][$sqm]['style'] = "width:200px;whiteSpace: normal;min-width:200px;";

    $obj[0][$this->gridname]['columns'] = $this->tabClass->delcol($obj, $this->gridname);

    $this->modulename .= " - [ PROJECT : " . $this->getproject($config) . " ] [ PHASE : " . $this->getphase($config) . " ] ";
  ...
  public function add($config)
  {
    $data = [];
    $data['line'] = 0;
    $data['phaseid'] = $config['params']['sourcerow']['line'];
  ...
  {
    return "line, phaseid, projectid, modelid, blk, lot, sqm, case when isinactive=0 then 'false' else 'true' end as isinactive, clientid";
  }

  public function saveallentry($config)
  {
    $data = $config['params']['data'];
    $config['params']['doc'] = 'ENTRYBLKLOT';
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['line'] == 0) {
          ...
          $this->logger->sbcmasterlog($id, $config, ' CREATE - ' . $data[$key]['blk'] . ' - ' . $data[$key]['lot'] . ' - ' . $data[$key]['sqm'] . ' - ' . $data[$key]['projectid'] . ' - ' . $data[$key]['phaseid']);
        } else {
  ...
  public function save($config)
  {
    ...
    if ($row['line'] == 0) {
      ...
      if ($line != 0) {
        ...
        $this->logger->sbcmasterlog($line, $config, ' CREATE - ' . $data['blk'] . ' - ' . $data['lot'] . ' - ' . $data['lot'] . ' - ' . $data['projectid'] . ' - ' . $data['phaseid']);
  ...
  public function delete($config)
  {
    ...
    $this->logger->sbcdelmaster_log($row['line'], $config, 'REMOVE - ' . $row['blk'] . ' - ' . $row['lot'] . ' - ' . $row['sqm'] . ' - ' . $row['projectid'] . ' - ' . $row['phaseid']);
  ...
  {
    $filtersearch = "";
    $searcfield = ['blk', 'lot', 'sqm'];
    $limit = "1000";

    if (isset($config['params']['row']['line'])) {
      $modelid = $config['params']['row']['line'];
    } else {
      $modelid = $config['params']['sourcerow']['line'];
    }
    ...
    $qry = "select " . $select . " 
    from " . $this->table . "
    where phaseid=" . $modelid . " " . $filtersearch . "
  ...
  {
    if (isset($config['params']['row']['projectid'])) {
      $projectid = $config['params']['row']['projectid'];
    } else {
      $projectid = $config['params']['sourcerow']['projectid'];
    }

    $qry = "select name as value from projectmasterfile where line = ?";
    return $this->coreFunctions->datareader($qry, [$projectid]);
  }

  private function getphase($config)
  {
    if (isset($config['params']['row']['line'])) {
      $phaseid = $config['params']['row']['line'];
    } else {
      $phaseid = $config['params']['sourcerow']['line'];
    }

    $qry = "select code as value from phase where line = ?";
    return $this->coreFunctions->datareader($qry, [$phaseid]);
  }
  ...
  private function getmodel($config)
  {
    ...
    $qry = "select model as value from housemodel where line = ?";
    return $this->coreFunctions->datareader($qry, [$modelid]);
  }
  ...
  private function report_default_query($config)
  {
    ...
    $query = "select line, code, name from phase
    order by line";

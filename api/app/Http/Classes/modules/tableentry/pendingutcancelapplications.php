...
class pendingutcancelapplications
{
    private $fieldClass;
    private $tabClass;
    private $logger;
    public $modulename = 'PENDING UNDERTIME CANCELLATIONS';
    ...
    public function createTab($config)
    {
        ...
        $cols = ['action', 'clientname', 'dateid', 'rem1'];
        ...
        $obj[0][$this->gridname]['columns'][3]['label'] = 'Reason';
        $obj[0][$this->gridname]['columns'][3]['readonly'] = false;
        $obj[0][$this->gridname]['columns'][3]['style'] = 'width:250px;min-width:250px;';
        return $obj;
    }
    ...
    {
        $qry = "select ut.line as trno,ut.line,concat(emp.emplast,', ',emp.empfirst,' ',emp.empmiddle) as clientname,
            dept.clientname as deptname,ut.forapproval,date(ut.dateid) as dateid,jt.jobtitle,ut.rem,
            m.modulename as doc, m.sbcpendingapp, ut.reason as rem1, emp.empid
            ...
            left join pendingapp as p on p.line=ut.line and p.doc='UNDERTIMECANCELLATION'
            left join moduleapproval as m on m.modulename=p.doc
            where ut.status = 'E' and ut.forapproval is not null and canceldate is null
    ...
    public function updateapp($config, $status)
    {
        ...
        $update = $this->coreFunctions->sbcupdate("undertime", $tempdata, ['line' => $line, 'empid' => $row['empid']]);
        if ($update) {
            $this->coreFunctions->execqry("delete from pendingapp where doc='UNDERTIMECANCELLATION' and line=".$line, 'delete');
            $config['params']['doc'] = 'UNDERTIMECANCELLATION';
            $this->logger->sbcmasterlog($line.' '.$row['trno'], $config, $label.' ('.$row['clientname'].') - '.$row['dateid']);

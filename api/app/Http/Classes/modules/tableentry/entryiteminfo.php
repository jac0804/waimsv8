...
class entryiteminfo
{
  ...
  private $fields = ['itemid', 'itemdescription', 'accessories'];
  ...
  public function createTab($config)
  {
    ...
    $tab = [$this->gridname => ['gridcolumns' => ['action', 'itemdescription', 'accessories']]];
    ...
    $obj[0][$this->gridname]['columns'][$itemdescription]['style'] = "width:400px;whiteSpace: normal;min-width:100%;";
    $obj[0][$this->gridname]['columns'][$accessories]['style'] = "width:400px;whiteSpace: normal;min-width:100%;";
  ...
  {
    $qry = "line, itemid, itemdescription, accessories
    ";
    return $qry;
  }
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['line'] == 0) {
          ...
          $this->logger->sbcmasterlog($line, $config, ' CREATE - ' . $data[$key]['itemdescription'] . '~' . $data[$key]['accessories']);
        } else {
  ...
  public function delete($config)
  {
    ...
    $this->logger->sbcdelmaster_log($row['line'], $config, 'REMOVE - ' . $row['itemdescription'] . '~' . $row['accessories']);
  ...
  private function loaddataperrecord($line, $config = [])
  {
    ...
    where line = ? and itemid = " . $config['params']['tableid'] . "
    order by line";
  ...
  public function loaddata($config)
  {
    ...
    $select = $select . ", '' as bgcolor ";
    $qry = "select " . $select . " from " . $this->table . "
    where itemid = " . $config['params']['tableid'] . "
    order by line";

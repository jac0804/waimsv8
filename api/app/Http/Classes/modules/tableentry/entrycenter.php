...
class entrycenter
{
  ...
  public function saveallentry($config)
  {
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($config['params']['companyid'] == 40) { //cdo
          $data2["accountno"] = $this->othersClass->sanitizekeyfieldFast("acno", $data[$key]['accountno'], $lookups);
        }
        if ($data[$key]['line'] == 0) {
  ...
  public function save($config)
  {
    ...
    if ($config['params']['companyid'] == 40) { //cdo
      $data["accountno"] = $this->othersClass->sanitizekeyfieldFast("acno", $row['accountno'], $lookups);
      //$data2["accountno"] ....("acno", $data[$key]['accountno']);
    }
    if ($row['line'] == 0) {

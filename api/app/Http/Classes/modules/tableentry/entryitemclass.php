...
class entryitemclass
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'CLASS MASTERFILE';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'item_class';
  private $othersClass;
  public $style = 'width:100%;';
  public $tablelogs = 'masterfile_log';
  public $tablelogs_del = 'del_masterfile_log';
  private $fields = ['cl_name'];
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['cl_id'] == 0 && $data[$key]['cl_name'] != '') {
          ...
          if (!empty($resultdata[0]['cl_name'])) {
            if (trim($resultdata[0]['cl_name']) == trim($data[$key]['cl_name'])) {
              return ['status' => false, 'msg' => ' Class Name ( ' . $resultdata[0]['cl_name'] . ' )' . ' is already exist', 'data' => [$resultdata]];
            }
          }
        }
        if (trim($data[$key]['cl_name'] == '')) {
          return ['status' => false, 'msg' => 'Class Name is empty'];
        }
        if ($companyid == 37)  $data2['cl_name'] = strtoupper($data2['cl_name']); //megacrystal

        if ($data[$key]['cl_id'] == 0) {
          $classid = $this->coreFunctions->insertGetId($this->table, $data2);
          $this->logger->sbcmasterlog($classid, $config, ' CREATE - ' . $data[$key]['cl_name']);
        } else {
          if ($data[$key]['cl_id'] != 0 && $data[$key]['cl_name'] != '') {
            $qry = "select cl_name,cl_id from item_class where cl_name = '" . $data[$key]['cl_name'] . "' limit 1";
            $opendata = $this->coreFunctions->opentable($qry);
            $resultdata =  json_decode(json_encode($opendata), true);
            if (!empty($resultdata[0]['cl_name'])) {
              if (trim($resultdata[0]['cl_name']) == trim($data[$key]['cl_name'])) {
                if ($data[$key]['cl_id'] == $resultdata[0]['cl_id']) {
                  goto update;
                }
                return ['status' => false, 'msg' => ' Class Name ( ' . $resultdata[0]['cl_name'] . ' )' . ' is already exist', 'data' => [$resultdata], 'rowid' => [$data[$key]['cl_id']  . ' -- ' . $resultdata[0]['cl_id']]];
              } else {
                update:
                ...
                $this->coreFunctions->sbcupdate($this->table, $data2, ['cl_id' => $data[$key]['cl_id']]);
                $this->logger->sbcmasterlog($data[$key]['cl_id'], $config, ' UPDATE - ' . $data[$key]['cl_name']);
              }
            } else {
              goto update;
            }
  ...
  public function save($config)
  {
    ...
    if ($row['cl_id'] == 0 && $row['cl_name'] != '') {
      $qry = "select cl_name from item_class where cl_name = '" . $row['cl_name'] . "' limit 1";
      $opendata = $this->coreFunctions->opentable($qry);
      $resultdata =  json_decode(json_encode($opendata), true);
      if (!empty($resultdata[0]['cl_name'])) {
        if (trim($resultdata[0]['cl_name']) == trim($row['cl_name'])) {
          return ['status' => false, 'msg' => ' Class Name ( ' . $resultdata[0]['cl_name'] . ' )' . ' is already exist', 'data' => [$resultdata]];
        }
      }
    }
    ...
    if ($companyid == 37) $data['cl_name'] = strtoupper($data['cl_name']); //megacrystal

    if ($row['cl_id'] == 0) {

      $cl_id = $this->coreFunctions->insertGetId($this->table, $data);
      if ($cl_id != 0) {
        $returnrow = $this->loaddataperrecord($cl_id);
        $this->logger->sbcmasterlog($cl_id, $config, ' CREATE - ' . $data['cl_name']);
      ...
      if ($row['cl_id'] != 0 && $row['cl_name'] != '') {
        $qry = "select cl_name,cl_id from  item_class where cl_name = '" . $row['cl_name'] . "' limit 1";
        $opendata = $this->coreFunctions->opentable($qry);
        $resultdata =  json_decode(json_encode($opendata), true);
        if (!empty($resultdata[0]['cl_name'])) {
          if (trim($resultdata[0]['cl_name']) == trim($row['cl_name'])) {
            if ($row['cl_id'] == $resultdata[0]['cl_id']) {
              goto update;
            }
            return ['status' => false, 'msg' => ' Class Name ( ' . $resultdata[0]['cl_name'] . ' )' . ' is already exist', 'data' => [$resultdata], 'rowid' => [$row['cl_id']  . ' -- ' . $resultdata[0]['cl_id']]];
          } else {
            update:
            ...
            if ($this->coreFunctions->sbcupdate($this->table, $data, ['cl_id' => $row['cl_id']]) == 1) {
              $returnrow = $this->loaddataperrecord($row['cl_id']);
              $this->logger->sbcmasterlog($row['cl_id'], $config, ' UPDATE - ' . $data['cl_name']);
              return ['status' => true, 'msg' => 'Successfully saved.', 'row' => $returnrow];
            } else {
              return ['status' => false, 'msg' => 'Saving failed.'];
            }
  ...
  public function delete($config)
  {
    ...
    $qry = "select class as value from item where class=?";
    $count = $this->coreFunctions->datareader($qry, [$row['cl_id']]);

    if ($count != '') {
      return ['clientid' => $row['cl_id'], 'status' => false, 'msg' => 'Already have transaction...'];
    }

    $qry = "delete from " . $this->table . " where cl_id=?";
    $this->coreFunctions->execqry($qry, 'delete', [$row['cl_id']]);
    $this->logger->sbcdelmaster_log($row['cl_id'], $config, 'REMOVE - ' . $row['cl_name']);
    return ['status' => true, 'msg' => 'Successfully deleted.'];
  }


  private function loaddataperrecord($cl_id)
  {
    $select = $this->selectqry();
    $select = $select . ",'' as bgcolor ";
    $qry = "select " . $select . " from " . $this->table . " where cl_id=?";
    $data = $this->coreFunctions->opentable($qry, [$cl_id]);
    return $data;
  }
  ...
  public function loaddata($config)
  {
    ...
    $qry = "select " . $select . " from " . $this->table . " where 1=1 " . $filtersearch . " order by cl_id $l";
  ...
  private function report_default_query($config)
  {
    ...
    $query = "select cl_id, cl_code, cl_name from item_class
      order by cl_id";
  ...
  public function reportdata($config)
  {
    ...
    if ($config['params']['dataparams']['print'] == "default") {
      $str = $this->rpt_class_masterfile_layout($data, $config);
  ...
  private function rpt_default_header($data, $filters)
  {
    ...
    $str .= $this->reporter->begintable('800');
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('CLASS MASTERFILE', '800', null, false, '1px solid ', '', 'L', 'Century Gothic', '18', 'B', '', '');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();
    ...
    $str .= $this->reporter->begintable('800');
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('Class Name', '400', null, false, '1px solid ', 'B', 'L', 'Century Gothic', '12', 'B', '', '2px');
    $str .= $this->reporter->endrow();
    return $str;
  }

  private function rpt_class_masterfile_layout($data, $filters)
  {
    $companyid = $filters['params']['companyid'];
    $decimal = $this->companysetup->getdecimal('currency', $filters['params']);
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $str .= $this->reporter->col($data[$i]['cl_name'], '400', null, false, '1px solid ', '', 'L', 'Century Gothic', '11', '', '', '3px');
      $str .= $this->reporter->endrow();
  ...
  private function rpt_class_PDF_header_PDF($data, $filters)
  {
    ...
    PDF::MultiCell(600, 20, "Class Name", '', 'L', false, 0);
  ...
  private function rpt_class_PDF($data, $filters)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      PDF::MultiCell(600, 10, $data[$i]['cl_name'], '', 'L', 0, 0, '', '', true, 0, true, false);

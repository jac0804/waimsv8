...
class entryhousemodel
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'HOUSE MODEL MASTERFILE';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'housemodel';
  private $othersClass;
  public $style = 'width:100%;';
  public $tablelogs = 'masterfile_log';
  public $tablelogs_del = 'del_masterfile_log';
  private $fields = ['projectid', 'model', 'price'];
  ...
  public function createTab($config)
  {
    ...
    $tab = [$this->gridname => ['gridcolumns' => ['action', 'model', 'price']]];
    ...
    $obj[0][$this->gridname]['columns'][$price]['style'] = "width:200px;whiteSpace: normal;min-width:200px;";
    $obj[0][$this->gridname]['columns'][$price]['label'] = "Price per SQM";

    $obj[0][$this->gridname]['columns'] = $this->tabClass->delcol($obj, $this->gridname);

    $this->modulename .= " - [ PROJECT : " . $this->getproject($config) . " ]";
  ...
  public function add($config)
  {
    $data = [];
    $data['line'] = 0;
    $data['projectid'] = $config['params']['sourcerow']['line'];
    $data['model'] = '';
    $data['price'] = 0.00;
  ...
  {
    return "line, phaseid, projectid, model, round(price,2) as price";
  }
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['line'] == 0) {
          ...
          $this->logger->sbcmasterlog($id, $config, ' CREATE - ' . $data[$key]['model'] . ' - ' . $data[$key]['price'] . ' - ' . $data[$key]['projectid']);
        } else {
  ...
  public function save($config)
  {
    ...
    if ($row['line'] == 0) {
      ...
      if ($line != 0) {
        ...
        $this->logger->sbcmasterlog($line, $config, ' CREATE - ' . $data['model'] . ' - ' . $data['price'] . ' - ' . $data['projectid']);
  ...
  public function delete($config)
  {
    ...
    $this->logger->sbcdelmaster_log($row['line'], $config, 'REMOVE - ' . $row['model'] . ' - ' . $row['price'] . ' - ' . $row['projectid'] . ' - ' . $row['phaseid']);
  ...
  public function loaddata($config)
  {
    ...
    $qry = "select " . $select . " 
    from " . $this->table . "
    where projectid=" . $projectid . " " . $filtersearch . "
  ...
  private function report_default_query($config)
  {
    ...
    $query = "select line, model, price from $this->table
    order by line";

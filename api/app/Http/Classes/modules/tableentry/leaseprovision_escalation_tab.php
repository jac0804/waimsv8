...
class leaseprovision_escalation_tab
{
  private $fieldClass;
  private $tabClass;
  ...
  private $fields = ['line', 'dateid', 'rate', 'clientid', 'trno'];
  ...
  public function createTab($config)
  {
    ...
    $tab = [$this->gridname => ['gridcolumns' => ['action', 'dateid', 'rate', 'remarks']]];
    ...
    $obj[0][$this->gridname]['columns'][$remarks]['type'] = "";
    $obj[0][$this->gridname]['columns'][$remarks]['label'] = "";
  ...
  public function delete($config)
  {
    ...
    $params = $config;
    $params['params']['doc'] = "ENTRYESCALATION_TAB";
    $this->logger->sbcdel_log(
      $tableid,
      $params,
      'DELETE',
      'DELETE - ESCALATION: ' . $row['line'] . ''
        . ', DATE: ' . $row['dateid']
        . ', RATE: ' . $row['rate']
    );
  ...
  private function loaddataperrecord($config, $line)
  {
    ...
    select '' as bgcolor, escal.line, left(escal.dateid, 10) as dateid, escal.remarks, 
      escal.isapplied, escal.clientid, escal.trno,
      escal.rate, escal.dateapplied, escal.oldrate
    from " . $this->table . " as escal
    where escal.trno = '" . $tableid . "' and escal.line = '" . $line . "'
    order by escal.line";

...
class entryclientindustry
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'SUPP/CUST. INDUSTRY';
  ...
  private $fields = ['category', 'reqtype', 'isindustry', 'description'];
  ...
  {
    $columns = ['action', 'category', 'reqtype', 'description'];
    ...
    $obj[0][$this->gridname]['columns'][$category]['label'] = "Industry";
    $obj[0][$this->gridname]['columns'][$reqtype]['label'] = "Sub Industry";

    $obj[0][$this->gridname]['columns'][$category]['style'] = "width:100px;whiteSpace: normal;min-width:50px;";
    $obj[0][$this->gridname]['columns'][$reqtype]['style'] = "width:100px;whiteSpace: normal;min-width:50px;";
    //3.6.2026
    $obj[0][$this->gridname]['columns'][$description]['label'] = "Category";
    return $obj;
  }
  ...
  private function loaddataperrecord($cat_id)
  {
    ...
    $qry = "select " . $select . " from " . $this->table . " where isindustry=1 and  line=?";
  ...
  public function loaddata($config)
  {
    ...
    $select = $this->selectqry();
    $select = $select . ",'' as bgcolor ";
    $qry = "select " . $select . " from " . $this->table . " where isindustry=1 " . $filtersearch . " order by line limit " . $limit;
  ...
  private function report_default_query($config)
  {
    ...
    $query = "select category, reqtype, description from reqcategory
                 where isindustry=1  order by category";
  ...
  private function rpt_class_PDF_header_PDF($data, $filters)
  {
    ...
    PDF::MultiCell(800, 20, "SUPPLIER/CUSTOMER INDUSTRY", '', 'L', false);
    PDF::SetFont($font, '', 9);
    PDF::MultiCell(800, 20, "Page " . PDF::PageNo() . "  ", '', 'L', false);

    PDF::SetFont($fontbold, '', 11);
    PDF::MultiCell(200, 20, "Industry", '', 'L', false, 0);
    PDF::MultiCell(200, 20, "Sub-Industry", '', 'L', false, 0);
    PDF::MultiCell(200, 20, "Category", '', 'L', false, 0);
  ...
  private function rpt_class_PDF($data, $filters)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      PDF::MultiCell(200, 10, $data[$i]['category'], '', 'L', 0, 0, '', '', true, 0, true, false);
      PDF::MultiCell(200, 10, $data[$i]['reqtype'], '', 'L', 0, 0, '', '', true, 0, true, false);
      PDF::MultiCell(200, 10, $data[$i]['description'], '', 'L', 0, 0, '', '', true, 0, true, false);

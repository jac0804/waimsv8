...
class entrymodel
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'MODEL MASTERFILE';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $sbcscript;
  private $table = 'model_masterfile';
  private $othersClass;
  public $style = 'width:100%;';
  public $tablelogs = 'masterfile_log';
  public $tablelogs_del = 'del_masterfile_log';
  private $fields = ['model_code', 'model_name'];
  ...
  {
    return $this->sbcscript->entrymodel($config);
  }
  ...
  public function createTab($config)
  {
    ...
    switch ($companyid) {
      case 14: //majesty
        $this->modulename = 'GENERIC MASTERFILE';
        break;
      case 22: //eipi
        $this->modulename = 'Category 4';
        break;
    }

    $tab = [
      $this->gridname => [
        'gridcolumns' => ['action', 'model_code', 'model_name']
      ]
    ];
    ...
    switch ($companyid) {
      ...
      case 28: //xcomp
        $obj[0][$this->gridname]['columns'][2]['maxlength'] = 50;
        $obj[0][$this->gridname]['columns'][1]['type'] = 'coldel';
        break;
      default:
        $obj[0][$this->gridname]['columns'][1]['type'] = 'coldel';
        break;
    ...
    $obj[0][$this->gridname]['columns'][2]['error'] = false;
  ...
  public function saveallentry($config)
  {
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['model_id'] == 0 && $data[$key]['model_name'] != '') {
          $qry = "select model_name from model_masterfile where model_name = '" . $data[$key]['model_name'] . "' limit 1";
          $opendata = $this->coreFunctions->opentable($qry);
          $resultdata =  json_decode(json_encode($opendata), true);
          if (!empty($resultdata[0]['model_name'])) {
            if (trim($resultdata[0]['model_name']) == trim($data[$key]['model_name'])) {
              return ['status' => false, 'msg' => '( ' . $resultdata[0]['model_name'] . ' )' . ' Already exist', 'data' => [$resultdata]];
            } else {
              goto insert;
            }
          }
        ...
        insert:
        if (trim($data[$key]['model_name'] == '')) {
          return ['status' => false, 'msg' => 'Description is empty'];
        }
        if ($companyid == 37) $data2['model_name'] = strtoupper($data2['model_name']); //megacrystal
        if ($data[$key]['model_id'] == 0) {
          if ($companyid == 10 || $companyid == 12) { //afti, afti usd
            $m = $this->coreFunctions->getfieldvalue($this->table, "model_name", "model_name = '" . $data[$key]['model_name'] . "'");
            if (strlen($m) != 0) {
              return ['status' => false, 'msg' => $m . ' Already exist', 'data' => []];
            } else {
              $modelid = $this->coreFunctions->insertGetId($this->table, $data2);
              $this->logger->sbcmasterlog($modelid, $config, ' CREATE - ' . $data[$key]['model_name']);
            }
          } else {
            $modelid = $this->coreFunctions->insertGetId($this->table, $data2);
            $this->logger->sbcmasterlog($modelid, $config, ' CREATE - ' . $data[$key]['model_name']);
          ...
          if ($data[$key]['model_id'] != 0 && $data[$key]['model_name'] != '') {
            $qry = "select model_name,model_id from model_masterfile where model_name = '" . $data[$key]['model_name'] . "' limit 1";
            $opendata = $this->coreFunctions->opentable($qry);
            $resultdata =  json_decode(json_encode($opendata), true);
            if (!empty($resultdata[0]['model_name'])) {
              if (trim($resultdata[0]['model_name']) == trim($data[$key]['model_name'])) {
                if ($data[$key]['model_id'] == $resultdata[0]['model_id']) {
                  goto update;
                }
                return ['status' => false, 'msg' => '( ' . $resultdata[0]['model_name'] . ' )' . ' Already exist', 'data' => [$resultdata], 'rowid' => [$data[$key]['model_id']  . ' -- ' . $resultdata[0]['model_id']]];
              } else {
                update:
                $data2['editdate'] = $this->othersClass->getCurrentTimeStamp();
                $data2['editby'] = $config['params']['user'];
                $data2['ismirror'] = 0;
                $this->coreFunctions->sbcupdate($this->table, $data2, ['model_id' => $data[$key]['model_id']]);
              }
            } else {
              goto update;
            }
  ...
  public function save($config)
  {
    ...
    if ($row['model_id'] == 0 && $row['model_name'] != '') {
      $qry = "select model_name from model_masterfile where model_name = '" . $row['model_name'] . "' limit 1";
      $opendata = $this->coreFunctions->opentable($qry);
      $resultdata =  json_decode(json_encode($opendata), true);
      if (!empty($resultdata[0]['model_name'])) {
        if (trim($resultdata[0]['model_name']) == trim($row['model_name'])) {
          return ['status' => false, 'msg' => ' Description ( ' . $resultdata[0]['model_name'] . ' )' . ' is already exist', 'data' => [$resultdata]];
        }
      }
    }
    if (trim($row['model_name'] == '')) {
      return ['status' => false, 'msg' => 'Description is empty'];
    }
    if ($companyid == 37)  $data['model_name'] = strtoupper($data['model_name']); //mega crystal

    if ($row['model_id'] == 0) {
      if ($companyid == 10 || $companyid == 12) { //afti, afti usd
        $m = $this->coreFunctions->getfieldvalue($this->table, "model_name", "model_name = '" . $row['model_name'] . "'");
        if (strlen($m) != 0) {
          return ['status' => false, 'msg' => 'Saving failed, ' . $m . ' had already been exist.'];
        } else {
          $model_id = $this->coreFunctions->insertGetId($this->table, $data);
          if ($model_id != 0) {
            $returnrow = $this->loaddataperrecord($model_id);
            $this->logger->sbcmasterlog($model_id, $config, ' CREATE - ' . $data['model_name']);
      ...
      if ($row['model_id'] != 0 && $row['model_name'] != '') {
        $qry = "select model_name,model_id from model_masterfile where model_name = '" . $row['model_name'] . "' limit 1";
        $opendata = $this->coreFunctions->opentable($qry);
        $resultdata =  json_decode(json_encode($opendata), true);
        if (!empty($resultdata[0]['model_name'])) {

          if (trim($resultdata[0]['model_name']) == trim($row['model_name'])) {
            if ($row['model_id'] == $resultdata[0]['model_id']) {
              goto update;
            }
            return ['status' => false, 'msg' => ' Description ( ' . $resultdata[0]['model_name'] . ' )' . ' is already exist', 'data' => [$resultdata], 'rowid' => [$row['model_id'] . ' -- ' . $resultdata[0]['model_id']]];
          } else {
            update:
            $data['editdate'] = $this->othersClass->getCurrentTimeStamp();
            $data['editby'] = $config['params']['user'];
            $data['ismirror'] = 0;
            if ($this->coreFunctions->sbcupdate($this->table, $data, ['model_id' => $row['model_id']]) == 1) {
              $this->logger->sbcmasterlog($row['model_id'], $config, ' UPDATE - ' . $data['model_name']);
              $returnrow = $this->loaddataperrecord($row['model_id']);
              return ['status' => true, 'msg' => 'Successfully saved.', 'row' => $returnrow];
            } else {
              return ['status' => false, 'msg' => 'Saving failed.'];
            }
  ...
  public function delete($config)
  {
    ...
    $data = $this->loaddataperrecord($row['model_id']);
    $qry = "select model as value from item where model=?";
    $count = $this->coreFunctions->datareader($qry, [$row['model_id']]);

    if ($count != '') {
      return ['clientid' => $row['model_id'], 'status' => false, 'msg' => 'Already have transaction...'];
    }
    $qry = "delete from " . $this->table . " where model_id=?";
    $this->coreFunctions->execqry($qry, 'delete', [$row['model_id']]);
    $this->logger->sbcdelmaster_log($row['model_id'], $config, 'REMOVE - ' . $row['model_name']);
  ...
  private function loaddataperrecord($model_id)
  {
    ...
    $qry = "select " . $select . " from " . $this->table . " where model_id=?";
    $data = $this->coreFunctions->opentable($qry, [$model_id]);
    return $data;
  }
  ...
  public function loaddata($config)
  {
    ...
    $select = $this->selectqry();
    $select = $select . ",'' as bgcolor ";
    $qry = "select " . $select . " from " . $this->table . " where 1=1 " . $filtersearch . " order by model_name "  . $l;
  ...
  private function rpt_model_PDF_header_PDF($data, $filters)
  {
    ...
    PDF::MultiCell(600, 20, "Model Name", '', 'L', false, 0);
  ...
  private function rpt_model_PDF($data, $filters)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $modelname = $this->reporter->fixcolumn([$data[$i]['model_name']], '150');
      $maxrow = 1;

      $countarr = count($modelname);
      $maxrow = $countarr;
      ...
      if ($data[$i]['model_name'] == '') {
        ...
        for ($r = 0; $r < $maxrow; $r++) {
          ...
          PDF::MultiCell(700, 10, isset($modelname[$r]) ? $modelname[$r] : '', '', 'L', 0, 0, '', '', true, 0, true, false);

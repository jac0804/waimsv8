...
class supplierlist
{
    ...
    private $table = 'supplierlist';
    private $othersClass;
    public $style = 'width:100%';
    public $tablelogs = 'masterfile_log';
    public $tablelogs_del = 'del_masterfile_log';
    private $fields = ['itemid', 'clientid', 'startdate', 'enddate', 'remarks'];
    ...
    public function createTab($config)
    {
        ...
        $columns = ['action', 'client', 'clientname', 'startdate', 'enddate', 'createdate', 'createby', 'status', 'remarks'];
        ...
        $obj[0][$this->gridname]['columns'][$client]['lookupclass'] = "supplierlist";
    ...
    public function saveallentry($config)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if ($data[$key]['bgcolor'] != '') {
                ...
                if (empty($data[$key]['clientid']) || empty($data[$key]['clientid'])) {
                    return ['status' => false, 'msg' => 'Saving failed.' . ' Supplier is empty'];
                }
                ...
                if ($data[$key]['line'] == 0) {
                    ...
                    $config['params']['doc'] = 'SUPPLIERLIST';
                    $this->logger->sbcmasterlog(
                        $tableid,
                        $config,
                        ' CREATE - LINE: ' . $status . ''
                            . ', Supplier Name: ' . $data[$key]['clientname']
                    ...
                    $dateqry = "select line  from supplierlist where startdate < '" . $startdates . "' and enddate >= '" . $startdates . "' and itemid = '$tableid' ";
                    ...
                    $test = "select clientid from supplierlist  where itemid = '$tableid'  and  '$datenow' >= startdate and '$datenow'<=enddate";
                    $new2 = $this->coreFunctions->opentable($test);
                    if (!empty($new2)) {
                        $this->coreFunctions->sbcupdate('item', ['supplier' => $new2[0]->clientid], ['itemid' => $tableid]);
                    }
                } else {
    ...
    public function delete($config)
    {
        ...
        $params = $config;
        $params['params']['doc'] = strtoupper("supplierlist_tab");
        $this->logger->sbcmasterlog(
            $tableid,
            $params,
            ' DELETE - LINE: ' . $row['line'] . ''
                . ', Supplier Name: ' . $row['clientname']
    ...
    public function loaddata($config)
    {
        ...
        client.clientname,client.client, date_format(sp.startdate,'%m/%d/%Y') as startdate,
        date_format(sp.enddate,'%m/%d/%Y') as enddate,sp.createdate,
        sp.createby,sp.remarks,
        case when date(sp.startdate) <= '$date' then 'APPLIED' else '' end as status,'' as bgcolor
        from " . $this->table . " as sp
        left join client on client.clientid = sp.clientid
        left join item on item.itemid = sp.itemid
        where sp.itemid = " . $tableid . " order by line";

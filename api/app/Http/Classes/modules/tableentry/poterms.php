...
class poterms
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'PO TERMS';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'poterms';
  private $othersClass;
  public $style = 'width:100%;';
  public $tablelogs = 'masterfile_log';
  public $tablelogs_del = 'del_masterfile_log';
  private $fields = ['poterms'];
  ...
  {
    $tab = [$this->gridname => ['gridcolumns' => ['poterms']]];
    $stockbuttons = [];
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['line'] == 0) {
          ...
          $this->logger->sbcmasterlog($line, $config, ' CREATE - ' . $data[$key]['poterms']);
        } else {
  ...
  public function save($config)
  {
    ...
    if ($row['line'] == 0) {
      ...
      if ($line != 0) {
        ...
        $this->logger->sbcmasterlog($line, $config, ' CREATE - ' . $data['poterms']);
  ...
  {
    $qry = "select line, poterms, '' as bgcolor
    from " . $this->table . "";
    $data = $this->coreFunctions->opentable($qry);
  ...
  private function report_default_query($config)
  {
    ...
    $query = "select line, poterms from poterms
      order by line";
  ...
  private function rpt_default_header($data, $filters)
  {
    ...
    $str .= $this->reporter->begintable('800');
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('PO TERMS MASTERFILE', '800', null, false, '1px solid ', '', 'L', 'Century Gothic', '18', 'B', '', '');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();
    ...
    $str .= $this->reporter->begintable('800');
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('PO Terms', '800', null, false, '1px solid ', 'B', 'L', 'Century Gothic', '12', 'B', '', '2px');
    $str .= $this->reporter->endrow();
    return $str;
  }
  ...
  private function rpt_terms_masterfile_layout($data, $filters)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $str .= $this->reporter->col($data[$i]['poterms'], '800', null, false, '1px solid ', '', 'L', 'Century Gothic', '11', '', '', '3px');
      $str .= $this->reporter->endrow();

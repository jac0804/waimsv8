...
class entrypart
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'PART MASTERFILE';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'part_masterfile';
  private $othersClass;
  public $style = 'width:100%;';
  public $tablelogs = 'masterfile_log';
  public $tablelogs_del = 'del_masterfile_log';
  private $fields = ['part_code', 'part_name'];
  ...
  public function createTab($config)
  {
    ...
    switch ($companyid) {
      case 14: //majesty
        $this->modulename = 'PRINCIPAL MASTERFILE';
        break;
      case 22: //EIPI
        $this->modulename = 'Category 5';
        break;
    }

    $tab = [
      $this->gridname => [
        'gridcolumns' => ['action', 'part_name']
      ]
    ];
    ...
    // action
    $obj[0][$this->gridname]['columns'][0]['style'] = "width:20px;whiteSpace: normal;min-width:20px;";
    $obj[0][$this->gridname]['columns'][1]['label'] = "Description";
    $obj[0][$this->gridname]['columns'][1]['style'] = "width:210px;whiteSpace: normal;min-width:210px;";
    return $obj;
  }
    ...
    foreach ($data as $key => $value) {
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['part_id'] == 0 && $data[$key]['part_name'] != '') {
          ...
          if (!empty($resultdata[0]['part_name'])) {
            if (trim($resultdata[0]['part_name']) == trim($data[$key]['part_name'])) {
              return ['status' => false, 'msg' => ' Description ( ' . $resultdata[0]['part_name'] . ' )' . ' is already exist', 'data' => [$resultdata]];
            }
          }
        }
        if (trim($data[$key]['part_name'] == '')) {
          return ['status' => false, 'msg' => 'Description is empty'];
        }
        if ($companyid == 37) $data2['part_name'] = strtoupper($data2['part_name']); //megacrystal

        if ($data[$key]['part_id'] == 0) {
          $partid = $this->coreFunctions->insertGetId($this->table, $data2);
          $this->logger->sbcmasterlog($partid, $config, ' CREATE - ' . $data[$key]['part_name']);
        } else {
          if ($data[$key]['part_id'] != 0 && $data[$key]['part_name'] != '') {
            $qry = "select part_name,part_id from part_masterfile where part_name = '" . $data[$key]['part_name'] . "' limit 1";
            $opendata = $this->coreFunctions->opentable($qry);
            $resultdata =  json_decode(json_encode($opendata), true);
            if (!empty($resultdata[0]['part_name'])) {
              if (trim($resultdata[0]['part_name']) == trim($data[$key]['part_name'])) {
                if ($data[$key]['part_id'] == $resultdata[0]['part_id']) {
                  goto update;
                }
                return ['status' => false, 'msg' => ' Description ( ' . $resultdata[0]['part_name'] . ' )' . ' is already exist', 'data' => [$resultdata], 'rowid' => [$data[$key]['part_id']  . ' -- ' . $resultdata[0]['part_id']]];
              } else {
                update:
                $data2['editdate'] = $this->othersClass->getCurrentTimeStamp();
                $data2['editby'] = $config['params']['user'];
                $data2['ismirror'] = 0;
                $this->coreFunctions->sbcupdate($this->table, $data2, ['part_id' => $data[$key]['part_id']]);
                $this->logger->sbcmasterlog($data[$key]['part_id'], $config, ' UPDATE - ' . $data[$key]['part_name']);
              }
            } else {
              goto update;
            }
  ...
  public function save($config)
  {
    ...
    if ($row['part_id'] == 0 && $row['part_name'] != '') {
      ...
      $resultdata =  json_decode(json_encode($opendata), true);
      if (!empty($resultdata[0]['part_name'])) {
        if (trim($resultdata[0]['part_name']) == trim($row['part_name'])) {
          return ['status' => false, 'msg' => ' Description ( ' . $resultdata[0]['part_name'] . ' )' . ' is already exist', 'data' => [$resultdata]];
        }
      }
    }
    ...
    if ($companyid == 37)  $data['part_name'] = strtoupper($data['part_name']); //megacrystal

    if ($row['part_id'] == 0) {
      $part_id = $this->coreFunctions->insertGetId($this->table, $data);
      if ($part_id != 0) {
        $returnrow = $this->loaddataperrecord($part_id);
        $this->logger->sbcmasterlog($part_id, $config, ' CREATE - ' . $data['part_name']);
      ...
      if ($row['part_id'] != 0 && $row['part_name'] != '') {
        $qry = "select part_name,part_id from part_masterfile where part_name = '" . $row['part_name'] . "' limit 1";
        $opendata = $this->coreFunctions->opentable($qry);
        $resultdata =  json_decode(json_encode($opendata), true);
        if (!empty($resultdata[0]['part_name'])) {
          if (trim($resultdata[0]['part_name']) == trim($row['part_name'])) {
            if ($row['part_id'] == $resultdata[0]['part_id']) {
              goto update;
            }
            return ['status' => false, 'msg' => ' Description ( ' . $resultdata[0]['part_name'] . ' )' . ' is already exist', 'data' => [$resultdata], 'rowid' => [$row['part_id'] . ' -- ' . $resultdata[0]['part_id']]];
          } else {
            update:
            ...
            if ($this->coreFunctions->sbcupdate($this->table, $data, ['part_id' => $row['part_id']]) == 1) {
              $returnrow = $this->loaddataperrecord($row['part_id']);
              $this->logger->sbcmasterlog($row['part_id'], $config, ' UPDATE - ' . $data['part_name']);
              return ['status' => true, 'msg' => 'Successfully saved.', 'row' => $returnrow];
            } else {
              return ['status' => false, 'msg' => 'Saving failed.'];
            }
  ...
  public function delete($config)
  {
    ...
    $qry = "select part as value from item where part=?";
    $count = $this->coreFunctions->datareader($qry, [$row['part_id']]);

    if ($count != '') {
      return ['clientid' => $row['part_id'], 'status' => false, 'msg' => 'Already have transaction...'];
    }

    $qry = "delete from " . $this->table . " where part_id=?";
    $this->coreFunctions->execqry($qry, 'delete', [$row['part_id']]);
    $this->logger->sbcdelmaster_log($row['part_id'], $config, 'REMOVE - ' . $row['part_name']);
    return ['status' => true, 'msg' => 'Successfully deleted.'];
  }


  private function loaddataperrecord($part_id)
  {
    $select = $this->selectqry();
    $select = $select . ",'' as bgcolor ";
    $qry = "select " . $select . " from " . $this->table . " where part_id=?";
    $data = $this->coreFunctions->opentable($qry, [$part_id]);
    return $data;
  }
  ...
  public function loaddata($config)
  {
    ...
    $qry = "select " . $select . " from " . $this->table . " where 1=1 " . $filtersearch . " order by part_id $l";

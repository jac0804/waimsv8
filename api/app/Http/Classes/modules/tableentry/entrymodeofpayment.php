...
class entrymodeofpayment
{
    private $fieldClass;
    private $tabClass;
    public $modulename = 'Mode of Payment Masterfile';
    ...
    private $fields = ['category',  'inactive','ispaymode'];
    ...
    public function createTab($config)
    {
        ...
        $obj[0][$this->gridname]['columns'][$category]['label'] = 'Mode of Payment';
    ...
    private function selectqry()
    {
        ...
        $qry = " r.line,r.category,r.reqtype, case when r.inactive=0 then 'false' else 'true' end as inactive,r.ispaymode";
        return $qry;
    }
        ...
        foreach ($data as $key => $value) {
            ...
            if ($data[$key]['bgcolor'] != '') {
                ...
                if ($data[$key]['line'] == 0 && $data[$key]['category'] != '') {
                    $qry = "select category from reqcategory where category = '" . $data[$key]['category'] . "' and ispaymode =1 limit 1";
                ...
                if (trim($data[$key]['category'] == '')) {
                    return ['status' => false, 'msg' => 'Mode is empty'];
                }
                ...
                if ($data[$key]['line'] == 0) {
                    ...
                    if ($data[$key]['category'] != '') {
                        $qry = "select category,line from reqcategory where category = '" . $data[$key]['category'] . "' and line <>".$data[$key]['line']."  and ispaymode =1 limit 1";
                        $opendata = $this->coreFunctions->opentable($qry);
                        $resultdata =  json_decode(json_encode($opendata), true);
                        if (!empty($resultdata)) {
                            return ['status' => false, 'msg' => $resultdata[0]['category'] .  ' is already exist', 'data' => [$resultdata], 'rowid' => [$data[$key]['line']  . ' -- ' . $resultdata[0]['line']]];
                        } else {
                            update:
    ...
    public function save($config)
    {
        ...
        if ($row['line'] == 0 && $row['category'] != '') {
            $qry = "select category from reqcategory where category = '" . $row['category'] . "'  and ispaymode =1 limit 1";
            $opendata = $this->coreFunctions->opentable($qry);
            $resultdata =  json_decode(json_encode($opendata), true);
            if (!empty($resultdata[0]['category'])) {
        ...
        if ($row['line'] == 0) {
            ...
            if ($row['category'] != '') {
                $qry = "select line,category,reqtype,case when inactive=0 then 'false' else 'true' end as inactive from  reqcategory where category = '" . $row['category'] . "' and line = ".$row['line']."  and ispaymode =1 limit 1";
                $opendata = $this->coreFunctions->opentable($qry);
                $resultdata =  json_decode(json_encode($opendata), true);
                if (!empty($resultdata)) {
    ...
    private function loaddataperrecord($line)
    {
        ...
        $qry = "select " . $select . " from " . $this->table . " as r  where  r.ispaymode = 1 and line=?";
    ...
    public function loaddata($config)
    {
        ...
        $qry = "select " . $select . " from " . $this->table . " as r  where  r.ispaymode = 1 and  1=1 " . $filtersearch . " order by line $l";
    ...
    public function lookuplogs($config)
    {
        ...
        $lookupsetup = array(
            'type' => 'show',
            'title' => 'MODE OF PAYMENT MASTER LOGS',
    ...
    private function report_default_query($config)
    {
        ...
        $query = "select line, category, reqtype,inactive from reqcategory
         order by line";
    ...
    public function reportdata($config)
    {
        ...
        if ($config['params']['dataparams']['print'] == "default") {
            $str = $this->rpt_modeofpayment_masterfile_layout($data, $config);
        } else if ($config['params']['dataparams']['print'] == "PDFM") {
            $str = $this->rpt_modeofpayment_PDF($data, $config);
        }
    ...
    private function rpt_default_header($data, $filters)
    {
        ...
        $str .= $this->reporter->begintable('800');
        $str .= $this->reporter->startrow();
        $str .= $this->reporter->col('MODE OF PAYMENT MASTERFILE', '800', null, false, '1px solid ', '', 'L', 'Century Gothic', '18', 'B', '', '');
        $str .= $this->reporter->endrow();
        $str .= $this->reporter->endtable();
    ...
    private function rpt_modeofpayment_masterfile_layout($data, $filters)
    {
        $companyid = $filters['params']['companyid'];
        $decimal = $this->companysetup->getdecimal('currency', $filters['params']);
    ...
    private function rpt_modeofpayment_PDF($data, $filters)
    {
        $companyid = $filters['params']['companyid'];
        $decimal = $this->companysetup->getdecimal('currency', $filters['params']);

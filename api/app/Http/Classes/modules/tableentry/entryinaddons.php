...
class entryinaddons
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'INTERIOR ADDONS';
  ...
  {
    $qry = "select line, trno, addons, qty, '' as bgcolor from qtaddons where trno=? and line=? and addontype=1 union all select line, trno, addons, qty, '' as bgcolor from hqtaddons where trno=? and line=? and addontype=1";
  ...
  {
    $tableid = $config['params']['tableid'];
    $qry = "select line, trno, addons, qty, '' as bgcolor from qtaddons where trno=? and addontype=1 union all select line, trno, addons, qty, '' as bgcolor from hqtaddons where trno=? and addontype=1 order by line";
  ...
  public function lookupsetup($config)
  {
    ...
    switch ($lookupclass2) {
      case 'addons':
        ...
        $qry = "select distinct addons from qtaddons where addontype=1 order by addons";
      ...
      case 'addonqty':
        ...
        $data = $this->coreFunctions->opentable("select distinct qty from qtaddons where addontype=1 order by qty");
        return ['status' => true, 'msg' => 'ok', 'data' => $data, 'lookupsetup' => $lookupsetup, 'cols' => $cols, 'plotsetup' => $plotsetup, 'index' => $rowindex];
        break;
      default:

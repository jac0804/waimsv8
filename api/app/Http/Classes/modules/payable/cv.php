...
class cv
{
  ...
  public function paramsdatalisting($config)
  {
    ...
    if ($companyid == 39) {
      ...
      data_set($col1, 'pickpo.label', 'pick pl');
      data_set($col1, 'pickpo.lookupclass', 'plshortcut');
      data_set($col1, 'pickpo.action', 'pendingcvplsummary');
      data_set($col1, 'pickpo.confirmlabel', 'Proceed to pick PL?');
    }
  ...
  public function loaddoclisting($config)
  {
    ...
    switch ($companyid) {
      ...
      where d.trno= head.trno and coa.alias like 'CB%' ) as cr ";
      ...
      where d.trno= head.trno and coa.alias like 'CB%' ) as cr ";
        ...
        $laleftjoin = 'left join ladetail as detail on detail.trno=head.trno';
        $glleftjoin = 'left join gldetail as detail on detail.trno=head.trno';
        $grpby = 'group by head.trno,head.docno,head.clientname,head.dateid,
      ...
      where d.trno= head.trno) as cr ";
        ...
        break;
      case 56: //homeworks
        $laleftjoin = " left join client as cl on cl.client= head.client";
        $glleftjoin = " left join client as cl on cl.clientid= head.clientid";
        $field = ", cl.client";
        $dateid = "left(head.dateid,10) as dateid";
        $orderby =  "order by  dateid desc, docno desc";
        break;
  ...
  public function createtab2($access, $config)
  {
    ...
    if ($config['params']['companyid'] == 60) { //transpower      
      $changecode = $this->othersClass->checkAccess($config['params']['user'], 5500);
      if ($changecode) {
  ...
  public function createTab($access, $config)
  {
    ...
    if ($companyid == 39) { //cbbsi
      if ($release) {
        $tab['customform'] = ['event' => ['action' => 'customform', 'lookupclass' => 'cvvrelease', 'access' => 'release'], 'label' => 'RELEASE'];
      }
    }
    ...
    switch ($config['params']['companyid']) {
      case 10:
        $obj[0]['accounting']['columns'][$project]['label'] = 'Item Group';
        $obj[0]['accounting']['columns'][$invoiceno]['type'] = 'label';
        $obj[0]['accounting']['columns'][$invoicedate]['type'] = 'label';
        $obj[0]['accounting']['columns'][$invoiceno]['label'] = 'Supplier Invoice#';
        $obj[0]['accounting']['columns'][$invoicedate]['label'] = 'Supplier Invoice Date';
        $obj[0]['accounting']['columns'][$clientname]['label'] = 'Payee';
        $obj[0]['accounting']['columns'][$clientname]['type'] = 'lookup';
        $obj[0]['accounting']['columns'][$clientname]['lookupclass'] = 'vendordetail';
        $obj[0]['accounting']['columns'][$clientname]['action'] = 'lookupclient';
        $obj[0]['accounting']['columns'][$rem]['type'] = 'coldel';
        $obj[0]['accounting']['columns'][$client]['type'] = 'coldel';
        break;
      case 19:
  ...
  public function createtabbutton($config)
  {
    ...
    if ($companyid == 26) {
      $obj[1 + $index]['label'] = "SAVE ACCOUNT";
      $obj[2 + $index]['label'] = "DELETE ACCOUNT";
    } else {
      $obj[1 + $index]['label'] = "ADD ACCOUNT";
      $obj[1 + $index]['action'] = "adddetail";
      $obj[2 + $index]['label'] = "SAVE ACCOUNT";
      $obj[3 + $index]['label'] = "DELETE ACCOUNT";
    }
    if ($companyid == 10) {
      $obj[4 + $index]['label'] = "CA";
      $obj[5 + $index]['label'] = "WTAX";
      $obj[5 + $index]['action'] = "pendingwtax";
    }
    if ($companyid == 43) {
      $obj[5 + $index]['addedparams'] = ['contra'];
    }
    return $obj;
  }
  ...
  public function createHeadField($config)
  {
    ...
    switch ($companyid) {
      case 8:
        $fields = ['docno', 'client', 'clientname', ['hacno', 'acnoname']];
        $col1 = $this->fieldClass->create($fields);
        data_set($col1, 'acnoname.type', 'cinput');
        data_set($col1, 'acnoname.maxlength', '50');
        data_set($col1, 'acnoname.name', 'hacnoname');
        break;
    ...
    switch ($companyid) {
      case 3:
        $fields = ['dateid', 'dacnoname', 'dprojectname'];
    ...
    switch ($companyid) {
      case 3:
        ...
        break;
      case 8:
        $fields = [['yourref', 'ourref'], ['cur', 'forex'], ['brdocno', 'paymode']];
        break;
      case 10:
      case 12:
        $fields = [['yourref', 'ourref'], ['cur', 'forex'], 'radioencash'];
        break;
      case 26:
    ...
    if ($companyid == 39) {
      data_set($col3, 'brdocno.name', 'pldocno');
      data_set($col3, 'brdocno.type', 'input');
      data_set($col3, 'brdocno.label', 'PL#');
      data_set($col3, 'brdocno.class', 'sbccsreadonly');
    }
    if ($companyid == 8) {
    ...
    if ($companyid == 26) {
      data_set($col3, 'dcostcode.type', 'qselectedit');
      $qry = "select line as costcodeid, code as costcode, name as costcodename, concat(code,'~',name) as label, line as value from costcode_masterfile order by line";
      $costcodes = $this->coreFunctions->opentable($qry);
      if (!empty($costcodes)) $costcodes = json_decode(json_encode($costcodes), true);
      data_set($col3, 'dcostcode.options', $costcodes);
      data_set($col3, 'dcostcode.class', 'cscostcode');
      data_set($col3, 'dcostcode.plottype', 'plothead');
      data_set($col3, 'dcostcode.plotting', ['costcode' => 'costcode', 'costcodename' => 'costcodename', 'costcodeid' => 'costcodeid']);
    ...
    if ($companyid == 10 || $companyid == 12) {
      data_set($col3, 'yourref.label', 'BRN');

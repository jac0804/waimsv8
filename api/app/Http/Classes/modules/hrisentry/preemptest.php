...
class preemptest
{
  ...
  private $table = 'preemp';
  public $tablelogs = 'masterfile_log';
  public $tablelogs_del = 'del_masterfile_log';
  private $othersClass;
  public $style = 'width:100%;';
  private $fields = ['code', 'test'];
  ...
  public function createTab($config)
  {
    ...
    $obj[0][$this->gridname]['columns'][$test]['style'] = "width:700px;whiteSpace: normal;min-width:700px;";
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['line'] == 0) {
          ...
          $this->logger->sbcmasterlog(
            $line,
            $config,
            'CREATE' . ' - ' . $data[$key]['code'] . ' - ' . $data[$key]['test']
          );
        } else {
  ...
  public function delete($config)
  {
    ...
    $emptestid = $config['params']['row']['code'];
    $qry1 = "select emptestid as value from apreemploy where pin=?";
    $count = $this->coreFunctions->datareader($qry1, [$emptestid], '', true);

    if (($count != 0)) {
      return ['clientid' => $emptestid, 'status' => false, 'msg' => 'Already have transaction...'];
    }

    $qry = "delete from " . $this->table . " where line=?";
    $this->coreFunctions->execqry($qry, 'delete', [$row['line']]);
    $this->logger->sbcdelmaster_log($row['line'], $config, 'REMOVE - ' . $row['code'] . ' - ' . $row['test']);
  ...
  private function report_default_query($config)
  {
    ...
    $query = "
      select line, code, test from preemp
    ";
  ...
  public function reportdata($config)
  {
    ...
    if ($config['params']['dataparams']['print'] == "default") {
      ...
      $str = $this->default_preemptest_PDF($config, $data);
    }
  ...
  private function rpt_default_header($filters, $data)
  {
    ...
    $str .= $this->reporter->begintable('800');
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('PRE-EMPLOYMENT TEST LIST', '600', null, false, '1px solid ', '', 'L', 'Century Gothic', '18', 'B', '', '');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();
    ...
    $str .= $this->reporter->col('PRE-EMPLOYMENT TEST', '100', null, false, '1px solid ', 'B', 'L', 'Century Gothic', '12', 'B', '30px', '8px');
    return $str;
  }
  ...
  private function rpt_DEFAULT_STATUS_MASTER_LAYOUT($filters, $data)
  {
    ...
    foreach ($data as $key => $value) {
      ...
      $str .= $this->reporter->col($value['test'], '50px', null, false, '1px solid ', '', 'L', 'Century Gothic', '11', '', '', '2px');
  ...
  private function default_preemptest_header_PDF($params, $data)
  {
    ...
    PDF::MultiCell(360, 0, "PRE-EMPLOYMENT TEST", 'B', 'L', false, 1);
    PDF::MultiCell(0, 0, "\n");
  }

  private function default_preemptest_PDF($params, $data)
  {
    ...
    $this->default_preemptest_header_PDF($params, $data);

    for ($i = 0; $i < count($data); $i++) {
      $section_height = PDF::GetStringHeight(360, $data[$i]['code']);
      $section_desc_height = PDF::GetStringHeight(360, $data[$i]['test']);
      ...
      PDF::MultiCell(360, $max_height, $data[$i]['test'], '', 'L', 0, 1, '', '');
      if (intVal($i) + 1 == $page) {
        $this->default_preemptest_header_PDF($params, $data);
        $page += $count;
      }

...
class viewarea
{
    ...
    public function createTab($config)
    {
        ...
        $cols = ['action', 'area', 'oqty', 'itemname', 'counts', 'clientquota'];
        ...
        $obj[0][$this->gridname]['columns'][$area]['style'] = "width:350px;whiteSpace: normal;min-width:350px;";
    ...
    public function loaddata($config)
    {
        ...
        $query  = "select count(emp.empid) as counts, emp.divid,emp.sectid,bran.area,'$sectname' as sectname,'$company' as company,0 as oqty, 0 as clientquota from employee as emp
                  left join client as bran on bran.clientid = emp.branchid and bran.area <> ''
                  where emp.isactive = 1 and emp.divid = $divid and emp.sectid = $sectid
                  group by emp.divid,emp.sectid,bran.area";

        $data = $this->coreFunctions->opentable($query);

        foreach ($data as $key => $value) {
            $data_branch = $this->checkbranchjob($value->area, $divid, $sectid);
            if ($data_branch != 0) {
    ...
    public function checkbranchjob($area, $divid, $sectid)
    {
        ...
        $job = $this->coreFunctions->opentable("select emp.jobid,emp.branchid from employee as emp 
            ...
            where emp.divid = $divid and emp.sectid = $sectid and bran.area = '$area' and (emp.jobid <> 0 and emp.branchid <> 0) and emp.isactive = 1
        ...
        foreach ($job as $key => $value) {
            $oqty = $this->coreFunctions->datareader("select sum(jobs.qty) as value from cljobs as jobs 
                ...
                where jobs.jobid = $value->jobid and jobs.clientid = $value->branchid and bran.area = '$area'", [], '', true);
            if ($oqty != 0) {
                $qty += $oqty;
            }
        }

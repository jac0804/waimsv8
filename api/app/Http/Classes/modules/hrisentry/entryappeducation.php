...
class entryappeducation
{
  ...
  public function createTab($config)
  {
    $tab = [
      $this->gridname => [
        'gridcolumns' => ['action', 'school', 'address', 'course', 'sy', 'gpa', 'honor']
      ]
    ];
  ...
  public function save($config)
  {
    ...
    if ($row['line'] == 0) {
      ...
      if ($line != 0) {
        ...
        $config['params']['doc'] = strtoupper('app_education');
        $this->logger->sbcmasterlog(
          $row['empid'],
          $config,
          'CREATE EDUCATION - LINE: ' . $line
            . ' - SCHOOL: ' . $data['school']
  ...
  public function delete($config)
  {
    ...
    $this->logger->sbcmasterlog(
      $row['empid'],
      $config,
      'DELETE EDUCATION - LINE: ' . $row['line']
        . ' - SCHOOL: ' . $row['school']
        . ' , ADDRESS: ' . $row['address']
        . ' , COURSE: ' . $row['course']
        . ' , SY: ' . $row['sy']

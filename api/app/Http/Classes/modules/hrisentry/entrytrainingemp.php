...
class entrytrainingemp
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'EMPLOYEES';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'trainingdetail';
  private $htable = 'htrainingdetail';
  private $othersClass;
  public $style = 'width:100%;';
  private $fields = ['empid', 'empname', 'notes'];
  ...
  {
    $tab = [$this->gridname => ['gridcolumns' => ['action', 'empcode', 'empname', 'notes']]];
  ...
  private function loaddataperrecord($trno, $line)
  {
    ...
    $qry = $select . " 
        from " . $this->table . " as i left join client as c on c.clientid=i.empid  
  ...
  {
    return "select '' as bgcolor, i.trno, i.line, i.empid,c.clientid, c.client as empcode, c.clientname as empname, i.notes";
  }
  ...
  public function loaddata($config)
  {
    ...
    $select = $this->selectqry();
    $qry = $select . " 
        from " . $this->table . " as i left join client as c on c.clientid=i.empid  where i.trno=?
        union all " . $select . " 
        from " . $this->htable . " as i left join client as c on c.clientid=i.empid  where i.trno=?";

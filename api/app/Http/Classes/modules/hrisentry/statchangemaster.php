...
//ginamit eto sa Employment Status Change module
class statchangemaster
{
  private $fieldClass;
  private $tabClass;
  public $modulename = 'STATUS CHANGE MASTER';
  public $gridname = 'inventory';
  private $companysetup;
  private $coreFunctions;
  private $table = 'statchange';
  public $tablelogs = 'masterfile_log';
  public $tablelogs_del = 'del_masterfile_log';
  private $othersClass;
  public $style = 'width:100%;';
  private $fields = ['code', 'stat'];
  ...
  public function createTab($config)
  {
    ...
    $obj[0][$this->gridname]['columns'][$stat]['style'] = "width:700px;whiteSpace: normal;min-width:700px;";
      ...
      if ($data[$key]['bgcolor'] != '') {
        ...
        if ($data[$key]['line'] == 0) {
          ...
          $this->logger->sbcmasterlog(
            $line,
            $config,
            'CREATE' . ' - ' . $data[$key]['code'] . ' - ' . $data[$key]['stat']
          );
        } else {
  ...
  public function delete($config)
  {
    ...
    $stat = $config['params']['row']['code'];
    $qry1 = "select empid as value from eschange where scode=? limit 1";
    $count = $this->coreFunctions->datareader($qry1, [$stat], '', true);

    $qry1 = "select empid as value from heschange where scode=? limit 1";
    $count1 = $this->coreFunctions->datareader($qry1, [$stat], '', true);

    if (($count != 0 || $count1 != 0)) {
      return ['clientid' => $stat, 'status' => false, 'msg' => 'Already have transaction...'];
    }

    $qry = "delete from " . $this->table . " where line=?";
    $this->coreFunctions->execqry($qry, 'delete', [$row['line']]);
    $this->logger->sbcdelmaster_log($row['line'], $config, 'REMOVE - ' . $row['code'] . ' - ' . $row['stat']);
  ...
  private function report_default_query($config)
  {
    ...
    $query = "
      select line, code, stat from statchange
    ";
  ...
  public function reportdata($params)
  {
    ...
    if ($params['params']['dataparams']['print'] == "default") {
      $str = $this->rpt_DEFAULT_STATUS_CHANGE_MASTER_LAYOUT($params, $data);
    } else if ($params['params']['dataparams']['print'] == "PDFM") {
      $str = $this->default_statchangemaster_PDF($params, $data);
    }
  ...
  private function rpt_default_header($filters, $data)
  {
    ...
    $str .= $this->reporter->begintable('800');
    $str .= $this->reporter->startrow();
    $str .= $this->reporter->col('STATUS CHANGE MASTER LIST', '600', null, false, '1px solid ', '', 'L', 'Century Gothic', '18', 'B', '', '');
    $str .= $this->reporter->endrow();
    $str .= $this->reporter->endtable();
    ...
    $str .= $this->reporter->col('STATUS CHANGE', '100', null, false, '1px solid ', 'B', 'C', 'Century Gothic', '12', 'B', '30px', '8px');
    return $str;
  }

  private function rpt_DEFAULT_STATUS_CHANGE_MASTER_LAYOUT($filters, $data)
  {
    $companyid = $filters['params']['companyid'];
    $decimal = $this->companysetup->getdecimal('currency',  $filters['params']);
    ...
    foreach ($data as $key => $value) {
      ...
      $str .= $this->reporter->col($value['stat'], '50px', null, false, '1px solid ', '', 'C', 'Century Gothic', '11', '', '', '2px');
  ...
  private function default_statchangemaster_header_PDF($params, $data)
  {
    ...
    PDF::MultiCell(360, 0, "STATUS CHANGE", 'B', 'L', false, 1);
    PDF::MultiCell(0, 0, "\n");
  }

  private function default_statchangemaster_PDF($params, $data)
  {
    ...
    $this->default_statchangemaster_header_PDF($params, $data);

    for ($i = 0; $i < count($data); $i++) {
      $section_height = PDF::GetStringHeight(360, $data[$i]['code']);
      $section_desc_height = PDF::GetStringHeight(360, $data[$i]['stat']);
      ...
      PDF::MultiCell(360, $max_height, $data[$i]['stat'], '', 'L', 0, 1, '', '');
      if (intVal($i) + 1 == $page) {
        $this->default_statchangemaster_header_PDF($params, $data);
        $page += $count;
      }

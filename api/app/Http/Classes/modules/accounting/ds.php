...
class ds
{
  ...
  private $fields = ['trno', 'docno', 'dateid', 'contra', 'yourref', 'ourref', 'rem', 'projectid', 'phaseid', 'modelid', 'blklotid', 'amenityid', 'subamenityid'];
  ...
  public function loaddoclisting($config)
  {
    ...
    switch ($companyid) {
      ...
      case 29://sbc
        ...
        if ($searchfilter == "") $limit = 'limit 150';
        $orderby =  "order by  dateid desc, docno desc";
        $ladb = ", (select coa.acnoname from ladetail as d 
                    left join coa on coa.acnoid = d.acnoid 
                    where d.trno= head.trno and left(coa.alias,2)='CB') as acnoname ";
        $gldb = ", (select coa.acnoname from gldetail as d 
                    left join coa on coa.acnoid = d.acnoid 
                    where d.trno= head.trno and left(coa.alias,2)='CB') as acnoname";
        break;
      default:
    ...
    $qry = "select head.trno,head.docno,head.clientname,$dateid $ladb, 'DRAFT' as status,
  ...
  public function createHeadbutton($config)
  {
    ...
    if ($this->companysetup->getisshowmanual($config['params'])) {
      $buttons['others']['items']['manual'] = ['label' => 'View Manual', 'todo' => ['lookupclass' => 'ds', 'title' => 'DS_MANUAL', 'action' => 'viewpdf',  'access' => 'view', 'type' => 'viewmanual']];
    }
    return $buttons;
  } // createHeadbutton
  ...
  public function createtab2($access, $config)
  {
    ...
    if ($config['params']['companyid'] == 60) { //transpower      
      $changecode = $this->othersClass->checkAccess($config['params']['user'], 5505);
      if ($changecode) {
  ...
  {
    $tbuttons = ['projchecks', 'checks', 'additem', 'saveitem', 'deleteallitem'];

    if ($config['params']['companyid'] != 48) { //not seastar
      $tbuttons = ['checks', 'additem', 'saveitem', 'deleteallitem'];
    }
  ...
  {
    $qry = " 
      head.trno,left(head.dateid,10) as dateid,d.ref,d.line,d.sortline,coa.acno,coa.acnoname,
  ...
  public function stockstatus($config)
  {
    switch ($config['params']['action']) {
      ...
      case 'getcheckselected':
        ...
        break;
      case 'getprojcheckselected':
        return $this->getprojcheckselected($config);
        break;
      default:
  ...
  public function additem($action, $config)
  {
    ...
    if ($action == 'insert') {
      ...
      if ($this->coreFunctions->sbcinsert($this->detail, $data) == 1) {
        ...
        $this->logger->sbcwritelog($trno, $config, 'ACCTG', 'ADD - Line:' . $line . ' Code:' . $acno . ' db:' . $db . ' cr:' . $cr . ' client:' . $client . ' date:' . $postdate . ' Refx:' . $refx . ' Linex:' . $linex);
        if ($refx != 0) {
      ...
      if ($this->coreFunctions->sbcupdate($this->detail, $data, ['trno' => $trno, 'line' => $line]) == 1) {
        if ($refx != 0) {
          if (!$this->sqlquery->setupdatebal($refx, $linex, $acno, $config)) {
            $this->coreFunctions->sbcupdate($this->detail, ['db' => 0, 'cr' => 0, 'fdb' => 0, 'fcr' => 0], ['trno' => $trno, 'line' => $line]);
            $this->sqlquery->setupdatebal($refx, $linex, $acno, $config, 1);
            $return = false;
          }
        }
      } else {
  ...
  public function deleteitem($config)
  {
    ...
    $this->logger->sbcwritelog($trno, $config, 'ACCTG', 'REMOVED - Line:' . $line . ' code:' . $data[0]['acno'] . ' db:' . $data[0]['db'] . ' cr:' . $data[0]['cr'] . ' client:' . $data[0]['client'] . ' date:' . $data[0]['postdate'] . ' ref:' . $data[0]['ref'] . ' Refx:' . $data[0]['refx'] . ' Linex:' . $data[0]['linex']);
  ...
  public function getcheckselected($config)
  {
    ...
    foreach ($data as $key => $value) {
      ...
      $config['params']['data']['yourref'] = $data[$key]['yourref'];
    ...
    if ($config['params']['companyid'] != 11) { //not summit
      $auto = $this->autoentry($config);
      if ($auto['status']) {
        array_push($rows, $auto['row']);
      }
    }

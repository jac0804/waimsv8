...
class cbaexit {
  ...
  public function getLayout() {
    ...
    array_push($cLookupHeadButtons, ['form'=>'docLookupHeadButtons', 'btns'=>$docLookupButtons]);
    ...
    array_push($cLookupHeadFields, ['form'=>'docLookupFields', 'fields'=>$docLookupFields]);

    $fields = ['itemname', 'designation', 'weight'];
    $exitLookupTableCols = $this->fieldClass->create($fields);
    data_set($exitLookupTableCols, 'itemname.type', 'label');
    data_set($exitLookupTableCols, 'itemname.field', 'itemname');
    data_set($exitLookupTableCols, 'designation.type', 'label');
    data_set($exitLookupTableCols, 'designation.field', 'designation');
    data_set($exitLookupTableCols, 'weight.type', 'label');
    data_set($exitLookupTableCols, 'weight.field', 'weight');
    array_push($cLookupTableCols, ['form'=>'exitLookupTableCols', 'fields'=>$exitLookupTableCols]);

    $cLookupHeadFields = [];
    $fields = ['barcode', 'itemname', 'groupid', 'class', 'thickness', 'width', 'shiftt', 'seltype', 'designation', 'mass', 'weight', 'color', 'len', 'prd', 'qty', 'scsono', 'custname', 'dpr', 'rem', 'addexit'];
    $stockLookupFields = $this->fieldClass->create($fields);
    data_set($stockLookupFields, 'barcode.type', 'lookup');
    data_set($stockLookupFields, 'barcode.action', 'itemlookup');
    data_set($stockLookupFields, 'barcode.fields', 'barcode');
    data_set($stockLookupFields, 'barcode.readonly', 'true');
    data_set($stockLookupFields, 'groupid.type', 'input');
    data_set($stockLookupFields, 'groupid.readonly', false);
    data_set($stockLookupFields, 'designation.type', 'input');
    data_set($stockLookupFields, 'designation.readonly', false);
    data_set($stockLookupFields, 'qty.type', 'input');
    data_set($stockLookupFields, 'qty.readonly', false);
    data_set($stockLookupFields, 'rem.type', 'input');
    data_set($stockLookupFields, 'rem.readonly', false);
    array_push($cLookupHeadFields, ['form'=>'exitStockLookupFields', 'fields'=>$stockLookupFields]);

    $cLookupButtons = [];
    $btns = ['saverecord', 'cancelrecord'];
    $stockLookupButtons = $this->buttonClass->create($btns);
    data_set($stockLookupButtons, 'saverecord.label', 'Submit');
    data_set($stockLookupButtons, 'saverecord.func', 'submitExitProd');
    data_set($stockLookupButtons, 'saverecord.functype', 'global');
    data_set($stockLookupButtons, 'cancelrecord.func', 'cancelExitProd');
    data_set($stockLookupButtons, 'cancelrecord.functype', 'global');
    array_push($cLookupButtons, ['form'=>'exitStockLookupButtons', 'btns'=>$stockLookupButtons]);

    return ['cfHeadFields'=>$cfHeadFields, 'cfTableCols'=>$cfTableCols, 'cfTableButtons'=>$cfTableButtons, 'cfTableHeadButtons'=>$cfTableHeadButtons, 'cLookupHeadFields'=>$cLookupHeadFields, 'cLookupHeadButtons'=>$cLookupHeadButtons, 'cLookupTableCols'=>$cLookupTableCols, 'cLookupTableButtons'=>$cLookupTableButtons, 'cLookupButtons'=>$cLookupButtons];
  }
  ...
  public function getFunc() {
    return '({
      ...
      loadTableData: function () {
        ...
        if (sbc.modulefunc.docForm.scanstat === "1") scanned = " and isexit=1";
        sbc.db.transaction(function (tx) {
          tx.executeSql("select trno, line, docno, bref, barcode, bundleno, itemname, isexit, thickness, designation, color from myexit where bref=?" + filter + scanned, data, function (tx, res) {
